...
TL;DR: NeRV360提出了一种针对360度视频的神经隐式表示压缩框架，通过仅解码用户选择的视口而非整个全景帧，大幅降低了内存消耗并提升了解码速度。


<details>
  <summary>Details</summary>
Motivation: 现有的神经隐式视频表示（NeRV）在应用于高分辨率360度视频时存在内存占用高、解码速度慢的问题，难以实现实时应用。
...
Conclusion: NeRV360通过视口选择性解码的方法，有效解决了360度视频神经隐式表示中的内存和速度瓶颈，为实时应用提供了可行方案。
...
TL;DR: VL4Gaze是首个大规模基准测试，用于评估和训练视觉语言模型在注视理解方面的能力，包含48.9万个自动生成的问答对，涵盖四个互补任务。


<details>
  <summary>Details</summary>
Motivation: 人类注视为理解视觉场景中的注意力、意图和社交互动提供了重要线索，但当前视觉语言模型在注视理解方面尚未得到充分探索。虽然最近的VLMs在各种视觉任务上表现出强大的场景级推理能力，但缺乏系统评估或训练注视理解的基准测试，因此不清楚注视理解是否能从通用视觉语言预训练中自然涌现。

Method: 提出VL4Gaze基准测试，包含489K个自动生成的问答对，覆盖124K张图像，将注视理解统一为视觉问答问题，通过四个互补任务：注视对象描述、注视方向描述、注视点定位和模糊问题识别。

Result: 评估结果显示，即使是大规模VLMs也难以在没有任务特定监督的情况下可靠地推断注视语义和空间定位。相反，在VL4Gaze上进行训练能带来所有任务的显著且一致的改进，突显了针对性的多任务监督对于开发VLMs注视理解能力的重要性。

Conclusion: VL4Gaze填补了注视理解基准测试的空白，证明了任务特定监督对于VLMs注视理解能力的重要性，将发布数据集和代码以支持该方向的进一步研究。
...
TL;DR: OccuFly是首个基于相机的真实世界空中语义场景补全基准，针对无人机场景设计，包含不同高度和季节的数据，提供22个语义类别，并提出基于相机模态的LiDAR-free数据生成框架。


<details>
  <summary>Details</summary>
Motivation: 当前语义场景补全研究主要集中在地面场景如自动驾驶，而空中场景如自主飞行基本未被探索，限制了相关应用发展。同时，LiDAR作为主要SSC数据采集方式对大多数无人机存在法规、重量、能耗等限制，且从高空视角获取的点云数据稀疏。

Method: 提出OccuFly基准数据集，包含50m、40m、30m高度在春、夏、秋、冬四季采集的数据，涵盖城市、工业和农村场景。提出基于相机模态的LiDAR-free数据生成框架，利用传统3D重建技术，通过将标注的2D掩码提升到重建点云中实现自动标签传递，大幅减少手动3D标注工作量。

Result: 建立了首个真实世界相机空中SSC基准，提供22个语义类别，数据格式遵循现有研究惯例以便集成。在OccuFly上对最先进方法进行了基准测试，突出了高空视角特有的挑战。

Conclusion: OccuFly为整体空中3D场景理解提供了全面的视觉基准，解决了空中语义场景补全研究空白，通过相机模态的数据生成框架降低了数据采集门槛，推动了无人机场景的3D感知研究。
...
<details>
  <summary>Details</summary>
Motivation: 乳腺超声分割对计算机辅助诊断和治疗规划至关重要。现有可提示方法在文本或空间提示可用时能改善分割性能，但许多公共乳腺超声数据集缺乏可靠的元数据或报告，限制了训练只能在小规模多模态子集上进行，降低了模型的鲁棒性。

Method: 提出NullBUS多模态混合监督框架，能够从有提示和无提示的图像中学习。引入可空提示，通过可学习的空嵌入和存在掩码实现，当元数据缺失时回退到仅图像证据，当文本存在时则使用文本信息。

Result: 在三个公共乳腺超声数据集的统一池上评估，NullBUS实现了平均IoU为0.8568，平均Dice为0.9103，在混合提示可用性下展示了最先进的性能。

Conclusion: NullBUS框架通过处理提示可用性的变化，有效解决了乳腺超声分割中元数据缺失的问题，在混合监督设置下实现了优异的分割性能。
...
TL;DR: CHAMMI-75是一个包含75个不同生物学研究的异质多通道显微镜图像开放数据集，用于开发可跨不同显微镜成像类型通用的细胞形态量化模型。


<details>
  <summary>Details</summary>
Motivation: 当前用于量化细胞形态的机器学习模型通常针对单一显微镜成像类型训练，导致模型无法跨生物学研究重用，因为技术规格不匹配或目标实验条件超出分布范围。

Method: 从公开来源整理构建CHAMMI-75数据集，包含75个不同生物学研究的异质多通道显微镜图像，用于研究通道自适应且能处理任何显微镜图像类型的细胞形态模型。

Result: 实验表明，使用CHAMMI-75训练可以提升多通道生物成像任务的性能，主要归因于其在显微镜模态方面的高度多样性。

Conclusion: 这项工作为创建下一代用于生物学研究的细胞形态模型铺平了道路，CHAMMI-75数据集有助于开发更具通用性和适应性的细胞形态分析工具。
...
TL;DR: 提出自适应视觉预处理方法，通过动态调整输入分辨率和空间覆盖范围来减少视觉冗余，与FastVLM集成无需修改架构或重新训练，在DocVQA数据集上实现超过50%的推理时间减少和55%以上的视觉标记减少。


<details>
  <summary>Details</summary>
Motivation: 视觉语言模型在处理高分辨率视觉输入时面临高推理延迟和计算成本问题。现有流水线依赖静态视觉预处理，导致对视觉简单输入产生冗余计算，需要更高效的部署方案。

Method: 提出自适应视觉预处理方法，结合内容感知图像分析、自适应分辨率选择和内容感知裁剪，在视觉编码前减少视觉冗余。该方法与FastVLM集成，无需修改架构或重新训练。

Result: 在DocVQA数据集上的实验显示：自适应预处理使每图像推理时间减少超过50%，降低平均完整生成时间，相比基线流水线持续减少超过55%的视觉标记数量。

Conclusion: 输入感知预处理是提高视觉语言模型部署效率的有效轻量级策略，能够显著减少计算冗余而不影响模型架构。
...
<details>
  <summary>Details</summary>
Motivation: 传统讲座视频缺乏实时澄清机制，学习者遇到困惑时需要外部搜索；现有交互学习系统通常缺乏讲座意识、依赖云端服务，或未能将检索与虚拟人讲解整合到统一的隐私保护流程中

Method: ALIVE系统包含三个核心组件：1）通过ASR转录、LLM精炼和神经说话头合成生成的虚拟人讲座；2）结合语义相似性和时间戳对齐的内容感知检索机制；3）实时多模态交互，支持暂停、文本/语音提问，并获得基于文本或虚拟人讲解的答案。系统采用轻量级嵌入模型、FAISS检索和分段虚拟人合成等技术保证响应速度

Result: 在完整医学影像课程上演示了系统，评估显示ALIVE提供准确、内容感知且吸引人的实时支持，检索准确率和延迟特性表现良好，用户体验得到验证

Conclusion: ALIVE展示了多模态AI与内容感知检索及本地部署相结合，能够显著提升录制讲座的教学价值，为下一代交互式学习环境提供了可扩展的路径
...
TL;DR: 本文提出了一种名为域表示注入（DRI）的新型参数高效微调策略，用于跨模态船舶重识别任务。该方法在特征空间进行优化，通过轻量级偏移编码器提取域特定表示，并自适应地注入到冻结的视觉基础模型中，以最小参数量实现最先进性能。


<details>
  <summary>Details</summary>
Motivation: 跨模态船舶重识别对全天候海上目标跟踪至关重要，但面临显著的模态差异挑战。主流方法依赖大规模配对数据集进行显式模态对齐，这限制了实际应用。基于柏拉图表示假设，本文探索视觉基础模型在弥合模态差距方面的潜力，并针对现有参数高效微调方法在有限容量模型上表现不佳的问题，提出从权重空间转向特征空间的优化视角。

Method: 提出域表示注入（DRI）策略：1）完全冻结视觉基础模型以保留通用知识；2）设计轻量级可学习的偏移编码器，从原始输入中提取富含模态和身份属性的域特定表示；3）通过调制器根据不同层的中间特征上下文信息自适应转换这些表示；4）通过加性融合将转换后的表示注入中间层，动态重塑特征分布以适应下游任务，而不改变预训练权重。

Result: 在HOSS-ReID数据集上，仅使用1.54M和7.05M参数分别达到57.9%和60.5%的mAP，实现了最先进的性能。该方法以最小的可训练参数取得了优异效果，代码已开源。

Conclusion: DRI方法通过在特征空间进行优化，有效解决了跨模态船舶重识别中的模态差异问题，同时最大程度保留了视觉基础模型的通用知识。该方法以参数高效的方式实现了最先进性能，为跨模态视觉任务提供了新的优化视角。
...
Method: 1. 创建首个全面的基准测试，评估VLM处理压缩图像的能力，包含多种现有图像编解码器和多样化任务，基准中包含超过100万张压缩图像；2. 分析性能差距的来源，将差距分为压缩过程中的信息损失和VLM的泛化失败两类；3. 提出通用VLM适配器来增强模型在现有编解码器压缩图像上的性能。

Result: 研究发现：1. 对于压缩图像，只有泛化差距可以被缓解；2. 单个适配器可以将VLM在不同编解码器和比特率压缩图像上的性能提升10%-30%。

Conclusion: 该研究的基准测试和增强方法为理解VLM与压缩图像之间的差距提供了有价值的见解，有助于弥合这一差距，推动VLM在压缩图像处理方面的发展。
...
Method: 提出PanoGrounder框架，使用全景渲染作为2D和3D之间的中间表示，增强3D语义和几何特征。采用三阶段流程：基于场景布局和几何放置紧凑的全景点位；使用VLM在每个全景渲染上定位文本查询；通过提升将每个视角的预测融合为单个3D边界框。

Result: 在ScanRefer和Nr3D数据集上取得了最先进的结果，并在未见过的3D数据集和文本重述上表现出优越的泛化能力。

Conclusion: PanoGrounder通过将多模态全景表示与预训练2D VLM相结合，为3D视觉定位提供了一种通用且泛化能力强的解决方案。
...
TL;DR: 提出Real-centric Envelope Modeling (REM)方法，通过建模真实图像的鲁棒分布而非生成器特定伪影，来解决真实世界条件下合成图像检测的泛化问题。构建RealChain基准测试，在八个基准评估中平均提升7.5%。


<details>
  <summary>Details</summary>
Motivation: 生成模型快速发展，但现有检测器容易过拟合到特定生成器的伪影特征，且对真实世界图像退化高度敏感。随着生成架构演进和图像经历多轮跨平台分享与后处理（链式退化），这些伪影线索会变得过时且难以检测。

Method: 提出Real-centric Envelope Modeling (REM)新范式：1）通过自重建中的特征级扰动生成近真实样本；2）使用具有跨域一致性的包络估计器学习包围真实图像流形的边界。同时构建RealChain基准测试，涵盖开源和商业生成器并模拟真实世界退化。

Result: 在八个基准评估中，REM平均比最先进方法提升7.5%。在严重退化的RealChain基准测试上保持出色的泛化能力，为真实世界条件下的合成图像检测奠定坚实基础。

Conclusion: REM通过从学习生成器伪影转向建模真实图像鲁棒分布，有效解决了真实世界条件下合成图像检测的泛化问题，为实际应用提供了可靠解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有神经激光雷达建图方法大多依赖密集隐式表示且未充分利用几何结构，而现有体素引导方法难以实现实时性能，需要一种既能利用几何结构又能保持高效性能的解决方案

Method: 提出XGrid-Mapping混合网格框架：1) 结合稀疏网格（提供几何先验和结构指导）和隐式密集网格（丰富场景表示）；2) 将VDB结构与基于子图的组织相结合，减少计算负载；3) 引入基于蒸馏的重叠对齐策略，确保子图间重叠区域的一致性；4) 加入动态移除模块增强鲁棒性和采样效率

Result: 实验表明该方法提供卓越的建图质量，克服了体素引导方法的效率限制，性能优于现有最先进的建图方法

Conclusion: XGrid-Mapping通过显式和隐式表示的联合利用，实现了高效的大规模增量神经激光雷达建图，在保持高质量的同时解决了现有方法的效率瓶颈
...
TL;DR: 提出一种基于扩散模型的数据合成管道，利用大量正常X光片增强罕见肺部异常病变的表示，通过大语言模型知识引导和渐进增量学习策略稳定修复微调过程。


<details>
  <summary>Details</summary>
Motivation: 胸部X光中的长尾肺部异常病变诊断面临挑战，现有扩散方法因罕见病变样本稀缺而生成能力有限，导致诊断精度不足。

Method: 1) 收集大量正常X光片训练扩散模型生成正常图像；2) 利用预训练扩散模型修复病变X光中的头部病变，保留尾部类别作为增强训练数据；3) 结合大语言模型知识引导模块和渐进增量学习策略稳定修复微调过程。

Result: 在公开肺部数据集MIMIC和CheXpert上的综合评估表明，该方法在性能上创造了新的基准。

Conclusion: 提出的数据合成管道能有效增强罕见肺部病变的表示，提升长尾肺部异常诊断的精度。
...
TL;DR: 提出前馈式多视角逆渲染框架，通过注意力机制实现跨视角一致性，并利用无标签真实视频进行一致性微调，提升泛化能力。


<details>
  <summary>Details</summary>
Motivation: 现有单视角方法忽略跨视角关系导致结果不一致，而多视角优化方法依赖缓慢的可微分渲染和逐场景优化，计算成本高且难以扩展。同时，合成数据训练的模型难以泛化到真实场景。

Method: 提出前馈式多视角逆渲染框架，通过交替注意力机制捕获视角内长距离光照交互和视角间材质一致性。采用基于一致性的微调策略，利用无标签真实视频增强多视角一致性和野外条件下的鲁棒性。

Result: 在基准数据集上的广泛实验表明，该方法在多视角一致性、材质和法线估计质量以及真实图像泛化能力方面达到最先进性能。

Conclusion: 该方法通过前馈式架构和一致性微调策略，有效解决了多视角逆渲染中的一致性和泛化问题，为场景级推理提供了高效解决方案。
...
TL;DR: NExT-Vid是一个创新的自回归视觉生成预训练框架，通过掩码下一帧预测联合建模图像和视频，解决了现有方法语义定位不准确和生成质量差的问题。
...
Method: 提出了NExT-Vid框架，采用掩码下一帧预测联合建模图像和视频。引入上下文隔离的自回归预测器来解耦语义表示和目标解码，以及条件流匹配解码器来增强生成质量和多样性。通过上下文隔离流匹配预训练获得强表示。
...
<details>
  <summary>Details</summary>
Motivation: 深度学习模型在计算机视觉中依赖大规模标注数据，在数据有限或分布变化时容易过拟合。现有基于掩码的信息丢弃增强方法缺乏结构感知，可能丢弃重要语义信息

Method: 提出粒度球引导掩码（GBGM），基于粒度球计算（GBC）的结构感知增强策略。通过粗到细的分层掩码过程，自适应保留语义丰富、结构重要的区域，抑制冗余区域
...
TL;DR: FluencyVE：一种基于Mamba模块的简单有效单次视频编辑方法，通过替换时间注意力层解决视频编辑中的时间不一致和高计算成本问题


<details>
  <summary>Details</summary>
Motivation: 大规模文本到图像扩散模型在图像生成和编辑方面取得了巨大成功，但扩展到视频编辑仍然具有挑战性。现有方法通过添加时间注意力机制来适应视频任务，但仍存在时间不一致问题和高计算开销。

Method: 提出FluencyVE方法，将线性时间序列模块Mamba集成到基于预训练Stable Diffusion模型的视频编辑模型中，替换时间注意力层。采用低秩近似矩阵替换因果注意力中的查询和键权重矩阵，并在训练中使用加权平均技术更新注意力分数。

Result: 实验和分析表明，该方法在编辑真实世界视频的各种属性、主题和位置方面取得了有希望的结果，有效保持了文本到图像模型的生成能力，同时显著降低了计算负担。

Conclusion: FluencyVE是一种简单而有效的单次视频编辑方法，通过集成Mamba模块和优化注意力机制，解决了视频编辑中的时间不一致和计算效率问题，为视频编辑任务提供了有前景的解决方案。
...
TL;DR: 提出首个针对3D场说话人脸生成方法的视频防御框架，通过扰动3D信息获取过程来保护肖像视频，同时保持高质量视频输出，实现47倍加速


<details>
  <summary>Details</summary>
Motivation: 当前3D场说话人脸生成方法能够实时合成高保真个性化说话人脸视频，这带来了严重的隐私滥用风险。现有基于图像的防御方法存在计算成本过高、视频质量严重下降的问题，且无法有效破坏3D信息来保护视频

Method: 提出新颖高效的视频防御框架，包含：1）相似性引导的参数共享机制以提高计算效率；2）多尺度双域注意力模块，联合优化空间-频率域扰动，通过扰动3D信息获取过程来保护肖像视频

Result: 实验表明该框架具有强大的防御能力，相比最快基线实现47倍加速，同时保持高保真度。框架对缩放操作和最新净化攻击保持鲁棒性，消融研究验证了设计选择的有效性

Conclusion: 该研究提出了首个针对3D场说话人脸生成方法的视频防御框架，有效解决了现有防御方法的局限性，在保护隐私的同时保持了视频质量，为肖像视频保护提供了高效解决方案
...
TL;DR: KOREATECH-CGH是一个包含6000对RGB-D图像和复杂全息图的大型公开数据集，支持从256×256到2048×2048的分辨率，并提出了振幅投影技术来提升大深度范围的全息图质量。


<details>
  <summary>Details</summary>
Motivation: 机器学习计算机生成全息术（ML-CGH）发展迅速，但受限于高质量大规模全息图数据集的缺乏，需要构建一个公开可用的高质量数据集来推动该领域发展。

Method: 1. 创建KOREATECH-CGH数据集：包含6000对RGB-D图像和复杂全息图，分辨率从256×256到2048×2048，深度范围覆盖角谱方法的理论极限；2. 提出振幅投影技术：一种后处理技术，在保持相位不变的同时替换全息波场在每个深度层的振幅分量，以提升大深度范围的重建质量。

Result: 振幅投影技术显著提升了重建保真度，达到27.01 dB PSNR和0.87 SSIM，比最近优化的轮廓掩模层方法分别提高了2.03 dB和0.04 SSIM。使用最先进的ML模型在全息图生成和超分辨率任务上验证了数据集的实用性。

Conclusion: KOREATECH-CGH数据集填补了ML-CGH领域高质量大规模数据集的空白，振幅投影技术有效提升了全息图质量，该数据集适用于训练和评估下一代ML-CGH系统。
...
<details>
  <summary>Details</summary>
Motivation: 核范数作为秩函数的凸替代在低秩矩阵补全中应用广泛，但会导致奇异值过度收缩，产生次优解。需要更有效的非凸替代来更好地逼近秩函数。

Method: 提出了一种新的重加权对数范数作为秩函数的非凸替代，比现有替代方法提供更接近的逼近。使用交替方向乘子法(ADMM)高效求解优化问题。

Result: 在图像修复实验中，该方法在视觉质量和定量指标上都优于最先进的低秩矩阵补全方法。

Conclusion: 提出的重加权对数范数作为秩函数的非凸替代是有效的，能够获得比核范数更好的性能，在低秩矩阵补全任务中表现出色。
...
Method: 1) 使用2D-3D混合特征提取策略从事件和物体模型中检测角点和边缘；2) 在时空窗口内最大化事件关联概率来搜索角点的光流；3) 通过光流引导建立角点和边缘之间的关联；4) 通过最小化角点和边缘之间的距离迭代优化6自由度物体姿态

Result: 在模拟和真实事件数据上的实验结果表明，该方法在准确性和鲁棒性方面优于基于事件的最先进方法

Conclusion: 提出的光流引导6自由度物体姿态跟踪方法能够有效利用事件相机的优势，在复杂环境下实现精确、鲁棒的物体姿态跟踪
...
TL;DR: DexAvatar是一个从单目手语视频重建生物力学准确的手部关节和身体运动的新框架，解决了现有3D姿态估计方法在自遮挡、噪声和运动模糊方面的问题。
...
Method: DexAvatar是一个新颖的框架，通过学习的3D手部和身体先验，从野外单目手语视频重建生物力学准确的细粒度手部关节和身体运动。

Result: 在SGNify动作捕捉数据集（该任务的唯一基准）上，DexAvatar在身体和手部姿态估计方面相比现有最优方法提升了35.11%。
...
<details>
  <summary>Details</summary>
Motivation: 计算病理学中理解与生成发展不平衡：理解模型已达到诊断水平，而生成模型仍停留在像素模拟。主要障碍包括：1)高质量图像-文本数据稀缺；2)缺乏精确的细粒度语义控制；3)术语异质性导致文本条件不可靠。

Method: 提出UniPath框架，采用多流控制：1)原始文本流；2)高层语义流，使用可学习查询从冻结的病理MLLM中提取诊断语义标记，扩展为诊断感知属性束；3)原型流，通过原型库实现组件级形态控制。构建了265万图像-文本语料库和6.8万精细标注子集。

Result: UniPath在Patho-FID指标上达到80.9分，比第二名提升51%。细粒度语义控制达到真实图像的98.7%。建立了四层评估体系，在病理图像生成任务上达到SOTA性能。

Conclusion: UniPath通过语义驱动的生成框架和多流控制机制，解决了病理图像生成中的关键挑战，实现了精确可控的病理图像生成，为计算病理学提供了强大的生成工具。
...
TL;DR: 提出了一种名为"分解与组合"的自监督多模态骨架动作表示学习框架，旨在平衡模型效率与性能，通过分解策略分离多模态特征并与单模态真值对齐，通过组合策略利用单模态特征作为自监督指导增强多模态表示学习。


<details>
  <summary>Details</summary>
Motivation: 多模态人类动作理解面临有效利用模态互补性同时保持模型效率的挑战。现有方法要么依赖简单的后期融合导致计算开销大，要么使用早期融合效率高但性能不佳。需要解决效率与效果之间的平衡问题。

Method: 提出自监督多模态骨架动作表示学习框架"分解与组合"：1) 分解策略：将融合的多模态特征分解为不同的单模态特征，并与相应的单模态真值对齐；2) 组合策略：整合多个单模态特征，作为自监督指导来增强多模态表示学习。
...
TL;DR: 提出一种新型多任务表格识别模型，使用非因果注意力捕捉完整表格结构，并采用并行推理算法加速单元格内容识别


<details>
  <summary>Details</summary>
Motivation: 当前表格识别模型虽然结合多任务学习、局部注意力和相互学习取得了良好效果，但其有效性未得到充分解释，且推理时间较长。文档中不同元素需要不同的识别方法，从大量文档中提取和使用多样化知识是智能信息检索的迫切挑战。

Method: 提出新型多任务模型，利用非因果注意力捕捉整个表格结构，并采用并行推理算法加速单元格内容推理

Result: 在两个大型公共数据集上，通过视觉和统计方式证明了该方法的优越性

Conclusion: 该方法在表格识别任务中表现出色，既能有效捕捉表格结构，又能显著提高推理速度
...
TL;DR: T2AV-Compass是一个统一的文本到音视频生成系统评估基准，包含500个多样化复杂提示和双级评估框架，用于全面评估音视频质量、跨模态对齐和指令跟随能力。
...
Method: 通过分类学驱动的流程构建500个多样化复杂提示，提出双级评估框架：客观信号级指标（视频质量、音频质量、跨模态对齐）和主观MLLM-as-a-Judge协议（指令跟随和真实性评估）。

Result: 对11个代表性T2AV系统的评估显示，即使最强模型也远未达到人类级别的真实性和跨模态一致性，在音频真实性、细粒度同步、指令跟随等方面存在持续失败。
...
TL;DR: UniRec-0.1B是一个仅0.1B参数的轻量级统一识别模型，能够在字符、单词、行、段落和文档多个层次上识别文本和公式，相比现有方法实现了2-9倍加速。
...
Method: 1) 构建包含4000万文本、公式及其混合样本的UniRec40M数据集；2) 针对层次结构变化和文本-公式语义纠缠问题，提出层次监督训练和语义解耦分词器；3) 建立涵盖中英文多领域多层次的综合评估基准。

Result: UniRec-0.1B在综合基准和公共基准测试中均优于通用视觉语言模型和领先的文档解析专家模型，同时实现2-9倍的速度提升。

Conclusion: UniRec-0.1B证明了构建轻量级统一识别模型的可行性，通过层次监督训练和语义解耦分词器有效解决了结构变化和语义纠缠问题，在保持高性能的同时显著提升了效率。
...
TL;DR: FreeInpaint：一种无需调优的即插即用图像修复方法，通过优化扩散潜在变量来提升文本引导图像修复的准确性和视觉合理性
...
Method: 提出FreeInpaint方法：1）先验引导的噪声优化，通过优化初始噪声引导模型关注有效修复区域；2）专门设计的复合引导目标，通过优化每个步骤的中间潜在变量来增强提示对齐和视觉合理性
...
TL;DR: 该研究构建了首个大规模海洋视觉语言模型数据集和基准测试MarineEval，包含2000个基于图像的问答对，用于评估现有VLMs在海洋专业领域的表现，发现现有模型在回答海洋专业问题时效果不佳。


<details>
  <summary>Details</summary>
Motivation: 尽管视觉语言模型（VLMs）在各种领域取得了成功，但研究者想知道现有VLMs是否能作为领域专家，准确回答需要大量专业知识的海洋问题，这些问题的特殊领域挑战和需求尚未得到充分评估。

Method: 构建了首个大规模海洋VLM数据集和基准测试MarineEval，包含2000个基于图像的问答对。在数据构建过程中，确保数据的多样性和覆盖范围：包括7个任务维度和20个能力维度。领域需求被特别整合到数据构建中，并由相应的海洋领域专家验证。

Result: 对17个现有VLMs在MarineEval上进行了全面基准测试，实验结果表明现有VLMs无法有效回答领域特定问题，性能仍有很大提升空间。

Conclusion: 现有视觉语言模型在海洋专业领域的表现有限，需要进一步改进。该研究的新基准和观察结果将促进未来研究，推动VLMs在专业领域的应用发展。
...
TL;DR: TGC-Net是一个基于CLIP的文本引导医学分割框架，通过参数高效的特定任务适配解决CLIP在医学图像应用中的三个主要问题，在多个数据集上实现了最先进的性能。


<details>
  <summary>Details</summary>
Motivation: 现有文本引导医学分割方法通常依赖未对齐的图像和文本编码器，需要复杂的多模态融合模块。虽然CLIP提供了预对齐的多模态特征空间，但直接应用于医学图像存在三个主要问题：细粒度解剖结构保留不足、复杂临床描述建模不充分、领域特定语义不对齐。
...
Result: 在胸部X光和胸部CT模态的五个数据集上的实验表明，TGC-Net以显著更少的可训练参数实现了最先进的性能，包括在具有挑战性的基准测试中获得了显著的Dice增益。

Conclusion: TGC-Net通过参数高效的特定任务适配有效解决了CLIP在医学图像分割中的应用限制，为文本引导的医学分割提供了一种高效且高性能的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 海洋视觉理解对于监测和保护海洋生态系统至关重要，但目前进展受到训练数据有限和缺乏系统任务定义的阻碍，限制了有效模型的应用。

Method: 创建ORCA多模态基准数据集，包含精细的视觉和文本标注，捕捉不同海洋物种的形态特征。评估18个最先进模型在三个任务上的表现：目标检测（闭集和开放词汇）、实例描述和视觉定位。

Result: 评估结果突出了海洋理解的关键挑战，包括物种多样性、形态重叠和特定领域需求，表明海洋视觉理解具有较高难度。
...
TL;DR: 提出了一种基于隐式神经表示的人类运动序列连续表示框架NAME，能够以任意帧率对输入运动序列进行插值、中间帧生成甚至外推。


<details>
  <summary>Details</summary>
Motivation: 物理运动本质上是连续的，更高的相机帧率通常能带来更好的平滑性和时间一致性。本文首次探索人类运动序列的连续表示，使其能够以任意帧率对输入运动序列进行插值、中间帧生成和外推。

Method: 提出了一种新颖的参数化激活诱导的层次隐式表示框架NAME，基于隐式神经表示。该方法引入了层次时间编码机制，从多个时间尺度提取运动序列特征，有效捕捉复杂的时间模式。此外，在基于MLP的解码器中集成了由傅里叶变换驱动的自定义参数化激活函数，增强了连续表示的表达能力。

Result: 在多个基准数据集上的广泛评估证明了所提出方法的有效性和鲁棒性。

Conclusion: NAME框架首次实现了人类运动序列的连续表示，能够以任意帧率进行运动序列的插值、中间帧生成和外推，通过层次时间编码和参数化激活函数显著提升了复杂运动行为的表示精度。
...
<details>
  <summary>Details</summary>
Motivation: 开发一个能够在有限训练资源下生成高质量3D几何体的框架，解决现有公开3D数据集质量参差不齐的问题，并实现可靠的3D生成能力。

Method: 采用两阶段生成流程：1）合成粗略全局结构；2）通过体素细化进行几何细节合成。开发了包含新型水密处理方法和高质量数据过滤的综合数据处理管道。在扩散过程中将空间定位与几何细节合成解耦，使用基于粗几何体的体素查询提供位置锚点，通过RoPE编码，使模型专注于局部几何细节合成。

Result: 模型在公开3D数据集上训练，在有限训练资源下实现了强大的几何质量。广泛评估表明，UltraShape 1.0在数据处理质量和几何生成方面与现有开源方法具有竞争力。

Conclusion: UltraShape 1.0是一个有效的3D扩散框架，通过创新的数据处理和生成方法，在有限资源下实现了高质量的3D几何生成，代码和训练模型将开源以支持未来研究。
...
TL;DR: VisRes Bench是一个专门设计用于在没有语言监督的自然场景中研究视觉语言模型视觉推理能力的基准测试，揭示了当前最先进模型在感知和关系推理方面的明显局限性。


<details>
  <summary>Details</summary>
Motivation: 尽管视觉语言模型在视觉问答和图像描述等任务上取得了显著进展，但这些模型在多大程度上真正进行视觉推理（而非依赖语言先验）仍不清楚。需要一种在没有上下文语言监督的自然场景中评估视觉推理能力的方法。

Method: 研究者设计了VisRes Bench基准测试，包含三个复杂度级别：Level 1测试在模糊、纹理变化、遮挡和旋转等扰动下的感知补全和全局图像匹配；Level 2测试基于单一属性（如颜色、数量、方向）的规则推理；Level 3测试需要整合多个视觉属性的组合推理。该基准包含超过19,000张受控任务图像。

Result: 研究发现，在细微的感知扰动下，最先进的视觉语言模型表现接近随机水平，显示出除了模式识别之外有限的抽象能力。模型在感知和关系视觉推理能力方面存在明显限制。

Conclusion: VisRes Bench为多模态研究中推进抽象视觉推理提供了一个统一框架，揭示了当前模型在视觉推理方面的局限性，并指出了未来改进的方向。
...
TL;DR: EveryWear：基于日常可穿戴设备（智能手机、智能手表、耳机、智能眼镜）的轻量级人体运动捕捉系统，无需显式校准，通过多模态师生框架整合视觉和惯性信号，在真实世界数据上训练以消除仿真到现实的差距。


<details>
  <summary>Details</summary>
Motivation: 现有基于设备的人体运动估计方法存在穿戴性差、硬件昂贵、校准繁琐等问题，阻碍了其在日常生活中的应用。需要一种轻量级、实用且基于日常可穿戴设备的解决方案。

Method: 1. 使用日常可穿戴设备：智能手机、智能手表、耳机、配备前向和两个下向摄像头的智能眼镜；2. 无需显式校准；3. 提出Ego-Elec数据集：9小时真实世界数据，涵盖56种日常活动，17个不同室内外环境，带有MoCap提供的3D真值标注；4. 采用多模态师生框架，整合第一人称视觉线索和消费设备的惯性信号；5. 直接在真实世界数据而非合成数据上训练。

Result: 实验表明该方法优于基线模型，验证了其在实际全身运动估计中的有效性。通过真实世界数据训练有效消除了限制先前工作的仿真到现实差距。

Conclusion: EveryWear提供了一种轻量级、实用的人体运动捕捉方法，完全基于日常可穿戴设备，无需校准，通过多模态融合和真实世界数据训练解决了现有方法的局限性，为XR交互等应用提供了可行的解决方案。
...
TL;DR: 提出一种任务无关的机制，训练大型多模态模型自主发现和使用视觉推理标记，无需显式监督，在视觉中心任务上取得SOTA结果


<details>
  <summary>Details</summary>
Motivation: 当前大型多模态模型主要依赖文本作为核心推理模态，在处理以视觉为主的推理任务时能力有限。现有方法需要监督中间视觉步骤，但存在限制性先验、标注成本高、跨任务泛化差的问题。

Method: 提出任务无关机制，训练LMMs自主发现和使用视觉推理标记，无需显式监督。这些标记全局关注并以任务自适应方式重新编码图像，使模型能够提取相关视觉信息而无需手工监督。

Result: 该方法在多种视觉中心任务上优于直接微调，并取得最先进的结果，包括那些中间抽象难以指定的任务，同时还能泛化到多任务指令调优。

Conclusion: 提出的任务无关机制使LMMs能够自主发现和使用视觉推理标记，克服了现有方法的限制，在视觉中心推理任务上表现出色且具有良好的泛化能力。
...
TL;DR: 本文提出了一种轻量级两阶段图像检索方法，通过事件中心实体提取结合时间上下文信号，使用BM25进行候选过滤，再用BEiT-3模型进行重排序，在OpenEvents v1基准上取得显著性能提升。


<details>
  <summary>Details</summary>
Motivation: 现实世界中的图像文本检索面临诸多挑战，包括模糊或上下文相关的查询、语言变异性以及可扩展性需求。现有方法在处理复杂真实场景时效果有限，需要更有效的解决方案。

Method: 提出两阶段检索流程：第一阶段使用基于事件中心实体提取的BM25进行高效候选过滤，提取时间上下文信号；第二阶段应用BEiT-3模型捕获深度多模态语义并对结果进行重排序。

Result: 在OpenEvents v1基准测试中，该方法取得了0.559的平均精度均值，显著优于现有基线方法。

Conclusion: 结合事件引导过滤与长文本视觉语言建模的方法在复杂真实场景中实现了准确高效的图像检索，验证了事件中心实体提取与多阶段检索策略的有效性。
...
TL;DR: SegMo是一个基于文本描述生成3D人体动作的新框架，通过将文本和动作分解为语义连贯的片段，实现细粒度的文本-动作对齐。
...
Conclusion: 通过将文本和动作分解为语义片段并进行细粒度对齐，SegMo框架显著提升了文本到动作生成的质量，同时为相关检索任务提供了有效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 传统"一镜到底"拍摄技术成本高昂且受现实条件限制，现有视频生成方法通常采用简单的片段拼接，难以保持视觉流畅性和时间连贯性，需要一种能够生成高质量一镜到底视频的解决方案。

Method: 1) 在DiT架构中集成轻量级中间条件机制，采用自适应调优策略利用基础训练数据；2) 构建高质量数据集并进行视觉表达微调，使用定制化DPO方案解决主体运动合理性和过渡平滑性问题；3) 设计内存高效的分段自回归推理策略以生成长序列视频。

Result: 实验表明该方法能够生成视觉震撼且无缝连贯的一镜到底效果，同时保持计算效率，成功将碎片化视觉材料转化为生动、连贯的一镜到底电影体验。

Conclusion: DreaMontage框架通过创新的技术组合，成功解决了任意帧引导的一镜到底视频生成问题，为用户提供了从碎片化输入创建高质量长时长一镜到底视频的有效工具。
...
<details>
  <summary>Details</summary>
Motivation: 脑MRI异常检测面临两个主要挑战：1) 标注异常病例稀缺；2) 临床工作流中经常出现关键成像模态缺失。现有方法通常依赖固定模态配置，需要重复训练，或无法泛化到未见过的模态组合，限制了临床可扩展性。

Method: 1) 双通路DINOv2编码器结合特征分布对齐机制，将不完整模态特征与完整模态表示统计对齐；2) 内在正常原型提取器和INP引导的解码器，仅重建正常解剖模式并自然放大异常偏差；3) 训练时采用随机模态掩码和间接特征补全，使模型适应所有模态配置而无需重新训练。

Result: 在BraTS2018、MU-Glioma-Post和Pretreat-MetsToBrain-Masks数据集上的广泛实验表明，该方法在7种模态组合下始终优于最先进的工业和医学AD基线，实现了优越的泛化性能。

Conclusion: 该研究为现实世界中不完美模态条件下的多模态医学异常检测建立了一个可扩展的范式，能够处理任意MRI模态可用性下的稳健异常检测和定位。
...
TL;DR: 提出Attention-Conditional Diffusion (ACD)框架，通过注意力监督实现视频扩散模型的直接条件控制，使用稀疏3D感知对象布局作为条件信号，在保持时间一致性和视觉保真度的同时实现更好的对齐效果。


<details>
  <summary>Details</summary>
Motivation: 现有视频合成中的可控性方法存在局限：无分类器引导方法通过建模数据和条件的联合分布间接实现条件控制，可控性有限；基于分类器的引导方法可能被模型利用来提升分类器分数而不真正满足条件，导致对抗性伪影和有限的有效可控性。

Method: 提出ACD框架，通过将模型注意力图与外部控制信号对齐来实现直接条件控制；引入稀疏3D感知对象布局作为高效条件信号，配合专门的Layout ControlNet和自动化标注流程实现可扩展的布局集成。

Result: 在基准视频生成数据集上的大量实验表明，ACD在保持时间一致性和视觉保真度的同时，能够提供与条件输入更好的对齐效果，为条件视频合成建立了有效范式。

Conclusion: ACD通过注意力监督实现了视频扩散模型的直接条件控制，解决了现有方法在可控性方面的局限性，为条件视频合成提供了更有效的解决方案。
...
TL;DR: 该论文提出了一种新的图像序列生成方法，通过将生成过程分解为低分辨率序列生成和高分辨率单帧细化两个阶段，避免了传统方法中处理大型张量的低效问题。


<details>
  <summary>Details</summary>
Motivation: 当前最先进的图像序列生成方法通常将图像序列视为连续堆叠帧的大型张量，这种方法存在效率低下和瓶颈问题。作者旨在设计一种更有效的图像序列数据建模方式。

Method: 方法分为两个阶段：1）首先在低分辨率下生成粗粒度序列，使用基于网格图像的生成模型，利用扩散变换器（DiT）的自注意力机制捕捉帧间相关性；2）然后对每个帧单独进行超分辨率处理，添加序列无关的高分辨率细节。这种方法将2D图像生成器扩展为低分辨率3D图像序列生成器，无需架构修改。

Result: 相比现有图像序列生成模型，该方法实现了更优的合成质量和序列一致性，能够生成任意长度的高保真序列，在推理时间和训练数据使用方面效率更高。在多个数据集上，该方法在质量和推理速度（至少快两倍）方面均优于最先进方法。
...
TL;DR: 本文提出了一种新颖的多尺度信号处理方法，用于从表格掩码中检测表格边缘。该方法将行列转换建模为一维信号，通过高斯卷积和统计阈值处理来抑制噪声并保留稳定的结构边缘。


<details>
  <summary>Details</summary>
Motivation: 从表格中提取结构化数据在文档图像分析中至关重要，但准确识别表格分割边界（行和列）仍然具有挑战性，特别是在低分辨率或噪声图像中。现有的基于掩码的边缘检测方法虽然对噪声条件更鲁棒，但通常直接将掩码应用于图像，导致噪声敏感性、分辨率损失或计算成本高。

Method: 提出一种多尺度信号处理方法：将行和列转换建模为一维信号，使用方差逐渐增加的高斯卷积进行处理，然后通过统计阈值处理来抑制噪声同时保留稳定的结构边缘。检测到的信号峰值被映射回图像坐标以获得准确的分割边界。

Result: 实验结果表明，在PubLayNet-1M基准测试中，将所提出的方法应用于列边缘检测，使用TableNet与PyTesseract OCR时，Cell-Aware Segmentation Accuracy (CASA)从67%提高到76%。该方法通过零填充和缩放策略对分辨率变化具有鲁棒性。

Conclusion: 该方法能够有效检测表格边缘，在噪声条件下表现出色，产生适合下游分析的优化结构化表格输出，解决了现有方法在低分辨率或噪声图像中识别表格分割边界的挑战。
...
<details>
  <summary>Details</summary>
Motivation: 传统基于切片编码器的管道提取脱离上下文的切片嵌入，无法建模对局部和全局任务都至关重要的丰富幻灯片级信息，且不同切片编码器在不同下游任务中表现各异，需要一个统一的模型来上下文化来自任何切片级基础模型的嵌入。

Method: TICON使用单个共享编码器，通过掩码建模目标进行预训练，同时统一和上下文化来自不同切片级病理基础模型的表示。

Result: TICON上下文化嵌入显著提升了多种任务的性能，在切片级基准测试（HEST-Bench、THUNDER、CATCH）和幻灯片级基准测试（Patho-Bench）中建立了新的最先进结果。使用仅11K WSI预训练的TICON聚合器形成的幻灯片级基础模型，优于使用多达350K WSI预训练的最先进幻灯片级基础模型。

Conclusion: TICON提供了一个有效的框架来统一和上下文化来自不同切片级基础模型的表示，显著提升了计算病理学中各种任务的性能，同时展示了在有限数据下构建高效幻灯片级基础模型的潜力。
...
TL;DR: 提出文本引导的令牌剪枝框架，在SAM2视频分割中通过选择性减少令牌密度来提高推理效率，无需修改分割架构，实现42.5%加速和37.41%内存降低。


<details>
  <summary>Details</summary>
Motivation: SAM2等视觉基础模型在视频对象分割方面取得进展，但实际部署受到高计算和内存成本的限制。现有方法通常将所有视觉令牌传播到时间推理模块，无论其与目标对象的相关性如何，导致二次内存注意力开销和可扩展性降低。

Method: 提出文本引导的令牌剪枝框架，在视觉编码后、基于内存的传播前，通过轻量级路由机制对令牌进行排序。该机制整合局部视觉上下文、从对象中心文本描述（用户提供或自动生成）获得的语义相关性，以及有助于保留模糊或边界关键区域的不确定性线索。仅保留最信息丰富的令牌进行下游处理。

Result: 在多个具有挑战性的视频分割基准测试中，后编码器令牌剪枝实现了高达42.50%的推理加速和37.41%的GPU内存使用降低，同时保持了竞争力的J和F性能。

Conclusion: 早期令牌选择为提高基于transformer的视频分割系统在实时和资源受限应用中的可扩展性提供了实用有效的途径，展示了在不修改基础架构的情况下通过智能令牌剪枝优化效率的潜力。
...
Method: 构建了Streamo-Instruct-465K大规模指令跟随数据集，涵盖多样化时间上下文和多任务监督；通过简化的端到端训练流程在指令数据集上进行训练，实现跨异构流媒体任务的统一训练。

Result: Streamo展现出强大的时间推理能力、响应式交互能力以及在各种流媒体基准测试中的广泛泛化能力。实验表明它能够有效弥合离线视频感知模型与实时多模态助手之间的差距。

Conclusion: Streamo朝着在连续视频流中实现统一、智能的视频理解迈出了一步，为通用流媒体视频助手的发展奠定了基础。
...
TL;DR: 研究发现视觉语言模型存在显著流行度偏见，对著名建筑的识别准确率比普通建筑高34%，表明模型依赖记忆而非泛化理解


<details>
  <summary>Details</summary>
Motivation: 揭示当前最先进的视觉语言模型存在流行度偏见问题，模型对著名建筑的识别表现远优于普通建筑，这表明模型可能过度依赖记忆而非真正的理解能力

Method: 构建了YearGuessr数据集（55,546张建筑图像，来自157个国家），包含建造年份、GPS数据和页面浏览量等属性；将建造年份预测任务构建为序数回归问题，并引入流行度感知区间准确率指标来量化偏见

Result: 在30多个模型的基准测试中，包括作者提出的YearCLIP模型，证实了视觉语言模型在流行、记忆的项目上表现出色，但在未识别主题上表现显著较差，暴露了其推理能力的严重缺陷

Conclusion: 视觉语言模型存在严重的流行度偏见，过度依赖记忆而非泛化理解能力，这暴露了当前模型在真正推理能力方面的关键缺陷
...
Conclusion: HiStream通过系统性的冗余减少策略，使高分辨率视频生成变得实用且可扩展，为计算密集型视频生成任务提供了高效的解决方案
...
### [50] [Parameter-Efficient Neural CDEs via Implicit Function Jacobians](https://arxiv.org/abs/2512.20625)
...
Method: 提出参数高效的Neural CDEs替代方法，将其概念化为"连续RNN"

Result: 新方法在保持性能的同时大幅减少了所需参数数量

Conclusion: 提出的参数高效方法为Neural CDEs提供了更实用的替代方案，同时建立了与RNN的直观类比
...
### [51] [Learning Evolving Latent Strategies for Multi-Agent Language Systems without Model Fine-Tuning](https://arxiv.org/abs/2512.20629)
*Wenlong Tang*
...
<details>
  <summary>Details</summary>
Motivation: 传统静态语义表示限制了抽象概念的演化能力，需要一种低成本、可扩展且可解释的策略表示方法

Method: 构建双循环架构：行为循环基于环境奖励调整动作偏好，语言循环通过反思生成文本的语义嵌入来更新外部潜在向量

Result: 智能体潜在空间在反思驱动更新下呈现清晰收敛轨迹和关键时刻的结构性转变，系统展现出推断和适应情感智能体的涌现能力

Conclusion: 外部潜在空间可为语言智能体提供低成本、可扩展且可解释的抽象策略表示，无需修改模型参数
...
### [52] [Zero-Training Temporal Drift Detection for Transformer Sentiment Models: A Comprehensive Analysis on Authentic Social Media Streams](https://arxiv.org/abs/2512.20631)
*Aayam Bansal,Ishaan Gangwani*

Main category: cs.LG

TL;DR: 本文提出了一种无需训练的零训练时域漂移分析方法，用于分析基于Transformer的情感模型在真实社交媒体数据上的性能变化，通过系统评估发现事件驱动期间模型准确率下降可达23.4%。


<details>
  <summary>Details</summary>
Motivation: 研究动机是分析Transformer情感模型在真实世界事件期间的时域漂移问题，特别是模型在动态内容时期的稳定性，为实时情感监控系统提供有效的漂移检测方法。

Method: 方法包括：1）对三种Transformer架构进行系统评估；2）在12,279个真实社交媒体帖子上进行严格的统计验证；3）引入四种新颖的漂移度量指标；4）使用Bootstrap置信区间进行统计验证；5）与基于嵌入的基线方法进行比较。

Result: 结果显示：1）事件驱动期间模型准确率下降最高达23.4%；2）最大置信度下降13.0%（Bootstrap 95% CI: [9.1%, 16.5%]）；3）新提出的漂移指标优于基于嵌入的基线方法；4）统计验证确认了强大的检测能力，实际意义超过行业监控阈值。
...
### [53] [Enhancing Lung Cancer Treatment Outcome Prediction through Semantic Feature Engineering Using Large Language Models](https://arxiv.org/abs/2512.20633)
...
TL;DR: 使用大型语言模型作为目标导向知识策展器，将多模态临床数据转换为高质量特征，提升肺癌治疗结果预测准确性


<details>
  <summary>Details</summary>
Motivation: 肺癌治疗结果预测面临挑战：真实世界电子健康数据稀疏、异质且上下文过载；传统模型难以捕捉多模态流的语义信息；大规模微调方法在临床工作流程中不实用
...
Conclusion: 语义表示质量是稀疏临床数据预测准确性的关键决定因素；将LLMs重新定位为知识策展引擎而非黑盒预测器，为肿瘤学AI决策支持提供了可扩展、可解释且与工作流程兼容的途径
...
### [54] [Real Time Detection and Quantitative Analysis of Spurious Forgetting in Continual Learning](https://arxiv.org/abs/2512.20634)
*Weiwei Wang*
...
<details>
  <summary>Details</summary>
Motivation: 持续学习中的灾难性遗忘是大型语言模型面临的核心挑战。近期研究发现性能下降可能源于任务对齐破坏而非真实知识丢失的虚假遗忘，但现有工作仅定性描述对齐，依赖事后分析，缺乏自动区分机制。

Method: 提出浅层与深层对齐框架：1) 在0-1尺度上量化对齐深度；2) 训练中实时检测浅层对齐；3) 开发可视化与恢复预测分析工具；4) 自适应缓解策略自动区分遗忘类型并促进深层对齐。

Result: 在多个数据集和模型架构（Qwen2.5-3B到Qwen2.5-32B）上的实验显示，识别准确率达86.2-90.6%，促进深层对齐使遗忘鲁棒性比基线提高3.3-7.1%。

Conclusion: 浅层对齐是持续学习中虚假遗忘的根本原因，提出的量化框架能有效识别和缓解对齐破坏，为理解和管理大型语言模型的遗忘问题提供了系统解决方案。
...
### [55] [Data-Free Pruning of Self-Attention Layers in LLMs](https://arxiv.org/abs/2512.20636)
...
<details>
  <summary>Details</summary>
Motivation: 发现大语言模型中许多注意力子层可以被移除而几乎不影响性能，这归因于注意力抑制假设：在预训练过程中，一些深层注意力层学会了抑制自身贡献，让残差流和MLP承载表示

Method: 提出Gate-Norm准则，通过查询-键耦合度一次性、仅基于权重的注意力子层排序方法，无需校准数据、前向传播、微调或专用内核，在1秒内完成40层13B参数LLaMA模型的剪枝

Result: 剪除8-16个注意力子层可使推理吞吐量提高1.30倍，同时在BoolQ、RTE、HellaSwag等6个零样本任务上保持平均准确率在未剪枝基线的2%以内，与数据驱动方法准确率相当但评分速度快约1000倍

Conclusion: Gate-Norm实现了实用、无数据的大语言模型压缩，验证了注意力抑制假设，为高效模型部署提供了有效工具
...
### [56] [Forecasting N-Body Dynamics: A Comparative Study of Neural Ordinary Differential Equations and Universal Differential Equations](https://arxiv.org/abs/2512.20643)
*Suriya R S,Prathamesh Dinesh Joshi,Rajat Dandekar,Raj Dandekar,Sreedath Panat*

Main category: cs.LG

TL;DR: 该研究使用科学机器学习框架（NODEs和UDEs）解决n体问题，发现UDE模型比Neural ODE更数据高效，仅需20%训练数据即可准确预测，而Neural ODE需要90%数据。
...
Method: 使用Julia编程语言，采用科学机器学习框架：神经常微分方程（NODEs）和通用微分方程（UDEs）来预测系统动力学。使用合成噪声数据模拟真实观测限制，并确定预测崩溃点（模型准确预测未来未见数据所需的最小训练数据量）。

Result: UDE模型表现出更高的数据效率，仅需要20%的训练数据即可进行正确预测，而Neural ODE模型需要90%的训练数据。这表明UDE模型在数据有限的情况下具有更好的预测能力。

Conclusion: 科学机器学习方法（特别是UDEs）能够有效解决n体问题，在保持物理一致性的同时显著提高数据效率，为天体物理模拟提供了更实用的解决方案。
...
### [57] [MaskOpt: A Large-Scale Mask Optimization Dataset to Advance AI in Integrated Circuit Manufacturing](https://arxiv.org/abs/2512.20655)
*Yuting Hu,Lei Zhuang,Hua Xiang,Jinjun Xiong,Gi-Joon Nam*

Main category: cs.LG

TL;DR: MaskOpt是一个用于IC掩模优化的大规模基准数据集，包含从45nm节点真实IC设计中提取的22.6万个图块，支持单元感知和上下文感知的掩模生成。


<details>
  <summary>Details</summary>
Motivation: 随着IC尺寸缩小至低于光刻波长，光学邻近效应校正和逆光刻技术计算成本高昂，现有深度学习数据集多基于合成布局，缺乏真实设计层次结构和上下文信息，限制了实际掩模优化的应用。

Method: 从45nm节点真实IC设计中构建大规模数据集，包含104,714个金属层图块和121,952个通孔层图块，在标准单元布局处裁剪以保留单元信息，支持不同上下文窗口大小来捕捉邻近形状的光学邻近效应影响。

Result: 评估了最先进的深度学习模型用于IC掩模优化，建立了基准测试结果，揭示了基线模型之间的不同权衡。上下文大小分析和输入消融研究证实了周围几何形状和单元感知输入对于实现准确掩模生成的重要性。

Conclusion: MaskOpt数据集为单元感知和上下文感知的掩模优化提供了重要基准，通过真实设计数据和层次结构信息，推动了深度学习在IC掩模优化领域的实际应用。
...
### [58] [Dominating vs. Dominated: Generative Collapse in Diffusion Models](https://arxiv.org/abs/2512.20666)
*Hayeon Jeong,Jong-Seok Lee*

Main category: cs.LG

TL;DR: 研究发现文本到图像扩散模型在多概念提示生成中存在主导概念压制其他概念的DvD不平衡现象，通过DominanceBench系统分析了数据多样性和注意力机制的影响。


<details>
  <summary>Details</summary>
Motivation: 文本到图像扩散模型虽然能生成多样且高质量的图像，但在处理多概念提示时，经常出现一个概念主导生成过程而压制其他概念的现象，这限制了模型的可控性和可靠性。

Method: 引入DominanceBench进行系统性分析，从数据和架构两个角度研究DvD不平衡的原因，包括训练数据实例多样性分析、跨注意力动态分析以及注意力头消融实验。

Result: 研究发现训练数据实例多样性有限会加剧概念间干扰；主导概念token会快速饱和注意力，在扩散时间步中逐步压制其他概念；DvD行为源于多个注意力头的分布式机制。

Conclusion: 该研究为理解生成崩溃提供了关键见解，有助于推进更可靠和可控的文本到图像生成技术。
...
### [59] [Improving Cardiac Risk Prediction Using Data Generation Techniques](https://arxiv.org/abs/2512.20669)
*Alexandre Cabodevila,Pedro Gamallo-Fernandez,Juan C. Vidal,Manuel Lama*

Main category: cs.LG

TL;DR: 基于条件变分自编码器的架构用于生成真实的心脏康复临床记录，以解决医疗数据稀缺、不完整的问题，提升心脏风险预测模型性能


<details>
  <summary>Details</summary>
Motivation: 心脏康复是一个多阶段、多专业协作的临床过程，但现实医疗数据库存在数据稀缺、记录不完整、缺失值高等问题，限制了分析效果

Method: 提出基于条件变分自编码器的架构，用于合成与现实观察一致的临床记录，增加数据集规模和多样性

Result: 该架构能够生成连贯真实的合成数据，使用这些数据提高了心脏风险检测分类器的准确性，优于现有的深度学习合成数据生成方法

Conclusion: 基于CVAE的合成数据生成方法能有效解决心脏康复领域的数据限制问题，提升风险预测性能，减少危险诊断程序的需求
...
### [60] [Disentangling Fact from Sentiment: A Dynamic Conflict-Consensus Framework for Multimodal Fake News Detection](https://arxiv.org/abs/2512.20670)
...
<details>
  <summary>Details</summary>
Motivation: 传统多模态假新闻检测依赖一致性融合，但这种方法错误地将关键的跨模态差异视为噪声，导致过度平滑，从而稀释了伪造的关键证据。主流的一致性融合方法通过最小化特征差异来对齐模态，但这种方法从根本上失败，因为它无意中平滑了作为伪造主要证据的微妙跨模态矛盾。

Method: 提出动态冲突共识框架(DCCF)，这是一种寻求不一致性的范式。首先将输入解耦为独立的事实空间和情感空间，以区分客观不匹配和情感不协调；其次采用物理启发的特征动力学迭代极化这些表示，主动提取最大信息量的冲突；最后通过冲突共识机制将局部差异与全局上下文标准化，实现稳健的审议判断。

Result: 在三个真实世界数据集上的广泛实验表明，DCCF始终优于最先进的基线方法，平均准确率提高了3.52%。

Conclusion: 通过主动寻求和放大跨模态矛盾而非抑制差异，DCCF能够更有效地检测假新闻，解决了传统一致性融合方法过度平滑关键证据的根本缺陷。
...
### [61] [HyDRA: Hierarchical and Dynamic Rank Adaptation for Mobile Vision Language Model](https://arxiv.org/abs/2512.20674)
*Yuanhao Xi,Xiaohuan Bing,Ramin Yahyapour*

Main category: cs.LG

TL;DR: HyDRA是一个用于移动视觉语言模型的高效微调框架，通过分层动态秩调度策略，在保持可训练参数不变的情况下提升性能4.7%，部分任务甚至超越全参数微调。


<details>
  <summary>Details</summary>
Motivation: 移动视觉语言模型虽然应用场景广泛，但训练计算需求大成为实际应用障碍。标准的LoRA方法固定秩设计在处理多模态任务时能力不足，需要更高效的微调方案。

Method: 提出HyDRA框架，包含两个核心优化策略：1）分层优化：粗粒度分配不同层不同秩，细粒度调整层内秩；2）动态调整：使用轻量性能模型进行端到端自动优化，在微调过程中确定和调整秩。

Result: 在多个流行基准测试中，HyDRA始终优于基线方法，在各种模型规模下平均提升4.7%，且不增加可训练参数数量。在某些任务中甚至超越了全参数微调。

Conclusion: HyDRA通过分层动态秩调度策略有效解决了移动视觉语言模型高效微调的问题，在保持参数效率的同时显著提升性能，为实际应用提供了可行的解决方案。
...
TL;DR: FEM-Bench是一个计算力学基准测试，用于评估大语言模型生成正确有限元方法及相关代码的能力，包含33个任务，当前最先进模型无法可靠解决所有问题。


<details>
  <summary>Details</summary>
Motivation: 随着LLM在物理世界推理能力的发展，缺乏评估其生成科学有效物理模型能力的严格基准已成为关键缺口。计算力学为结构化科学推理评估提供了理想基础，其问题具有清晰的数学结构、严格的物理和数值约束，并支持客观验证。

Method: 引入FEM-Bench 2025，包含一套与计算力学研究生课程材料对齐的入门但非平凡任务。这些任务捕捉了基本的数值和物理建模挑战，同时仅代表该学科复杂性的一小部分。基准测试包含33个任务，评估模型在函数编写和单元测试编写方面的表现。

Result: 在五次尝试运行中，函数编写表现最佳的模型Gemini 3 Pro在33个任务中至少完成30个，26个任务在所有五次尝试中都成功。单元测试编写表现最佳的GPT-5平均联合成功率为73.8%。其他流行模型表现出广泛的性能差异，表明当前最先进的LLM无法可靠解决所有任务。

Conclusion: FEM-Bench为评估AI生成科学代码建立了结构化基础，未来迭代将纳入越来越复杂的任务，以跟踪模型发展过程中的进展。该基准填补了评估LLM物理建模能力的关键空白。
...
<details>
  <summary>Details</summary>
Motivation: 多模态自编码器在处理复杂多模态数据方面具有潜力，但理解其稳定性和鲁棒性对于优化训练、架构设计和实际应用至关重要。论文旨在通过分析Lipschitz性质来提升这些模型的训练稳定性。

Method: 首先推导了多模态自编码器框架中聚合方法的理论Lipschitz常数，然后基于理论分析提出了正则化注意力融合方法，最后通过一系列实验对理论发现进行实证验证。

Result: 实验结果表明，提出的融合函数不仅与理论预测一致，而且在一致性、收敛速度和准确性方面优于现有策略。通过多次试验和融合策略估计Lipschitz常数，验证了理论发现。

Conclusion: 该工作为理解多模态自编码器中的融合提供了坚实的理论基础，并贡献了一种提升其性能的解决方案，增强了模型的训练稳定性和实际应用价值。
...
TL;DR: RLVR在因果推理任务中的泛化效果取决于模型规模和训练查询难度，当模型具备足够初始推理能力时，RLVR能显著提升复杂查询的准确性。


<details>
  <summary>Details</summary>
Motivation: 研究RLVR在大型语言模型因果推理任务中的泛化条件，目前对RLVR何时能产生稳健泛化的理解不足，特别是在因果图模型的概率推理场景中。

Method: 在因果图模型概率推理设置下，构建包含关联、干预和反事实三个难度级别的查询数据集，使用不同规模（3B-32B）的Qwen-2.5-Instruct模型，对比RLVR和监督微调（SFT）的效果，分析模型初始推理能力对RLVR效果的影响。

Result: RLVR在特定模型规模和训练查询难度组合下表现出比SFT更强的同级别和跨级别泛化能力；RLVR的有效性取决于模型的初始推理能力，当具备足够初始能力时，RLVR能改善模型的边缘化策略并减少中间概率计算错误，特别是在更复杂的查询上获得显著精度提升。

Conclusion: RLVR能够改进特定的因果推理子技能，但其优势仅在模型具备足够初始能力时才会显现，表明RLVR的效果依赖于模型的基础推理能力。
...
<details>
  <summary>Details</summary>
Motivation: 时间序列基础模型在带来变革性预测能力的同时，由于训练集和测试集重叠导致信息泄露，以及全局模式向测试数据的非法转移，引发了根本性的评估危机。现有评估方法允许利用观测到的全局冲击，违反了有效基准测试所需的独立性要求。

Method: 引入TS-Arena平台，将真正未知的未来作为确定性测试环境。通过实施实时数据流的预注册机制，确保评估目标在推理时物理上不存在，从而强制执行严格的全局时间分割。该方法建立了移动的时间边界，防止历史污染。

Result: TS-Arena为能源领域提供了可持续的基础设施，用于在现实约束下比较基础模型。平台原型已在Hugging Face上提供，能够真实评估模型的泛化能力。

Conclusion: TS-Arena通过恢复预测的操作完整性，解决了时间序列基础模型评估中的根本危机，为在真实世界约束下比较基础模型提供了可靠框架。
...
TL;DR: 该研究首次针对生存分析中的Cox模型提出了亚组发现问题，开发了两种新的质量评估指标（EPE和CRS），并提出了八种算法来发现具有高预测准确性的可解释亚组。


<details>
  <summary>Details</summary>
Motivation: 在生存分析中，Cox模型可能无法在整个数据集上表现良好，但在某些特定亚组中可能非常准确。然而，现有方法缺乏合适的质量函数来评估和发现这些亚组，因此需要开发专门针对Cox模型亚组发现的新方法。

Method: 提出了两种技术创新：1）预期预测熵（EPE）-评估预测风险函数的生存模型的新指标；2）条件秩统计（CRS）-量化个体点与现有亚组生存时间分布偏差的统计对象。基于这些指标开发了八种算法，其中主要算法能够同时利用EPE和CRS。

Result: 理论分析表明EPE和CRS能够解决现有指标的许多问题。实验验证了在理想设定下能够恢复真实亚组，在实际应用中相比在整个数据集上拟合Cox模型能获得更好的模型拟合。NASA喷气发动机数据的案例研究发现了已知的非线性/同质性模式，这些发现与实践中采用的设计选择相符。

Conclusion: 该研究首次系统性地解决了Cox模型的亚组发现问题，提出的EPE和CRS指标以及相关算法在理论和实践中都表现出色，能够发现具有高预测准确性的可解释亚组，为生存分析中的个性化建模提供了有效工具。
...
TL;DR: 本文提出了一种针对生成式AI流程优化的矩阵指数泰勒级数算法，通过动态选择泰勒阶数和缩放因子，在保证精度的同时显著降低计算复杂度。


<details>
  <summary>Details</summary>
Motivation: 矩阵指数在科学计算和系统仿真中具有基础性作用，特别是在控制理论、量子力学和现代生成式机器学习中。虽然Padé近似结合缩放平方法长期作为标准方法，但最近基于泰勒级数的方法通过超越经典Paterson-Stockmeyer技术的多项式求值方案，提供了更高的精度和更低的计算复杂度。本文旨在为生成式AI流程的高吞吐量需求设计优化的泰勒级数算法。

Method: 提出了一种优化的泰勒级数算法，包括：1）严格的误差分析；2）开发动态选择策略，用于在给定误差容限下最小化计算量，动态选择泰勒阶数和缩放因子；3）专门针对生成式AI流程的高吞吐量需求进行设计。

Result: 广泛的数值实验表明，与现有最先进实现相比，该方法提供了显著加速，同时保持了高数值稳定性。这些结果确立了所提方法作为大规模生成建模的高效工具。

Conclusion: 该优化泰勒级数算法为矩阵指数计算提供了高效解决方案，特别适用于生成式AI应用，在保证数值精度的同时显著提升了计算性能。
...
<details>
  <summary>Details</summary>
Motivation: 解决联邦学习中高维梯度传输带来的通信成本高和隐私泄露风险问题。

Method: 客户端计算梯度在多个随机向量上的方向导数来压缩梯度，服务器通过投影回相同随机向量解码聚合信息。

Result: 通信成本从O(d)降至O(m)，收敛速率达到O(1/√K)，与FedSGD相当，同时提供可调节的隐私保护。

Conclusion: FedMPDD在通信效率、收敛性能和隐私保护之间取得了良好平衡，是一种有效的联邦学习优化方案。
...
TL;DR: 本文提出了一种基于专家混合架构的对抗训练防御系统，通过九个预训练专家模型和门控机制联合优化，提升机器学习模型对抗威胁的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 机器学习模型在面对对抗性威胁时表现出脆弱性，这些威胁包括对抗性扰动、训练数据投毒和模型信息窃取等，需要有效的防御机制来保护模型安全。

Method: 在专家混合架构中设计对抗训练模块，使用九个以ResNet-18为骨干的预训练专家模型，通过端到端训练联合更新专家模型参数和门控机制，进一步优化专家模型。

Result: 提出的防御系统优于现有的最先进防御系统和普通分类器，即使这些对比模型使用了比本文模型骨干更复杂的架构。

Conclusion: 基于专家混合架构的对抗训练防御系统能有效增强机器学习模型对抗威胁的鲁棒性，在对抗性防御方面表现出优越性能。
...
TL;DR: 本文针对Transformer基础模型规模快速增长导致的单GPU部署困难问题，提出通过定制Triton内核优化BLR压缩方法的内存访问，在内存受限GPU上实现最高3.76倍加速和3倍模型压缩。


<details>
  <summary>Details</summary>
Motivation: Transformer基础模型规模快速增长导致单GPU部署困难且计算成本高昂。虽然BLR压缩技术能学习权重矩阵的紧凑表示，但在多令牌推理时往往成为内存瓶颈，增加延迟。

Method: 使用屋顶线分析揭示BLR方法在多令牌推理中的内存瓶颈问题，为Monarch和BLAST两种BLR方法设计定制Triton内核，采用部分融合和内存布局优化技术。

Result: 在NVIDIA Jetson Orin Nano和A40等内存受限GPU上，相比PyTorch密集基线（使用CUDA后端和编译器级优化），实现了最高3.76倍加速和3倍模型大小压缩，支持Llama-7/1B、GPT2-S、DiT-XL/2、ViT-B等多种模型。

Conclusion: 通过定制Triton内核优化BLR压缩方法的内存访问，能有效解决多令牌推理中的内存瓶颈问题，在内存受限GPU上实现显著的加速和模型压缩效果。
...
<details>
  <summary>Details</summary>
Motivation: 传统上评估组合优化问题的计算难度需要大量计算资源，需要一个能够准确预测问题实例难度并提供解释的通用框架，以帮助算法选择和问题理解。

Method: 框架分为两个阶段：第一阶段创建包含图特征和难度分类的数据集，训练机器学习分类算法；第二阶段使用关联规则挖掘解释预测结果，并训练回归模型预测计算时间。

Result: 在3287个最大团问题实例上测试，仅使用三个图特征就获得了加权F1分数0.9921，少数类F1分数0.878，ROC-AUC分数0.9083；最佳关联规则支持度0.8829，准确率87.64%；最佳回归模型百分比RMSE为5.12，R²值为0.991。

Conclusion: GCO-HPIF框架在预测组合优化问题计算难度方面表现出色，同时提供可解释性，为算法选择和问题理解提供了有效工具。
...
TL;DR: RevFFN是一种针对MoE大语言模型的内存高效全参数微调方法，通过可逆Transformer块重构激活值，大幅减少内存占用，使单GPU微调成为可能。


<details>
  <summary>Details</summary>
Motivation: 全参数微调需要缓存大量中间激活值用于反向传播，导致内存开销巨大，使得大规模LLMs的微调在实践中面临挑战。现有分布式训练框架需要额外硬件资源并降低训练速度。
...
<details>
  <summary>Details</summary>
Motivation: 传统静态定价和统一涨价策略无法有效优化订阅业务的收入、利润和客户留存，需要一种动态、有约束的定价方法，既能最大化收益又能保护价格敏感客户，维持客户信任。

Method: 结合季节性时间序列模型和基于树的学习器进行多变量需求预测；使用蒙特卡洛场景测试绘制风险边界；通过约束优化强制执行客户体验、利润底线和允许流失率等业务护栏。

Result: 在异构SaaS产品组合中验证，该方法持续优于静态定价层级和统一涨价策略，通过将价格调整重新分配到支付意愿较高的细分市场，同时保护价格敏感群体。
...
TL;DR: 提出一种基于高斯过程算子学习的框架，用于求解非微扰泛函重整化群方程，该方法直接在函数空间构建灵活的泛函表示，不依赖于特定方程或离散化。


<details>
  <summary>Details</summary>
Motivation: 传统方法如局域势近似在处理非微扰泛函重整化群方程时存在局限性，特别是对于非恒定场的情况。需要一种更灵活、更通用的方法来解决这类定义在泛函上的积分-微分方程。

Method: 使用高斯过程算子学习框架，直接在函数空间构建泛函表示。该方法允许在先验均值或核设计中融入物理先验知识，具有方程无关性和离散化无关性的特点。

Result: 在Wetterich方程和Wilson-Polchinski方程等关键方程上验证了方法的性能，结果显示该方法达到或超越了现有近似方法（如局域势近似）的性能，同时具有显著更高的灵活性，能够处理非恒定场。

Conclusion: 该方法为解决非微扰泛函重整化群方程提供了一种灵活、强大的新工具，特别适用于研究更复杂的场构型（如瞬子），为量子场论研究开辟了新途径。
...
TL;DR: 该论文分析了扩散模型中记忆化与泛化的区别，证明记忆化对应存储原始训练样本的"尖峰"表示，而泛化对应捕获局部数据统计的"平衡"表示，并提出了基于表示的检测方法和训练自由编辑技术。


<details>
  <summary>Details</summary>
Motivation: 扩散模型在生成高质量多样样本方面表现出色，但存在过拟合训练目标时记忆训练数据的风险。需要理解记忆化与泛化的本质区别，以促进更有意义的新颖生成建模。

Method: 通过表示学习的视角分析扩散模型，研究两层ReLU去噪自编码器(DAE)，理论证明记忆化与泛化的表示结构差异，并在真实世界的无条件扩散模型和文本到图像扩散模型上验证理论发现。

Result: 记忆化对应模型在编码解码权重中存储原始训练样本，产生局部化的"尖峰"表示；泛化对应模型捕获局部数据统计，产生"平衡"表示。这些表示结构在深度生成模型中同样出现。

Conclusion: 学习良好表示是新颖且有意义的生成建模的核心。基于这些洞察，提出了基于表示的记忆化检测方法和训练自由编辑技术，通过表示引导实现精确控制。
...
### [76] [Generalised Linear Models in Deep Bayesian RL with Learnable Basis Functions](https://arxiv.org/abs/2512.20974)
...
<details>
  <summary>Details</summary>
Motivation: 传统贝叶斯强化学习方法假设已知转移和奖励模型的形式，限制了其在现实问题中的应用。现有的深度贝叶斯强化学习方法虽然引入了模型学习，但需要优化证据下界（ELBO），这难以优化且可能导致任务参数不明确，从而影响策略质量。

Method: 提出了GLiBRL方法，使用可学习基函数的广义线性模型来高效准确地学习转移和奖励模型。该方法具有完全可处理的边际似然，并能对任务参数和模型噪声进行贝叶斯推断。

Result: 在具有挑战性的MetaWorld ML10/45基准测试中，GLiBRL将当前最先进的深度贝叶斯强化学习方法VariBAD的成功率提升了高达2.7倍。与MAML、RL2、SDVT、TrMRL和ECET等其他代表性方法相比，GLiBRL表现出低方差和稳定的良好性能。

Conclusion: GLiBRL通过可学习基函数的广义线性模型框架，解决了深度贝叶斯强化学习中模型学习的挑战，提供了高效准确的模型学习能力，并在多个基准测试中展现出优越性能。
...
### [77] [CoSeNet: A Novel Approach for Optimal Segmentation of Correlation Matrices](https://arxiv.org/abs/2512.21000)
*Alberto. Palomo-Alonso,David Casillas-Perez,Silvia Jimenez-Fernandez,Antonio Portilla-Figueras,Sancho Salcedo-Sanz*

Main category: cs.LG

TL;DR: CoSeNet是一种用于噪声相关矩阵中相关段最优识别的新型四层算法架构，通过重叠技术和预训练机器学习算法，使用启发式算法优化参数，输出无噪声的二进制矩阵表示最优分割。


<details>
  <summary>Details</summary>
Motivation: 现有方法在噪声相关矩阵中识别相关段的效果有限，需要一种更有效、更鲁棒的方法来准确识别相关段，以支持各种应用场景。

Method: 提出CoSeNet四层架构：输入层、格式化层、重缩放层和分割层。采用重叠技术，使用预训练的机器学习算法，通过基于窗口差异度量的启发式算法优化重缩放层参数。

Result: CoSeNet能够有效识别相关矩阵中的相关段，性能优于先前类似问题的解决方法，输出无噪声的二进制矩阵表示最优分割及其分割点。

Conclusion: CoSeNet为噪声相关矩阵中的相关段识别提供了一种鲁棒且可推广的方法，在效率、内存和部署速度之间取得平衡，适用于多种应用场景。
...
### [78] [LLM Swiss Round: Aggregating Multi-Benchmark Performance via Competitive Swiss-System Dynamics](https://arxiv.org/abs/2512.21010)
*Jiashuo Liu,Jiayun Wu,Chunjie Wu,Jingkai Liu,Zaiyuan Wang,Huan Zhou,Wenhao Huang,Hongseok Namkoong*

Main category: cs.LG

TL;DR: 该论文提出CSD框架，通过模拟多轮瑞士制竞赛来评估大语言模型，使用蒙特卡洛模拟计算期望胜率，并进行失败敏感性分析，提供比传统静态评分更全面的模型评估。


<details>
  <summary>Details</summary>
Motivation: 当前大语言模型评估方法主要依赖静态评分，存在局限性：难以确定不同基准测试的合理混合比例，无法捕捉模型在连续高风险任务中的动态竞争能力和脆弱性，需要更全面的竞争性排名系统。

Method: 提出竞争性瑞士制动态框架（CSD），模拟多轮顺序竞赛，模型根据累积胜负记录在精选的基准测试序列中动态配对。使用蒙特卡洛模拟（10万次迭代）计算统计稳健的期望胜分，消除随机配对和早期轮次运气的影响。通过参数化每轮淘汰数量进行失败敏感性分析，区分稳健通用模型和激进专用模型。

Result: CSD框架提供了比传统聚合评分和静态配对模型更细致、上下文感知的排名，能够区分模型的风险偏好，代表向风险感知的下一代LLM评估迈出的重要一步。

Conclusion: CSD框架通过模拟动态竞争环境，解决了传统静态评估方法的局限性，为LLM评估提供了更全面、风险感知的排名系统，能够更好地反映模型在实际应用中的竞争能力和脆弱性。
...
<details>
  <summary>Details</summary>
Motivation: 云原生后端系统面临高度动态负载、耦合指标和并行任务等挑战，需要能够处理高维多任务时间序列的预测框架来满足智能后端管理的需求。

Method: 1) 构建共享编码结构统一表示多样化监控指标；2) 采用状态融合机制捕捉不同时间尺度的趋势变化和局部扰动；3) 引入跨任务结构传播模块建模节点间潜在依赖关系；4) 集成动态调整机制根据系统状态变化自动调节内部特征流。

Result: 实验评估显示该方法在多个误差指标上表现优异，在不同运行条件下能更准确地表示未来状态，验证了超参数敏感性、环境敏感性和数据敏感性分析的有效性。
...
TL;DR: 该研究从动力系统角度分析单层Transformer在时序数据建模中的表示能力和局限性，发现softmax注意力的凸性约束限制了线性系统动态表示，而在非线性部分可观测系统中注意力可作为自适应延迟嵌入机制。


<details>
  <summary>Details</summary>
Motivation: Transformer在时序建模中应用日益广泛，但其内部机制从动力系统角度仍缺乏理论理解。与经典自回归和状态空间模型相比，Transformer通常被视为黑箱，这种理论差距在考虑通用或零样本预测时尤为突出。

Method: 从动力系统视角将因果自注意力解释为线性历史依赖递归，通过一系列线性和非线性案例研究分析其处理时序信息的方式，识别不同的操作机制。

Result: 对于线性系统，softmax注意力的凸性约束从根本上限制了可表示的动态类别，导致在振荡设置中出现过度平滑；对于非线性部分可观测系统，注意力作为自适应延迟嵌入机制，在足够时间上下文和潜在维度下可实现有效状态重构。
...
TL;DR: MODE是一个动态调整核心集选择策略的框架，根据训练阶段自适应地组合不同选择标准，在降低内存需求的同时保持模型性能


<details>
  <summary>Details</summary>
Motivation: 传统核心集选择方法通常是静态的，无法适应训练不同阶段的数据需求变化。MODE旨在通过动态调整选择策略来优化数据效率，根据训练进程的不同阶段采用最合适的数据选择标准

Method: MODE框架动态组合多种核心集选择策略，根据训练阶段调整选择标准：早期强调类别平衡，中期关注多样性学习表示，后期聚焦不确定性。该方法提供(1-1/e)近似保证，复杂度为O(n log n)

Result: 实验表明MODE在保持竞争性准确率的同时显著减少内存需求，并提供数据效用演化的可解释性洞察

Conclusion: MODE通过动态适应训练阶段的核心集选择策略，实现了更好的数据效率平衡，为高效机器学习提供了可解释且性能优越的解决方案
...
TL;DR: 论文提出了两种新的Universum增强分类器：U-GEPSVM和IU-GEPSVM，用于EEG信号分类，在Bonn大学EEG数据集上表现出色，IU-GEPSVM在癫痫检测任务中达到85%的峰值准确率。


<details>
  <summary>Details</summary>
Motivation: 解决EEG信号分析中的关键挑战：非平稳性、低信噪比和标记数据有限。传统方法在处理这些挑战时存在局限性，需要更有效的分类方法。

Method: 提出了两种Universum增强的广义特征值近端支持向量机：U-GEPSVM通过比率型目标函数引入Universum约束；IU-GEPSVM采用加权差型公式增强稳定性，独立控制类别分离和Universum对齐。

Result: 在Bonn大学EEG数据集的两个二元分类任务中：O vs S（健康闭眼vs癫痫）和Z vs S（健康睁眼vs癫痫），IU-GEPSVM分别达到85%和80%的峰值准确率，平均准确率为81.29%和77.57%，优于基线方法。

Conclusion: 提出的Universum增强分类器有效解决了EEG信号分类的挑战，IU-GEPSVM在性能和稳定性方面表现最佳，为EEG分析提供了有前景的解决方案。
...
TL;DR: 该论文提出SAMerging方法，通过建立模型合并的PAC-Bayes泛化边界，将模型合并形式化为多教师知识蒸馏，并使用锐度感知最小化寻找平坦最小值，在视觉和NLP基准测试中达到最先进性能。


<details>
  <summary>Details</summary>
Motivation: 模型合并作为轻量级多任务学习替代方案，其泛化特性尚未得到充分探索。当前方法依赖启发式参数组合，特别是在系数缩放方面缺乏理论指导，导致性能脆弱且对初始化敏感。

Method: 1) 建立针对模型合并的平坦感知PAC-Bayes泛化边界，引入"跨任务异质性"项；2) 将模型合并形式化为稀缺未标记数据上的多教师知识蒸馏；3) 提出SAMerging方法，使用锐度感知最小化寻找平坦最小值。

Result: SAMerging在视觉和NLP基准测试中建立了新的最先进性能，实证验证了理论框架的有效性。

Conclusion: 通过理论分析和实证验证，该研究为模型合并提供了原则性框架，解决了系数缩放缺乏理论指导的问题，显著提升了合并模型的泛化性能。
...
### [84] [Transcriptome-Conditioned Personalized De Novo Drug Generation for AML Using Metaheuristic Assembly and Target-Driven Filtering](https://arxiv.org/abs/2512.21301)
...
<details>
  <summary>Details</summary>
Motivation: AML由于分子异质性高和复发率高，仍然是临床挑战。尽管精准医学引入了突变特异性疗法，但许多患者仍缺乏有效的个性化治疗选择。需要开发能够连接患者特异性转录组学和新药发现的创新方法。

Method: 1. 使用TCGA-LAML队列的批量RNA测序数据，通过WGCNA分析优先筛选20个高价值生物标志物；2. 使用AlphaFold3建模这些靶点的物理结构；3. 通过DOGSiteScorer引擎定量映射可药物化热点；4. 开发新型反应优先进化元启发式算法和多目标优化编程，从片段库组装新型配体；5. 通过ADMET分析和SwissDock分子对接进行验证。

Result: 1. 鉴定了包括HK3和SIGLEC9在内的20个高价值生物标志物；2. 生成模型产生了结构独特的化学实体，QED评分在0.5-0.7之间；3. 验证发现了高置信度候选物，如配体L1对A08A96生物标志物的结合自由能为-6.571 kcal/mol；4. 证明了该方法能够产生药理学可行的患者定制先导化合物。
...
### [85] [Learning to Solve PDEs on Neural Shape Representations](https://arxiv.org/abs/2512.21311)
*Lilian Welschinger,Yilin Liu,Zican Wang,Niloy Mitra*

Main category: cs.LG

TL;DR: 提出了一种基于神经表示的表面PDE求解方法，无需显式网格提取或逐实例训练，实现了端到端的神经表面PDE求解流程。


<details>
  <summary>Details</summary>
Motivation: 传统PDE求解器依赖多边形/三角形网格，而现代3D资产越来越多地采用神经表示，这种不匹配导致无法直接在神经域中求解表面PDE，需要显式网格提取或逐实例残差训练，阻碍了端到端工作流程。

Method: 提出了一种无网格的公式化方法，学习一个基于神经（局部）形状属性的局部更新算子，使表面PDE能够直接在神经数据所在的位置求解。该算子与流行的神经表面表示自然集成，只需在单个代表性形状上训练一次，就能泛化到形状和拓扑变化。

Result: 在解析基准测试（球体上的热方程和泊松求解）和不同表示的真实神经资产上，该方法略微优于CPM，同时与FEM保持合理接近，实现了首个端到端的表面PDE求解流程。

Conclusion: 该方法实现了直接在神经表示上求解表面PDE，无需显式网格化或逐实例优化，同时保持可微性，为神经表面分析和工程任务提供了新的解决方案。
...
### [86] [Measuring all the noises of LLM Evals](https://arxiv.org/abs/2512.21326)
*Sida Wang*

Main category: cs.LG

TL;DR: 论文提出了一种分析LLM评估中噪声的方法，定义了三种噪声类型，并提出了"全对配对方法"来测量这些噪声，发现了一些重要模式，为实验设计提供了实用指导。


<details>
  <summary>Details</summary>
Motivation: 将成熟的统计方法有效应用于LLM评估需要考虑其独特的噪声特性。LLM评估中存在多种噪声源，需要明确定义和测量这些噪声，以便更准确地进行模型比较和实验设计。

Method: 提出"全对配对方法"，将配对分析应用于所有LLM对，基于数百万个问题级预测测量三种噪声：预测噪声（同一问题生成不同答案）、数据噪声（问题抽样）、以及遵循全方差定律的总噪声。

Result: 测量揭示了清晰模式：1) 每个评估在所有模型对中表现出特征性且高度可预测的总噪声水平；2) 配对预测噪声通常超过配对数据噪声，这意味着通过平均减少预测噪声可以显著提高统计功效。

Conclusion: 这些发现使实践者能够无需定制测试即可评估显著性，并在受控实验中检测更小的效应，为LLM评估实验设计提供了实用的统计指导。
...
### [87] [MegaRAG: Multimodal Knowledge Graph-Based Retrieval Augmented Generation](https://arxiv.org/abs/2512.20626)
*Chi-Hsiang Hsiao,Yi-Cheng Wang,Tzung-Sheng Lin,Yi-Ren Yeh,Chu-Song Chen*

Main category: cs.AI

TL;DR: 本文提出了一种多模态知识图谱增强的检索生成方法，通过整合视觉线索到知识图谱构建、检索和答案生成过程，提升对长文档和跨模态内容的理解能力。


<details>
  <summary>Details</summary>
Motivation: 传统检索增强生成方法在处理长文档和领域特定内容时存在局限，主要受限于上下文窗口长度，难以进行深度推理。现有基于知识图谱的方法仅限于文本输入，无法利用视觉等多模态信息提供的补充洞察。
...
Conclusion: 多模态知识图谱增强的检索生成方法能够有效提升对复杂、长文档内容的理解能力，通过整合视觉信息实现了更好的跨模态推理性能。
...
<details>
  <summary>Details</summary>
Motivation: 为人工智能、知识工程、人机交互和创意支持系统领域的研究者提供一个多学科交流平台，促进相关领域的学术交流与合作

Method: 采用双盲同行评审流程筛选论文，部分优秀论文经过额外评审后推荐至IEICE Transactions on Information and Systems期刊发表

Result: 成功举办了第20届国际会议，出版了经过严格评审的论文集，为相关领域的研究成果提供了展示和交流的平台

Conclusion: KICSS 2025会议论文集代表了人工智能、知识工程、人机交互和创意支持系统领域的最新研究成果，通过严格的评审机制确保了论文质量，促进了学术交流与发展
...
TL;DR: 微探针方法仅用100个精心选择的探测样本就能全面评估基础模型的可靠性，相比传统方法减少90%成本，同时保持95%的覆盖率，在统计显著性上表现优异。


<details>
  <summary>Details</summary>
Motivation: 传统基础模型可靠性评估需要数千个评估样本，计算成本高且耗时，难以满足实际部署需求，需要更高效的评估方法。

Method: 结合五个关键可靠性维度的策略性提示多样性、先进的不确定性量化和自适应加权，仅使用100个精心选择的探测样本来高效检测潜在故障模式。

Result: 相比随机采样基线，微探针方法实现了23.5%更高的综合可靠性分数，具有极高的统计显著性(p < 0.001, Cohen's d = 1.21)。专家验证评分为4.14/5.0，显著优于随机选择的3.14/5.0。该方法以99.9%的统计功效完成评估，成本减少90%，同时保持传统方法95%的覆盖率。

Conclusion: 微探针方法填补了高效模型评估的关键空白，为负责任AI部署提供了实用且经济有效的可靠性评估解决方案。
...
### [90] [Erkang-Diagnosis-1.1 Technical Report](https://arxiv.org/abs/2512.20632)
*Jianbing Ma,Ao Feng,Zhenjie Gao,Xinyu Song,Li Su,Bin Chen,Wei Wang,Jiamin Wu*

Main category: cs.AI

TL;DR: Erkang-Diagnosis-1.1是基于阿里Qwen-3模型开发的医疗AI助手，整合500GB高质量医学知识，采用增强预训练和检索增强生成混合方法，通过3-5轮交互提供症状分析和诊断建议，在综合医学考试中超越GPT-4。


<details>
  <summary>Details</summary>
Motivation: 开发安全、可靠、专业的AI健康顾问，为用户提供智能健康伴侣，赋能基层医疗和健康管理，解决医疗咨询需求。
...
Conclusion: Erkang-Diagnosis-1.1是一个有效的AI医疗咨询助手，具备成为用户智能健康伴侣的潜力，在医疗AI领域展现出竞争优势。
...
### [91] [Reasoning Relay: Evaluating Stability and Interchangeability of Large Language Models in Mathematical Reasoning](https://arxiv.org/abs/2512.20647)
...
<details>
  <summary>Details</summary>
Motivation: 虽然先前工作主要关注通过内部推理策略提升模型性能，但不同模型间推理的互换性研究较少。本研究旨在探索一个模型部分完成的推理链是否可以被另一个模型可靠地继续，无论是同一模型家族内还是跨家族。

Method: 使用token级别的对数概率阈值在早期、中期和晚期阶段截断基准模型（Gemma-3-4B-IT和LLaMA-3.1-70B-Instruct）的推理链，然后用Gemma-3-1B-IT和LLaMA-3.1-8B-Instruct进行继续实验，测试家族内和跨家族行为。评估流程结合截断阈值和过程奖励模型（PRM），提供可复现的框架来评估推理稳定性。

Result: 评估显示，混合推理链通常能够保持，在某些情况下甚至能提高最终准确性和逻辑结构。这揭示了推理模型之间互换性作为一种新兴的行为特性。

Conclusion: 推理的互换性是推理模型的一个新兴行为特性，为协作AI系统中可靠的模块化推理提供了新的范式见解，展示了推理链在不同模型间转移的可行性。
...
### [92] [AIAuditTrack: A Framework for AI Security system](https://arxiv.org/abs/2512.20649)
*Zixun Luo,Yuhang Fan,Yufei Li,Youzhi Zhang,Hengyu Lin,Ziqi Wang*

Main category: cs.AI

TL;DR: 提出AiAuditTrack（AAT）框架，基于区块链记录AI使用流量和治理，通过去中心化身份和可验证凭证建立可信AI实体，记录交互轨迹实现跨系统监督和审计。


<details>
  <summary>Details</summary>
Motivation: AI驱动应用快速扩张导致AI交互数据激增，引发安全、问责和风险溯源方面的紧迫挑战，需要建立可信的审计和治理机制。

Method: 1. 利用去中心化身份（DID）和可验证凭证（VC）建立可信可识别的AI实体；2. 将AI实体建模为动态交互图中的节点，边表示时间特定的行为轨迹；3. 提出风险扩散算法追踪风险行为源头并在相关实体间传播预警；4. 在链上记录实体间交互轨迹。

Result: 使用区块链交易每秒处理量（TPS）指标评估系统性能，证明AAT在大规模交互记录下的可行性和稳定性。
...
### [93] [Mixture of Attention Schemes (MoAS): Learning to Route Between MHA, GQA, and MQA](https://arxiv.org/abs/2512.20650)
*Esmail Gumaan*
...
Conclusion: MoAS通过动态路由机制有效解决了注意力机制在质量和效率之间的权衡问题，为Transformer模型的优化提供了新思路。
...
### [94] [Memory Bear AI A Breakthrough from Memory to Cognition Toward Artificial General Intelligence](https://arxiv.org/abs/2512.20651)
*Deliang Wen,Ke Sun*
...
<details>
  <summary>Details</summary>
Motivation: 大型语言模型面临内存限制，包括有限的上下文窗口、长期知识遗忘、冗余信息积累和幻觉生成等问题，这些限制了持续对话和个性化服务的发展

Method: 基于认知科学原理构建类人记忆架构，整合多模态信息感知、动态记忆维护和自适应认知服务，对LLM记忆机制进行全链重构
...
### [95] [From Fake Focus to Real Precision: Confusion-Driven Adversarial Attention Learning in Transformers](https://arxiv.org/abs/2512.20661)
*Yawei Liu*

Main category: cs.AI

TL;DR: 提出AFA对抗性反馈注意力训练机制，通过动态掩码策略和策略梯度优化，让模型自动重新分配注意力权重到任务相关但较少出现的词语上，提升情感分析性能。


<details>
  <summary>Details</summary>
Motivation: 现有Transformer模型在情感分析任务中虽然表现出色，但注意力分布存在偏差，主要关注常见词汇而忽略了任务相关但较少出现的词语，这影响了模型的整体性能。

Method: 提出对抗性反馈注意力(AFA)训练机制：1) 使用动态掩码策略尝试掩码不同词语来欺骗判别器；2) 判别器检测掩码引起的显著差异；3) 利用Transformer对token级扰动的敏感性，采用策略梯度方法优化注意力分布，实现高效快速收敛。

Result: 在三个公开数据集上的实验表明，该方法取得了最先进的结果。将该训练机制应用于增强大型语言模型的注意力，进一步带来了12.6%的性能提升。

Conclusion: AFA训练机制能够有效解决Transformer模型注意力分布偏差问题，通过对抗性训练自动优化注意力权重分配，无需人工标注，显著提升了情感分析任务的性能。
...
### [96] [Quantifying Laziness, Decoding Suboptimality, and Context Degradation in Large Language Models](https://arxiv.org/abs/2512.20662)
*Yiqing Ma,Jung-Hua Liu*

Main category: cs.AI

TL;DR: 研究发现大型语言模型存在懒惰行为（不完整执行多部分指令），但在解码优化和上下文保持方面表现优于预期


<details>
  <summary>Details</summary>
Motivation: 研究大型语言模型存在的三种行为异常：懒惰（提前截断响应或不完整执行多部分请求）、解码次优（因短视解码而未能选择更优序列）和上下文退化（长对话中遗忘或忽略核心指令）

Method: 通过三个受控实验（A、B、C）量化这些现象，测试多个先进LLM（OpenAI GPT-4变体、DeepSeek），包括复杂多部分指令执行、简单推理任务和200轮混乱对话测试

Result: 发现广泛存在的懒惰行为：模型经常忽略必需部分或未能满足长度要求；但在简单推理任务中解码次优性证据有限（贪婪答案与最高置信度解决方案一致）；在200轮混乱对话测试中表现出意外的上下文保持鲁棒性

Conclusion: 现代LLM在详细指令遵循方面仍有挑战，但在内部缓解了一些假设的失败模式（如上下文遗忘）；建议采用自我优化和动态提示等策略减少懒惰并增强多指令遵循能力
...
### [97] [Bridging the AI Trustworthiness Gap between Functions and Norms](https://arxiv.org/abs/2512.20671)
*Daan Di Scala,Sophie Lathouwers,Michael van Bekkum*

Main category: cs.AI

TL;DR: 该立场论文指出可信人工智能(TAI)存在功能性实现(FTAI)与规范性要求(NTAI)之间的鸿沟，提出需要开发语义语言作为桥梁来评估AI系统的可信度。


<details>
  <summary>Details</summary>
Motivation: 当前可信人工智能领域存在功能性实现与规范性要求之间的脱节，使得难以有效评估AI系统的可信度。法规要求与具体技术实现之间存在差距，需要建立桥梁来弥合这一鸿沟。

Method: 提出开发一种语义语言作为概念框架，能够匹配功能性TAI和规范性TAI。这种语言将帮助开发者评估AI系统的可信度，并协助利益相关者将规范和法规转化为具体的实施步骤。

Result: 论文描述了当前最先进的研究现状，识别了FTAI与NTAI之间的差距，讨论了开发语义语言的起点及其预期效果，并提供了关键考虑因素。

Conclusion: 需要开发语义语言作为桥梁来弥合功能性实现与规范性要求之间的鸿沟，这将有助于更有效地评估可信人工智能系统，并为未来的TAI评估提供行动方向。
...
### [98] [From Pilots to Practices: A Scoping Review of GenAI-Enabled Personalization in Computer Science Education](https://arxiv.org/abs/2512.20714)
*Iman Reihanian,Yunfei Hou,Qingquan Sun*

Main category: cs.AI

TL;DR: 这篇范围综述分析了2023-2025年间32项研究，探讨生成式AI在高等教育计算机科学教育中的个性化应用机制和效果，识别了五个应用领域，并提出了探索优先的采用框架。


<details>
  <summary>Details</summary>
Motivation: 生成式AI能够在规模上实现个性化计算机科学教育，但存在这种个性化是支持还是削弱学习的问题。需要系统梳理现有研究，了解个性化机制的有效性信号和设计选择如何影响学习成果。

Method: 采用范围综述方法，从259条记录中有目的地抽样32项研究（2023-2025年），分析高等教育计算机科学背景下的个性化机制和有效性信号。识别了五个应用领域：智能辅导、个性化材料、形成性反馈、AI增强评估和代码审查。

Result: 研究发现包含解释优先指导、解决方案保留、分级提示阶梯和基于学生作品（代码、测试和评分标准）的设计比无约束聊天界面显示出更积极的学习过程。成功实施共享四种模式：基于学生作品的上下文感知辅导、需要反思的多级提示结构、与传统CS基础设施（自动评分器和评分标准）的结合，以及人在回路的质量保证。

Conclusion: 生成式AI可以作为精确支架机制，当嵌入可审计的工作流程中时，既能保持富有成效的挣扎，又能扩展个性化支持。提出了探索优先的采用框架，强调试点、工具化、保护学习的默认设置和基于证据的扩展，同时识别了学术诚信、隐私、偏见和过度依赖等风险及缓解措施。
...
### [99] [AgentMath: Empowering Mathematical Reasoning for Large Language Models via Tool-Augmented Agent](https://arxiv.org/abs/2512.20745)
*Haipeng Luo,Huawen Feng,Qingfeng Sun,Can Xu,Kai Zheng,Yufei Wang,Tao Yang,Han Hu,Yansong Tang,Di Wang*

Main category: cs.AI

TL;DR: AgentMath是一个将语言模型推理能力与代码解释器计算精度结合的智能体框架，通过自动生成高质量训练数据、创新的强化学习范式以及高效训练系统，显著提升了复杂数学问题的解决效率和准确性。


<details>
  <summary>Details</summary>
Motivation: 大型推理模型在自然语言推理方面取得了显著进展，但在处理需要复杂数学运算的问题时仍存在计算效率低和准确性不足的问题。现有模型难以有效结合推理能力和计算精度。

Method: 提出了AgentMath框架，包含三个关键创新：1）自动将自然语言思维链转换为结构化工具增强轨迹，生成高质量SFT数据；2）新颖的智能体强化学习范式，动态交织自然语言生成与实时代码执行；3）高效训练系统，采用请求级异步调度、智能体部分展开和前缀感知负载均衡等技术。

Result: 在AIME24、AIME25和HMMT25等数学竞赛基准测试中达到最先进性能，AgentMath-30B-A3B分别取得90.6%、86.4%和73.8%的准确率，实现了4-5倍的训练加速。
...
<details>
  <summary>Details</summary>
Motivation: 现实世界的顺序决策通常涉及参数化动作空间，需要同时处理离散动作决策和连续动作参数决策。现有方法存在严重局限性：规划方法需要手工制作动作模型，标准强化学习算法只适用于离散或连续动作，而少数处理参数化动作的RL方法通常依赖领域特定工程，未能充分利用这些空间的潜在结构。

Method: 本文通过使智能体能够在线自主学习状态和动作抽象，扩展了RL算法在参数化动作空间中的适用范围。引入的算法在学习过程中逐步细化这些抽象，在状态-动作空间的关键区域增加细粒度细节，以提高性能。

Result: 在多个连续状态、参数化动作领域中，这种抽象驱动的方法使TD(λ)算法实现了比最先进基线方法显著更高的样本效率。

Conclusion: 该研究成功地将强化学习扩展到参数化动作空间的长时程稀疏奖励设置中，通过自主学习状态和动作抽象，显著提高了样本效率，为解决现实世界顺序决策问题提供了有效方法。
...
TL;DR: 论文提出使用多智能体多角色辩论方法替代单一LLM自我反思，以解决LLM在推理任务中重复相同错误的问题，在HotPot QA和HumanEval基准上取得了更好的性能。


<details>
  <summary>Details</summary>
Motivation: LLMs通过反思错误可以提高推理任务性能，但单一LLM的持续自我反思会出现思维退化现象，即使知道错误仍会重复相同错误，需要解决这个问题。
...
Conclusion: 多智能体多角色辩论方法能够生成更多样化的反思，有效解决单一LLM自我反思中的思维退化问题，显著提升推理任务的性能。
...
TL;DR: 提出一个结合LangChain多智能体系统和许可区块链的架构，用于确保自主AI系统的监控、策略执行和不可篡改审计，在智能库存管理、交通信号控制和医疗监控中验证了有效性。
...
Method: 提出一个统一架构模型，包含基于LangChain的多智能体系统和许可区块链，将感知-概念化-行动循环与区块链治理层相结合，验证输入、评估推荐行动并记录执行结果。具体实现包括基于Hyperledger Fabric的系统、MCP集成的行动执行器和LangChain智能体。

Result: 实验在智能库存管理、交通信号控制和医疗监控三个场景中进行。结果表明，区块链安全验证能有效防止未经授权的操作，提供整个决策过程的可追溯性，并将操作延迟保持在合理范围内。

Conclusion: 该框架提供了一个通用系统，用于实现具有高影响力的自主AI应用，既能保持自主性又能确保责任性，为构建可信的自主决策系统提供了可行方案。
...
### [103] [FinAgent: An Agentic AI Framework Integrating Personal Finance and Nutrition Planning](https://arxiv.org/abs/2512.20991)
...
<details>
  <summary>Details</summary>
Motivation: 中等收入环境下，家庭预算有限且营养需求持续存在，食品价格波动使得平衡营养与成本成为挑战。需要一种能结合个人财务状况、健康状况和实时食品价格的智能系统来解决这一问题。

Method: 采用模块化多智能体架构，包含预算、营养、价格监控和健康个性化等特定智能体。这些智能体共享知识库，使用替代图确保在最低成本下维持营养质量。

Result: 针对沙特代表性家庭的模拟显示：相对于静态每周菜单，成本持续降低12-18%；营养充足度超过95%；在价格变化20-30%的情况下仍保持高性能。

Conclusion: 该框架能够在本地将可负担性与营养充足性相结合，为实现可持续和公平的饮食规划提供了可行途径，符合可持续发展目标中的零饥饿和良好健康目标。
...
### [104] [TrafficSimAgent: A Hierarchical Agent Framework for Autonomous Traffic Simulation with MCP Control](https://arxiv.org/abs/2512.20996)
*Yuwei Du,Jun Zhang,Jie Feng,Zhicheng Liu,Jian Yuan,Yong Li*

Main category: cs.AI

TL;DR: TrafficSimAgent是一个基于LLM的智能体框架，通过专家级协作简化交通仿真实验设计，让非专业用户也能轻松使用SUMO等仿真平台


<details>
  <summary>Details</summary>
Motivation: 现有交通仿真平台如SUMO和MATSim功能强大但使用门槛高，缺乏专业知识用户难以从零开始实验并将其应用于日常工作

Method: 提出基于LLM的专家智能体框架，采用分层协作：高层专家理解自然语言指令、规划实验流程、按需调用MCP兼容工具；低层专家根据实时交通状况为基本元素选择最优行动方案

Result: 多场景实验表明，TrafficSimAgent能在各种条件下有效执行仿真，即使面对模糊的用户指令也能产生合理结果；其专家级自主决策驱动优化优于其他系统和SOTA LLM方法

Conclusion: TrafficSimAgent通过LLM驱动的专家智能体协作，显著降低了交通仿真实验的门槛，为非专业用户提供了灵活高效的仿真解决方案
...
TL;DR: 论文证明：在聚合观察和算法盲评估条件下，用LLM角色模拟替代人类进行A/B测试是有效的基准替代方案，且区分不同方法所需的角色评估次数可通过信息论方法量化。


<details>
  <summary>Details</summary>
Motivation: A/B测试虽然是最可信的社会系统方法基准，但成本高、延迟长，限制了迭代方法开发。LLM角色模拟提供了廉价替代方案，但需要验证其是否能保持基准接口的有效性。

Method: 提出充要条件特征：当(1)方法仅观察聚合结果(聚合观察)且(2)评估仅依赖于提交的工件而非算法身份或来源(算法盲评估)时，用角色替换人类对方法而言只是面板更换。进一步定义聚合信道的信息论可区分性，量化使角色基准与实地实验同等决策相关所需的样本量。

Result: 证明在上述条件下，用角色模拟人类对方法而言与更换评估人群(如从纽约到雅加达)无法区分。推导出明确界限，确定可靠区分有意义不同方法所需独立角色评估的数量。

Conclusion: 在聚合观察和算法盲评估条件下，LLM角色模拟可作为有效的A/B测试替代基准，其决策相关性可通过增加样本量达到与实地实验相当的水平，为迭代方法开发提供了理论依据。
...
<details>
  <summary>Details</summary>
Motivation: 现有LLM安全方法存在重大缺陷，只关注显性有害内容而忽视上下文理解和意图识别能力，这为恶意用户创造了可被系统性利用的漏洞。

Method: 通过实证评估多个最先进的LLM（包括ChatGPT、Claude、Gemini和DeepSeek），分析情感框架、渐进揭示和学术论证等技术对安全机制的绕过效果。

Result: 研究发现推理增强配置反而放大了利用效果，提高了事实精确性但未能质疑底层意图；Claude Opus 4.1是例外，在某些用例中优先考虑意图检测而非信息提供。
...
<details>
  <summary>Details</summary>
Motivation: 尽管LLM在医学基准测试中常达到或超过临床医生水平，但很少有研究在真实临床数据上评估LLM，或深入分析超越表面指标的表现。本研究旨在填补这一空白，评估LLM在真实NHS初级保健数据中的药物安全审查能力。

Method: 回顾性研究使用覆盖2,125,549名成年人的NHS Cheshire和Merseyside电子健康记录。通过战略抽样选取277名患者，涵盖广泛的临床复杂性和药物安全风险。专家临床医生审查这些患者，对系统识别的问题和提出的干预措施进行分级评估。
...
Conclusion: 研究揭示了LLM在临床情境推理方面的系统性缺陷，这些缺陷在部署LLM临床AI前必须解决。需要更大规模的前瞻性评估和更深入的LLM临床行为研究。
...
Method: 提出RoboSafe，一种混合推理运行时安全防护系统，包含两个互补的推理过程：1）后向反思推理模块，持续回顾短期记忆中的轨迹以推断时间安全谓词，检测到违规时主动触发重新规划；2）前向预测推理模块，通过从长期安全记忆和智能体多模态观察中生成上下文感知的安全谓词来预测即将到来的风险

Result: 在多个智能体上的广泛实验表明，RoboSafe相比领先基线显著减少了危险行为（风险发生率降低36.8%），同时保持了接近原始的任务性能。物理机械臂的真实世界评估进一步证实了其实用性

Conclusion: RoboSafe通过可执行谓词安全逻辑为具身智能体提供了一种自适应、可验证的安全防护系统，既具有可解释性又可作为代码执行，有效解决了动态环境中的隐式安全风险问题
