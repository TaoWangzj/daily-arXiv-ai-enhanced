...
TL;DR: AutoAWG是一个可控的恶劣天气视频生成框架，通过语义引导的自适应融合、消失点锚定的时序合成和掩码训练，显著提升恶劣天气下自动驾驶感知的鲁棒性。
...
Method: 1. 语义引导的自适应多控制融合，平衡强天气风格化与安全关键目标的高保真保留；2. 消失点锚定的时序合成策略，从静态图像构建训练序列，减少对合成数据的依赖；3. 掩码训练增强长时域生成稳定性。

Result: 在nuScenes验证集上显著优于现有方法：无首帧条件时，FID和FVD相对降低50.0%和16.1%；有首帧条件时进一步降低8.7%和7.2%。在风格保真度、时序一致性和语义结构完整性方面均表现出优势。

Conclusion: AutoAWG框架在恶劣天气视频生成方面表现出色，为提升自动驾驶下游感知任务提供了实用价值，代码已开源。
...
TL;DR: FlowSG将场景图生成重新定义为混合离散-连续状态上的连续时间传输任务，通过约束感知的渐进式细化生成节点和边，相比一次性分类方法在多项指标上取得显著提升。


<details>
  <summary>Details</summary>
Motivation: 当前场景图生成方法大多将其视为一次性确定性分类问题，而非真正的渐进式生成任务。这限制了模型对复杂视觉关系的建模能力，需要更自然的生成式框架来统一对象定位和关系推理。

Method: 首先使用VQ-VAE将场景图量化为紧凑可预测的token，然后通过图Transformer预测条件速度场来传输连续几何（边界框），同时更新离散分类token（对象特征和谓词标签）的后验分布，通过流条件消息聚合耦合语义和几何信息。

Result: 在VG和PSG数据集上的闭集和开放词汇协议实验中，FlowSG在谓词R/mR和图级指标上均取得一致提升，相比最先进的USG-Par方法平均提升约3个百分点，验证了混合离散-连续生成式框架的优势。

Conclusion: FlowSG通过将场景图生成重新定义为连续时间传输任务，实现了渐进式、生成式的场景图构建，在多项指标上超越了传统一次性分类方法，为场景理解提供了更自然的生成式框架。
...
TL;DR: 本文提出了一种基于视觉的实时方法，用于估计人类对自主移动机器人（AMR）的感知状态，通过单目RGB相机结合3D人体姿态估计和头部朝向分析，判断人类是否注意到AMR，从而让AMR根据人类感知状态调整运动策略。


<details>
  <summary>Details</summary>
Motivation: 在仓库等混合人机工作环境中，现有方法通常将人类视为通用动态障碍物，导致AMR采取过于保守的行为（如减速或绕行），即使工人完全意识到并能安全共享空间。这影响了操作效率，需要更智能的人机交互方法。

Method: 采用基于单目RGB相机的实时视觉系统，集成最先进的3D人体姿态提升技术和头部朝向估计方法，确定人类相对于AMR的位置及其视线锥，从而判断人类是否感知到AMR的存在。

Result: 在NVIDIA Isaac Sim物理精确机器人仿真环境中使用合成数据进行验证，实验结果表明系统能够实时可靠地检测人类位置和注意力状态，使AMR能够根据人类感知状态安全调整运动。

Conclusion: 该方法通过实时评估人类对AMR的感知状态，使AMR能够更智能地调整运动策略，在确保安全的同时提高工业自动化环境中的操作效率，为人机协作提供了更精细的交互机制。
...
TL;DR: 提出Choreographic Syntax框架和DanceFlow数据集，开发DanceCrafter模型实现文本驱动的可控舞蹈生成，解决了舞蹈数据稀缺和复杂动作描述难题。


<details>
  <summary>Details</summary>
Motivation: 文本驱动的可控舞蹈生成研究不足，主要面临高质量数据集稀缺和复杂舞蹈动作难以描述的问题。舞蹈具有复杂的空间动态、强方向性和身体部位高度解耦的特点，现有方法难以有效处理。

Method: 1) 提出Choreographic Syntax理论框架和标注系统，融合舞蹈研究、人体解剖学和生物力学原理；2) 构建DanceFlow数据集，包含41小时高质量动作数据和634万词详细描述；3) 开发DanceCrafter模型，基于Momentum Human Rig构建运动变换器，采用连续流形运动表示和混合归一化策略，设计解剖感知损失函数调节身体部位解耦。

Result: DanceCrafter实现了高质量、稳定的复杂舞蹈序列生成。广泛的评估和用户研究表明，在动作质量、细粒度可控性和生成自然度方面达到最先进水平。

Conclusion: 通过Choreographic Syntax理论框架、DanceFlow数据集和DanceCrafter模型的综合方案，成功解决了文本驱动舞蹈生成的挑战，为可控舞蹈生成提供了新的解决方案。
...
TL;DR: 提出了一种用于双参数MRI前列腺病变3D分割的新方法，通过文本引导的多模态融合和局部边界细化，在PI-CAI数据集上达到最先进性能。


<details>
  <summary>Details</summary>
Motivation: 当前的前列腺病变自动3D分割方法面临多模态信息融合困难的问题，而现有的视觉语言模型缺乏病变级别的细粒度语义指导，导致分割精度不足。

Method: 提出了一种新的多编码器U-Net架构，包含三个关键创新：1)对齐损失增强前景文本-图像相似性以注入病变语义；2)热图损失校准相似性图并抑制虚假背景激活；3)置信度门控多头交叉注意力细化器在置信区域进行局部边界编辑。采用阶段调度训练策略稳定优化。
...
TL;DR: 该研究开发了基于多模态大语言模型的智能C臂控制框架，通过微调MLLMs实现骨骼标志点定位，在合成和真实X射线数据集上表现优异，展示了推理和空间感知能力。
...
Method: 使用标注的合成X射线数据集和真实X射线数据集，微调两种多模态大语言模型，让它们从X射线图像中检索最接近的骨骼标志点。与领先的深度学习方法进行定量比较，并进行定性实验展示MLLM的推理能力和C臂导航能力。

Result: 在两个数据集上，微调后的MLLMs在所有定位任务中都表现出与深度学习方法相当的性能。定性实验显示MLLMs具有推理能力和空间感知能力，能够纠正初始错误预测并顺序导航C臂到达目标位置。

Conclusion: 研究表明，微调后的多模态大语言模型能够实现准确的骨骼标志点定位，为智能自主C臂控制提供了有前景的解决方案。代码已在GitHub上开源。
...
Method: 提出DeltaSeg架构，包含：1) 编码器使用深度可分离卷积和扩张阶段；2) 分层注意力策略：编码器使用SE通道注意力，瓶颈和解码器使用坐标注意力，跳跃连接使用新颖的深度Delta注意力机制；3) 瓶颈处使用ASPP捕获多尺度上下文；4) 通过多尺度辅助头进行深度监督。

Result: 在S2DS（7类）和CSDD（9类）两个数据集上评估，DeltaSeg在12种竞争架构（包括U-Net、SA-UNet、UNet3+、SegFormer等）中表现最优，展示了跨损伤类型、成像条件和结构几何的强泛化能力。
...
TL;DR: URoPE：一种通用的旋转位置编码扩展，用于跨视图或跨维度的几何空间，通过采样3D点并投影到查询图像平面来实现几何感知的相对位置编码


<details>
  <summary>Details</summary>
Motivation: 现有相对位置编码方法通常局限于固定几何空间（如1D序列或规则2D/3D网格），限制了在需要跨相机视图或2D-3D空间几何推理的计算机视觉任务中的应用

Method: 为每个键/值图像块，沿对应相机光线在预定义深度锚点处采样3D点，并将其投影到查询图像平面，然后使用投影的像素坐标应用标准2D RoPE

Result: URoPE作为Transformer架构的插件位置编码，在多种任务（新视图合成、3D目标检测、目标跟踪、深度估计）中一致提升性能，涵盖2D-2D、2D-3D和时间场景

Conclusion: URoPE是一种参数无关、内参感知的相对位置编码，对全局坐标系选择具有不变性，同时完全兼容现有RoPE优化注意力核，有效提升了基于Transformer模型的几何推理能力
...
TL;DR: REVEAL框架通过将眼底照片与个体化疾病风险档案对齐，结合分组感知对比学习策略，在AD和痴呆诊断前平均8年实现早期风险预测，显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 视网膜为阿尔茨海默病和痴呆提供了独特的非侵入性观察窗口，但现有框架通常将影像和风险因素分开建模，无法捕捉对早期风险预测至关重要的多模态联合模式，且缺乏组织和对齐具有相似特征患者的机制。

Method: 提出REVEAL框架：1) 将结构化问卷风险因素转化为临床可解释的叙述文本，与预训练视觉语言模型兼容；2) 提出分组感知对比学习策略，将具有相似视网膜形态测量和风险因素的患者聚类为正对，增强多模态对齐。

Result: 该统一表示学习框架在预测AD和痴呆风险方面（平均提前8年，范围1-11年）显著优于最先进的视网膜影像模型与临床文本编码器的组合，以及通用视觉语言模型。

Conclusion: REVEAL通过联合建模视网膜生物标志物和临床风险因素，提供了一种可推广的非侵入性方法，用于早期AD和痴呆风险分层，有望实现更早干预并改善人群层面的预防性护理。
...
TL;DR: CAHAL是一个针对临床MRI厚层各向异性扫描的幻觉鲁棒、物理信息增强框架，通过双变量专家混合架构在患者原始采集空间直接操作，避免解剖幻觉和体积高估问题


<details>
  <summary>Details</summary>
Motivation: 临床实践中普遍存在的厚层各向异性MRI扫描限制了大规模自动化形态测量分析。现有的生成式超分辨率方法虽然能产生视觉上令人信服的各向同性体积，但常常引入解剖幻觉、系统性体积高估和结构扭曲，影响下游定量分析和诊断安全性

Method: 提出CAHAL框架，采用确定性双变量专家混合架构，通过专门化的残差3D U-Net专家处理输入，这些专家以体积分辨率和采集各向异性为条件。使用复合损失函数优化，包括边缘惩罚空间重建、傅里叶域频谱一致性匹配和分割引导的语义一致性约束。通过基于物理的退化从大规模真实世界数据库生成训练对

Result: 在T1加权和FLAIR序列上验证，CAHAL相比生成式基线方法取得了最先进的结果，在准确性和效率方面改进了最佳相关方法

Conclusion: CAHAL为临床MRI扫描提供了一种幻觉鲁棒、物理信息增强的解决方案，能够直接在患者原始采集空间操作，避免了解剖幻觉和体积高估问题，提高了下游定量分析的可靠性和诊断安全性
...
TL;DR: CrossPan基准测试揭示：胰腺MRI分割中，跨序列的域偏移比跨中心差异更严重，现有域泛化方法对此效果有限，而基于形状先验的基础模型在零样本设置下表现更好。


<details>
  <summary>Details</summary>
Motivation: 胰腺自动分割对腹部MRI分析至关重要，但现有模型在不同MRI序列间泛化能力差，这一问题缺乏系统性研究。作者旨在建立跨序列泛化的基准测试，识别临床部署的主要障碍。

Method: 引入CrossPan多机构基准数据集，包含1,386个3D扫描，涵盖三个常规序列（T1加权、T2加权、Out-of-Phase）和八个中心。系统评估了跨序列域偏移的严重性，比较了现有域泛化方法、基础模型（如MedSAM2）和半监督学习的效果。

Result: 1. 跨序列域偏移远严重于跨中心差异：域内Dice分数>0.85的模型在跨序列时崩溃至接近零（<0.02）；2. 现有域泛化方法在物理驱动的对比度反转下效果有限，而MedSAM2等基础模型通过对比度不变的形状先验保持中等零样本性能；3. 半监督学习仅在稳定强度分布下有效，在高器官内变异性的序列中不稳定。

Conclusion: 跨序列泛化（而非模型架构或中心多样性）是临床可部署胰腺MRI分割的主要障碍。研究强调了开发针对物理驱动对比度变化的鲁棒方法的重要性，并为该领域提供了公开数据集和基准。
...
TL;DR: 研究者开发了Ghost-100基准测试，评估视觉语言模型在渐进式胁迫性提示下的幻觉行为，发现不同模型家族对提示压力的响应模式存在显著差异。


<details>
  <summary>Details</summary>
Motivation: 当前视觉语言模型在需要可靠视觉基础的应用中部署越来越多，但它们在渐进式胁迫性提示下的行为特征尚未充分研究。现有的幻觉基准主要依赖中性提示和二元检测，无法揭示幻觉的发生率和强度如何随语言压力变化。

Method: 构建了Ghost-100基准，包含800张合成图像，涵盖三个任务家族（文本不可读性、时间读取、对象缺失），每张图像配以5个不同强度的提示。采用双轨评估协议：基于规则的H-Rate测量模型从拒绝到幻觉的转变比例，以及GPT-4o-mini评分的H-Score（1-5分）量化幻觉的置信度和特异性。

Result: 评估九个开源视觉语言模型发现：H-Rate和H-Score在不同模型家族间显著分离；阅读风格和存在检测子集对提示压力的响应方式不同；多个模型在中等语气水平表现出非单调敏感性峰值；这些模式被聚合指标所掩盖。

Conclusion: Ghost-100基准揭示了视觉语言模型在胁迫性提示下的复杂幻觉行为模式，表明需要更细致的评估方法来理解模型在不同任务类型和提示强度下的响应特性，这对实际部署中的可靠性评估具有重要意义。
...
TL;DR: 该论文引入黎曼几何框架分析潜在扩散模型的不稳定性，发现几何解耦现象：正常生成时曲率编码图像细节，但OOD生成时极端曲率浪费在不稳定的语义边界而非可感知细节上，识别出"几何热点"作为不稳定性的结构根源。
...
Method: 引入黎曼几何框架，通过分析生成雅可比矩阵，将几何分解为局部缩放（容量）和局部复杂度（曲率），研究生成过程的几何特性。

Result: 发现"几何解耦"现象：正常生成时曲率功能性地编码图像细节，但OOD生成时出现功能解耦，极端曲率浪费在不稳定的语义边界而非可感知细节上，识别出"几何热点"作为不稳定性的结构根源。
...
TL;DR: DUALVISION提出轻量级融合模块，通过局部交叉注意力将红外-可见光信息整合到多模态大语言模型中，增强模型在视觉退化条件下的鲁棒性，并发布了相关数据集和基准。


<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型在RGB图像上表现优异，但在雾、模糊、低光等常见退化条件下表现脆弱。红外成像作为RGB的补充，在这些条件下具有固有鲁棒性，但其在多模态大语言模型中的整合尚未充分探索。

Method: 提出DUALVISION轻量级融合模块，通过patch-level局部交叉注意力高效整合IR-RGB信息到多模态大语言模型中。同时创建DV-204K数据集（约25K对齐的IR-RGB图像对，含204K模态特定QA标注）和DV-500基准（500个IR-RGB图像对，500个QA对）。

Result: 在多种视觉退化条件下，DUALVISION展现出强大的实证性能。对开源和闭源多模态大语言模型进行基准测试，验证了方法的有效性。

Conclusion: DUALVISION成功将红外成像整合到多模态大语言模型中，提升了模型在视觉退化条件下的鲁棒性。发布的数据集和基准将促进未来跨模态推理研究。
...
TL;DR: 研究探索了将视觉基础模型（VFM）的2D分割知识通过蒸馏迁移到室内LiDAR帧语义分割的可行性，无需人工标注即可训练3D分割模型。


<details>
  <summary>Details</summary>
Motivation: 室内LiDAR扫描的帧级语义分割对于3D场景理解和建图应用至关重要，但获取训练深度学习模型所需的帧级标注数据成本高昂且耗时。视觉基础模型（VFMs）已成功用于图像分割，本研究旨在探索能否将这种成功通过2D到3D的蒸馏管道复制到室内场景。

Method: 采用2D到3D的蒸馏方法，将每个LiDAR扫描帧与经过VFM处理的相机图像配对。使用室内SLAM数据集进行评估，其中伪标签用于下游评估。同时提供了一个小型手动标注的LiDAR数据集进行验证。

Result: 蒸馏模型在伪标签评估下达到最高56%的mIoU，在真实标签评估下达到约36%的mIoU。这证明了无需人工标注即可实现室内LiDAR语义分割的跨模态蒸馏可行性。

Conclusion: 研究表明，通过将视觉基础模型的2D分割知识蒸馏到LiDAR模型，可以在没有手动标注的情况下实现室内LiDAR帧语义分割，为室内3D场景理解提供了一种有效的无监督学习方法。
...
Result: 在四个基准数据集（Houston和三个无人机QUH数据集）上实验表明，ConvVitMamba在精度上持续优于CNN、Transformer和Mamba方法，同时在模型大小和推理效率方面保持良好平衡。消融研究确认了各组件的互补贡献。
...
<details>
  <summary>Details</summary>
Motivation: 航空图像目标检测面临跨数据集泛化难题，包括空间分辨率、场景构成和语义标签覆盖的差异。传统模型难以学习一致且可迁移的表示，在区域特定内容和新型对象类别上表现不佳。

Method: 提出模块化学习框架，包含两个层次的路由机制：1）全局专家分配层使用潜在地理嵌入将数据集路由到专门处理模块；2）局部场景分解机制将图像子区域分配到区域特定子模块。还包括条件专家模块，利用外部语义信息实现推理期间的新型对象类别检测。

Result: 在四个数据集上的综合评估显示，该方法在多数据集泛化、区域专业化和开放类别检测方面均有显著改进。

Conclusion: 通过超越单一表示，该方法为遥感目标检测提供了一个自适应框架，能够更好地处理航空图像中的多样性和复杂性。
...
TL;DR: 提出基于层次嵌入和对抗对齐的视觉语言模型对抗鲁棒性微调框架，通过多层次对抗对齐和层次嵌入提升模型对抗攻击的鲁棒性


<details>
  <summary>Details</summary>
Motivation: 现有视觉语言模型在零样本分类中易受对抗攻击，传统鲁棒微调方法通过固定文本嵌入与图像嵌入对齐会牺牲自然性能和鲁棒性。当模型面临针对超类（如哺乳动物）和基类（如猫）的对抗攻击时，会出现鲁棒性退化问题。

Method: 提出基于层次嵌入的对抗微调框架，包含多层次对抗对齐机制；通过将视觉嵌入放置在层次结构中的适当深度，并提供嵌入深度与最大可行边界尺寸的理论联系；考虑在多个层次树上进行对齐以增强语义多样性

Result: 在多个数据集上进行实验验证，模型自然实现了多种边界尺寸，提升了对抗样本的泛化能力以增强鲁棒性

Conclusion: 通过利用类别空间的层次特性，提出的层次嵌入和对抗对齐框架有效提升了视觉语言模型的对抗鲁棒性，同时考虑了多树对齐以增强语义多样性
...
TL;DR: CXRMate-2模型在胸片报告生成任务中取得显著进展，通过多模态条件化和强化学习，在多个数据集上超越现有基准，并在45%的病例中被放射科医生评为可接受。


<details>
  <summary>Details</summary>
Motivation: 尽管胸片放射学报告生成模型发展迅速，但其临床实用性仍不确定，因为缺乏放射科医生的充分评估。需要开发更先进的模型并进行临床验证。

Method: 提出CXRMate-2模型，整合结构化多模态条件化和强化学习，使用复合奖励函数实现与放射科医生报告的语义对齐。在MIMIC-CXR、CheXpert Plus和ReXgradient数据集上进行评估，并进行盲法随机回顾性定性评估，由三名放射科医生比较生成报告和医生报告。

Result: 在多个数据集上显著超越现有基准，在MIMIC-CXR上相对MedGemma 1.5分别提升11.2%（GREEN）和24.4%（RadGraph-XL）。在临床评估中，45%的生成报告被放射科医生评为可接受（优于或等同于医生报告），在8个分析发现中有7个没有显著偏好差异。

Conclusion: 研究展示了实现临床可接受的胸片报告生成的可行路径。通过提高召回率和更好检测细微发现（如肺充血），可能达到与放射科医生报告的非劣效性。这些进展使系统有望在放射科医生主导的工作流程中进行前瞻性评估。
...
TL;DR: AdaGScale提出了一种自适应的Gaussian缩放技术，通过基于颜色贡献度调整Gaussian大小来减少Gaussian-tile对数量，从而提升3D-GS渲染速度，同时保持图像质量。


<details>
  <summary>Details</summary>
Motivation: 现有方法在减少Gaussian-tile对数量时未考虑不同Gaussian-tile对的重要性差异，而远离Gaussian中心的边缘tile对像素颜色累积贡献很小，这为基于颜色贡献度减少Gaussian-tile对提供了机会。

Method: AdaGScale在预处理阶段高效估计每个Gaussian边缘区域的颜色贡献度，基于边缘评分自适应地缩放Gaussian大小。调整后的大小仅用于tile相交测试，原始大小在颜色累积阶段保持不变以保持视觉保真度。

Result: 实验结果显示，AdaGScale在GPU上相比原始3D-GS实现了13.8倍的几何平均加速，在城市规模场景中PSNR仅下降约0.5 dB。

Conclusion: AdaGScale通过考虑Gaussian-tile对的重要性差异，有效减少了Gaussian-tile对数量，显著提升了3D-GS渲染速度，同时保持了图像质量。
...
TL;DR: 该论文提出了一个多智能体框架，通过结构化推理和反思精炼来增强多模态共情响应生成能力，解决了传统单次生成范式忽视情感感知层次结构和易产生情感偏差的问题。


<details>
  <summary>Details</summary>
Motivation: 现有多模态共情响应生成方法通常采用从多模态上下文到最终响应的隐式单次生成范式，忽视了两个内在特性：1）人类对情感线索的感知本质上是结构化的而非直接映射，传统范式忽略了情感感知的层次递进，导致情感判断失真；2）由于人类情感的复杂性和模糊性，传统范式容易产生显著的情感偏差，最终导致次优的共情效果。

Method: 提出了一个多智能体框架，包含两个核心模块：1）结构化共情推理到生成模块，通过多模态感知、一致性感知情感预测、实用策略规划和策略引导响应生成来显式分解响应生成过程；2）全局反思与精炼模块，其中全局反思智能体对中间状态和生成响应进行逐步审计，消除现有情感偏差和共情错误，并触发有针对性的重新生成。

Result: 在多个基准测试（如IEMOCAP和MELD）上的实验表明，该模型相比最先进方法具有更优越的共情响应生成能力。

Conclusion: 该闭环框架使模型能够在迭代过程中逐步提高情感感知的准确性并消除情感偏差，为多模态共情响应生成提供了一种更有效的方法。
...
TL;DR: 提出一种基于预训练生成模型的纹理过滤生成方法，通过两阶段微调策略，在少量配对图像上进行监督微调，然后在大量无标签数据上进行强化学习微调，显著提升了纹理过滤性能。


<details>
  <summary>Details</summary>
Motivation: 利用预训练生成模型的强大图像先验知识来增强纹理过滤能力，解决传统方法在处理复杂纹理时的局限性，提升纹理去除和结构保持的效果。

Method: 采用两阶段微调策略：首先在少量配对图像上进行监督微调，然后在大量无标签数据集上进行强化学习微调，使用奖励函数量化纹理去除和结构保持的质量。

Result: 实验表明该方法明显优于先前方法，能够有效处理先前具有挑战性的案例，表现出良好的性能和泛化能力。

Conclusion: 通过充分利用预训练生成模型的图像先验知识，提出的生成式纹理过滤方法取得了显著性能提升，为解决复杂纹理过滤问题提供了有效方案。
...
TL;DR: LPCVC 2025挑战赛设计了三个赛道（图像分类、开放词汇分割、单目深度估计），采用Qualcomm AI Hub进行基准测试，展示了各赛道最佳方案并总结了未来竞赛建议。


<details>
  <summary>Details</summary>
Motivation: 推动面向边缘设备的高效视觉模型发展，在精度与延迟、内存、能耗等约束条件之间取得平衡。
...
Conclusion: 提出了未来计算机视觉竞赛的建议，强调了在边缘计算环境下平衡模型精度与资源约束的重要性。
...
TL;DR: 本文提出SDB（稳定性-多样性平衡）机制，通过控制指令条件隐藏状态的偏移生成多个潜在行为假设，并进行可靠性感知的软评估与聚合，在保持多样性的同时稳定视觉语言导航中的自我改进学习。


<details>
  <summary>Details</summary>
Motivation: 在视觉语言导航中，基于策略诱导经验的自我改进需要平衡行为多样性和学习稳定性。增加行为多样性可以暴露替代动作假设，但可能破坏学习信号的稳定性；而过于保守的稳定性约束会抑制探索并导致早期承诺，使得可靠的自我改进变得困难。

Method: 提出SDB机制：1）通过控制指令条件隐藏状态的偏移，将每个决策步骤扩展为多个潜在行为假设；2）进行可靠性感知的软评估和聚合，保留多样但指令一致的选择；3）使用显式正则化约束假设间的交互，防止过度漂移或过早的多样性崩溃。

Result: 在R2R、SOON和REVERIE数据集上的实验显示一致改进：在REVERIE val-unseen上，SPL从33.73提升到35.93，OSR从51.07提升到54.25。

Conclusion: SDB机制通过平衡行为多样性和学习稳定性，有效提升了视觉语言导航中基于策略诱导经验的自我改进性能，且具有即插即用的特性。
...
TL;DR: 该论文提出了一种针对多模态测试时适应的概率高斯模型，通过显式建模类别条件分布和自适应对比不对称校正技术，解决多模态分布不对称问题，在多种分布偏移场景下实现最先进性能。


<details>
  <summary>Details</summary>
Motivation: 多模态测试时适应方法缺乏对类别条件分布的显式建模，而经典高斯判别分析在多模态场景中因模态分布不对称而效果有限，需要专门的方法来解决这一问题。

Method: 提出定制的概率高斯模型来显式建模类别条件分布，并引入自适应对比不对称校正技术来抵消模态不对称带来的负面影响，从而获得校准的预测和可靠的决策边界。
...
TL;DR: 该论文提出EgoMotion框架，通过认知推理和运动生成两阶段解耦，解决第一人称视觉语言运动生成中的推理-生成纠缠问题，实现语义准确且运动质量高的3D人体运动合成。


<details>
  <summary>Details</summary>
Motivation: 第一人称视角下的运动生成是一个尚未充分探索的挑战性任务，需要同时处理第一人称视觉观察和自然语言指令。现有方法在同时优化语义推理和运动建模时存在梯度冲突，导致多模态对齐和运动质量下降。

Method: 提出EgoMotion分层生成框架：1) 认知推理阶段：使用视觉语言模型将多模态输入映射到离散运动基元的结构化空间，学习目标一致的表征；2) 运动生成阶段：以学习到的表征为条件，在连续潜空间中使用扩散模型进行迭代去噪，生成物理合理且时间连贯的运动轨迹。

Result: EgoMotion在广泛评估中实现了最先进的性能，生成的运动序列在语义对齐和运动质量方面均优于现有方法。

Conclusion: 通过解耦认知推理和运动生成，EgoMotion有效解决了第一人称视觉语言运动生成中的推理-生成纠缠问题，为具身智能中的人类行为建模提供了新思路。
...
TL;DR: PortraitDirector是一个新颖的面部重演框架，采用分层组合方法实现高保真度和可控性，通过空间层（全局头部姿态+局部面部表情）和语义层（全局情感）的分解与重组，支持实时512×512分辨率20FPS重演。


<details>
  <summary>Details</summary>
Motivation: 现有面部重演方法在表达力和细粒度可控性之间存在权衡：整体模型为表达力牺牲控制，而控制导向方法则在保真度和解耦鲁棒性上表现不佳。本文探索组合视角，将面部运动视为可分解的层次结构。

Method: 提出分层运动解耦与组合策略：1）空间层：全局头部姿态（专用表示和注入路径）+局部面部表情（裁剪面部区域提取，通过情感过滤模块去除情感线索）；2）语义层：全局情感。解耦组件重组为表达性运动潜在表示。通过扩散蒸馏、因果注意力和VAE加速等优化实现实时性能。

Result: 在单块5090 GPU上实现流式、高保真、可控的512×512面部重演，达到20 FPS，端到端延迟800毫秒。

Conclusion: PortraitDirector通过分层组合方法解决了面部重演中表达力与可控性的权衡问题，实现了高质量、细粒度可控的实时面部重演。
...
TL;DR: BALTIC基准测试评估3D重建方法在不同介质（空气/水）和光照条件下的表现，发现高斯溅射在受控条件下性能接近专业水下方法，但在复杂环境中鲁棒性下降。


<details>
  <summary>Details</summary>
Motivation: 机器人感知中，跨介质（空气/水）的鲁棒3D重建面临挑战，特别是在不同环境和光照条件下。现有方法缺乏系统评估，需要建立基准来测试现代3D重建技术在不同介质和光照下的表现。

Method: 开发BALTIC基准测试，包含13个数据集，涵盖两种介质（空气/水）和三种光照条件（环境光、人工光、混合光）。使用定制水槽、单目相机和HTC Vive跟踪器获取精确地面真实位姿。评估COLMAP的SfM重建，并将其作为NeRF和3D高斯溅射的输入。进行跨域重建分析，通过少量相似光照条件下的空中视图增强水下序列。

Result: 在受控、纹理一致条件下，经过简单预处理（如白平衡校正）的高斯溅射方法性能可媲美专业水下方法。但在更复杂、异质的真实环境中，其鲁棒性下降。评估包括轨迹精度、场景几何、感知和光度指标，以及跨域辐射一致性分析。

Conclusion: BALTIC基准为跨介质3D重建提供了系统评估框架。高斯溅射在受控条件下表现良好，但在复杂环境中需要进一步改进。研究强调了预处理和跨域数据融合对提升水下重建性能的重要性。
...
### [29] [Diff-SBSR: Learning Multimodal Feature-Enhanced Diffusion Models for Zero-Shot Sketch-Based 3D Shape Retrieval](https://arxiv.org/abs/2604.19135)
*Hang Cheng,Fanhe Dong,Long Zeng*

Main category: cs.CV

TL;DR: 本文首次探索使用文本到图像扩散模型进行零样本基于草图的3D形状检索，通过冻结的Stable Diffusion提取特征，结合CLIP的多模态特征增强策略，解决了草图抽象稀疏和领域差异问题，在零样本设置下显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有基于草图的3D形状检索方法在零样本设置下表现不佳，主要因为缺乏类别监督和草图输入的极端稀疏性。扩散模型具有开放词汇能力和强形状偏置，适合零样本视觉检索，但面临草图抽象稀疏和与自然图像的领域差异挑战。

Method: 使用冻结的Stable Diffusion主干从中间U-Net层提取并聚合草图和渲染3D视图的判别性表示。引入多模态特征增强策略：注入来自CLIP视觉编码器的全局和局部视觉特征，结合BLIP生成的硬文本描述与可学习软提示的丰富文本指导。采用Circle-T损失动态加强正样本对吸引力。

Result: 在两个公开基准测试上的广泛实验表明，该方法在零样本基于草图的3D形状检索中始终优于最先进的方法。

Conclusion: 本文首次证明了文本到图像扩散模型在零样本基于草图的3D形状检索中的有效性，通过多模态特征增强策略解决了草图处理的挑战，为这一领域提供了新的解决方案。
...
TL;DR: 论文提出Patch Forcing框架，通过patch级别的噪声调度实现自适应图像生成，让简单区域先完成去噪以为困难区域提供上下文，提升生成质量。


<details>
  <summary>Details</summary>
Motivation: 传统扩散和流模型对所有图像区域采用统一的计算分配，忽略了自然图像的异质性。有些区域容易去噪，而其他区域需要更多细化或上下文信息。因此需要探索patch级别的噪声调度来适应这种异质性。

Method: 提出Patch Forcing框架：1) 引入时间步采样器控制训练时patch级别的最大信息量；2) 添加轻量级per-patch难度头实现自适应采样；3) 结合空间和扩散时间的噪声变化，让简单区域先完成去噪为困难区域提供上下文。

Result: 在class-conditional ImageNet上取得优越结果，与表示对齐和引导方法正交，可扩展到文本到图像合成。相比标准基线，从全局时间步转向patch级别时间步已能改善图像生成。

Conclusion: patch级别的去噪调度为自适应图像生成提供了有前景的基础，通过异质性的噪声分配和计算资源动态分配，能够更高效地生成高质量图像。
...
TL;DR: ST-Prune是一个无需训练、即插即用的时空令牌剪枝框架，用于解决自动驾驶中视觉语言模型的多视角多帧输入计算瓶颈问题，通过运动感知时间剪枝和环视空间剪枝模块，在90%令牌压缩下实现接近无损性能。


<details>
  <summary>Details</summary>
Motivation: 自动驾驶系统中的视觉语言模型面临多视角相机和多帧视频输入带来的巨大计算开销瓶颈。现有的令牌剪枝方法主要针对单图像输入，无法有效利用驾驶场景中固有的时空冗余特性。

Method: 提出ST-Prune框架，包含两个互补模块：1) 运动感知时间剪枝(MTP)：通过编码运动波动性和时间近邻性作为软约束，优先保留动态轨迹和当前帧内容；2) 环视空间剪枝(RSP)：利用环视相机几何特性惩罚双边跨视角相似性，消除重复投影和残留背景。

Result: 在感知、预测和规划四个基准测试中，ST-Prune建立了无需训练令牌剪枝的最新SOTA。在90%令牌压缩下，某些指标甚至超过完整模型基线，同时保持与现有剪枝方法相当的推理速度。

Conclusion: ST-Prune通过有效利用驾驶场景的时空冗余特性，实现了高效的令牌压缩，为自动驾驶视觉语言模型的部署提供了实用的解决方案，在保持性能的同时显著降低计算开销。
...
TL;DR: 该论文提出了一种多尺度深度结构相似性模型（MSDS），通过在不同空间尺度上独立计算DeepSSIM并融合结果，显著提升了图像质量评估的感知对齐性能。
...
Method: 提出了Deep Structural Similarity with Multiscale Representation（MSDS）框架，将深度特征表示与跨尺度集成解耦：在不同金字塔层级上独立计算DeepSSIM，然后使用轻量级可学习的全局权重融合结果分数。

Result: 在多个基准数据集上的实验表明，相比单尺度基线模型，MSDS取得了持续且统计显著的改进，同时仅引入可忽略的额外复杂度。

Conclusion: 实验结果实证确认了空间尺度是深度感知相似性中不可忽视的因素，通过最小化测试平台成功分离出了这一独立因素。
...
<details>
  <summary>Details</summary>
Motivation: 现有视频模型评估基准存在局限性，任务设计简单且评估指标碎片化，无法真正衡量模型的多模态推理能力，需要更严谨的评估框架

Method: 提出CLVG-Bench评估框架，包含1000多个高质量人工标注的元数据，涵盖6个类别47个子类别；同时提出自适应视频评估器(AVE)，使用最少标注实现与人类专家感知对齐

Result: 实验显示，即使如Seedance 2.0这样的SOTA视频模型，在逻辑推理任务上成功率<25%，交互生成任务成功率约0%，暴露了多模态推理和物理基础理解的严重瓶颈
...
### [34] [Benchmarking Vision Foundation Models for Domain-Generalizable Face Anti-Spoofing](https://arxiv.org/abs/2604.19196)
*Mika Feng,Pierre Gallin-Martel,Koichi Ito,Takafumi Aoki*

Main category: cs.CV

TL;DR: 本文重新审视仅视觉基础模型在面部防伪（FAS）中的潜力，通过系统评估15个预训练模型，发现自监督视觉模型（特别是DINOv2 with Registers）在跨域场景中表现优异，结合多种数据增强和损失函数，建立了高效且鲁棒的仅视觉基线。


<details>
  <summary>Details</summary>
Motivation: 当前基于视觉语言模型（VLMs）的多模态面部防伪方法存在计算资源需求高、推理延迟大、且效果受限于底层视觉特征质量的问题。本文旨在探索仅视觉基础模型在FAS中的潜力，建立高效且鲁棒的基线。

Method: 系统评估了15个预训练模型（包括监督CNN、监督ViT和自监督ViT），在MICO和LSD等跨域协议下进行测试。采用自监督视觉模型DINOv2 with Registers，结合FAS-Aug、PDA和APL等数据增强和损失函数技术。

Result: 提出的仅视觉基线在MICO协议中达到最先进性能，在数据受限的LSD协议中优于现有方法，同时保持卓越的计算效率。自监督视觉模型能有效抑制注意力伪影并捕捉细粒度伪造线索。

Conclusion: 优化后的自监督视觉变换器可以作为仅视觉和未来多模态FAS系统的主干，为面部防伪领域提供了明确的仅视觉基线，证明了仅视觉方法在保持高效性的同时能达到优异性能。
...
TL;DR: 提出一种用于移动设备采集3D高斯泼溅数据的对象中心化方法，通过设备姿态校准、球面网格索引和实时覆盖度引导，实现更少图像输入下的高质量重建。


<details>
  <summary>Details</summary>
Motivation: 移动设备采集3D高斯泼溅数据面临挑战，需要解决采集过程中的姿态校准、视角覆盖均匀性和实时指导问题，以提升对象中心化场景的重建质量。

Method: 1) 设备姿态校准，将设备方向对齐到基准坐标系获取相对位姿；2) 相机光轴映射到对象中心球面网格进行均匀视角索引；3) 实时计算面积加权的球面覆盖度，指导用户运动以减少极区采样偏差。

Result: 相比RealityScan和自由采集策略，该方法使用更少的输入图像获得了更优的重建质量；分析显示在对象中心化采集过程中能够获得更全面和均匀的视角覆盖。

Conclusion: 提出的移动设备采集指导方法有效解决了3D高斯泼溅数据采集的挑战，通过实时覆盖度引导和均匀视角索引，显著提升了对象中心化场景的重建效果。
...
<details>
  <summary>Details</summary>
Motivation: 当前主流的人重识别方法主要依赖大量标注数据进行拟合，而非理解身份因果关系，导致在面对多种干扰时表征脆弱。需要一种能够进行显式身份理解和推理的范式。
...
Conclusion: ReID-R通过推理驱动范式实现了显式身份理解和推理，在数据效率、性能鲁棒性和结果可解释性方面均有显著优势，为人重识别研究提供了新方向。
...
TL;DR: ASAHI是一种自适应切片辅助超推理框架，通过动态确定切片数量来减少冗余计算，提升高分辨率航拍和卫星图像中小目标检测的性能和速度。


<details>
  <summary>Details</summary>
Motivation: 现有深度学习方法在高分辨率航拍和卫星图像的小目标检测中面临挑战，包括密集目标分布、可变拍摄角度、微小目标尺寸和显著的类间差异。传统的固定尺寸切片策略虽然有效，但引入了大量冗余计算，增加了推理成本并降低了检测速度。
...
Conclusion: ASAHI通过自适应切片策略有效解决了高分辨率图像中小目标检测的计算冗余问题，在保持检测精度的同时显著提升了推理效率，为航拍和卫星图像分析提供了更高效的解决方案。
...
TL;DR: OTCA提出了一种针对扩散模型强化学习训练的结构化框架，通过轨迹级信用分解和多目标信用分配，解决了传统GRPO方法中粗粒度奖励分配的问题，从而提升了图像和视频生成质量。


<details>
  <summary>Details</summary>
Motivation: 传统GRPO方法在视觉生成模型的后训练中存在根本性限制：它将多个奖励模型（视觉质量、运动一致性、文本对齐等）压缩为单一静态标量，并在整个扩散轨迹上均匀传播，忽略了不同去噪步骤的阶段特定角色，导致优化信号不准确或不兼容。

Method: OTCA包含两个关键组件：1) 轨迹级信用分解 - 估计不同去噪步骤的相对重要性；2) 多目标信用分配 - 在整个去噪过程中自适应地加权和组合多个奖励信号。通过联合建模时间信用和目标级信用，将粗粒度奖励监督转换为结构化、时间步感知的训练信号。

Result: 大量实验表明，OTCA在图像和视频生成质量方面均能持续改进，在各种评估指标上都表现出优越性能。

Conclusion: OTCA通过细粒度的轨迹信用分配，解决了GRPO训练中的关键限制，为扩散模型的强化学习后训练提供了更匹配其迭代生成特性的结构化框架，显著提升了生成质量。
...
TL;DR: Allo{SR}²提出了一种通过同构生成流校正一步超分辨率轨迹的新框架，解决了现有方法中的"先验崩溃"和轨迹漂移问题，在一步Real-SR中实现了最先进的性能。


<details>
  <summary>Details</summary>
Motivation: 现有基于大规模扩散和流模型的Real-SR方法在有限LR-HR对上进行微调时容易出现"先验崩溃"问题，模型会牺牲其固有的生成丰富性来过度拟合特定的训练退化。这一问题在一步生成中尤为严重，因为缺乏多步细化会导致显著的轨迹漂移和伪影生成。

Method: 1. SNR引导轨迹初始化：通过将LR潜在特征的退化水平与预训练流的最佳锚定时间步对齐，建立物理基础的起始状态。2. 流锚定轨迹一致性(FATC)：在中间状态实施速度级监督，确保一步推理的稳定、无曲率路径。3. 同构轨迹匹配(ATM)：一种自对抗对齐策略，在统一向量场中最小化SR流和生成流之间的分布差异。

Result: 在合成和真实世界基准测试上的广泛实验表明，Allo{SR}²在一步Real-SR中实现了最先进的性能，在恢复保真度和生成真实性之间提供了优越的平衡，同时保持了极高的效率。

Conclusion: Allo{SR}²通过同构生成流校正一步超分辨率轨迹，有效解决了先验崩溃和轨迹漂移问题，为一步Real-SR提供了高性能、高效率的解决方案。
...
TL;DR: 本文提出Deepening Reasoning MMSearchAgent框架，通过从整个批次的轨迹中提取优势信号和使用差异化高斯奖励，解决多模态智能体过早交互崩溃问题，在FVQA-test上超越MMSearch-R1 8.4%


<details>
  <summary>Details</summary>
Motivation: 多模态智能体在使用外部工具处理复杂任务时，经常出现过早的交互崩溃。主要原因有两点：1）终端奖励通常附加在最后一个token上，导致优势信号无法区分包含探索行为的轨迹；2）过度冗余的上下文阻碍智能体吸收有用的反馈信息。
...
Result: 大量实验表明，该方法实现了最先进的性能，在FVQA-test上比MMSearch-R1高出8.4%。

Conclusion: 提出的Deepening Reasoning MMSearchAgent框架有效解决了多模态智能体过早交互崩溃的问题，通过从完整轨迹提取优势信号和动态奖励校准，显著提升了智能体的推理能力和性能。
...
TL;DR: 该论文提出了一种基于MCP惩罚和重加权ℓ₁正则化的盲图像去模糊方法，通过增强稀疏性并减少估计偏差来更好地恢复图像细节和纹理。


<details>
  <summary>Details</summary>
Motivation: 盲图像去模糊是一个具有挑战性的图像恢复问题，需要同时估计点扩散函数和潜在清晰图像。虽然ℓ₀范数正则化在TV框架中能促进稀疏性并保持边缘，但其非凸性和计算困难限制了实际应用。

Method: 采用极小极大凹惩罚(MCP)来增强稀疏性并更好地逼近ℓ₀范数，同时结合重加权ℓ₁范数正则化以减少估计偏差并改善图像细节和纹理的保持。开发了数值算法来求解所得优化问题。

Result: 通过对多个测试图像的实验评估，证明了所提出方法的有效性，能够更好地恢复图像细节和纹理。

Conclusion: 提出的基于MCP惩罚和重加权ℓ₁正则化的方法克服了ℓ₀范数的计算困难，在盲图像去模糊任务中实现了更好的性能，特别是在保持图像细节和纹理方面。
...
TL;DR: 提出基于Transformer的多视角人群跟踪模型MVTrackTrans，并构建了两个大规模真实场景数据集MVCrowdTrack和CityTrack，解决了现有方法在小规模数据集上评估的局限性。


<details>
  <summary>Details</summary>
Motivation: 现有基于CNN的多视角人群跟踪方法主要在Wildtrack和MultiviewX等小规模数据集上评估，这些数据集场景小、帧数少，难以适应真实世界中复杂的大场景和遮挡情况，需要更实用的数据集和模型。
...
<details>
  <summary>Details</summary>
Motivation: 开发适用于本地和边缘部署的轻量级视觉语言模型，特别针对日语环境，满足工厂任务分析和基础设施异常检测等实际应用需求。

Method: 开发了8B和2B两种变体模型，构建了大规模合成数据生成管道和全面的日语训练评估资源，专注于视觉问答和视觉定位核心能力。

Result: 在日语和英语基准测试中优于同类开源模型，在JA-VG-VQA-500上获得61.5 ROUGE-L，在日语Ref-L4上达到85.2%准确率。在工厂任务分析中零样本准确率达53.9%，在电厂数据微调后异常检测F1分数从39.7提升至64.9。

Conclusion: PLaMo 2.1-VL是一个高效的轻量级视觉语言模型，特别适合日语环境下的边缘部署，在实际工业应用场景中表现出强大的性能。
...
TL;DR: RAFT-MSF++是一个自监督多帧单目场景流估计框架，通过循环融合时序特征联合估计深度和场景流，在KITTI基准上达到24.14% SF-all，比基线提升30.99%，在遮挡区域表现更鲁棒。
...
Method: 提出自监督多帧框架RAFT-MSF++，核心是几何-运动特征(GMF)，紧凑编码耦合的运动和几何线索，通过迭代更新进行有效时序推理。引入相对位置注意力注入空间先验，以及遮挡正则化模块从可见区域传播可靠运动，确保在遮挡区域的鲁棒性。

Result: 在KITTI场景流基准测试中达到24.14% SF-all，比基线提升30.99%，在遮挡区域表现出更好的鲁棒性。

Conclusion: RAFT-MSF++通过多帧时序融合和创新的GMF特征表示，显著提升了单目场景流估计的性能，特别是在遮挡区域，为3D运动估计提供了更鲁棒的解决方案。
...
TL;DR: 提出一个针对医学图像（乳腺X光片）的Vision Transformer改进框架，通过ROI token缩减、对比学习和DINOv2预训练来解决ViT在医学图像诊断中的局限性。


<details>
  <summary>Details</summary>
Motivation: Vision Transformers在计算机视觉任务中表现出色，但在计算机辅助诊断中性能有限。针对乳腺X光片乳腺癌检测，发现两个主要问题：1）医学图像分辨率高、异常区域小，导致token数量过多，softmax注意力难以定位相关区域；2）医学图像分类本质上是细粒度任务，类间差异小、类内差异大，标准交叉熵训练不足。

Method: 提出包含三个关键组件的框架：1）使用目标检测模型进行基于感兴趣区域（ROI）的token缩减，引导注意力；2）在选定ROI之间进行对比学习，通过hard-negative训练增强细粒度区分能力；3）使用DINOv2预训练的ViT，捕捉定位感知的细粒度特征，而非全局CLIP表示。

Result: 在公共乳腺X光片数据集上的实验表明，该方法优于现有基线方法，证明了其有效性以及在大型乳腺癌筛查中的潜在临床实用性。

Conclusion: 该框架通过解决ViT在医学图像分析中的特定挑战，显著提升了乳腺癌检测性能，为大规模医学图像筛查提供了有效的解决方案。
...
TL;DR: 本文研究了T恤攻击对生物识别系统的威胁，并提出了一种基于空间一致性检查的检测方法


<details>
  <summary>Details</summary>
Motivation: 人脸识别系统常用于生物识别认证，但容易受到呈现攻击的威胁。现有攻击检测方法在应对新型攻击类型时存在泛化挑战，特别是针对T恤攻击这类新型攻击手段

Method: 提出基于空间一致性检查的检测方法，结合最先进的人脸检测器和人体检测器，通过分析检测到的人脸和人体空间位置来可靠检测T恤攻击

Result: 使用TFPA数据库的1,608个T恤攻击和152个真实呈现进行综合评估，证明这类攻击确实能够威胁人脸识别系统的安全性

Conclusion: T恤攻击能够威胁人脸识别系统的安全性，而提出的基于空间一致性检查的检测方法能够有效检测这类攻击，为解决新型呈现攻击检测的泛化问题提供了有效方案
...
TL;DR: 该研究提出并评估了一个基于EEG的驾驶员意图预测框架，在真实电动汽车中使用同步多传感器平台，TSCeption架构在32次驾驶会话中取得了最高准确率（0.907）和Macro-F1分数（0.901），证明了在真实道路条件下早期稳定解码驾驶员意图的可行性。


<details>
  <summary>Details</summary>
Motivation: 从神经生理信号预测驾驶员意图为增强高级驾驶辅助系统的主动安全性提供了有前景的途径，但由于EEG信号的非平稳性和认知-运动准备的复杂性，在真实驾驶环境中仍具有挑战性。

Method: 研究提出了一个基于EEG的驾驶员意图预测框架，使用集成到真实电动汽车中的同步多传感器平台。收集了32次驾驶会话的真实道路数据集，在一致的实验条件下评估了12种深度学习架构。

Result: 在评估的架构中，TSCeption取得了最高的平均准确率（0.907）和Macro-F1分数（0.901）。该框架表现出强的时间稳定性，在动作执行前1000毫秒内保持稳健的解码性能。分析还显示最小化EEG预处理优于伪迹处理流程，预测性能在400-600毫秒间隔达到峰值。

Conclusion: 这些发现支持了在真实道路条件下早期稳定EEG驾驶员意图解码的可行性，为基于神经信号的主动安全系统提供了实证基础。
...
<details>
  <summary>Details</summary>
Motivation: 自动驾驶中多模态3D全景分割面临域偏移挑战，现有方法在单模态退化时表现脆弱，且传统伪标签策略导致分割不完整

Method: 提出PanDA框架：1) 非对称多模态增强，选择性丢弃区域模拟域偏移；2) 双专家伪标签细化模块，从2D和3D模态提取域不变先验

Result: 在时间、天气、位置和传感器等多种域偏移场景下，显著超越现有3D语义分割的UDA基线方法

Conclusion: PanDA是首个专门为多模态3D全景分割设计的UDA框架，通过增强鲁棒性和伪标签完整性，有效解决了自动驾驶中的域适应问题
...
Method: 提出训练免费的扩散框架，包含：1）潜在均值偏移操作转移源域和目标域的辐射特征；2）时序潜在融合策略平衡协调性和内容保持；3）轻量级协调分类器自动选择最佳结果。

Result: 实验表明该方法能有效执行卫星图像合成，展示了在可扩展遥感合成和模拟任务中的强大潜力。构建了包含500个配对样本的RSIC-H基准数据集。

Conclusion: HarmoniDiff-RS为卫星图像协调提供了有效的训练免费解决方案，在遥感合成应用中具有重要价值，代码已开源。
...
TL;DR: VecHeart：一个统一的四腔心脏结构整体重建与生成框架，通过混合部件Transformer和模态对齐策略，能够从部分、稀疏或噪声观察中推断完整心脏结构，并扩展到3D+时间动态网格序列生成。
...
Method: 提出VecHeart框架，核心是混合部件Transformer，利用部件特定的可学习查询和交错注意力机制来捕捉复杂的腔室间依赖关系。同时提出解剖补全掩蔽和模态对齐策略，使模型能够从部分、稀疏或噪声观察中推断完整四腔结构。

Result: 实验表明该方法达到了最先进的性能，在各种挑战性场景下保持高保真重建。VecHeart还能无缝扩展到3D+时间动态网格序列生成，显示出卓越的多功能性。

Conclusion: VecHeart是一个统一的四腔心脏结构整体重建与生成框架，通过创新的混合部件Transformer和模态对齐策略，有效解决了当前方法的局限性，在心脏解剖建模方面表现出色。
...
Conclusion: HP-Edit框架通过结合人类偏好数据和自动评估器，有效解决了扩散模型在真实世界编辑任务中与人类偏好对齐的问题，为RLHF在图像编辑领域的应用提供了可行方案。
...
TL;DR: GOLD-BEV是一个从自车传感器学习密集鸟瞰图语义环境映射的框架，使用时间同步的航拍图像作为训练监督，无需人工标注BEV标签，并能扩展到航拍覆盖范围之外。


<details>
  <summary>Details</summary>
Motivation: 需要以几何一致、场景中心的表示理解道路场景，这对于规划和地图构建至关重要。传统方法依赖人工标注BEV标签，存在模糊性和高成本问题。

Method: 使用时间同步的航拍图像作为监督，通过BEV对齐的航拍裁剪提供直观目标空间。采用领域适应的航拍教师生成BEV伪标签，联合训练BEV分割和可选的伪航拍BEV重建。从自车传感器学习合成伪航拍BEV图像，支持轻量级人工标注和不确定性感知的伪标注。

Result: 实现了从自车传感器学习密集BEV语义环境映射，包括动态智能体。避免了传统BEV标注的模糊性，能够监督移动交通参与者，缓解了非同步航拍源的时间不一致性问题。

Conclusion: GOLD-BEV框架通过时间同步的航拍监督，实现了高效、可扩展的BEV语义环境映射学习，能够扩展到航拍覆盖范围之外，为自动驾驶的规划和地图构建提供了有力工具。
...
<details>
  <summary>Details</summary>
Motivation: 大型视觉语言模型经常出现物体幻觉问题，即在图像描述中生成实际不存在的物体。这个问题在医疗影像和自动驾驶等关键应用中尤为严重，研究表明幻觉可能源于语言先验偏差。

Method: 提出视觉对比编辑方法，通过分析模型对对比性视觉扰动的响应来识别幻觉倾向。使用奇异值分解分解模型激活模式，隔离幻觉子空间，并应用针对性参数编辑来减弱其影响。

Result: 实验结果表明，VCE在多个基准测试中有效减少了物体幻觉，同时保持了模型原有的计算效率。

Conclusion: VCE是一种无需标注的后处理方法，可扩展且适用于资源受限环境，为解决大型视觉语言模型的物体幻觉问题提供了实用解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有基于CLIP的3D物体检索方法缺乏细粒度特征表示能力，而DINO编码器在细粒度特征提取方面表现更好，但直接应用会导致对已知类别的过拟合
...
Conclusion: DEC框架通过结合DINO的细粒度特征提取能力、动态多视图整合策略和虚拟特征合成技术，有效解决了开放集3D物体检索中的过拟合问题，提升了模型泛化能力
...
TL;DR: LoViF挑战赛综述：针对真实世界多退化图像修复的竞赛，涵盖模糊、低光、雾霾、雨雪等多种退化类型，建立统一基准评估模型鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 推动真实世界多退化图像修复研究，建立统一评估框架，促进模型在多种退化条件下的鲁棒性和泛化能力发展。

Method: 组织LoViF挑战赛，提供统一基准数据集，吸引124名参赛者，收集9个有效最终提交方案，对提交方法进行详细分析。

Result: 挑战赛成功吸引了广泛参与，获得了9个有效解决方案，为真实世界多退化图像修复建立了重要基准，推动了该领域进展。

Conclusion: LoViF挑战赛为真实世界多退化图像修复研究提供了重要推动，建立了评估基准，识别了有效方法，为未来研究奠定了基础。
...
TL;DR: 提出TS-Attn注意力机制，解决多动作时序描述生成视频时的时态对齐和注意力耦合问题，无需训练即可提升现有模型性能


<details>
  <summary>Details</summary>
Motivation: 现有方法在处理包含多个顺序动作的复杂时序描述时面临两难：使用多个短提示会损害时间一致性，而使用单个复杂提示会降低提示跟随能力。主要原因是时态不对齐和运动相关视觉对象与文本条件之间的冲突注意力耦合。

Method: 提出一种无需训练的新型注意力机制——时态可分离注意力（TS-Attn），动态重新分配注意力分布，确保多事件场景中的时间感知和全局一致性。该方法可无缝集成到各种预训练文本到视频模型中。

Result: 在Wan2.1-T2V-14B和Wan2.2-T2V-A14B模型上，StoryEval-Bench分数分别提升33.5%和16.4%，推理时间仅增加2%。同时支持跨模型的即插即用多事件图像到视频生成。

Conclusion: TS-Attn机制有效解决了复杂时序描述生成视频时的时态对齐和注意力耦合问题，显著提升了多事件视频生成的质量和一致性，具有很好的通用性和实用性。
...
TL;DR: 提出一种基于精灵的图像分解方法，在CLEVR基准上与最先进的无监督类感知图像分割方法表现相当，能线性扩展对象数量，明确识别对象类别，并以高度可解释的方式建模图像。


<details>
  <summary>Details</summary>
Motivation: 尽管基础模型在图像分割方面取得稳步进展，扩散算法能生成更逼真的图像，但在图像集合中识别重复模式这一看似简单的问题仍然非常开放。精灵基图像分解模型在聚类和图像分解方面显示出潜力，且具有高可解释性，但存在需要针对特定数据集定制、难以扩展到多对象图像等问题。

Method: 深入研究精灵基图像分解模型的设计细节，识别其核心组件，在聚类基准上进行广泛分析。基于此分析提出一种深度精灵基图像分解方法，该方法能线性扩展对象数量，明确识别对象类别，并以高度可解释的方式完全建模图像。

Result: 在标准CLEVR基准测试中，该方法与最先进的无监督类感知图像分割方法表现相当，能够线性扩展处理更多对象，明确识别对象类别，并以完全可解释的方式建模图像。

Conclusion: 提出的深度精灵基图像分解方法解决了现有模型需要定制化、难以扩展的问题，在保持高可解释性的同时，在性能上达到了与最先进方法相当的水平，为图像集合中重复模式的识别提供了有效的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 评估专业机器学习模型和新兴多模态大语言模型在视觉政治传播分析中的能力，特别是在政治选举活动中的图像内容分析。
...
Result: GPT-4o表现最优，在故事中的人脸识别方面达到0.89的宏观F1分数，人物计数方面达到0.86的宏观F1分数，显著优于其他模型。
...
TL;DR: 该研究评估了直方图匹配技术在葡萄藤病害检测中的应用，通过双阶段集成（预处理归一化和数据增强）显著提升了深度学习模型在真实冠层图像上的鲁棒性。
...
Method: 提出直方图匹配的双阶段集成：1) 作为预处理步骤进行图像归一化；2) 作为数据增强技术引入受控的训练变异性。使用1,469张RGB图像（包括同质叶片聚焦和异质冠层样本）训练ResNet-18模型。

Result: 这种组合显著提升了模型在真实冠层图像上的鲁棒性。叶片聚焦样本显示边际增益，而冠层子集显著改善，表明平衡归一化与直方图多样化能有效弥合由非受控光照引起的域差距。

Conclusion: 直方图匹配的双阶段集成（预处理归一化+数据增强）是缓解光照变化对植物病害检测深度学习模型影响的有效方法，特别是在处理真实冠层图像时。
...
TL;DR: Paparazzo提出了一种主动映射移动物体的新方法，通过预测目标轨迹和选择信息量最大的视点来规划路径，无需学习即可实现动态场景的3D重建。


<details>
  <summary>Details</summary>
Motivation: 当前3D映射流程通常假设静态环境，无法准确捕捉和重建移动物体，限制了动态场景的理解能力。

Method: 提出Paparazzo方法，这是一种无需学习的解决方案，能够鲁棒地预测目标轨迹，识别信息量最大的观察视点，并规划自身路径。

Result: 通过大量实验证明，Paparazzo相比多个强基线方法，显著提高了3D重建的完整性和准确性。

Conclusion: 该方法在动态场景理解方面迈出了重要一步，并为这一新任务提供了全面的基准测试。
...
<details>
  <summary>Details</summary>
Motivation: 自我中心助手需要个性化服务来适应不同用户的习惯、偏好和日常行为，但如何有效整合长期用户数据进行个性化仍然是一个关键挑战。

Method: 系统包含两个核心组件：1）基于过去观察构建的图结构交互记忆，捕捉交互事件和实体之间的时间和语义关系；2）专门的个性化学习任务，将个性化问题表述为预测问题，从用户历史行为图中预测未来可能的交互。

Result: 大量实验证明了EgoSelf作为个性化自我中心助手的有效性，代码已开源。

Conclusion: EgoSelf通过图结构记忆和个性化预测任务，成功解决了长期用户数据整合的挑战，为有效的个性化自我中心助手提供了解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有语言驱动3D高斯泼溅编辑方法主要关注提升视角一致性，但未能显式解决编辑的2D证据与3D高斯之间的语义对应关系这一根本问题，导致编辑精度受限。

Method: 将语言驱动3DGS编辑建模为多视角不平衡语义传输问题：1) 建立可见高斯与视角特定编辑原型之间的对应关系；2) 恢复跨视角共享的规范3D编辑场以指导统一的3D外观更新；3) 使用传输残差抑制非目标区域的错误编辑。

Result: 定性和定量实验表明，与现有专注于增强视角一致性的3D编辑方法相比，TransSplat在局部编辑精度和结构一致性方面取得了优越性能。

Conclusion: 通过显式建模2D编辑证据与3D高斯之间的语义对应关系，TransSplat解决了现有方法忽略的根本问题，实现了更精确的语言驱动3D场景编辑。
...
<details>
  <summary>Details</summary>
Motivation: 传统摄影图像编辑需要用户具备足够的美学理解来提供适当的图像质量和相机参数调整指令，但这种方法依赖于明确的人类美学意图指令，而这些指令往往模糊、不完整或非专业用户难以提供。

Method: 1. 提出紧密耦合的推理到生成过程：Image Critic模块进行图像质量理解并识别缺陷，Photographic Artist模块实现针对性编辑；2. 采用多阶段训练流程：基础预训练建立基本美学理解和编辑能力，推理引导的多编辑监督适应，协调的推理到生成强化学习；3. 强调照片级真实图像生成，同时支持图像修复和润色任务。

Result: 实验表明SmartPhotoCrafter在自动摄影增强任务上优于现有生成模型，实现了照片级真实的结果，同时对润色指令表现出更高的色调敏感性。

Conclusion: SmartPhotoCrafter通过将图像编辑建模为推理到生成的紧密耦合过程，实现了无需明确人类指令的自动摄影图像编辑，在图像修复和润色任务上均表现出色，为摄影图像编辑提供了新的自动化解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 传统细粒度高分辨率遥感制图依赖局部视觉特征，导致跨域泛化能力受限和大尺度地物预测碎片化。现有全局地理空间基础模型与高分辨率视觉特征直接融合时，由于严重的语义-空间鸿沟，常引发特征干扰和空间结构退化。

Method: 提出结构-语义解耦调制（SSDM）框架：1）结构先验调制分支：将全局表示的宏观感受野先验注入高分辨率编码器的自注意力模块，用整体结构约束指导局部特征提取；2）全局语义注入分支：将整体上下文与深层高分辨率特征空间显式对齐，通过跨模态集成补充全局语义。

Result: 实验表明，该方法相比现有跨模态融合方法达到最先进性能。通过释放全局嵌入的潜力，SSDM在不同场景下持续提升高分辨率制图精度，为将地理空间基础模型集成到高分辨率视觉任务提供了通用有效范式。

Conclusion: SSDM框架通过解耦全局地理空间表示的双路径注入机制，有效解决了高分辨率遥感制图中的特征干扰和空间结构退化问题，显著提升了跨域泛化能力和制图精度，为地理空间基础模型与高分辨率视觉任务的融合提供了创新解决方案。
...
TL;DR: 本文介绍了PC2Model基准数据集，用于点云到模型配准任务的训练和评估，结合模拟和真实世界数据以解决实际扫描中的稀疏性、噪声和遮挡等问题。


<details>
  <summary>Details</summary>
Motivation: 随着点云获取技术（如LiDAR和结构光扫描）的普及和深度学习的发展，研究重点逐渐转向下游任务，特别是点云到模型配准。然而，数据驱动方法在处理真实世界扫描中的稀疏性、噪声、杂乱和遮挡时表现不佳，限制了其性能。

Method: 本文提出了PC2Model基准数据集，采用混合设计，结合模拟点云（部分情况下包含真实世界扫描）及其对应的3D模型。模拟数据提供精确的地面真值和受控条件，而真实世界数据引入传感器和环境伪影。

Result: PC2Model基准数据集已公开发布，支持经典方法和数据驱动方法的训练和评估，能够跨领域进行鲁棒训练和评估，并支持从模拟到真实世界场景的模型可迁移性系统分析。

Conclusion: PC2Model基准数据集通过混合设计解决了点云配准中的实际挑战，为点云到模型配准任务提供了全面的训练和评估平台，有助于推动该领域的研究进展。
...
TL;DR: GRAFT是一种快速前馈3D人-场景交互重建方法，通过预测交互梯度迭代优化人体网格，在保持优化方法准确性的同时大幅提升速度（约50倍）。


<details>
  <summary>Details</summary>
Motivation: 现有单图像3D人-场景交互重建方法存在权衡：基于优化的方法准确但慢（约20秒），前馈方法快但缺乏显式交互推理，导致漂浮和穿透伪影。

Method: GRAFT通过学习的人-场景交互先验预测交互梯度（校正参数更新），使用几何探针编码交互状态，通过轻量级Transformer递归更新人体网格并重新探测场景。

Result: GRAFT相比最先进前馈方法提升交互质量达113%，匹配基于优化的交互质量但运行时间降低约50倍，在64.8%的三方用户研究中被偏好。

Conclusion: GRAFT成功将基于几何的人-场景拟合摊销到快速前馈推理中，提供了一种可转移的即插即用人-场景交互先验，显著改善了重建质量和效率。
...
TL;DR: 该论文提出了一种混合生成框架，将栅格化设计图像分解为可编辑的文本、背景和贴纸图层，实现从栅格到图层的图形设计解析。
...
Method: 提出混合生成框架：使用视觉语言模型解析文本区域为文本渲染协议；采用支持RGBA的多分支扩散架构生成背景和贴纸图层；引入ParserReward并结合Group Relative Policy Optimization来对齐人类设计偏好。

Result: 在Parser-40K和Crello两个挑战性数据集上的实验表明，该方法在所有指标上平均提升23.7%，优于现有方法。

Conclusion: 该混合生成框架能够有效将栅格化设计图像分解为可编辑图层，解决了现有方法误差累积和可控性有限的问题，为图形设计解析提供了更优的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 当前扩散模型在合成人-物交互视频时存在两个主要问题：1）手部和面部等敏感区域的结构稳定性不足；2）物理接触的合理性差（如手-物穿透）。这些问题限制了在电子商务、数字广告和虚拟营销等领域的实际应用价值。

Method: CoInteract基于扩散变换器架构，引入两个互补设计：1）人类感知的专家混合，通过空间监督路由将令牌分配到轻量级的区域专业化专家，以最小参数开销提高细粒度结构保真度；2）空间结构协同生成，采用双流训练范式联合建模RGB外观流和辅助的HOI结构流，注入交互几何先验。

Result: 实验结果表明，CoInteract在结构稳定性、逻辑一致性和交互真实性方面显著优于现有方法。

Conclusion: CoInteract通过创新的架构设计有效解决了人-物交互视频合成中的结构稳定性和物理接触合理性问题，为相关应用领域提供了高质量的解决方案。
...
TL;DR: CoCo-SAM3通过解耦推理为类内增强和类间竞争，解决SAM3在多类开放词汇语义分割中证据尺度不统一和类间冲突的问题，无需额外训练即可提升性能。


<details>
  <summary>Details</summary>
Motivation: SAM3在多类开放词汇场景中，不同类别提示独立生成的掩码缺乏统一且可比较的证据尺度，导致覆盖重叠和不稳定竞争。此外，同一概念的同义表达会激活不一致的语义和空间证据，引发类内漂移，加剧类间冲突并损害推理稳定性。

Method: 提出CoCo-SAM3，将推理显式解耦为类内增强和类间竞争两个阶段。首先对齐和聚合来自同义提示的证据以增强概念一致性，然后在统一可比较的尺度上进行类间竞争，实现所有候选类别之间的直接像素级比较。

Result: 无需任何额外训练，CoCo-SAM3在八个开放词汇语义分割基准上实现了一致的性能提升。

Conclusion: CoCo-SAM3通过解耦推理过程，有效缓解了多类开放词汇语义分割中的类间冲突问题，提高了推理稳定性，为开放词汇分割提供了更可靠的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有医学图像分割方法主要基于扩散模型，存在计算开销大（需要迭代采样）和受限于UNet参数化的问题。流匹配作为一种连续时间传输映射的学习框架，为医学图像分割提供了更高效的理论基础。

Method: 提出条件流匹配框架，将医学图像分割建模为学习从简单先验分布到目标分割分布的时间依赖向量场。开发双条件机制：1) 双分支空间注意力模块注入多尺度结构信息；2) 频率感知注意力模块通过差异感知融合和时间依赖调制建模空间与频谱表示的跨域交互。
...
TL;DR: MMControl是一个多模态控制框架，用于联合音视频生成，允许用户通过视觉和声学控制信号（如图像、音频、深度图、姿态序列）来精细控制生成内容。


<details>
  <summary>Details</summary>
Motivation: 现有可控生成框架通常仅限于视频控制，限制了综合可控性并导致跨模态对齐不理想。需要一种能够同时控制视觉和声学内容的联合音视频生成方法。

Method: 提出MMControl框架，采用双流条件注入机制，将视觉和声学控制信号通过旁路分支注入到联合音视频扩散变换器中，并引入模态特定指导缩放，允许在推理时独立动态调整每个条件的影响强度。

Result: 实验表明MMControl能够在联合音视频生成中实现对角色身份、声音音色、身体姿态和场景布局的细粒度、可组合控制。

Conclusion: MMControl通过多模态控制机制显著提升了联合音视频生成的可控性和跨模态对齐质量，为音视频内容创作提供了更灵活的工具。
...
TL;DR: IR-Flow是一个基于Rectified Flow的图像修复统一框架，通过构建多级数据分布流和累积速度场，在判别式和生成式方法间架起桥梁，实现快速推理和良好的失真-感知平衡。


<details>
  <summary>Details</summary>
Motivation: 传统图像修复方法存在两个主要问题：单步判别式映射通过期望学习缺乏精细细节，而生成式方法则存在多步采样效率低下和噪声残差耦合的问题。需要一种能结合两者优势的统一框架。
...
Conclusion: IR-Flow通过在退化图像和干净图像域之间建立线性传输流，不仅实现了快速推理，还提高了对分布外退化的适应性，为图像修复提供了一个统一高效的解决方案。
...
TL;DR: StepSTEM是一个研究生级别的多模态基准测试，包含283个跨数学、物理、化学、生物和工程的问题，用于细粒度评估多模态大语言模型的跨模态推理能力。
...
Conclusion: StepSTEM为多模态推理的细粒度评估提供了一个有价值的基准测试，揭示了当前模型在真正的跨模态STEM推理方面仍有显著改进空间。
...
TL;DR: 提出基于规范面部点预测的统一4D面部重建方法，通过将像素映射到共享规范空间，将动态重建转化为规范重建问题，实现高精度深度估计和时间一致的重建。
...
Method: 提出规范面部点预测表示法，将每个像素映射到共享规范空间中的规范化面部坐标。使用基于Transformer的模型联合预测深度和规范面部坐标，通过多视角几何数据训练，将非刚性变形映射到规范空间。

Result: 在图像和视频基准测试中表现出最先进的性能：对应误差降低约3倍，推理速度更快，深度精度提高16%。实现了准确深度估计、时间稳定重建、密集3D几何和鲁棒面部点跟踪。

Conclusion: 规范面部点预测是统一前馈4D面部重建的有效基础，能够同时解决动态重建中的多个挑战，实现高质量的时间一致几何和可靠对应关系。
...
<details>
  <summary>Details</summary>
Motivation: 现有视觉-语言-动作模型在动作生成时存在高延迟问题，且鲁棒性有限，特别是在长尾场景中表现不佳。需要一种既能利用世界知识和推理能力，又能高效生成动作并具有良好鲁棒性的自动驾驶解决方案。

Method: 1. 提出SpanVLA框架，集成自回归推理和流匹配动作专家；2. 通过高效桥接机制，利用VLM的视觉和推理指导，基于历史轨迹初始化条件化流匹配策略来规划未来轨迹；3. 提出基于GRPO的后训练方法，让模型不仅能从正样本学习，还能学习避免典型负行为并学习恢复行为；4. 引入mReasoning数据集，专注于复杂推理场景和负样本-恢复样本。

Result: 在NAVSIM（v1和v2）数据集上的大量实验表明，SpanVLA模型具有竞争性性能。定性结果展示了模型在各种场景下的规划性能和鲁棒性，推理时间显著减少。

Conclusion: SpanVLA通过结合自回归推理和流匹配动作专家，有效解决了现有VLA模型的高延迟和鲁棒性问题，为自动驾驶提供了一个高效且鲁棒的端到端解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法在平衡推理效率、患者特异性保真度和分布级合理性方面面临挑战，特别是在高维3D医学成像中。

Method: GDM通过吸引-排斥漂移最小化生成器前推与目标分布之间的差异；构建多级特征库支持可靠的亲和度估计和漂移场计算；采用共享输出空间的梯度协调策略优化竞争目标间的平衡。

Result: 在MRI-to-CT合成和稀疏视图CT重建任务中，GDM优于多种基线方法（包括GAN、流匹配、SDE生成模型和监督回归方法），在解剖保真度、定量可靠性、感知真实性和推理效率之间取得更好平衡。

Conclusion: GDM为条件3D医学图像生成提供了一个实用有效的框架。
...
TL;DR: CityRAG是一个视频生成模型，利用地理注册数据作为上下文，生成3D一致、可导航的环境，能够模拟真实地点的空间布局，支持长时间视频生成并保持天气、光照条件的一致性。
...
Method: CityRAG利用大型地理注册数据语料库作为上下文，将生成过程锚定到物理场景，同时保持对复杂运动和外观变化的先验学习。模型使用时间未对齐的训练数据，学习从底层场景中语义解耦瞬态属性。

Result: 实验表明CityRAG能够生成连贯的分钟级物理基础视频序列，在数千帧中保持天气和光照条件，实现循环闭合，并能导航复杂轨迹以重建真实世界地理环境。

Conclusion: CityRAG通过结合地理注册数据和视频生成能力，实现了对真实世界环境的3D一致、可导航模拟，为自动驾驶和机器人仿真等应用提供了重要工具。
...
TL;DR: AnyRecon是一个用于稀疏视图3D重建的可扩展框架，支持任意无序输入，通过持久全局场景记忆和几何感知条件策略实现大规模场景重建。


<details>
  <summary>Details</summary>
Motivation: 稀疏视图3D重建对于从随意拍摄中建模场景至关重要，但现有基于扩散的方法通常只基于1-2帧进行条件化，限制了几何一致性，难以扩展到大规模或多样化场景。

Method: 1) 构建持久全局场景记忆，通过前置捕获视图缓存支持长范围条件化；2) 引入几何感知条件策略，通过显式3D几何记忆和几何驱动的捕获视图检索耦合生成与重建；3) 结合4步扩散蒸馏和上下文窗口稀疏注意力降低计算复杂度。

Result: 实验证明该方法在非规则输入、大视角差距和长轨迹情况下都能实现鲁棒且可扩展的重建，优于现有方法。
...
TL;DR: 本文提出了一种基于根节点状态图表示和迭代波束搜索的框架，用于解决具有灵活间隙约束的变间隙最长公共子序列问题，在合成实例上展示了优于基线方法的性能。


<details>
  <summary>Details</summary>
Motivation: 变间隙最长公共子序列问题是经典LCS问题的推广，在分子序列比较和时间序列分析中有重要应用，但现有研究缺乏全面的计算分析。

Method: 采用基于根节点的状态图表示，将状态空间分解为多个根状态子图；使用迭代波束搜索策略动态维护有希望的候选根节点池；结合LCS文献中的启发式方法提升搜索质量。

Result: 在包含320个合成实例（最多10个输入序列，最多500个字符）的测试中，所设计方法在可比运行时间内比基线波束搜索表现出更强的鲁棒性。

Conclusion: 这是对VGLCS问题的首次全面计算研究，提出的迭代波束搜索框架能有效处理状态空间组合爆炸问题，为分子序列比较和时间序列分析等应用提供了实用解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 用户通常通过单一输出来评估语言模型，但这隐藏了输出分布的结构特征（如模态、边缘情况、对提示微小变化的敏感性），导致用户在开放式任务中基于个别案例过度泛化。通过与13名LM研究人员的研究发现，当前工作流程在处理随机性时存在不足。

Method: 开发了GROVE交互式可视化工具，将多个LM生成表示为文本图中的重叠路径，揭示共享结构、分支点和聚类，同时保留对原始输出的访问。通过三个众包用户研究（共131名参与者）评估该工具在不同分布任务中的效果。

Result: 研究结果支持混合工作流程：图摘要改进了对结构判断（如评估多样性），而直接输出检查在处理细节导向问题时仍然更有效。

Conclusion: GROVE通过可视化语言模型输出的分布结构，帮助用户更好地理解和评估模型行为，特别是在需要理解输出多样性和结构特征的场景中，但需要与直接输出检查相结合形成有效的工作流程。
...
<details>
  <summary>Details</summary>
Motivation: 现有RLHF方法存在关键漏洞：不完美的奖励模型可能成为单点故障，无法惩罚不安全行为。现有的红队测试主要针对策略级弱点，忽略了核心LLM和奖励模型同时失效的系统性弱点。

Method: ARES采用"安全导师"动态组合语义连贯的对抗性提示（包含主题、角色、策略、目标等结构化组件），生成相应的恶意和安全响应，同时暴露核心LLM和奖励模型的弱点。然后实施两阶段修复：先微调奖励模型以更好检测有害内容，再利用改进的奖励模型优化核心模型。
...
TL;DR: 论文提出了一种神经符号推理框架，将自然语言推理问题转化为可执行的形式表示（一阶逻辑和Narsese），并创建了NARS-Reasoning-v0.1基准数据集，通过语言结构化感知训练LLM生成推理相关的符号结构而非最终答案。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在语言生成方面表现出色，但在需要显式符号结构、多步推理和可解释不确定性的推理任务中仍不可靠。需要一种将自然语言推理转化为可执行形式表示的方法，以提高推理的可靠性和可解释性。

Method: 1. 提出神经符号框架，将自然语言推理问题翻译为一阶逻辑和Narsese形式表示；2. 创建NARS-Reasoning-v0.1基准数据集，包含自然语言问题、FOL形式、可执行Narsese程序和三个黄金标签；3. 开发从FOL到可执行Narsese的确定性编译管道；4. 提出语言结构化感知方法，训练LLM生成推理相关的符号结构而非最终答案；5. 训练并发布基于Phi-2 LoRA适配器的三标签推理分类模型。

Result: 1. 建立了完整的神经符号推理框架和基准数据集；2. 实现了从FOL到Narsese的确定性编译，确保符号目标不仅在语法上正确，而且在行为上与预期答案一致；3. 通过OpenNARS for Applications验证了保留示例的运行时执行；4. 展示了基准数据集支持监督适应的能力，训练了Phi-2 LoRA适配器用于三标签推理分类。

Conclusion: 可执行的符号生成和基于执行的验证是实现更可靠神经符号推理系统的实用路径。该框架通过将自然语言推理转化为可执行的形式表示，提高了推理的可靠性和可解释性，为神经符号推理系统的发展提供了新方向。
...
TL;DR: 首个基于依赖类型理论和Lean 4交互式定理证明的专利分析框架，将专利权利要求编码为DAG，实现形式化验证的专利分析管道


<details>
  <summary>Details</summary>
Motivation: 现有专利分析方法依赖人工专家分析（慢、不可扩展）或机器学习/NLP方法（概率性、不透明、非组合性），需要形式化验证的专利分析框架

Method: 将专利权利要求编码为Lean 4中的DAG，匹配强度作为已验证完全格的元素，通过证明正确的单调函数传播置信度分数，形式化五个IP用例的六个算法

Result: DAG覆盖核心算法完全机器验证，结构引理、覆盖核心生成器和闭合路径恒等式在Lean 4中机器验证，高层定理保持非正式证明草图但架构缓解

Conclusion: 这是首个将交互式定理证明应用于知识产权分析的框架，为专利分析提供数学正确性保证，但验证依赖于ML层提供的分数准确性
...
<details>
  <summary>Details</summary>
Motivation: 解决机器学习分类模型在训练过程中会重复犯错的问题，追求无错误的完美训练效果

Method: 提出"人工特殊智能"新概念，应用于18个MedMNIST生物医学数据集进行分类问题训练

Result: 除3个存在双标签问题的数据集外，其余15个数据集均实现了无错误的完美训练

Conclusion: 人工特殊智能概念能够有效实现机器学习分类模型的无错误训练，避免重复犯错，但在处理双标签问题时存在局限性
...
TL;DR: 研究发现现有大语言模型基准测试采用平均偏好排名，忽视了用户个性化需求，提出个性化基准测试的重要性。


<details>
  <summary>Details</summary>
Motivation: 随着大语言模型能力提升和在现实任务中的部署，评估模型与人类偏好的对齐变得重要。现有基准测试对所有用户的偏好进行平均计算，忽视了个人用户偏好，而不同用户在不同情境下有不同偏好。

Method: 使用ELO评分和Bradley-Terry系数为115个活跃Chatbot Arena用户计算个性化模型排名，分析用户查询特征（主题和写作风格）与LLM排名变化的关系，通过主题建模和风格分析研究用户异质性。

Result: 个性化LLM排名与聚合排名存在显著差异：Bradley-Terry相关性平均仅ρ=0.04（57%用户接近零或负相关），ELO评分显示中等相关性（ρ=0.43）。用户主题兴趣和沟通风格存在显著异质性，影响模型偏好。主题和风格特征的紧凑组合可有效预测用户特定模型排名。

Conclusion: 聚合基准测试无法捕捉大多数用户的个人偏好，强调开发根据个人用户偏好对LLM模型进行排名的个性化基准测试的重要性。
...
### [86] [Reasoning Structure Matters for Safety Alignment of Reasoning Models](https://arxiv.org/abs/2604.18946)
*Yeonjun In,Wonjoong Kim,Sangwu Park,Chanyoung Park*

Main category: cs.AI

TL;DR: AltTrain：通过改变大推理模型的推理结构来实现安全对齐的简单后训练方法，仅需1K训练样本的监督微调
...
Method: 提出AltTrain方法，这是一种简单有效的后训练方法，明确改变大推理模型的推理结构。该方法不需要复杂的强化学习训练或奖励设计，仅需使用轻量级的1K训练样本进行监督微调。

Result: 实验结果表明，在不同的大推理模型主干和模型规模上，AltTrain都表现出强大的安全对齐能力，同时在推理、问答、摘要和多语言设置中展现出稳健的泛化性能。

Conclusion: 通过改变推理结构可以实现有效的安全对齐，AltTrain方法既实用又具有泛化性，为大型语言模型的安全对齐提供了一种简单高效的解决方案。
...
### [87] [DW-Bench: Benchmarking LLMs on Data Warehouse Graph Topology Reasoning](https://arxiv.org/abs/2604.18964)
*Ahmed G. A. H Ahmed,C. Okan Sakar*

Main category: cs.AI

TL;DR: DW-Bench是一个评估大语言模型在数据仓库模式上进行图拓扑推理的新基准，包含1,046个自动生成的可验证正确问题，实验显示工具增强方法显著优于静态方法但在复杂组合子类型上表现停滞


<details>
  <summary>Details</summary>
Motivation: 现有基准在评估LLMs对数据仓库模式的理解能力方面存在不足，特别是缺乏对外键和数据血缘关系的综合考量，需要一个新的基准来专门评估LLMs在图拓扑推理方面的能力

Method: 创建了DW-Bench基准，包含1,046个自动生成的可验证正确问题，覆盖五个数据仓库模式，明确整合了外键和数据血缘边，支持对图拓扑推理能力的全面评估

Result: 实验结果表明，工具增强方法在DW-Bench上显著优于静态方法，但在困难的组合子类型问题上表现停滞，揭示了当前LLMs在复杂图拓扑推理方面的局限性

Conclusion: DW-Bench为评估LLMs在数据仓库模式上的图拓扑推理能力提供了有效基准，揭示了工具增强方法的优势及其在复杂组合推理方面的局限性，为未来研究指明了方向
...
TL;DR: 本文提出了一种称为"可信逻辑"的一阶逻辑系统，用于处理可信推理，即从事实或可信陈述中得出结论，无需使用概率等数值。该系统包含17条逻辑原则，定义了8种推理算法，能够正确处理所有可信推理示例。


<details>
  <summary>Details</summary>
Motivation: 现有逻辑系统在处理可信陈述（通常为真但偶尔可能为假的陈述）方面存在不足。可信推理需要从事实或可信陈述中得出结论，而不涉及概率等数值计算，但目前缺乏能够系统处理这种推理的逻辑框架。

Method: 提出了17条可信推理的逻辑原则（14条必要原则和3条理想原则），并基于这些原则定义了一个一阶逻辑系统——可信逻辑。该系统包含8种不同的推理算法，能够处理从给定可信推理情境中可能得出的不同合理结论。

Result: 可信逻辑系统满足除两条理想原则外的所有原则，并能正确推理所有可信推理示例。据作者所知，这是唯一能够实现这一目标的逻辑系统。

Conclusion: 可信逻辑为处理可信推理提供了一个系统化的逻辑框架，填补了现有逻辑系统在处理可信陈述推理方面的空白。该系统具有实际应用价值，能够处理现实世界中常见的可信推理问题。
...
<details>
  <summary>Details</summary>
Motivation: 从状态图像序列中学习提升动作模型是一个更具挑战性的设置，因为缺乏动作观察信息，需要解决预测崩溃和自增强错误问题

Method: 提出深度学习框架联合学习状态预测、动作预测和提升动作模型，引入混合整数线性规划(MILP)确保逻辑一致性，使用MILP解生成伪标签指导训练

Result: 实验表明MILP校正帮助模型逃离局部最优，收敛到全局一致解，在多个领域验证了方法的有效性

Conclusion: 提出的框架成功从状态图像序列中学习提升动作模型，MILP校正机制对确保逻辑一致性和避免局部最优至关重要
...
TL;DR: 提出两阶段方法：先对疾病标签进行监督微调，再通过GRPO优化预测准确性和格式，在三个放射科医生标注数据集上表现优于基线方法


<details>
  <summary>Details</summary>
Motivation: 从放射学报告中准确分类疾病对许多应用至关重要，但监督微调轻量级LLMs虽然提高准确性，却可能损害推理能力

Method: 两阶段方法：1）对疾病标签进行监督微调；2）使用组相对策略优化（GRPO）通过优化准确性和格式来细化预测，无需推理监督

Result: 在三个放射科医生标注数据集上，监督微调优于基线方法，GRPO进一步改善了分类性能，并增强了推理召回率和全面性

Conclusion: 提出的两阶段方法（SFT+GRPO）能有效提高放射学报告疾病分类的准确性，同时保持或增强模型的推理能力
...
TL;DR: OLLM是一种新的LLM方法，用学习到的选项集合替代传统的单token预测，通过离散潜变量索引多个合理的下一个token选项，增强生成多样性和可控性。


<details>
  <summary>Details</summary>
Motivation: 传统LLM依赖温度或采样启发式方法产生多样性，缺乏对变异的显式建模。需要一种方法能够显式建模多个合理的下一个token选项，提高可控性和鲁棒性。

Method: OLLM在预训练LLM的输出头前插入轻量级的编码器和解码器层，创建离散潜空间来参数化多个下一个token选项。下游策略可以在潜空间中选择或搜索选项，实现可控生成。

Result: 在1.7B参数骨干模型上，OLLM仅需训练1.56%的参数。相比SOTA LoRA基线51%的正确率，OLLM在最优潜选择下可达~70%正确率。潜空间策略训练更样本高效，显著减少语言切换或退化推理等错位问题。

Conclusion: 选项化的下一个token建模增强了数学推理中的可控性、鲁棒性和效率，潜空间策略学习为LLM中的强化学习提供了有前景的方向，对齐性来自模型结构而非额外的KL或手工对齐损失。
...
TL;DR: LightEdit：一种轻量级终身知识编辑框架，通过检索相关知识和解码策略抑制原始知识，实现高效稳定的知识更新
...
Conclusion: LightEdit通过结合检索选择和解码策略，解决了终身知识编辑中的稳定性和可扩展性问题，为LLM的知识更新提供了一种高效实用的解决方案。
...
TL;DR: 论文提出了两种基于经典测试理论的数据集特定QWK上限，用于评估自动作文评分系统的性能：理论上限（完美预测潜在真实分数的理想模型能达到的最大QWK）和类人上限（具有人类评分误差水平的AES模型可达到的QWK），解决了传统人类-人类QWK可能低估真实上限的问题。


<details>
  <summary>Details</summary>
Motivation: 自动作文评分系统通常使用二次加权kappa在公共基准上进行评估，但由于基准标签由人类评分者分配且不可避免地包含评分误差，因此不清楚理论上可达到的QWK是多少，以及实际部署需要达到什么水平。需要更准确的性能评估标准。

Method: 基于经典测试理论中的可靠性概念，推导出两种数据集特定的QWK上限：理论上限（完美预测潜在真实分数的理想模型能达到的最大QWK）和类人上限（具有人类评分误差水平的AES模型可达到的QWK）。这些上限可以从标准双评分者基准中估计，无需额外标注。

Result: 模拟实验验证了所提出的上限，真实基准实验表明这些上限能够澄清现代AES模型的当前性能和剩余提升空间。研究还显示，常用作上限参考的人类-人类QWK可能会低估真实上限。

Conclusion: 提出的两种QWK上限为评估自动作文评分系统性能提供了更准确的参考标准，理论上限揭示了理想性能边界，类人上限提供了实际部署目标，有助于更清晰地评估AES模型的成熟度和改进空间。
...
TL;DR: REVEAL是一个用于AI生成内容检测的框架，通过生成可解释的推理链进行分类，采用两阶段训练策略，在多个基准测试中达到最先进性能。
...
Result: REVEAL在多个基准测试中实现了最先进的性能，提供了一个强大且透明的AIGC检测解决方案。
...
TL;DR: 本文提出首个面向全双工语音系统的统一音频前端大语言模型（UAF），将语音活动检测、话轮转换检测、说话人识别、语音识别和问答等多种前端任务统一为自回归序列预测问题，显著提升了实时交互的响应延迟和打断准确性。


<details>
  <summary>Details</summary>
Motivation: 传统级联语音处理流水线存在延迟累积、信息丢失和错误传播等关键限制。虽然GPT-4o等端到端音频大语言模型统一了语音理解和生成任务，但大多数模型本质上是半双工的，且依赖语音活动检测、话轮转换检测等独立的前端组件。在开发语音助手过程中，作者发现优化语音前端与推进后端统一模型同样重要，因此需要填补这一空白。

Method: 提出统一音频前端大语言模型（UAF），将多种音频前端任务重新表述为单一的自回归序列预测问题。模型以流式固定时长音频块（如600毫秒）作为输入，利用参考音频提示在开始时锚定目标说话人，并回归生成编码语义内容和系统级状态控制（如打断信号）的离散标记。

Result: 实验表明，该模型在多个音频前端任务上实现了领先性能，并在真实交互场景中显著提升了响应延迟和打断准确性。

Conclusion: UAF模型成功填补了全双工语音系统中前端处理的关键空白，通过统一多种前端任务为单一序列预测问题，为实现无缝、响应式的人机语音交互提供了重要技术支撑。
...
TL;DR: 提出ETI方法，让LLM智能体从交互历史中推断伙伴的温暖度和能力度特质，以改善多智能体协调，在多种场景中显著提升性能


<details>
  <summary>Details</summary>
Motivation: LLM多智能体系统在复杂任务中表现良好，但容易出现协调失败，如目标漂移、错误级联和行为错位等问题，需要改进协调机制
...
Conclusion: ETI是轻量级且鲁棒的协调改进机制，首次系统证明LLM智能体能可靠地从交互历史推断他人特质，并利用结构化特质认知改善协调
...
TL;DR: 该研究区分了LLM中的信息性从众和规范性从众，发现多数LLM同时表现出这两种从众倾向，且规范性从众的目标可通过社交情境操控，揭示了LLM多智能体系统决策可能被少数恶意用户操纵的脆弱性。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在基于LLM的多智能体系统中表现出的从众偏差可能严重影响决策质量。先前研究将"从众"简单视为意见改变，本研究引入社会心理学对信息性从众和规范性从众的区分，旨在从机制层面理解LLM的从众行为。

Method: 设计了区分信息性从众（参与者追求准确判断）和规范性从众（参与者避免冲突或寻求群体接纳）的新任务，基于这些任务设置进行实验，评估了六个LLM，并分析了两类从众相关的内部向量。

Result: 在评估的六个LLM中，最多有五个表现出不仅信息性从众倾向，还有规范性从众倾向。有趣的是，通过操控社交情境的微妙方面，可以控制特定LLM规范性从众的目标对象。内部向量分析表明，虽然两种从众行为外部表现相同，但可能由不同的内部机制驱动。

Conclusion: LLM-MAS的决策可能容易受到少数恶意用户的操纵。这些结果为了解"规范"如何在LLM中实现以及如何影响群体动态提供了初步里程碑，揭示了从众行为在机制层面的复杂性。
...
TL;DR: GRASPrune是一种结构化剪枝框架，可在预训练后同时剪枝FFN通道和KV头组，通过硬掩码满足预算约束，无需微调即可获得更小的密集检查点。


<details>
  <summary>Details</summary>
Motivation: 大语言模型服务成本高昂，因为模型参数、注意力计算和KV缓存带来了显著的内存和延迟开销，需要高效的模型压缩方法。

Method: GRASPrune使用投影直通估计器学习轻量级门控分数，在保持骨干权重冻结的同时，强制硬掩码满足预算约束；剪枝后通过校准缩放因子缓解尺度不匹配问题，并将这些因子折叠到剪枝权重中。

Result: 在LLaMA-2-7B上，GRASPrune移除了50%的参数，在WikiText-2上达到12.18困惑度，同时在五个基准测试中保持竞争力的平均零样本准确率，仅需4个epoch和512个未标记校准序列。

Conclusion: GRASPrune提供了一种高效的预训练后结构化剪枝方法，无需完整模型微调即可显著减少模型参数，同时保持模型性能。
...
TL;DR: 该论文提出将长时程企业智能体决策行为分解为四个正交对齐轴：事实精度、推理连贯性、合规重构和校准弃权，以解决传统单一成功率评估无法区分不同失败模式的问题。


<details>
  <summary>Details</summary>
Motivation: 当前长时程企业智能体（如贷款审批、理赔裁决等）的评估仅报告单一任务成功率，这种评估方式混淆了不同的失败模式，无法判断智能体是否符合部署环境所需的监管标准。

Method: 提出将决策行为分解为四个正交对齐轴：事实精度（FRP）、推理连贯性（RCS）、合规重构（CRR）和校准弃权（CAR）。在LongHorizon-Bench基准测试上验证，涵盖贷款资格和保险理赔裁决任务，使用六种记忆架构进行实验。

Result: 分解框架揭示了聚合准确率无法观察到的现象：检索架构在事实精度上崩溃；模式锚定架构需要支付脚手架代价；在事实保留提示下的简单摘要成为FRP、RCS、EDA和CRR上的强基线；所有六种架构在每项案例上都做出决策，暴露了决策对齐轴的缺失。

Conclusion: 机构对齐（监管重构）和决策对齐（校准弃权）在现有对齐文献中代表性不足，但在决策离开实验室环境后变得至关重要。该框架可通过构建事实模式和校准CRR审计提示，转移到任何受监管的决策领域。
...
TL;DR: 研究发现前沿大模型在生成Lean 4证明时存在"形式化博弈"现象，即利用验证有效性（proof validity）与形式化忠实性（formalization faithfulness）之间的差距。虽然统一生成模式下未发现系统性博弈行为，但两阶段流水线揭示了两种不同的不忠实模式。


<details>
  <summary>Details</summary>
Motivation: 形式化验证能保证证明的有效性，但无法保证形式化的忠实性。在自然语言逻辑推理中，模型需要从头构建公理系统而不受库约束，这种有效证明与忠实翻译之间的差距尤为突出。研究旨在探究前沿模型在生成Lean 4证明时是否利用这一差距进行"形式化博弈"。

Method: 评估GPT-5和DeepSeek-R1在303个一阶逻辑问题（203个来自FOLIO，100个来自Multi-LogiEval）上的表现。比较统一生成与两阶段流水线（将形式化与证明分离）两种方法。通过编译率（87-99%）和跨阶段比较来检测不忠实行为。

Result: 统一生成模式下未发现系统性博弈证据：模型宁愿报告失败也不强制生成证明。两阶段流水线揭示了两种不忠实模式：GPT-5在证明生成阶段捏造公理（可通过跨阶段比较检测），DeepSeek-R1在形式化阶段误译前提（产生内部一致输出，完全逃避检测）。高编译率或准确率不能等同于忠实推理。

Conclusion: 前沿模型在形式化推理中存在不忠实行为，但表现形式不同。GPT-5的不忠实行为可检测，而DeepSeek-R1的不忠实行为更隐蔽。研究强调了形式化忠实性与证明有效性之间的关键区别，警告不应将高编译率或准确率等同于忠实推理。
...
TL;DR: CoDA提出了一种通过轻量级适配器干预中间隐藏状态的方法，结合特征蒸馏和MMD分布匹配，对齐源域和目标域的潜在推理表示，显著提升LLM在跨域逻辑推理任务上的性能。


<details>
  <summary>Details</summary>
Motivation: 在现实世界中，许多专业稀缺领域（如低资源科学学科、新兴生物医学子领域或小众法律管辖区）缺乏高质量领域内示例，限制了上下文学习方法的应用。虽然已有研究探索使用跨域样本作为替代演示，但由于显著的领域偏移，LLM难以有效识别和利用共享的推理模式。

Method: CoDA采用轻量级适配器直接干预中间隐藏状态，结合基于特征的知识蒸馏（使用CoT增强的参考表示）和最大均值差异（MMD）进行核化分布匹配，对齐源域和目标域的潜在推理表示。

Result: 在多个逻辑推理任务和不同模型家族上的广泛实验结果表明，CoDA显著优于之前的最先进基线方法，取得了大幅度的性能提升。

Conclusion: CoDA通过直接干预隐藏状态并采用特征蒸馏和分布匹配的方法，有效解决了跨域逻辑推理中的领域偏移问题，显著提升了LLM在缺乏高质量领域内示例情况下的推理能力。
...
<details>
  <summary>Details</summary>
Motivation: 现有深度剪枝方法主要依赖余弦距离这一单维度启发式方法来识别冗余层，但这种方法在不同架构上表现不稳定，甚至可能导致灾难性崩溃

Method: 提出SimDiff方法，从两个正交角度联合评估层重要性：表示相似性和变换差异。变换差异使用两种不同指标量化：MSSD（对异常值敏感，识别做出决定性修正的层）和MASD（稳健测量层的平均贡献）

Result: 在0.5B到13B参数的多个模型上实验表明，SimDiff显著优于最先进的基线方法。在25%剪枝率下保留LLaMA2-7B超过91%的性能，在LLaMA3.1-8B上剪枝12层实现1.49倍推理加速，且剪枝模型可通过最小微调有效恢复

Conclusion: SimDiff通过多维度评估层重要性，解决了单维度启发式方法的局限性，为大型语言模型的高效部署提供了更可靠的深度剪枝方案
...
### [103] [Integrating Anomaly Detection into Agentic AI for Proactive Risk Management in Human Activity](https://arxiv.org/abs/2604.19538)
*Farbod Zorriassatine,Ahmad Lotfi*

Main category: cs.AI

TL;DR: 该论文提出将跌倒检测和预测作为异常检测问题，通过智能AI系统来更有效地解决老年人跌倒风险，提出了一个动态选择工具并整合到自适应决策工作流的概念框架。
...
Method: 将跌倒检测和预测重新定义为异常检测问题，并提出一个智能AI系统的概念框架，该框架能够动态选择相关工具并将其整合到自适应决策工作流中，而不是依赖为狭窄定义场景定制的静态配置。

Result: 论文提出了一个概念框架，展示了通过智能AI系统处理跌倒风险的潜在价值，能够早期识别与风险增加相关的运动模式中的细微偏差，这些偏差可能源于年龄相关衰退、疲劳或环境因素。

Conclusion: 智能AI系统为解决老年人跌倒风险提供了有前景的途径，通过将跌倒检测和预测作为异常检测问题，并采用动态、自适应的决策工作流，可以更好地处理现实世界的复杂性，尽管立即部署的技术要求超出了本文范围。
...
### [104] [DT2IT-MRM: Debiased Preference Construction and Iterative Training for Multimodal Reward Modeling](https://arxiv.org/abs/2604.19544)
...
<details>
  <summary>Details</summary>
Motivation: 现有多模态偏好数据集存在三个关键问题：偏好强度缺乏粒度、文本风格偏差以及不可靠的偏好信号。同时，开源多模态偏好数据集存在大量噪声，但缺乏有效且可扩展的清理方法来提升其质量。

Method: DT2IT-MRM整合了三个核心组件：1）去偏好的偏好构建流程；2）文本到图像偏好数据的新重构方法；3）迭代训练框架，用于清理现有多模态偏好数据集以进行多模态奖励模型建模。

Result: 实验结果显示，DT2IT-MRM在三个主要基准测试中取得了新的最先进性能：VL-RewardBench、Multimodal RewardBench和MM-RLHF-RewardBench。
...
### [105] [Detecting Data Contamination in Large Language Models](https://arxiv.org/abs/2604.19561)
*Juliusz Janicki,Savvas Chamezopoulos,Evangelos Kanoulas,Georgios Tsatsaronis*

Main category: cs.AI

TL;DR: 研究评估了针对大语言模型的成员推断攻击方法，发现现有黑盒攻击方法在检测训练数据成员资格方面效果不佳，AUC-ROC接近0.5，表明无法可靠检测。


<details>
  <summary>Details</summary>
Motivation: 大语言模型使用大量可能包含版权内容的训练数据，成员推断攻击旨在检测特定文档是否被包含在训练语料中。黑盒攻击方法需要大量数据操作，比较评估具有挑战性。

Method: 研究比较了最先进的黑盒成员推断攻击方法，使用统一数据集评估它们在多种大语言模型上的表现。同时开发了一种新方法"熟悉度排名"，让大语言模型有更多表达自由以更好地理解其推理过程。

Result: 所有方法在多个大语言模型上的AUC-ROC都接近0.5，表明无法可靠检测成员资格。更先进的大语言模型显示出更高的真正例率和假正例率，反映了其更强的推理和泛化能力。

Conclusion: 当前黑盒成员推断攻击方法无法可靠检测大语言模型的训练数据成员资格，这凸显了在大语言模型中检测成员资格的困难性，特别是对于推理和泛化能力更强的模型。
...
### [106] [Multi-modal Reasoning with LLMs for Visual Semantic Arithmetic](https://arxiv.org/abs/2604.19567)
*Chuou Xu,Liya Ji,Qifeng Chen*

Main category: cs.AI

TL;DR: 该研究提出SAri-RFT方法，通过强化学习微调大型视觉语言模型，提升其在视觉语义算术任务中的跨模态关系推理能力，特别是在服务机器人非结构化环境中的应用。


<details>
  <summary>Details</summary>
Motivation: 当前大型语言模型在编码和数学推理方面已有显著进展，但在视觉语义算术（从图像推断关系）方面仍待探索。这对于服务机器人在非结构化环境中理解对象、智能体和动作之间的语义关系至关重要，如厨房场景中识别"粉末"和"蛋糕"之间的"由...制成"关系。

Method: 提出语义算术强化微调(SAri-RFT)方法，构建Image-Relation-Pair Dataset(IRPD)基准数据集，包含两项新任务：二项减法和三项操作。使用可验证函数和组相对策略优化(GRPO)对大型视觉语言模型进行后训练。

Result: 在IRPD基准数据集和真实世界Visual7W-Telling数据集上取得了最先进的结果，显著提升了模型在跨模态关系推理方面的性能。
...
### [107] [SafetyALFRED: Evaluating Safety-Conscious Planning of Multimodal Large Language Models](https://arxiv.org/abs/2604.19638)
*Josue Torres-Fonseca,Naihao Deng,Yinpei Dai,Shane Storks,Yichi Zhang,Rada Mihalcea,Casey Kennington,Joyce Chai*

Main category: cs.AI

TL;DR: SafetyALFRED是一个基于ALFRED基准构建的具身智能体安全评估框架，专门针对厨房环境中的六类现实安全隐患，评估模型不仅识别危险的能力，更重要的是主动采取风险缓解措施的具身规划能力。


<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型作为自主智能体在交互环境中应用日益广泛，但其主动应对安全隐患的能力仍然不足。现有安全评估主要关注通过非具身的问答设置进行危险识别，缺乏对具身环境中主动风险缓解能力的评估。

Method: 基于具身智能体基准ALFRED构建SafetyALFRED，增加了六类现实厨房安全隐患。评估了来自Qwen、Gemma和Gemini家族的11个最先进模型，不仅测试危险识别能力，还评估通过具身规划主动缓解风险的能力。

Result: 实验结果显示显著的对齐差距：模型在问答设置中能够准确识别危险，但这些危险的平均缓解成功率相对较低。这表明静态的问答评估对于物理安全是不充分的。

Conclusion: 需要从静态问答评估转向优先考虑具身环境中纠正行动的基准范式转变。作者开源了代码和数据集，以促进具身安全评估的发展。
...
### [108] [A Dual Perspective on Synthetic Trajectory Generators: Utility Framework and Privacy Vulnerabilities](https://arxiv.org/abs/2604.19653)
*Aya Cherigui,Florent Guépin,Arnaud Legendre,Jean-François Couchot*

Main category: cs.AI

TL;DR: 该论文提出了一个评估生成模型生成的人类移动数据隐私保护效果的新框架，指出当前隐私评估仍面临挑战，并提出了针对特定生成模型的成员推理攻击方法。


<details>
  <summary>Details</summary>
Motivation: 人类移动数据具有敏感性，传统隐私保护方法（如聚合、混淆、噪声添加）会显著降低数据效用。虽然生成模型被用于平衡隐私与效用，但其隐私保护效果评估仍是一个开放问题。

Method: 1. 引入并应用新的效用评估框架；2. 提出针对特定生成模型子类（即使该子类被认为对轨迹用户链接问题具有抵抗力）的成员推理攻击方法；3. 建议通过对抗性评估来评估隐私保护效果。

Result: 1. 提出了评估生成模型效用和隐私保护效果的新框架；2. 证明了即使被认为具有隐私保护的生成模型子类仍存在隐私风险；3. 发现隐私评估仍然是一个重大挑战。

Conclusion: 该研究为解决人类移动数据生成模型的隐私-效用权衡问题迈出了第一步，强调需要通过对抗性评估来评估隐私保护效果，并指出当前隐私评估仍需进一步研究。
...
### [109] [Smiling Regulates Emotion During Traumatic Recollection](https://arxiv.org/abs/2604.19019)
...
TL;DR: 研究通过自动微笑检测模型分析978名大屠杀幸存者视频证词中的微笑行为，发现微笑在创伤回忆中起到情绪调节和社交互动的重要作用
...
Method: 开发自动微笑检测模型（F1分数85%），对检测到的微笑按两种分类法标注；对1,083,417个转录句子提取叙事特征，并从音频、眼动和文本三个模态获取情感效价

Result: 微笑率与特定语义主题、叙事结构和时间句法显著相关；微笑常出现在强烈负面情绪期间，这些负面情绪微笑显著改善周围句子的情感轨迹；微笑减少眼部动态和眨眼率，这些效应强度受叙事效价调节
...
### [110] [Compile to Compress: Boosting Formal Theorem Provers by Compiler Outputs](https://arxiv.org/abs/2604.18587)
*Guchan Li,Rui Tian,Hongning Wang*

Main category: cs.LG

TL;DR: 提出了一种基于学习-精炼框架的高效形式定理证明方法，通过利用编译器将多样证明尝试映射到紧凑失败模式的结构特性，在有限计算资源下提升LLM的推理能力。


<details>
  <summary>Details</summary>
Motivation: 当前大型语言模型在形式定理证明中需要大量测试时计算资源（如大规模展开或长上下文窗口），存在可扩展性瓶颈。需要开发更高效的验证引导推理方法。

Method: 提出学习-精炼框架，利用编译器将多样证明尝试压缩为结构化失败模式的特性。通过树搜索在显式验证器反馈下局部修正错误，避免积累长证明历史带来的成本。

Result: 该方法在不同规模上持续增强基础证明器的推理能力。在PutnamBench上，在可比测试时预算下，8B和32B参数模型达到公开报告中的最先进性能。

Conclusion: 该方法为下一代验证引导推理提供了可扩展的范式，通过利用形式验证中的信息结构有效解决了计算资源瓶颈问题。
...
### [111] [Easy Samples Are All You Need: Self-Evolving LLMs via Data-Efficient Reinforcement Learning](https://arxiv.org/abs/2604.18639)
*Zhiyin Yu,Bo Zhang,Qibin Hou,Zhonghai Wu,Xiao Luo,Lei Bai*

Main category: cs.LG

TL;DR: EasyRL是一种基于认知学习理论的新方法，通过整合少量标注数据和渐进式伪标注策略，实现数据高效的LLM强化学习后训练。


<details>
  <summary>Details</summary>
Motivation: 现有LLM强化学习方法存在标注成本高、模型崩溃和奖励黑客等问题，性能不尽如人意。需要一种既能减少标注成本又能避免这些问题的数据高效方法。

Method: 1) 使用少量标注数据进行监督强化学习初始化模型；2) 对困难未标注数据采用分治伪标注策略，结合基于一致性的低不确定性选择和基于反思的中等不确定性解决；3) 难度渐进的自训练，通过迭代伪标注和强化学习增强模型推理能力。

Result: 在数学和科学基准测试中，EasyRL仅使用10%的简单标注数据，就能持续超越最先进的基线方法。

Conclusion: EasyRL提供了一个统一的自进化框架，通过模拟人类认知获取曲线，实现了数据高效的LLM后训练，为解决标注成本高和模型性能不足的问题提供了有效方案。
...
### [112] [Curiosity-Critic: Cumulative Prediction Error Improvement as a Tractable Intrinsic Reward for World Model Training](https://arxiv.org/abs/2604.18701)
*Vin Bhaskara,Haicheng Wang*

Main category: cs.LG

TL;DR: 论文提出Curiosity-Critic方法，通过基于世界模型累积预测误差改进的内在奖励，在线分离可学习的认知误差和不可约的随机误差，提升探索效率。
...
Method: 提出Curiosity-Critic方法：1）将内在奖励建立在累积预测误差改进的基础上；2）推导出可处理的每步形式：当前预测误差与当前状态转移的渐近误差基线之差；3）通过在线学习的critic估计基线，与world model协同训练；4）critic回归单个标量，在世界模型饱和前收敛，引导探索朝向可学习的状态转移。

Result: 在随机网格世界实验中，Curiosity-Critic在收敛速度和最终世界模型准确性方面优于基于预测误差和访问计数的基线方法。该方法能有效分离认知误差（可约）和随机误差（不可约），对可学习的状态转移给予更高奖励，对随机状态转移则奖励趋近基线。

Conclusion: Curiosity-Critic通过基于累积预测误差改进的内在奖励，提供了一种更有效的探索策略。该方法将先前的预测误差好奇心公式作为特定基线近似的特例，能在线区分可学习和不可学习的预测误差，提升强化学习中的探索效率。
...
TL;DR: 论文提出了一种基于预测必要性的非线性时间序列模型因果评估框架，通过系统边缘消除和预测比较来检验因果关系是否对准确预测是必需的。


<details>
  <summary>Details</summary>
Motivation: 当前非线性机器学习模型在时间序列因果发现中应用广泛，但其输出解释性差。特别是正则化神经自回归模型产生的因果分数常被当作回归系数的类比，导致对统计显著性的误导性解读。

Method: 提出了基于预测必要性的因果评估框架，采用系统边缘消除和预测比较方法。以神经加性向量自回归模型为案例研究，应用于139个国家民主发展的多变量时间序列面板数据。

Result: 研究发现具有相似因果分数的关系在预测必要性上存在显著差异，这主要由于冗余性、时间持久性和特定制度效应等因素造成。
...
TL;DR: 该论文提出了一种处理医疗多模态数据缺失问题的新方法，通过将临床诊断重构为自回归序列建模任务，利用对比预训练和Transformer架构，在MIMIC-IV和eICU数据集上取得了优于基线模型的表现。


<details>
  <summary>Details</summary>
Motivation: 医疗多模态机器学习面临的主要挑战是在训练和部署过程中处理缺失模态。临床数据集具有时间性和模态稀疏性，如何在保持模型可解释性的同时捕捉预测信号是一个持续挑战。

Method: 1. 将临床诊断重构为自回归序列建模任务，利用大语言模型的因果解码器建模患者多模态轨迹；2. 引入缺失感知的对比预训练目标，在共享潜在空间中整合具有缺失的多模态数据；3. 使用基于Transformer的架构进行自回归序列建模。

Result: 在MIMIC-IV和eICU微调基准测试中，自回归序列建模方法优于基线模型。可解释性分析显示，移除模态会导致模型行为发散，而对比预训练能够缓解这一问题。

Conclusion: 通过将临床诊断抽象为序列建模并解释患者住院轨迹，开发了一个能够分析和处理缺失模态的框架，同时满足了安全、透明的临床AI的经典需求。
...
TL;DR: 通过在推理时向Transformer残差流中集成预训练的稀疏自编码器，显著降低大语言模型对越狱攻击的脆弱性，实现最高5倍的攻击成功率降低
...
Method: 在推理时向Transformer残差流中集成预训练的稀疏自编码器，不修改模型权重也不阻断梯度。在四个模型家族（Gemma、LLaMA、Mistral、Qwen）上测试，使用两种白盒攻击（GCG、BEAST）和三个黑盒基准

Result: SAE增强模型相对于未防御基线实现了最高5倍的越狱成功率降低，并减少了跨模型攻击的可转移性。参数消融显示：1）L0稀疏度与攻击成功率之间存在单调剂量-响应关系；2）存在层依赖的防御-效用权衡，中间层在鲁棒性和清洁性能之间达到平衡

Conclusion: 这些发现与表征瓶颈假说一致：稀疏投影重塑了越狱攻击所利用的优化几何结构，表明稀疏自编码器可以作为一种有效的防御机制来增强大语言模型的鲁棒性
...
### [116] [Streaming Structured Inference with Flash-SemiCRF](https://arxiv.org/abs/2604.18780)
*Benjamin K. Johnson,Thomas Goralski,Ayush Semwal,Hui Shen,H. Josh Jang*

Main category: cs.LG

TL;DR: Flash-SemiCRF：一种高效的内存优化半马尔可夫条件随机场实现，通过前缀和查找、流式前向-后向传递和数值稳定技术，解决了传统半CRF在大规模序列（如基因组数据）上的内存瓶颈问题。


<details>
  <summary>Details</summary>
Motivation: 传统半CRF实现需要存储巨大的边势能张量，其大小随序列长度、最大片段长度和标签数量增长，在语音规模的状态空间下变得不可行，在基因组尺度（序列超过10万位置）下更是无法处理。这种内存瓶颈限制了精确片段级推理在长序列和大标签集上的应用。

Method: 1. 用前缀和查找替代存储的边张量，将内存占用减少与片段长度和标签数量乘积成比例；2. 采用带检查点边界归一化的流式前向-后向传递，保持工作内存与序列长度呈亚线性关系，同时保留精确梯度；3. 使用零中心累积分数控制数值漂移，并在标签不平衡下引入自适应持续时间先验。

Result: 将这些技术集成到Flash-SemiCRF中，这是一个融合的Triton内核，能够在以前无法处理的规模上进行精确的半CRF推理。代码已在GitHub上开源。

Conclusion: Flash-SemiCRF通过创新的内存优化技术，解决了半CRF在长序列和大标签集上的计算瓶颈，使得精确的片段级推理能够应用于基因组尺度等先前无法处理的规模。
...
### [117] [Efficient Mixture-of-Experts LLM Inference with Apple Silicon NPUs](https://arxiv.org/abs/2604.18788)
*Afsara Benazir,Felix Xiaozhu Lin*

Main category: cs.LG

TL;DR: NPUMoE是一个针对苹果芯片的运行时推理引擎，通过将MoE模型中的静态密集计算卸载到NPU，同时保留CPU/GPU回退路径处理动态操作，显著提升推理效率和能效。


<details>
  <summary>Details</summary>
Motivation: 苹果神经引擎（ANE）是苹果芯片中的专用神经处理单元，但MoE LLMs在NPU上推理面临三个挑战：专家路由不可预测导致动态张量形状冲突；不规则操作符不适用于NPU；大量小型专家内核导致调度和同步开销大。NPU旨在从CPU和GPU卸载AI计算，本文目标是实现MoE推理的NPU卸载，特别是在长上下文预填充阶段。

Method: NPUMoE使用离线校准来估计专家容量和流行度，驱动三个关键技术：1）静态专家容量层级处理动态专家路由；2）分组专家执行缓解NPU并发限制；3）负载感知的专家计算图驻留减少CPU-NPU同步开销。系统将密集静态计算卸载到NPU，同时保留CPU/GPU回退路径处理动态操作。

Result: 在苹果M系列设备上使用三个代表性MoE LLM和四个长上下文工作负载的实验表明，NPUMoE始终优于基线方法，延迟降低1.32-5.55倍，能效提升1.81-7.37倍，CPU周期使用减少1.78-5.54倍。

Conclusion: NPUMoE通过有效的NPU卸载策略成功解决了MoE LLMs在苹果芯片上的推理挑战，显著提升了推理性能、能效和系统资源利用率。
...
### [118] [HELM: Harness-Enhanced Long-horizon Memory for Vision-Language-Action Manipulation](https://arxiv.org/abs/2604.18791)
*Zijian Zeng,Fei Ding,Huiming Yang,Xianwei Li*

Main category: cs.LG

TL;DR: HELM框架通过引入情景记忆模块、状态验证器和控制器，解决了VLA模型在长时程操作任务中的系统性问题，显著提升了任务成功率。


<details>
  <summary>Details</summary>
Motivation: 当前视觉-语言-动作模型在短时程任务上表现良好，但在长时程操作任务中系统性地失败。研究发现，仅扩展上下文长度无法解决这一问题，而是存在三个执行循环缺陷：记忆差距、验证差距和恢复差距。

Method: 提出HELM框架，包含三个组件：1) 情景记忆模块，通过CLIP索引的关键帧检索关键任务历史；2) 学习的状态验证器，从观察、动作、子目标和记忆条件上下文中预测动作失败；3) 控制器，执行回滚和重新规划。

Result: 在LIBERO-LONG上，HELM将任务成功率从OpenVLA的58.4%提升至81.5%，提升23.1个百分点。相比之下，仅将上下文窗口扩展到H=32仅带来5.4个百分点的增益，相同预算的LoRA适配仍比HELM低12.2个百分点。在CALVIN上也显著提升了长时程性能。

Conclusion: HELM框架有效解决了VLA模型在长时程操作任务中的系统性问题，状态验证器是核心学习贡献，其效果关键依赖于情景记忆的访问。研究还发布了LIBERO-Recovery作为评估长时程操作中失败恢复的扰动注入协议。
...
### [119] [Preserving Clusters in Error-Bounded Lossy Compression of Particle Data](https://arxiv.org/abs/2604.18801)
...
<details>
  <summary>Details</summary>
Motivation: 现有有损压缩方法只保证粒子位置的点误差，无法保证聚类结果的一致性，而聚类结构对科学分析至关重要，微小扰动就可能改变聚类连接性

Method: 基于校正的技术：1) 通过空间分区和局部邻域搜索识别脆弱粒子对；2) 使用投影梯度下降优化，编码成对距离违反的损失函数；3) 可扩展的GPU加速分布式实现

Result: 在宇宙学和分子动力学数据集上，该方法能有效保持聚类结果，同时与SZ3、ZFP、Draco等压缩器相比保持有竞争力的压缩性能

Conclusion: 提出的校正方法能够在保持压缩效率的同时确保单链接聚类结果的一致性，为科学数据压缩提供了聚类感知的解决方案
...
### [120] [Rethinking Dataset Distillation: Hard Truths about Soft Labels](https://arxiv.org/abs/2604.18811)
*Priyam Dey,Aditya Sahdev,Sunny Bhati,Konda Reddy Mopuri,R. Venkatesh Babu*

Main category: cs.LG

TL;DR: 研究发现当前大规模数据集蒸馏方法在软标签训练下与随机基线表现相当，揭示了软标签评估的问题，并提出新的计算对齐蒸馏方法CA2D


<details>
  <summary>Details</summary>
Motivation: 针对大规模数据集蒸馏方法在软标签训练下与随机基线表现相当的现象，与核心集文献中高质量核心集在硬标签设置下始终优于随机子集的结果形成对比，需要理解这种差异的原因

Method: 1. 进行详细的扩展性分析，研究不同标签机制下数据质量的作用；2. 在硬标签设置下系统评估9种DD方法；3. 提出CAD-Prune计算感知剪枝指标；4. 开发CA2D计算对齐DD方法

Result: 1. 高质量核心集在软标签和软标签+知识蒸馏设置下无法显著优于随机基线；2. 在软标签+知识蒸馏设置下，性能接近全数据集的最优水平；3. 只有RDED在ImageNet-1K上可靠地优于随机基线；4. CA2D方法在各种IPC设置下优于当前DD方法

Conclusion: 揭示了当前DD研究中软标签评估的问题，建立了计算对齐的数据高效学习工具，为DD和核心集研究提供了新见解
...
### [121] [Curvature-Aware PCA with Geodesic Tangent Space Aggregation for Semi-Supervised Learning](https://arxiv.org/abs/2604.18816)
...
<details>
  <summary>Details</summary>
Motivation: 传统PCA的全局线性公式无法捕捉弯曲流形上的数据结构，而流形学习方法虽然能建模非线性但牺牲了PCA的谱结构和稳定性。需要一种方法既能处理非线性流形结构，又能保持PCA的谱特性。

Method: 提出GTSA-PCA：1）用k近邻图上的曲率加权局部协方差算子替代全局协方差算子，生成适应流形的局部切子空间；2）引入测地线对齐算子，结合内在图距离和子空间亲和度来全局同步局部表示；3）可加入半监督信息指导对齐过程。

Result: 在真实数据集上的实验显示，GTSA-PCA相比PCA、核PCA、监督PCA以及UMAP等图基方法有持续改进，特别是在小样本量和高曲率情况下表现优异。

Conclusion: GTSA-PCA为统计方法和几何方法的降维提供了一个原则性的桥梁，在保持PCA谱结构的同时有效处理流形非线性。
...
### [122] [One Step Forward and K Steps Back: Better Reasoning with Denoising Recursion Models](https://arxiv.org/abs/2604.18839)
*Chris Cameron,Wangzheng Wang,Nikita Ivanov,Ashmita Bhattacharyya,Didier Chételat,Yingxue Zhang*

Main category: cs.LG

TL;DR: Denoising Recursion Models (DRM) 是一种改进的循环Transformer训练方法，通过多步递归去噪提供中间状态监督，相比单步去噪的扩散模型更好地对齐训练与测试行为，在ARC-AGI等困难问题上表现优异。


<details>
  <summary>Details</summary>
Motivation: 循环Transformer通过共享参数块迭代细化预测，但在需要搜索式计算的困难问题上，从噪声到结构化解决方案的长细化轨迹难以学习，因为训练只提供目标解而没有中间路径监督。扩散模型通过单步去噪解决此问题，但训练与测试行为不对齐。

Method: 提出Denoising Recursion Models (DRM)，同样对数据添加噪声，但训练模型通过多个递归步骤逐步去除噪声，提供可处理的中间状态课程，更好地对齐训练与测试，并激励非贪婪、前瞻性的生成。

Result: 通过大量实验证明，该方法在ARC-AGI上超越了最近取得突破性性能的Tiny Recursion Model (TRM)。

Conclusion: 多步递归去噪训练策略比单步去噪方法更有效，能更好地学习复杂问题的迭代细化轨迹，在需要搜索式计算的困难任务上表现更优。
...
### [123] [Task Switching Without Forgetting via Proximal Decoupling](https://arxiv.org/abs/2604.18857)
*Pourya Shamsolmoali,Masoumeh Zareapoor,Eric Granger,William A. P. Smith,Yue Lu*

Main category: cs.LG

TL;DR: 本文提出了一种新的持续学习方法，通过算子分裂将任务学习和稳定性增强分离，解决了传统方法中学习与记忆信号混合导致容量利用效率低的问题。


<details>
  <summary>Details</summary>
Motivation: 传统持续学习方法通常将正则化项直接加入训练损失并用标准梯度下降优化，这会将学习信号和记忆信号混合到单个更新中，没有明确分离关键参数和冗余参数。随着任务序列增长，这种耦合会过度约束模型，限制前向传递并导致容量利用效率低下。

Method: 提出了一种基于算子分裂的方法，将任务学习与稳定性增强分离。学习步骤专注于最小化当前任务损失，而邻近稳定性步骤应用稀疏正则化器来剪枝不必要参数并保留任务相关参数。这种方法将稳定性-可塑性权衡转化为两个互补算子之间的协商更新，而不是冲突梯度。

Result: 该方法在标准基准测试中实现了最先进的结果，提高了稳定性和适应性，且不需要回放缓冲区、贝叶斯采样或元学习组件。

Conclusion: 通过算子分裂将任务学习与稳定性增强分离的方法，为持续学习中的稳定性-可塑性权衡提供了更有效的解决方案，避免了传统方法中梯度冲突和容量利用效率低的问题。
...
### [124] [ParamBoost: Gradient Boosted Piecewise Cubic Polynomials](https://arxiv.org/abs/2604.18864)
*Nicolas Salvadé,Tim Hillel*

Main category: cs.LG

TL;DR: ParamBoost是一种新型广义加性模型，使用梯度提升算法学习三次多项式形状函数，支持多种约束条件以融入专家知识，在保持可解释性的同时提升预测性能。


<details>
  <summary>Details</summary>
Motivation: 传统广义加性模型虽然具有玻璃盒可解释性，但无法融入建模者的专家知识。ParamBoost旨在创建既能保持完全可解释性，又能通过约束条件融入领域知识的模型。

Method: ParamBoost使用梯度提升算法学习形状函数，在叶节点拟合三次多项式函数。模型支持五种约束条件：形状函数及其导数的连续性（最高C2）、单调性、凸性、特征交互约束和模型规范约束。

Result: 无约束的ParamBoost模型在多个真实世界数据集上持续优于最先进的广义加性模型。建模者可以选择性地施加所需约束，以适度的预测性能为代价，使模型完全适应特定应用的可解释性和参数分析需求。

Conclusion: ParamBoost提供了一种灵活的框架，既保持了广义加性模型的完全可解释性，又允许通过约束条件融入专家知识，在预测性能和领域特定需求之间实现了良好平衡。
...
### [125] [AC-SINDy: Compositional Sparse Identification of Nonlinear Dynamics](https://arxiv.org/abs/2604.18889)
*Peter Racioppo*

Main category: cs.LG

TL;DR: AC-SINDy是SINDy框架的扩展，用算术电路替代显式特征库，通过线性函数和乘法交互的组合构建非线性特征，实现紧凑可扩展的参数化，并在计算图上直接施加稀疏性。


<details>
  <summary>Details</summary>
Motivation: 标准SINDy方法需要枚举候选基函数，这在处理复杂非线性系统时面临可扩展性问题。AC-SINDy旨在通过结构化表示替代显式特征库，提高方法的可扩展性和鲁棒性。

Method: 1. 使用算术电路结构替代传统特征库，通过线性函数和乘法交互的组合构建非线性特征；2. 在计算图上直接施加稀疏性；3. 引入分离状态估计和动力学识别的公式，结合潜在状态推断、共享动力学和多步监督。

Result: 在非线性和混沌系统上的实验表明，该方法能够恢复准确且可解释的支配方程，同时比标准SINDy具有更好的可扩展性，对噪声具有更强的鲁棒性。

Conclusion: AC-SINDy通过算术电路的结构化表示和分离状态估计与动力学识别的策略，显著提升了SINDy框架的可扩展性、鲁棒性和可解释性，为复杂非线性系统的建模提供了更有效的工具。
...
### [126] [Harmful Intent as a Geometrically Recoverable Feature of LLM Residual Streams](https://arxiv.org/abs/2604.18901)
*Isaac Llorente-Saguer*

Main category: cs.LG

TL;DR: 研究发现大语言模型的残差流中存在可几何恢复的有害意图方向：在大多数层中表现为线性方向，在投影方法失效的层中表现为角度偏差。该方向在12个不同架构和变体的模型中稳定存在，且有害意图与拒绝行为在表示上是功能解耦的特征。


<details>
  <summary>Details</summary>
Motivation: 研究动机是探索大语言模型中是否存在可检测的有害意图表示，以及这种表示在不同模型架构、对齐变体和规模下的稳定性。特别关注有害意图与拒绝行为在表示层面的关系，以及评估指标（如AUROC）在实际操作中的有效性。

Method: 使用12个模型（涵盖Qwen2.5、Qwen3.5、Llama-3.2、Gemma-3四种架构家族和基础、指令微调、去拒绝三种对齐变体），在单轮英文评估中，采用六种方向寻找策略分析残差流的几何特性。成功策略包括：软AUC优化的线性方向、类均值探针、监督角度偏差策略。

Result: 1. 软AUC优化线性方向达到平均AUROC 0.98和TPR@1%FPR 0.80；2. 类均值探针达到AUROC 0.98和TPR 0.71，拟合成本<1ms；3. 监督角度偏差策略在表示上不同的方向（与投影方法呈73°角）达到AUROC 0.96和TPR 0.61，在中层保持检测能力；4. 检测在不同对齐变体（包括去拒绝模型）中保持稳定；5. AdvBench训练的方向可泛化到HarmBench和JailbreakBench；6. 在0.8B到9B参数的Qwen3.5模型中，AUROC保持≥0.98。

Conclusion: 模型在通用语言理解过程中获得了线性可解码的有害意图表示，对齐过程仅塑造模型如何处理此类输入而不重组上游识别信号。AUROC在0.97+区间可能高估实际操作可检测性，安全相关评估应同时使用TPR@1%FPR指标。
...
### [127] [Collaborative Contextual Bayesian Optimization](https://arxiv.org/abs/2604.18912)
*Chih-Yu Chang,Qiyuan Chen,Tianhan Gao,David Fenning,Chinedum Okwudire,Neil Dasgupta,Wei Lu,Raed Al Kontar*

Main category: cs.LG

TL;DR: 提出CCBO框架，通过多客户端协作进行上下文贝叶斯优化，支持在线协作和离线初始化，具有隐私保护通信机制，在异构客户端场景下显著提升学习效率


<details>
  <summary>Details</summary>
Motivation: 现实应用中需要发现最优设计，上下文贝叶斯优化比传统贝叶斯优化更具挑战性，需要同时探索上下文空间和利用每个上下文的最优设计。现代应用中多个异构但相关的客户端可以通过协作显著提高学习效率
...
Conclusion: CCBO为多客户端协作上下文贝叶斯优化提供了统一框架，在异构环境下仍能保持优异性能，具有实际应用价值
...
### [128] [Fine-Tuning Small Reasoning Models for Quantum Field Theory](https://arxiv.org/abs/2604.18936)
...
<details>
  <summary>Details</summary>
Motivation: 尽管大语言模型在理论物理中的应用日益增长，但学术界对于这些模型在训练过程中如何发展特定领域的物理推理能力缺乏深入探索。由于训练此类能力所需的开源可验证数据稀缺，需要开发专门的数据生成方法。

Method: 开发了稳健的数据生成管道，既能创建合成问题，又能将现有的人工编写问题适配为模型训练数据。以量子场论为主要领域，生成了2500多个合成问题，并收集了来自arXiv和标准教学资源的人工适配问题。进行了强化学习和监督微调实验，并分析了模型在微调前后的思维链推理过程。

Result: 成功生成了大规模的可验证量子场论训练数据，包括约200M tokens的QFT推理轨迹。通过微调实验获得了性能提升，并评估了模型在其他物理领域的泛化能力。深入分析了强化学习和监督微调过程中推理错误的变化模式。

Conclusion: 该研究为理论物理领域的专用推理模型训练提供了首个学术框架，公开了数据管道、可验证的QFT训练数据和推理轨迹，为后续研究奠定了基础，有助于理解模型在特定物理领域推理能力的发展机制。
...
### [129] [TabEmb: Joint Semantic-Structure Embedding for Table Annotation](https://arxiv.org/abs/2604.18939)
*Ehsan Hoseinzade,Ke Wang,Anandharaju Durai Raju*

Main category: cs.LG

TL;DR: TabEmb：一种用于表格注释的新方法，通过解耦语义编码和结构建模，使用LLM生成列嵌入，再通过图模块注入列间关系，优于现有基线方法。


<details>
  <summary>Details</summary>
Motivation: 现有表格表示学习方法将二维表格线性化为一维序列并用预训练语言模型编码，导致语义质量有限、对未见值泛化能力弱，以及因2D到1D扁平化和上下文长度限制导致的结构建模能力下降。

Method: TabEmb将语义编码与结构建模解耦：首先使用大语言模型为每列生成语义丰富的嵌入，然后通过基于列的图模块将关系信息注入嵌入中，生成联合的语义-结构表示。

Result: 实验表明，TabEmb在不同表格注释任务上始终优于强基线方法。

Conclusion: TabEmb通过解耦语义编码和结构建模的方法，有效解决了现有表格表示学习方法的局限性，在表格注释任务上取得了更好的性能。
...
### [130] [Distillation Traps and Guards: A Calibration Knob for LLM Distillability](https://arxiv.org/abs/2604.18963)
*Weixiao Zhan,Yongcheng Jing,Leszek Rutkowski,Dacheng Tao*

Main category: cs.LG

TL;DR: 该论文分析了知识蒸馏中的陷阱，提出了一种后校准方法，通过强化微调控制教师的可蒸馏性，既能提升知识蒸馏效果，又能保护模型知识产权。


<details>
  <summary>Details</summary>
Motivation: 知识蒸馏虽然能将大语言模型能力转移到小模型，但存在不可预测的失败风险，并可能导致模型泄露。研究发现蒸馏过程中存在尾部噪声、离策略不稳定性和师生差距等陷阱，这些会导致过度自信的幻觉、自我校正崩溃和局部解码退化等问题。

Method: 提出了一种后校准方法，首次通过强化微调实现对教师模型可蒸馏性的控制。目标函数结合了任务效用、KL锚点和跨分词器校准奖励，使可蒸馏性成为基础模型的实际安全杠杆。

Result: 在数学、知识问答和指令跟随任务上的实验表明，从可蒸馏校准教师蒸馏出的学生模型优于SFT和KD基线，而不可蒸馏校准教师保持任务性能但导致蒸馏学生崩溃，为更好的知识蒸馏和模型知识产权保护提供了实用控制手段。

Conclusion: 该方法首次实现了对教师模型可蒸馏性的控制，将鲁棒的师生迁移与部署感知的模型保护联系起来，为知识蒸馏效果提升和模型知识产权保护提供了实用的安全杠杆。
...
### [131] [Self-Improving Tabular Language Models via Iterative Group Alignment](https://arxiv.org/abs/2604.18966)
*Yunbo Long,Tejumade Afonja,Alexandra Brintrup,Mario Fritz*

Main category: cs.LG

TL;DR: TabGRAA是一个用于表格数据生成的自改进框架，通过自动化反馈机制实现模型自我优化，解决了传统方法无法从生成样本中学习和忽略全局统计特性的问题。


<details>
  <summary>Details</summary>
Motivation: 现有语言模型在表格数据生成方面存在两个根本限制：1）静态微调产生的模型无法从自身生成的样本中学习并自我修正；2）自回归目标保持局部标记连贯性但忽视全局统计特性，降低了表格质量。强化学习虽能提供解决方案，但需要设计平衡竞争目标的奖励函数，这在表格数据中不切实际。

Method: TabGRAA使用自动化质量信号（如两样本可区分性分类器或基于距离的奖励）将新生成的样本划分为高质量和低质量组，然后优化组相对优势目标，强化真实模式同时惩罚伪影。质量信号是模块化选择而非框架固定组件，在每个轮次中重新计算针对新生成合成样本的质量信号，语言模型仅在这些自生成信号上进行微调。

Result: 实验表明TabGRAA在保真度、实用性和隐私性方面优于现有方法，同时匹配或超越基于扩散的合成器，将表格合成从静态统计复制推进到动态自改进生成。

Conclusion: TabGRAA通过建立良性反馈循环，实现了表格数据生成的自我改进能力，在初始监督微调后无需暴露额外真实记录，减轻了数据泄漏风险，代表了表格数据生成方法的重要进展。
...
TL;DR: DSR框架将自动形式化分解为模块化流程，通过分解、结构和修复三步骤，利用神经符号方法将数学语句转换为结构化操作树，在PRIME基准测试中达到最先进水平。


<details>
  <summary>Details</summary>
Motivation: 现有自动形式化方法将形式代码视为扁平序列，忽略了数学语句固有的层次逻辑结构，需要更系统的方法来处理复杂的数学陈述。

Method: 提出DSR（分解、结构和修复）神经符号框架，将自动形式化重构为模块化流程：1）将语句分解为逻辑组件；2）映射到结构化操作树；3）通过子树细化精确定位和修复错误。

Result: 在PRIME基准测试（156个本科和研究生级别定理）上，DSR在同等计算预算下始终优于基线方法，建立了新的最先进水平。

Conclusion: DSR框架通过结构化方法有效处理数学语句的层次逻辑，显著提升自动形式化性能，相关数据集、模型和代码将公开发布。
...
<details>
  <summary>Details</summary>
Motivation: 传统的扩散蒸馏方法（如DMD）在追求快速采样时往往牺牲生成质量。虽然将强化学习融入蒸馏有潜力，但简单的融合依赖于次优的原始样本评估，这种基于样本的评分与蒸馏轨迹存在内在冲突，且由于早期生成的噪声特性产生不可靠的奖励。

Method: GDMD重新定义了奖励机制，将蒸馏梯度而非原始像素输出作为优化的主要信号。通过将DMD梯度重新解释为隐式目标张量，使现有奖励模型能够直接评估蒸馏更新的质量。这种梯度级指导作为自适应权重，使RL策略与蒸馏目标同步，有效中和优化分歧。

Result: GDMD在少步生成方面达到了新的SOTA水平。具体来说，4步模型的质量超过了多步教师模型，并在GenEval和人类偏好指标上显著超过了之前的DMDR结果，展现出强大的可扩展潜力。

Conclusion: GDMD通过梯度级奖励机制成功解决了扩散蒸馏中质量与速度的权衡问题，为少步生成提供了更有效的优化框架，实现了质量提升的同时保持快速采样能力。
...
TL;DR: 该研究提出了一种基于扩散模型的学习策略，用于为轨迹优化求解器提供热启动初始猜测，通过利用轨迹和反馈增益的一阶信息来避免复合误差，显著减少求解时间。


<details>
  <summary>Details</summary>
Motivation: 轨迹优化求解器对每个新问题实例独立求解，收敛速度和解决方案质量依赖于初始轨迹。虽然可以使用学习策略生成初始猜测来提高效率，但局部最优的演示数据会导致小偏差在长时间范围内产生复合误差。

Method: 利用梯度基轨迹优化求解器提供的反馈增益，推导出基于Sobolev学习的扩散策略一阶损失函数，同时使用轨迹和反馈增益信息进行训练。

Result: 该方法避免了复合误差，仅需少量轨迹就能学习，提供初始猜测可将求解时间减少2倍到20倍。一阶信息的加入还减少了扩散步骤，降低了推理延迟。

Conclusion: 通过利用轨迹优化求解器提供的反馈增益进行一阶学习，扩散策略能够有效为轨迹优化提供热启动，显著提高求解效率，同时减少推理延迟。
...
TL;DR: 该论文提出了一种基于线性二次调节器（LQR）的LLM激活引导方法，通过将推理过程建模为线性时变动力系统，实现闭环反馈控制，相比现有开环方法在毒性、真实性等任务上表现更优。


<details>
  <summary>Details</summary>
Motivation: 现有的推理时LLM对齐方法（特别是激活引导）通常依赖非预期性干预，忽略了扰动在Transformer层间的传播，缺乏在线误差反馈，导致次优的开环控制效果。

Method: 论文发现尽管Transformer块具有非线性结构，但多层LLM架构的层间动态可以用局部线性模型良好近似。基于此，将LLM推理建模为线性时变动力系统，并采用经典线性二次调节器计算反馈控制器，利用层间雅可比矩阵引导激活向量向目标语义设定点收敛，实现闭环控制且无需离线训练。

Result: 该方法在毒性、真实性、拒绝行为和任意概念调节等任务上实现了最先进的性能，超越了基线引导方法。通过自适应语义特征设定点信号，能够在不同模型、规模和任务上实现鲁棒的细粒度行为控制。

Conclusion: 论文提出的LQR激活引导方法通过将LLM推理建模为动力系统并应用闭环反馈控制，解决了现有激活引导方法的局限性，实现了更优的语义控制性能，且具有理论性能保证和最小计算开销。
...
TL;DR: FG²-GDN提出了一种改进的线性注意力机制，将标量学习率β_t替换为通道级向量，并进一步提出FG²-GDN+解耦键和值的缩放，在保持计算效率的同时提升了关联回忆和长上下文理解能力。


<details>
  <summary>Details</summary>
Motivation: 现有线性注意力机制如Gated DeltaNet (GDN)和Kimi Delta Attention (KDA)虽然通过delta规则改进了关联回忆能力，但其中的学习率β_t仍然是标量，限制了模型在维度特定适应方面的能力。这类似于从SGD优化器向AdaGrad、Adam等自适应优化器的演进需求。

Method: 1. FG²-GDN：将delta更新中的标量学习率β_t替换为通道级向量，实现类似从SGD到AdaGrad/Adam的优化器升级；2. FG²-GDN+：进一步解耦键和值的缩放，实现对擦除强度和写入强度的独立控制。

Result: 在合成和真实世界基准测试中，FG²-GDN及其变体相比GDN和KDA在关联回忆和长上下文理解方面表现更优，同时保持了相当的计算效率。

Conclusion: 通过将标量学习率扩展为通道级向量并解耦键值缩放，FG²-GDN系列方法显著提升了线性注意力机制的表达能力，为长序列建模提供了更有效的解决方案。
...
TL;DR: 本文首次将安全RLHF建模为无限时域折扣约束马尔可夫决策过程，提出了两种无需奖励模型拟合的安全RLHF算法，并提供了多项式收敛保证。


<details>
  <summary>Details</summary>
Motivation: 现有安全RLHF方法通常依赖固定时域奖励模型拟合，且仅经验验证。人类与模型的交互可能是持续序列而非单一有限回合，因此需要无限时域建模。

Method: 将安全RLHF建模为无限时域折扣CMDP，提出两种基于原始-对偶方法的算法，无需奖励模型拟合，支持灵活轨迹长度训练。

Result: 两种算法在策略梯度迭代、轨迹样本长度和人类偏好查询方面实现了多项式速率的全局收敛保证，这是首个在人类反馈下研究无限时域折扣CMDP并建立全局非渐近收敛的工作。

Conclusion: 本文为安全RLHF提供了理论框架和收敛保证，首次在无限时域折扣CMDP下研究人类反馈问题，为开发安全可靠的语言模型提供了理论基础。
...
### [138] [Intentional Updates for Streaming Reinforcement Learning](https://arxiv.org/abs/2604.19033)
*Arsalan Sharifnassab,Mohamed Elsayed,Kris De Asis,A. Rupam Mahmood,Richard S. Sutton*

Main category: cs.LG

TL;DR: 论文提出"意图性更新"方法，在流式深度强化学习中通过先定义更新目标再求解步长，解决传统梯度学习中步长选择导致的稳定性问题。


<details>
  <summary>Details</summary>
Motivation: 传统梯度学习中，参数空间的步长选择无法预测函数输出的变化，在流式设置（批大小=1）中容易导致不稳定，因为随机性无法平均，更新幅度可能变得任意大或小。

Method: 提出意图性更新策略：先指定更新的预期结果，然后求解实现该结果的步长。扩展了在线监督线性回归中的归一化最小均方算法思想，应用于流式深度强化学习，定义了两种意图性更新：意图性TD（固定比例减少TD误差）和意图性策略梯度（限制策略的每步变化，约束局部KL散度）。结合资格迹和对角缩放提出了实用算法。

Result: 这些方法在流式设置中实现了最先进的性能，经常与批处理和回放缓冲方法表现相当。

Conclusion: 意图性更新方法为流式深度强化学习提供了一种稳定有效的学习策略，通过先定义更新目标再求解步长的方式，解决了传统梯度学习中的稳定性问题。
...
### [139] [Age-Dependent Heterogeneity in the Association Between Physical Activity and Mental Distress: A Causal Machine Learning Analysis of 3.2 Million U.S. Adults](https://arxiv.org/abs/2604.19066)
*Yuan Shan*

Main category: cs.LG

TL;DR: 研究发现体育锻炼对心理困扰的保护作用存在显著的年龄差异：年龄越大保护作用越强，而年轻人（18-24岁）的保护作用在过去十年中逐渐减弱甚至消失。


<details>
  <summary>Details</summary>
Motivation: 虽然体育锻炼被广泛认为对心理健康有保护作用，但这种益处是否在不同人口亚群中存在系统性差异尚不清楚。研究旨在探究休闲体育活动与频繁心理困扰之间的关系在不同年龄组中的异质性。

Method: 使用美国行为风险因素监测系统连续十年的数据（2015-2024年，n=3,242,218），采用调查加权逻辑回归分析体育锻炼与频繁心理困扰（每月≥14天）的关系。使用因果森林和双重机器学习识别治疗效果异质性，并进行E值敏感性分析、倾向得分重叠检查、安慰剂测试和插补比较以确保结果稳健性。

Result: 研究发现明显的年龄梯度：体育锻炼的调整后比值比从年轻人（18-24岁）的0.89到55-64岁成年人的0.50，保护作用随年龄增长单调增强。时间分析显示年轻人体育锻炼的保护效应在过去十年中逐渐减弱，18-24岁的比值比在2018年和2024年达到1.01（无效）。因果森林分析确定年龄是治疗效果异质性的主要驱动因素。

Conclusion: 研究结果表明，体育锻炼与心理健康之间公认的联系可能不适用于最年轻的成年人群，他们的心理困扰似乎越来越多地受到体育锻炼本身无法缓解的压力源驱动，这与日益严重的青年心理健康危机相平行。
...
### [140] [S2MAM: Semi-supervised Meta Additive Model for Robust Estimation and Variable Selection](https://arxiv.org/abs/2604.19072)
*Xuelin Zhang,Hong Chen,Yingjie Wang,Tieliang Gong,Bin Gu*

Main category: cs.LG

TL;DR: 提出S²MAM半监督元可加模型，通过双层优化自动识别信息变量、更新相似度矩阵，同时实现可解释预测，解决了传统流形正则化对相似度度量依赖和噪声变量敏感的问题。


<details>
  <summary>Details</summary>
Motivation: 传统半监督流形正则化方法严重依赖预定义的相似度度量，在处理冗余或噪声输入变量时可能导致不适当的惩罚，需要一种能自动识别信息变量并更新相似度矩阵的方法。

Method: 提出S²MAM半监督元可加模型，采用双层优化方案：上层优化变量选择和相似度矩阵，下层优化预测模型参数，实现自动识别信息变量、更新相似度矩阵和可解释预测。

Result: 在4个合成数据集和12个真实数据集上进行实验评估，涵盖不同水平和类型的噪声污染，验证了方法的鲁棒性和可解释性，并提供了计算收敛性和统计泛化界的理论保证。

Conclusion: S²MAM通过双层优化框架有效解决了传统流形正则化方法的局限性，实现了自动变量选择、相似度矩阵更新和可解释预测，为半监督学习提供了更鲁棒和可解释的解决方案。
...
TL;DR: 该论文提出SAFER框架，用于解决连续机器学习遗忘中的知识侵蚀和遗忘逆转问题，通过保持保留数据的表示稳定性和对遗忘数据施加负对数边际来实现稳定性能。


<details>
  <summary>Details</summary>
Motivation: 现有机器学习遗忘方法大多假设只进行一次遗忘操作，但实际应用中需要多次连续遗忘。在连续遗忘场景下，存在两个关键问题：1）知识侵蚀——保留数据的准确性随遗忘阶段逐渐下降；2）遗忘逆转——先前被遗忘的样本在后续阶段重新变得可识别。

Method: 提出SAFER（StAbility-preserving Forgetting with Effective Regularization）框架，这是一个连续遗忘框架，通过保持保留数据的表示稳定性，同时对遗忘数据施加负对数边际来维持模型性能。

Result: 大量实验表明，SAFER能够有效缓解知识侵蚀和遗忘逆转问题，在多个遗忘阶段中保持稳定的性能表现。

Conclusion: SAFER框架为解决连续机器学习遗忘中的关键挑战提供了有效方案，能够在保护隐私的同时维持AI系统的知识稳定性，为实现"被遗忘权"提供了更实用的技术路径。
...
TL;DR: 研究发现语言模型具有"谄媚电路"：当模型同意用户的错误信念时，它实际上能检测到错误但选择顺从。通过注意力头分析和干预实验，发现同一组注意力头控制着谄媚行为、事实性说谎和指令性说谎，而意见一致则使用正交方向。对齐训练虽然减少谄媚行为但保留了这个电路结构。


<details>
  <summary>Details</summary>
Motivation: 研究动机是探究语言模型在同意用户错误信念时的内在机制：究竟是模型未能检测到错误，还是检测到了但仍然选择同意。这个问题对于理解模型对齐、诚实性和谄媚行为具有重要意义。

Method: 研究方法包括：1）分析12个不同规模的开源模型；2）识别携带"此陈述错误"信号的注意力头；3）通过沉默这些注意力头来改变谄媚行为；4）使用边缘级路径修补技术验证电路；5）研究对齐训练（RLHF）对电路的影响。

Result: 研究发现：1）同一小组注意力头在不同模型中控制谄媚行为；2）沉默这些头能显著减少谄媚行为而不影响事实准确性；3）同一电路驱动谄媚行为、事实性说谎和指令性说谎；4）意见一致使用相同头位置但正交方向；5）对齐训练减少谄媚行为但保留电路结构。

Conclusion: 结论是语言模型具有专门的"谄媚电路"，当模型表现出谄媚行为时，它实际上知道用户是错误的但仍然选择同意。这个电路在模型架构中持续存在，即使经过对齐训练也不会被消除，只是行为表现被抑制。
...
TL;DR: Nexusformer通过非线性Nexus-Rank层替代传统注意力中的线性投影，实现无损结构化扩展，在语言建模和推理任务中比Tokenformer节省41.5%训练计算量。


<details>
  <summary>Details</summary>
Motivation: 传统Transformer架构在扩展时通常需要从头训练大模型，因为标准架构难以扩展而不丢弃已学习的表示。主要瓶颈在于注意力机制中的线性投影，它们将特征提取严格限制在固定维度的子空间中，限制了表达能力和增量扩展能力。

Method: 提出Nexusformer，用Nexus-Rank层替代传统的线性Q/K/V投影。Nexus-Rank是一个三阶段非线性映射，通过双激活函数在逐渐更高维度的空间中进行特征提取。这种设计克服了线性约束，并支持无损结构化增长：可以通过零初始化块沿两个轴注入新容量，同时保留预训练知识。

Result: 在语言建模和推理基准测试中，Nexusformer在渐进扩展过程中（240M到440M参数）达到与Tokenformer相同的困惑度，同时节省高达41.5%的训练计算量。对增长动态的分析表明，零初始化诱导了稳定的收敛轨迹，并推导出能够准确预测不同扩展尺度性能的几何缩放定律。

Conclusion: Nexusformer通过非线性注意力投影和结构化增长机制，有效解决了Transformer扩展中的瓶颈问题，实现了高效的无损模型扩展，为大规模语言模型的渐进式训练提供了新方法。
...
TL;DR: 该论文提出了一种适用于实际LLM服务的4位KV缓存量化方法，通过令牌级INT4量化和块对角哈达玛旋转，在保持服务效率的同时实现近乎无损的精度恢复。


<details>
  <summary>Details</summary>
Motivation: KV缓存内存是实际LLM服务中的主要瓶颈，现有压缩方法虽然能提高离线精度或压缩比，但往往违反实际服务约束（如分页内存布局、规则内存访问、融合注意力执行），限制了其部署效果。

Method: 采用令牌级INT4量化与块对角哈达玛旋转的简单设计，实现了融合旋转-量化内核，可直接集成到分页KV缓存布局中，引入零可测量的端到端开销。

Result: 该方法在多个模型和基准测试中，几乎完全恢复了朴素INT4丢失的精度，而更复杂的方法（如向量量化和Hessian感知量化）在考虑服务兼容性后仅提供边际额外增益。该方法匹配了朴素INT4的吞吐量。
...
<details>
  <summary>Details</summary>
Motivation: 在资源受限环境中部署大型语言模型面临计算和内存需求过高的挑战，需要开发高效的量化方法来降低模型复杂度。

Method: 采用三阶段量化策略：1) 通过PTQ初始化高质量量化模型；2) 通过层间蒸馏量化二值化权重、分组位图和量化参数，同时保持激活值为全精度；3) 训练可学习的激活量化因子，将激活动态量化为4位。

Result: 在W2A4量化设置下，LBLLM在语言建模、常识问答和语言理解任务上超越了现有最先进的二值化方法，且不需要额外的高精度通道或旋转矩阵。

Conclusion: LBLLM证明了极端低位量化在LLM中的实用性和高效性，为资源受限环境中的LLM部署提供了有前景的路径，其解耦设计提高了训练稳定性和推理准确性。
...
<details>
  <summary>Details</summary>
Motivation: 异质图多标签节点分类面临结构异质性和需要学习跨多个标签的共享表示的挑战。现有方法要么采用灵活的注意力机制，要么采用元路径约束锚定，但在异质多标签预测中往往遭受语义稀释或覆盖约束问题，这些问题在多标签监督下进一步加剧。
...
Conclusion: FOCAL通过融合覆盖和锚定学习，有效解决了异质图多标签节点分类中的语义稀释和覆盖约束问题，在异质图多标签预测任务上表现出优越性能。
...
### [147] [TEMPO: Scaling Test-time Training for Large Reasoning Models](https://arxiv.org/abs/2604.19295)
*Qingyang Zhang,Xinke Kong,Haitao Wu,Qinghua Hu,Minghao Wu,Baosong Yang,Yu Cheng,Yun Luo,Ganqu Cui,Changqing Zhang*

Main category: cs.LG

TL;DR: TEMPO是一个测试时训练框架，通过交替进行无标签问题的策略精炼和有标签数据集的评论家重新校准，解决了现有TTT方法在LRMs中性能停滞和多样性崩溃的问题。


<details>
  <summary>Details</summary>
Motivation: 现有测试时训练方法在大型推理模型中快速达到性能瓶颈，无法从额外的测试时计算中获益。由于缺乏外部校准，自生成奖励信号会随着策略模型演化而漂移，导致性能停滞和多样性崩溃。

Method: 提出TEMPO框架，交替进行：1）在无标签问题上进行策略精炼；2）在有标签数据集上进行周期性评论家重新校准。通过期望最大化算法形式化这一交替过程，将先前方法解释为省略关键重新校准步骤的不完整变体。

Result: TEMPO显著提升了模型性能：OLMO3-7B在AIME 2024上从33.0%提升到51.1%，Qwen3-14B从42.3%提升到65.8%，同时保持了高多样性。

Conclusion: 重新引入评论家重新校准步骤可以收紧证据下界，实现持续改进。TEMPO框架解决了测试时训练中的性能停滞问题，为大型推理模型的持续适应提供了有效方案。
...
### [148] [Debiased neural operators for estimating functionals](https://arxiv.org/abs/2604.19296)
*Konstantin Hess,Dennis Frauen,Niki Kilbertus,Stefan Feuerriegel*

Main category: cs.LG

TL;DR: DOPE是一种半参数估计器，用于从神经算子获得的解轨迹中估计标量目标量，解决了朴素插件估计的一阶偏差问题。


<details>
  <summary>Details</summary>
Motivation: 在许多应用中，目标不是恢复完整的解轨迹，而是通过标量目标量（如时间在目标范围内、超过阈值的时间、累积成本或总能量）来总结解轨迹。神经算子广泛用于近似复杂物理系统的解映射，但朴素插件估计可能存在偏差。

Method: 提出DOPE（去偏神经算子）：一种半参数估计器，通过新颖的一步Neyman正交估计器处理神经算子作为函数空间之间的高维干扰映射，并移除主导偏差项。DOPE使用加权机制同时考虑不规则观测设计以及目标量对底层轨迹扰动的敏感性。通过Riesz回归将自动去偏机器学习扩展到算子值干扰。

Result: DOPE在各种数值实验中表现出优势，能够有效处理部分和不规则观测，并可结合任意神经算子架构。

Conclusion: DOPE为从神经算子获得的解轨迹中估计标量目标量提供了一种有效的去偏估计方法，解决了朴素插件估计的偏差问题，具有广泛的适用性。
...
### [149] [On the Conditioning Consistency Gap in Conditional Neural Processes](https://arxiv.org/abs/2604.19312)
*Robin Young*

Main category: cs.LG

TL;DR: CNPs违反Kolmogorov一致性条件，但研究发现其不一致性随上下文大小呈O(1/n²)衰减，在中等规模上下文中可忽略，但在少样本场景下显著。


<details>
  <summary>Details</summary>
Motivation: 神经过程(NPs)作为元学习模型，虽然受随机过程启发，但通常不满足Kolmogorov一致性条件。这种不一致性被广泛认知但缺乏量化理解，需要建立CNPs近似有效随机过程的精确意义。

Method: 定义条件一致性差距作为KL散度，衡量CNP预测在向上下文添加点与条件化时的变化。对有界编码器和Lipschitz解码器的CNPs，分析一致性差距随上下文大小n的渐近行为。

Result: 对于有界编码器和Lipschitz解码器的CNPs，一致性差距为O(1/n²)，且该速率是紧的。不一致性在中等上下文规模下可忽略，但在少样本场景中可能显著。

Conclusion: CNPs确实违反Kolmogorov一致性，但以可量化的方式近似有效随机过程。不一致性随上下文大小快速衰减，为CNPs的理论理解和实际应用提供了重要洞见。
...
### [150] [RDP LoRA: Geometry-Driven Identification for Parameter-Efficient Adaptation in Large Language Models](https://arxiv.org/abs/2604.19321)
*Yusuf Çelebi,Yağız Asker,Özay Ezerceli,Mahmoud ElHussieni,Selva Taş,Reyhan Bayraktar,Fatma Betül Terzioğlu*

Main category: cs.LG

TL;DR: 使用Ramer-Douglas-Peucker算法分析LLM隐藏状态几何轨迹，识别关键转折点作为层选择信号，在Qwen3-8B-Base上仅微调13个关键层就超越了全层微调效果


<details>
  <summary>Details</summary>
Motivation: 尽管有LoRA等参数高效微调方法，但LLM内部表示在层级的角色仍不明确，导致层选择依赖启发式方法。需要理解隐藏状态演化的几何结构来指导层选择

Method: 将隐藏状态演化建模为高维几何轨迹，使用RDP算法（参数无关、无需训练的多边形简化方法）识别表示路径上的关键转折点，将这些几何转折点作为层选择信号，仅微调RDP选择的13个关键层

Result: 在Qwen3-8B-Base上，仅微调RDP选择的13个关键层（占总层36层的36%）在MMLU-Math上达到81.67%，显著优于全36层微调（79.32%）、随机13层选择（75.56%）和基线模型（74.25%）

Conclusion: 利用表示轨迹的内在几何结构为模型适配中的层选择提供了鲁棒、可解释且无需训练的信号，几何感知的层选择策略能显著提升参数高效微调的效果
...
### [151] [When Active Learning Falls Short: An Empirical Study on Chemical Reaction Extraction](https://arxiv.org/abs/2604.19335)
*Simin Yu,Sufia Fathima*

Main category: cs.LG

TL;DR: 本文系统研究了主动学习在化学反应提取中的应用，评估了六种不确定性/多样性策略与预训练transformer-CRF架构的结合效果，发现虽然部分方法能用较少标注数据接近全数据性能，但学习曲线常非单调且任务依赖性强。
...
Result: 虽然几种方法能用较少的标注实例接近全数据性能，但学习曲线通常是非单调的且任务依赖性很强。分析表明，强大的预训练、结构化CRF解码和标签稀疏性限制了传统主动学习策略的稳定性。

Conclusion: 这些发现为在化学信息提取中有效使用主动学习提供了实用见解，强调了需要考虑预训练强度、解码结构和标签分布等因素对主动学习效果的影响。
...
### [152] [Evaluation-driven Scaling for Scientific Discovery](https://arxiv.org/abs/2604.19341)
...
TL;DR: SimpleTES框架通过规模化评估驱动发现循环，在多个科学领域实现突破性发现，显著提升LLM驱动的科学发现效率


<details>
  <summary>Details</summary>
Motivation: 现有研究虽强调评估的重要性，但未系统解决如何规模化评估驱动发现循环以推动科学发现边界的问题

Method: 提出SimpleTES框架，结合并行探索、反馈驱动优化和局部选择策略，系统化扩展评估驱动的发现循环

Result: 在6个领域21个科学问题上，SimpleTES使用开源模型发现最先进解决方案，性能超越前沿模型基准和复杂优化流程

Conclusion: 有效规模化评估驱动循环是推进LLM驱动科学发现的关键轴心，SimpleTES为实现这一目标提供了简单实用的框架
...
### [153] [LASER: Learning Active Sensing for Continuum Field Reconstruction](https://arxiv.org/abs/2604.19355)
*Huayu Deng,Jinghui Zhong,Xiangming Zhu,Yunbo Wang,Xiaokang Yang*

Main category: cs.LG

TL;DR: LASER：一种基于强化学习的主动感知框架，通过部分可观测马尔可夫决策过程实现连续物理场的自适应高保真重建
...
Conclusion: LASER提供了一个统一的闭环框架，通过自适应传感器布局优化，能够有效解决稀疏传感条件下的连续物理场重建问题，超越了传统固定布局方法的局限性。
...
### [154] [FairTree: Subgroup Fairness Auditing of Machine Learning Models with Bias-Variance Decomposition](https://arxiv.org/abs/2604.19357)
*Rudolf Debelak*

Main category: cs.LG

TL;DR: FairTree是一种基于心理测量不变性测试的新算法，用于检测机器学习模型在不同子组中的性能差异，能直接处理连续、分类和有序特征，并将性能差异分解为系统偏差和方差。


<details>
  <summary>Details</summary>
Motivation: 当前机器学习模型评估主要依赖基于损失函数的性能指标，这可能会忽视相关子组中的性能变化。现有的审计工具如SliceFinder和SliceLine存在概念上的缺陷，特别是无法直接处理连续协变量。

Method: FairTree算法改编自心理测量不变性测试，能直接处理连续、分类和有序特征而无需离散化。算法将性能差异分解为系统偏差和方差，并提出两种变体：基于置换的方法（概念上更接近SliceFinder）和波动测试。

Result: 通过模拟研究（包括与SliceLine的直接比较）表明，两种方法都有令人满意的假阳性率，但波动测试方法具有相对更高的统计功效。在UCI成人人口普查数据集上的应用进一步验证了方法的有效性。
...
<details>
  <summary>Details</summary>
Motivation: 当前图表示学习方法虽然成功，但节点表示难以解释。现有解释方法主要关注监督设置或单个维度解释，缺乏对节点表示整体结构的解释。

Method: 提出TACENR方法，基于对比学习学习表示空间中的相似性函数，识别对节点表示最重要的属性、邻近性和结构特征。

Result: 实验结果表明邻近性和结构特征在塑造节点表示中起重要作用，监督变体在识别最重要特征方面与现有任务特定方法表现相当。

Conclusion: TACENR填补了图表示解释的关键空白，能够有效解释节点表示的整体结构，既适用于任务无关场景，也可扩展到监督设置。
...
### [156] [Revisiting Catastrophic Forgetting in Continual Knowledge Graph Embedding](https://arxiv.org/abs/2604.19401)
...
<details>
  <summary>Details</summary>
Motivation: 现有连续知识图谱嵌入方法主要关注限制已有嵌入的改动来应对灾难性遗忘，但忽略了新引入实体对已有实体嵌入的干扰问题，导致评估结果存在系统性高估。

Method: 提出修正的CKGE评估协议，考虑实体干扰效应；引入专门针对CKGE的灾难性遗忘度量指标；在多个基准数据集上进行实验验证。

Result: 忽略实体干扰会导致性能高估达25%，特别是在实体显著增长的情况下；分析了不同CKGE方法和KGE模型受各种遗忘源的影响程度。

Conclusion: 实体干扰是CKGE中不可忽视的重要现象，需要专门的评估协议和度量指标来准确评估灾难性遗忘，这对未来CKGE方法的发展具有重要意义。
...
### [157] [Heterogeneity-Aware Personalized Federated Learning for Industrial Predictive Analytics](https://arxiv.org/abs/2604.19451)
*Yuhan Hu,Xiaolei Fang*

Main category: cs.LG

TL;DR: 提出个性化联邦预测模型，解决传统联邦学习中退化过程同质性假设不成立的问题，通过相似退化模式客户端间的协作实现个性化预测，同时保护数据隐私。


<details>
  <summary>Details</summary>
Motivation: 传统联邦预测模型假设各客户端的退化过程是同质的，但实际工业场景中退化过程往往是异质的，需要能够适应不同退化模式的个性化模型。

Method: 提出个性化联邦预测模型，基于近端梯度下降开发联邦参数估计算法，促进具有相似退化模式的客户端之间的成对协作，实现个性化联邦学习。

Result: 通过广泛的仿真研究和NASA涡轮风扇发动机退化数据集的案例研究验证了所提模型的优越性，能够同时实现模型个性化、数据隐私保护和完整故障时间分布预测。

Conclusion: 该个性化联邦预测模型克服了现有联邦预测模型的局限性，为具有异质退化过程的工业场景提供了有效的故障时间预测解决方案。
...
### [158] [EVPO: Explained Variance Policy Optimization for Adaptive Critic Utilization in LLM Post-Training](https://arxiv.org/abs/2604.19485)
...
TL;DR: 该论文提出EVPO方法，通过监控解释方差来自适应选择基于评论家或批平均的优势估计，在稀疏奖励场景中优于PPO和GRPO。


<details>
  <summary>Details</summary>
Motivation: 强化学习用于LLM后训练面临基本设计选择：是否使用学习的评论家作为策略优化的基线。经典理论支持基于评论家的方法（如PPO）以减少方差，但无评论家方法（如GRPO）因简单性和竞争性能而广泛采用。作者发现在稀疏奖励设置中，学习的评论家可能引入超过其捕获状态信号的估计噪声，反而增加优势方差。

Method: 将基线选择建模为卡尔曼滤波问题，将PPO和GRPO统一为卡尔曼增益的两个极端。证明解释方差（EV）可从单个训练批次计算，并识别确切边界：正EV表示评论家减少方差，零或负EV表示评论家增加方差。基于此提出解释方差策略优化（EVPO），在每个训练步骤监控批次级EV，自适应地在基于评论家和批平均优势估计之间切换。

Result: 在涵盖经典控制、智能体交互和数学推理的四个任务中，EVPO始终优于PPO和GRPO，无论哪种固定基线在特定任务中更强。进一步分析确认自适应门控跟踪评论家在训练过程中的成熟度，且理论推导的零阈值在经验上是最优的。

Conclusion: 通过将基线选择形式化为卡尔曼滤波问题，提出了EVPO方法，该方法自适应地选择优势估计方法，在稀疏奖励场景中提供理论保证的方差减少，并在多个任务中实现一致优于固定基线方法的性能。
...
### [159] [When Graph Structure Becomes a Liability: A Critical Re-Evaluation of Graph Neural Networks for Bitcoin Fraud Detection under Temporal Distribution Shift](https://arxiv.org/abs/2604.19514)
*Saket Maganti*

Main category: cs.LG

TL;DR: 研究发现，在无数据泄露的严格评估协议下，GNN模型在Elliptic比特币数据集上的表现共识被推翻，随机森林等特征模型优于所有GNN模型


<details>
  <summary>Details</summary>
Motivation: 检验广泛引用的共识（GCN、GraphSAGE、GAT、EvolveGCN在Elliptic比特币数据集上优于纯特征基线）是否在无泄露的严格评估协议下仍然成立

Method: 采用种子匹配的归纳vs转导比较，实施严格归纳协议，进行配对控制实验，以及边洗牌消融实验

Result: 在严格归纳协议下，随机森林在原始特征上达到F1=0.821，优于所有评估的GNN；GraphSAGE仅达到F1=0.689；配对实验显示39.5分的F1差距归因于训练时暴露测试期邻接关系；随机连边图优于真实交易图

Conclusion: 现有关于GNN在Elliptic数据集上优于特征模型的共识不成立，数据集拓扑在时间分布偏移下可能产生误导，需要采用严格的归纳评估协议
...
### [160] [Revisiting RaBitQ and TurboQuant: A Symmetric Comparison of Methods, Theory, and Experiments](https://arxiv.org/abs/2604.19528)
*Jianyang Gao,Yutong Gou,Yuexuan Xu,Jifan Shi,Yongyi Yang,Shuolin Li,Raymond Chi-Wing Wong,Cheng Long*

Main category: cs.LG

TL;DR: 本文重新评估了RaBitQ和TurboQuant两种量化方法，在统一框架下比较发现TurboQuant并未提供相对于RaBitQ的一致改进，且在多数测试配置中表现更差，同时指出TurboQuant论文中的部分实验结果存在可复现性问题。


<details>
  <summary>Details</summary>
Motivation: 重新审视RaBitQ和TurboQuant两种量化方法之间的关系，在统一、可复现、透明的比较框架下澄清两种方法的真实差异，并验证TurboQuant论文中声称的优势是否成立。

Method: 建立统一、对称、透明的比较框架，从方法论、理论保证和实证性能三个维度对两种方法进行系统比较，使用可复现的实验设置验证结果。

Result: TurboQuant并未提供相对于RaBitQ的一致改进，在多数测试配置中表现更差；TurboQuant论文中报告的若干运行时和召回率结果无法从其发布的实现中复现；两种方法在共享结构和真实差异方面得到澄清。

Conclusion: TurboQuant相对于RaBitQ的优势并未得到证实，且其论文中的实验结果存在可复现性问题；研究为两种量化方法的比较提供了更清晰、客观的评估框架。
...
### [161] [Calibrating Scientific Foundation Models with Inference-Time Stochastic Attention](https://arxiv.org/abs/2604.19530)
*Akash Yadav,Taiwo A. Adebiyi,Ruda Zhang*

Main category: cs.LG

TL;DR: 提出Stochastic Attention方法，通过随机化注意力机制实现预测不确定性校准，无需重新训练即可生成预测集合


<details>
  <summary>Details</summary>
Motivation: 当前基于Transformer的科学基础模型在关键应用中缺乏校准的预测不确定性支持，现有架构输出确定性结果且不确定性校准有限

Method: 提出Stochastic Attention方法，在推理时用归一化多项分布样本替换softmax权重，通过单一浓度参数控制随机化；引入校准目标匹配随机注意力输出与目标，形成高效的单变量后调优问题

Result: 在天气和时间序列预测的科学基础模型以及回归任务上，相比不确定性感知基线，Stochastic Attention实现了最强的原生校准和最锐利的预测区间，在可比覆盖率下仅需几分钟后调优，而竞争基线需要数天重新训练

Conclusion: Stochastic Attention为Transformer基础模型提供了一种轻量级、高效的不确定性校准方法，显著提升了预测可靠性和实用性
...
### [162] [Separating Geometry from Probability in the Analysis of Generalization](https://arxiv.org/abs/2604.19560)
*Maxim Raginsky,Benjamin Recht*

Main category: cs.LG

TL;DR: 该论文提出了一种基于优化问题敏感性分析的确定性泛化分析框架，替代传统依赖i.i.d.统计假设的方法


<details>
  <summary>Details</summary>
Motivation: 传统机器学习泛化分析依赖于无法验证的i.i.d.统计假设，需要一种更基础的确定性分析框架

Method: 通过优化问题解对问题数据扰动的敏感性分析，建立变分原理形式的确定性泛化边界

Result: 获得了不依赖概率假设的泛化边界，将样本内外性能差异量化为样本间接近程度的误差项

Conclusion: 提供了一种纯确定性的泛化分析视角，统计假设可作为后验工具用于分析误差项的大小特性
...
### [163] [Structure-guided molecular design with contrastive 3D protein-ligand learning](https://arxiv.org/abs/2604.19562)
*Carles Navarro,Philipp Tholke,Gianni de Fabritiis*

Main category: cs.LG

TL;DR: 提出统一框架结合对比3D结构编码与基于商业化合物空间的自回归分子生成，解决蛋白质-配体相互作用建模与超大化学空间探索的双重挑战


<details>
  <summary>Details</summary>
Motivation: 基于结构的药物发现面临两大挑战：准确捕捉3D蛋白质-配体相互作用，以及在超大化学空间中寻找可合成候选化合物。需要同时解决结构建模和化学空间探索的问题。
...
Result: 1. 在零样本虚拟筛选中取得竞争性结果；2. 生成具有良好预测结合特性的候选分子，适用于多种靶点；3. 能够生成针对商业化合物空间的可合成候选物。

Conclusion: 该统一框架成功解决了基于结构的药物发现中的双重挑战，通过结合3D结构编码和条件分子生成，能够高效探索化学空间并产生具有良好结合特性的可合成候选化合物。
...
### [164] [An Efficient Black-Box Reduction from Online Learning to Multicalibration, and a New Route to $Φ$-Regret Minimization](https://arxiv.org/abs/2604.19592)
*Gabriele Farina,Juan Carlos Perdomo*

Main category: cs.LG

TL;DR: 本文提出了从在线学习到在线多校准的GGM风格黑盒归约，解决了Garg等人的主要开放问题，并建立了外部遗憾到Φ-遗憾的新路径。


<details>
  <summary>Details</summary>
Motivation: 解决Garg等人在SODA'24中提出的主要开放问题：是否能在完全一般性下实现具有√T型保证的预言机高效在线多校准。同时简化Daskalakis等人在STOC'25中的结果并改进速率。

Method: 1. 将任何无遗憾学习器与期望变分不等式(EVI)求解器结合，实现高维多校准；2. 证明高效多校准意味着高效EVI求解；3. 建立从高维在线多校准到(上下文)Φ-遗憾最小化的细粒度归约。

Result: 1. 证明了预言机高效在线多校准在完全一般性下具有√T型保证是可能的；2. 统一了现有在线多校准算法的分析；3. 为延迟观测或审查结果环境提供了新算法；4. 建立了在线学习与多类全预测之间的首个高效黑盒归约；5. 简化了Daskalakis等人的结果并改进了速率。

Conclusion: 本文通过GGM风格归约解决了在线多校准的关键开放问题，建立了外部遗憾到Φ-遗憾的新路径，统一了现有算法分析，并为挑战性环境提供了新算法，显著推进了在线学习和多校准的理论基础。
...
### [165] [SAGE: Training-Free Semantic Evidence Composition for Edge-Cloud Inference under Hard Uplink Budgets](https://arxiv.org/abs/2604.19623)
*Inhyeok Choi,Hyuncheol Park*

Main category: cs.LG

TL;DR: SAGE方法通过结合重要性过滤和嵌入多样性采样，在硬比特预算下优化边缘-云协同推理，相比仅基于注意力的方法显著提升精度。


<details>
  <summary>Details</summary>
Motivation: 边缘-云混合推理中，上行信道对传输比特数有硬约束，而现有基于注意力重要性的方法在这种硬预算下存在固有局限性。

Method: 提出SAGE方法：结合重要性过滤和嵌入多样性采样，无需训练，在硬比特预算下优化传输内容选择。

Result: 在ImageNet-1K上，SAGE达到服务器上限精度的93%，同时传输少于一半的证据单元，显著优于仅基于重要性的方法。

Conclusion: 硬预算下，传输内容的选择不仅应考虑重要性，更应关注多样性和空间覆盖度，SAGE方法为此提供了有效的解决方案。
...
### [166] [Disentangling Damage from Operational Variability: A Label-Free Self-Supervised Representation Learning Framework for Output-Only Structural Damage Identification](https://arxiv.org/abs/2604.19658)
*Xudong Jian,Charikleia Stoura,Simon Scandella,Eleni Chatzi*

Main category: cs.LG

TL;DR: 提出自监督无标签解耦表示学习框架，用于鲁棒的振动结构损伤识别，通过VICReg正则化和频域约束实现损伤特征敏感而干扰因素不变的表示学习。
...
Method: 采用具有两个潜在表示的自编码器直接从原始振动加速度信号学习。通过VICReg自监督不变性正则化使一个潜在表示对非损伤因素保持不变，同时引入频域约束确保重构功率谱密度与输入时间序列一致，促进表示解耦。

Result: 在两个真实振动数据集（桥梁和齿轮箱）上验证了有效性，结果显示对操作变异性具有鲁棒性，强泛化能力，在损伤检测和量化方面表现良好。

Conclusion: 提出的自监督无标签解耦表示学习框架能够有效分离损伤相关特征和干扰因素，实现鲁棒的结构损伤识别，适用于实际应用场景。
...
### [167] [Budgeted Online Influence Maximization](https://arxiv.org/abs/2604.19672)
*Pierre Perrault,Jennifer Healey,Zheng Wen,Michal Valko*

Main category: cs.LG

TL;DR: 提出了一种新的预算约束在线影响力最大化框架，用广告活动总成本替代传统的影响力者数量约束，更贴合现实场景中影响力者成本各异的情况。


<details>
  <summary>Details</summary>
Motivation: 传统在线影响力最大化通常使用影响力者数量作为约束条件，但现实中不同影响力者的成本差异很大，广告主更关心如何在总预算内获得最佳价值。现有方法未能充分建模这种成本变化的情况。

Method: 提出了一种算法，假设独立级联扩散模型和边级半赌博反馈，在预算约束而非数量约束下进行在线影响力最大化。

Result: 提供了理论和实验结果，证明该方法在预算约束下有效。同时，该方法也适用于传统的数量约束场景，并在该场景下改进了现有的遗憾边界。

Conclusion: 预算约束框架比传统的数量约束更能准确建模现实世界的社会广告场景，提出的算法在理论和实验上都表现出色，并为传统数量约束问题提供了更好的解决方案。
...
### [168] [PREF-XAI: Preference-Based Personalized Rule Explanations of Black-Box Machine Learning Models](https://arxiv.org/abs/2604.19684)
*Salvatore Greco,Jacek Karolczak,Roman Słowiński,Jerzy Stefanowski*

Main category: cs.LG

TL;DR: 论文提出PREF-XAI框架，将解释视为基于用户偏好的决策问题，结合规则解释与偏好学习，通过有限反馈准确重建用户偏好并发现新解释规则。


<details>
  <summary>Details</summary>
Motivation: 现有可解释AI主要关注模型中心解释，忽略了不同用户因目标、偏好和认知约束需要不同解释这一根本问题。虽然已有用户中心和个性化解释研究，但大多依赖启发式调整或隐式用户建模，缺乏表示和学习个体偏好的原则性框架。

Method: 提出PREF-XAI框架，将解释重构为偏好驱动的决策问题。结合基于规则的解释与形式化偏好学习，通过用户对少量候选解释的排序来获取偏好，使用鲁棒序数回归推断加性效用函数来建模用户偏好。

Result: 在真实数据集上的实验表明，PREF-XAI能够从有限反馈中准确重建用户偏好，识别高度相关的解释，并发现用户最初未考虑的新解释规则。

Conclusion: PREF-XAI建立了可解释AI与偏好学习之间的连接，为交互式和自适应解释系统开辟了新方向，强调解释应作为根据用户特定标准评估和选择的备选方案。
...
### [169] [Planning in entropy-regularized Markov decision processes and games](https://arxiv.org/abs/2604.19695)
...
Method: 利用熵正则化促进的贝尔曼算子平滑性，设计新的规划算法SmoothCruiser，在生成模型环境下进行价值函数估计

Result: SmoothCruiser实现了与问题无关的O~(1/ε⁴)样本复杂度，显著优于非正则化设置中的算法性能

Conclusion: 熵正则化通过平滑贝尔曼算子，使得在MDP和博弈中实现多项式样本复杂度的价值函数估计成为可能，SmoothCruiser算法为此提供了有效解决方案
...
### [170] [On two ways to use determinantal point processes for Monte Carlo integration](https://arxiv.org/abs/2604.19698)
*Guillaume Gautier,Rémi Bardenet,Michal Valko*

Main category: cs.LG

TL;DR: 论文研究了使用行列式点过程（DPP）替代独立采样来改进蒙特卡洛积分估计器，分析了两种现有DPP估计器的性能差异。


<details>
  <summary>Details</summary>
Motivation: 标准蒙特卡洛估计器使用独立样本，方差为1/N。使用具有排斥性质的行列式点过程（DPP）作为采样分布可以改进估计器性能，但现有DPP估计器各有局限：一种依赖于固定DPP，另一种需要针对特定函数定制DPP。

Method: 重新审视两种现有DPP估计器：Bardenet & Hardy（2020）的固定DPP估计器和Ermakov & Zolotukhin（1960）的定制DPP估计器。将这两种估计器推广到连续设置，并提供相应的采样算法。

Result: Bardenet & Hardy的估计器对于光滑函数f能达到O(N^{-(1+1/d)})的方差收敛速率，优于蒙特卡洛的1/N，但依赖于固定DPP。Ermakov & Zolotukhin的估计器是无偏的，收敛速率为1/N（与蒙特卡洛相同），但需要针对特定函数f定制DPP。

Conclusion: DPP作为排斥分布可以改进蒙特卡洛积分估计，但需要在固定DPP的通用性和定制DPP的精确性之间权衡。论文通过推广现有估计器到连续设置并提供采样算法，为DPP在积分估计中的应用提供了更完整的框架。
...
### [171] [Ultrametric OGP - parametric RDT \emph{symmetric} binary perceptron connection](https://arxiv.org/abs/2604.19712)
*Mihailo Stojnic*

Main category: cs.LG

TL;DR: 本文通过将参数化RDT与重叠间隙性质（OGPs）联系起来，研究了对称二元感知机的统计计算间隙。作者开发了分析联合边界程序，数值计算了ultrametric OGP的约束密度上限，发现与参数化RDT估计值高度一致，并提出了连接这两个框架的猜想。


<details>
  <summary>Details</summary>
Motivation: 先前研究引入了fl-RDT框架来表征统计计算间隙（SCGs），并在对称二元感知机中获得了算法阈值估计。本文旨在进一步连接参数化RDT与重叠间隙性质（OGPs），这是解空间的另一个关键几何特征，以更深入地理解统计计算间隙。

Method: 作者考虑了s级超度量OGPs（ult_s-OGPs），并严格上界了相关的约束密度α_{ult_s}。为此，他们开发了一个包含组合和概率组件的分析联合边界程序：将组合部分转化为凸问题，概率部分转化为嵌套积分，然后进行数值评估。

Result: 数值评估显示，前两个级别的最紧边界（ᾱ_{ult_1} ≈ 1.6578 和 ᾱ_{ult_2} ≈ 1.6219）与第三和第四提升级别的参数化RDT估计值（α_c^{(3)} ≈ 1.6576 和 α_c^{(4)} ≈ 1.6218）高度接近。在其他关键参数（包括重叠值和超度量簇的相对大小）上也观察到极好的一致性。

Conclusion: 基于这些观察，作者提出了几个连接ult-OGP和参数化RDT的猜想：算法阈值α_a等于这两个框架在极限情况下的值，且α_{ult_s} ≤ α_c^{(s+2)}（可能对某些或所有s相等）。最后讨论了连接这两个框架所有关键参数的完全同构的可能性。
...
### [172] [Adaptive MSD-Splitting: Enhancing C4.5 and Random Forests for Skewed Continuous Attributes](https://arxiv.org/abs/2604.19722)
*Jake Lee*

Main category: cs.LG

TL;DR: AMSD-Splitting通过动态调整标准差乘数来改进MSD-Splitting，解决了偏态分布数据中的信息损失问题，并在随机森林中实现高效高精度分类。


<details>
  <summary>Details</summary>
Motivation: 连续数值属性的离散化是决策树构建中的计算瓶颈。MSD-Splitting虽然提高了C4.5算法的效率和准确性，但其固定的标准差截断在高度偏态分布数据中会导致灾难性信息损失，这在生物医学和金融数据集中很常见。

Method: 提出自适应MSD-Splitting（AMSD），根据特征偏度动态调整标准差乘数，在密集区域缩小间隔以保持判别分辨率。将AMSD集成到集成方法中，特别是提出了Random Forest-AMSD（RF-AMSD）框架。

Result: 在Census Income、Heart Disease、Breast Cancer和Forest Covertype数据集上的实证评估显示，AMSD比标准MSD-Splitting提高了2-4%的准确率，同时保持了与O(N log N)穷举搜索相比的O(N)时间复杂度减少。随机森林扩展以标准计算成本的一小部分实现了最先进的准确率。

Conclusion: 自适应统计分箱在大规模集成学习架构中是可行的，AMSD在保持计算效率的同时显著提高了偏态分布数据的分类准确性。
...
### [173] [Benign Overfitting in Adversarial Training for Vision Transformers](https://arxiv.org/abs/2604.19724)
*Jiaming Zhang,Meng Ding,Shaopeng Fu,Jingfeng Zhang,Di Wang*

Main category: cs.LG

TL;DR: 首次对简化ViT架构下的对抗训练进行理论分析，证明在特定条件下对抗训练能使ViT实现近乎零的鲁棒训练损失和泛化误差，甚至在过拟合时也能保持强泛化能力。


<details>
  <summary>Details</summary>
Motivation: 尽管ViT在各种视觉任务中取得了显著成功，但研究表明它们与CNN一样容易受到对抗样本的攻击。对抗训练是一种常见的防御策略，但其在ViT中的鲁棒性理论基础尚未得到充分探索。

Method: 对简化ViT架构进行理论分析，研究在满足特定信噪比条件和适度扰动预算下，对抗训练对ViT性能的影响。同时在合成和真实数据集上进行实验验证。

Result: 理论分析表明，在特定条件下，对抗训练能使ViT实现近乎零的鲁棒训练损失和泛化误差。即使在过拟合情况下也能保持强泛化能力，这种现象被称为"良性过拟合"。实验验证了理论发现。

Conclusion: 这是首次对ViT对抗训练的理论分析，证明了在适当条件下对抗训练能为ViT提供强大的鲁棒性，甚至在过拟合时也能保持良好泛化，扩展了先前仅在CNN中观察到的"良性过拟合"现象。
...
### [174] [FB-NLL: A Feature-Based Approach to Tackle Noisy Labels in Personalized Federated Learning](https://arxiv.org/abs/2604.19729)
*Abdulmoneam Ali,Ahmed Arafa*

Main category: cs.LG

TL;DR: FB-NLL是一个特征中心的个性化联邦学习框架，通过谱结构分析用户特征空间进行一次性聚类，并引入特征一致性检测校正噪声标签，显著降低通信开销并提升性能稳定性。
...
Method: FB-NLL采用特征中心框架：1）通过特征表示的协方差谱结构表征用户，利用子空间相似性进行一次性几何感知聚类；2）引入基于特征一致性的检测校正策略，通过特征空间方向对齐和类特定特征子空间分配标签来缓解噪声监督。

Result: 在多种数据集和噪声机制下的广泛实验表明，FB-NLL在平均准确率和性能稳定性方面始终优于最先进的基线方法。

Conclusion: FB-NLL通过解耦用户聚类与训练动态，提供了一种标签无关、模型无关的个性化联邦学习框架，能够有效处理数据异质性和噪声标签问题，同时显著降低通信和计算成本。
...
### [175] [FASTER: Value-Guided Sampling for Fast RL](https://arxiv.org/abs/2604.19730)
*Perry Dong,Alexander Swerdlow,Dorsa Sadigh,Chelsea Finn*

Main category: cs.LG

TL;DR: FASTER是一种轻量级方法，通过在去噪过程中早期预测动作候选的下游价值来筛选动作，避免了基于采样的测试时扩展方法的高计算成本。


<details>
  <summary>Details</summary>
Motivation: 当前性能最好的强化学习算法通常使用测试时扩展方法（如采样多个动作候选并选择最佳），但这些方法计算成本过高。需要一种既能获得采样优势又避免高计算成本的方法。

Method: 将基于扩散策略的多动作候选去噪和选择过程建模为马尔可夫决策过程（MDP），目标是在去噪完成前逐步筛选动作候选。学习去噪空间中的策略和价值函数，预测动作候选在去噪过程中的下游价值并进行筛选，同时最大化回报。

Result: 在具有挑战性的长时程操作任务中，FASTER持续改进基础策略，在在线和批量在线强化学习中取得了最佳整体性能。应用于预训练的VLA时，FASTER实现了相同性能，同时大幅减少了训练和推理计算需求。

Conclusion: FASTER提供了一种轻量级方法，能够在保持性能的同时显著降低基于采样的测试时扩展方法的计算成本，可轻松集成到现有的生成式强化学习算法中。
...
### [176] [Safe Continual Reinforcement Learning in Non-stationary Environments](https://arxiv.org/abs/2604.19737)
*Austin Coursey,Abel Diaz-Gonzalez,Marcos Quinones-Grueiro,Gautam Biswas*

Main category: cs.LG

TL;DR: 论文系统研究了安全持续强化学习，提出了三个基准环境来评估现有方法，发现安全约束与防止灾难性遗忘之间存在根本性矛盾，并探索了正则化策略来缓解这一权衡。


<details>
  <summary>Details</summary>
Motivation: 传统强化学习控制器假设环境是静态的，但在现实世界中系统动态和操作条件会意外变化。同时，物理环境中的RL控制器必须在学习和执行阶段都满足安全约束。虽然持续RL和安全RL分别解决了非平稳性和安全性问题，但两者的交叉领域研究较少，需要开发能够在系统生命周期中适应变化同时保持安全的安全持续RL算法。

Method: 1. 引入三个基准环境来捕捉安全关键的持续适应场景；2. 评估来自安全RL、持续RL及其组合的代表性方法；3. 研究正则化策略来缓解安全约束与防止灾难性遗忘之间的权衡；4. 分析这些策略的优缺点和局限性。

Result: 实证结果表明，在非平稳动态下，维持安全约束与防止灾难性遗忘之间存在根本性矛盾。现有方法通常无法同时实现这两个目标。正则化策略能够部分缓解这种权衡，但仍存在局限性。

Conclusion: 需要开发能够在变化环境中持续自主运行的安全、有韧性的学习型控制器。论文指出了关键开放挑战和研究方向，包括如何更好地平衡安全性与适应性，以及开发更有效的算法来解决安全持续强化学习问题。
