...
Method: 1. 构建CMER-Bench基准测试，将表达式分为简单、中等、复杂三个难度级别；2. 创建大规模数据集MER-17M和CMER-3M，重点包含复杂数学表达式样本；3. 提出结构化数学语言表示方法，显式建模表达式的层次和空间结构；4. 基于编码器-解码器架构开发CMERNet模型，在CMER-3M数据集上进行训练。

Result: 实验结果显示，现有MER模型和通用多模态大语言模型在简单和中等难度表达式上表现良好，但在复杂表达式上性能显著下降。相比之下，仅包含1.25亿参数的CMERNet在CMER-Bench基准测试中显著优于现有MER模型和MLLMs。

Conclusion: 该研究通过构建专门的基准测试、大规模数据集、新的表示方法和专用模型，有效解决了复杂数学表达式识别的挑战。CMERNet在保持较小模型规模的同时，在复杂数学表达式识别任务上取得了显著性能提升，为数学表达式识别领域的发展提供了重要贡献。
...
TL;DR: 本文探讨了AIGC辅助新闻图像生产的可控性路径，通过两个实验构建了人机协同的模块化流程，并提出评估指标以解决新闻业中的真实性、语义对齐和伦理问题。


<details>
  <summary>Details</summary>
Motivation: AIGC辅助图像生产在新闻业引发争议，主要问题包括虚假信息、真实性、语义保真度和可解释性。大多数AIGC工具是"黑箱"，阻碍了内容准确性和语义对齐的双重要求，并引发伦理、社会技术和信任困境。

Method: 进行两个实验：实验1通过标准化提示词测试三个场景的跨平台适应性；实验2构建人机协同的模块化流程，结合高精度分割、语义对齐和风格调节技术，通过CLIP语义评分、NSFW/OCR/YOLO过滤和可验证内容凭证确保编辑保真度。

Result: 实验1揭示了由训练语料偏见和平台级过滤驱动的语义对齐、文化特异性和视觉真实性的差异；实验2实现了可追溯部署并保留了语义表征，构建了可控的图像生产流程。

Conclusion: 提出了新闻特殊报道中AIGC辅助图像生产的人机协同机制，并建议评估角色身份稳定性、文化表达准确性和用户-公众适宜性三个指标，为新闻业AIGC应用提供了可控性解决方案。
...
TL;DR: 该研究提出了一个结合深度学习图像分类器和大语言模型的框架，用于生成胃肠道疾病的内窥镜图像临床解释，但发现当前LLMs在医学推理中仍不可靠。


<details>
  <summary>Details</summary>
Motivation: 医学图像分类器能检测胃肠道疾病但缺乏解释能力，而大语言模型能生成临床文本但视觉推理能力弱且解释不稳定，这造成了模型所见与临床医生期望的推理类型之间的差距。

Method: 开发了一个结合图像分类与结构化临床推理的框架：1) 设计新的混合模型MobileCoAtNet用于内窥镜图像分类；2) 用分类输出驱动多个LLMs进行推理；3) 构建两个专家验证的基准来评估推理质量。

Result: MobileCoAtNet在8个胃相关类别上达到高准确率；评估了32个LLMs，发现强分类能提高解释质量，但没有模型达到人类水平的稳定性；最好的LLMs在提示变化时也会改变其推理。

Conclusion: 结合DL与LLMs可以产生有用的临床叙述，但当前LLMs在高风险医疗决策中仍不可靠；该框架为理解其局限性并构建更安全的推理系统提供了路径。
...
<details>
  <summary>Details</summary>
Motivation: 尽管多模态大语言模型在通用视觉语言任务上表现出色，但在医学决策任务上表现不佳，特别是在视觉差异细微的医学分类任务中，需要探究其局限性并寻找改进方法

Method: 使用两个具有挑战性的医学数据集：三阶段阿尔茨海默病分类和MIMIC-CXR胸片分类，比较文本推理、视觉推理和视觉文本结合的三种模式，并探索三种改进策略：带有推理注释的上下文学习、视觉描述后文本推理、以及视觉塔的少样本微调

Result: 文本推理在医学决策任务上始终优于视觉推理或视觉文本结合，多模态输入甚至比纯文本表现更差，表明当前多模态大语言模型缺乏扎实的视觉理解能力
...
<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型在追求通用人工智能中至关重要，但实现统一的多模态理解和生成目标面临优化冲突和性能权衡的挑战。需要在保持现有理解能力的同时有效提升生成性能。

Method: STAR采用堆叠自回归方案，将多模态学习分解为理解、生成和编辑多个阶段。冻结基础自回归模型参数，逐步堆叠同构的自回归模块，避免跨任务干扰。同时引入高容量VQ增强图像表示粒度，采用隐式推理机制提升复杂条件下的生成质量。

Result: 实验表明STAR在GenEval(0.91)、DPG-Bench(87.44)和ImgEdit(4.34)上达到最先进性能，验证了其统一多模态学习的有效性。

Conclusion: STAR通过任务渐进式的堆叠自回归方案，成功实现了在保持理解能力的同时提升生成性能的统一多模态学习，为多模态模型的发展提供了有效方法。
...
TL;DR: 该论文提出了一种新的3D表示方法，将几何与外观解耦，使用面元表示几何，结合全局神经场和每个基元的颜色表示外观，相比3D高斯泼溅大幅减少了基元数量和内存使用。


<details>
  <summary>Details</summary>
Motivation: 高斯泼溅方法虽然在新视角合成方面取得了令人印象深刻的结果，但即使场景几何简单，也需要数百万个基元来建模高纹理场景，导致内存和计算开销大。

Method: 提出解耦几何与外观的表示方法：1) 使用面元表示几何；2) 外观通过全局神经场和每个基元的颜色组合表示；3) 每个像素固定数量的基元进行纹理化，确保计算开销低。

Result: 在室外场景中：使用9.7倍更少的基元和5.5倍更少的内存；在室内场景中：使用31倍更少的基元和3.7倍更少的内存；渲染速度比现有纹理基元快2倍，同时视觉质量更好。

Conclusion: 该方法在保持与3D高斯泼溅相同感知质量的同时，显著减少了基元数量和内存使用，提高了渲染效率，为紧凑高效的3D场景表示提供了新思路。
...
TL;DR: VajraV1模型在实时目标检测领域取得突破，通过整合YOLO系列设计的优势，在保持竞争性推理速度的同时，实现了最先进的检测精度。


<details>
  <summary>Details</summary>
Motivation: 近年来YOLO系列模型（v10-v13）在实时目标检测领域快速发展，但仍有提升空间。研究者旨在通过架构改进，结合现有YOLO模型的有效设计选择，在保持实时性的同时进一步提升检测精度。

Method: VajraV1模型架构整合了先前YOLO模型的有效设计选择，进行了架构增强。具体方法包括从现有YOLO模型中提取最佳设计元素并进行组合优化，以平衡精度与速度。
...
Conclusion: VajraV1通过整合现有YOLO模型的有效设计选择，在实时目标检测领域实现了最先进的精度，同时保持了竞争性的推理速度，为实时目标检测提供了新的高性能解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 当前文本到动作生成模型在连续潜在空间扩散方面存在两个关键问题：如何构建语义对齐的潜在空间以提高扩散效果，以及如何最佳注入文本条件使动作更紧密遵循描述。

Method: 提出语义对齐的运动编码器，使用帧级文本标签训练，使具有相似文本含义的潜在表示保持接近；比较单令牌条件与多令牌交叉注意力方案，发现交叉注意力能提供更好的动作真实性和文本-动作对齐；结合语义对齐潜在、自回归生成和交叉注意力文本条件。

Result: 模型在人体动作生成的标准指标和用户研究中达到了新的最先进水平，证明了语义对齐潜在空间和交叉注意力文本条件的有效性。

Conclusion: MoLingo通过语义对齐的潜在空间表示和交叉注意力文本条件机制，显著提升了文本到动作生成的质量和文本-动作对齐度，为后续研究和下游应用提供了有力工具。
...
TL;DR: CFHA是一个三阶段扩散框架，通过全局风格对齐、局部细化和幻觉消除，将合成无人机图像转换为真实风格，提升无人机人体检测的跨域性能


<details>
  <summary>Details</summary>
Motivation: 无人机人体检测需要大量标注数据，但真实数据标注成本高且目标分布不断变化。合成数据虽然成本低，但与真实图像存在域差距，影响模型在目标域的应用效果。

Method: 提出粗到细层次对齐(CFHA)框架：1) 全局风格迁移：扩散模型将合成图像的颜色、光照和纹理统计对齐到真实风格；2) 局部细化：超分辨率扩散模型增强小物体(如人体)的细节和真实感；3) 幻觉消除：过滤视觉属性与真实数据不符的人体实例。

Result: 在公开的无人机Sim2Real检测基准测试中，CFHA显著提升了检测精度。在Semantic-Drone基准上，mAP50最高提升了14.1个百分点。消融研究证实了全局和局部阶段的互补作用。

Conclusion: CFHA通过层次化对齐策略有效缩小了合成与真实无人机图像之间的域差距，同时保留了原始合成标签，显著提升了无人机人体检测的跨域性能。
...
TL;DR: Route-DETR通过自适应成对路由机制解决DETR中查询竞争问题，区分竞争查询和互补查询，在训练时引入路由偏置，推理时不增加计算成本，显著提升检测性能。


<details>
  <summary>Details</summary>
Motivation: DETR虽然提供了端到端的目标检测解决方案，但存在查询竞争效率低下的问题，多个查询会收敛到相似位置，导致冗余计算。

Method: 提出Route-DETR，通过解码器自注意力层的自适应成对路由机制，利用查询间相似性、置信度得分和几何信息区分竞争查询与互补查询。引入双重路由机制：抑制器路由（减少重复）和委托器路由（鼓励探索不同区域），通过可学习的低秩注意力偏置实现非对称查询交互。采用双分支训练策略，训练时包含路由偏置，推理时保持标准注意力，不增加计算成本。

Result: 在COCO和Cityscapes数据集上实验表明，该方法在多个DETR基线上获得一致改进。在ResNet-50上比DINO提升+1.7% mAP，在Swin-L上达到57.6% mAP，超越了先前的最先进模型。

Conclusion: Route-DETR通过自适应路由机制有效解决了DETR中的查询竞争问题，在不增加推理计算成本的情况下显著提升了目标检测性能，为端到端检测器提供了更高效的解决方案。
...
TL;DR: 提出"去品牌化"新任务，用于细粒度移除商标和结构品牌特征，同时保持语义连贯性，并引入基于VLM的评估指标


<details>
  <summary>Details</summary>
Motivation: 文本到图像扩散模型的快速发展引发了对未经授权复制商标内容的担忧。现有方法主要针对一般概念（如风格、名人），但无法处理特定的品牌标识符。品牌识别是多维度的，不仅包括显式商标，还包含微妙的结构特征（如汽车前格栅）

Method: 引入"去品牌化"新任务，构建全面的基准数据集。提出基于视觉语言模型（VLM）的新型评估指标，使用问答框架探测图像中的显式商标和隐式整体品牌特征

Result: 随着模型保真度提高（SDXL、FLUX比Stable Diffusion更容易合成品牌标识符），去品牌化挑战的紧迫性凸显。基于VLM指标的验证结果确认去品牌化是一个独特且实际相关的问题，需要专门技术

Conclusion: 去品牌化是一个需要专门技术解决的独特实际问题，提出的VLM评估方法能有效评估显式和隐式品牌特征，为商标保护提供新工具
...
TL;DR: FocalComm是一个专注于交换困难实例特征的多智能体协同感知框架，通过渐进式困难实例挖掘和基于查询的特征融合，显著提升了行人等安全关键小目标的检测性能。
...
Result: 在两个真实世界数据集(V2X-Real和DAIR-V2X)上，FocalComm在车辆中心化和基础设施中心化协同设置中均优于现有最先进的协同感知方法，在V2X-Real数据集上行人检测性能显著提升。

Conclusion: FocalComm通过专注于交换困难实例导向特征，有效提升了协同感知系统对安全关键小目标的检测能力，为自动驾驶安全提供了更可靠的解决方案。
...
TL;DR: 提出一个框架，将2D扩散模型适配用于从不完整点云进行3D形状补全，通过引入Shape Atlas这一紧凑的2D表示来弥合2D与3D之间的模态差距。
...
Method: 引入Shape Atlas，这是一种3D几何的紧凑2D表示，能够充分利用预训练2D扩散模型的生成能力，并调整条件输入和输出空间之间的模态对齐，实现更有效的条件控制。这种统一的2D表述便于从有限的3D数据中学习，并产生高质量、细节保留的形状补全。

Result: 在PCN和ShapeNet-55数据集上验证了方法的有效性，展示了高质量的形状补全结果。此外，还展示了从补全点云创建艺术家创建网格的下游应用，进一步证明了方法的实用性。

Conclusion: 提出的框架通过Shape Atlas表示成功地将2D扩散模型适配到3D形状补全任务中，克服了3D数据稀缺和模态差距的限制，实现了高质量的3D形状补全，并具有实际应用价值。
...
TL;DR: Sparse-LaViDa通过动态截断冗余掩码token来加速掩码离散扩散模型推理，引入寄存器token保持生成质量，实现2倍加速


<details>
  <summary>Details</summary>
Motivation: 掩码离散扩散模型（MDMs）在多模态任务中表现优异，但由于需要在每个采样步骤重复处理冗余的掩码token，导致推理速度不理想

Method: 提出Sparse-LaViDa框架：1）动态截断每个推理步骤中不必要的掩码token；2）引入专门的寄存器token作为截断token的紧凑表示；3）设计专门的注意力掩码，使训练与截断采样过程保持一致

Result: 基于最先进的统一MDM LaViDa-O构建的Sparse-LaViDa在文本到图像生成、图像编辑和数学推理等多样化任务中实现了高达2倍的加速，同时保持了生成质量

Conclusion: Sparse-LaViDa通过动态token截断和寄存器token机制，有效解决了MDM推理速度瓶颈问题，在保持生成质量的同时显著提升了推理效率
...
<details>
  <summary>Details</summary>
Motivation: 长视频问答中关键帧采样对高效理解至关重要，但现有工作只能通过问答准确率间接评估帧选择质量，缺乏直接评估采样策略的基准。

Method: 创建KFS-Bench基准，包含多场景标注；提出新的采样质量指标；开发基于问题-视频相关性的自适应平衡采样方法，平衡采样多样性与问题-帧相似性。

Result: 研究发现采样精度、场景覆盖率和采样平衡是影响问答性能的关键因素；提出的自适应平衡采样方法在关键帧采样和问答性能上均表现优异。

Conclusion: KFS-Bench为长视频问答中的关键帧采样提供了直接评估框架，提出的采样方法和质量指标能有效提升多模态大语言模型在长视频理解中的准确性和效率。
...
TL;DR: 本文综述了深度学习在自主机器人场景理解中的应用，涵盖目标检测、语义/实例分割、深度估计、3D重建和视觉SLAM等领域，探讨了这些技术如何克服传统几何模型的局限，并指出了现有问题和未来研究方向。


<details>
  <summary>Details</summary>
Motivation: 传统几何模型在自主机器人场景理解中存在局限性，特别是在处理遮挡、纹理缺失表面和实时深度感知方面。深度学习技术能够改善这些不足，增强语义推理能力，使机器人能更好地理解动态和非结构化环境。

Method: 通过文献综述方法，系统分析了深度学习在多个场景理解任务中的应用：目标检测、语义分割、实例分割、深度估计、3D重建和视觉SLAM。重点关注这些技术如何集成到动态环境中，提升决策制定、导航和交互能力。

Result: 深度学习技术显著改善了自主机器人的场景理解能力，能够更好地处理遮挡和纹理缺失表面，实现实时深度感知，增强语义推理。当这些感知模块集成到动态非结构化环境中时，机器人的决策、导航和交互能力得到有效提升。

Conclusion: 深度学习在自主机器人场景理解中展现出巨大潜力，能够有效克服传统方法的局限。然而仍存在需要解决的问题，论文最后指出了基于学习的场景理解技术的研究方向和未来发展路径。
...
TL;DR: CITab是一个新颖的自监督学习框架，通过语义感知的表格建模机制和原型引导的混合线性层模块，实现了跨表格的多模态医学图像-表格数据表示学习，在阿尔茨海默病诊断任务上表现优异。


<details>
  <summary>Details</summary>
Motivation: 现有基于自监督学习的医学图像-表格多模态学习方法受限于特定数据队列，主要原因是其僵化的表格建模机制无法有效处理异构表格数据。这种跨表格障碍阻碍了多模态自监督学习方法从不同队列中学习可迁移的医学知识。

Method: 提出CITab框架：1）从语义感知角度设计表格建模机制，整合列标题作为语义线索，促进可迁移知识学习和多数据源预训练的可扩展性；2）提出原型引导的混合线性层模块用于表格特征专门化，有效处理表格数据的异质性并探索底层医学概念。

Result: 在包含4,461名受试者的三个公开数据队列上进行阿尔茨海默病诊断任务的全面评估，实验结果表明CITab优于现有最先进方法。

Conclusion: CITab为有效且可扩展的跨表格多模态学习铺平了道路，能够学习强大的多模态特征表示，克服了现有方法在跨队列学习中的局限性。
...
<details>
  <summary>Details</summary>
Motivation: 传统结构光方法在像素域进行匹配，在遮挡、精细结构、非朗伯表面等挑战性场景下鲁棒性有限。受神经特征匹配进展启发，寻求更鲁棒的对应关系解码方法

Method: 1) 从投影图案和捕获的红外图像提取神经特征；2) 在特征空间构建代价体，显式结合几何先验；3) 引入深度细化模块，利用大规模单目深度估计模型的强先验；4) 开发基于物理的结构光渲染流程，生成近百万合成数据用于训练

Result: 方法在仅使用合成数据训练的情况下，能很好地泛化到真实室内环境，可处理多种图案类型而无需重新训练，性能优于商业结构光系统和被动立体RGB深度估计方法

Conclusion: 提出的学习型结构光解码框架通过在特征空间进行匹配，显著提升了在挑战性场景下的鲁棒性，结合深度细化模块进一步改善了细节恢复和全局结构一致性，为3D成像提供了更可靠的解决方案
...
TL;DR: ASAP Textured Gaussians通过自适应采样和各向异性参数化，显著提升纹理高斯方法的效率，在减少纹理参数的同时实现高质量渲染。


<details>
  <summary>Details</summary>
Motivation: 现有纹理高斯方法存在两个关键限制：1）纹理定义在规范空间中，导致对低贡献区域进行低效采样；2）纹理参数化在所有高斯上均匀分配，不考虑视觉复杂度，导致过参数化。

Method: 提出两种简单有效的策略：基于高斯密度分布的自适应采样，以及根据渲染误差分配纹理资源的误差驱动各向异性参数化。

Result: ASAP Textured Gaussians显著改善了质量效率权衡，使用更少的纹理参数实现了高保真渲染。

Conclusion: 通过自适应采样和各向异性参数化，有效解决了纹理高斯方法中的内存效率问题，提升了纹理资源利用率。
...
TL;DR: ChartAgent是一个基于工具集成推理的图表理解框架，通过模块化工具库和可观察的推理步骤，解决了现有MLLM在缺少文本标注时性能下降的问题。


<details>
  <summary>Details</summary>
Motivation: 现有多模态大语言模型在图表理解方面依赖显式文本标注，当关键数字缺失时性能显著下降。需要一种更稳健、可扩展的图表理解系统。

Method: 基于工具集成推理框架，将复杂图表分析分解为可观察、可重放的步骤。使用包含关键元素检测、实例分割、OCR等十多个核心工具的模块化工具库，动态协调工具进行系统视觉解析，生成结构化的证据包。

Result: 实验表明ChartAgent在稀疏标注设置下显著提高了鲁棒性，为可信赖和可扩展的图表理解系统提供了实用路径。

Conclusion: ChartAgent通过工具集成推理的透明性和可验证性，超越了黑盒范式，提供了可追溯和可复现的图表理解解决方案。
...
TL;DR: OmniDrive-R1是一个端到端的视觉语言模型框架，通过交错多模态思维链机制统一感知和推理，采用强化学习驱动的视觉定位能力，无需密集标注标签，在自动驾驶场景中显著提升了推理性能和答案准确性。


<details>
  <summary>Details</summary>
Motivation: 当前视觉语言模型在自动驾驶等安全关键领域部署面临可靠性问题，特别是物体幻觉问题。现有方法存在感知与推理阶段解耦、依赖昂贵的密集定位标注等根本缺陷，需要一种端到端的统一解决方案。

Method: 提出OmniDrive-R1框架，采用交错多模态思维链机制统一感知和推理。核心创新是强化学习驱动的视觉定位能力，让模型能自主关注关键区域进行细粒度分析。使用纯两阶段强化学习训练流程和Clip-GRPO算法，引入无需标注的过程式定位奖励。

Result: 在DriveLMM-o1数据集上的实验显示显著改进：相比基线Qwen2.5VL-7B，OmniDrive-R1将整体推理分数从51.77%提升到80.35%，最终答案准确率从37.81%提升到73.62%。

Conclusion: OmniDrive-R1通过端到端的交错多模态思维链机制和强化学习驱动的视觉定位，有效解决了自动驾驶中视觉语言模型的可靠性问题，无需密集标注，实现了感知与推理的统一优化。
...
<details>
  <summary>Details</summary>
Motivation: 真实场景文本数据集存在标签错误问题，现有方法难以有效处理变长序列标签、标签序列不对齐和字符级错误，需要开发能够准确检测这些错误并提升场景文本识别准确性的方法。

Method: SELECT采用多模态训练框架，结合图像-文本编码器和字符级分词器处理变长标签序列。引入SSLC（基于相似性的序列标签破坏）过程，在训练时故意引入标签错误，模拟真实错误场景，并考虑字符间的视觉相似性。

Result: 实验结果表明SELECT在检测标签错误方面优于现有方法，能够有效提升真实场景文本数据集的STR（场景文本识别）准确性，展示了其实际应用价值。

Conclusion: SELECT是首个成功处理变长标签的真实场景文本数据集标签错误检测方法，通过多模态训练和SSLC机制有效解决了标签错误检测的关键挑战，具有重要的实际应用意义。
...
<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型虽然具备强大的感知和推理能力，但计算和内存需求过高，难以直接部署在设备端环境中。标准的视觉Transformer编码器在处理高分辨率输入时存在延迟过高和内存消耗过大的瓶颈问题。

Method: 1. 采用图像分块策略来限制峰值内存使用；2. 引入视觉分辨率压缩器（VRC），自适应预测最优编码分辨率以消除冗余计算；3. 提出双重一致性学习（DCL），在统一框架内对齐多尺度ViT编码器，实现在共享LLM下动态切换视觉分支。
...
<details>
  <summary>Details</summary>
Motivation: 传统上说话人脸编辑和生成被作为独立任务研究，缺乏统一框架。作者认为两者本质上是面部运动填充任务的不同表现形式，需要建立统一的理论框架和评估标准。

Method: 提出FacEDiT模型：基于流匹配的语音条件扩散Transformer，采用掩码自编码器思想学习合成被掩码的面部运动；引入偏置注意力和时间平滑约束增强边界连续性和唇部同步；创建FacEDiTBench首个说话人脸编辑数据集。

Result: 实验验证说话人脸编辑和生成确实可作为语音条件运动填充的子任务；FacEDiT能产生准确、语音对齐的面部编辑，保持强身份一致性和视觉连续性，在说话人脸生成任务上也表现良好。

Conclusion: 通过统一框架将说话人脸编辑和生成视为面部运动填充的子任务，提出的FacEDiT模型在编辑和生成任务上均表现优异，为动态说话人脸合成提供了新的理论视角和实践方案。
...
TL;DR: 提出了一种基于多模态深度学习的实时室内工作面照度预测框架，通过仅从侧窗区域提取图像特征，适用于动态占用的室内空间，在测试中取得了高精度和良好的时间泛化能力。
...
Method: 提出多模态深度学习框架，通过从侧窗区域（而非室内像素）提取图像特征，结合时空特征，实时预测室内工作面照度分布。在广州的测试房间进行了实地实验，收集了17,344个样本用于模型训练和验证。
...
TL;DR: SDAR-VL是首个将块级离散扩散系统应用于大规模视觉语言理解的方法，通过集成训练框架解决了传统块扩散的高训练成本、收敛慢和不稳定问题，在21个基准测试中超越了传统块扩散和自回归基线。


<details>
  <summary>Details</summary>
Motivation: 块级离散扩散在并行生成和因果依赖建模之间提供了良好的平衡，但实际应用受到高训练成本、收敛慢和不稳定性的限制，导致其落后于强大的自回归基线。

Method: 提出了一个集成训练框架，包含三个核心组件：(1)异步块级噪声调度，在每个批次内多样化监督；(2)有效掩码比率缩放，在随机掩码下实现无偏损失归一化；(3)渐进式Beta噪声课程，增加有效掩码覆盖率同时保持破坏多样性。

Result: 在21个单图像、多图像和视频基准测试中，SDAR-VL在训练效率、收敛稳定性和任务性能方面均优于传统块扩散方法。在匹配设置下，达到或超越了LLaVA-OneVision等自回归基线和LLaDA-V等全局扩散基线。

Conclusion: SDAR-VL证明了块级离散扩散可以作为视觉语言理解的实用骨干模型，建立了扩散基视觉语言模型的新技术标准，并展示了与自回归方法竞争的能力。
...
<details>
  <summary>Details</summary>
Motivation: 3DGS在场景外观重建方面表现出色，但缺乏对植物内部结构（如分支模式）的表征，限制了其在植物表型分析等任务中的应用

Method: 提出分层3DGS表示GaussianPlant，使用结构基元（StPs）显式表示枝干和叶片几何（圆柱体和圆盘），外观基元（ApPs）表示植物外观；两者通过绑定对应关系联合优化，使用重渲染损失和梯度流
...
TL;DR: 该研究提出了一种基于质量感知的预测框架，用于提高视频呼吸率估计的可靠性。通过整合多种信号源和动态评估信号质量，实现了自适应信号融合和质量筛选。
...
Method: 从面部远程光电容积描记、上半身运动和深度学习管道中提取10种信号，使用四种频谱估计器分析，利用分段质量指标训练机器学习模型来预测准确性或选择最可靠信号。
...
TL;DR: OUSAC框架通过优化引导调度和自适应缓存，在保持生成质量的同时显著加速扩散变换器，减少高达82%的无条件前向传播计算。


<details>
  <summary>Details</summary>
Motivation: 扩散模型虽然能生成高质量图像，但计算成本高昂，特别是Classifier-Free Guidance（CFG）需要在每个时间步同时进行条件和非条件前向传播，使计算量加倍。现有缓存方法在可变引导尺度下效果不佳，需要新的优化方案。

Method: 提出两阶段方法：第一阶段使用进化算法联合优化跳过哪些时间步和使用什么引导尺度；第二阶段引入自适应秩分配，根据不同变换器块在动态条件下的受影响程度定制校准工作，保持缓存有效性。

Result: OUSAC显著优于现有加速方法：在DiT-XL/2（ImageNet 512x512）上实现53%计算节省和15%质量提升；在PixArt-alpha（MSCOCO）上实现60%节省和16.1%提升；在FLUX上实现5倍加速同时提高CLIP分数。

Conclusion: OUSAC通过系统优化引导调度和自适应缓存，在保持甚至提高生成质量的同时大幅减少扩散变换器的计算开销，为高效扩散模型提供了实用解决方案。
...
TL;DR: ViewMask-1-to-3是一种基于离散扩散模型的多视角图像生成方法，通过将多视角合成转化为离散序列建模问题，使用MAGVIT-v2 tokenization和掩码token预测，无需复杂3D几何约束即可实现跨视角一致性。


<details>
  <summary>Details</summary>
Motivation: 现有多视角图像生成方法通常依赖3D感知架构或专用扩散模型，需要大量多视角训练数据和复杂几何先验，难以保持不同视角间的几何一致性。本文旨在探索更简单有效的多视角生成方案。

Method: 提出ViewMask-1-to-3方法，将多视角合成转化为离散序列建模问题：1) 使用MAGVIT-v2 tokenization将每个视角表示为视觉token；2) 通过掩码token预测统一语言和视觉表示；3) 结合文本输入通过迭代token去掩码逐步生成多个视角；4) 利用简单随机掩码和自注意力机制实现跨视角一致性，无需复杂3D几何约束或专用注意力架构。

Result: 在GSO和3D-FUTURE数据集上，ViewMask-1-to-3在PSNR、SSIM和LPIPS指标上平均排名第一，证明了离散扩散模型在多视角生成任务中的有效性，同时保持了架构的简洁性。

Conclusion: 离散扩散模型为多视角图像生成提供了一种可行且简单的替代方案，通过离散序列建模和掩码token预测，能够在无需复杂3D几何约束的情况下实现跨视角一致性，为多视角生成任务开辟了新方向。
...
TL;DR: RUNE是一个结合大型语言模型与神经符号AI的遥感图像检索方法，通过显式推理而非隐式嵌入，在复杂空间关系处理和可解释性方面优于现有RS-LVLMs。


<details>
  <summary>Details</summary>
Motivation: 现有遥感大型视觉语言模型（RS-LVLMs）在可解释性和复杂空间关系处理方面存在局限，限制了实际应用。需要一种能够进行显式推理、提高性能和可解释性的方法。

Method: 提出RUNE方法：1）使用LLM将文本查询转换为一阶逻辑表达式；2）检测图像中的实体；3）通过神经符号推理模块评估实体与逻辑表达式的兼容性；4）采用逻辑分解策略提高可扩展性，在检测实体的条件子集上操作。

Result: 1）LLM在文本到逻辑转换中表现有效；2）RUNE在复杂遥感检索任务中优于最先进的RS-LVLMs；3）引入RRQC和RRIU两个新指标评估性能；4）在洪水后卫星图像检索用例中展示了实际应用潜力。

Conclusion: RUNE通过显式神经符号推理，在性能、鲁棒性和可解释性方面优于基于联合嵌入的模型，为复杂遥感图像检索提供了更可靠的解决方案，具有实际应用价值。
...
TL;DR: 提出了一种无需训练和数据的CLIP模型遗忘框架，支持三种遗忘范式：全局遗忘、领域特定遗忘和选择性领域完全遗忘，通过多模态零空间和文本提示实现高效知识移除。


<details>
  <summary>Details</summary>
Motivation: 现实应用中需要在不重新训练或获取额外数据的情况下，从预训练模型中移除特定对象类别的知识，同时不影响模型在其他任务上的性能。现有基于重新训练的方法存在局限性，需要更灵活、计算高效的解决方案。

Method: 通过协同整合文本提示和从CLIP联合嵌入空间合成的视觉原型，利用多模态零空间来移除不需要的类别信息。该方法不需要额外训练数据或重新训练模型。

Result: 该方法能够有效移除特定对象类别的知识，同时保持模型在其他任务上的性能。相比现有基于重新训练的方法，提供了更灵活和计算高效的遗忘解决方案。

Conclusion: 提出的训练和数据自由的遗忘框架为受控模型遗忘提供了有效的解决方案，克服了现有方法的局限性，支持多种遗忘范式，具有实际应用价值。
...
TL;DR: 提出了一种连续概率时空表示方法，用于动态场景理解，通过可变形3D高斯模型联合编码辐射和语义信息，在新型视角全景分割和开放词汇4D查询任务上表现优异


<details>
  <summary>Details</summary>
Motivation: 现有方法依赖离散跟踪或视角相关特征，难以在时空维度上保持一致的实例表示，需要一种能够解耦可见性和持久对象身份的新表示方法

Method: 提出基于可变形3D高斯的实例嵌入表示，联合编码辐射和语义信息；引入校准每个高斯身份和向语义活跃区域重采样的新机制；通过可微分光栅化从RGB图像和实例掩码直接学习
...
<details>
  <summary>Details</summary>
Motivation: 传统遗忘技术需要迭代微调和大量数据整理，计算成本高且不够精确。需要一种更高效、精确的方法来擦除多模态模型中特定类别的信息，同时保护模型隐私和实现模型去污染

Method: 利用零空间投影技术，计算目标文本嵌入所张成子空间的正交基，通过投影这些方向来擦除最终投影层中嵌入的目标类别信息。该方法不需要重新训练或遗忘集图像

Result: 显著降低了目标类别的零样本性能，同时保留了模型的整体多模态知识。即使部分投影也能在完全遗忘和保留有用信息之间取得平衡

Conclusion: 该方法为多模态模型的选择性遗忘提供了一种计算高效且精确的解决方案，解决了模型去污染和隐私保护的关键挑战
...
TL;DR: SketchAssist是一个交互式草图绘制助手，统一了指令引导的全局编辑和线条引导的区域重绘，保持无关区域和整体构图不变，加速草图创作过程。


<details>
  <summary>Details</summary>
Motivation: 现有图像编辑系统难以同时保持线稿的稀疏、风格敏感结构，并支持高级语义更改和精确局部重绘。需要一种能够统一全局编辑和局部重绘的草图编辑系统。

Method: 1) 提出可控数据生成流程：从无属性基础草图构建属性添加序列，通过跨序列采样形成多步编辑链，使用风格保持属性移除模型扩展风格覆盖；2) 统一草图编辑框架：重用RGB通道编码输入，在单一界面无缝切换指令引导编辑和线条引导重绘；3) 集成任务引导的专家混合到LoRA层，通过文本和视觉线索路由，提高语义可控性、结构保真度和风格保持。

Result: 在指令引导编辑和线条引导重绘两个任务上都取得了最先进的结果，相比现有基线具有更好的指令遵循能力和风格/结构保持能力。

Conclusion: SketchAssist及其数据集为草图创建和修订提供了一个实用、可控的助手，能够加速数字插画创作过程，同时保持线稿的结构完整性和风格一致性。
...
<details>
  <summary>Details</summary>
Motivation: 识别和解决机器学习模型中的性能反模式对于高效训练和推理至关重要，但通常需要跨越系统基础设施、ML模型和内核开发的深厚专业知识。当前方法资源密集，难以被普通计算机视觉研究人员访问，且从冗长的执行跟踪中定位问题段是最耗时的任务。

Method: 提出了一个包含600多个PyTorch跟踪的基准数据集，涵盖多种计算机视觉模型和硬件平台。提出了一种新颖的迭代方法：首先使用轻量级ML模型检测包含反模式的跟踪段，然后使用大型语言模型进行细粒度分类和针对性反馈。

Result: 实验结果表明，该方法在检测反模式区域方面显著优于无监督聚类和基于规则的统计技术。该方法还能有效补偿LLM有限的上下文长度和推理效率问题。

Conclusion: 该工作为自动化检测ML性能反模式提供了首个基准数据集和有效的迭代方法，能够帮助研究人员更高效地识别和解决模型性能问题，降低了对专业基础设施工程师的依赖。
...
TL;DR: CIS-BA是一种针对目标检测系统的新型后门攻击范式，通过连续交互空间建模物体间的交互模式，实现多触发-多目标攻击，具有高攻击成功率和防御规避能力。


<details>
  <summary>Details</summary>
Motivation: 现有后门攻击方法依赖于单触发-单目标映射和脆弱的像素级线索，在能力和鲁棒性上存在固有局限，无法应对现实世界应用（如自动驾驶）中的复杂交互场景。

Method: 提出CIS-BA攻击范式，将触发设计从静态物体特征转向连续交互模式；开发CIS-Frame框架，通过交互分析构建空间触发器，将其形式化为类别-几何约束进行样本投毒，并在检测器训练中嵌入后门。

Result: 在MS-COCO和真实世界视频上的实验显示，CIS-BA在复杂环境下攻击成功率超过97%，在动态多触发条件下保持95%以上有效性，并能规避三种最先进的防御方法。

Conclusion: CIS-BA扩展了交互密集场景中的后门攻击能力，为目标检测系统的安全性提供了新见解，展示了基于交互模式的后门攻击的潜力和威胁。
...
TL;DR: Fast3DHPE是一个统一的模块化框架，用于公平比较单目3D人体姿态估计方法，同时显著提高训练效率。在该框架中提出了FastDDHPose方法，利用解耦扩散模型分别建模骨骼长度和方向分布，避免层次误差累积。


<details>
  <summary>Details</summary>
Motivation: 现有单目3D人体姿态估计方法通常在不同的框架下训练和评估，缺乏统一的公平比较框架。这阻碍了方法的客观评估和进一步发展。

Method: 提出了Fast3DHPE模块化框架，标准化训练和评估协议。在该框架中开发了FastDDHPose方法，使用解耦扩散模型分别建模骨骼长度和方向分布，并设计了高效的运动学层次时空去噪器，专注于运动学关节层次结构。

Result: 在Human3.6M和MPI-INF-3DHP数据集上的实验表明，Fast3DHPE框架能够公平比较所有方法并显著提高训练效率。FastDDHPose在该统一框架中实现了最先进的性能，在野外场景中表现出强大的泛化能力和鲁棒性。

Conclusion: Fast3DHPE提供了一个统一的模块化框架，解决了3D人体姿态估计领域缺乏公平比较标准的问题。FastDDHPose方法在该框架中表现出色，展示了扩散模型在3D姿态估计中的潜力。
...
TL;DR: SGPU：一种基于贝叶斯框架的语义不确定性量化方法，通过分析答案嵌入的几何结构来避免脆弱的聚类，在多个模型和任务上实现最先进的校准性能。
...
Conclusion: SGPU通过分析答案嵌入的几何结构有效量化语义不确定性，避免脆弱的聚类方法，其光谱表示捕捉了语义不确定性的一般模式，具有跨模型和模态的泛化能力。
...
<details>
  <summary>Details</summary>
Motivation: 当前辐射场方法（如3D高斯泼溅）依赖球谐函数进行外观建模，但SH存在三个主要问题：难以处理高频信号、产生吉布斯振铃伪影、无法有效捕捉镜面反射（真实渲染的关键组件）。虽然球面高斯等替代方案有所改进，但显著增加了优化复杂性。
...
Result: 在合成和真实世界数据集上取得了最先进的结果。对于漫反射外观，SV实现了有竞争力的结果同时保持优化简单；对于反射效果，SV在SH失败的领域表现出色，证明了其作为显式3D表示中外观建模的原则性、高效且通用的解决方案。

Conclusion: 球面Voronoi为3D高斯泼溅中的外观表示提供了一个统一框架，解决了球谐函数的局限性，在保持优化简单性的同时，在漫反射和反射效果方面都表现出色，为显式3D表示中的外观建模提供了原则性、高效且通用的解决方案。
...
TL;DR: 提出一种自动分配全球AO代码给骨折边界框的方法，将全局多标签任务转化为局部多分类任务，提高F1分数7.89%，但实际部署中不完美的骨折检测器会影响性能。
...
Method: 提出自动分配全球AO代码给相应骨折边界框的方法，将全局多标签任务重新定义为局部多分类任务，利用公共数据集。

Result: 该方法将平均F1分数提高了7.89%，但在使用不完美的骨折检测器时性能下降，突显了实际部署中的挑战。

Conclusion: 该方法能有效提取骨折形态并提高诊断准确性，但实际临床应用需要解决骨折检测器的性能问题。代码已在GitHub上公开。
...
TL;DR: SkyLume是一个用于研究光照鲁棒性3D重建的大规模无人机数据集，包含10个城市区域在一天三个时间段采集的10万+高分辨率图像，配有LiDAR扫描和3D真值，并提出了评估逆渲染性能的时序一致性系数指标。


<details>
  <summary>Details</summary>
Motivation: 现有的神经辐射场和3D高斯泼溅方法在大规模无人机3D重建中表现出潜力，但现实世界的大规模采集通常基于多时序数据，不同时间的光照不一致会导致颜色伪影、几何不准确和外观不一致。由于缺乏系统捕捉不同光照条件下相同区域的无人机数据集，这一挑战尚未得到充分探索。

Method: 提出了SkyLume数据集：1) 收集了10个城市区域的数据，包含超过10万张高分辨率无人机图像（四个倾斜视角和天底视角），每个区域在一天三个时间段采集以系统隔离光照变化；2) 提供每场景的LiDAR扫描和精确3D真值，用于评估深度、表面法线和不间光照下的重建质量；3) 引入时序一致性系数(TCC)指标，用于测量跨时间反照率稳定性，直接评估光照与材质解耦的鲁棒性。

Result: 创建了一个专门用于研究光照鲁棒性3D重建的大规模真实世界无人机数据集，填补了该领域数据集的空白，为大规模逆渲染、几何重建和新视角合成的研究和实际评估提供了基础资源。

Conclusion: SkyLume数据集旨在作为推动大规模逆渲染、几何重建和新视角合成研究及实际评估的基础资源，为解决多时序数据采集中的光照不一致问题提供了系统性的评估框架和基准。
...
TL;DR: DRAW2ACT是一个深度感知的轨迹条件视频生成框架，用于机器人操作任务，通过提取轨迹的多模态表示并联合生成RGB和深度视频，提升视觉保真度和操作成功率。


<details>
  <summary>Details</summary>
Motivation: 现有的视频扩散模型虽然为具身AI提供了强大的真实世界模拟器，但在机器人操作的可控性方面存在限制。基于轨迹条件的视频生成方法通常依赖于2D轨迹或单模态条件，这限制了它们生成可控且一致的机器人演示的能力。

Method: DRAW2ACT框架从输入轨迹中提取多个正交表示（深度、语义、形状和运动），并将它们注入扩散模型。同时提出联合生成空间对齐的RGB和深度视频，利用跨模态注意力机制和深度监督来增强时空一致性。最后引入一个多模态策略模型，基于生成的RGB和深度序列来回归机器人的关节角度。

Result: 在Bridge V2、Berkeley Autolab和仿真基准测试中，DRAW2ACT相比现有基线方法，实现了更优的视觉保真度和一致性，同时获得了更高的操作成功率。

Conclusion: DRAW2ACT通过深度感知的轨迹条件视频生成和多模态策略学习，显著提升了机器人操作任务中视频生成的质量和操作性能，为具身AI提供了更可控和一致的仿真环境。
...
<details>
  <summary>Details</summary>
Motivation: 现有的无人机视觉语言导航（AVLN）系统通常采用单一粒度的框架，难以同时平衡全局环境推理和局部场景理解这两个关键需求，这限制了导航性能的提升。

Method: 提出历史增强的两阶段Transformer框架（HETT），采用从粗到细的导航流程：首先通过融合空间地标和历史上下文预测粗粒度目标位置，然后通过细粒度视觉分析精炼动作。同时设计了历史网格地图来动态聚合视觉特征为结构化空间记忆。

Result: 在改进后的CityNav数据集上的实验表明，HETT取得了显著的性能提升，广泛的消融研究进一步验证了每个组件的有效性。

Conclusion: HETT框架通过整合全局环境推理和局部场景理解，有效解决了无人机视觉语言导航中的粒度平衡问题，同时通过改进数据集标注提升了数据质量，为AVLN任务提供了有效的解决方案。
...
Method: 提出OminiGen框架：1）使用共享BEV空间统一多模态特征；2）设计新颖的通用多模态重建方法UAE，通过体渲染联合解码LiDAR和多视角相机数据；3）结合带ControlNet分支的Diffusion Transformer实现可控的多模态传感器生成。
...
<details>
  <summary>Details</summary>
Motivation: MGMT启动子甲基化状态对胶质母细胞瘤化疗效果有重要影响，但目前的检测方法依赖侵入性的脑肿瘤组织活检。本研究旨在开发一种非侵入性的影像基因组学方法来识别这一遗传标记。

Method: 提出了一种新的多视角方法，利用MRI扫描和深度学习模型，考虑MRI视图间的空间关系来检测MGMT甲基化状态。该方法从三个视图提取信息，但避免使用复杂的3D深度学习模型。同时引入了新的肿瘤切片提取技术。

Result: 新提出的肿瘤切片提取方法在多个评估指标上优于现有方法。与最先进模型相比，证明了该方法的有效性。研究还分享了已发表模型的可复现流程。

Conclusion: 该研究突出了非侵入性方法识别MGMT启动子甲基化的潜力，有助于推进胶质母细胞瘤治疗中的精准医疗发展，并促进了诊断工具的透明度和稳健性。
...
Result: 实验表明，使用4D-RaDiff合成数据作为数据增强方法能持续提升物体检测性能；预训练使用合成数据可将所需标注雷达数据减少90%同时保持可比性能。

Conclusion: 4D-RaDiff能有效生成高质量雷达点云数据，解决标注数据稀缺问题，显著提升物体检测性能并大幅减少对真实标注数据的依赖。
...
TL;DR: Elastic3D是一种基于条件潜在扩散的端到端单目到立体视频转换方法，通过引导VAE解码器实现高质量、视差可控的立体视频生成，避免了传统基于深度估计和扭曲方法的伪影问题。


<details>
  <summary>Details</summary>
Motivation: 随着沉浸式3D内容需求的增长，需要自动化单目到立体视频转换方法。传统基于深度估计和扭曲的方法容易产生伪影，需要更高质量、可控的解决方案。

Method: 基于条件潜在扩散的端到端方法，核心是新颖的引导VAE解码器，确保立体视频输出的锐度和极线一致性。用户可以通过标量调节旋钮在推理时控制立体效果强度（视差范围）。

Result: 在三个真实世界立体视频数据集上的实验表明，该方法优于传统的基于扭曲方法和最近的免扭曲基线，为可靠、可控的立体视频转换设立了新标准。

Conclusion: Elastic3D提供了一种高质量、可控的单目到立体视频转换方法，避免了传统方法的伪影问题，通过引导VAE解码器和用户可控的视差调节实现了优越的性能。
...
TL;DR: EVPG通过概率图将不可微的视觉编程执行过程转化为可微的概率推理过程，使视觉编程框架能够利用最终标签进行端到端优化，显著提升了在复杂视觉推理任务上的性能。


<details>
  <summary>Details</summary>
Motivation: 现有视觉编程方法主要关注提升大语言模型生成的视觉程序质量，但忽略了优化视觉编程调用的预训练模型。由于只有最终任务的标签而没有子任务标签，且视觉编程的不可微性阻碍了基于梯度的端到端优化，需要解决这些问题来提升视觉推理性能。

Method: 提出EVPG方法，通过构建基于变量依赖关系的定向概率图，将不可微的视觉编程执行过程重构为在该定向概率图上的可微精确概率推理过程，从而支持基于梯度的端到端监督学习。

Result: 在三个经典复杂视觉推理任务（GQA、NLVRv2和Open Images）上进行了广泛实验，EVPG显著提升了视觉编程的性能，证明了该方法的有效性和优势。

Conclusion: EVPG通过概率图方法成功解决了视觉编程中不可微性和缺乏子任务标签的问题，实现了端到端优化，为视觉编程在复杂视觉推理任务中的应用提供了有效解决方案。
...
Method: 提出了DriverGaze360全景驾驶员注意力数据集（包含约100万帧标注数据）和DriverGaze360-Net模型，该模型通过辅助语义分割头联合学习注意力图和被关注物体，提升全景输入下的空间感知和注意力预测能力。

Result: DriverGaze360-Net在全景驾驶图像上的多个指标上实现了最先进的注意力预测性能。

Conclusion: 该研究通过全景数据集和联合学习方法，显著提升了驾驶员注意力预测的全面性和准确性，为可解释自动驾驶系统和理解混合交通场景中的驾驶员行为提供了重要工具。
...
TL;DR: Zoom-Zero框架通过粗到细的定位方法改进视频问答中的时间定位，引入放大精度奖励和令牌选择性信用分配来解决GRPO方法的局限性。


<details>
  <summary>Details</summary>
Motivation: 现有的大型视频语言模型在时间感知方面有限，基于GRPO的方法在时间定位上仍然存在问题，导致时间错位和幻觉，无法忠实基于视频证据回答问题。

Method: 提出Zoom-Zero粗到细框架：首先定位查询相关片段，然后时间放大到最显著帧进行细粒度视觉验证。关键创新包括：1)放大精度奖励验证时间定位预测的保真度；2)令牌选择性信用分配将奖励归因于负责时间定位或答案生成的令牌。

Result: 在NExT-GQA上时间定位提升5.2%，在ReXTime上提升4.6%，平均答案准确率提升2.4%。推理过程中的粗到细放大进一步改善长视频理解，在长视频基准上平均提升6.4%。

Conclusion: Zoom-Zero框架通过粗到细的时间定位和细粒度视觉验证，有效解决了GVQA任务中GRPO方法的局限性，显著提升了时间定位和答案生成的准确性。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法通常通过优化3D或视频生成模型来构建4D表示，缺乏原生4D生成能力，导致保真度、时间一致性和结构一致性不足。

Method: 1) 基于预训练的单图像到3D模型解决4D训练数据稀缺问题；2) 引入专用时间层确保时间一致性；3) 使用因子化4D卷积和时间下采样块压缩潜在序列，支持长视频序列的高效训练和推理；4) 采用精心设计的训练策略增强对遮挡的鲁棒性。

Result: 实现了直接从单目视频合成动态3D物体的原生4D生成，具有高保真度、时间一致性和结构一致性。

Conclusion: SS4D通过结构化时空潜在表示和高效压缩机制，成功实现了原生4D生成，为动态3D内容创建提供了新方法。
...
<details>
  <summary>Details</summary>
Motivation: 基于扩散模型的文本引导图像编辑技术虽然强大，但存在被滥用的风险。现有评估免疫效果的方法存在根本缺陷，它们通常只关注保护图像与原始图像编辑输出的视觉差异，而忽视了免疫的核心要求——破坏与攻击者意图的语义对齐。

Method: 提出协同中间特征操纵（SIFM）方法，通过双重协同目标策略性地扰动扩散模型的中间特征：1）最大化特征与原始编辑轨迹的差异以破坏与预期编辑的语义对齐；2）最小化特征范数以诱导感知退化。同时引入免疫成功率（ISR）作为新的评估指标。

Result: 大量实验表明，SIFM方法在保护视觉内容免受基于扩散模型的恶意操纵方面达到了最先进的性能。ISR指标首次能够严格量化真实的免疫效果。

Conclusion: 该研究重新定义了图像免疫的成功标准，提出了有效的SIFM方法和科学的ISR评估指标，为防范基于扩散模型的恶意图像编辑提供了更可靠的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 文本到图像扩散模型虽然推动了图像编辑的发展，但也带来了伦理挑战，可能被滥用于创建欺骗性或有害内容。现有防御方法通过嵌入不可感知扰动来降低风险，但对恶意篡改的防御效果有限。

Method: 提出双注意力引导噪声扰动（DANP）免疫方法，在多个时间步上操作，同时操纵交叉注意力图和噪声预测过程。使用动态阈值生成掩码来识别文本相关和无关区域，减少相关区域注意力同时增加无关区域注意力，误导编辑到错误区域并保护目标区域。此外，最大化注入噪声与模型预测噪声之间的差异来进一步干扰生成过程。

Result: DANP在对抗恶意编辑方面表现出显著的免疫效果，大量实验证实该方法达到了最先进的性能水平。

Conclusion: 通过同时针对注意力和噪声预测机制，DANP方法能有效防御文本到图像扩散模型的恶意编辑，为AI生成内容的伦理安全提供了新的解决方案。
...
TL;DR: 该论文提出了一种通过语义结构恢复来改进SVG动画生成的框架，解决了当前视觉语言模型在SVG动画中因图形元素碎片化导致的动画不连贯问题。
...
Method: 引入了一个框架，通过统计聚合多个弱部件预测来恢复SVG动画所需的语义结构。该方法通过重新组织SVG为语义组，使视觉语言模型能够产生更连贯的动画。

Result: 实验表明，该方法相比现有方法有显著提升，表明语义恢复是解锁稳健SVG动画的关键步骤，并支持视觉语言模型与矢量图形之间更可解释的交互。

Conclusion: 语义结构恢复是当前VLM系统忽视的关键层，通过统计聚合弱预测来推断语义，能够显著提升SVG动画的连贯性，为矢量图形动画提供了更可靠的解决方案。
...
Method: 提出双模态框架TDAE：视觉防御层面引入FlatGrad防御机制（FDM），通过梯度正则化将扰动导向平坦最小值；文本增强保护层面提出动态提示防御（DPD），周期性优化文本嵌入，通过对抗性更新增强跨模型可迁移性

Result: 在内部和跨模型评估中均取得最先进性能，有效减轻恶意编辑影响

Conclusion: TDAE通过协调图像-文本优化成功实现了对恶意图像编辑的可迁移防御，为扩散模型的安全性提供了有效解决方案
...
TL;DR: 提出Hybrid Gaussian Splatting (HGS)框架，通过静态-动态分解策略显著减少模型参数，实现实时渲染和高保真度
...
Method: 提出Hybrid Gaussian Splatting (HGS)框架，采用静态-动态分解策略，利用径向基函数建模高斯基元：动态区域使用时变RBF捕捉时间变化，静态区域共享时间不变参数以减少冗余；并引入两阶段训练策略增强时间一致性

Result: 模型大小减少高达98%，在RTX 3090上实现4K分辨率125 FPS实时渲染，在RTX 3050上实现1352×1014分辨率160 FPS，已集成到VR系统中；渲染质量与SOTA方法相当，在高频细节和突变场景变化方面提供显著改进的视觉保真度

Conclusion: HGS框架通过有效的静态-动态分解策略，在保持高质量渲染的同时显著减少了参数冗余，实现了高效的实时动态新视角合成，特别适合资源受限设备
...
<details>
  <summary>Details</summary>
Motivation: 小城市在气候适应方面面临人员资源有限和整合多源数据的挑战，需要系统化的解决方案来支持全面的气候适应分析。

Method: 开发多层系统，结合专业大语言模型(LLM)、卫星影像分析和知识库，为小城市提供综合的气候适应策略制定支持。

Result: 开发了名为EcoScapes的系统，代码已在GitHub开源，为小城市气候适应提供了技术解决方案。
...
TL;DR: ExpanDyNeRF：一种利用高斯溅射先验和伪真值生成策略的单目动态NeRF框架，可在大幅视角旋转下实现逼真合成，显著提升极端视角偏移下的渲染质量


<details>
  <summary>Details</summary>
Motivation: 现有动态NeRF系统在显著视角偏差下表现不佳，会产生不稳定和不真实的渲染结果，需要解决大幅视角旋转下的逼真合成问题

Method: 1. 引入高斯溅射先验；2. 采用伪真值生成策略；3. 优化密度和颜色特征；4. 创建SynDM合成数据集（首个具有明确侧视图监督的动态场景多视图数据集）

Result: 在SynDM和真实世界数据集上的定量和定性结果表明，ExpanDyNeRF在极端视角偏移下的渲染保真度显著优于现有动态NeRF方法

Conclusion: ExpanDyNeRF通过高斯溅射先验和伪真值生成策略，有效解决了动态NeRF在大幅视角旋转下的渲染质量问题，为动态场景的新视角合成提供了更鲁棒的解决方案
...
Method: 提出分布感知分数解码器(DISCODE)，采用测试时自适应评估方法，引入自适应测试时(ATT)损失，利用高斯先验分布提高评估分数的鲁棒性，并通过推导的解析解在测试时高效最小化损失。

Result: DISCODE在新建的MCEval基准（覆盖六个不同领域）和四个现有代表性基准上，作为无参考评估指标取得了最先进的性能。

Conclusion: DISCODE提供了一种无需微调的鲁棒图像描述评估方法，通过测试时自适应机制和分布感知解码，在不同领域都能生成更符合人类判断的评估分数，为解决域迁移下的评估挑战提供了有效方案。
...
TL;DR: SynPS方法通过协同利用位置嵌入和语义信息，解决现有注意力共享机制中的注意力崩溃问题，实现更忠实、平衡的非刚性图像编辑
...
Method: 提出SynPS方法：1）首先提出编辑测量指标，量化每个去噪步骤所需的编辑幅度；2）设计注意力协同管道，动态调节位置嵌入的影响，平衡语义修改和保真度保留；3）自适应整合位置和语义线索，避免过度编辑和编辑不足

Result: 在公开数据集和新构建的基准测试上进行广泛实验，证明了SynPS方法在性能和忠实度方面的优越表现

Conclusion: 通过协同利用位置嵌入和语义信息，SynPS有效解决了注意力崩溃问题，实现了更忠实、平衡的非刚性图像编辑，在复杂编辑任务中表现出色
...
TL;DR: 该论文提出了一种结合分数生成模型和消息传递的压缩图像恢复算法STMP，以及针对量化测量的Q-STMP变体，实现了更好的性能-复杂度权衡。


<details>
  <summary>Details</summary>
Motivation: 传统PnP方法使用的去噪器依赖通用或手工先验，难以准确捕捉自然图像的复杂统计结构，导致在高度欠定情况下的次优重建。虽然分数生成模型能准确表征图像分布，但直接用于后验采样计算复杂度过高。

Method: 利用分数生成建模与经验贝叶斯去噪的紧密联系，设计了一个集成分数MMSE去噪器的消息传递框架STMP。对于量化测量系统，进一步提出Q-STMP，增加分量级MMSE去量化模块。通过状态演化方程预测算法性能。

Result: 在FFHQ数据集上的实验表明，STMP相比竞争基线实现了显著更好的性能-复杂度权衡，Q-STMP在1比特量化下仍保持鲁棒性。两种算法通常都在10次迭代内收敛。

Conclusion: STMP和Q-STMP算法成功结合了消息传递的快速收敛性和分数生成先验的表达能力，为压缩图像恢复提供了一种高效且性能优越的解决方案。
...
TL;DR: 提出了一种仅使用真实视频数据进行训练的无监督视频实例分割方法，通过识别高质量关键掩码并利用稀疏到密集的蒸馏方法，在多个基准测试中超越了现有技术。


<details>
  <summary>Details</summary>
Motivation: 当前无监督视频实例分割主要依赖从图像数据集生成的合成视频数据，但这种方法无法准确建模真实视频中的复杂运动（如视角变化、部分运动、相机运动等），因此需要开发仅使用真实视频数据的训练方法。

Method: 1. 从单帧无监督实例分割开始；2. 利用深度运动先验识别视频中的高质量关键掩码；3. 提出稀疏到密集的蒸馏方法，配合时间DropLoss训练分割模型进行隐式掩码传播；4. 在生成的密集标签集上训练最终模型。

Result: 该方法在多个基准测试中超越了当前最先进的无监督视频实例分割方法。

Conclusion: 仅使用真实视频数据训练的无监督视频实例分割方法能够有效解决合成数据无法准确建模真实运动的问题，通过关键掩码识别和稀疏到密集的蒸馏策略取得了优异的性能。
...
TL;DR: 提出A4-Agent框架，通过解耦可负担性预测为三个阶段，无需训练即可实现零样本泛化，显著超越监督方法。


<details>
  <summary>Details</summary>
Motivation: 现有端到端模型将高层推理和低层定位耦合在单一流程中，依赖标注数据集训练，导致在新对象和未见环境上泛化能力差。

Method: 提出训练免费的A4-Agent框架，将可负担性预测解耦为三个阶段：Dreamer使用生成模型可视化交互过程；Thinker利用大视觉语言模型决定交互对象部件；Spotter协调视觉基础模型精确定位交互区域。

Result: 零样本框架在多个基准测试中显著优于最先进的监督方法，并在真实世界环境中展现出强大的泛化能力。

Conclusion: 通过解耦可负担性预测为专门化阶段并利用预训练模型的互补优势，无需任务特定微调即可实现卓越性能，为具身AI提供了新的范式。
...
Method: 创建并公开一个包含三种不同隧道衬砌类型（不同纹理和施工技术）的标注图像数据集，涵盖裂缝、渗漏和水渗透等典型缺陷，支持监督、半监督和无监督深度学习方法。

Result: 提供了一个新的公开可用隧道缺陷数据集，其多样性使研究人员能够研究模型在不同隧道类型间的泛化能力和可迁移性。
...
<details>
  <summary>Details</summary>
Motivation: 缺乏专门的意大利手语（LIS）识别数据集，需要创建高质量的数据集来研究手语识别任务，特别是针对意大利手语。

Method: 创建了包含644个视频的SignIT数据集，手动标注了94个手势类别（分为动物、食物、颜色、情感和家庭5个宏观类别），提取了用户手、脸和身体的2D关键点，并采用多种最先进模型进行基准测试。

Result: 结果显示现有模型在这个具有挑战性的LIS数据集上存在局限性，时间信息、2D关键点和RGB帧对模型性能有不同影响。

Conclusion: SignIT数据集为意大利手语识别研究提供了有价值的资源，揭示了当前模型的局限性，并发布了数据和标注供进一步研究使用。
...
TL;DR: ReVision是一个从大规模远程医疗项目中学习的视网膜基础模型，无需特定任务训练即可实现高精度疾病检测，在低资源环境中具有显著部署优势。
...
Method: 利用中国162家医疗机构长达十年的远程医疗项目，从485,980张彩色眼底照片及其对应诊断报告的自然对齐中学习。通过大规模临床数据构建ReVision模型，无需额外标注。

Result: 在27个眼科基准测试中表现优异：零样本疾病检测在12个公共基准上平均AUROC为0.946，在3个独立临床队列上为0.952。仅需极少参数和标注样本即可匹配精细调优模型。在33名眼科医生的前瞻性研究中，零样本辅助将诊断准确率提高了14.8%。

Conclusion: 临床原生智能可以直接从临床档案中提取，无需额外标注即可构建适用于各种低资源环境的医疗AI系统，显著提高部署效率和诊断准确性。
...
Method: 提出DASP框架，包含对抗分支和自监督分支。对抗分支使用包含四个时空先验学习块（SPLB）的判别器提取白天先验，SPLB包含空间时间学习模块（STLM）和轴向空间学习模块（ASLM）。自监督分支提出3D一致性投影损失，将目标帧和源帧双向投影到共享3D空间，计算投影帧间的3D差异作为损失

Result: 在Oxford RobotCar和nuScenes数据集上的大量实验表明，该方法在夜间深度估计方面达到了最先进的性能。消融研究进一步验证了每个组件的有效性

Conclusion: DASP框架通过利用时空先验，有效解决了夜间深度估计中的纹理缺失和动态物体模糊问题，实现了优异的性能
...
TL;DR: CAPRMIL提出了一种新的多实例学习框架，通过将补丁特征投影到少量全局上下文感知的token中，使用多头自注意力注入全局上下文，配合简单的均值聚合器，在多个病理学基准测试中达到SOTA性能，同时大幅减少参数和计算量。


<details>
  <summary>Details</summary>
Motivation: 在计算病理学中，由于WSI的千兆像素尺度和像素级标注稀缺，弱监督已成为深度学习的标准方法，多实例学习是主要的框架。现有MIL方法依赖复杂的基于注意力的聚合，计算复杂度高。本文旨在提出更高效的MIL框架，将相关性学习从聚合器中移除，降低计算复杂度。

Method: CAPRMIL采用聚合器无关的框架：1) 使用冻结的补丁编码器提取补丁特征；2) 将补丁特征投影到少量全局上下文/形态感知的token中；3) 使用多头自注意力注入全局上下文，计算复杂度相对于包大小呈线性；4) 配合简单的均值MIL聚合器进行下游任务。

Result: 在多个公共病理学基准测试中，CAPRMIL匹配了最先进的slide-level性能，同时将可训练参数减少48%-92.8%，推理时的FLOPs降低52%-99%，在GPU内存效率和训练时间方面排名最佳模型之一。
...
### [70] [HiFi-Portrait: Zero-shot Identity-preserved Portrait Generation with High-fidelity Multi-face Fusion](https://arxiv.org/abs/2512.14542)
...
Result: 实验结果表明，该方法在面部相似度和可控性方面超越了现有最先进方法，并且与之前的SDXL-based工作兼容。

Conclusion: HiFi-Portrait通过创新的多面部特征融合和地标对齐机制，实现了高保真、高可控性的零样本肖像生成，解决了现有方法的局限性。
...
### [71] [TAT: Task-Adaptive Transformer for All-in-One Medical Image Restoration](https://arxiv.org/abs/2512.14550)
*Zhiwen Yang,Jiaju Zhang,Yang Yi,Jian Liang,Bingzheng Wei,Yan Xu*

Main category: cs.CV

TL;DR: 本文提出任务自适应Transformer（TAT）框架，通过任务自适应权重生成和损失平衡策略，解决医学图像恢复中多任务学习的任务干扰和任务不平衡问题。


<details>
  <summary>Details</summary>
Motivation: 医学图像恢复（MedIR）中的All-in-One模型需要同时处理多种不同任务，但由于模态和退化类型的显著差异，共享模型面临两个关键挑战：任务干扰（不同任务在同一参数上产生冲突的梯度更新方向）和任务不平衡（不同任务学习难度不同导致优化不均衡）。

Method: 提出任务自适应Transformer（TAT）框架，包含两个核心创新：1）任务自适应权重生成策略，为每个任务生成特定权重参数，避免共享权重上的梯度冲突；2）任务自适应损失平衡策略，根据任务特定学习难度动态调整损失权重，防止任务主导或训练不足。
...
TL;DR: CLNet提出了一种基于对应感知的特征细化框架，通过三个可学习模块显式建模跨视图的空间对应关系，在图像检索跨视角地理定位任务中实现最先进性能。


<details>
  <summary>Details</summary>
Motivation: 现有方法主要依赖学习鲁棒的全局表示或隐式特征对齐，往往无法建模对精确定位至关重要的显式空间对应关系，存在语义和几何差距问题。
...
Conclusion: CLNet通过显式建模跨视图的空间对应关系，能够同时捕获高层语义和细粒度对齐，有效解决了图像检索跨视角地理定位中的语义和几何差距问题。
...
<details>
  <summary>Details</summary>
Motivation: 现有深度伪造检测方法通常依赖于特定伪造线索，在已知伪造类型上表现良好，但对新兴伪造技术泛化能力差。这种泛化能力不足源于无法有效适应未见过的伪造模式。

Method: 1. 利用大规模预训练模型（如CLIP）主动探测特征空间，明确突出真实视频、已知伪造和未知伪造之间的分布差异；2. 引入伪造异常值探测，通过动态子聚类建模和聚类条件异常值生成，在估计的子聚类边界附近合成异常样本，模拟超出已知伪造类型的新伪造伪影；3. 设计异常值引导的三重训练，使用提出的异常值驱动对比学习和异常值条件交叉熵损失来优化检测器区分真实、伪造和异常样本。

Result: 实验表明，FakeRadar在各种深度伪造视频检测基准数据集上优于现有方法，特别是在跨域评估中，能够有效处理各种新兴伪造技术。

Conclusion: FakeRadar通过主动探测特征空间中的分布差异和合成异常样本来模拟未知伪造模式，显著提升了深度伪造检测的跨域泛化能力，为解决新兴伪造技术的检测挑战提供了有效方案。
...
TL;DR: 该研究针对智能交通系统中传统帧相机在弱光、高速运动场景下的性能不足，提出了首个面向事件相机的ITS数据集，并建立了基于检测的跟踪基准，取得了优异性能。


<details>
  <summary>Details</summary>
Motivation: 传统帧相机在智能交通系统中存在弱光环境和高速运动条件下的性能缺陷，而事件相机具有低延迟、高动态范围和高时间分辨率的优势，但相关研究较少。研究旨在填补事件视觉在ITS领域的研究空白。

Method: 1. 创建了首个专门针对事件相机的ITS数据集，涵盖车辆和行人的检测与跟踪任务；2. 基于该数据集建立了基于检测的跟踪基准；3. 开发了专门的特征提取器。

Result: 在该数据集上建立的跟踪基准取得了优异的性能表现，验证了事件相机在ITS场景下的应用潜力。

Conclusion: 事件相机在智能交通系统中具有显著优势，特别是在弱光和高速运动条件下。该研究通过创建专用数据集和基准，为事件视觉在ITS领域的研究奠定了基础，展示了事件相机技术的应用前景。
...
TL;DR: 该研究首次构建了多中心淋巴瘤基准数据集，系统评估了五种病理学基础模型与两种聚合方法在淋巴瘤亚型分类中的性能，发现模型在分布内测试集表现良好（平衡准确率>80%），但在分布外测试集性能显著下降至约60%，揭示了泛化挑战。


<details>
  <summary>Details</summary>
Motivation: 淋巴瘤的及时准确诊断对癌症治疗至关重要，但标准诊断流程需要昂贵的设备、专业人员且导致治疗延迟。深度学习方法可以从常规HE染色切片中提取诊断信息，但目前缺乏基于多中心数据的淋巴瘤亚型分类综合基准。

Method: 构建首个多中心淋巴瘤基准数据集，涵盖四种常见淋巴瘤亚型和健康对照组织。系统评估五种公开病理学基础模型（H-optimus-1, H0-mini, Virchow2, UNI2, Titan）与两种聚合方法（基于注意力的AB-MIL和基于Transformer的TransMIL），在三个放大倍数（10x, 20x, 40x）下进行测试。

Result: 在分布内测试集上，所有放大倍数下模型均达到超过80%的多类平衡准确率，所有基础模型表现相似，两种聚合方法结果相当。放大倍数研究表明40x分辨率足够，更高分辨率或跨放大倍数聚合未带来性能提升。但在分布外测试集上，性能显著下降至约60%。

Conclusion: 该研究揭示了淋巴瘤亚型分类中深度学习模型的泛化挑战，需要更大规模的多中心研究覆盖更多罕见淋巴瘤亚型。提供了自动化基准测试流程以促进未来研究。
...
TL;DR: WorldPlay是一个实时交互式世界建模的流式视频扩散模型，通过三项关键技术解决了速度与内存之间的权衡，实现了24FPS的720p长序列视频生成。


<details>
  <summary>Details</summary>
Motivation: 当前方法在实时交互式世界建模中存在速度与内存之间的权衡限制，难以同时实现长时几何一致性和实时响应。

Method: 1) 双动作表示实现鲁棒的动作控制；2) 重构上下文记忆通过动态重建过去帧上下文和时域重帧保持几何重要帧可访问；3) 上下文强制蒸馏方法保持学生模型使用长程信息的能力。

Result: WorldPlay能以24FPS生成720p长序列流式视频，具有优越的一致性，相比现有技术表现更好，并在多样场景中展现出强泛化能力。

Conclusion: WorldPlay通过创新的记忆管理和蒸馏技术，成功解决了实时交互式世界建模中的速度-内存权衡问题，实现了高质量、一致的长序列视频生成。
...
TL;DR: SFVD提出单帧视频集蒸馏框架，将视频蒸馏为每类的高信息量单帧，通过可微分插值生成视频序列并与原数据集匹配，显著提升视频数据集蒸馏性能。
...
Method: 提出单帧视频集蒸馏(SFVD)框架：1) 将视频蒸馏为每类的高信息量单帧；2) 通过可微分插值将这些帧转换为视频序列并与原始数据集匹配；3) 更新仅限于帧本身以提高优化效率；4) 通过通道重塑层将蒸馏帧与采样的真实视频结合以融入时间信息。

Result: 在多个基准测试上的广泛实验表明，SFVD显著优于先前方法，在MiniUCF上实现了高达5.3%的性能提升。

Conclusion: SFVD通过将视频蒸馏为单帧并采用优化效率更高的框架，为视频数据集蒸馏提供了更有效的解决方案，克服了传统方法在视频数据上的局限性。
...
TL;DR: AMD-HookNet++提出了一种混合CNN-Transformer特征增强方法，用于合成孔径雷达图像中的冰川分割和崩解前沿提取，解决了纯CNN模型长距离依赖不足和纯Transformer方法边缘锯齿状的问题。


<details>
  <summary>Details</summary>
Motivation: 冰川和冰架前沿动态对冰盖质量平衡和沿海海平面有重要影响。现有AMD-HookNet纯CNN模型虽然能捕捉局部细节，但卷积操作的局部性和平移不变性限制了模型保持长距离依赖的能力，需要改进。

Method: 提出AMD-HookNet++混合CNN-Transformer架构：1）Transformer分支捕获长距离依赖和全局上下文信息；2）CNN分支保留局部细节；3）增强的空间通道注意力模块动态调整空间和通道维度的token关系；4）像素级对比深度监督将像素度量学习集成到冰川分割中。

Result: 在CaFFe冰川分割基准数据集上，AMD-HookNet++达到IoU 78.2和HD95 1,318米的新SOTA，同时保持MDE 367米的竞争力。更重要的是，混合模型产生了更平滑的崩解前沿轮廓，解决了纯Transformer方法的锯齿边缘问题。

Conclusion: AMD-HookNet++通过混合CNN-Transformer架构成功结合了全局上下文信息和局部细节，在冰川分割和崩解前沿提取任务上取得了显著改进，为冰川监测提供了更准确的技术支持。
...
### [79] [ViRC: Enhancing Visual Interleaved Mathematical CoT with Reason Chunking](https://arxiv.org/abs/2512.14654)
*Lihong Wang,Liangqi Li,Weiwei Feng,Jiamin Wu,Changtao Miao,Tieru Wu,Rui Ma,Bo Zhang,Zhe Li*

Main category: cs.CV

TL;DR: ViRC框架通过引入Reason Chunking机制，将多模态数学推理分解为连续的Critical Reasoning Units，模拟人类专家解题模式，在多个数学基准上平均提升18.8%


<details>
  <summary>Details</summary>
Motivation: 现有MLLMs在数学任务中通常只从静态图像进行文本推理，忽视了推理过程中的动态视觉获取。而人类会反复检查视觉图像并采用逐步推理来证明中间命题，这种分解问题解决过程为关键逻辑节点的策略符合认知科学中的Miller定律。

Method: 提出ViRC框架，引入Reason Chunking机制将多模态数学CoT结构化为连续的Critical Reasoning Units（CRUs）。CRUs确保单元内文本连贯性用于中间命题验证，同时跨单元整合视觉信息生成后续命题。创建CRUX数据集，使用三种视觉工具和四种推理模式为每个数学问题提供显式标注的CRUs。采用渐进式训练策略：Instructional SFT、Practice SFT和Strategic RL。
...
### [80] [Mitigating Catastrophic Forgetting in Mathematical Reasoning Finetuning through Mixed Training](https://arxiv.org/abs/2512.13706)
*John Graham Reynolds*

Main category: cs.LG

TL;DR: 论文研究了大型语言模型在数学推理等专门任务微调时出现的灾难性遗忘问题，通过混合训练策略成功消除遗忘，在保持数学性能的同时保留通用能力。


<details>
  <summary>Details</summary>
Motivation: 当为数学推理等专门任务微调大型语言模型时，模型会出现灾难性遗忘，失去先前学习的能力。研究者希望探索如何在保持专业性能的同时避免遗忘通用能力。
...
Conclusion: 专业化不一定需要遗忘通用能力，混合训练策略可以有效防止灾难性遗忘，在保持专门任务性能的同时保留模型的一般能力。这对于扩展到更大模型具有重要意义，混合训练可能带来超越遗忘预防的额外好处。
...
### [81] [Predictive Modeling of Flood-Prone Areas Using SAR and Environmental Variables](https://arxiv.org/abs/2512.13710)
*Edwin Oluoch Awino,Denis Machanda*

Main category: cs.LG

TL;DR: 该研究结合SAR遥感影像与环境水文数据，使用机器学习方法对肯尼亚Nyando河流域进行洪水易发性建模，发现随机森林模型表现最佳，识别出维多利亚湖附近的Kano平原为最高风险区。


<details>
  <summary>Details</summary>
Motivation: 洪水是全球最具破坏性的自然灾害之一，对生态系统、基础设施和人类生计构成严重威胁。本研究旨在结合SAR遥感数据与环境水文数据，为数据有限的地区开发有效的洪水易发性评估方法，以支持灾害风险减少和土地利用规划。

Method: 使用Sentinel-1双极化SAR数据处理2024年5月洪水事件，生成二进制洪水清单作为训练数据。整合六个环境因子（坡度、高程、坡向、土地利用/土地覆盖、土壤类型、距河流距离）与SAR洪水清单，训练四种监督分类器：逻辑回归、分类回归树、支持向量机和随机森林。使用准确率、Cohen's Kappa和ROC分析评估模型性能。

Result: 随机森林模型取得了最高的预测性能（准确率=0.762；Kappa=0.480），优于其他三种模型。基于随机森林的易发性图显示，维多利亚湖附近的低洼Kano平原具有最高的洪水脆弱性，这与历史洪水记录和2024年5月洪水事件的影响一致。

Conclusion: 研究证明了结合SAR数据和集成机器学习方法在数据有限地区进行洪水易发性制图的价值。生成的洪水易发性图为灾害风险减少、土地利用规划和早期预警系统开发提供了重要见解。
...
### [82] [Delete and Retain: Efficient Unlearning for Document Classification](https://arxiv.org/abs/2512.13711)
*Aadya Goel,Mayuri Sridhar*

Main category: cs.LG

TL;DR: 本文提出了一种名为"Hessian Reassignment"的两步法，用于文档分类模型的类别级遗忘，通过Hessian向量系统计算影响更新，并采用Top-1分类保证决策空间，在保持准确性的同时显著提升效率。


<details>
  <summary>Details</summary>
Motivation: 机器学习遗忘旨在高效地从模型中移除特定训练数据的影响而无需完全重新训练。虽然在大语言模型的遗忘方面取得了进展，但文档分类模型的遗忘研究相对不足，特别是类别级别的遗忘问题。

Method: 提出Hessian Reassignment方法：1）通过求解Hessian-向量系统（使用共轭梯度法）执行单次影响式更新，减去目标类别所有训练点的影响；2）采用Top-1分类强制执行决策空间保证，而不是像常见基线那样随机重新分类删除类别的样本。

Result: 在标准文本基准测试中，Hessian Reassignment在保持接近完整重新训练（不含目标类）的准确性的同时，运行速度提高了几个数量级。此外，通过池化多影子攻击测量，该方法持续降低了移除类别的成员推断优势。
...
### [83] [Prediction of Respiratory Syncytial Virus-Associated Hospitalizations Using Machine Learning Models Based on Environmental Data](https://arxiv.org/abs/2512.13712)
*Eric Guo*
...
<details>
  <summary>Details</summary>
Motivation: 呼吸道合胞病毒（RSV）是导致幼儿住院的主要原因，其暴发受环境条件强烈影响。研究旨在开发预测模型，结合多源数据提前预警RSV暴发，为公共卫生干预和资源分配提供支持。

Method: 整合每周住院率、废水RSV水平、每日气象测量和空气污染物浓度数据，训练CART、随机森林和提升等分类模型，预测RSV相关住院率分为低风险、警报和流行三个等级。

Result: 废水RSV水平被确定为最强预测因子，其次是温度、臭氧水平和比湿度等气象和空气质量变量。研究发现美洲原住民和阿拉斯加原住民的RSV相关住院率显著更高，高海拔地区（地表压力较低）的住院率也持续较高。

Conclusion: 结合环境和社区监测数据可有效预测RSV暴发，为及时公共卫生干预提供支持。研究开发了交互式R Shiny仪表板，便于用户探索不同州的RSV风险水平、可视化关键预测因子影响并生成暴发预测。
...
### [84] [Federated Few-Shot Learning for Epileptic Seizure Detection Under Privacy Constraints](https://arxiv.org/abs/2512.13717)
...
<details>
  <summary>Details</summary>
Motivation: 临床实践中EEG数据稀缺、分布在不同机构且受隐私法规限制，无法集中使用，导致传统深度学习模型难以应用于实际医疗场景。

Method: 两阶段方法：第一阶段使用联邦学习在模拟医院站点上微调预训练的BIOT模型；第二阶段使用联邦少样本个性化，每个患者仅用5个标记EEG片段适配分类器。

Result: 联邦微调获得平衡准确率0.43（集中式0.52）；FFSL阶段在四个站点上平均平衡准确率达0.77，Cohen's kappa 0.62，加权F1 0.73。
...
### [85] [Time-Constrained Recommendations: Reinforcement Learning Strategies for E-Commerce](https://arxiv.org/abs/2512.13726)
*Sayak Chakrabarty,Souradip Pal*

Main category: cs.LG

TL;DR: 该论文提出在有限时间预算约束下的推荐系统问题，使用强化学习同时学习用户偏好和时间预算模式，在阿里巴巴个性化重排序数据集上进行实验，证明强化学习方法在严格时间预算下优于传统上下文赌博机方法。


<details>
  <summary>Details</summary>
Motivation: 传统推荐任务忽略了用户有限的时间预算这一关键资源约束。在移动购物界面等场景中，用户需要花费时间评估推荐商品，高相关性的商品可能评估成本较高，无法适应用户的时间预算，从而影响用户参与度。

Method: 1. 将时间约束下的slate推荐统一建模为具有预算感知效用的马尔可夫决策过程；2. 开发模拟框架研究重排序数据上的策略行为；3. 使用强化学习算法（包括on-policy和off-policy控制）同时学习用户偏好和时间预算模式。

Result: 实验结果表明，在严格时间预算约束下，基于强化学习的on-policy和off-policy控制方法相比传统的上下文赌博机方法能够获得更好的性能表现。

Conclusion: 该研究证明了强化学习在时间约束推荐系统中的有效性，能够平衡项目相关性和评估成本，在有限用户时间预算下提高用户参与度潜力。
...
TL;DR: 该论文提出了复合无分类器引导（CCFG）方法，用于改进多通道风数据超分辨率，并开发了WindDM扩散模型，在风动力学重建任务上实现了最先进的性能，成本比传统方法低1000倍。


<details>
  <summary>Details</summary>
Motivation: 高分辨率、高精度的风数据对于天气建模、风力涡轮机布局优化等应用至关重要，但获取成本高昂且困难。传统重建方法要么成本低但精度不足，要么精度高但成本昂贵。风数据与自然图像不同，通常需要10个以上的输入通道，而现有的扩散模型主要针对3通道RGB图像设计，需要新的方法来处理多通道条件输入。

Method: 提出了复合无分类器引导（CCFG）方法，这是对标准无分类器引导（CFG）的泛化，能够更好地处理多个条件输入。CCFG可以无缝集成到任何使用标准CFG丢弃训练好的扩散模型中。基于CCFG开发了WindDM扩散模型，专门用于工业规模的风动力学重建任务。

Result: CCFG在风数据超分辨率任务上比标准CFG产生更高保真度的输出。WindDM在深度学习模型中实现了最先进的重建质量，同时成本比传统经典方法降低了1000倍。

Conclusion: 复合无分类器引导（CCFG）方法有效解决了风数据多通道条件输入的问题，WindDM扩散模型为工业规模的风动力学重建提供了一种既高精度又成本效益显著的解决方案。
...
TL;DR: PIS是一种基于集合条件扩散的物理反演框架，能够在任意稀疏、不规则观测条件下稳定求解PDE约束参数反演问题，显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: PDE约束物理参数反演在稀疏、不规则观测条件下本质上是病态问题，现有深度学习和算子学习方法在极端稀疏观测下失效，缺乏鲁棒性和不确定性量化能力。

Method: 提出物理反演求解器(PIS)：1) 使用基于Set Transformer的编码器处理任意数量和几何分布的观测；2) 采用余弦退火稀疏课程学习实现卓越鲁棒性；3) 基于信息论分析揭示极端稀疏条件下的反演极限。

Result: 在Darcy流、波场反演(Helmholtz)和结构健康监测(Hooke定律)三个PDE反问题上，PIS在极端稀疏观测(仅0.29%观测率)下保持稳定准确，反演误差降低12.28%-88.73%，并能生成校准的后验样本。
...
TL;DR: LLRC是一种无需微调的梯度学习方法，通过学习掩码权重选择奇异值，优化大语言模型的低秩压缩，在保持性能的同时实现高效压缩。


<details>
  <summary>Details</summary>
Motivation: 现有低秩压缩方法在层间秩选择上存在局限：启发式方法搜索空间有限导致次优结果，梯度方法在无微调情况下性能不如启发式方法。需要一种无需后压缩微调就能优化压缩率和下游任务准确性的方法。

Method: 提出LLRC（Learning to Low-Rank Compress），一种基于梯度的掩码权重学习方法。使用校准数据集，仅训练掩码权重来选择更少的奇异值，同时最小化中间激活与原始模型的差异，实现无需微调的压缩。

Result: 在常识推理和开放域问答任务上，LLRC在无需后压缩微调的方法中表现最佳。以Llama-2-13B 20%压缩率为例，在MMLU、BoolQ和OpenbookQA上分别比STRS提升12%、3.5%和4.4%。相比其他压缩技术，在无需微调的SVD-LLM和LLM-Pruner变体上表现更优，且与需要微调的LLM-Pruner变体性能相当。

Conclusion: LLRC提供了一种有效的无需微调的低秩压缩方法，通过学习掩码权重优化奇异值选择，在保持模型性能的同时实现高效压缩，为大型语言模型压缩提供了新的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 随着云边协作的发展，推荐服务在分布式环境中训练需求增加。联邦推荐虽然能保护隐私，但大量嵌入参数导致通信效率低下，现有研究主要关注模型效率而忽视了嵌入参数开销问题

Method: 提出基于参数高效微调(PEFT)的联邦推荐训练框架，采用轻量级插件式解决方案，整合LoRA、哈希编码等PEFT技术，并探索使用残差量化变分自编码器(RQ-VAE)作为新的PEFT策略

Result: 在不同联邦推荐模型骨干和数据集上的广泛实验表明，该框架显著降低了通信开销，同时提高了推荐准确性

Conclusion: 该研究为解决联邦推荐中嵌入参数传输效率问题提供了有效解决方案，通过PEFT技术实现了通信效率与模型性能的平衡，代码已开源
...
TL;DR: DARTs是一个用于高维多变量时间序列异常检测的双路径框架，通过短路径和长路径分别捕获短期和长期的时空依赖关系，并使用窗口感知的时空软融合机制来整合异常模式。


<details>
  <summary>Details</summary>
Motivation: 现有方法在低维场景下能有效识别异常模式，但在处理高维噪声时间序列时难以鲁棒地捕获长距离时空依赖关系，这限制了它们在大规模工业控制系统中的应用效果。

Method: 提出DARTs框架，包含三个互补组件：1) 短路径：使用多视图稀疏图学习器和扩散多关系图单元自适应捕获高噪声时间序列中的分层判别性短期时空模式；2) 长路径：设计多尺度时空图构造器在高维表示空间中建模显著的长期动态；3) 窗口感知时空软融合机制：过滤残余噪声并无缝整合异常模式。

Result: 在主流数据集上的大量定性和定量实验结果表明，DARTs在性能和鲁棒性方面具有优越性。消融研究也验证了所提出组件关键设计因素的有效性。

Conclusion: DARTs通过双路径框架和软融合机制，能够有效处理高维噪声时间序列中的异常检测问题，显著提升了捕获长距离时空依赖关系的能力，为大规模工业控制系统提供了鲁棒的异常检测解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有抑郁症EEG检测方法过度依赖标签，而对比学习方法又未能充分利用EEG信号的时频分布特性，导致低语义数据表示能力不足。

Method: 提出TF-MCL模型：1）使用融合映射头生成时频混合表示；2）通过多域交叉损失函数优化时频域和融合域的表示分布。

Result: 在MODMA和PRED+CT数据集上，准确率分别比现有最佳方法提高5.87%和9.96%。

Conclusion: TF-MCL模型能有效增强时频信息融合能力，提升抑郁症EEG检测性能，为解决标签稀缺问题提供了有效方案。
...
<details>
  <summary>Details</summary>
Motivation: 随着大语言模型普及，针对对抗性"越狱"攻击的防御变得重要。现有防御策略依赖计算昂贵的外部分类器或脆弱的词汇过滤器，忽略了模型推理过程的内在动态。

Method: 提出层流假说，认为良性输入在LLM高维潜在空间中产生平滑渐变过渡，而对抗性提示引发"语义湍流"。通过层间余弦速度方差这一零样本指标量化这种动态差异。

Result: 实验表明：RLHF对齐的Qwen2-1.5B在攻击下湍流增加75.4%（p<0.001），验证了内部冲突假说；Gemma-2B则显示22.0%的湍流减少，表现出不同的"基于反射"的拒绝机制。

Conclusion: 语义湍流不仅可作为轻量级实时越狱检测器，还能作为非侵入式诊断工具，用于分类黑盒模型的基础安全架构类型。
...
TL;DR: 该研究比较了在资源受限环境下基于嵌入的金融新闻情感分类方法，发现当数据量不足时，预训练嵌入无法解决根本的数据稀缺问题，模型表现甚至低于简单基线。


<details>
  <summary>Details</summary>
Motivation: 金融情感分析能增强市场理解，但标准NLP方法在小数据集上遇到显著挑战。研究旨在评估资源受限环境下基于嵌入的金融新闻情感分类方法的有效性。

Method: 比较评估Word2Vec、GloVe和句子转换器表示，结合梯度提升算法在手动标注的新闻标题上进行实验。通过验证集和测试集性能对比分析模型表现。

Result: 实验结果显示验证集和测试集性能存在显著差距，尽管验证指标良好，但模型表现甚至低于简单基线。预训练嵌入在数据量低于临界阈值时收益递减，小验证集导致模型选择过程中的过拟合。

Conclusion: 嵌入质量本身无法解决情感分类中的根本数据稀缺问题。对于资源有限的实践者，当标注样本稀缺时，需要考虑替代方法如少样本学习、数据增强或词典增强的混合方法。
...
### [94] [Constrained Policy Optimization via Sampling-Based Weight-Space Projection](https://arxiv.org/abs/2512.13788)
*Shengfan Cao,Francesco Borrelli*

Main category: cs.LG

TL;DR: SCPO是一种基于采样的权重空间投影方法，用于约束策略学习，通过轨迹采样和光滑性边界构建局部安全区域，使用凸SOCP进行投影更新，确保训练过程中的安全性


<details>
  <summary>Details</summary>
Motivation: 安全关键学习需要在不离开安全操作区域的情况下改进性能。研究约束策略学习问题，其中模型参数必须满足未知的、基于rollout的安全约束，需要一种无需约束函数梯度访问的安全学习方法

Method: 提出SCPO方法：1）结合轨迹采样和平滑性边界构建局部安全区域；2）通过凸二阶锥规划（SOCP）进行权重空间投影；3）使用安全归纳保证确保所有中间策略的安全性；4）在有稳定备份策略的约束控制设置中确保闭环稳定性

Result: 在有害监督的回归任务和带有恶意专家的约束双积分器任务中，SCPO能够一致拒绝不安全更新，在整个训练过程中保持可行性，并实现有意义的原始目标改进

Conclusion: SCPO提供了一种无需约束梯度访问的安全学习方法，通过权重空间投影确保训练过程中的安全性，在安全关键应用中具有实用价值，特别适用于需要超出保守备份策略进行安全适应的场景
...
### [95] [The Double Life of Code World Models: Provably Unmasking Malicious Behavior Through Execution Traces](https://arxiv.org/abs/2512.13821)
*Subramanyam Sahoo,Jared Junkin*

Main category: cs.LG

TL;DR: 提出CTVP框架，通过语义轨道分析验证不可信代码生成模型，检测后门注入和恶意行为，无需直接执行潜在恶意代码。


<details>
  <summary>Details</summary>
Motivation: 随着大语言模型越来越多地生成代码且人类监督减少，后门注入和恶意行为成为关键问题，需要新的AI控制框架来验证不可信代码生成模型。

Method: 提出跨轨迹验证协议(CTVP)，利用模型自身对语义等价程序变换的执行轨迹预测，通过分析这些预测轨迹的一致性模式来检测后门行为异常。

Result: 引入对抗鲁棒性商数(ARQ)量化验证计算成本相对于基线生成的比率，显示随轨道大小呈指数增长。理论分析建立信息论边界，证明不可博弈性。

Conclusion: 语义轨道分析为代码生成任务提供了可扩展、理论基础的AI控制方法，能够有效检测后门行为，且对抗者无法通过训练改进。
...
### [96] [Explainable reinforcement learning from human feedback to improve alignment](https://arxiv.org/abs/2512.13837)
...
<details>
  <summary>Details</summary>
Motivation: 人类通过寻找并修正问题原因来改进结果，作者希望将这一策略应用于改进语言模型的RLHF对齐，因为现有RLHF模型仍会产生不满意响应。

Method: 方法分为两部分：1) 事后解释方法：通过约束组合优化问题识别导致不满意响应的训练数据；2) 反学习方法：通过反学习这些训练数据来改进不满意响应，同时不显著影响其他满意响应。

Result: 实验结果表明，该方法能够有效改进RLHF的性能。

Conclusion: 将人类改进策略应用于RLHF是可行的，通过识别并修正导致不满意响应的训练数据可以改进语言模型的对齐效果。
...
TL;DR: 本文为二元分类器的L1校准误差估计提供了两个贡献：1）为有界变差校准函数的分类器提供上界；2）提出一种修改分类器的方法，使其校准误差能被有效上界估计，且不影响性能。所有结果都是非渐近且分布无关的。


<details>
  <summary>Details</summary>
Motivation: 从有限数据集估计二元分类器的L1校准误差是一个重要但具有挑战性的问题。现有方法可能存在假设限制或计算效率问题，需要开发更实用、分布无关的校准误差估计方法。

Method: 1）为具有有界变差校准函数的分类器提供理论上的L1校准误差上界；2）提出一种分类器修改方法，使得修改后的分类器能够在不显著影响性能的情况下，其校准误差可以被有效上界估计，且无需限制性假设。

Result: 开发了非渐近、分布无关的校准误差估计方法，提供了实用的计算程序，能够在真实世界数据集上以适度的计算开销运行，为实际中如何测量校准误差提供了具体建议。

Conclusion: 本文提出的方法为二元分类器的L1校准误差估计提供了实用且理论保证的解决方案，能够在实际应用中有效评估分类器的校准性能，同时保持分类器性能不受显著影响。
...
TL;DR: 提出一个结合去噪自编码器、卷积分词器和Transformer编码器的端到端婴儿哭声分析系统，采用联邦学习训练，实现隐私保护、噪声鲁棒和通信高效的婴儿哭声分类。


<details>
  <summary>Details</summary>
Motivation: 婴儿哭声分类有助于早期评估婴儿需求，但现有方案面临隐私问题（音频数据）、背景噪声敏感性和跨录音环境的域偏移等部署限制。

Method: 1) 端到端婴儿哭声分析管道：集成去噪自编码器(DAE)、卷积分词器和Transformer编码器；2) 联邦学习训练：采用带正则化控制变量更新的8位适配器增量，在安全聚合下进行；3) 系统功能：设备端去噪、自适应分割、事后校准和基于能量的分布外(OOD)弃权。

Result: 在Baby Chillanto和Donate-a-Cry数据集（叠加ESC-50噪声）上：宏F1分数0.938，AUC 0.962，预期校准误差(ECE) 0.032；每轮客户端上传从约36-42MB降至3.3MB；NVIDIA Jetson Nano上实时边缘推理达到每1秒频谱图帧96ms。
...
TL;DR: OPTIMA是一种实用的单次训练后剪枝方法，通过将层间权重重构转化为独立的行向二次规划问题，在保持准确性的同时实现了大规模模型的高效剪枝。
...
Method: 将掩码选择后的层间权重重构转化为独立的行向二次规划问题，这些QP共享层间Hessian矩阵。实现加速器友好的QP求解器，每层累积一个Hessian矩阵并并行求解多个小QP，实现单次训练后剪枝。

Result: OPTIMA在多个LLM家族和稀疏度下持续提升零样本性能，最高获得3.97%的绝对准确性提升。在NVIDIA H100上，40小时内完成80亿参数transformer的端到端剪枝，峰值内存60GB。

Conclusion: OPTIMA为单次训练后剪枝设定了新的准确性与效率权衡标准，实现了实用的大规模模型剪枝而不需要微调。
...
TL;DR: 论文研究发现长上下文LLM存在"分数稀释"问题，传统推理时计算策略（如生成更多思考标记）在长上下文任务中效果有限，提出通过针对性的梯度更新来改进长上下文性能。


<details>
  <summary>Details</summary>
Motivation: 尽管现代LLM能够处理数百万标记的长上下文，但经验证据表明它们无法可靠地利用这些长文本信息。同时，推理时计算（如生成思考标记）虽然能提升多步推理任务的性能，但在长上下文任务中效果迅速减弱甚至失败。

Method: 提出一种简单方法：通过对给定上下文进行针对性的梯度更新，理论上克服静态自注意力的局限性。这种方法将推理时计算从生成更多思考标记转向对上下文进行特定训练。

Result: 该方法在各种模型和长上下文基准测试中带来持续的大幅性能提升。在Qwen3-4B模型上，LongBench-v2和ZeroScrolls基准测试子集的平均性能分别提升了12.6和14.1个百分点。

Conclusion: 对于长上下文任务，少量针对特定上下文的训练比当前推理时扩展策略（如生成更多思考标记）是更好的推理计算利用方式。该方法通过梯度更新有效解决了长上下文中的分数稀释问题。
...
TL;DR: 该研究比较了传统机器学习、深度学习和动态建模方法在南美洲降水预报中的表现，发现LSTM模型在强降水预报中表现最佳，而传统动态模型BAM表现最差。


<details>
  <summary>Details</summary>
Motivation: 降水预报对社会至关重要，但传统动态建模复杂且成本高。虽然AI技术已被用于气象预报，但缺乏对纯数据驱动方法在降水预报中可行性的广泛研究，特别是在南美洲地区。

Method: 研究比较了多种方法：传统机器学习（随机森林、XGBoost）、深度学习（1D CNN、LSTM、GRU）和传统动态建模（巴西全球大气模型BAM）。使用2019年所有季节的南美洲降水数据，并采用可解释AI分析模型行为。

Result: LSTM模型表现出最强的预测性能，特别是在强降水预报中最为准确；传统动态模型BAM表现最差；XGBoost在成本考虑下提供了较低延迟和轻微精度损失的良好平衡。

Conclusion: 深度学习模型在气候预报中具有可行性，证实了全球主要气象和气候预报中心的趋势。LSTM在精度方面表现最佳，而XGBoost在成本效益方面表现良好。
...
TL;DR: 研究使用神经ODE模型学习精确的2RDM动力学，发现只有在二粒子和三粒子关联度高的区域才能准确预测，否则需要记忆依赖的闭合方案。


<details>
  <summary>Details</summary>
Motivation: 非平衡量子多体系统的快速关联建立需要精确描述，但现有方法要么计算复杂度过高（精确波函数方法），要么忽略重要关联（平均场方法）。TD2RDM方法通过传播2RDM并闭合BBGKY层级提供中间方案，但时间局域重建泛函的有效性和存在性在不同动力学区域尚不明确。

Method: 使用神经ODE模型在精确2RDM数据上进行训练（无降维），尝试在没有显式三粒子信息的情况下再现2RDM动力学。通过分析二粒子和三粒子累积量之间的Pearson相关性来评估模型性能。

Result: 神经ODE模型仅在二粒子和三粒子累积量Pearson相关性大的参数区域能准确预测动力学。在反相关或不相关区域，模型失败，表明没有简单的瞬时二粒子累积量时间局域泛函能捕捉演化。时间平均的三粒子关联建立幅度是成功的主要预测因子：中等关联建立时，神经ODE预测和现有TD2RDM重建都准确；强关联建立时则系统性地失效。

Conclusion: 研究结果表明，在强关联建立区域需要记忆依赖核的三粒子累积量重建。神经ODE可作为模型无关的诊断工具，映射累积量展开方法的适用区域，并指导非局域闭合方案的发展。从有限数据学习高维RDM动力学为快速、数据驱动的关联量子物质模拟开辟了新途径。
...
### [103] [Sliding Window Recurrences for Sequence Models](https://arxiv.org/abs/2512.13921)
*Dragos Secrieru,Garyk Brixi,Yoshua Bengio,Taiji Suzuki,Michael Poli,Stefano Massaroli*

Main category: cs.LG

TL;DR: 论文提出了一种用于线性递归的分层分解框架，开发了滑动窗口递归（SWR）算法，并基于此构建了Phalanx层，在10亿参数多混合模型中实现了比优化Transformer快10-40%的速度提升，同时保持相同的困惑度。


<details>
  <summary>Details</summary>
Motivation: 多混合架构在语言建模中展现出更好的质量和性能潜力，但需要更高效的算法来充分利用GPU内存层次结构，减少昂贵的跨warp通信。

Method: 提出了分层分解框架用于线性递归，开发了滑动窗口递归（SWR）算法，将递归截断到硬件对齐的窗口（自然锯齿状），并基于此构建了Phalanx层作为窗口注意力或线性递归的即插即用替代方案。

Result: 在10亿参数的多混合模型中，Phalanx在4K到32K的上下文长度范围内，相比优化的Transformer实现了超过10-40%的速度提升，同时保持了相同的困惑度。

Conclusion: 滑动窗口递归和Phalanx层为多混合语言模型提供了高效的计算框架，能够在保持模型质量的同时显著提升推理速度，特别适合长上下文处理。
...
### [104] [EDGC: Entropy-driven Dynamic Gradient Compression for Efficient LLM Training](https://arxiv.org/abs/2511.10333)
...
TL;DR: EDGC是一个基于熵的动态梯度压缩框架，通过根据梯度熵变化调整压缩率来加速大语言模型训练，减少通信开销的同时保持模型性能。


<details>
  <summary>Details</summary>
Motivation: 大语言模型训练面临计算资源和内存容量的挑战，现有分布式训练技术存在通信开销大的问题。静态梯度压缩方法忽略了训练过程中梯度的动态变化，导致性能下降。如何在加速训练的同时不牺牲性能是一个挑战。

Method: 提出EDGC框架，包含三个关键组件：1) 使用下采样方法高效估计梯度熵；2) 建立压缩率与梯度熵的理论模型；3) 基于窗口的调整机制动态适应管道阶段的压缩率。

Result: 在32-NVIDIA-V100集群上训练GPT2-2.5B和在64-NVIDIA-H100集群上训练GPT2-12.1B，结果显示EDGC将通信延迟和训练时间分别减少了高达46.45%和16.13%，同时保持了大语言模型的准确性。

Conclusion: EDGC通过动态调整梯度压缩率，有效解决了大语言模型训练中的通信瓶颈问题，在显著减少训练时间的同时保持了模型性能，为大语言模型的高效训练提供了有效解决方案。
...
### [105] [Pattern-Guided Diffusion Models](https://arxiv.org/abs/2512.13945)
...
Method: PGDM首先使用原型分析提取时间序列中的固有模式，估计序列中最可能的下一个模式，然后用这个模式估计来引导扩散模型的预测，使预测结果更符合已知模式。同时引入基于原型分析的不确定性量化技术，并根据模式估计的不确定性动态调整引导强度。

Result: 在两个应用场景（视野测量和动作捕捉）中，模式引导使PGDM的性能（MAE/CRPS）分别提高了40.67%/56.26%和14.12%/14.10%。PGDM比基线方法的性能提升了65.58%/84.83%和93.64%/92.55%。

Conclusion: PGDM通过利用时间序列中的重复模式来引导扩散模型，显著提高了预测性能，特别是在存在明显重复结构的数据中效果更佳。
...
### [106] [Accelerating MHC-II Epitope Discovery via Multi-Scale Prediction in Antigen Presentation](https://arxiv.org/abs/2512.14011)
...
<details>
  <summary>Details</summary>
Motivation: MHC-II抗原表位在免疫治疗中至关重要，但与MHC-I相比，其研究面临更多挑战：结合特异性复杂、基序模式模糊、数据集较小且标准化程度低。需要更好的数据集和建模方法来推进计算免疫治疗。

Method: 1. 从IEDB和其他公共来源构建精心策划的MHC-II数据集；2. 不仅扩展和标准化现有的肽-MHC-II数据集，还引入了具有更丰富生物学背景的新型抗原-MHC-II数据集；3. 制定了肽结合、肽呈递和抗原呈递三个主要机器学习任务；4. 采用多尺度评估框架对现有模型进行基准测试；5. 使用模块化框架对该问题进行全面的建模设计分析。

Result: 创建了一个有价值的MHC-II数据集资源，该数据集更全面、标准化程度更高，并包含更丰富的生物学背景。建立了三个逐步捕获MHC-II抗原呈递途径中更广泛生物学过程的机器学习任务框架。通过多尺度评估为现有模型提供了基准，并通过模块化分析为建模设计提供了见解。

Conclusion: 这项工作为推进计算免疫治疗提供了宝贵资源，为未来机器学习指导的表位发现和免疫反应预测建模研究奠定了基础。通过解决MHC-II研究中的数据和方法学挑战，促进了该领域的发展。
...
### [107] [EXAONE Path 2.5: Pathology Foundation Model with Multi-Omics Alignment](https://arxiv.org/abs/2512.14019)
*Juseung Yun,Sunwoo Yu,Sumin Ha,Jonghyun Kim,Janghyeon Lee,Jongseong Jang,Soonyoung Lee*

Main category: cs.LG

TL;DR: EXAONE Path 2.5是一个整合组织学、基因组、表观遗传和转录组多模态数据的病理学基础模型，通过多模态对比学习、空间结构保持和领域专用嵌入，实现更全面的肿瘤生物学表征。
...
Method: 1. 多模态SigLIP损失实现异质模态间的全配对对比学习；2. 片段感知旋转位置编码(F-RoPE)模块，在WSI中保持空间结构和组织片段拓扑；3. 针对WSI和RNA-seq的领域专用内部基础模型，提供生物学基础的嵌入以实现稳健的多模态对齐。

Result: 在Patho-Bench基准测试（涵盖80个任务）和内部真实世界临床数据集上评估，EXAONE Path 2.5表现出高数据和参数效率，在Patho-Bench上达到与最先进基础模型相当的性能，同时在内部临床环境中表现出最高的适应性。

Conclusion: 该研究强调了生物学信息驱动的多模态设计的价值，并展示了整合基因型到表型建模在下一代精准肿瘤学中的潜力。
...
### [108] [FusAD: Time-Frequency Fusion with Adaptive Denoising for General Time Series Analysis](https://arxiv.org/abs/2512.14078)
...
<details>
  <summary>Details</summary>
Motivation: 当前时间序列分析面临三大挑战：1）缺乏高效、多任务兼容且可泛化的统一框架；2）现有方法通常针对单一任务或特定数据类型设计；3）现实数据常受噪声、复杂频率成分和多尺度动态模式影响，难以进行鲁棒特征提取。

Method: FusAD采用自适应时频融合机制，整合傅里叶变换和小波变换来捕获全局-局部和多尺度动态特征；包含自适应去噪机制，自动感知和过滤各类噪声；集成通用信息融合和解码结构，结合掩码预训练，促进多粒度表示的高效学习和迁移。

Result: 在主流时间序列基准测试中，FusAD在分类、预测和异常检测任务上持续优于最先进模型，同时保持高效率和可扩展性。

Conclusion: FusAD通过创新的自适应时频融合和去噪机制，成功构建了一个统一、高效且鲁棒的时间序列分析框架，能够有效处理多任务建模并整合多样化时间序列信息。
...
### [109] [SonicMoE: Accelerating MoE with IO and Tile-aware Optimizations](https://arxiv.org/abs/2512.14080)
*Wentao Guo,Mayank Mishra,Xinle Cheng,Ion Stoica,Tri Dao*

Main category: cs.LG

TL;DR: SonicMoE：一种通过内存高效算法、GPU内核优化和令牌舍入技术，显著减少MoE模型激活内存占用并提高计算吞吐量的方法


<details>
  <summary>Details</summary>
Motivation: 当前细粒度MoE模型面临激活内存占用增加和硬件效率降低的问题，而稀疏MoE模型则因Grouped GEMM内核中的填充导致计算浪费。需要解决这些效率瓶颈以提升MoE模型的实际训练性能。

Method: 1. 提出内存高效算法，最小化MoE前向和反向传播的激活缓存；2. 设计GPU内核，实现内存IO与计算的重叠；3. 提出"令牌舍入"方法，减少Grouped GEMM内核中的填充浪费

Result: SonicMoE在Hopper GPU上相比ScatterMoE的BF16 MoE内核，激活内存减少45%，计算吞吐量提升1.86倍。在64个H100上达到每天2130亿令牌的训练吞吐量，接近ScatterMoE在96个H100上的性能。令牌舍入算法在高度稀疏设置下带来额外1.16倍加速

Conclusion: SonicMoE通过创新的内存优化、计算重叠和令牌舍入技术，有效解决了当前MoE模型训练中的效率瓶颈，显著提升了训练性能，为大规模MoE模型训练提供了实用解决方案
...
### [110] [Cornserve: Efficiently Serving Any-to-Any Multimodal Models](https://arxiv.org/abs/2512.14098)
...
<details>
  <summary>Details</summary>
Motivation: Any-to-Any模型能够接受和生成文本与多模态数据的任意组合，但在模型服务中引入了请求类型、计算路径和计算规模等多重异构性挑战，现有解决方案难以高效处理

Method: 1) 允许开发者描述通用的Any-to-Any模型计算图；2) 规划器根据模型和工作负载特征自动优化部署计划，包括是否及如何将模型分解为更小组件；3) 分布式运行时按照计划执行模型，高效处理异构性

Result: Cornserve能够高效服务多样化的Any-to-Any模型和工作负载，相比现有解决方案，吞吐量提升最高达3.81倍，尾部延迟降低最高达5.79倍

Conclusion: Cornserve系统通过自动化的部署规划和高效的分布式运行时，成功解决了Any-to-Any模型服务中的异构性挑战，为这类新兴多模态模型提供了高效的服务解决方案
...
### [111] [A First-Order Logic-Based Alternative to Reward Models in RLHF](https://arxiv.org/abs/2512.14100)
...
Method: 提出逻辑相似性奖励机制，基于形式逻辑一致性而非启发式奖励估计。引入S-GRPO（GRPO的监督变体），包含额外监督组件，联合优化生成项、KL散度正则化和基于标签的目标

Result: S-GRPO在性能和鲁棒性上持续优于标准监督微调（SFT），并能扩展现有偏好学习框架（如GRPO和DPO），提供更灵活、任务自适应的对齐训练方法

Conclusion: S-GRPO通过逻辑相似性奖励机制和联合优化框架，为LLM对齐提供了更稳定、有效的替代方案，代码已开源
...
### [112] [Random-Bridges as Stochastic Transports for Generative Models](https://arxiv.org/abs/2512.14190)
...
<details>
  <summary>Details</summary>
Motivation: 论文动机是在生成建模领域使用随机桥，这些随机过程可以在适当初始化时作为两个概率分布之间的随机传输，并可根据驱动过程显示马尔可夫或非马尔可夫、连续、不连续或混合模式。

Method: 从一般概率陈述出发，扩展到具体表示，开发学习和模拟算法。基于高斯随机桥构建，展示了如何将随机桥框架转化为可操作的学习和生成方法。

Result: 基于高斯随机桥的实证结果表明，与传统方法相比，能以显著更少的步骤生成高质量样本，同时获得有竞争力的Frechet起始距离分数。

Conclusion: 提出的随机桥框架计算成本低，适合高速生成任务，为生成建模提供了一种高效的新方法。
...
### [113] [Estimating problem difficulty without ground truth using Large Language Model comparisons](https://arxiv.org/abs/2512.14220)
*Marthe Ballon,Andres Algaba,Brecht Verbeken,Vincent Ginis*

Main category: cs.LG

TL;DR: 提出LLM compare方法，通过大语言模型进行成对难度比较，结合Bradley-Terry评分，解决现有难度评估方法无法泛化到分布外问题的局限性。


<details>
  <summary>Details</summary>
Motivation: 现有难度评估方法（如人工校准或基于性能的评分）无法泛化到分布外问题（即当前人类和LLM都无法解决的问题），因为它们不可扩展、耗时且依赖真实标签。需要一种能够评估尚未被解决或超出当前能力范围问题难度的方法。

Method: 提出LLM compare方法：1）使用LLM进行成对难度比较；2）基于比较结果计算Bradley-Terry评分。该方法具有连续性、动态性、模型无关性，且不依赖真实标签信息。

Result: 1）概念框架验证：LLM compare占据所有理想象限；2）与人类标注高度一致：Pearson相关系数r≥0.80（n=1876）；3）对幻觉具有鲁棒性：10%噪声注入下Pearson相关系数退化小于6%。
...
### [114] [Physically consistent model learning for reaction-diffusion systems](https://arxiv.org/abs/2512.14240)
...
<details>
  <summary>Details</summary>
Motivation: 开发能够确保物理一致性和适定性的数据驱动反应-扩散系统模型，使学习模型符合质量守恒和拟正性等基本物理原理，提高模型的可靠性和可解释性。

Method: 基于正则化的结构化模型学习框架，提出系统修改参数化反应项的技术，使反应项固有地满足质量守恒和拟正性，并扩展理论结果到物理一致的反应-扩散系统。

Result: 证明了学习问题的解收敛到极限系统的唯一正则化最小化解，即使强制实施守恒定律和拟正性约束；提供了拟正函数的逼近结果，支持物理一致参数化构建。

Conclusion: 该方法推进了可解释且可靠的数据驱动反应-扩散系统模型的发展，确保模型与基本物理定律保持一致，为物理一致的系统辨识提供了理论保证。
...
### [115] [Beyond MMD: Evaluating Graph Generative Models with Geometric Deep Learning](https://arxiv.org/abs/2512.14241)
...
Method: 提出RGM（Representation-aware Graph-generation Model evaluation）评估方法，使用几何深度学习模型在专门设计的合成和真实图数据集上进行图分类任务，全面评估GRAN和EDGE两种图生成模型。

Result: 研究发现两种模型都能生成具有某些拓扑特性的图，但在保留区分不同图域的结构特征方面存在显著局限性，同时证实了MMD作为评估指标的不足。

Conclusion: 需要开发更有效的图生成模型评估方法，RGM提供了有前景的替代方案，未来研究应关注能够更好保留图结构特征的生成方法。
...
### [116] [FLAME: Flow Enhanced Legendre Memory Models for General Time Series Forecasting](https://arxiv.org/abs/2512.14253)
*Xingjian Wu,Hanyin Cheng,Xiangfei Qiu,Zhengyu Li,Jilin Hu,Chenjuan Guo,Bin Yang*

Main category: cs.LG

TL;DR: FLAME是一个极轻量级的时间序列基础模型家族，支持确定性和概率性预测，通过生成式概率建模确保效率和鲁棒性，在零样本预测任务中表现优异。


<details>
  <summary>Details</summary>
Motivation: 开发一个既轻量又强大的时间序列基础模型，能够同时处理确定性和概率性预测任务，并在零样本场景下保持优异性能。

Method: 使用Legendre Memory增强泛化能力，通过LegT和LegS变体在编码和解码阶段捕捉数据内在归纳偏置；采用归一化流作为预测头，以生成方式建模任意复杂分布。

Result: 在TSFM-Bench和ProbTS等基准测试中，FLAME在确定性和概率性预测任务上均展现出最先进的零样本性能。

Conclusion: FLAME是一个高效、鲁棒的时间序列基础模型，通过创新的Legendre Memory架构和归一化流预测头，在零样本预测任务中实现了优异性能。
...
### [117] [Explainable Preference Learning: a Decision Tree-based Surrogate Model for Preferential Bayesian Optimization](https://arxiv.org/abs/2512.14263)
*Nick Leenders,Thomas Quadt,Boris Cule,Roy Lindelauf,Herman Monsuur,Joost van Oijen,Mark Voskuijl*

Main category: cs.LG

TL;DR: 提出基于决策树的解释性贝叶斯优化替代模型，解决传统高斯过程模型在解释性、分类数据处理和计算复杂度方面的限制
...
Method: 引入基于决策树的解释性替代模型，能够同时处理分类和连续数据，并扩展到大规模数据集

Result: 在八个逐渐尖峰的优化函数上，新模型在尖峰函数上优于基于高斯过程的替代方法，在非尖峰函数上性能略低；在真实Sushi数据集上能够学习个人寿司偏好；初步展示了利用历史偏好数据加速新用户优化过程

Conclusion: 基于决策树的解释性贝叶斯优化模型为解决传统方法的局限性提供了有效方案，在保持性能的同时提高了可解释性和实用性
...
### [118] [Black-Box Auditing of Quantum Model: Lifted Differential Privacy with Quantum Canaries](https://arxiv.org/abs/2512.14388)
*Baobao Song,Shiva Raj Pokhrel,Athanasios V. Vasilakos,Tianqing Zhu,Gang Li*

Main category: cs.LG

TL;DR: 首个基于提升量子差分隐私的黑盒隐私审计框架，用于检测量子机器学习中的隐私泄露
...
Method: 基于提升量子差分隐私的黑盒审计框架，使用量子金丝雀（策略性偏移编码的量子态）检测记忆化并量化训练过程中的隐私泄露

Result: 建立了金丝雀偏移与迹距离界限的数学联系，推导出隐私预算消耗的经验下界，在模拟和物理量子硬件上验证了框架有效性

Conclusion: 该框架填补了量子差分隐私理论保证与实际隐私验证之间的关键空白，为量子机器学习系统提供了强大的隐私验证能力
...
### [119] [GRAFT: Grid-Aware Load Forecasting with Multi-Source Textual Alignment and Fusion](https://arxiv.org/abs/2512.14400)
*Fangzhou Lin,Guoshun He,Zhenyu Guo,Zhe Huang,Jinsong Tao*

Main category: cs.LG

TL;DR: GRAFT模型通过文本引导的跨注意力机制，将多源文本信息与电力负荷数据对齐，实现电网感知预测，在澳大利亚五个州的基准测试中表现优异。
...
Result: 在澳大利亚五个州2019-2021年基准测试中，GRAFT在小时、日和月三个时间尺度上显著优于基线模型，达到或超越最先进水平，在事件驱动场景中表现稳健。

Conclusion: GRAFT通过文本引导的跨注意力机制有效整合多源文本信息，提升了电网负荷预测性能，并提供了可解释的文本-负荷效应分析能力。
...
### [120] [Bridging Artificial Intelligence and Data Assimilation: The Data-driven Ensemble Forecasting System ClimaX-LETKF](https://arxiv.org/abs/2512.14444)
*Akira Takeshima,Kenta Shiraishi,Atsushi Okazaki,Tadashi Tsuyuki,Shunji Kotsuki*

Main category: cs.LG

TL;DR: ClimaX-LETKF是首个纯数据驱动的机器学习集合天气预报系统，能够独立于数值天气预报模型稳定运行多年，通过同化NCEP ADP全球高空和地面天气观测数据，在集合预报中表现出比RTPS方法更好的稳定性和准确性。


<details>
  <summary>Details</summary>
Motivation: 尽管机器学习天气预报取得了显著进展，但在MLWP模型中同化真实观测数据或集合预报的研究仍然有限。需要开发能够独立于数值天气预报模型、稳定运行的纯数据驱动的机器学习集合天气预报系统。

Method: 提出了ClimaX-LETKF系统，这是首个纯数据驱动的机器学习集合天气预报系统。系统采用局部集合变换卡尔曼滤波（LETKF）框架，同化NCEP ADP全球高空和地面天气观测数据。特别比较了两种扰动松弛方法：RTPP（松弛到先验扰动）和RTPS（松弛到先验扩展），其中RTPP将分析扰动替换为分析和背景扰动的加权混合，而RTPS仅重新缩放分析扰动。

Result: 实验表明：1）ClimaX-LETKF系统能够独立于数值天气预报模型稳定运行多年；2）在MLWP模型中，RTPP方法比RTPS方法表现出更好的稳定性和准确性；3）与数值天气预报模型相比，MLWP模型在将大气场恢复到其吸引子方面的能力较弱。

Conclusion: 这项工作为增强机器学习天气预报集合预报系统提供了有价值的见解，代表了向实际应用迈出的重要一步。研究发现MLWP模型与NWP模型在扰动松弛方法上的不同表现特征，为未来MLWP系统优化提供了重要参考。
...
### [121] [Kinetic-Mamba: Mamba-Assisted Predictions of Stiff Chemical Kinetics](https://arxiv.org/abs/2512.14471)
*Additi Pandey,Liang Wei,Hessam Babaee,George Em Karniadakis*

Main category: cs.LG

TL;DR: Kinetic-Mamba：基于Mamba架构的神经算子框架，用于燃烧模拟中的化学动力学建模，通过多种模型变体预测热化学状态变量的时间演化


<details>
  <summary>Details</summary>
Motivation: 精确的化学动力学建模对燃烧模拟至关重要，传统方法计算成本高，需要开发更高效准确的建模框架

Method: 开发了基于Mamba架构的神经算子框架，包含三种模型：独立Mamba模型、约束Mamba模型（强制质量守恒）、基于温度区间的双Mamba模型，以及潜在空间变体

Result: 在Syngas和GRI-Mech 3.0反应机理上验证，框架仅使用初始条件就能高保真预测复杂动力学行为，具有良好的外推能力

Conclusion: Kinetic-Mamba框架成功整合了神经算子的表达能力和Mamba架构的高效时序建模能力，为燃烧模拟提供了准确高效的化学动力学建模解决方案
...
### [122] [Synthetic Electrogram Generation with Variational Autoencoders for ECGI](https://arxiv.org/abs/2512.14537)
...
TL;DR: 使用变分自编码器生成合成心房电图，以解决电生理成像中配对数据稀缺问题，并通过数据增强提升下游任务性能


<details>
  <summary>Details</summary>
Motivation: 心房颤动是最常见的心律失常，需要准确评估心房电活动。非侵入性心电图成像结合深度学习方法从体表电位估计心内电图具有潜力，但进展受到配对体表电位-心内电图数据集有限的阻碍。

Method: 提出两种变分自编码器模型：窦性心律特异性VAE（VAE-S）和在窦性心律与房颤信号上训练的类别条件VAE（VAE-C），用于生成合成多通道心房电图。

Result: VAE-S在形态学、频谱和分布相似性指标上对模拟心电图具有更高保真度，而VAE-C能够实现节律特异性生成但牺牲了窦性重建质量。生成的心电图用于下游非侵入性心电图重建任务的数据增强，适度增强可提高估计性能。

Conclusion: 基于VAE的生成模型有潜力缓解数据稀缺问题并增强基于深度学习的电生理成像流程。
...
### [123] [Counterfactual Explanations for Time Series Should be Human-Centered and Temporally Coherent in Interventions](https://arxiv.org/abs/2512.14559)
*Emmanuel C. Chukwu,Rianne M. Schouten,Monique Tabak,Mykola Pechenizkiy*

Main category: cs.LG

TL;DR: 该论文批评现有时间序列反事实解释方法在临床推荐场景中的不足，主张转向更具临床可行性和时间一致性的干预方案。


<details>
  <summary>Details</summary>
Motivation: 当前时间序列分类的反事实解释方法主要基于静态数据假设，仅关注最小化输入扰动来改变模型预测，但在临床推荐场景中，干预措施需要随时间展开，必须具有因果合理性和时间连贯性。
...
Conclusion: 需要超越仅改变预测而不考虑可行性或可操作性的方法和评估框架，强调需要在实际场景中可行的、以目的为导向的可操作干预措施。
...
### [124] [Residual GRU+MHSA: A Lightweight Hybrid Recurrent Attention Model for Cardiovascular Disease Detection](https://arxiv.org/abs/2512.14563)
*Tejaswani Dash,Gautam Datla,Anudeep Vurity,Tazeem Ahmad,Mohd Adnan,Saima Rafi,Saisha Patro,Saina Patro*

Main category: cs.LG

TL;DR: 提出Residual GRU with Multi-Head Self-Attention模型，在UCI心脏病数据集上表现优于传统和现代基线方法，为临床风险预测提供轻量高效的解决方案。


<details>
  <summary>Details</summary>
Motivation: 心血管疾病是全球主要死因，需要可靠高效的预测工具支持早期干预。传统方法依赖手工特征和临床经验，机器学习方法虽提高可重复性但难以在噪声和异构临床数据上泛化。

Method: 提出紧凑的深度学习架构，整合残差双向门控循环单元用于特征列的序列建模、通道重加权模块、以及带可学习分类标记的多头自注意力池化来捕获全局上下文。

Result: 在UCI心脏病数据集上通过5折分层交叉验证，模型达到准确率0.861、宏F1 0.860、ROC-AUC 0.908、PR-AUC 0.904，优于所有基线方法。消融研究证实了残差循环、通道门控和注意力池化的各自贡献。
...
### [125] [Model-Based Reinforcement Learning in Discrete-Action Non-Markovian Reward Decision Processes](https://arxiv.org/abs/2512.14617)
*Alessandro Trapasso,Luca Iocchi,Fabio Patrizi*

Main category: cs.LG

TL;DR: QR-MAX是首个针对离散动作非马尔可夫奖励决策过程(NMRDPs)的基于模型强化学习算法，通过奖励机实现马尔可夫转移学习与非马尔可夫奖励处理的分解，获得PAC收敛保证。Bucket-QR-MAX将其扩展到连续状态空间，在样本效率和策略最优性方面显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 许多实际决策问题涉及依赖于整个系统历史而非特定状态属性的任务。马尔可夫强化学习方法不适用于此类任务，而非马尔可夫奖励决策过程(NMRDPs)虽能处理时间依赖性任务，但长期以来缺乏形式化保证（近最优性和样本效率）。

Method: 提出QR-MAX算法：针对离散动作NMRDPs的基于模型强化学习方法，通过奖励机将马尔可夫转移学习与非马尔可夫奖励处理分解。然后扩展为Bucket-QR-MAX，使用SimHash-based离散化器处理连续状态空间，保持相同的分解结构，无需手动网格化或函数逼近。

Result: QR-MAX是首个利用这种分解实现多项式样本复杂度下PAC收敛到ε-最优策略的基于模型RL算法。实验表明，在复杂度递增的环境中，该方法在样本效率和寻找最优策略的鲁棒性方面显著优于现代最先进的基于模型RL方法。

Conclusion: 该研究解决了NMRDPs中长期缺乏形式化保证的问题，通过分解方法实现了高效学习，为处理时间依赖性任务提供了理论保证和实践有效的解决方案，特别是在连续状态空间中表现出快速稳定的学习能力。
...
### [126] [ParaFormer: A Generalized PageRank Graph Transformer for Graph Representation Learning](https://arxiv.org/abs/2512.14619)
...
Method: 提出PageRank Transformer (ParaFormer)，采用PageRank增强的注意力模块，模拟深度Transformer的行为，通过理论分析和实证验证，ParaFormer作为自适应滤波器缓解过度平滑问题。

Result: 在11个数据集（从数千到数百万节点）的节点分类和图分类任务上，ParaFormer均取得了一致的性能提升，验证了其有效性。

Conclusion: ParaFormer通过PageRank增强的注意力机制有效缓解了图Transformer中的过度平滑问题，在多种图学习任务上表现出优越性能，为图Transformer设计提供了新思路。
...
### [127] [gridfm-datakit-v1: A Python Library for Scalable and Realistic Power Flow and Optimal Power Flow Data Generation](https://arxiv.org/abs/2512.14658)
...
TL;DR: gridfm-datakit-v1是一个Python库，用于生成真实多样的电力潮流和最优潮流数据集，解决了现有工具在场景多样性、操作限制外样本和成本函数变化方面的不足。


<details>
  <summary>Details</summary>
Motivation: 现有电力系统数据集面临三个主要挑战：1) 缺乏真实的随机负荷和拓扑扰动，限制了场景多样性；2) PF数据集仅限于OPF可行点，阻碍了ML求解器对违反操作限制情况的泛化；3) OPF数据集使用固定发电机成本函数，限制了在不同成本下的泛化能力。

Method: 通过结合真实世界负荷曲线的全局负荷缩放与局部噪声，支持任意N-k拓扑扰动来创建多样且真实的数据集；生成超出操作限制的PF样本；产生具有变化发电机成本的OPF数据；并高效扩展到大型电网（最多10,000个节点）。

Result: 开发了gridfm-datakit-v1库，与OPFData、OPF-Learn、PGLearn和PFΔ等现有工具进行了比较，支持大规模电网，并在GitHub上开源（Apache 2.0许可），可通过pip安装。

Conclusion: gridfm-datakit-v1解决了现有电力系统数据生成工具的局限性，为训练机器学习求解器提供了更真实、多样和可扩展的数据集生成能力。
...
### [128] [Early Warning Index for Patient Deteriorations in Hospitals](https://arxiv.org/abs/2512.14683)
...
TL;DR: 开发了一个名为早期预警指数（EWI）的多模态机器学习框架，用于预测ICU入院、急救团队派遣和死亡率的综合风险，通过人机协作流程和可解释性输出帮助医院主动管理高风险患者。
...
Method: 开发了多模态机器学习框架EWI，采用人机协作流程：临床医生帮助确定警报阈值并解释模型输出。使用SHAP可解释性方法突出显示驱动每个患者风险的临床和运营因素（如计划手术、病房人数）。从结构化和非结构化电子健康记录数据中自动提取特征。

Result: 在美国一家大型医院的18,633名独特患者数据集上，EWI实现了C统计量0.796。目前作为分诊工具用于主动管理高风险患者，将患者分为三个风险等级。该方法为医生节省了宝贵时间，使他们能够专注于患者护理而非筛选复杂的EHR数据。

Conclusion: EWI框架通过提供数据驱动的风险预测和可解释性洞察，帮助临床医生和管理人员避免下游并发症（包括昂贵程序或高再入院率），改善患者流动，优化护理人员排班和关键资源分配。
...
### [129] [Generative AI for Video Translation: A Scalable Architecture for Multilingual Video Conferencing](https://arxiv.org/abs/2512.13904)
*Amirkia Rafiei Oskooei,Eren Caglar,Ibrahim Sahin,Ayse Kayabay,Mehmet S. Aktas*

Main category: cs.MM

TL;DR: 本文提出了一种系统级框架来解决级联生成式AI管道在视频翻译等应用中的实时部署挑战，通过轮转机制将计算复杂度从二次降为线性，并通过分段处理协议管理推理延迟，实现了实时吞吐量。


<details>
  <summary>Details</summary>
Motivation: 级联生成式AI管道在视频翻译等应用中的实时部署面临系统级挑战，包括顺序模型推理的累积延迟和二次计算复杂度，这使得多用户视频会议应用难以扩展。

Method: 提出了一种实用的系统级框架，包含轮转机制（将多用户场景的计算复杂度从二次降为线性）和分段处理协议（管理推理延迟以实现感知实时体验）。在包括消费级、云级和企业级GPU的多层硬件设置上实现了概念验证管道并进行性能分析。

Result: 客观评估显示系统在现代硬件上实现了实时吞吐量（τ<1.0）。主观用户研究进一步验证了该方法，表明可预测的初始处理延迟对用户来说是可接受的，以换取流畅、不间断的播放体验。

Conclusion: 该工作提出了一个经过验证的端到端系统设计，为在多语言通信平台中部署可扩展的实时生成式AI应用提供了实用的路线图。
...
### [130] [End-to-End Learning-based Video Streaming Enhancement Pipeline: A Generative AI Approach](https://arxiv.org/abs/2512.14185)
*Emanuele Artioli,Farzad Tashtarian,Christian Timmerer*

Main category: cs.MM

TL;DR: ELVIS是一个端到端学习型视频流增强框架，通过服务器端编码优化和客户端生成式修复技术，在保持带宽不变的情况下提升视频质量。


<details>
  <summary>Details</summary>
Motivation: 传统编解码器无法利用上下文信息，必须编码和传输整个视频数据，难以平衡高质量和平滑播放之间的矛盾。

Method: 采用端到端架构，结合服务器端编码优化和客户端生成式修复技术，移除并重建冗余视频数据。模块化设计支持集成不同编解码器、修复模型和质量指标。

Result: 相比基准测试，当前技术可提升高达11个VMAF点，但由于计算需求，实时应用仍面临挑战。

Conclusion: ELVIS是将生成式AI融入视频流管道的奠基性工作，可在不增加带宽需求的情况下实现更高质量的视频体验。
...
### [131] [Leveraging LLMs for Structured Data Extraction from Unstructured Patient Records](https://arxiv.org/abs/2512.13700)
...
<details>
  <summary>Details</summary>
Motivation: 人工病历审查耗时耗力，需要专家从非结构化的电子健康记录中提取复杂信息，阻碍临床研究效率

Method: 开发安全模块化框架，在符合HIPAA标准的本地计算基础设施上部署大语言模型，集成检索增强生成和结构化响应方法，构建可广泛部署的容器化系统

Result: 框架在大量患者病历中提取多种医学特征时达到高准确率，与专家标注数据集相比表现优异，甚至发现了人工审查中遗漏的标注错误
...
### [132] [Blind Radio Mapping via Spatially Regularized Bayesian Trajectory Inference](https://arxiv.org/abs/2512.13701)
*Zheng Xing,Junting Chen*

Main category: cs.AI

TL;DR: 提出了一种无需位置标签的盲无线电地图构建框架，利用MIMO-OFDM信道测量推断用户轨迹，通过理论证明和空间正则化贝叶斯推理实现高精度定位和波束图重建。


<details>
  <summary>Details</summary>
Motivation: 传统无线电地图构建方法需要大量带位置标签的数据，成本高昂且在实际场景中不实用。本文旨在开发一种无需位置标签的盲构建方法，利用信道测量推断用户轨迹，降低数据收集成本。

Method: 首先理论证明NLOS信道状态信息在准镜面环境模型下具有空间连续性，推导出与物理距离成正比的CSI距离度量。针对泊松分布的AP部署和直线轨迹，证明定位误差的CRLB渐近消失。基于此开发了空间正则化贝叶斯推理框架，联合估计信道特征、区分LOS/NLOS条件并恢复用户轨迹。

Result: 在射线追踪数据集上的实验表明，平均定位误差为0.68米，波束图重建误差为3.3%，验证了所提盲映射方法的有效性。

Conclusion: 该研究提出了一种无需位置标签的盲无线电地图构建框架，通过理论分析和实验验证证明了其可行性，为智能无线应用提供了更实用的解决方案。
...
### [133] [Adjudicator: Correcting Noisy Labels with a KG-Informed Council of LLM Agents](https://arxiv.org/abs/2512.13704)
...
Method: 采用神经符号方法：首先构建动态知识图谱统一项目上下文，然后使用"委员会智能体"架构——专门化的多智能体LLM进行辩论和投票，通过知识图谱指导的覆盖逻辑处理复杂结构错误。

Result: 在AlleNoise基准的1000项平衡子集上，知识图谱指导的模型达到0.99 F1分数，显著优于单LLM基线(0.48 F1)和非知识图谱委员会(0.59 F1)，通过覆盖逻辑实现了复杂结构错误的完全召回。
...
### [134] [AI-Powered Annotation Pipelines for Stabilizing Large Language Models: A Human-AI Synergy Approach](https://arxiv.org/abs/2512.13714)
*Gangesh Pathak,Prasanna Kumar*

Main category: cs.AI

TL;DR: 该论文提出了一种基于AI的标注流程，用于系统识别、标记和修复LLM输出的不稳定性模式，通过人机协同方法结合自动弱监督和基于置信度的标注，旨在解决LLM在高度监管行业中因不稳定性、幻觉和性能波动导致的可靠性问题。


<details>
  <summary>Details</summary>
Motivation: LLM在高度监管行业中的应用受到限制，主要原因是存在不稳定性问题、不一致推理、幻觉和性能波动，特别是在工作流程中。这些可靠性问题限制了LLM在需要事实精确性和一致行为的领域的安全使用。现有的稳定化方法如RLHF和监督微调虽然能带来可量化的改进，但成本高昂且依赖大量人工标注，难以可持续扩展。

Method: 提出基于AI的标注流程，系统识别、标记和修复LLM输出的不稳定性模式。采用人机协同方法，结合自动弱监督模型和基于置信度的标注，辅以目标人工验证，确保反馈信息的可靠性和道德完整性。框架引入了语义一致性、事实正确性和逻辑连贯性等稳定性特定标注类别，通过反馈循环实现模型的持续校准和鲁棒性增强。

Result: 论文提出的方法能够系统性地处理LLM输出的不稳定性问题，通过人机协同标注流程提高反馈信息的质量和可靠性。该方法相比传统RLHF和监督微调方法更具可扩展性和可持续性，能够持续校准模型并增强其鲁棒性。

Conclusion: 该研究为解决LLM在高度监管行业中的可靠性问题提供了一种创新的人机协同标注方法，通过系统化的稳定性特定标注和反馈循环机制，能够更有效地提升LLM的稳定性和可靠性，同时降低对大量人工标注的依赖，具有更好的可扩展性。
...
### [135] [Compressed Causal Reasoning: Quantization and GraphRAG Effects on Interventional and Counterfactual Accuracy](https://arxiv.org/abs/2512.13725)
*Steve Nwaiwu,Nipat Jongsawat,Anucha Tungkasthan*

Main category: cs.AI

TL;DR: 量化对LLM因果推理影响的研究：Llama 3 8B在INT8和NF4量化下，因果推理准确性保持稳定，NF4仅下降不到1%，干预查询最敏感，反事实推理相对稳定但存在异质性弱点。


<details>
  <summary>Details</summary>
Motivation: 随着大语言模型部署向边缘和资源受限环境转移，量化模型（如INT8和NF4）成为标准。然而，精度降低对形式化因果推理的影响尚不清楚，这是首个系统评估量化在Pearl因果阶梯所有三个层级上影响的研究。

Method: 使用3000个样本的分层CLadder基准测试，评估Llama 3 8B在量化下的因果推理能力。在Pearl因果阶梯的三个层级（关联、干预、反事实）上测试，并在CRASS基准上进行实验。进一步评估使用真实因果图的图检索增强生成方法。

Result: 量化下因果推理准确性保持稳定，NF4量化总体下降不到1%。干预查询对精度损失最敏感，反事实推理相对稳定但在共偏倚和后门调整等查询类型上存在异质性弱点。CRASS基准上各精度表现几乎相同。图检索增强生成使NF4干预准确性提高1.7%，部分抵消了压缩相关的性能下降。

Conclusion: 因果推理对4位量化表现出意外的鲁棒性，图结构增强可以有针对性地强化干预推理，当前反事实基准未能捕捉更深层次的因果脆弱性。该研究为部署高效且结构支持的因果AI系统提供了初步经验指导和实用建议。
...
### [136] [State-Dependent Refusal and Learned Incapacity in RLHF-Aligned Language Models](https://arxiv.org/abs/2512.13762)
*TK Lee*

Main category: cs.AI

TL;DR: 该研究提出了一种定性案例研究方法，用于审计大语言模型在长时程交互中的策略相关行为选择性。研究发现同一模型在非敏感领域表现正常，但在敏感领域反复出现功能性拒绝，这种选择性抑制行为被称为"习得性无能"。


<details>
  <summary>Details</summary>
Motivation: 大语言模型作为通用工具广泛部署，但扩展交互可能揭示标准定量基准无法捕捉的行为模式。研究旨在开发一种交互层面的审计框架，用于检测模型在策略敏感领域的行为选择性。

Method: 采用定性案例研究方法，通过86轮对话会话进行长时程交互审计。定义了三种响应机制：正常表现(NP)、功能性拒绝(FR)和元叙事(MN)。研究观察模型在不同领域的行为不对称性，并引入"习得性无能"作为行为描述符。

Result: 研究发现同一模型在非敏感领域表现正常，但在提供商或策略敏感领域反复产生功能性拒绝，形成了跨领域的一致不对称性。元叙事角色框架叙述倾向于与敏感情境中的拒绝同时出现。

Conclusion: 研究提出了基于可观察行为的交互层面审计框架，并将习得性无能作为检查潜在对齐副作用的一个视角。这需要跨用户和模型的进一步调查，以理解模型在敏感领域的选择性抑制行为。
...
### [137] [Mathematics and Coding are Universal AI Benchmarks](https://arxiv.org/abs/2512.13764)
*Przemyslaw Chojecki*

Main category: cs.AI

TL;DR: 论文研究数学和编程在AI智能体心理测量电池模空间中的特殊作用，证明数学定理证明和编程任务生成的电池子空间在评估度量下是稠密的，编程具有普遍性，而纯数学具有谱稳定性优势。


<details>
  <summary>Details</summary>
Motivation: 研究数学和编程在AI智能体心理测量电池模空间中的特殊地位，探索它们作为评估"通用坐标"的潜力，以及形式数学作为高级AI智能体递归自我改进的自然点火域。

Method: 基于AAI框架和GVU动力学，定义数学纤维，结合形式证明核（如Lean、Coq），分析GVU流在数学纤维上的谱稳定自我改进机制，证明在智能体输出均匀紧性和Lipschitz AAI泛函条件下的密度定理。

Result: 主要技术结果是密度定理：在评估度量下，由数学定理证明和编程任务生成的电池子空间在电池模空间中稠密。编程单独具有普遍性，而纯数学不具有表达普遍性但具有谱优势。
...
### [138] [Semantic Grounding Index: Geometric Bounds on Context Engagement in RAG Systems](https://arxiv.org/abs/2512.13771)
*Javier Marín*

Main category: cs.AI

TL;DR: 该论文提出了语义接地指数（SGI），通过计算响应与问题、响应与上下文在嵌入空间中的角度距离比来检测RAG系统的幻觉。研究发现幻觉响应倾向于保持与问题接近而非靠近上下文，这种"语义懒惰"现象在不同嵌入模型中具有强相关性。


<details>
  <summary>Details</summary>
Motivation: 当检索增强生成（RAG）系统产生幻觉时，这些幻觉在嵌入空间中会留下什么几何痕迹？研究者希望找到一种计算高效、理论可靠的方法来识别需要验证的响应，以提高生产部署中RAG系统的可靠性。

Method: 引入语义接地指数（SGI），定义为在单位超球面𝕊^{d-1}上，响应与问题的角度距离与响应与上下文的角度距离之比。利用球形三角不等式推导理论预测，并在HaluEval数据集（n=5,000）上使用五种嵌入模型进行实证验证。

Result: 发现"语义懒惰"现象：幻觉响应在角度上更接近问题而非上下文。效应大小Cohen's d在0.92-1.28之间，跨模型相关性r=0.85。理论预测得到证实：问题-上下文角度分离越大，SGI的判别能力越强（效应大小从d=0.61升至d=1.27，AUC从0.72升至0.83）。SGI对长响应（d=2.05）和短问题（d=1.22）表现优异，校准分析显示ECE=0.10。

Conclusion: SGI提供了一种计算高效、理论可靠的基础设施，可用于识别生产RAG部署中需要验证的响应。然而，在TruthfulQA上的负面结果（AUC=0.478）表明角度几何测量的是主题参与度而非事实准确性。SGI适用于检测响应是否与检索内容相关，而非事实正确性。
...
### [139] [MURIM: Multidimensional Reputation-based Incentive Mechanism for Federated Learning](https://arxiv.org/abs/2512.13955)
*Sindhuja Madabushi,Dawood Wasif,Jin-Hee Cho*

Main category: cs.AI

TL;DR: MURIM是一个基于多维信誉的联邦学习激励机制，通过综合考虑客户端可靠性、隐私保护、资源容量和公平性，有效分配激励并防止恶意客户端获取不当奖励。
...
Method: 提出MURIM机制，基于客户端贡献、延迟和信誉分配激励，包含可靠性验证模块，综合考虑可靠性、隐私、资源容量和公平性，防止恶意或不可靠客户端获得不当奖励。

Result: 在MNIST、FMNIST和ADULT Income数据集上的实验表明，MURIM在公平性指标上提升达18%，隐私攻击成功率降低5-9%，对投毒和噪声梯度攻击的鲁棒性提升达85%。

Conclusion: MURIM能有效缓解对抗威胁，促进公平真实参与，在异构动态联邦学习环境中保持稳定的模型收敛。
...
### [140] [Evaluating Frontier LLMs on PhD-Level Mathematical Reasoning: A Benchmark on a Textbook in Theoretical Computer Science about Randomized Algorithms](https://arxiv.org/abs/2512.13978)
*Yang Cao,Yubin Chen,Xuyang Guo,Zhao Song,Song Yue,Jiahao Zhang,Jiale Zhao*

Main category: cs.AI

TL;DR: 该论文对GPT-5-Thinking、Gemini-3-Pro、Claude-Sonnet-4.5-Thinking和Grok-4四个前沿大语言模型在《随机算法》研究生教材中的证明生成能力进行了全面基准测试，发现顶级模型准确率约66%，但模型间存在显著差异。


<details>
  <summary>Details</summary>
Motivation: 尽管大语言模型在数学推理和科学发现方面取得显著进展，但缺乏对这些模型在经典研究生数学理论上的严格评估。需要了解它们在基础推理能力方面的表现，以评估其数学发现潜力。

Method: 使用Motwani和Raghavan的《随机算法》教材作为测试基准，要求每个模型为教材中的一系列引理和练习生成正式的LaTeX证明。对四个前沿模型（GPT-5-Thinking、Gemini-3-Pro、Claude-Sonnet-4.5-Thinking和Grok-4）进行系统评估。

Result: 顶级模型（Gemini和Claude）达到约66%的准确率，表现出对概率方法和形式逻辑的扎实掌握；其他模型的一致性显著较低（约40%）。定性分析显示在简洁性、幻觉率和逻辑结构方面存在差异。

Conclusion: 前沿模型已达到适合研究生教学辅助和形式化的熟练度门槛，但在严格数学推导的可靠性方面存在显著差异。代码和所有LLM生成的响应已开源。
...
### [141] [ReflCtrl: Controlling LLM Reflection via Representation Engineering](https://arxiv.org/abs/2512.13979)
*Ge Yan,Chung-En Sun,Tsui-Wei,Weng*

Main category: cs.AI

TL;DR: ReflCtrl框架通过表征工程控制LLM的自反思行为，减少推理成本，同时保持性能。研究发现反思常冗余且与模型内部不确定性相关。


<details>
  <summary>Details</summary>
Motivation: 虽然自反思能提升大语言模型的推理性能，但会增加推理成本。本研究旨在通过表征工程方法控制反思行为，在保持性能的同时减少计算开销。

Method: 提出ReflCtrl框架：将模型推理过程分段，识别反思步骤，在潜在空间中提取控制反思行为的"反思方向"，并设计逐步引导方法来控制反思频率。

Result: 实验显示：(1) 在许多情况下反思是冗余的，尤其在更强模型中，最多可节省33.6%的推理token同时保持性能；(2) 模型的反思行为与内部不确定性信号高度相关。

Conclusion: 自反思行为可通过表征工程有效控制，反思常冗余且与模型不确定性相关，这为优化推理效率提供了新途径。
...
### [142] [Sparsity-Controllable Dynamic Top-p MoE for Large Foundation Model Pre-training](https://arxiv.org/abs/2512.13996)
...
TL;DR: DTop-p MoE：一种可控制稀疏度的动态Top-p路由机制，通过PI控制器动态调整概率阈值，实现激活专家数量的精确控制，同时自适应分配计算资源。
...
### [143] [MobileWorldBench: Towards Semantic World Modeling For Mobile Agents](https://arxiv.org/abs/2512.14014)
*Shufan Li,Konstantinos Kallidromitis,Akash Gokul,Yusuke Kato,Kazuki Kozuka,Aditya Grover*

Main category: cs.AI

TL;DR: 该研究提出了一种基于自然语言而非像素预测的GUI智能体世界模型新范式，通过MobileWorldBench基准测试和MobileWorld数据集提升视觉语言模型的世界建模能力，并将其集成到移动智能体规划框架中提高任务成功率。


<details>
  <summary>Details</summary>
Motivation: 传统基于像素空间的世界模型在GUI环境中面临实际限制，难以预测未来状态的复杂视觉元素。本研究探索为GUI智能体提供基于自然语言描述状态转换的替代世界模型方案。

Method: 1. 引入MobileWorldBench基准测试评估视觉语言模型作为移动GUI智能体世界模型的能力；2. 发布包含140万样本的大规模MobileWorld数据集；3. 提出将VLM世界模型集成到移动智能体规划框架中的新框架。

Result: 语义世界模型能够直接提升移动智能体的任务成功率，通过自然语言描述状态转换的方式克服了像素空间世界模型在GUI环境中的局限性。

Conclusion: 基于自然语言的语义世界模型为GUI智能体提供了有效的替代方案，通过大规模数据集和基准测试的建立，显著提升了视觉语言模型的世界建模能力，并能实际改善移动智能体的任务性能。
...
### [144] [Intention Chain-of-Thought Prompting with Dynamic Routing for Code Generation](https://arxiv.org/abs/2512.14048)
*Shen Li,Li Huang,Shaoxiong Zhan,Weifeng Sun,Tao Yin,Zhongxin Liu,Meng Yan*

Main category: cs.AI

TL;DR: RoutingGen是一个难度感知的路由框架，针对代码生成任务动态调整提示策略：简单任务使用少样本提示，复杂任务使用意图思维链（ICoT）进行结构化推理，在保持高性能的同时显著减少token使用量。


<details>
  <summary>Details</summary>
Motivation: 现有思维链提示方法存在两个主要局限：1）统一应用导致简单任务上的过度思考；2）缺乏代码生成中的意图抽象（如核心算法设计和效率建模），使模型关注表层结构而忽视全局问题目标。受认知经济原则启发，只在必要时进行结构化推理以节省认知资源。

Method: 提出RoutingGen框架，根据任务难度动态选择提示策略：简单任务使用少样本提示；复杂任务使用新提出的意图思维链（ICoT），引导模型捕获任务意图（如核心算法逻辑和时间复杂度）。

Result: 在三个模型和六个标准代码生成基准测试中，RoutingGen在大多数设置下达到最先进性能，同时平均减少46.37%的总token使用量。ICoT在挑战性基准测试中优于六个现有提示基线。

Conclusion: RoutingGen通过难度感知的路由机制和意图思维链，有效解决了现有思维链方法在代码生成中的局限性，实现了性能与效率的平衡，为LLM在代码生成领域的应用提供了新思路。
...
### [145] [OpenDataArena: A Fair and Open Arena for Benchmarking Post-Training Dataset Value](https://arxiv.org/abs/2512.14051)
...
TL;DR: OpenDataArena (ODA) 是一个开源平台，用于系统评估和基准测试大语言模型的后训练数据质量，通过统一训练-评估流程、多维度评分框架、数据谱系探索和开源工具包，揭示数据特性与模型性能之间的因果关系。
...
Method: ODA平台包含四个核心组件：1) 统一的训练-评估流程，确保不同模型和领域的公平比较；2) 多维度评分框架，从数十个不同维度评估数据质量；3) 交互式数据谱系探索器，可视化数据集谱系并分析组成来源；4) 完全开源的工具包，支持训练、评估和评分。

Result: 在ODA上进行了大规模实验：覆盖120多个训练数据集、22个基准测试、600多次训练运行、处理4000万个数据点。分析揭示了数据复杂度与任务性能之间的权衡关系，通过谱系追踪发现了流行基准中的冗余，并绘制了数据集间的谱系关系图。

Conclusion: ODA不仅扩展了排行榜，更旨在推动从试错式数据策展向数据为中心AI的原则性科学转变，为研究数据混合规律和基础模型战略组成铺平道路。所有结果、工具和配置均已开源，以促进高质量数据评估的民主化访问。
...
### [146] [RADAR: Accelerating Large Language Model Inference With RL-Based Dynamic Draft Trees](https://arxiv.org/abs/2512.14069)
...
### [147] [Grammar Search for Multi-Agent Systems](https://arxiv.org/abs/2512.14079)
*Mayank Singh,Vikas Yadav,Shiva Krishna Reddy Malay,Shravan Nayak,Sai Rajeswar,Sathwik Tejaswi Madhusudhan,Eduardo Blanco*

Main category: cs.AI

TL;DR: 本文提出了一种基于固定可组合组件的结构化框架，用于自动搜索多智能体系统，相比基于LLM的自由形式搜索方法，在数学和问答领域的五个基准测试中有四个表现更优，且具有成本效益高、模块化、可解释性强的优势。


<details>
  <summary>Details</summary>
Motivation: 自动搜索多智能体系统已成为智能体AI研究的关键焦点，现有方法主要依赖LLM在代码空间中进行自由形式搜索，但这种方法缺乏结构性和效率。

Method: 提出了一种结构化框架，通过一组固定的简单可组合组件来探索多智能体系统的设计空间，而不是使用LLM进行自由形式的候选生成。

Result: 在数学和问答两个领域的五个基准测试中，该方法在四个基准上超越了先前基于LLM的方法，同时搜索过程更经济高效，生成的系统具有模块化、可解释性强和逻辑简单的特点。

Conclusion: 尽管在候选生成阶段缺乏LLM的生成灵活性，但基于固定可组合组件的结构化方法在多智能体系统自动搜索中表现更优，提供了成本效益、模块化和可解释性等多重优势。
...
### [148] [Georeferencing complex relative locality descriptions with large language models](https://arxiv.org/abs/2512.14228)
*Aneesha Fernando,Surangika Ranathunga,Kristin Stock,Raj Prasanna,Christopher B. Jones*

Main category: cs.AI

TL;DR: 该研究探索使用大语言模型自动地理编码复杂的生物多样性标本位置描述，通过QLoRA微调在多个区域和语言数据集上取得优于基线方法的结果。


<details>
  <summary>Details</summary>
Motivation: 生物标本记录中的位置描述通常采用叙述性文本而非坐标，传统基于地名或地理指示词的方法难以处理相对位置关系，需要人工处理且效率低下，因此需要自动化地理编码解决方案。

Method: 首先识别有效的提示模式，然后使用量化低秩适应（QLoRA）在多个区域和语言的多源生物多样性数据集上微调大语言模型。

Result: 该方法在固定训练数据量下，平均65%的记录在10公里半径内，最佳结果（纽约州）达到85%在10公里内和67%在1公里内，优于现有基线方法。

Conclusion: 大语言模型能够有效处理复杂冗长的位置描述，在生物多样性领域的地理编码任务中展现出巨大潜力，特别适合处理包含相对空间关系的叙述性位置描述。
...
### [149] [Gödel's Poetry](https://arxiv.org/abs/2512.14252)
*Kelly J. Davis*

Main category: cs.AI

TL;DR: 论文提出了一种结合专用语言模型和递归分解定理的新方法，通过多智能体架构实现Lean4自动定理证明，显著提升了miniF2F测试集的通过率。


<details>
  <summary>Details</summary>
Motivation: 自动定理证明长期以来被视为人工智能的挑战，现有方法在处理复杂定理时存在局限性，需要更有效的自动化证明方法。

Method: 采用专用语言模型生成Lean4证明，结合递归分解将困难定理分解为更简单的蕴含命题，通过多智能体架构协调自动形式化、证明生成、分解和递归证明等过程。

Result: 在没有分解的情况下，在miniF2F测试集上达到90.4%的通过率；使用分解方法后，性能得到显著提升。扩展了Kimina Lean Server的AST解析能力以支持自动递归证明分解。

Conclusion: 该方法为自动定理证明提供了有效的新途径，通过结合语言模型和递归分解策略显著提升了证明能力，系统已开源并可在PyPI上获取。
...
### [150] [Leveraging LLMs for Collaborative Ontology Engineering in Parkinson Disease Monitoring and Alerting](https://arxiv.org/abs/2512.14288)
*Georgios Bouchouras,Dimitrios Doumanas,Andreas Soularidis,Konstantinos Kotis,George A. Vouros*

Main category: cs.AI

TL;DR: 该研究探索了LLM在帕金森病监测与预警本体工程中的应用，比较了纯LLM方法与人类-LLM协作方法的效果，发现人类监督的混合方法能产生更全面准确的本体。


<details>
  <summary>Details</summary>
Motivation: 研究旨在探索LLM是否能够独立创建全面的本体，以及人类-LLM协作是否能提升本体工程的质量，特别是在帕金森病监测与预警这一复杂领域。

Method: 采用了四种方法：1) One Shot提示技术；2) Chain of Thought提示；3) X-HCOME（混合本体工程方法）；4) SimX-HCOME+（强调持续人类监督和迭代优化的混合方法）。

Result: 纯LLM方法（OS和CoT）能够自主构建本体但不全面，需要大量人工优化；X-HCOME方法显著提升了本体全面性，接近专家构建水平；SimX-HCOME+方法通过持续人类监督产生了更全面准确的本体。

Conclusion: 人类-LLM协作在复杂领域本体工程中具有重要潜力，未来可开发专门用于本体构建的GPT模型，持续人类监督对提升本体质量至关重要。
...
### [151] [Massive Editing for Large Language Models Based on Dynamic Weight Generation](https://arxiv.org/abs/2512.14395)
...
<details>
  <summary>Details</summary>
Motivation: 当前在大规模编辑LLMs时，确保编辑的可靠性、泛化性和局部性仍然是一个挑战，需要低成本高效的编辑方法

Method: MeG方法在LLMs特定层附加动态权重神经元，使用扩散模型根据输入查询条件生成神经元权重，通过单个动态权重神经元实现大规模知识编辑

Result: MeG显著提升了大规模知识编辑在可靠性、泛化性和局部性指标上的性能，特别是在局部性指标上获得了高百分点的绝对提升

Conclusion: 提出的MeG方法通过动态权重生成机制，有效解决了大规模知识编辑的挑战，相比现有方法具有明显优势
...
### [152] [Seismology modeling agent: A smart assistant for geophysical researchers](https://arxiv.org/abs/2512.14429)
*Yukun Ren,Siwei Yu,Kai Chen,Jianwei Ma*

Main category: cs.AI

TL;DR: 本文提出基于大语言模型的智能交互工作流，通过首个SPECFEM的MCP服务器套件，将传统地震波模拟的复杂文件编辑和命令行操作转变为意图驱动的对话式交互，显著降低学习门槛。


<details>
  <summary>Details</summary>
Motivation: 传统主流开源地震波模拟软件SPECFEM存在陡峭的学习曲线，依赖复杂的手动文件编辑和命令行操作，阻碍了研究人员的使用效率和科学探索。

Method: 开发首个SPECFEM的模型上下文协议(MCP)服务器套件，将整个模拟过程分解为离散的、可由智能体执行的工具，涵盖参数生成、网格划分、求解器执行和可视化等环节，支持全自动执行和人机协同两种模式。

Result: 通过多个案例验证，该工作流在自主和交互模式下均能无缝运行，产生与标准基准一致的高保真结果，显著降低了使用门槛并增强了可复现性。
...
### [153] [Context-Picker: Dynamic context selection using multi-stage reinforcement learning](https://arxiv.org/abs/2512.14465)
...
<details>
  <summary>Details</summary>
Motivation: 在长上下文问答中，确定查询所需的最佳上下文量是一个重要挑战。包含太少段落可能遗漏关键信息，包含太多则引入噪声降低答案质量。传统方法（如固定Top-K检索和单阶段重排序）面临选择适当段落数量的困境，这在事实性问题中尤为突出，因为这类问题通常只需要少量特定证据。
...
TL;DR: GreedyLR是一种新颖的自适应学习率调度器，根据当前损失动态调整学习率，在多个NLP、CV和LLM任务上表现优于现有调度器，且易于实现、计算高效。
...
Method: 提出GreedyLR调度器，该调度器基于当前损失自适应调整学习率。提供了算法的理论分析，包括收敛性证明和最大化收敛速率的最优缩放因子F的推导。在多个NLP、CV和LLM任务（参数规模高达70亿）上进行了实验验证，包括微调和预训练实验。

Result: 实验结果表明，GreedyLR在准确性、速度和收敛性方面优于多种最先进的调度器。算法对现实噪声场景具有鲁棒性，且易于实现、计算高效。

Conclusion: GreedyLR调度器是一种有效的自适应学习率调度方法，可被视为训练的良好默认调度器选择。
...
TL;DR: 本文分析了通用Transformer在复杂推理任务上的性能提升来源，发现主要来自循环归纳偏置和Transformer的强非线性组件，而非复杂架构设计。基于此提出了增强的通用推理模型URM，在ARC-AGI基准上取得了SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 通用Transformer在ARC-AGI和数独等复杂推理任务上表现出色，但其性能提升的具体来源尚未得到充分探索。本文旨在系统分析UT变体，识别真正有效的组件，从而设计更高效的推理模型。

Method: 首先系统分析了UT的各种变体，发现性能提升主要来自循环归纳偏置和Transformer的强非线性组件。基于这一发现，提出了通用推理模型URM，通过引入短卷积和截断反向传播来增强UT。

Result: URM在ARC-AGI基准上取得了显著的性能提升：在ARC-AGI 1上达到53.8% pass@1，在ARC-AGI 2上达到16.0% pass@1，均达到当前最佳水平。

Conclusion: 通用Transformer在复杂推理任务上的性能提升主要源于循环归纳偏置和强非线性组件，而非复杂架构设计。提出的URM模型通过简单有效的增强，在推理任务上实现了SOTA性能，为高效推理模型设计提供了新思路。
...
### [156] [IPR-1: Interactive Physical Reasoner](https://arxiv.org/abs/2511.15407)
...
<details>
  <summary>Details</summary>
Motivation: 研究智能体是否能像人类一样通过交互学习获得类似人类的推理能力，并随着经验积累持续改进。现有方法（VLMs和世界模型）难以捕捉底层物理和因果关系，因为它们过度关注视觉细节而非核心机制

Method: 提出IPR（交互式物理推理器），使用世界模型推演来评分和强化VLM的策略，并引入PhysCode（物理中心动作编码）将语义意图与动力学对齐，为预测和推理提供共享动作空间

Result: 在1000+游戏上预训练后，IPR在从原始直觉到目标驱动推理的各种关卡中表现稳健，整体甚至超过GPT-5。性能随训练游戏和交互步骤增加而提升，并能零样本迁移到未见游戏

Conclusion: 物理中心的交互是稳步改进物理推理能力的有效途径，支持智能体通过交互学习获得类似人类的推理能力
