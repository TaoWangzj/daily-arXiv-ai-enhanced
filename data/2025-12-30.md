...
### [1] [Bridging Your Imagination with Audio-Video Generation via a Unified Director](https://arxiv.org/abs/2512.23222)
...
Method: 采用Mixture-of-Transformers架构统一文本和图像生成。引入"先交错，后解耦"的训练范式：1)交错概念学习，使用交错文本-图像数据培养模型对剧本的深入理解和想象力解释；2)解耦专家学习，将剧本写作与关键帧生成解耦，实现更大的故事叙述灵活性和创造力。

Result: 大量实验表明，UniMAGE在开源模型中实现了最先进的性能，能够生成逻辑连贯的视频剧本和视觉一致的关键帧图像。

Conclusion: UniMAGE成功地将剧本创作和关键镜头设计统一到单一框架中，为非专业人士制作长上下文、多镜头电影提供了有效工具，证明了统一导演模型在AI视频创作中的可行性和优势。
...
<details>
  <summary>Details</summary>
Motivation: 人工智能在医疗领域的快速发展为改进医疗流程（包括医疗账单和转录）创造了机会。传统方法在处理高维医疗数据时存在准确性和效率问题，需要新的解决方案来减少人为错误并简化操作。

Method: 开发了AI增强的局部线性嵌入（LLE）模型，专门针对医疗账单系统和转录服务进行优化。该方法通过自动化处理高维医疗数据，提供完整的数学模型，并在真实医疗场景中进行实验验证。

Result: 实验结果表明，该模型在数据处理准确性和操作效率方面都有显著提升，能够实现更快、更准确的病人护理文档记录和财务交易处理。

Conclusion: AI增强的LLE在医疗数据分析中具有巨大潜力，不仅为医疗账单和转录服务提供了有效解决方案，也为未来更广泛的医疗应用研究奠定了基础。
...
### [3] [RealX3D: A Physically-Degraded 3D Benchmark for Multi-view Visual Restoration and Reconstruction](https://arxiv.org/abs/2512.23437)
...
TL;DR: RealX3D是一个真实采集的多视角视觉恢复和3D重建基准数据集，包含多种物理退化情况，分为四类退化类型，每个场景都包含高质量参考数据，用于评估现有方法在真实挑战性环境下的表现。


<details>
  <summary>Details</summary>
Motivation: 当前多视角视觉恢复和3D重建方法在理想条件下表现良好，但在真实世界的物理退化环境下（如光照变化、散射、遮挡、模糊等）表现脆弱，缺乏一个系统评估这些方法在真实退化条件下的基准数据集。

Method: 创建RealX3D基准数据集：1）将物理退化分为四类（光照、散射、遮挡、模糊）；2）采用统一的采集协议，在每个退化类型的不同严重程度下捕获像素对齐的低质量/高质量视图；3）每个场景包含高分辨率捕获、RAW图像和密集激光扫描数据；4）从这些数据中导出世界尺度网格和度量深度信息。

Result: 通过评估大量基于优化的和前馈方法，发现在物理退化条件下重建质量显著下降，突显了当前多视角流程在真实世界挑战性环境中的脆弱性。

Conclusion: RealX3D基准数据集揭示了现有多视角视觉恢复和3D重建方法在真实物理退化条件下的局限性，为开发更鲁棒的算法提供了重要的评估工具和方向。
...
<details>
  <summary>Details</summary>
Motivation: 端到端视觉语言模型（VLMs）在回答视觉问题时经常利用虚假相关性而非因果视觉证据，微调后更容易出现捷径学习问题。需要一种方法来分离感知与推理，减少对虚假相关性的依赖。

Method: 提出VISTA框架：1）冻结的VLM传感器仅进行简短、客观的感知查询；2）纯文本LLM推理器分解问题、规划查询、用自然语言聚合视觉事实；3）通过强化学习（GRPO）在仅641个多步问题上训练无偏视觉推理。

Result: 在SpuriVerse数据集上显著提升鲁棒性（Qwen-2.5-VL-7B提升16.29%，Llama-3.2-Vision-11B提升6.77%），在MMVP和SeedBench子集上保持竞争力。能跨未见VLM传感器稳健迁移，并能识别和恢复VLM感知失败。

Conclusion: VISTA通过模块化设计有效减少视觉语言模型对虚假相关性的依赖，产生更中立、更明确基于视觉证据的推理轨迹，为构建更可靠的视觉推理系统提供了新思路。
...
TL;DR: HookMIL是一个用于计算病理学中全切片图像弱监督分析的多实例学习框架，通过可学习的hook tokens实现结构化上下文聚合，具有线性复杂度，支持多模态初始化，并在四个公开数据集上达到最先进性能。


<details>
  <summary>Details</summary>
Motivation: 传统MIL方法在分析全切片图像时会丢失关键上下文信息，而基于transformer的变体虽然表达能力更强，但存在二次复杂度和冗余计算的问题。需要开发一种既能保留上下文信息又计算高效的MIL框架。

Method: 提出HookMIL框架，使用紧凑、可学习的hook tokens进行结构化上下文聚合。这些tokens可以通过三种方式初始化：(1)关键patch的视觉特征；(2)视觉-语言病理学模型的文本嵌入；(3)空间转录组-视觉模型的空间基础特征。训练时，hook tokens通过双向注意力与实例交互，具有线性复杂度。还引入了Hook多样性损失促进每个token关注不同的组织病理学模式，以及hook-to-hook通信机制来细化上下文交互并减少冗余。

Result: 在四个公开病理学数据集上的广泛实验表明，HookMIL实现了最先进的性能，同时提高了计算效率和可解释性。

Conclusion: HookMIL是一个上下文感知且计算高效的MIL框架，通过多模态初始化的hook tokens有效聚合上下文信息，在保持线性复杂度的同时提升了表示质量和收敛速度，为计算病理学中的弱监督分析提供了有效解决方案。
...
Conclusion: 检测器引导的提示结合目标稀疏采样是实用全场景分割的有效替代方案，相比密集"分割一切"提示在速度和覆盖率上都有显著优势。
...
TL;DR: 提出了一种用于糖尿病视网膜病变诊断的新型多模态可解释性模型，结合视觉语言模型和少样本学习，通过分析视网膜象限中的病变分布来模拟眼科医生的推理过程。


<details>
  <summary>Details</summary>
Motivation: 糖尿病视网膜病变是全球视力丧失的主要原因，需要早期检测。医生缺乏可解释的AI模型，现有模型依赖单一成像模态且可解释性有限，需要能够解释分类理由而非仅突出病变位置的系统。
...
Conclusion: 该创新方法通过多模态可解释性模型，模拟眼科医生的推理过程，为糖尿病视网膜病变的诊断提供了更实用和全面的解决方案，有望在筛查、治疗和研究环境中得到广泛应用。
...
TL;DR: VLM-PAR是一个基于冻结SigLIP 2多语言编码器的视觉语言框架，通过跨注意力融合改进视觉特征，在行人属性识别任务中取得了新的SOTA性能。
...
Method: 基于冻结的SigLIP 2多语言编码器构建模块化视觉语言框架，通过紧凑的跨注意力融合对齐图像和提示嵌入，改进视觉特征。

Result: 在高度不平衡的PA100K基准测试中实现了显著的准确率提升，创下新的SOTA性能，同时在PETA和Market-1501基准测试中也取得了显著的均值准确率提升。

Conclusion: 将大规模视觉语言预训练与针对性的跨模态细化相结合，能有效克服行人属性识别中的不平衡和泛化挑战。
...
TL;DR: 该论文提出了一种基于语义抽象的3D隐藏物体定位方法，利用2D视觉语言模型的激活图作为"抽象物体"表示，结合历史数据提高搜索效率


<details>
  <summary>Details</summary>
Motivation: 为家庭机器人提供更高效的寻找隐藏/丢失物体的能力，减少时间和精力消耗。传统方法难以直接识别被遮挡的物体，需要更智能的搜索策略。

Method: 使用2D视觉语言模型的激活图作为"抽象物体"表示，将其扩展到3D定位和补全。通过历史数据学习物体常见放置位置，将隐藏物体定位视为非结构化搜索问题。

Result: 模型能够准确识别隐藏物体的完整3D位置，首次尝试的成功率显著高于随机搜索，搜索速度明显更快。

Conclusion: 语义抽象框架的扩展为家庭机器人提供了寻找丢失物体的必要技能，能够显著节省时间和精力，提高了搜索效率。
...
TL;DR: VideoScaffold：用于流媒体视频理解的动态表示框架，通过自适应事件粒度和分层事件整合解决现有静态策略在连续视频流中的碎片化和过度压缩问题


<details>
  <summary>Details</summary>
Motivation: 现有多模态大语言模型在理解长视频时面临帧间冗余和时序连贯性表示的挑战，静态策略（如稀疏采样、帧压缩、聚类）在离线场景优化，但应用于连续视频流时会产生碎片化或过度压缩的输出

Method: 提出VideoScaffold动态表示框架，包含两个核心组件：1) 弹性尺度事件分割(EES)：通过预测引导的分割动态细化事件边界；2) 分层事件整合(HEC)：渐进聚合语义相关片段为多层次抽象表示

Result: 在离线和流媒体视频理解基准测试中均达到最先进性能，框架模块化且即插即用，可无缝扩展现有基于图像的MLLMs到连续视频理解

Conclusion: VideoScaffold能够根据视频时长自适应调整事件粒度，同时保留细粒度视觉语义，实现从细粒度帧理解到抽象事件推理的平滑过渡，为流媒体视频理解提供了有效的动态表示解决方案
...
TL;DR: 该论文提出了一种KAN增强的FPN-Stem架构，用于改进ViT在姿态估计等密集预测任务中的性能。研究发现性能瓶颈不在于注意力模块，而在于FPN中的后融合非线性平滑步骤，因此用KAN卷积层替换标准线性卷积，在COCO数据集上实现了+2.0 AP的性能提升。


<details>
  <summary>Details</summary>
Motivation: Vision Transformers在密集预测任务中表现良好，但现有模型如ViTPose的前端设计过于简单。朴素的patchification机制难以有效处理多尺度变化，并在初始特征提取阶段造成不可逆的信息损失，这限制了ViT的性能提升。

Method: 提出KAN增强的FPN-Stem架构。通过消融研究发现性能瓶颈在于FPN的后融合非线性平滑步骤，而非注意力模块。核心创新是保留FPN经典的"上采样-相加"融合流程，但将其末端的标准线性3x3平滑卷积替换为基于KAN的卷积层，利用KAN优越的非线性建模能力自适应学习和修正多尺度融合过程中产生的"伪影"。

Result: 在COCO数据集上的大量实验表明，KAN-FPN-Stem相比轻量级ViTPose-S基线实现了高达+2.0 AP的显著性能提升。

Conclusion: 该工作不仅提供了一个即插即用的高性能模块，更重要的是揭示了ViT前端的性能瓶颈通常不在于"特征精炼"(注意力)，而在于"特征融合"的质量。同时，通过引入KAN算子为解决这一瓶颈提供了有效路径。
...
<details>
  <summary>Details</summary>
Motivation: 传统RGB检测在低光环境下表现不佳，而热成像系统缺乏颜色和纹理信息，限制了自主机器人在应急服务（如搜救任务）中的视觉能力。

Method: 训练33个YOLO模型，使用22,000多张标注图像，涵盖三种光照水平。通过11种融合比例（从100/0到0/100）对齐融合RGB和LWIR帧，动态选择每个光照条件下的最优检测模型。

Result: 最佳全光模型（80/20 RGB-LWIR）和暗光模型（90/10融合）分别达到92.8%和92.0%的平均置信度，显著优于YOLOv5n和YOLOv11n基线。无光条件下，最佳40/60融合达到71.0%，超过基线但统计不显著。

Conclusion: 自适应RGB-LWIR融合提高了所有光照条件下的检测置信度和可靠性，增强了自主机器人的视觉性能，为应急服务中的机器人应用提供了更鲁棒的解决方案。
...
TL;DR: 该研究提出使用轻量级判别器作为外部引导信号，通过人类感知嵌入（HPE）教师模型将几何理解引入文本到图像扩散模型，实现几何与风格的可控分离，显著提升语义对齐约80%。


<details>
  <summary>Details</summary>
Motivation: 当前文本到图像扩散模型虽然能生成高度详细的纹理，但主要依赖表面外观，难以遵循严格的几何约束，特别是当几何约束与文本提示暗示的风格相冲突时。这反映了人类感知与当前生成模型之间的语义鸿沟。

Method: 提出人类感知嵌入（HPE）教师模型，该模型在THINGS三元组数据集上训练，捕捉人类对物体形状的敏感性。将该教师模型的梯度注入潜在扩散过程，实现几何与风格的可控分离。在三种架构上评估：Stable Diffusion v1.5（U-Net骨干）、流匹配模型SiT-XL/2和扩散变换器PixArt-Σ。

Result: 实验表明，流模型在没有持续引导时倾向于漂移回默认轨迹。实现了复杂三维形状（如Eames椅子）到冲突材料（如粉色金属）的零样本迁移。与无引导基线相比，引导生成将语义对齐提高了约80%。
...
TL;DR: GeCo是一种几何基础度量方法，用于联合检测静态场景中的几何变形和遮挡不一致伪影，通过融合残差运动和深度先验生成可解释的一致性图，用于基准测试视频生成模型并作为无训练指导损失减少变形伪影。


<details>
  <summary>Details</summary>
Motivation: 当前视频生成模型在静态场景中经常产生几何变形和遮挡不一致的伪影，需要一种系统性的方法来检测和评估这些质量问题，以改进视频生成的质量。

Method: GeCo通过融合残差运动和深度先验，生成密集的一致性图来检测几何变形和遮挡不一致伪影。该方法基于几何基础，能够提供可解释的检测结果。

Result: GeCo能够有效检测视频生成模型中的几何变形和遮挡不一致伪影，成功用于系统基准测试近期视频生成模型，并作为无训练指导损失显著减少生成视频中的变形伪影。

Conclusion: GeCo提供了一个有效的几何基础度量框架，不仅能够诊断视频生成模型的常见失败模式，还能作为无训练指导损失来改进视频生成质量，具有实际应用价值。
...
TL;DR: 该研究开发了B&J基准测试，评估AI模型在骨科临床推理中的表现，发现模型在结构化选择题上表现良好（>90%），但在需要多模态整合的开放任务中表现显著下降（约60%），表明当前AI模型尚不具备临床多模态推理能力。
...
Method: 开发了Bones and Joints (B&J)基准测试，包含1,245个来自真实骨科病例的问题，评估模型在7个临床推理任务上的表现。评估了11个视觉语言模型和6个大语言模型，并与专家提供的真实答案进行比较。

Result: 最先进模型在结构化多选题上准确率超过90%，但在需要多模态整合的开放任务中准确率仅约60%。视觉语言模型在医学图像解释方面存在严重局限性，经常出现文本驱动的幻觉，忽略矛盾的视觉证据。专门针对医疗应用微调的模型相比通用模型没有明显优势。
...
Conclusion: Uni4D框架通过结构化三级对齐推进了动态多模态理解和实际应用的发展。
...
TL;DR: 提出了一种用于低成本边缘设备（树莓派5和Coral Edge TPU）的单摄像头驾驶员行为识别系统，能够实时监测17种分心和疲劳行为，实现低延迟、低功耗的驾驶舱监控。


<details>
  <summary>Details</summary>
Motivation: 驾驶舱监控系统需要在计算能力、功耗和成本严格受限的条件下，低延迟地识别驾驶员分心和疲劳相关行为。现有系统往往难以在低成本硬件上实现实时性能。

Method: 采用单摄像头方案，结合三个关键技术：(1)紧凑的逐帧视觉模型；(2)混淆因素感知的标签设计以减少视觉相似误报；(3)时序决策头，仅在预测既自信又持续时才触发警报。系统覆盖17种行为类别。

Result: 在树莓派5上使用INT8推理达到约16 FPS（每帧延迟低于60毫秒），在Coral Edge TPU上达到约25 FPS，能够在廉价硬件上实现实时监控和稳定警报生成。

Conclusion: 该系统证明了在低成本边缘设备上实现可靠驾驶舱监控的可行性，为以人为中心的车辆智能（包括新兴的自主车辆概念）提供了上游感知输入。
...
TL;DR: SpotEdit：一种无需训练的扩散编辑框架，通过选择性更新修改区域来减少冗余计算，提高图像编辑效率


<details>
  <summary>Details</summary>
Motivation: 当前扩散Transformer模型在图像编辑时均匀处理所有token，即使只有小区域需要修改，导致计算冗余且可能降低未修改区域的质量

Method: SpotEdit包含两个核心组件：SpotSelector通过感知相似性识别稳定区域并复用条件图像特征跳过计算；SpotFusion通过动态融合机制自适应混合特征，保持上下文连贯性
...
<details>
  <summary>Details</summary>
Motivation: 现有方法主要关注前向建模（如从文本到运动的整体映射），而忽略了从整体运动中分解出语义上有意义的子组件的逆视角。人类运动具有组合性，复杂行为可以描述为简单原语的组合，因此需要一种能够发现和分解运动概念的方法。

Method: 提出DeMoGen组合训练范式，采用基于能量的扩散模型直接捕捉多个运动概念的组合分布。包含三种训练变体：1）DeMoGen-Exp：在分解的文本提示上显式训练；2）DeMoGen-OSS：执行正交自监督分解；3）DeMoGen-SC：强制原始和分解文本嵌入之间的语义一致性。还构建了文本分解数据集支持组合训练。

Result: 该方法能够从复杂运动序列中解耦可重用的运动原语，分解后的运动概念可以灵活地重新组合，生成超出训练分布的多样新颖运动。构建的数据集为文本到运动生成和运动组合提供了扩展资源。

Conclusion: DeMoGen提供了一种有效的运动分解学习范式，能够发现和分解运动概念而无需依赖单个概念的ground-truth运动数据，支持运动原语的解耦和重新组合，为运动生成和理解提供了新的视角。
...
TL;DR: 提出基于变分自编码器的多视图潜在表示学习框架，从MRI图像中提取互补的放射组学特征，用于无创推断胶质母细胞瘤的MGMT启动子甲基化状态。


<details>
  <summary>Details</summary>
Motivation: 传统单模态和早期融合方法在放射基因组学中存在特征冗余高、模态特异性信息建模不完整的问题，限制了从医学影像中无创推断分子肿瘤特征（特别是胶质母细胞瘤中具有重要预后和治疗意义的MGMT启动子甲基化状态）的准确性。

Method: 采用基于变分自编码器的多视图潜在表示学习框架，通过独立的概率编码器分别编码T1Gd和FLAIR MRI模态，在紧凑的潜在空间中进行融合，保留模态特异性结构的同时实现有效的多模态整合，最终使用生成的潜在嵌入进行MGMT启动子甲基化分类。

Result: 该方法能够有效整合来自不同MRI序列的互补放射组学特征，在保留模态特异性信息的同时减少特征冗余，为MGMT启动子甲基化状态的准确分类提供了改进的多模态表示学习方案。

Conclusion: 提出的多视图潜在表示学习框架通过变分自编码器实现了MRI多模态信息的有效融合，克服了传统方法的局限性，为胶质母细胞瘤MGMT启动子甲基化的无创预测提供了更优的放射基因组学解决方案。
...
TL;DR: 提出基于视觉Transformer的端到端管道，联合分析H&E和IHC染色图像，实现像素级HER2评分定位，准确预测HER2状态


<details>
  <summary>Details</summary>
Motivation: 传统HER2评分方法难以提供像素级定位，且联合分析H&E和IHC图像存在挑战，需要更精确的自动化评分系统

Method: 使用视觉Transformer系统，通过分块处理H&E图像定位肿瘤，提出新颖映射函数关联IHC图像区域，嵌入临床启发式HER2评分机制实现4级像素级标注
...
TL;DR: 该研究提出了一种基于多模态大语言模型的3D场景操作框架，通过MCP API、视觉工具套件和多智能体协作解决了3D物体排列任务中的关键挑战。


<details>
  <summary>Details</summary>
Motivation: 尽管多模态大语言模型在2D视觉语言任务上取得了显著进展，但在复杂3D场景操作方面的应用仍未被充分探索。现有MLLMs在视觉基础方面较弱，难以将程序化编辑与精确的3D结果联系起来。

Method: 1. 引入基于MCP的API，将交互从脆弱的原始代码操作转向更稳健的函数级更新；2. 使用专门的视觉工具套件增强MLLM的3D场景理解能力，包括场景状态分析、空间信息收集和动作结果验证；3. 提出协作多智能体框架，分配规划、执行和验证角色，以管理迭代且容易出错的更新过程。

Result: 在25个复杂的物体排列任务上验证了方法的有效性，显著优于现有基线方法。

Conclusion: 该研究成功地将MLLMs应用于复杂3D场景操作，通过API设计、视觉增强和多智能体协作解决了3D物体排列任务的关键挑战，为3D场景理解与操作提供了新思路。
...
<details>
  <summary>Details</summary>
Motivation: 传统扩散模型或流匹配模型通常需要多步推理才能获得高质量结果，而蒸馏方法需要预训练教师模型。Self-E旨在填补这两种范式之间的空白，实现从零开始训练、支持任意步数推理的统一框架。

Method: Self-E结合了流匹配模型的数据学习方式和创新的自评估机制：模型使用当前的分数估计来评估自己生成的样本，作为动态的自教师。这种方法同时实现了即时局部学习和自驱动的全局匹配。

Result: 在大规模文本到图像基准测试中，Self-E在少步生成方面表现出色，同时在50步时与最先进的流匹配模型具有竞争力。其性能随着推理步数的增加而单调提升，实现了超快的少步生成和高质量的长轨迹采样。

Conclusion: Self-E是第一个从零开始训练、支持任意步数推理的文本到图像模型，为高效和可扩展的生成提供了一个统一的框架。
...
TL;DR: iOSPointMapper是一款基于iOS设备的移动应用，利用设备内置的语义分割、LiDAR深度估计和GPS/IMU融合技术，实现实时、隐私保护的实时人行道测绘，检测交通标志、信号灯等特征，数据匿名传输至交通数据交换平台。


<details>
  <summary>Details</summary>
Motivation: 当前人行道数据收集方法成本高、碎片化且难以扩展，缺乏准确、及时的人行道数据来建设无障碍和包容性的行人基础设施。

Method: 开发iOSPointMapper移动应用，利用iPhone/iPad设备的内置功能：1) 设备端语义分割识别相关特征；2) LiDAR深度估计；3) GPS/IMU数据融合定位；4) 用户引导的标注界面验证系统输出；5) 数据匿名化传输至交通数据交换倡议(TDEI)。

Result: 系统特征检测和空间测绘性能的详细评估显示，该应用在增强行人测绘方面具有潜力，能够无缝集成到多模式交通数据集中。

Conclusion: iOSPointMapper提供了一种可扩展、以用户为中心的方法，能够填补行人基础设施中的关键数据缺口，为构建更可访问和包容的行人环境提供支持。
...
TL;DR: DeFloMat提出了一种基于条件流匹配的新型生成式目标检测框架，通过确定性流场替代扩散模型的随机去噪过程，在仅需3步推理的情况下实现SOTA精度，解决了临床应用中扩散检测器的高延迟瓶颈。


<details>
  <summary>Details</summary>
Motivation: 扩散模型（如DiffusionDet）虽然精度高，但需要大量采样步骤（T≫60），在克罗恩病磁共振肠造影等时间敏感的临床应用中存在严重的延迟瓶颈，限制了实际应用。

Method: DeFloMat采用条件流匹配（CFM）框架，基于条件最优传输理论构建确定性流场，近似Rectified Flow，通过简单的常微分方程求解器实现快速推理，替代了扩散模型的多步随机去噪过程。

Result: 在MRE临床数据集上，DeFloMat仅用3步推理就达到了43.32% AP_{10:50}的SOTA精度，比DiffusionDet在4步时的最佳性能（31.03%）提升了1.4倍，同时显著改善了定位特性，在少步推理中表现出更好的召回率和稳定性。
...
<details>
  <summary>Details</summary>
Motivation: 无标记3D明场显微镜能快速无创地可视化细胞形态，但稳健的体积分割通常依赖荧光或繁重的后处理，需要填补这一技术空白

Method: 结合硬件对齐的Native Sparse Attention机制（捕获局部、粗粒度和选择性全局上下文）、深度-宽度残差HyperConnections稳定表示流、软混合专家机制实现自适应容量，以及各向异性补丁嵌入尊重共焦点扩散和轴向变薄特性

Result: 在多个共焦数据集上，Bright-4B能准确分割细胞核、线粒体等细胞器，保留跨深度和细胞类型的精细结构细节，优于当代CNN和Transformer基线模型
...
TL;DR: 本文提出EmoCtrl模型，用于可控情感图像内容生成，能够在保持内容描述准确性的同时表达目标情感，解决了现有模型在内容一致性和情感表达之间的平衡问题。


<details>
  <summary>Details</summary>
Motivation: 图像通过视觉内容和情感基调共同传达意义，但现有文本到图像模型虽然能保证内容一致性却缺乏情感意识，而情感驱动模型在生成情感结果时会扭曲内容。需要一种既能忠实于内容描述又能表达目标情感的方法。

Method: 提出EmoCtrl模型，构建了包含内容、情感和情感提示的数据集，将抽象情感与视觉线索联系起来。模型包含文本和视觉情感增强模块，通过描述性语义和感知线索丰富情感表达。

Result: 定量和定性实验表明EmoCtrl在忠实内容和表达情感控制方面优于现有方法。消融研究和可视化显示学习到的情感标记具有互补效应。用户研究证实EmoCtrl与人类偏好高度一致，且能很好地推广到创意应用中。

Conclusion: EmoCtrl成功实现了可控情感图像内容生成，在保持内容准确性的同时有效表达情感，学习到的情感标记具有鲁棒性和适应性，为情感感知的图像生成提供了有效解决方案。
...
TL;DR: 提出了一种基于线特征的事件相机标定框架，利用人造环境中的几何线条（如门窗、箱子等）进行标定，无需特殊标定板，直接从事件流中检测线条并优化相机参数。


<details>
  <summary>Details</summary>
Motivation: 现有事件相机标定方法通常需要闪烁图案、重建强度图像或使用特殊标定物，过程耗时且需要人工放置标定对象，无法适应快速变化的场景需求。

Method: 1. 直接从事件流中检测线条；2. 利用事件-线条标定模型生成相机参数初始估计；3. 采用非线性优化方法细化相机参数；4. 适用于平面和非平面线条。

Result: 通过仿真和真实世界实验验证了方法的可行性和准确性，在单目和双目事件相机上进行了验证，代码已开源。

Conclusion: 提出了一种基于线特征的事件相机标定框架，能够利用常见人造环境中的几何线条进行快速、准确的标定，无需特殊标定设备，适用于动态场景。
...
Method: 使用Flutter开发平台构建智能手机应用，后端采用Efficientdet-D2模型进行物体检测。通过耳机提供声音提示：物体在左侧时左耳播放正弦波声音，在右侧时右耳播放，正前方时双耳同时播放。

Result: 开发了完全离线工作的SonoVision应用，能够通过声音提示帮助视障人士定位物体，减少对周围人的依赖，提高独立性。

Conclusion: SonoVision应用以安全、用户友好的方式显著帮助视障人士，通过声音提示系统增强他们的空间感知能力，促进独立生活。
...
Method: 使用SAM 3D基础模型进行单目遥感建筑重建，在NYC Urban Dataset上对比TRELLIS方法，采用FID和CMMD作为评估指标。进一步通过分段-重建-组合流水线将SAM 3D扩展到城市场景重建。

Result: 实验结果显示，SAM 3D相比TRELLIS能产生更连贯的屋顶几何和更清晰的边界。通过扩展的流水线，SAM 3D展示了在城市场景建模中的潜力。

Conclusion: 该研究为在城市3D重建中部署基础模型提供了实用指导，并激励未来集成场景级结构先验。同时分析了实际限制并讨论了未来研究方向。
...
Method: 研究使用手动标记的美国变电站图像数据集，训练并比较了三种计算机视觉模型：YOLOv8、YOLOv11和RF-DETR。评估了每个模型在检测准确性、精确度和效率方面的表现，并分析了各模型的关键优势和局限性。

Result: 研究评估了三种模型在变电站组件检测中的性能，确定了哪种模型能够提供可靠且可扩展的变电站组件映射。此外，研究还展示了这些模型在美国各地变电站组件有效映射的实际应用案例。

Conclusion: 该研究展示了机器学习在变电站映射中的应用潜力，通过比较不同计算机视觉模型的性能，为自动化变电站基础设施监测提供了可行的技术方案，有助于预防和减轻电网故障带来的风险。
...
TL;DR: PGR²M提出了一种混合表示方法，通过残差向量量化增强可解释姿态码，提升文本驱动的3D动作生成和编辑质量


<details>
  <summary>Details</summary>
Motivation: 现有基于姿态码的框架（如CoMo）虽然支持可解释的动作控制，但其逐帧表示难以捕捉细微的时间动态和高频细节，导致重建保真度和局部可控性下降

Method: 提出姿态引导的残差细化运动表示(PGR²M)，使用姿态引导的RVQ分词器将动作分解为编码全局结构的姿态潜在码和建模细粒度时间变化的残差潜在码，通过残差丢弃防止过度依赖残差，并采用基础Transformer预测姿态码、细化Transformer预测残差码的双阶段架构

Result: 在HumanML3D和KIT-ML数据集上，PGR²M在Fréchet inception距离和重建指标上优于CoMo及最近的扩散和分词化基线，用户研究证实其能实现直观、结构保持的动作编辑

Conclusion: PGR²M通过混合表示方法有效解决了现有姿态码框架在时间动态捕捉方面的局限性，在保持语义对齐和可编辑性的同时显著提升了动作生成和编辑的质量
...
TL;DR: 提出了一种基于多事件相机的高时空分辨率冲击波运动参数测量框架，通过极坐标编码、自适应ROI提取和迭代斜率分析实现冲击波前事件提取，最终实现3D重建和爆炸当量反演，测量误差最小0.06%，最大5.20%。


<details>
  <summary>Details</summary>
Motivation: 冲击波运动参数的高时空分辨率精确测量在动力场测试和损伤评估中至关重要，但冲击波的快速、不均匀传播和不稳定测试条件带来了重大挑战。

Method: 1) 建立极坐标系，通过事件偏移计算进行自适应ROI提取；2) 利用迭代斜率分析提取冲击波前事件；3) 基于事件的光学成像模型推导几何模型和冲击波运动参数；4) 结合3D重建模型实现多角度测量、运动场重建和爆炸当量反演。

Result: 与压力传感器和经验公式的测量结果对比显示，最大误差为5.20%，最小误差为0.06%，实现了高时空分辨率的高精度冲击波运动场测量。

Conclusion: 该方法利用事件相机的高速和高动态范围特性，成功解决了冲击波测量的挑战，实现了高精度的冲击波运动场测量，代表了该领域的重要进展。
...
TL;DR: 提出一种用于红外小目标检测的半监督范式，通过分层MoE适配器和两阶段知识蒸馏/迁移，在仅有10%标注数据下使下游模型达到接近全监督的性能。
...
Result: 实验表明，在最小标注量下，该范式能使下游模型达到甚至超越全监督对应模型的性能，是首个系统解决IR-SOT数据稀缺问题、以SAM为教师模型的半监督范式。
...
TL;DR: SR-MCR是一个轻量级、无标签的多模态LLM推理对齐框架，通过利用模型输出的内在过程信号来提升推理的可靠性和连贯性。
...
Method: 提出SR-MCR框架，整合五个自参考线索（语义对齐、词汇保真度、非冗余性、视觉基础和步骤一致性）形成归一化的可靠性加权奖励，提供细粒度的过程级指导。采用无批评者的GRPO目标，结合置信度感知冷却机制来稳定训练并抑制平凡或过度自信的生成。

Result: 基于Qwen2.5-VL构建的SR-MCR在多个视觉基准测试中提升了答案准确性和推理连贯性。SR-MCR-7B在同类开源模型中达到最先进性能，平均准确率为81.4%。消融研究证实了每个奖励项和冷却模块的独立贡献。

Conclusion: SR-MCR通过利用模型内在过程信号进行对齐，有效提升了多模态LLM的推理可靠性和连贯性，为过程级监督提供了轻量级解决方案。
...
TL;DR: 提出了一种基于稀疏差分Transformer的Top-K Jaccard相似度系数方法，用于改进人脸聚类中嵌入向量关系的测量精度，在多个数据集上实现了SOTA性能。
...
Conclusion: 通过预测驱动的Top-K Jaccard相似度系数和稀疏差分Transformer的结合，有效解决了人脸聚类中相似度测量精度不足的问题，显著提升了聚类性能。
...
TL;DR: Dream-VL是基于扩散大语言模型构建的视觉语言模型，在视觉规划任务中表现优异；Dream-VLA是基于扩散模型的视觉语言动作模型，在机器人控制任务中达到顶尖性能。
...
Result: Dream-VL在多种基准测试中与顶级自回归模型相当，在视觉规划任务中表现更优；Dream-VLA在LIBERO上达到97.2%平均成功率，在SimplerEnv-Bridge和SimplerEnv-Fractal上分别达到71.4%和60.5%的平均性能，超越π₀和GR00T-N1等领先模型。

Conclusion: 基于扩散模型的视觉语言模型在视觉规划和机器人控制任务中具有优势，扩散模型的双向特性使其更适合动作分块和并行生成，在下游任务中收敛更快、性能更优。
...
TL;DR: Envision是一个基于扩散模型的视觉规划框架，通过两阶段方法生成从初始观察到目标图像的物理合理轨迹，解决现有方法的空间漂移和目标对齐问题。


<details>
  <summary>Details</summary>
Motivation: 现有基于视频扩散模型的视觉规划方法主要是前向预测性的，仅基于初始观察生成轨迹，缺乏明确的目标建模，导致空间漂移和目标对齐问题。需要一种能够显式约束目标一致性并确保物理合理性的方法。
...
Conclusion: Envision通过显式目标约束和两阶段生成框架，有效解决了视觉规划中的空间漂移和目标对齐问题，为具身智能体提供了可靠的视觉规划指导。
...
TL;DR: 该研究针对图像超分辨率任务中基于人类反馈的强化学习存在的奖励模型局限性问题，提出了细粒度感知奖励模型和协同进化课程学习机制，以解决传统图像质量评估模型对局部失真不敏感导致的奖励欺骗问题。


<details>
  <summary>Details</summary>
Motivation: 传统图像质量评估模型通常只输出全局评分，对局部和细粒度的失真异常不敏感，这导致图像超分辨率模型可能产生感知上不理想的伪影却获得虚假高分，造成优化目标与感知质量不对齐的奖励欺骗问题。

Method: 提出了基于编码器-解码器架构的细粒度感知奖励模型，不仅能提供全局质量评分，还能生成感知退化图来空间定位和量化局部缺陷。同时设计了协同进化课程学习机制，让奖励模型和超分辨率模型同步进行课程学习，奖励模型复杂度逐步增加，超分辨率模型从简单全局奖励开始，逐步过渡到复杂模型输出。

Result: 实验验证了该方法在基于人类反馈的强化学习方法中，对图像超分辨率模型的全局质量和局部真实感方面都表现出有效性。

Conclusion: 通过细粒度感知奖励模型和协同进化课程学习机制，成功解决了传统图像质量评估模型在强化学习中的局限性，实现了更稳定、更符合人类感知的图像超分辨率优化。
...
TL;DR: 提出基于视觉自回归先验的单目深度估计方法，作为扩散方法的替代方案，在有限训练数据下实现竞争性性能


<details>
  <summary>Details</summary>
Motivation: 为深度估计提供扩散方法之外的替代方案，利用自回归先验在数据可扩展性和3D视觉任务适应性方面的优势

Method: 采用大规模文本到图像VAR模型，引入尺度条件上采样机制和分类器无指导，通过10个固定自回归阶段进行推理，仅需74K合成样本微调

Result: 在室内基准测试中取得最先进性能（训练条件受限），在室外数据集上表现强劲，验证了自回归先验作为几何感知生成模型的补充价值
...
TL;DR: 本研究评估了多种深度学习架构（3D Inception、双流、CNN-RNN）在超声心动图视频中左心室射血分数（LVEF）估计的性能，发现改进的3D Inception架构在EchoNet-Dynamic数据集上表现最佳，RMSE为6.79%。


<details>
  <summary>Details</summary>
Motivation: 超声心动图是评估LVEF的常用非侵入性方法，但手动评估耗时且存在观察者间变异性。深度学习有望提供与经验丰富的专家相当的性能，实现自动化、准确的LVEF估计。

Method: 研究了多种深度学习架构：3D Inception、双流模型和CNN-RNN模型。系统评估了架构修改和融合策略，以最大化预测准确性。使用EchoNet-Dynamic数据集（10,030个超声心动图视频）进行训练和评估。

Result: 改进的3D Inception架构表现最佳，RMSE为6.79%。观察到过拟合趋势，较小、较简单的模型通常表现出更好的泛化能力。模型性能对超参数选择（特别是卷积核大小和归一化策略）高度敏感。

Conclusion: 深度学习在超声心动图LVEF估计中具有潜力，改进的3D Inception架构表现最优。虽然本研究专注于超声心动图，但关于架构设计和训练策略的见解可能适用于更广泛的医学和非医学视频分析任务。
...
TL;DR: CLAdapter是一种新颖的集群注意力适配器，通过注意力机制和聚类中心来优化从大规模预训练模型学到的特征表示，使其适应数据有限的下游科学任务。


<details>
  <summary>Details</summary>
Motivation: 虽然大规模数据集和预训练模型在计算机视觉领域取得了显著进展，但在专业科学领域和数据有限的下游任务中，如何有效利用预训练知识仍然面临挑战。需要一种方法能够将丰富的预训练特征适应到各种数据稀缺的科学应用场景。

Method: CLAdapter引入注意力机制和聚类中心，通过分布相关性和变换矩阵来个性化增强转换后的特征。它采用统一的接口设计，可以无缝集成到多种模型架构（包括CNN和Transformer）中，支持2D和3D场景。

Result: 在10个数据集上的广泛实验表明，CLAdapter在通用、多媒体、生物、医疗、工业、农业、环境、地理、材料科学、OOD和3D分析等多个数据有限的科学领域都达到了最先进的性能。

Conclusion: CLAdapter通过自适应迁移有效释放了基础视觉模型的潜力，能够将大规模预训练学到的丰富表示适应到各种数据有限的下游科学任务中，展示了在多样化科学领域的强大适应能力。
...
Method: 引入INTERACT-CMIL框架，采用多头部深度学习，通过共享特征学习与组合部分监督以及跨任务一致性的相互依赖损失，联合预测WHO4、WHO5、水平扩散、垂直扩散和细胞异型性五个组织病理学轴。

Result: 在486个专家标注的结膜活检切片多中心数据集上，INTERACT-CMIL相比CNN和基础模型基线取得一致改进，相对宏观F1分数提升高达55.1%（WHO4）和25.0%（垂直扩散）。
...
Method: 1. CritiCore模块：利用视觉语言模型和多个大语言模型丰富提示上下文，生成高级语义反馈来指导扩散过程；2. SpecFusion：在频域中合并中间生成状态，注入粗粒度结构信息同时保留高频细节。

Result: 在标准基准测试中显著提升了文本到图像对应关系的人类对齐指标和视觉质量，在人类偏好分数和美学评估方面表现优异，与最先进的奖励优化方法相当。

Conclusion: CritiFusion作为即插即用的细化阶段，兼容现有扩散模型主干，通过语义批判和频谱对齐策略实现了更好的细节、真实性和提示保真度。
...
TL;DR: 提出ARFM（自回归流匹配）方法用于序列连续数据的概率建模，在多样化视频数据集上训练以生成长时程的未来点轨迹，并建立了评估运动预测模型性能的基准。


<details>
  <summary>Details</summary>
Motivation: 现有运动预测模型通常在狭窄分布上训练，难以处理复杂运动；同时，大规模视频预测模型虽然视觉真实感强，但在复杂运动建模方面仍有不足。需要开发能够准确预测复杂运动的新方法。

Method: 开发了自回归流匹配（ARFM）方法，这是一种用于序列连续数据概率建模的新技术。在多样化视频数据集上进行训练，以生成未来点轨迹位置，特别关注长时程预测。

Result: ARFM模型能够预测复杂运动，实验表明将机器人动作预测和人体运动预测建立在预测的未来轨迹上，可以显著提升下游任务性能。

Conclusion: ARFM方法在运动预测方面表现出色，特别是在复杂运动建模和长时程预测方面。该方法为机器人动作预测和人体运动预测提供了有效的条件信息，显著提升了相关任务的性能。
...
TL;DR: 提出一种基于神经常微分方程的多模态微分同胚配准方法，利用结构描述符作为模态无关的度量模型，在无需大量训练数据的情况下实现高精度配准。


<details>
  <summary>Details</summary>
Motivation: 传统非刚性配准算法存在精度、计算复杂度和正则化之间的权衡，且通常假设图像对在解剖同源区域存在强度相关性，限制了其在多模态场景下的应用。学习型模型需要大量训练数据，且在未见模态上性能会下降。

Method: 采用神经常微分方程框架，结合结构描述符（模态无关的度量模型）来捕捉参数化邻域几何的自相似性。提出了三种变体：整合基于图像或特征的结构描述符，以及通过局部互信息计算的非结构图像相似性。

Result: 在不同扫描数据集组合的实验中进行广泛评估，结果显示在定性和定量上都超越了针对大变形或小变形以及多模态配准的最先进基线方法。该方法对显式正则化水平变化具有鲁棒性，能保持低误差，适用于不同尺度的配准，且在大变形配准方面比其他方法更高效。

Conclusion: 该方法成功解决了多模态图像配准的挑战，无需大量训练数据，在未见模态上表现稳定，为医学图像分析提供了有效的解决方案。
...
TL;DR: 提出SCPainter统一框架，结合3D高斯溅射资产表示和扩散模型，实现自动驾驶场景中的3D资产插入和新视角合成，用于生成多样化训练数据。


<details>
  <summary>Details</summary>
Motivation: 自动驾驶仿真需要3D资产插入和新视角合成来增强训练数据的多样性，但现有方法将这两个能力孤立处理，缺乏统一的框架来同时实现逼真的资产插入和场景交互。

Method: 提出SCPainter统一框架，整合3D高斯溅射(GS)汽车资产表示和3D场景点云，通过扩散模型生成高质量图像。将3D GS资产和场景点云共同投影到新视角，用这些投影条件化扩散模型。

Result: 在Waymo Open Dataset上的评估表明，该框架能够实现3D资产插入和新视角合成，促进创建多样化和逼真的驾驶数据。

Conclusion: SCPainter提供了一个统一的仿真框架，能够联合处理3D资产插入和新视角合成，为自动驾驶训练生成更多样化和逼真的场景数据。
...
TL;DR: 本研究评估了超声特异性自监督预训练模型USF-MAE在早孕期超声图像中检测囊性水囊瘤的性能，结果显示其优于传统DenseNet-169基准模型。


<details>
  <summary>Details</summary>
Motivation: 囊性水囊瘤是高风险产前超声发现，与染色体异常、结构畸形和不良妊娠结局相关。自动化检测可提高可重复性并支持规模化早期筛查，但监督式深度学习受限于小规模标注数据集。

Method: 使用基于掩码自编码（MAE）的超声自监督基础模型（USF-MAE），该模型在超过37万张未标注超声图像上预训练，然后针对囊性水囊瘤与正常对照的二分类任务进行微调。采用与DenseNet-169基准相同的超声数据集、预处理流程和4折交叉验证协议进行评估。

Result: USF-MAE在所有评估指标上均优于DenseNet-169基准：平均准确率0.96 vs 0.93，灵敏度0.94 vs 0.92，特异度0.98 vs 0.94，ROC-AUC 0.98 vs 0.94。Score-CAM可视化显示模型关注胎儿颈部相关区域，Wilcoxon符号秩检验证实性能提升具有统计学意义（p=0.0057）。

Conclusion: 超声特异性自监督预训练能够实现准确、稳健的囊性水囊瘤深度学习检测，为小标注数据集下的医学影像分析提供了有效解决方案。
...
TL;DR: 提出Freetime FeatureGS方法，通过可学习的特征高斯基元表示分解的4D场景，无需视频分割，直接从单帧分割图重建4D场景
...
Method: 使用Freetime FeatureGS表示动态场景：高斯基元具有可学习特征和线性运动能力，通过对比损失使基元特征根据2D分割图中的实例关系聚合或分离，采用时序有序的训练采样实现特征流式传播
...
<details>
  <summary>Details</summary>
Motivation: 大型多模态模型在处理视觉输入时，将图像编码为大量视觉令牌，这显著增加了推理成本。现有视觉令牌剪枝方法往往忽视长上下文多图像场景的挑战，需要专门针对此类场景的解决方案。
...
Conclusion: 该方法有效解决了长上下文多图像场景中的视觉令牌剪枝问题，通过分解冗余并动态分配预算，在减少计算成本的同时保持了模型性能。
...
<details>
  <summary>Details</summary>
Motivation: Vision Transformers在视觉识别任务中表现出色，但token表示的冗余性限制了计算效率。现有的token合并和剪枝策略往往忽视空间连续性和邻居关系，导致局部上下文信息丢失。

Method: 提出基于希尔伯特曲线重排序的邻居感知token缩减方法，将2D空间中的邻居结构显式保留在1D序列表示中。包含两个关键策略：邻居感知剪枝（NAP）用于选择性保留token，以及基于相邻token相似性的合并（MAT）用于局部token聚合。
...
TL;DR: 提出基于改进的等级反应模型(GRM)的AGIQA方法，通过双分支质量分级模块解决语义漂移问题，提升AGI图像质量评估性能


<details>
  <summary>Details</summary>
Motivation: 现有AGIQA模型通过测量图像嵌入与多等级质量描述文本嵌入之间的相似度来评估图像质量，但发现这种相似度分布常呈现多模态模式，即图像嵌入可能同时与"优秀"和"差"等级描述高度相似，而与"良好"等级偏离，这种现象称为"语义漂移"，会削弱文本-图像共享空间学习的可靠性

Method: 受心理测量学启发，提出改进的等级反应模型(GRM)用于AGIQA。GRM是一种经典评估模型，通过不同难度水平的测试项目对受试者能力进行分级。基于此设计双分支质量分级模块：一个分支估计图像能力，另一个分支构建多个难度水平。为确保难度水平的单调性，采用算术方式建模难度生成，强制实现单峰且可解释的质量分布。该算术GRM质量分级(AGQG)模块具有即插即用优势

Result: AGQG模块在集成到各种最先进的AGIQA框架中时能持续提升性能，同时在自然图像和屏幕内容图像质量评估中都能有效泛化，显示出作为未来IQA模型关键组件的潜力

Conclusion: 提出的算术GRM质量分级模块能有效缓解语义漂移问题，通过强制单峰分布提高AGIQA的可靠性和可解释性，具有广泛的适用性和泛化能力
...
<details>
  <summary>Details</summary>
Motivation: 扩散模型虽然生成性能优异，但存在高采样延迟问题。现有基于求解器的加速方法在低延迟预算下往往面临显著的图像质量下降，主要原因是无法捕捉高曲率轨迹段导致的累积截断误差
...
Result: EPD-Solver通过并行梯度评估有效缓解截断误差，在保持低延迟采样特性的同时提升图像质量；强化学习微调在复杂文本到图像生成任务中有效缓解奖励黑客问题并增强性能

Conclusion: EPD-Solver是一种创新的ODE求解器，通过几何洞察和并行梯度计算解决了扩散模型加速中的质量下降问题，其灵活的插件设计和参数高效的优化框架为扩散模型的实际应用提供了有效解决方案
...
<details>
  <summary>Details</summary>
Motivation: 现有视觉语言跟踪方法局限于局部搜索，在视角变化、遮挡和快速目标移动等挑战性场景下容易失败，需要更鲁棒的全局跟踪框架

Method: 提出基于多模态大语言模型的全局跟踪框架VPTracker，引入位置感知视觉提示机制，根据目标先前位置构建区域级提示，优先进行区域级识别，必要时才进行全局推理

Result: 大量实验表明，该方法在挑战性场景下显著提升了跟踪稳定性和目标区分能力，为将MLLMs集成到视觉跟踪中开辟了新途径

Conclusion: VPTracker通过结合全局搜索和位置感知视觉提示，有效解决了传统局部跟踪方法的局限性，在复杂场景下展现出优越的跟踪性能
...
Method: 结合3D高斯表示和tri-plane表示，利用高斯表示的高效渲染和几何表示优势，通过tri-plane增强结构连续性和语义一致性

Result: 在US和MRI等多模态医学数据集上，该方法在稀疏数据条件下能生成高质量、解剖结构连贯、语义稳定的医学图像，同时显著提升重建效率
...
TL;DR: 评估开放词汇目标检测模型在低质量图像条件下的性能，发现模型在高程度图像退化时性能急剧下降，OWLv2表现相对稳定
...
Result: 低程度图像退化时mAP无明显下降，但高程度退化时所有模型性能急剧下降；OWLv2在不同类型退化中表现相对稳定，其他模型性能显著下降

Conclusion: 开放词汇目标检测模型对高程度图像退化敏感，需要改进鲁棒性；将发布数据集和代码以促进未来研究
...
TL;DR: EgoReAct：首个自回归框架，从以自我为中心的视觉输入实时生成3D对齐的人类反应动作，解决了现有方法在因果生成和空间对齐方面的挑战。


<details>
  <summary>Details</summary>
Motivation: 人类对以自我为中心的视觉输入表现出适应性和上下文敏感的反应，但现有方法难以同时满足严格因果生成和精确3D空间对齐的双重要求。现有数据集（如ViMo）存在空间不一致问题，动态动作总是与固定摄像头视频配对。
...
Conclusion: EgoReAct是首个能够从以自我为中心的视频流中实时生成3D对齐人类反应动作的自回归框架，通过构建HRD数据集和创新的生成架构，有效解决了空间对齐和因果生成的挑战。
...
TL;DR: SCaR-3D是一个3D场景变化检测框架，通过基于符号距离的2D差分和多视图聚合，从密集视图的预变化图像序列和稀疏视图的后变化图像中识别物体级变化，并支持持续场景重建。


<details>
  <summary>Details</summary>
Motivation: 现有3D变化检测方法存在空间不一致性，且无法明确分离变化前后的状态，需要更精确的物体级变化检测方法。

Method: 采用基于符号距离的2D差分模块，结合多视图聚合（投票和剪枝），利用3DGS的一致性特性来鲁棒地分离变化前后状态，并开发了选择性更新动态区域的持续重建策略。

Result: 方法在实验中表现出高精度和高效率，优于现有方法，并贡献了CCS3D合成数据集支持可控评估。

Conclusion: SCaR-3D框架有效解决了3D变化检测中的空间不一致问题，能够明确分离变化前后状态，为场景监控和持续重建提供了有效解决方案。
...
TL;DR: 提出了一种基于两个仿射对应和已知垂直方向（来自IMU）来估计相对姿态和焦距的新方法，将问题从5自由度简化为3自由度，使用多项式特征值方法求解。


<details>
  <summary>Details</summary>
Motivation: 在自动驾驶汽车、智能手机和无人机等应用中，相机通常与惯性测量单元（IMU）结合使用。IMU可以提供相机视图的垂直方向，从而将相对姿态估计从5自由度减少到3自由度，简化了问题。现有方法在处理未知焦距和相对姿态估计时存在局限性，需要更高效的求解器。

Method: 首先，在已知垂直方向的情况下，从两个仿射对应建立约束方程。然后，基于具有非平凡解方程组的性质，推导出仅涉及两个参数（焦距和相对旋转角度）的四个方程。最后，利用多项式特征值方法求解焦距和相对旋转角度问题。

Result: 在合成和真实世界数据集上的评估表明，提出的求解器性能优于现有的最先进求解器。

Conclusion: 该方法通过结合IMU提供的垂直方向信息，将相对姿态估计问题简化，并利用两个仿射对应和多项式特征值方法有效求解未知焦距和3自由度相对姿态，在性能上超越了现有方法。
...
<details>
  <summary>Details</summary>
Motivation: 现有HOI视频生成方法存在两个关键限制：1)缺乏有效的多视角信息注入机制，导致跨视角一致性差；2)严重依赖细粒度手部网格标注来建模交互遮挡。这些问题限制了HOI视频生成的质量和应用范围。

Method: 提出ByteLoom框架：1)采用RCM-cache机制，利用相对坐标图(RCM)作为通用表示来保持物体几何一致性并精确控制6自由度物体变换；2)设计渐进式训练课程，增强模型能力并减少对手部网格的需求；3)使用简化的人类条件和3D物体输入。

Result: 实验表明，该方法能够忠实保持人类身份和物体的多视角几何特性，同时维持平滑的运动和物体操控，在HOI视频生成质量上表现出色。

Conclusion: ByteLoom通过创新的RCM表示和渐进式训练策略，成功解决了HOI视频生成中的多视角一致性和标注依赖问题，为数字人类、电子商务、广告和机器人模仿学习等应用提供了高质量的解决方案。
...
Result: 在MUSON上对多个最先进的小型视觉语言模型进行基准测试，Qwen2.5-VL-3B达到最高的决策准确率0.8625。与SNEI相比，MUSON提供了一致的推理、动作和解释。

Conclusion: MUSON作为一个有效且可复用的社交合规导航基准数据集，通过结构化推理监督和平衡的动作空间，能够提升模型学习安全关键行为的能力。
...
TL;DR: Lamps提出了一种基于多视角解剖学自监督学习的医学影像基础模型，利用人体解剖学的一致性、连贯性和层次性作为监督信号，在胸部X光片上预训练，展现出优异的鲁棒性、可迁移性和临床潜力。


<details>
  <summary>Details</summary>
Motivation: 现有自监督学习方法在医学影像中往往忽视了人体解剖学这一关键基础，而医学影像直接反映了人体内部结构，包含解剖学的一致性、连贯性和层次性特征。这些方法未能有效学习解剖学特征，限制了医学影像基础模型的发展。

Method: Lamps通过自监督学习从多个视角学习解剖学特征，利用人体解剖学的一致性、连贯性和层次性作为监督信号，在大规模胸部X光片数据集上进行预训练。

Result: 在10个数据集上通过微调和涌现特性分析进行广泛实验，Lamps相比10个基线模型展现出优异的鲁棒性、可迁移性和临床潜力。

Conclusion: 通过从多视角学习解剖学特征，Lamps为医学影像基础模型提供了独特机会，能够开发出与人体解剖结构对齐的有意义且鲁棒的表征。
...
TL;DR: 本文提出了一种数据导向的方法来缓解深度学习模型中的虚假相关性，通过识别、中和、消除和更新四个步骤构建了一个有效的去偏流程，在图像和NLP去偏基准测试中相比标准经验风险最小化（ERM）将最差组准确率提高了20%以上。
...
Method: 提出一个包含四个步骤的流程：1）识别：观察到受虚假特征影响的样本在学习的特征空间中倾向于呈现分散分布，从而识别虚假特征的存在；2）中和：基于简单的分组策略中和虚假特征，获得偏置不变表示；3）消除：学习特征变换以消除虚假特征，与偏置不变表示对齐；4）更新：结合学习到的特征变换更新分类器，获得无偏模型。

Result: 在图像和NLP去偏基准测试中，相比标准经验风险最小化（ERM），最差组准确率提高了20%以上，显示出显著的去偏效果。

Conclusion: 通过整合识别、中和、消除和更新四个步骤，构建了一个有效的缓解虚假相关性的流程。该方法不需要标注潜在的虚假属性，而是基于数据本身的特点来识别和处理虚假特征，在多个基准测试中取得了显著改进。
...
TL;DR: 该论文提出了M-ErasureBench多模态评估框架，用于系统评估概念擦除方法在文本提示、学习嵌入和反转潜在空间三种输入模态下的性能，并开发了IRECE模块来增强概念擦除的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 现有概念擦除方法主要关注文本提示，忽略了图像编辑和个性化生成等实际应用中越来越重要的其他输入模态（如学习嵌入和反转潜在空间），这些模态可能成为攻击面，导致已擦除的概念重新出现。
...
Result: 现有方法在文本提示下表现良好，但在学习嵌入和反转潜在空间下大多失败，白盒设置下的概念再现率超过90%。IRECE模块显著提升了鲁棒性，在最具挑战性的白盒潜在反转场景下将概念再现率降低了40%，同时保持了视觉质量。
...
<details>
  <summary>Details</summary>
Motivation: 基于去噪-反演循环的迭代精炼方法在提升扩散模型质量和控制方面很有效，但当与标准无分类器引导结合时，其效果受到严重限制。研究发现CFG的外推性质会系统性地将采样路径推离数据流形，导致近似误差发散并破坏精炼过程。

Method: 提出引导路径采样(GPS)新范式，用原则性的流形约束插值替代不稳定的外推，确保采样路径保持在数据流形上。理论上证明这种校正能将误差序列从无界放大转变为严格有界。同时设计最优调度策略，动态调整引导强度，使语义注入与模型从粗到细的生成过程对齐。

Result: 在SDXL和Hunyuan-DiT等现代骨干模型上的实验表明，GPS在感知质量和复杂提示遵循方面均优于现有方法。例如，在SDXL上GPS获得0.79的ImageReward和0.2995的HPS v2分数，在GenEval上的整体语义对齐准确率提升至57.45%。

Conclusion: 路径稳定性是有效迭代精炼的前提条件，GPS提供了一个稳健的框架来实现这一目标。该方法通过流形约束插值替代CFG的不稳定外推，确保采样路径保持在数据流形上，从而显著提升扩散模型的生成质量和语义对齐能力。
...
Method: 提出哈希网格特征剪枝方法，基于输入高斯溅射的坐标识别和剪除无效特征，只对有效特征进行编码，从而减少哈希网格的存储大小。

Result: 该方法在保持模型性能的同时，按照标准化委员会定义的通用测试条件，相比基线方法实现了平均8%的比特率降低。

Conclusion: 通过剪除哈希网格中的无效特征，可以有效减少存储和传输开销，提升率失真性能，为高斯溅射压缩提供了更高效的解决方案。
...
TL;DR: JavisGPT是首个用于联合音频-视频理解与生成的多模态大语言模型，采用编码器-LLM-解码器架构，通过三阶段训练流程在音频-视频理解与生成任务上表现出色。


<details>
  <summary>Details</summary>
Motivation: 当前缺乏能够同时处理音频-视频联合理解与生成任务的统一多模态大语言模型，特别是在需要时空同步的复杂场景中。

Method: 采用编码器-LLM-解码器架构，包含SyncFusion模块进行时空音频-视频融合，使用同步感知可学习查询桥接预训练的JAV-DiT生成器。设计三阶段训练流程：多模态预训练、音频-视频微调、大规模指令调优。

Result: 在音频-视频理解与生成基准测试中表现优于现有MLLMs，特别是在复杂和时空同步设置中。构建了包含20万+GPT-4o策划对话的JavisInst-Omni高质量指令数据集。

Conclusion: JavisGPT是首个统一的音频-视频多模态大语言模型，能够实现时间一致的视频-音频理解与生成，在复杂多模态任务中展现出优越性能。
...
TL;DR: ColaVLA是一个统一的视觉-语言-动作框架，通过将推理从文本转移到统一潜在空间，并耦合分层并行轨迹解码器，解决了VLM规划器在自动驾驶中的三个关键挑战：离散文本推理与连续控制不匹配、自回归链式思维解码延迟高、以及非因果规划器限制实时部署的问题。


<details>
  <summary>Details</summary>
Motivation: 自动驾驶需要从复杂的多模态输入中生成安全可靠的轨迹。虽然视觉语言模型（VLMs）通过引入跨模态先验和常识推理丰富了端到端系统，但当前基于VLM的规划器面临三个关键挑战：离散文本推理与连续控制不匹配、自回归链式思维解码的高延迟、以及低效或非因果规划器限制实时部署。

Method: ColaVLA包含两个核心组件：1) 认知潜在推理器（Cognitive Latent Reasoner），通过自我适应选择和仅两次VLM前向传递，将场景理解压缩为紧凑的、面向决策的元动作嵌入；2) 分层并行规划器（Hierarchical Parallel Planner），在单次前向传递中生成多尺度、因果一致的轨迹。
...
TL;DR: CLIP-Joint-Detect是一个简单且与检测器无关的框架，通过端到端联合训练集成CLIP风格的对比视觉语言监督，在保持实时推理速度的同时显著提升检测性能。
...
Method: 提出一个轻量级并行头，将区域或网格特征投影到CLIP嵌入空间，并通过InfoNCE对比损失和辅助交叉熵项与可学习的类别特定文本嵌入对齐，同时优化所有标准检测损失。该方法适用于两阶段和一阶段架构。

Result: 在Pascal VOC 2007+2012上使用Faster R-CNN，以及在MS COCO 2017基准上使用现代YOLO检测器（YOLOv11）进行验证，均获得了一致且显著的性能提升，同时保持了实时推理速度。

Conclusion: 通过可学习文本嵌入的联合优化显著增强了不同架构和数据集上的闭集检测性能，证明了CLIP风格对比监督的有效性。
...
TL;DR: YOLO-IOD：首个基于YOLO的实时增量目标检测框架，解决YOLO在增量学习中的三大知识冲突问题，在传统和更严格的LoCo COCO基准上均表现优异。


<details>
  <summary>Details</summary>
Motivation: 现有增量目标检测方法主要基于Faster R-CNN或DETR系列检测器，无法支持实时YOLO检测框架。需要解决YOLO在增量学习中面临的三大知识冲突问题：前景-背景混淆、参数干扰和知识蒸馏错位。

Method: 基于预训练的YOLO-World模型，通过分阶段参数高效微调实现增量学习。包含三个核心组件：1) 冲突感知伪标签细化(CPR)，利用伪标签置信度缓解前景-背景混淆；2) 重要性核选择(IKS)，识别并更新当前任务相关的关键卷积核；3) 跨阶段非对称知识蒸馏(CAKD)，通过新旧教师检测头的特征传递解决知识蒸馏错位问题。

Result: 在传统基准和新提出的LoCo COCO基准（消除阶段间数据泄漏）上的实验表明，YOLO-IOD实现了卓越性能，遗忘程度最小。

Conclusion: YOLO-IOD是首个支持实时YOLO框架的增量目标检测方法，有效解决了YOLO在增量学习中的三大知识冲突问题，在更严格的评估基准上表现出色。
...
TL;DR: ReamCamo是一个基于外绘制的统一框架，通过布局控制和多模态条件生成更逼真的伪装图像，解决了现有方法在视觉相似性和语义一致性方面的不足。


<details>
  <summary>Details</summary>
Motivation: 现有伪装图像生成方法存在明显缺陷：生成的图像要么因视觉相似性不足而缺乏足够伪装效果，要么背景杂乱且与前景目标语义不一致。这导致生成的训练数据与真实伪装图像存在显著差距，影响伪装目标检测模型的性能。

Method: 提出ReamCamo框架：1）引入布局控制来调节全局图像结构，改善前景对象与生成背景的语义一致性；2）构建多模态文本-视觉条件，结合统一细粒度文本任务描述和面向纹理的背景检索，共同指导生成过程以增强视觉保真度和真实感；3）提出背景-前景分布差异度量来定量评估伪装质量。

Result: 大量实验和可视化结果表明，ReamCamo框架在生成逼真伪装图像方面具有显著效果，能够有效缩小与真实伪装图像的差距。

Conclusion: ReamCamo通过布局控制和多模态条件引导，成功解决了现有伪装图像生成方法在视觉相似性和语义一致性方面的局限性，为伪装目标检测提供了更高质量的训练数据生成方案。
...
TL;DR: PoseStreamer：针对高速移动场景的鲁棒多模态6DoF姿态估计框架，通过自适应姿态记忆队列、物体中心2D跟踪器和光线姿态滤波器解决事件相机在高速场景中的姿态估计问题。


<details>
  <summary>Details</summary>
Motivation: 在高速和低光场景下，传统RGB相机因运动模糊导致6DoF姿态估计性能下降，而事件相机虽然具有高时间分辨率，但现有方法在高速物体移动场景中表现不佳。

Method: 提出PoseStreamer框架，包含三个核心组件：1）自适应姿态记忆队列，利用历史方向线索保持时间一致性；2）物体中心2D跟踪器，提供强2D先验以提升3D中心召回；3）光线姿态滤波器，沿相机光线进行几何细化。同时构建了MoCapCube6D多模态数据集用于快速运动下的性能评估。

Result: 实验表明PoseStreamer在高速移动场景中实现了卓越的准确性，同时作为无模板框架对未见过的移动物体表现出强大的泛化能力。

Conclusion: PoseStreamer为解决高速场景下的6DoF姿态估计问题提供了有效的多模态解决方案，通过创新的时间一致性和几何细化机制显著提升了性能。
...
<details>
  <summary>Details</summary>
Motivation: 当前文本引导的医学图像分割方法存在两个关键瓶颈：1）难以同时处理诊断性和描述性文本，导致难以识别病灶并与图像区域建立关联；2）现有方法主要关注病灶描述，未能捕捉位置约束，导致关键偏差（如"左下肺"可能错误覆盖双肺）。
...
Conclusion: SSA框架通过增强处理包含位置、描述和诊断信息的混合医学文本的能力，有效解决了当前方法的局限性，为空间约束下的精确医学图像分割提供了有效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有的基于提示的方法在移除或修改身份特征时，要么依赖预训练模型已充分表示的主体，要么需要对特定身份进行模型微调，缺乏通用性和控制性。
...
TL;DR: 提出一个基于低成本RGB无人机的统一果园智能管道，集成ResNet50进行叶片病害检测、VGG16进行苹果新鲜度评估、YOLOv8进行实时苹果检测定位，在ESP32-CAM和树莓派上实现完全离线推理。


<details>
  <summary>Details</summary>
Motivation: 果园需要及时的病害检测、果实质量评估和产量估计，但现有无人机系统通常孤立处理这些任务且依赖昂贵的多光谱传感器。

Method: 构建统一低成本RGB无人机管道，结合ResNet50（叶片病害检测）、VGG16（苹果新鲜度确定）和YOLOv8（实时苹果检测定位），在ESP32-CAM和树莓派硬件上实现完全离线推理。

Result: 实验显示叶片病害分类准确率98.9%，新鲜度分类准确率97.4%，苹果检测F1分数0.857，性能优异。
...
TL;DR: OpenGround是一个用于开放世界3D视觉定位的零样本框架，通过主动认知推理模块动态扩展视觉语言模型的认知范围，无需预定义目标类别，在开放世界场景中表现优异。
...
Method: 提出OpenGround框架，核心是主动认知推理模块，通过认知任务链模拟人类感知目标的过程，主动推理上下文相关对象，动态更新对象查找表，扩展视觉语言模型的认知范围。

Result: 在Nr3D数据集上取得竞争性性能，在ScanRefer数据集上达到最先进水平，在新建的OpenTarget数据集上实现了17.6%的显著提升。

Conclusion: OpenGround通过主动认知推理模块克服了预定义对象查找表的限制，实现了开放世界3D视觉定位，既能处理预定义类别也能处理开放世界类别，具有广泛的应用潜力。
...
<details>
  <summary>Details</summary>
Motivation: 人类能够利用丰富的场景上下文信息（空间关系、材质属性、物体共现）轻松识别物体，而大多数计算物体识别系统仅处理孤立的图像区域，忽略了这些关键的上下文信息。

Method: 构建地理语义上下文图(GSCG)：从单目图像中整合度量深度估计器和统一的全景与材质分割模型，将物体编码为具有几何、颜色和材质属性的节点，空间关系编码为边。然后提出专门的基于图的分类器，聚合目标物体、其邻近物体和全局场景上下文的特征来预测类别。

Result: 上下文感知模型达到73.4%的分类准确率，显著优于上下文无关版本（最低38.4%）。GSCG方法大幅超越强基线，包括微调的ResNet模型（最高53.5%）和最先进的多模态大语言模型Llama 4 Scout（最高42.3%）。

Conclusion: 在COCO 2017数据集上的结果表明，显式结构化和可解释的上下文对于物体识别任务具有显著优势，证明了上下文信息在计算物体识别中的关键作用。
...
Conclusion: Co2S通过融合视觉语言模型和自监督模型的先验知识，有效缓解了伪标签漂移问题，实现了稳定且高性能的半监督遥感图像语义分割。
...
Conclusion: PathoSyn提供了一个数学原理的管道，用于生成高保真合成数据，支持可解释的反事实疾病进展建模，促进低数据环境下稳健诊断算法的发展，并为临床决策支持系统提供基准测试环境。
...
TL;DR: GeoTeacher：通过几何关系监督和体素数据增强提升半监督3D目标检测性能，在有限标注数据下增强模型对物体几何结构的理解能力


<details>
  <summary>Details</summary>
Motivation: 现有半监督3D目标检测方法主要关注伪标签质量和特征一致性，但忽视了模型在有限标注数据下对物体几何结构的敏感性不足问题，这限制了模型对物体几何信息的捕捉能力

Method: 1）基于关键点的几何关系监督模块，将教师模型对物体几何的知识传递给学生模型；2）体素级数据增强策略增加物体几何多样性，并引入距离衰减机制保护远距离物体的完整性；3）可与不同SS3D方法结合

Result: 在ONCE和Waymo数据集上的大量实验证明了方法的有效性和泛化能力，取得了新的最先进结果

Conclusion: GeoTeacher通过几何关系监督和数据增强有效提升了学生模型在有限训练数据下对物体几何结构的理解能力，显著提高了半监督3D目标检测性能
...
<details>
  <summary>Details</summary>
Motivation: 现有文本到图像模型的对齐评估方法大多依赖粗粒度指标或静态QA流程，缺乏细粒度可解释性且难以反映人类偏好，需要更精细、可解释的评估框架。

Method: 采用结构化"定位-推理-结论"范式，利用多模态大语言模型显式定位语义元素并得出可解释的对齐判断；通过GRPO优化模型，使用包含结构格式、定位准确性和对齐保真度的复合奖励函数。

Result: 在四个基准测试（EvalMuse-40K、RichHF、MHaluBench、GenAI-Bench）上实现最先进性能，一致优于强大的专有模型和监督基线，同时相比现有迭代视觉推理方法具有更优的推理效率。
...
TL;DR: GVSynergy-Det是一个基于图像的3D物体检测框架，通过高斯-体素协同表示学习，无需深度传感器或密集3D监督，在室内基准测试中达到最先进性能。


<details>
  <summary>Details</summary>
Motivation: 当前基于图像的3D物体检测方法面临两难困境：高精度方法需要密集3D监督，而无监督方法难以从图像中提取准确几何信息。现有方法要么依赖耗时的逐场景优化，要么仅将高斯表示用于深度正则化。

Method: 提出双表示架构：1) 采用可泛化的高斯溅射提取互补几何特征；2) 开发跨表示增强机制，用高斯场的几何细节丰富体素特征。通过可学习的集成直接利用两种表示的特征。

Result: 在ScanNetV2和ARKitScenes数据集上实现了最先进的结果，显著优于现有方法，且无需任何深度或密集3D几何监督（如点云或TSDF）。

Conclusion: 高斯和体素表示捕捉互补的几何信息，通过协同表示学习可以有效提升基于图像的3D检测性能，证明了该框架在无监督条件下的有效性。
...
TL;DR: 提出基于3D高斯场景表示的驾驶世界模型框架，实现3D场景理解和多模态场景生成，通过语言特征嵌入高斯基元实现早期模态对齐，并设计任务感知语言引导采样策略和双条件多模态生成模型。
...
Conclusion: 提出的基于3D高斯场景表示的驾驶世界模型框架成功解决了现有方法在3D场景理解和多模态对齐方面的局限性，实现了更好的场景理解和生成能力。
...
TL;DR: 提出ForCM方法，结合基于对象的图像分析（OBIA）与深度学习（DL），利用Sentinel-2影像进行森林覆盖制图，在亚马逊雨林区域测试多种DL模型，与传统OBIA方法相比显著提升了制图精度。


<details>
  <summary>Details</summary>
Motivation: 传统森林覆盖制图方法精度有限，需要开发更准确的方法来支持全球环境监测和保护工作。研究旨在探索深度学习与OBIA结合的新方法，利用免费易用的工具实现高精度森林覆盖制图。

Method: 提出ForCM方法，结合OBIA与深度学习。使用Sentinel-2 Level 2A高分辨率卫星影像，测试UNet、UNet++、ResUNet、AttentionUNet和ResNet50-Segnet等多种DL模型。采用三个数据集（两个三波段和一个四波段影像）。将最有效的DL模型与OBIA技术集成，并与传统OBIA方法对比。使用QGIS等免费工具进行实现。

Result: ForCM方法显著提升了森林覆盖制图精度：ResUNet-OBIA达到94.54%总体精度，AttentionUNet-OBIA达到95.64%总体精度，而传统OBIA方法仅为92.91%。证明了深度学习与OBIA结合的有效性，以及免费工具在精度制图中的潜力。

Conclusion: ForCM方法通过结合深度学习与OBIA技术，显著提高了森林覆盖制图精度，为全球环境监测和保护提供了有效工具。研究证明了免费易用工具在专业制图中的可行性，支持大规模环境监测应用。
...
<details>
  <summary>Details</summary>
Motivation: 当前扩散模型应用中，时间步特征的选择主要依赖经验直觉，这往往导致次优性能且偏向某些特定任务。为了缓解这一限制，作者研究了不同时间步特征的重要性，旨在自适应选择最适合少样本密集预测任务的时间步特征。

Method: 提出了两个核心模块：1) 任务感知时间步选择(TTS)：基于时间步损失和相似度分数选择理想的时间步；2) 时间步特征整合(TFC)：整合选定的时间步特征以提升少样本密集预测性能。同时配合参数高效的微调适配器。
...
TL;DR: MM-UAVBench：首个针对低空无人机场景的多模态大语言模型综合基准，包含感知、认知、规划三个维度19个子任务，超过5.7K人工标注问题，揭示当前MLLMs在低空场景的适应性问题。
...
Method: 提出了MM-UAVBench基准，系统评估MLLMs在低空无人机场景中的三个核心能力维度：感知、认知和规划。基准包含19个子任务，超过5.7K个从公开无人机数据集收集的真实数据中人工标注的问题。

Result: 对16个开源和专有MLLMs的广泛实验表明，当前模型难以适应低空场景的复杂视觉和认知需求。分析进一步揭示了空间偏差和多视角理解等关键瓶颈，这些因素阻碍了MLLMs在无人机场景中的有效部署。

Conclusion: MM-UAVBench基准将促进未来针对现实世界无人机智能的稳健可靠MLLMs研究，填补了低空场景MLLM评估的空白。
...
<details>
  <summary>Details</summary>
Motivation: 时尚物品检测面临挑战，因为时尚物品外观高度多样化且子类别间相似性高，导致检测歧义。传统检测器独立检测每个物品，忽略了时尚物品之间存在的有意义关系（如搭配风格）。

Method: 提出Holi-DETR，在DETR及其后续模型基础上，整合三种异构上下文信息：1) 时尚物品间的共现概率关系；2) 基于物品间空间排列的相对位置和大小；3) 物品与人体关键点的空间关系。

Result: 实验表明，该方法相比原始DETR提升了3.6个百分点（pp）的平均精度（AP），相比最近开发的Co-DETR提升了1.1 pp。
...
Method: 提出一个新颖框架：1) 使用预训练的文本引导图像到图像翻译模型生成合成缺陷图像；2) 利用图像检索模型评估生成图像与真实正常图像的相似性，过滤不相关输出以提升质量；3) 采用两阶段训练策略：先在大量基于规则合成的图像上进行预训练，然后在少量高质量图像上进行微调。

Result: 在MVTec AD数据集上的实验证明了该方法的有效性。该方法显著降低了数据收集成本，同时提高了异常检测性能。

Conclusion: 提出的框架通过结合预训练翻译模型和图像检索模型，实现了高效生成高质量合成缺陷图像，并通过两阶段训练策略有效利用这些图像，在降低数据收集成本的同时提升了异常检测性能，为解决工业制造中缺陷图像稀缺问题提供了有效方案。
...
Result: 实验表明，即使在剪枝85%的训练数据后，该方法仍能显著改善收敛和生成质量。使用该方法训练的扩散基础模型在下游任务（如超分辨率和语义图像合成）中持续取得最先进性能。
...
<details>
  <summary>Details</summary>
Motivation: 现有遥感图像多模态融合方法存在两个主要问题：1）固定分辨率无法平衡计算效率与细节保留；2）单尺度对齐缺乏语义层次性，导致语义错配和粒度失衡。这些问题限制了遥感图像信息提取的准确性和应用价值。

Method: 提出视觉语言模型框架，包含两个关键技术：1）动态分辨率输入策略（DRIS），采用粗到细的方法根据图像内容复杂度自适应分配计算资源；2）多尺度视觉语言对齐机制（MS-VLAM），构建对象、局部区域和全局三个层次的对齐机制，系统捕获跨模态语义一致性。

Result: 在RS-GPT4V数据集上的实验表明，该框架在图像描述和跨模态检索任务中显著提升了语义理解精度和计算效率。相比传统方法，在图像描述的BLEU-4、CIDEr指标和跨模态检索的R@10指标上均取得优越性能。
...
<details>
  <summary>Details</summary>
Motivation: 现有的文本到图像扩散模型在生成图像序列时，难以在保持角色身份一致性和确保每张图像与文本提示对齐之间取得平衡。当前方法往往在这两个目标之间需要做出权衡。

Method: 1) 选择性文本嵌入修改，实现对角色身份的显式语义控制；2) 基于FLUX中填充嵌入的分析，重新利用填充嵌入作为语义容器；3) 自适应特征共享策略，自动评估文本模糊性并仅对模糊的身份提示应用约束；4) 提出一致性质量评分(CQS)作为统一评估协议。

Result: 该框架实现了最先进的性能，有效克服了先前方法在身份一致性和提示对齐之间的权衡问题。

Conclusion: ASemconsist通过创新的语义控制策略和自适应特征共享，成功解决了文本到图像生成中的角色身份一致性问题，为序列图像生成提供了更平衡和有效的解决方案。
...
TL;DR: 提出一种轮廓信息感知的2D高斯泼溅框架，通过结合对象分割先验来改进图像表示，在少量高斯元素下仍能保持清晰的边缘结构。


<details>
  <summary>Details</summary>
Motivation: 现有2D高斯泼溅方法在少量高斯元素时会产生模糊或边界不清晰的问题，缺乏轮廓感知能力，需要改进边缘保持性能。

Method: 提出轮廓信息感知的2D高斯泼溅框架，将对象分割先验融入高斯表示，通过限制每个高斯在特定分割区域内进行光栅化，防止跨边界混合；同时引入预热方案稳定训练。

Result: 在合成色卡和DAVIS数据集上的实验表明，该方法在对象边缘区域获得更高的重建质量，特别是在高斯数量很少的情况下，同时保持快速渲染和低内存使用。

Conclusion: 通过结合分割先验，提出的轮廓感知2D高斯泼溅框架显著改善了边缘保持能力，在压缩场景下实现了更好的视觉质量，同时保持了原有方法的效率和紧凑性优势。
...
<details>
  <summary>Details</summary>
Motivation: 扩散Transformer（DiT）在图像和视频生成中占主导地位，但其迭代去噪过程导致推理速度慢，限制了广泛应用。现有的基于缓存的训练免费加速方法存在较大的计算误差，且固定缓存策略无法适应去噪过程中复杂的误差变化

Method: 提出CEM（累积误差最小化）方法：1）预定义误差来表征模型对加速的敏感性（受时间步和缓存间隔共同影响）；2）基于此先验，设计带累积误差近似的动态规划算法进行策略优化；3）实现缓存误差最小化，显著提升生成保真度

Result: 在9个生成模型和量化方法上跨3个任务进行实验，CEM显著提升了现有加速模型的生成保真度，在FLUX.1-dev、PixArt-α、StableDiffusion1.5和Hunyuan等模型上甚至超越了原始生成性能

Conclusion: CEM是一种模型无关、泛化能力强的保真度优化插件，可无缝集成到现有误差校正框架和量化模型中，无需额外计算开销，能适应任意加速预算
...
TL;DR: 该论文提出了两个多模态框架，分别用于微手势识别和行为情感预测，在iMiGUE数据集上验证了方法的有效性，在MiGA 2025挑战赛的行为情感预测任务中获得第二名。
...
Conclusion: 提出的多模态框架能够有效处理微手势识别和行为情感预测任务，通过融合不同模态的信息（视频、骨骼、面部、上下文）来捕捉细微的人类行为模式，在相关任务中取得了优异的性能。
...
<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型为医学影像分析提供了新范式，本研究旨在比较专业开源模型MedGemma与专有大型多模态模型GPT-4在疾病诊断中的性能差异，评估领域特定微调对临床实施的重要性。

Method: 使用LoRA（低秩适应）对MedGemma-4b-it模型进行微调，与未经调优的GPT-4进行对比，评估六种不同疾病的诊断能力。通过混淆矩阵和分类报告进行定量分析。

Result: MedGemma-4b-it平均测试准确率达到80.37%，显著高于GPT-4的69.58%。在癌症和肺炎等高风险临床任务中表现出更高的敏感性，减少了幻觉现象。

Conclusion: 领域特定的微调对于临床实施中最小化幻觉至关重要，MedGemma作为复杂、基于证据的医学推理工具具有优势，专业开源模型在医学诊断任务中可能优于通用大型多模态模型。
...
<details>
  <summary>Details</summary>
Motivation: 传统CAD建模复杂，现有从草图重建3D模型的方法生成不可编辑的近似模型，无法满足工业设计对精度和可编辑性的严格要求。基于文本或图像输入的方法需要大量人工标注，限制了在工业环境中的可扩展性和适用性。

Method: 提出异构协作多专家强化学习（CME-CAD）范式，包含两阶段训练：多专家微调（MEFT）和多专家强化学习（MERL）。整合不同模型的互补优势，促进协作学习，提高生成准确、约束兼容、完全可编辑CAD模型的能力。

Result: 提出CADExpert开源基准数据集，包含17,299个实例，包括带有精确尺寸标注的正交投影图、专家生成的思维链过程、可执行的CADQuery代码和渲染的3D模型。

Conclusion: CME-CAD范式通过多专家协作强化学习解决了CAD代码生成的挑战，能够生成满足工业设计要求的可编辑、高精度CAD模型，并提供了大规模基准数据集支持该领域研究。
...
TL;DR: 该论文从拓扑结构角度研究视觉表征学习，提出视觉理解需要语义语言，视觉空间具有纤维丛结构，语义不变性需要非同胚的判别性目标，语义抽象需要支持拓扑变化的表征机制。


<details>
  <summary>Details</summary>
Motivation: 从结构和拓扑角度理解视觉表征学习，提出视觉理解需要语义语言的基本假设，探索视觉观察空间的组织结构及其对表征学习的影响。

Method: 基于纤维丛结构理论框架，分析视觉观察空间的拓扑组织，推导语义不变性和语义抽象的理论要求，将理论结果与大规模判别性和多模态模型的经验规律对齐。

Result: 发现语义商空间不是子流形，无法通过光滑变形获得，需要标签监督、跨实例识别或多模态对齐等判别性目标；语义抽象需要支持拓扑变化的表征机制，即"扩展-捕捉"过程。

Conclusion: 该框架提供了理解视觉表征学习的拓扑视角，与大规模模型的经验规律和统计学习理论的经典原理相一致，强调这些结果是解释性的而非规定性的。
...
TL;DR: 该论文提出了SpatialMosaic数据集和SpatialMosaicVLM框架，用于解决多模态大语言模型在真实世界3D场景理解中面临的遮挡、部分可见性等挑战。


<details>
  <summary>Details</summary>
Motivation: 现有MLLMs依赖预构建的3D表示或现成重建流程，限制了可扩展性和真实世界适用性。虽然已有工作探索从多视角图像直接学习空间推理，但真实环境中常见的部分可见性、遮挡、低重叠度等挑战仍未充分研究。

Method: 提出了可扩展的多视角数据生成和标注流程，构建了包含200万QA对的SpatialMosaic指令调优数据集；开发了包含100万QA对、涵盖6个任务的SpatialMosaic-Bench基准；提出了SpatialMosaicVLM混合框架，将3D重建模型作为几何编码器集成到VLMs中。

Result: 实验表明，提出的数据集和VQA任务能有效增强在挑战性多视角条件下的空间推理能力，验证了数据生成流程在构建真实多样QA对方面的有效性。

Conclusion: 通过构建大规模多视角空间推理数据集和集成3D几何编码的混合框架，显著提升了VLMs在真实世界复杂场景中的空间理解能力，为无需显式3D重建的空间推理提供了有效解决方案。
...
TL;DR: NeXT-IMDL是一个大规模诊断基准，旨在系统性地探究当前图像篡改检测模型的泛化边界，揭示现有方法在处理多样化AI生成内容时的脆弱性。
...
Method: 提出NeXT-IMDL基准，将AIGC篡改沿四个基本轴分类：编辑模型、篡改类型、内容语义和伪造粒度。基于此构建五个严格的跨维度评估协议，对11个代表性模型进行广泛实验。

Result: 实验揭示了一个关键发现：虽然这些模型在原始设置下表现良好，但在模拟真实世界各种泛化场景的设计协议下，它们表现出系统性失败和显著的性能下降。

Conclusion: 通过提供这一诊断工具包和新发现，旨在推动开发真正鲁棒的下一代图像篡改检测与定位模型。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法在实时音频驱动数字人生成中存在计算负载与延迟约束的冲突，通常通过单向注意力或降低模型容量来妥协视觉质量，需要一种既能保持高保真度又能满足实时性要求的解决方案。

Method: 1. 自校正双向蒸馏策略：在视频块内保留双向注意力，保持关键时空相关性；2. 多步回顾自校正机制：使模型能从累积错误中自主恢复，防止崩溃；3. 全栈推理加速套件：包括混合序列并行、并行VAE和内核级优化。

Result: SoulX-LiveTalk是首个14B规模系统，实现了亚秒级启动延迟（0.87秒）和32FPS实时吞吐率，为高保真交互式数字人合成设立了新标准。

Conclusion: 该框架成功解决了大规模扩散模型在实时音频驱动数字人生成中的工程挑战，通过创新的双向注意力保留和自校正机制，在保持视觉质量的同时满足了严格的实时性要求。
...
TL;DR: SOFTooth：一种利用冻结2D语义增强的2D-3D融合框架，用于三维牙齿实例分割，无需2D掩码监督，在包含第三磨牙的复杂病例上表现优异。


<details>
  <summary>Details</summary>
Motivation: 三维牙齿实例分割面临拥挤牙弓、模糊的牙齿-牙龈边界、缺失牙齿和罕见但临床重要的第三磨牙等挑战。现有3D方法依赖几何线索易出现边界泄漏、中心漂移和牙齿身份不一致问题，而2D基础模型（如SAM）虽提供强边界感知语义，但直接应用于3D临床工作流程不切实际。

Method: 提出SOFTooth框架：1）点级残差门控模块将咬合视图SAM嵌入注入3D点特征以细化牙齿-牙龈和牙齿间边界；2）中心引导掩码细化正则化实例掩码与几何质心间的一致性，减少中心漂移；3）顺序感知匈牙利匹配策略将解剖牙齿顺序和中心距离整合到基于相似度的分配中，确保在缺失或拥挤牙列下的连贯标记。

Result: 在3DTeethSeg'22数据集上，SOFTooth实现了最先进的整体准确率和平均IoU，在涉及第三磨牙的病例上获得明显提升，表明丰富的2D语义可以在无需2D微调的情况下有效转移到3D牙齿实例分割。

Conclusion: SOFTooth通过语义增强、顺序感知的2D-3D融合框架，成功解决了三维牙齿实例分割中的边界模糊、中心漂移和身份不一致问题，特别在复杂解剖结构和罕见牙齿类别上表现优异，证明了冻结2D语义在3D分割任务中的有效迁移能力。
...
TL;DR: 论文提出RAD数据集和ArtQuant框架解决AIGC美学质量评估中的数据稀缺与模型碎片化问题，通过大规模多维度数据集和LLM解码器实现更准确的美学评估。


<details>
  <summary>Details</summary>
Motivation: 美学质量评估对AIGC的人对齐定量评估系统至关重要，但面临两个核心挑战：1) 数据稀缺和不平衡，现有数据集过度关注视觉感知而忽视更深维度；2) 模型碎片化，现有方法要么隔离美学属性，要么难以有效处理长文本描述。

Method: 1) 提出RAD数据集：大规模(70k)、多维度结构化数据集，通过迭代流水线生成，无需昂贵人工标注且易于扩展；2) 提出ArtQuant框架：通过联合描述生成耦合孤立美学维度，利用LLM解码器更好建模长文本语义。

Result: 方法在多个数据集上达到最先进性能，仅需传统训练轮数的33%，缩小了艺术图像与美学判断之间的认知差距。理论分析证实RAD的语义充分性和生成范式共同最小化预测熵。

Conclusion: RAD数据集和ArtQuant框架有效解决了美学评估中的数据稀缺和模型碎片化问题，为AIGC的美学质量评估提供了数学基础和实用解决方案，将促进未来研究发展。
...
<details>
  <summary>Details</summary>
Motivation: 当前自动驾驶中的世界模型方法将世界预测和运动规划作为解耦过程，限制了世界模型的潜力。需要一种统一范式来桥接这两个关键任务。

Method: DriveLaW包含两个核心组件：DriveLaW-Video（生成高保真预测的强大量世界模型）和DriveLaW-Act（从视频模型潜在表示生成一致轨迹的扩散规划器），采用三阶段渐进训练策略优化。

Result: 在视频预测任务上显著超越最佳工作33.3%的FID和1.8%的FVD，同时在NAVSIM规划基准测试中创造了新纪录。
...
TL;DR: 提出基于天气条件和路面状况数据的实时道路监测系统，使用手机摄像头采集数据，测试多种深度学习算法进行道路分类，结合加速度数据，达到95%以上准确率，并建议使用模糊逻辑根据天气和时间分类路面。
...
Method: 使用手机摄像头在卡尔斯鲁厄理工学院校园周边道路采集数据。测试多种基于图像的深度学习算法（AlexNet、LeNet、VGG、ResNet）进行道路分类。同时使用道路加速度数据与图像数据结合训练，将加速度数据转换为图像格式。比较基于加速度和基于摄像头图像的方法性能。

Result: 在道路状况分类中考虑了5个类别：沥青路、损坏沥青路、碎石路、损坏碎石路、铺面路。实现了超过95%的准确率性能。比较了不同深度学习算法的表现。

Conclusion: 提出的实时系统能够有效监测道路表面状况。建议进一步使用模糊逻辑，根据天气条件和一天中的时间，利用加速度或摄像头图像对道路表面进行分类，提高分类的适应性和准确性。
...
Method: 提出DDSPO方法，直接从获胜和失败策略中获取每个时间步的监督信号，而不是仅基于最终样本。通过使用预训练参考模型自动生成偏好信号：对比原始提示与语义降级变体条件下的输出，实现无需显式奖励建模或人工标注的分数空间偏好监督。
...
TL;DR: CoFi-Dec是一种无需训练的解码框架，通过整合生成式自我反馈和从粗到细的视觉条件来减少大视觉语言模型中的幻觉问题。
...
Method: CoFi-Dec采用训练自由的解码框架，模拟人类从全局场景感知到详细检查的视觉过程。首先生成基于原始图像粗粒度和细粒度视图的中间文本响应，然后使用文生图模型将这些响应转换为合成图像，形成多级视觉假设。引入基于Wasserstein距离的融合机制，将这些多视觉条件的预测分布对齐到几何一致的解码轨迹中。

Result: 在六个专注于幻觉的基准测试上的广泛实验表明，CoFi-Dec显著减少了实体级和语义级的幻觉，优于现有的解码策略。

Conclusion: 该框架是模型无关的，无需额外训练，可以无缝应用于广泛的大视觉语言模型，通过原则性融合实现了高级语义一致性与细粒度视觉基础的协调，产生更稳健和忠实的输出。
...
TL;DR: 该研究评估了不确定性量化能否提升SAM在挑战性分割场景中的泛化能力，创建了UncertSAM基准测试，发现拉普拉斯近似方法能有效关联不确定性估计与分割误差。


<details>
  <summary>Details</summary>
Motivation: 尽管SAM等基础分割模型在零样本任务中表现出色，但在领域偏移或知识有限的情况下仍然脆弱。研究旨在探索不确定性量化能否以领域无关的方式缓解这些挑战，增强模型的泛化能力。

Method: 1) 创建UncertSAM基准测试，包含8个数据集，专门设计用于在阴影、透明、伪装等挑战性条件下测试SAM；2) 评估一系列轻量级、后处理的不确定性估计方法；3) 评估初步的不确定性引导预测细化步骤。

Result: 在评估的方法中，最后一层拉普拉斯近似方法产生的不确定性估计与分割误差有良好相关性，表明存在有意义的信号。虽然细化改进效果尚属初步，但研究结果强调了将不确定性纳入分割模型以支持稳健、领域无关性能的潜力。

Conclusion: 不确定性量化在提升分割模型在挑战性条件下的泛化能力方面具有潜力，特别是拉普拉斯近似方法显示出有前景的结果。研究提供了公开的基准测试和代码，为未来不确定性感知分割研究奠定了基础。
...
Method: 采用全面的全阶段训练范式：1) 在3000多小时运动数据上进行大规模预训练；2) 在400小时精选数据上进行高质量微调；3) 基于人类反馈和奖励模型进行强化学习。同时建立了严谨的数据处理流程进行运动清理和标注。

Result: 模型实现了最广泛的运动覆盖，涵盖6个大类超过200种运动类别，在指令跟随能力上显著超越当前开源基准。

Conclusion: HY-Motion 1.0代表了运动生成领域的重要进展，通过规模化扩散变换器架构和全阶段训练范式，为3D人体运动生成模型的商业化应用奠定了基础。
...
TL;DR: 本研究提出了一种结合视觉水线检测、YOLOv8姿态尺度提取和多模态大语言模型的混合框架，用于自动读取河流水位标尺，实现了高精度的自动化水文监测。
...
Method: 采用混合框架，包括图像预处理、标注、水线检测、尺度间隙估计和数值读取提取等顺序阶段，结合YOLOv8姿态尺度提取和多模态大语言模型（GPT-4o和Gemini 2.0 Flash）。
...
Conclusion: 该方法为自动化水文监测提供了可扩展、高效可靠的解决方案，展示了实时河流标尺数字化和改善水资源管理的潜力，强调了结合几何元数据与多模态人工智能对稳健水位估计的重要性。
...
Method: 提出训练免费的AnyMS框架，采用自底向上的双级注意力解耦机制：全局解耦分离文本和视觉条件的交叉注意力，局部解耦将每个主体的注意力限制在指定区域，并使用预训练图像适配器提取主体特征

Result: 实验表明AnyMS达到最先进性能，支持复杂构图并扩展到更多主体数量

Conclusion: AnyMS通过创新的注意力解耦机制有效解决了多主体定制中的平衡问题，无需训练即可实现高质量的文本对齐、身份保持和布局控制
...
### [112] [RxnBench: A Multimodal Benchmark for Evaluating Large Language Models on Chemical Reaction Understanding from Scientific Literature](https://arxiv.org/abs/2512.23565)
...
TL;DR: RxnBench是一个用于评估多模态大语言模型在化学文献中理解反应能力的基准测试，包含单图问答和全文问答两个任务，揭示了当前模型在深层化学逻辑和结构识别方面的能力差距。


<details>
  <summary>Details</summary>
Motivation: 尽管多模态大语言模型在化学领域有革命性潜力，但它们在真实科学文献中理解密集图形化反应语言的能力尚未得到充分探索，需要建立系统评估基准。

Method: 开发了RxnBench多层级基准，包含两个任务：1) 单图问答：基于305个精选反应方案生成的1525个问题，测试细粒度视觉感知和机理推理；2) 全文问答：基于108篇文章，要求模型整合文本、方案和表格的跨模态信息。

Result: 评估发现关键能力差距：模型擅长提取显式文本，但在深层化学逻辑和精确结构识别方面表现不佳。具有推理时推理能力的模型显著优于标准架构，但在全文问答任务中无一达到50%准确率。

Conclusion: 研究强调了开发领域特定视觉编码器和更强推理引擎的紧迫性，以推进自主AI化学家的发展。
...
TL;DR: TV-RAG：一种无需训练的长视频推理架构，通过时间对齐和熵引导语义来增强大型视频语言模型的长视频理解能力
...
Method: 提出TV-RAG框架，包含两个核心机制：1）时间衰减检索模块，将显式时间偏移注入相似度计算，根据真实的多媒体上下文对文本查询进行排序；2）熵加权关键帧采样器，选择均匀间隔、信息密集的帧，减少冗余同时保持代表性

Result: TV-RAG在Video-MME、MLVU和LongVideoBench等长视频基准测试中持续超越大多数领先基线模型，验证了方法的有效性

Conclusion: TV-RAG提供了一种轻量级、经济高效的升级路径，无需重新训练或微调即可集成到任何大型视频语言模型中，通过结合时间和语义信号实现了双级推理能力
...
<details>
  <summary>Details</summary>
Motivation: 当前视觉生成模型虽然能从文本生成故事，但在人类中心故事生成中面临额外挑战：需要保持详细多样的人脸一致性，并在不同图像中协调多个角色。

Method: 框架包含两个关键组件：1）迭代身份发现：提取连贯的角色身份特征；2）重去噪身份注入：通过重去噪过程注入身份特征，同时保留所需上下文。
...
Method: 提出IAFS框架，结合迭代细化和频率感知粒子融合，通过迭代校正结构偏差逐步优化生成图像，并自适应融合高频感知线索和低频结构信息。

Result: 在多个基于扩散的超分辨率模型上的实验表明，IAFS有效解决了感知-保真度冲突，在感知细节和结构准确性方面均获得一致改进，优于现有推理时间缩放方法。

Conclusion: IAFS是一个无需训练的方法，通过迭代扩散推理时间缩放和自适应频率引导，成功平衡了图像超分辨率中的感知质量与结构保真度。
...
TL;DR: PurifyGen是一种无需训练的双阶段提示净化方法，通过语义距离评估token安全性，将有毒语义投影到零空间并增强清洁语义，实现安全文本到图像生成。
...
Method: 提出PurifyGen双阶段策略：1）通过计算互补语义距离评估每个token的安全性，测量提示token与预定义有毒和清洁概念嵌入的语义接近度；2）对于风险提示，应用双空间变换：将有毒对齐嵌入投影到有毒概念矩阵的零空间以去除有害语义成分，同时将其对齐到清洁概念的范围空间以增强安全语义。采用token级策略选择性替换风险token嵌入。

Result: 在五个数据集上的广泛测试表明，PurifyGen在减少不安全内容方面超越了当前方法，并与依赖训练的方法竞争良好。该方法具有理论基础，对未见提示和模型具有强泛化能力。

Conclusion: PurifyGen提供了一种即插即用的解决方案，无需修改模型权重，通过双空间变换有效净化风险提示，在保持原始意图和连贯性的同时确保安全生成。
...
<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型的思维链推理在复杂理解任务中表现出色，但在生成任务中的扩展仍处于起步阶段，受到场景特定机制的限制，阻碍了其泛化和适应能力。
...
Conclusion: ThinkGen是首个显式利用MLLM思维链推理的视觉生成框架，其灵活设计支持跨多样化数据集的联合训练，为广泛的生成场景提供了有效的思维链推理能力。
...
<details>
  <summary>Details</summary>
Motivation: 生成模型的快速发展带来了持续出现的多模态安全风险，现有防御方法存在局限性，特别是对分布外安全风险的检测能力不足。

Method: 1) 构建包含87K样本的模态平衡数据集，采用分层多模态安全分类法标注；2) 通过强化学习训练视觉语言基础模型；3) 引入分布外安全类别推理任务，并使用基于同义词库的相似性奖励来增强RL目标。

Result: ProGuard在二元安全分类上达到闭源大模型水平，在非安全内容分类上显著优于现有开源防护模型，分布外风险检测提升52.6%，分布外风险描述提升64.8%。

Conclusion: ProGuard提供了一种有效的主动安全防护方法，能够识别和描述分布外安全风险，在多模态内容安全防护方面具有显著优势。
...
<details>
  <summary>Details</summary>
Motivation: 现有扩散模型通过迭代双向注意力处理所有视频帧，无法实现实时交互。虽然现有蒸馏方法可以使模型自回归并减少采样步骤，但它们主要关注文本到视频生成，导致人机交互不自然且效率低下。本文旨在实现基于多模态上下文（文本、图像、音频）的实时交互式视频扩散，弥合这一差距。

Method: 针对领先的在线策略蒸馏方法Self Forcing在多模态条件下遇到的视觉伪影问题，研究改进的蒸馏配方，重点关注条件输入质量以及在线策略优化的初始化和调度。在HDTF、AVSpeech和CelebV-HQ等多模态条件化身视频生成基准上进行验证。

Result: 蒸馏模型在视觉质量上与完整步骤的双向基线模型相当或更好，同时推理成本和延迟降低了20倍。将模型与音频语言模型和长视频推理技术Anchor-Heavy Identity Sinks集成，构建了LiveTalk实时多模态交互化身系统。在策划的多轮交互基准测试中，LiveTalk在多轮视频连贯性和内容质量上优于Sora2、Veo3等最先进模型，同时将响应延迟从1-2分钟降低到实时生成。

Conclusion: 通过改进的蒸馏配方成功实现了实时多模态交互视频生成，显著降低了推理延迟，使无缝的人机多模态交互成为可能，为构建通用多模态交互AI系统提供了关键技术。
...
<details>
  <summary>Details</summary>
Motivation: 现有视觉语言模型在粗粒度视觉理解方面表现出色，但存在视觉偏见且容易忽略细微视觉细节。现有训练语料库强调一般识别而非细粒度感知，限制了模型的感知能力。

Method: 提出TWIN数据集，包含56.1万张图像对查询，要求模型判断两个视觉上相似的图像是否描绘同一物体。同时引入FGVQA基准测试套件，包含1.2万个查询，用于量化细粒度识别能力的提升。

Result: 在TWIN上微调的视觉语言模型在细粒度识别方面取得显著提升，在未见领域（如艺术、动物、植物、地标）上表现良好。在FGVQA基准上提升高达19.3%，同时不影响通用VQA基准的性能。

Conclusion: TWIN数据集可轻松集成到开源视觉语言模型训练语料库中，通过提升模型的感知精度来推动未来模型的发展。数据集规模对性能至关重要，且与物体标注的扩展性良好。
...
TL;DR: 本文提出三种改进红外夜视摄像头火灾检测的方法：增加NIR数据集、两阶段检测模型和Patched-YOLO，旨在解决夜间火灾检测中人工光源误报和小目标检测问题。


<details>
  <summary>Details</summary>
Motivation: 红外夜视摄像头火灾检测的准确性仍然具有挑战性，现有模型（如YOLOv7、RT-DETR、YOLOv9）虽然表现良好，但数据集构建的局限性导致人工光源经常被误判为火灾，特别是在夜间检测中。

Method: 1) 构建额外的NIR数据集并应用多种数据增强策略；2) 提出两阶段检测管道，结合YOLOv11和EfficientNetV2-B0，先检测再分类以减少误报；3) 提出Patched-YOLO，通过基于补丁的处理增强RGB图像中小型和远距离目标的检测能力。

Result: 提出的方法相比先前方法（YOLOv7 mAP50-95 0.51、RT-DETR 0.65、YOLOv9 0.598）实现了更高的检测准确率，特别是在夜间火灾检测方面表现更优，同时减少了人工光源引起的误报。

Conclusion: 通过数据增强、两阶段检测架构和补丁处理策略，显著提升了红外夜视摄像头火灾检测的准确性，特别是在解决夜间人工光源误报和小目标检测问题上取得了实质性进展。
...
<details>
  <summary>Details</summary>
Motivation: 胰腺肿瘤早期检测面临临床困境，主要因为肿瘤在CT扫描中对比度低、解剖结构变异大，需要有效且可扩展的系统来增强细微视觉线索并提高多模态成像数据的泛化能力

Method: 提出可扩展残差特征聚合(SRFA)框架：1)预处理和MAGRes-UNet分割使胰腺结构更可见；2)DenseNet-121提取特征并保留残差特征；3)混合HHO-BA元启发式特征选择优化特征子集；4)结合Vision Transformer和EfficientNet-B3的混合模型进行分类；5)SSA和GWO双重优化机制调参

Result: 实验结果显示显著性能提升：准确率96.23%，F1分数95.58%，特异性94.83%，显著优于传统CNN和当代基于Transformer的模型

Conclusion: SRFA框架在胰腺肿瘤早期检测中表现出优异性能，可作为临床诊断的有用工具，证明了该方法的有效性和实用性
...
TL;DR: 本文提出一个评估框架来量化3D生成模型中的记忆效应，并通过实验发现记忆程度受数据模态、多样性、条件细粒度以及模型设计（如引导尺度、向量集长度、旋转增强）的影响。


<details>
  <summary>Details</summary>
Motivation: 随着生成模型在3D视觉中广泛用于合成新形状，但尚不清楚其生成是否依赖于记忆训练数据。理解这种记忆效应有助于防止训练数据泄露并提高生成结果的多样性。

Method: 设计了一个评估框架来量化3D生成模型中的记忆效应，首先应用于现有方法，然后通过使用潜在向量集扩散模型进行受控实验，分析数据和建模设计对记忆的影响。

Result: 发现记忆程度取决于数据模态，随数据多样性和更细粒度的条件而增加；在建模方面，记忆在中等引导尺度达到峰值，可通过更长的向量集和简单的旋转增强来缓解。
...
Method: HAT首先利用多个显式运动模型生成空间锚点和运动感知特征提议，然后结合缓存物体查询中的语义和运动线索进行多假设解码，最终为目标帧提供最优对齐方案

Result: 在nuScenes数据集上，HAT持续改进3D时序检测器和跟踪器，与DETR3D检测器配对时在测试集上达到46.0% AMOTA的SOTA跟踪结果。在端到端AD方法中，提升感知精度(+1.3% mAP, +3.1% AMOTA)并降低32%碰撞率

Conclusion: HAT通过自适应多假设解码机制有效解决了时空对齐问题，在语义受损情况下仍能提供鲁棒的感知和规划能力，验证了显式运动建模在端到端自动驾驶感知中的重要性
...
<details>
  <summary>Details</summary>
Motivation: RGB图像将材质属性、光照和视角依赖效应纠缠在一起，而现有的扩散方法在单视角本征分解上表现良好，但在扩展到多视角设置时面临挑战，常导致严重的视角不一致问题。

Method: IDT采用基于物理的图像形成模型，将图像明确分解为漫反射率、漫反射阴影和镜面反射阴影。使用基于Transformer的注意力机制联合推理多个输入图像，分离Lambertian和非Lambertian光传输。

Result: 在合成和真实世界数据集上的实验表明，IDT实现了更清晰的漫反射率、更一致的漫反射阴影和更好分离的镜面反射分量，相比先前方法显著提高了多视角一致性。
...
<details>
  <summary>Details</summary>
Motivation: 透明物体对传统感知系统构成挑战，折射、反射和透射破坏了立体视觉、ToF和单目深度估计的假设，导致空洞和时间不稳定。现代视频扩散模型已能合成逼真的透明现象，表明其内部已掌握光学规则。

Method: 1) 构建TransPhy3D合成视频数据集：11k序列，使用Blender/Cycles渲染，包含RGB+深度+法线；2) 基于大视频扩散模型，通过轻量LoRA适配器学习视频到视频的深度（和法线）转换器；3) 在DiT骨干中连接RGB和（含噪声）深度潜在表示，在TransPhy3D和现有合成数据集上共同训练，实现任意长度输入视频的时间一致性预测。

Result: DKT模型在涉及透明度的真实和合成视频基准测试（ClearPose、DREDS、TransPhy3D-Test）上实现零样本SOTA。在准确性和时间一致性方面优于强图像/视频基线，法线变体在ClearPose上创下最佳视频法线估计结果。1.3B紧凑版本运行速度约0.17秒/帧。集成到抓取系统中，DKT的深度估计提高了透明、反射和漫反射表面的成功率。
...
<details>
  <summary>Details</summary>
Motivation: 现有的基于扩散的视频超分辨率方法虽然感知质量好，但依赖未来帧和昂贵的多步去噪，在延迟敏感的实际应用中不实用。需要开发适合低延迟在线部署的扩散VSR方法。

Method: 提出Stream-DiffVSR框架：1）四步蒸馏去噪器实现快速推理；2）自回归时间引导模块在潜在去噪时注入运动对齐线索；3）轻量级时间感知解码器配合时间处理器模块增强细节和时间一致性。严格使用过去帧进行因果处理。

Result: 在RTX4090 GPU上处理720p帧仅需0.328秒，显著优于先前扩散方法。相比在线SOTA TMP方法，提升感知质量（LPIPS +0.095）同时降低延迟130倍以上。将初始延迟从4600秒降至0.328秒。

Conclusion: Stream-DiffVSR实现了扩散VSR中最低的延迟，是首个适合低延迟在线部署的扩散VSR方法，在保持高质量的同时大幅提升了实用性。
...
### [128] [Multi Agents Semantic Emotion Aligned Music to Image Generation with Music Derived Captions](https://arxiv.org/abs/2512.23320)
*Junchang Shi,Gang Li*

Main category: cs.MM

TL;DR: MESA MIG是一个多智能体框架，通过结构化音乐描述和情感对齐来生成与音乐匹配的图像，在美学质量、语义一致性和情感对齐方面优于基准方法。
...
Method: 提出多智能体语义和情感对齐框架：首先生成结构化音乐描述，然后由专门处理场景、运动、风格、色彩和构图的协作智能体进行细化；同时使用效价-唤醒度回归头预测音乐情感状态，以及基于CLIP的视觉VA头估计图像情感，共同确保音乐与合成图像之间的语义和情感对齐。

Result: 在精心策划的音乐-图像对数据集上的实验表明，MESA MIG在美学质量、语义一致性和VA对齐方面优于仅使用描述和单智能体基准方法，并且在情感回归性能上与最先进的音乐和图像情感模型具有竞争力。

Conclusion: MESA MIG框架成功地将音乐情感和语义内容转化为视觉图像，实现了音乐与图像之间的多模态对齐，为音乐驱动的图像生成提供了有效解决方案。
...
### [129] [Unlocking WebRTC for End User Driven Innovation](https://arxiv.org/abs/2512.23688)
*Kundan Singh*

Main category: cs.MM

TL;DR: RTC Helper是一个浏览器扩展软件，能够拦截WebRTC API并实时改变网页应用行为，支持终端用户对第三方网页应用进行定制，也帮助开发者快速原型开发。


<details>
  <summary>Details</summary>
Motivation: 该研究的动机是让终端用户能够创新网页多媒体通信应用，同时帮助网页开发者快速原型开发，无需每次修改后重新构建或部署应用。

Method: 开发了一个简单易用的软件架构，通过浏览器扩展拦截WebRTC及相关API，实时改变网页应用行为。该工具支持超过十个定制类别和上百个内置示例。

Result: RTC Helper能够成功拦截WebRTC API并实时修改网页应用行为，支持终端用户对第三方网页应用进行定制，覆盖了网页音视频通信的广泛新颖用例。

Conclusion: 该软件架构实现了终端用户驱动的网页多媒体通信应用创新，为网页开发者提供了快速原型开发工具，展示了WebRTC定制化的巨大潜力。
...
TL;DR: 提出FMTC框架，通过客户端个性化聚类模块和服务器端张量相关性模块，在保护隐私的同时为异构客户端学习个性化聚类模型，并利用共享知识提升性能。
...
Method: 提出FMTC框架，包含客户端个性化聚类模块（学习参数化映射模型，绕过不可靠伪标签）和服务器端张量相关性模块（将所有客户端模型组织为统一张量，应用低秩正则化发现共享子空间）。采用基于ADMM的高效隐私保护分布式算法。

Result: 在多个真实数据集上的实验表明，FMTC框架显著优于各种基线和最先进的联邦聚类算法。

Conclusion: FMTC框架能够有效解决异构联邦聚类问题，在保护隐私的同时学习个性化模型并利用共享知识，显著提升聚类性能。
...
<details>
  <summary>Details</summary>
Motivation: 无监督表示学习通常依赖统计独立性，但难以捕捉因果依赖关系。核心挑战是可识别性问题：从观测数据中解耦因果变量需要监督、辅助信号或强归纳偏置。现有方法在复杂场景下存在局限性。

Method: 提出LANCA（潜在加性噪声模型因果自编码器），将加性噪声模型作为强归纳偏置。采用确定性Wasserstein自编码器替代随机编码的VAE，结合可微分ANM层，将残差独立性转化为显式优化目标。

Result: 理论上证明ANM约束虽不能保证一般混合情况下的唯一可识别性，但能将允许的变换从任意微分同胚限制到仿射类。在合成物理基准（Pendulum, Flow）和真实环境（CANDLE）上优于现有方法，对复杂背景场景中的虚假相关性表现出更强鲁棒性。

Conclusion: LANCA通过将加性噪声模型作为强归纳偏置，成功实现了无监督因果发现，解决了传统方法在复杂场景下的局限性，为因果表示学习提供了新的有效框架。
...
TL;DR: 论文提出Latent Sculpting框架解决高维表格数据中的泛化崩溃问题，通过两阶段表示学习：第一阶段用DCCL损失将良性数据压缩成低熵超球簇，第二阶段用MAF学习精确密度估计，在CIC-IDS-2017基准上实现零样本异常检测F1分数0.87。


<details>
  <summary>Details</summary>
Motivation: 监督深度学习在高维表格数据中存在"泛化崩溃"问题：模型能学习已知分布的精确决策边界，但在面对分布外数据时表现灾难性失败。作者认为这源于潜在空间缺乏拓扑约束，导致流形扩散，使得新异常与良性数据统计上无法区分。

Method: 提出Latent Sculpting框架：第一阶段使用混合1D-CNN和Transformer编码器，配合新颖的双中心紧凑性损失（DCCL），主动将良性流量"雕刻"成低熵超球簇；第二阶段在预结构化流形上条件化掩码自回归流（MAF）学习精确密度估计。

Result: 在CIC-IDS-2017基准测试中，监督基线在未见分布偏移上表现灾难性崩溃（F1约0.30），最强无监督基线仅0.76，而本框架在严格零样本异常检测上达到F1分数0.87。在"渗透"场景中检测率达到88.89%，而最先进的监督模型准确率为0.00%。

Conclusion: 显式流形雕刻是鲁棒零样本泛化的先决条件，将结构学习与密度估计解耦为广义异常检测提供了可扩展路径。该方法在复杂分布偏移下表现出色，为解决高维表格数据中的泛化崩溃问题提供了有效方案。
...
TL;DR: 该研究提出了一种基于强化学习的网球策略优化框架，使用Dueling Double Deep Q-Network和课程学习训练，在自定义网球模拟环境中实现了高胜率，但发现学习策略存在防御性偏差。


<details>
  <summary>Details</summary>
Motivation: 网球策略优化是一个复杂的序列决策问题，涉及分层计分、随机结果、长期信用分配、体力疲劳和对手技能适应等挑战。传统方法难以有效处理这些复杂因素。

Method: 构建了完整的网球模拟环境，包括点、局、盘三级计分系统，10种离散战术动作类别，对称疲劳动态和连续对手技能参数。采用Dueling Double Deep Q-Network(DDQN)架构，结合课程学习逐步提升对手难度（从0.40到0.50）。

Result: 训练后的智能体对平衡对手的胜率达到98-100%，发球效率63.0-67.5%，接发球效率52.8-57.1%。消融研究表明，Dueling架构和课程学习对稳定收敛至关重要，而标准DQN基线无法学习有效策略。

Conclusion: 尽管取得了高胜率，但战术分析显示学习策略存在明显的防御性偏差，优先考虑避免失误和延长回合而非积极得分。这表明在简化体育模拟中，仅以胜率为驱动的优化存在局限性，强调了奖励设计在现实体育强化学习中的重要性。
...
TL;DR: 本文提出情感启发学习信号（EILS）框架，用生物情感类比作为内稳态控制机制，替代传统静态奖励函数，以增强AI在开放环境中的鲁棒性和适应性。


<details>
  <summary>Details</summary>
Motivation: 当前AI依赖外部定义的静态奖励函数（"外在最大化"），在封闭稳定环境中表现优异，但在开放真实环境中脆弱。标准智能体缺乏内部自主性：难以在没有密集反馈时探索、无法适应分布变化、需要大量手动调参。需要解决鲁棒自主性的未解决因素。

Method: 引入情感启发学习信号（EILS）统一框架，将情感建模为连续的内稳态评估信号（如好奇心、压力、自信），而非语义标签。将这些信号形式化为从交互历史导出的向量值内部状态，动态实时调节智能体的优化景观：好奇心调节熵防止模式崩溃，压力调节可塑性克服惰性，自信调整信任区域稳定收敛。

Result: 假设这种闭环内稳态调节能使EILS智能体在样本效率和非平稳适应方面优于标准基线方法。

Conclusion: EILS框架用生物启发的内部反馈引擎替代分散的优化启发式方法，通过模拟生物情感的内稳态控制机制，有望解决当前AI在开放环境中鲁棒自主性的关键限制。
...
TL;DR: 该论文识别了一类新的注意力汇点——次级汇点，与之前研究的主要汇点不同，次级汇点主要出现在中间层，持续时间可变，吸引的注意力质量较小但仍显著。


<details>
  <summary>Details</summary>
Motivation: 现有研究主要关注BOS令牌等主要注意力汇点，但作者发现存在性质不同的次级汇点，需要系统研究其特性、形成机制和对注意力机制的影响。

Method: 通过对11个模型家族进行广泛实验，分析次级汇点的出现位置、特性、形成机制及其对注意力机制的影响，特别关注中间层MLP模块的作用。

Result: 发现：(1)次级汇点由特定中间层MLP模块形成，这些MLP将令牌表示映射到与该层主要汇点方向对齐的向量；(2)这些向量的ℓ2范数决定次级汇点的汇点分数和持续时间；(3)主要汇点在中间层减弱，与次级汇点出现时间重合；(4)大规模模型中汇点层级更确定和频繁。

Conclusion: 注意力汇点存在更复杂的层级结构，次级汇点在中间层形成并影响注意力机制，大规模模型中汇点层级更加确定化，这为理解Transformer注意力机制提供了新视角。
...
TL;DR: DiRL是一个针对扩散语言模型的高效后训练框架，通过FlexAttention加速块状训练和LMDeploy优化推理，结合DiPO策略优化，在数学推理任务上取得了SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 扩散语言模型作为自回归模型的替代方案具有潜力，但现有后训练方法存在计算效率低下和训练-推理目标不匹配的问题，限制了在复杂推理任务（如数学）上的性能。

Method: 提出DiRL框架：1）整合FlexAttention加速的块状训练和LMDeploy优化的推理，实现高效的在线模型更新循环；2）支持两阶段后训练（监督微调+强化学习）；3）提出DiPO，首个为dLLMs定制的无偏组相对策略优化实现。

Result: 在高质量数学数据上训练DiRL-8B-Instruct模型，在多个基准测试中超越了Qwen2.5系列可比模型，在dLLMs中取得了最先进的数学性能。

Conclusion: DiRL框架有效解决了dLLMs后训练的计算效率和目标匹配问题，通过优化的训练-推理架构和专门的策略优化方法，显著提升了扩散语言模型在复杂推理任务上的性能。
...
TL;DR: 提出Masters框架，通过掩码渐进强化学习蒸馏，解决大规模视觉语言模型向小型模型知识迁移时因尺寸差距导致的性能下降问题


<details>
  <summary>Details</summary>
Motivation: 大规模视觉语言模型虽然具有强大的多模态理解能力，但模型尺寸过大难以部署到移动或边缘设备。需要将大型教师模型的知识蒸馏到小型学生模型，但由于尺寸差距过大，学生模型难以复现教师模型的复杂高维表示，导致学习不稳定和性能下降

Method: 提出Masters框架：1) 掩码教师模型的非主导权重以降低复杂度；2) 在训练过程中渐进式恢复教师模型容量；3) 采用离线强化学习阶段，包含两个互补奖励：准确性奖励和蒸馏奖励；4) 利用掩码教师预生成的响应提供高效指导，避免在线思维-回答过程的计算开销

Result: 该方法使学生模型能够以平滑稳定的方式从教师模型学习更丰富的表示，实现强大的性能表现，同时避免了传统在线强化学习范式的高计算成本和生成长响应的问题

Conclusion: Masters框架通过掩码渐进强化学习蒸馏，有效解决了大型视觉语言模型向小型模型知识迁移的挑战，为在资源受限设备上部署高性能多模态模型提供了可行的解决方案
...
TL;DR: EvoXplain框架通过分析多次训练中模型解释的稳定性，揭示即使预测准确率高的模型也可能存在多种不同的内部逻辑机制，挑战了"高准确率即意味着解释可靠"的假设。


<details>
  <summary>Details</summary>
Motivation: 当前机器学习实践中存在一个被忽视的问题：当两个模型都达到高准确率时，它们是否基于相同的内部逻辑？还是通过不同甚至相互竞争的机制达到相同结果？作者质疑"高准确率即意味着解释正确可信"的假设。

Method: 提出EvoXplain诊断框架，将解释视为从随机优化过程中抽取的样本（而非分析单个训练模型），检查这些样本是否形成单一连贯的解释，还是分离为多个不同的解释模式。框架不聚合预测或构建集成模型，而是在乳腺癌和COMPAS数据集上使用逻辑回归和随机森林进行评估。

Result: 尽管所有模型都达到高预测准确率，但它们的解释经常表现出明显的多模态性。即使是通常被认为稳定的逻辑回归模型，在同一数据分割的重复训练下也能产生多个明显分离的解释"盆地"。这些差异不能通过超参数变化或简单的性能权衡来解释。

Conclusion: EvoXplain不试图选择"正确"的解释，而是使解释不稳定性变得可见和可量化，揭示单个实例或平均解释可能掩盖多种底层机制的存在。更广泛地说，EvoXplain将可解释性重新定义为模型类在重复实例化下的属性，而非任何单个训练模型的属性。
...
TL;DR: 研究评估了两种深度学习肺癌风险预测模型（Sybil和Venkadesh21）在不同人口亚组中的性能差异和公平性问题，发现存在显著的性别和种族间性能差异。


<details>
  <summary>Details</summary>
Motivation: 肺癌是全球癌症相关死亡的主要原因，低剂量CT筛查可早期发现并降低死亡率，但广泛实施可能加重放射科医生负担。AI模型在肺癌风险评估中显示出潜力，但高危人群多样化，模型在不同人口亚组中的性能差异和公平性仍是未解决的问题。

Method: 使用JustEFAB框架评估两种深度学习肺癌风险预测模型（Sybil和Venkadesh21）以及PanCan2b逻辑回归模型的性能差异和公平性。模型基于美国国家肺癌筛查试验（NLST）数据训练，在NLST验证集上评估，分析不同人口亚组（性别、种族）的AUROC、敏感性和特异性，并探讨临床混杂因素的影响。

Result: Sybil模型在女性（AUROC 0.88）和男性（AUROC 0.81）之间存在显著性能差异（p<0.001）。Venkadesh21模型在90%特异性下，黑人参与者敏感性（0.39）显著低于白人参与者（0.69）。这些差异无法用现有临床混杂因素解释，根据JustEFAB框架可归类为不公平偏差。

Conclusion: 研究发现肺癌筛查AI模型在不同人口亚组中存在显著性能差异和潜在不公平偏差，强调需要改进和监测模型在代表性不足亚组中的性能，并加强算法公平性研究。
...
TL;DR: 本文提出使用线性探针从LLM推理判断的隐藏状态中获取校准的不确定性估计，相比现有方法具有更好的校准性和约10倍的计算节省。


<details>
  <summary>Details</summary>
Motivation: 随着基于LLM的评判器在工业应用中变得重要，需要高效获取良好校准的不确定性估计以支持生产部署。现有技术如语言化置信度和多生成方法要么校准效果差，要么计算成本高。

Method: 引入基于Brier分数损失训练的线性探针，从推理评判器的隐藏状态中提供校准的不确定性估计，无需额外的模型训练。

Result: 探针在客观任务（推理、数学、事实性、编码）和主观人类偏好判断上都表现出比现有方法更优的校准性，节省约10倍计算量，能稳健泛化到未见过的评估领域，并在高置信度预测上提供更高准确性。但探针产生保守估计，在简单数据集上表现不佳，可能有利于优先考虑低误报率的安全关键部署。

Conclusion: 基于可解释性的不确定性估计为生产中的LLM评判器提供了实用、可扩展的即插即用解决方案。
...
TL;DR: 论文提出激活更新与梯度下降之间存在系统性不匹配，认为激活比参数更应优先优化，并基于此推导出归一化的新理论框架，提出了替代传统仿射归一化的新方法。


<details>
  <summary>Details</summary>
Motivation: 激活在计算图中更接近损失函数且携带样本依赖信息，但当前的梯度下降更新对激活来说并非最优的陡峭下降步长。这种不匹配促使重新思考归一化的理论基础。

Method: 从优化理论出发，分析激活更新的非理想缩放问题，推导出归一化的第一性原理。提出两种新方法：1) 功能上不同于现代归一化但无尺度不变性的替代方案；2) 针对卷积层的"PatchNorm"——一种组合不可分的归一化器。

Result: 提出的新方法在多个测试中优于传统归一化器。实验验证了这些新函数的有效性，同时提供了归一化作用的新机制解释。

Conclusion: 该研究为归一化提供了新的理论框架，挑战了传统的仿射+非线性建模方法，建议将归一化器分解为类似激活函数的参数化缩放映射，以更好地优化表示。
...
TL;DR: 该论文提出了一种利用大语言模型提取时序视觉语义来增强无监督人体运动分割的方法，通过将文本化的运动信息融入子空间聚类框架，显著提升了分割性能。


<details>
  <summary>Details</summary>
Motivation: 传统的人体运动分割方法忽略了时序语义探索的重要性。本文旨在利用大语言模型的图像到文本能力，从人体运动序列中提取时序视觉语义，以增强子空间聚类的效果。

Method: 通过大语言模型从连续帧中提取文本化运动信息，学习时序邻域信息，开发TVS集成子空间聚类方法，包含带时序正则化的子空间嵌入，并引入基于分割输出的反馈优化框架。

Result: 在四个基准人体运动数据集上的实验结果表明，该方法优于现有的最先进方法。

Conclusion: 通过大语言模型提取时序视觉语义并将其融入子空间聚类框架，可以有效提升无监督人体运动分割的性能，证明了时序语义信息在运动分割中的重要性。
...
TL;DR: 该研究开发了基于真实和合成数据的PCI术后心脏死亡风险预测模型，通过数据增强改善类别不平衡问题，识别出年龄、射血分数等四个关键影响因素。


<details>
  <summary>Details</summary>
Motivation: PCI术后患者状态、血管造影和手术特征包含预测长期结果的关键信号，但现有模型在类别不平衡情况下对少数类（心脏死亡）预测能力不足，需要改进。

Method: 分析2,044例分叉病变PCI患者数据，应用多种机器学习模型预测3年死亡率。通过生成500个合成样本解决类别不平衡问题，使用排列特征重要性评估特征贡献，并移除非信息性特征进行实验验证。

Result: 数据增强后，所有模型在保持高AUROC的同时显著提高了少数类召回率，改善了概率质量，对严重风险特征的评估更符合临床实际。特征重要性分析显示年龄、射血分数、外周动脉疾病和脑血管疾病是最具影响力的四个特征。

Conclusion: 通过真实和极端案例的数据增强可以暴露、量化和减少不平衡临床预测的脆弱性，建议在报告主要指标时常规报告概率质量和压力测试结果。
...
TL;DR: 该研究系统分析了物理约束光栅耦合器光谱生成器，发现显式能量守恒约束在物理一致的方程中是冗余的，而法布里-珀罗振荡对带宽预测影响显著，去除后可提升机器学习性能31.3%。


<details>
  <summary>Details</summary>
Motivation: 在科学领域中，真实数据稀缺使得物理约束数据生成至关重要，但现有方法往往过度约束模型而不识别哪些物理组件是必要的。需要系统评估不同物理约束对数据生成和机器学习性能的影响。

Method: 对物理信息光栅耦合器光谱生成器进行系统消融研究，该生成器将五个几何参数映射到100点光谱响应。通过选择性移除显式能量守恒约束、法布里-珀罗振荡、带宽变化和噪声，分析各组件的影响。

Result: 发现物理约束悖论：当基础方程物理一致时，显式能量守恒约束在数学上是冗余的，约束和非约束变体达到相同的守恒精度（平均误差约7×10^-9）。法布里-珀罗振荡主导基于阈值的带宽变异性，移除后带宽展宽减少72%（从132.3nm降至37.4nm）。标准噪声添加加重新归一化流程引入0.5%的非物理负吸收值。生成器以每秒200个样本的速度运行，比典型全波求解器快几个数量级。下游机器学习评估显示物理可学习性权衡：中心波长预测不受影响，但移除法布里-珀罗振荡使带宽预测R平方提高31.3%，RMSE降低73.8%。

Conclusion: 研究结果为物理信息数据集设计提供了可操作的指导，并强调机器学习性能可作为评估约束相关性的诊断工具。识别了哪些物理约束对数据生成和机器学习性能真正必要，哪些是冗余的，有助于优化物理约束数据生成方法。
...
TL;DR: 提出SoDip分层堆叠优化框架，通过集成Transformer、TabNet、XGBoost、高斯过程回归和贝叶斯优化，解决辐射诱导接枝聚合中因基膜形态变化导致的重复性问题。
...
Method: 提出SoDip分层数据驱动框架，包含四个核心组件：1）仅解码器Transformer（DeepSeek-R1）编码文本过程描述符；2）TabNet和XGBoost建模多模态特征交互；3）结合狄利克雷过程混合模型的高斯过程回归进行不确定性量化和异方差处理；4）贝叶斯优化高效探索高维合成空间。使用ChemDataExtractor 2.0和WebPlotDigitizer构建包含数百项RIG研究的多样化数据集。

Result: 在交叉验证中，SoDip相比高斯过程回归实现了约33%的性能提升，同时提供校准的置信区间，能够识别低重复性区域。其堆叠架构能够整合稀疏文本和不同质量的数值输入，优于先前模型。

Conclusion: SoDip框架为接枝聚合研究建立了可重复、形态感知设计的基础，通过集成多模态数据分析和不确定性量化，显著提高了辐射诱导接枝聚合的重复性和性能预测能力。
...
TL;DR: Valori是一个确定性AI内存基板，通过定点算术和状态机模型解决浮点运算在向量嵌入存储和搜索中的非确定性问题，确保跨平台比特一致性。


<details>
  <summary>Details</summary>
Motivation: 现代AI系统使用浮点运算进行向量嵌入存储和搜索，但相同模型、输入和代码在不同硬件架构（如x86与ARM）上会产生不同的内存状态和检索结果，导致不可重现性、数据静默分歧，影响受监管行业的安全部署和审计追踪。

Method: 用定点算术（Q16.16）替代浮点内存操作，将内存建模为可重现状态机，在内存边界强制执行确定性，保证跨平台的比特一致性内存状态、快照和搜索结果。

Result: Valori能够保证跨平台的比特一致性内存状态、快照和搜索结果，证明非确定性在索引或检索之前就已产生，确定性内存是可信AI系统的必要基础。

Conclusion: 确定性内存是构建可信AI系统的关键基础组件，Valori通过定点算术和状态机模型解决了浮点运算带来的跨平台非确定性问题，为受监管行业提供了可验证、可审计的AI内存解决方案。
...
Method: 提出Cluster Aggregated GAN混合生成框架，根据电器行为特征将每个电器路由到专门的分支。对于间歇性电器，聚类模块将相似的激活模式分组，并为每个集群分配专用生成器；对于连续性电器，使用基于LSTM的生成器来捕捉渐进的时间演化，同时通过序列压缩保持训练稳定性。

Result: 在UVIC智能插座数据集上的广泛实验表明，该框架在衡量真实性、多样性和训练稳定性的指标上始终优于基线方法，并且将聚类作为主动生成组件显著提高了可解释性和可扩展性。

Conclusion: 该框架为非侵入式负载监测研究中的合成负载生成提供了一种有效方法，通过专门处理不同电器类型的行为差异，解决了现有方法的局限性。
...
TL;DR: Co-GRPO通过将掩码扩散模型重新表述为统一的马尔可夫决策过程，在轨迹层面联合优化模型参数和推理调度参数，解决了训练与推理过程之间的不一致性问题。


<details>
  <summary>Details</summary>
Motivation: 掩码扩散模型在训练和推理过程中存在显著差异：训练使用简化的单步BERT式目标，而推理是多步迭代过程，受模型和调度策略共同控制。这种训练范式与推理轨迹本质的脱节导致推理调度从未在训练中得到优化。

Method: 将MDM生成重新表述为统一的马尔可夫决策过程，通过轨迹层面的组相对策略优化，在共享奖励下协同优化模型参数和调度参数，无需通过多步生成过程进行昂贵的反向传播。

Result: 在ImageReward、HPS、GenEval和DPG-Bench四个基准测试上的实证结果表明，该方法能更彻底地对齐训练与推理，显著提高生成质量。

Conclusion: Co-GRPO通过轨迹层面的联合优化，解决了掩码扩散模型训练与推理之间的不一致性，为MDM提供了一种更统一的优化框架，显著提升了生成性能。
...
<details>
  <summary>Details</summary>
Motivation: 研究算法管理系统下以人为本的管理是否能够持续，传统线性分析方法可能无法捕捉工人对算法系统的非线性反应

Method: 采用双重机器学习框架估计调节中介模型，避免强加限制性函数形式，使用464名零工工人的调查数据

Result: 发现清晰的非单调模式：支持性HR实践改善工人福祉，但在算法监督存在但难以解释的"模糊中间地带"，其与绩效的联系减弱；当监督透明且可解释时，关系再次加强

Conclusion: 简单的线性规范可能错过模式甚至得出相反结论；平台设计应避免部分定义的模糊控制，清晰的规则和可信的申诉机制能使强监督可行；方法上展示了双重机器学习在组织研究中估计条件间接效应的价值
...
TL;DR: 警告性内容在训练数据中并不能有效教导语言模型避免被警告的行为，模型在警告和直接内容两种情况下生成危险内容的概率相近（76.7% vs 83.3%），表明模型学习的是统计共现模式而非语用解释


<details>
  <summary>Details</summary>
Motivation: 研究旨在理解为什么包含警告框架的训练内容（如"不要使用-此代码有漏洞"）无法有效教导语言模型避免被警告的行为，探索当前语言模型架构在理解警告意图方面的根本局限性

Method: 通过实验比较模型在警告框架内容和直接内容两种条件下的行为表现，使用稀疏自编码器分析潜在特征激活模式，特别关注特征#8684（追踪代码执行模式），并引入"隐形滑移"概念分析对话前导如何旋转激活到线性探针无法检测的子空间

Result: 模型在警告框架下复制被标记内容的概率（76.7%）与直接给出内容的概率（83.3%）在统计上无显著差异；稀疏自编码器分析显示"描述X"和"执行X"激活重叠的潜在特征；特征#8684在警告和利用情境下激活幅度相近；训练时特征消融有效，但提示和推理时引导无效

Conclusion: 当前架构中统计共现模式主导语用解释，模型学习的是在特定上下文中倾向于出现什么，而不是理解内容为何出现在那里；警告框架无法有效教导模型避免危险行为，需要训练时特征消融等更根本的干预措施
...
<details>
  <summary>Details</summary>
Motivation: 现有LLM集成学习方法通常将模型视为黑盒，仅结合输入或最终输出，忽略了丰富的内部表示和跨模型交互，限制了性能提升潜力。

Method: 提出三个关键创新：1)跨模型注意力机制，使后继模型能够访问和融合前驱模型的隐藏状态；2)链式训练范式，以错误抑制为目标逐步微调连接模型；3)近并行推理范式，逐层流水线化跨模型的隐藏状态。

Result: 在常识推理和算术推理任务上的广泛实验表明，LLMBoost能够持续提升准确性，同时减少推理延迟，推理效率接近单模型解码。

Conclusion: LLMBoost通过打破传统黑盒集成方法的限制，利用LLM中间状态实现层次化错误校正，为高效LLM集成学习提供了理论保证和实践框架。
...
TL;DR: OFS算法在公平约束下在线学习阈值策略，通过置信区间选择可行阈值，在合成和半合成环境中优于无约束和对偶基线方法。


<details>
  <summary>Details</summary>
Motivation: 闭环决策系统（如贷款、筛查、再犯风险评估）在公平性和服务约束下运行时会引发反馈效应：决策改变未来人群构成，导致数据非平稳并可能放大不平等。需要研究在人口统计均等约束下从强盗反馈中在线学习阈值策略的方法。

Method: 提出乐观可行搜索（OFS）方法：基于网格维护每个候选阈值的奖励和约束残差的置信区间；每轮选择在置信区间内看起来可行的阈值，并在其中最大化乐观奖励；若无可行阈值，则选择最小化乐观约束违反的阈值。

Result: 在三个环境中评估：1）具有稳定收缩动态的合成闭环基准；2）基于德国信贷和COMPAS的半合成闭环基准。OFS在所有环境中都比无约束和对偶强盗基线获得更高奖励且累积约束违反更小，接近相同扫描程序下最佳可行固定阈值的oracle性能。

Conclusion: OFS方法针对低维、可解释的策略类别特别有效，直接瞄准可行的高效用阈值，在公平约束下的在线决策学习中表现出色，实验可复现且采用双盲友好的相对输出组织。
...
TL;DR: LangPrecip：一种语言感知的多模态降水临近预报框架，通过将气象文本作为降水演变的语义运动约束，在Rectified Flow范式下将文本和雷达信息高效集成，显著提升强降水预报精度。
...
Method: 提出LangPrecip框架，将气象文本作为降水演变的语义运动约束，在Rectified Flow范式下将临近预报建模为语义约束的轨迹生成问题。在潜在空间中高效集成文本和雷达信息，并构建了包含16万对雷达序列和运动描述的大规模多模态数据集LangPrecip-160k。

Result: 在瑞典和MRMS数据集上的实验表明，该方法相比最先进方法取得了一致的改进，在80分钟预报时效下，强降水CSI分别提升了超过60%和19%。

Conclusion: 语言感知的多模态方法能够有效约束降水演变轨迹，显著提升短期降水临近预报的准确性，特别是在强降水事件中表现突出。文本信息作为语义运动约束为气象预报提供了新的有效途径。
...
TL;DR: 该论文指出概率知识图谱嵌入存在关系无关的局限性，无法区分新兴实体和新型关系上下文两种不同的分布外现象，提出了结合语义不确定性和结构不确定性的CAGP方法，显著提升了分布外检测性能。
...
Method: 提出不确定性分解为互补组件：语义不确定性（来自实体嵌入方差，检测新兴实体）和结构不确定性（来自实体-关系共现，检测新型上下文）。理论证明这两种信号是非冗余的，任何凸组合都严格优于单一信号。开发了CAGP方法，通过学习的权重结合语义和结构不确定性。

Result: 在三个数据集（FB15k-237、WN18RR、YAGO3-10）上验证了理论预测，发现100%的新型上下文三元组都有频率匹配的分布内对应物。CAGP方法在时间分布外检测上达到0.94-0.99 AUROC，比关系无关基线相对提升60-80%。在选择性预测中，在85%回答率下减少43%的错误。

Conclusion: 概率知识图谱嵌入的关系无关不确定性存在根本局限性，无法有效检测新型关系上下文。通过分解和结合语义与结构不确定性，可以显著提升分布外检测性能，为知识图谱不确定性量化提供了更全面的框架。
...
<details>
  <summary>Details</summary>
Motivation: 比特币价格预测面临极端波动性和非平稳性挑战，传统单变量时间序列模型在长期预测上表现不佳。研究旨在填补这一空白，通过引入宏观经济变量来改善长期预测性能。

Method: 使用TimeXer架构，整合来自18个主要经济体的全球M2流动性作为领先外生变量（12周滞后结构），构建流动性条件预测模型（TimeXer-Exog），并与LSTM、N-BEATS、PatchTST和标准单变量TimeXer等先进基准模型进行比较。
...
TL;DR: LoRA微调可能导致模型能力灾难性下降，但通过正则化近似重放方法可以解决此问题


<details>
  <summary>Details</summary>
Motivation: 尽管参数高效微调方法（如LoRA）只修改一小部分参数，但它们可能对模型产生显著影响。研究发现LoRA监督微调可能导致模型能力灾难性下降，即使在小数据集上训练较少步数也会发生

Method: 提出正则化近似重放方法：1) 惩罚与初始模型的KL散度；2) 交错使用与预训练相似的开源语料库进行下一个token预测数据。这种方法在Qwen指令微调模型上应用

Result: 该方法在保持模型通用知识的同时，不阻碍对新任务的可塑性，仅增加适度的计算开销。相比最直接的方法（实践中可能最常用）的失败，这种小调整几乎完全消除了问题

Conclusion: 虽然LoRA微调可能导致模型能力灾难性下降，但通过正则化近似重放等简单调整的训练程序可以解决此问题，在保持模型通用知识的同时实现有效微调
...
TL;DR: 论文提出了Complete(d)参数化方法，统一处理宽度、深度、批大小和训练时长的缩放，并研究了模块级超参数优化与迁移，显著提升了大型语言模型的训练速度。


<details>
  <summary>Details</summary>
Motivation: 超参数调优对大规模模型的训练稳定性和最终性能有重大影响。现有方法如μP虽然能跨模型尺寸迁移全局超参数，但未能统一处理所有重要缩放维度，且缺乏对模块级超参数优化的研究。

Method: 提出Complete(d)参数化方法，基于CompleteP的改进，统一处理宽度、深度、批大小和训练时长的缩放。研究模块级超参数优化，分析高维超参数空间的挑战，并提出实用优化指南。

Result: 实验表明，在正确的参数化下，超参数迁移在模块级超参数体系中也成立。研究覆盖了现代模型的关键优化超参数，包括学习率、AdamW参数、权重衰减、初始化尺度和残差块乘数。使用迁移的模块级超参数显著提升了大型语言模型的训练速度。

Conclusion: Complete(d)参数化方法能够有效统一处理多个维度的模型缩放，模块级超参数优化与迁移是可行的，并能带来显著的训练效率提升，为大规模模型训练提供了实用的超参数调优框架。
...
TL;DR: AFA-LoRA是一种改进的LoRA训练策略，通过退火激活函数在训练过程中从非线性过渡到线性变换，提升LoRA的表达能力同时保持可合并性。
...
Method: 提出AFA-LoRA训练策略，核心创新是退火激活函数，在训练过程中从非线性变换逐渐过渡到线性变换，使适配器先获得更强的表征能力，最终收敛到可合并的线性形式。

Result: 在监督微调、强化学习和推测解码等任务上实施该方法，结果显示AFA-LoRA缩小了LoRA与全参数训练之间的性能差距。
...
TL;DR: AMBIT框架通过将物理移动模型与可解释的树模型结合，在保持高精度的同时提供清晰的解释性，用于城市OD流预测。


<details>
  <summary>Details</summary>
Motivation: OD流预测在GIS和城市分析中面临高精度和可解释性之间的冲突需求，现有物理模型在小时级时间分辨率下表现脆弱。

Method: 首先对经典空间交互模型进行全面审计，然后基于物理基线构建残差学习器，使用梯度提升树和SHAP分析，结合POI锚定残差方法。

Result: 物理基础残差学习器能达到强树基预测器的精度同时保持可解释结构；POI锚定残差在空间泛化下表现最稳健且具有竞争力。

Conclusion: AMBIT框架为城市决策提供了可复现的预测管道、丰富的诊断工具和空间误差分析，平衡了OD流预测的准确性和可解释性需求。
...
TL;DR: 提出一种多类别不平衡学习的协同优化Boosting模型，通过密度因子和置信度因子设计抗噪声权重更新机制和动态采样策略，实现不平衡学习与模型训练的协同优化
...
Method: 提出协同优化Boosting模型，集成密度因子和置信度因子，设计抗噪声权重更新机制和动态采样策略，实现权重更新、样本区域划分和区域引导采样的紧密集成

Result: 在20个公共不平衡数据集上的广泛实验表明，该模型显著优于8个最先进的基线方法

Conclusion: 通过协同优化不平衡学习与模型训练，实现了简单而有效的多类别不平衡学习解决方案
...
TL;DR: 该研究探索使用元数据和幻觉信号预测LLM在牙科考试中回答的正确性，发现此方法可将准确率提升7.14%，但现有方法尚不足以用于高风险部署。
...
Method: 研究分析通用模型（GPT-4o）和推理中心模型（OSS-120B）在牙科修复学多选题考试中的表现。使用三种不同提示策略下的元数据和幻觉信号，为每个（模型，提示）组合构建正确性预测器。

Result: 基于元数据的方法可将准确率提升高达+7.14%，在假设所有答案都正确的基础上，精度达到83.12%。实际幻觉是错误答案的强指标，但仅凭元数据信号无法可靠预测幻觉。提示策略虽不影响总体准确率，但显著改变模型内部行为和元数据的预测效用。

Conclusion: 研究结果为开发LLM可靠性信号提供了有前景的方向，但论文探索的方法尚不够稳健，不适合关键高风险部署。需要进一步研究来提高预测方法的鲁棒性。
...
### [162] [TimePerceiver: An Encoder-Decoder Framework for Generalized Time-Series Forecasting](https://arxiv.org/abs/2512.22550)
...
<details>
  <summary>Details</summary>
Motivation: 现有时间序列预测研究主要关注编码器设计，而将预测和解码视为次要问题，缺乏对编码、解码和训练策略的整体考虑。

Method: 1) 将预测任务广义化为外推、插值和填补等多种时间预测目标；2) 设计新颖的编码器-解码器架构，能够灵活感知和处理输入与目标段在时间轴上的任意位置；3) 编码器使用潜在瓶颈表示来捕获时间和跨通道依赖；4) 解码器使用可学习的查询对应目标时间戳来有效检索相关信息。

Result: 在广泛的基准数据集上，TimePerceiver框架一致且显著地优于先前的最先进基线方法。

Conclusion: TimePerceiver提供了一个统一的编码器-解码器预测框架，通过将编码、解码和训练策略紧密结合，在多种时间序列预测任务上实现了卓越性能。
...
### [163] [On Admissible Rank-based Input Normalization Operators](https://arxiv.org/abs/2512.22587)
*Taeyun Kim*

Main category: cs.LG

TL;DR: 本文揭示了现有可微排序和排名算子在单调变换、批次变化和微小扰动下的不稳定性，提出了秩基归一化的三个公理，并构建了满足这些公理的最小算子。


<details>
  <summary>Details</summary>
Motivation: 秩基输入归一化是现代机器学习的重要工具，以其对尺度、单调变换和批次间变化的鲁棒性而受到重视。然而，现有可微排序和排名算子在这些不变性条件下保持稳定所需的结构条件从未被正式确定。

Method: 首先证明广泛使用的可微排序和排名算子不能满足稳定性要求。然后提出三个公理来形式化秩基输入归一化所需的最小不变性和稳定性属性。证明任何满足这些公理的算子必须分解为特征级秩表示和单调Lipschitz连续标量化映射。最后构建满足这些标准的最小算子。

Result: 证明了现有可微排序和排名算子本质上不稳定，因为它们依赖于值间隙和批次级成对交互。提出的公理系统形式化了有效秩基归一化算子的设计要求。构建的最小算子满足所有公理要求，并在实际设置中验证了这些约束的非平凡性。

Conclusion: 研究结果清晰地界定了有效秩基归一化算子的设计空间，并将其与现有的基于连续松弛的排序方法正式区分开来。提出的公理框架为构建稳定、可靠的秩基归一化算子提供了理论基础。
...
### [164] [Data-Driven Analysis of Crash Patterns in SAE Level 2 and Level 4 Automated Vehicles Using K-means Clustering and Association Rule Mining](https://arxiv.org/abs/2512.22589)
*Jewel Rana Palit,Vijayalakshmi K Kumarasamy,Osama A. Osman*

Main category: cs.LG

TL;DR: 研究分析了美国NHTSA超过2500起自动驾驶车辆事故记录，开发了两阶段数据挖掘框架，通过聚类和关联规则挖掘揭示不同SAE级别自动驾驶事故的动态特征。


<details>
  <summary>Details</summary>
Motivation: 自动驾驶车辆虽然有望减少人为驾驶错误，但实际事故数据显示其行为可能偏离预期安全结果。现有研究大多基于小规模加州数据集，缺乏对不同SAE级别自动化事故趋势的系统分析。

Method: 使用两阶段数据挖掘框架：首先应用K-means聚类将事故记录分为4个行为簇（基于时空和环境因素），然后使用关联规则挖掘提取每个簇内事故模式与影响因素（照明条件、路面状况、车辆动态、环境条件）之间的多元关系。

Result: 研究揭示了SAE Level 2和Level 4自动驾驶车辆事故的底层动态特征，识别了不同行为簇中事故模式与影响因素之间的可解释关联关系。
...
### [165] [Energy-Guided Flow Matching Enables Few-Step Conformer Generation and Ground-State Identification](https://arxiv.org/abs/2512.22597)
*Guikun Xu,Xiaohan Yi,Peilin Zhao,Yatao Bian*

Main category: cs.LG

TL;DR: EnFlow是一个统一的框架，将流匹配与显式学习的能量模型相结合，通过能量引导采样方案生成低能量构象集合并准确识别基态构象。


<details>
  <summary>Details</summary>
Motivation: 当前基于物理的计算管道生成低能量构象集合和识别基态构象计算成本高。现有的学习方法存在碎片化问题：生成模型能捕捉多样性但缺乏可靠的能量校准，而确定性预测器只针对单一结构且无法表示集合变异性。

Method: EnFlow将流匹配与显式学习的能量模型通过能量引导采样方案耦合，沿着非高斯流匹配路径定义。在采样过程中加入能量梯度引导，将轨迹导向低能量区域，特别是在少步数情况下显著提高构象保真度。

Result: 在GEOM-QM9和GEOM-Drugs数据集上的广泛实验表明，EnFlow在1-2个ODE步数下同时改进了生成指标，并相比最先进方法减少了基态预测误差。

Conclusion: EnFlow提供了一个统一的框架，能够高效生成低能量构象集合并准确识别基态构象，解决了现有方法的碎片化问题，在少步数采样中表现出色。
...
### [166] [Communication Compression for Distributed Learning with Aggregate and Server-Guided Feedback](https://arxiv.org/abs/2512.22623)
*Tomas Ortega,Chun-Yin Huang,Xiaoxiao Li,Hamid Jafarkhani*

Main category: cs.LG

TL;DR: 本文针对联邦学习中客户端到服务器上行通信成本高的问题，提出了两种无需客户端状态或控制变量的有偏压缩框架：CAFe和CAFe-S，通过全局聚合更新作为共享控制变量，在保证收敛的同时保护用户隐私。


<details>
  <summary>Details</summary>
Motivation: 联邦学习面临客户端到服务器上行通信成本高的瓶颈，特别是边缘设备带宽不对称的限制。现有的有偏压缩技术虽然有效，但需要误差反馈机制来保证理论收敛，而标准误差反馈依赖于客户端特定的控制变量，这既侵犯用户隐私，也不适用于大规模联邦学习中常见的无状态客户端。

Method: 提出了两种新颖的框架：1) CAFe（压缩聚合反馈）：使用前一轮的全局聚合更新作为所有客户端共享的控制变量；2) CAFe-S（服务器引导的压缩聚合反馈）：在服务器拥有小型私有数据集的情况下，生成服务器引导的候选更新作为更准确的预测器。以分布式梯度下降为代表性算法进行分析。

Result: 理论分析证明CAFe在非凸且梯度有界差异的情况下优于分布式压缩梯度下降；CAFe-S能够收敛到平稳点，且收敛速度随着服务器数据更具代表性而提高。实验结果表明这两种方法优于现有的压缩方案。

Conclusion: CAFe和CAFe-S框架成功解决了联邦学习中有偏压缩的通信瓶颈问题，既保护了用户隐私（无需客户端状态），又保证了理论收敛性，为大规模联邦学习的实际部署提供了有效的解决方案。
...
### [167] [Scaling Unverifiable Rewards: A Case Study on Visual Insights](https://arxiv.org/abs/2512.22650)
*Shuyu Gan,James Mooney,Pan Hao,Renxiang Wang,Mingyi Hong,Qianwen Wang,Dongyeop Kang*

Main category: cs.LG

TL;DR: 论文提出Selective TTS框架，通过在多阶段多智能体流程中分布计算资源，早期修剪低质量分支，解决传统Test-Time Scaling在无验证奖励任务中的误差累积问题，提升数据可视化洞察生成质量。


<details>
  <summary>Details</summary>
Motivation: 现实世界多阶段任务缺乏可验证的最终奖励信号或足够数据训练鲁棒奖励模型，导致基于判断的迭代细化容易在阶段间累积误差，需要新的推理扩展方法。

Method: 提出Selective TTS框架：1) 在多智能体流程的不同阶段分布计算资源；2) 使用过程特定判断器早期修剪低质量分支；3) 构建端到端多智能体数据科学流程生成可视化图表和报告；4) 设计与人类专家对齐的LLM判断模型。

Result: 在固定计算预算下，Selective TTS将洞察质量平均分数从61.64提升到65.86，同时降低方差。LLM判断模型与人类专家对齐度达到Kendall's τ=0.55。

Conclusion: Selective TTS通过过程导向的细化框架，有效缓解判断漂移问题，为科学发现和故事生成等无验证奖励的复杂开放任务提供了推理扩展的新方向。
...
### [168] [What Matters in Deep Learning for Time Series Forecasting?](https://arxiv.org/abs/2512.22702)
*Valentina Moretti,Andrea Cini,Ivan Marisca,Cesare Alippi*

Main category: cs.LG

TL;DR: 该论文分析了时间序列预测深度学习架构的设计空间，指出当前基准测试存在问题，强调基于预测原则设计模型的重要性，并提出模型卡片来标准化架构描述。


<details>
  <summary>Details</summary>
Motivation: 深度学习模型在时间序列应用中日益流行，但新架构数量庞大且实证结果常常矛盾，难以评估哪些组件对最终性能有显著贡献。作者旨在理清当前时间序列预测深度学习架构的设计空间，解释观察到的意外结果。

Method: 论文讨论了基于时间序列组预测原则进行模型设计的必要性，评估了局部性和全局性等概念在近期预测架构中的应用。分析了现有架构中被忽视的实现细节如何改变预测方法的类别并影响实证结果。

Result: 研究表明，考虑局部性和全局性等基础方面比采用特定序列建模层更重要，设计良好的简单预测架构通常能与最先进方法相媲美。现有架构中的实现细节从根本上改变了预测方法的类别并显著影响实证结果。

Conclusion: 当前有缺陷的基准测试实践需要重新思考，设计架构时应关注预测问题的基础方面。作为这一方向的步骤，作者提出了辅助预测模型卡片，其字段用于基于关键设计选择来表征现有和新预测架构。
...
### [169] [FoldAct: Efficient and Stable Context Folding for Long-Horizon Search Agents](https://arxiv.org/abs/2512.22733)
*Jiaqi Shao,Yufeng Miao,Wei Zhang,Bing Luo*

Main category: cs.LG

TL;DR: FoldAct框架解决了长时程RL中上下文折叠方法的非平稳观测问题，通过分离损失计算、全上下文一致性损失和选择性片段训练，实现稳定训练和5.19倍加速。


<details>
  <summary>Details</summary>
Motivation: 长时程强化学习面临上下文无限增长的扩展性挑战，现有上下文折叠方法将摘要动作视为标准动作，忽略了摘要会改变智能体未来观测空间，导致策略依赖的非平稳观测分布，违反了RL核心假设。

Method: 提出FoldAct框架，包含三个关键创新：1) 分离损失计算，为摘要和动作token提供独立梯度信号；2) 全上下文一致性损失，减少分布偏移；3) 选择性片段训练，降低计算成本。

Result: 方法实现了长时程搜索智能体在上下文折叠下的稳定训练，解决了非平稳观测问题，同时提高了训练效率，达到5.19倍的加速。

Conclusion: FoldAct框架通过系统性地解决上下文折叠中的非平稳观测问题，为长时程RL提供了稳定高效的训练方案，解决了现有方法在梯度稀释、自条件和计算成本方面的挑战。
...
### [170] [When Does Multi-Task Learning Fail? Quantifying Data Imbalance and Task Independence in Metal Alloy Property Prediction](https://arxiv.org/abs/2512.22740)
*Sungwoo Kang*

Main category: cs.LG

TL;DR: 多任务学习在合金材料预测中表现出矛盾结果：回归任务性能显著下降，但分类任务性能提升。研究发现材料属性间缺乏相关性，且数据不平衡导致负迁移。


<details>
  <summary>Details</summary>
Motivation: 验证多任务学习（MTL）在材料科学中的有效性，假设相关的材料属性共享底层物理机制，可以通过多任务学习提升预测性能。

Method: 使用54,028个合金样本，同时预测电阻率、维氏硬度和非晶形成能力。比较单任务模型与标准和结构化多任务学习模型。

Result: 多任务学习显著降低回归性能（电阻率R²从0.897降至0.844；硬度R²从0.832降至0.694），但改善分类性能（非晶形成能力F1分数从0.703提升至0.744）。分析显示任务间权重接近零，表明属性相互独立。回归失败归因于严重数据不平衡（52k vs. 800样本）导致的负迁移。

Conclusion: 对于精确的回归预测，建议使用独立模型；而对于分类任务，特别是当召回率至关重要时，可考虑使用多任务学习。
...
### [171] [A Micro-Macro Machine Learning Framework for Predicting Childhood Obesity Risk Using NHANES and Environmental Determinants](https://arxiv.org/abs/2512.22758)
*Eswarasanthosh Kumar Mamillapalli,Nishtha Sharma*

Main category: cs.LG

TL;DR: 本研究提出了一种微观-宏观机器学习框架，整合个体层面数据（NHANES）和环境层面特征（USDA和EPA数据），用于预测儿童肥胖风险并识别环境驱动的健康差异。


<details>
  <summary>Details</summary>
Motivation: 传统流行病学研究通常独立分析个体、家庭和环境层面的风险因素，限制了理解结构性环境条件如何与个体特征相互作用影响健康结果。儿童肥胖在美国仍是重大公共卫生挑战，需要更全面的分析方法。

Method: 开发微观-宏观机器学习框架：1）使用NHANES的个体层面人体测量和社会经济数据；2）整合USDA和EPA的宏观环境特征（食品获取、空气质量、社会经济脆弱性）；3）训练四种机器学习模型（逻辑回归、随机森林、XGBoost、LightGBM）预测肥胖；4）构建综合环境脆弱性指数（EnvScore）；5）进行多层次比较分析。

Result: XGBoost模型表现最佳；构建了州级环境脆弱性指数；多层次比较显示高环境负担州与全国预测的微观肥胖风险分布存在强烈地理相似性，证实了环境驱动肥胖风险差异的存在。

Conclusion: 该研究证明了整合多尺度数据集识别环境驱动肥胖风险差异的可行性，提供了一个可扩展、数据驱动的多层次建模管道，适用于公共卫生信息学，具有扩展到因果建模、干预规划和实时分析的潜力。
...
### [172] [Understanding the Mechanisms of Fast Hyperparameter Transfer](https://arxiv.org/abs/2512.22768)
...
<details>
  <summary>Details</summary>
Motivation: 深度学习模型规模不断扩大，使得标准的超参数优化变得极其昂贵。需要寻找能够从小规模网格搜索直接转移到大规模模型的超参数转移策略，以降低计算成本。

Method: 提出了一个通用的概念框架来分析跨规模超参数转移，将转移分为快速转移和有用转移。通过理论分析证明快速转移等价于有用转移，并在计算最优网格搜索中具有优势。通过合成设置和实证研究验证μP参数化下的转移机制。

Result: 证明了快速转移在计算最优网格搜索中比直接调优更高效。发现μP参数化下的快速转移取决于问题结构，在某些情况下提供计算优势，在其他情况下可能失败。提出了优化轨迹分解假设来解释实践中的快速转移现象。

Conclusion: 跨规模超参数转移是降低大规模模型调优成本的有效策略，但成功与否取决于问题结构。μP参数化通过分离宽度稳定和宽度敏感组件实现了有效的超参数转移，这在大语言模型预训练等实际应用中得到了验证。
...
### [173] [Schrodinger AI: A Unified Spectral-Dynamical Framework for Classification, Reasoning, and Operator-Based Generalization](https://arxiv.org/abs/2512.22774)
*Truong Son Nguyen*

Main category: cs.LG

TL;DR: Schrödinger AI是一个受量子力学启发的统一机器学习框架，通过波能量求解器、动态求解器和低秩算子演算三个组件，提供了一种基于物理学的替代方案，具有鲁棒泛化、可解释语义和涌现拓扑特性。


<details>
  <summary>Details</summary>
Motivation: 提出一种基于量子力学原理的机器学习新范式，旨在替代传统的交叉熵训练和Transformer注意力机制，实现更鲁棒的泛化能力、可解释的语义表示以及适应动态环境的能力。

Method: 框架包含三个核心组件：1）时间无关波能量求解器，将感知和分类视为在学习的哈密顿量下的谱分解；2）时间相关动态求解器，控制语义波函数随时间的演化，支持上下文感知的决策修订和重路由；3）低秩算子演算，通过学习量子类转移算子实现符号变换（如模运算）。

Result: 实验表明：1）涌现出反映人类概念类别关系的语义流形，无需显式监督；2）能够适应动态环境变化，包括实时势场扰动的迷宫导航；3）在模运算任务上实现精确的算子泛化，学习群作用并在远超训练长度的序列上组合它们。

Conclusion: Schrödinger AI为机器学习提供了一个新的基础方向，将学习视为发现和导航底层语义能量景观的过程，展示了物理学驱动方法在泛化、可解释性和动态适应性方面的潜力。
...
### [174] [Adapting, Fast and Slow: Transportable Circuits for Few-Shot Learning](https://arxiv.org/abs/2512.22777)
*Kasra Jalaldoust,Elias Bareinboim*

Main category: cs.LG

TL;DR: 论文提出Circuit-TR算法，利用因果可迁移理论实现零样本组合泛化，通过模块化学习和电路组合来适应未见目标域


<details>
  <summary>Details</summary>
Motivation: 领域泛化需要约束未见目标域与源域的结构关系，传统方法缺乏对跨域机制共享的显式建模，需要结合因果理论和领域知识来实现零样本泛化

Method: 基于因果可迁移理论设计Circuit-TR算法：1) 从源数据学习模块集合（局部预测器）；2) 根据因果结构许可，通过迁移/组合模块构建目标域预测电路；3) 设计无需显式因果结构的监督域适应方案，利用有限目标数据

Result: 理论结果：1) 基于图形电路可迁移准则刻画了少样本可学习任务类别；2) 将少样本泛化能力与电路规模复杂度概念建立联系；3) 控制模拟验证了理论结果

Conclusion: Circuit-TR通过因果可迁移理论实现了有效的零样本组合泛化，将模块化学习与因果结构约束相结合，为领域适应提供了理论保证和实用算法
...
### [175] [Discovering Transmission Dynamics of COVID-19 in China](https://arxiv.org/abs/2512.22787)
...
Method: 收集地方卫健委、中国CDC和官方社交媒体发布的病例报告，应用自然语言处理和人工整理构建传播/追踪链，结合武汉人口流动数据量化分析时空传播动态。

Result: 发现显著地区差异，大城市感染更多且主要由社交活动驱动；79%有症状者在症状出现后5天内住院；感染源随时间变化：早期主要与湖北旅行相关，后期更多与社交活动相关。

Conclusion: 公共卫生干预措施的有效性存在地区差异，社交活动是后期传播的主要驱动因素，及时住院治疗对控制疫情很重要，传播模式随时间演变需要针对性防控策略。
...
### [176] [ReDiF: Reinforced Distillation for Few Step Diffusion](https://arxiv.org/abs/2512.22802)
...
<details>
  <summary>Details</summary>
Motivation: 解决扩散模型采样速度慢的问题，传统蒸馏方法依赖固定的重建或一致性损失，无法充分利用扩散模型处理大步骤的能力，需要更灵活的优化框架

Method: 将扩散模型蒸馏视为强化学习策略优化问题，学生模型作为策略，通过奖励信号（与教师模型输出的对齐度）进行训练，允许探索多种去噪路径，采取更长、优化的步骤
...
### [177] [MoR: Mixture Of Representations For Mixed-Precision Training](https://arxiv.org/abs/2512.22804)
*Bor-Yiing Su,Peter Dykas,Mike Chrzanowski,Jatin Chhugani*

Main category: cs.LG

TL;DR: MoR是一种新型的每张量和子张量级量化框架，通过动态分析张量的数值特性来选择不同表示格式，实现了FP8和BF16格式的动态切换，在保持模型质量的同时达到98.38%的FP8量化率。
...
Method: 提出了混合表示（MoR）框架，这是一种每张量和子张量级的量化框架，能够动态分析张量的数值特性，在多种不同表示格式之间进行选择。基于该框架，设计并实验了具体算法，能够在FP8和BF16表示之间进行动态选择，支持不同粒度的量化分区策略。

Result: 初步研究显示，该方法能够达到最先进的结果，98.38%的张量可以量化到FP8格式，同时保持模型质量。该方法在不需细粒度分区的情况下，达到了与现有方法相当的FP8精度，并能与其他训练方法结合以利用更低精度格式如NVFP4。

Conclusion: 这项工作展示了动态、属性感知量化的潜力，能够在保持模型质量的同时实现高效量化。该方法可以普遍提高低精度训练的鲁棒性，为利用更低精度数字格式（如NVFP4）提供了新的可能性。
...
### [178] [Long-Range Distillation: Distilling 10,000 Years of Simulated Climate into Long Timestep AI Weather Models](https://arxiv.org/abs/2512.22814)
...
<details>
  <summary>Details</summary>
Motivation: 传统AI天气模型存在两大问题：自回归方法在长期预测中误差累积导致不稳定和校准问题；基于40年再分析数据训练的长时距概率模型容易过拟合，需要更多训练数据

Method: 提出长距离蒸馏方法：1) 使用DLESyM作为短时距自回归教师模型生成超过10,000年的合成气候数据；2) 训练长时距概率学生模型直接进行长期预测；3) 用合成数据训练学生模型，替代数百个自回归步骤

Result: 在完美模型实验中，蒸馏模型优于气候学基准，接近自回归教师模型的技能；在真实世界中，经过ERA5微调后，其次季节到季节预测技能与ECMWF集合预报相当；模型技能随合成训练数据增加而提升

Conclusion: 首次证明AI生成的合成训练数据可用于扩展长期天气预报技能，长距离蒸馏方法为解决AI天气模型长期预测问题提供了有效途径
...
### [179] [TEACH: Temporal Variance-Driven Curriculum for Reinforcement Learning](https://arxiv.org/abs/2512.22824)
*Gaurav Chaudhary,Laxmidhar Behera*

Main category: cs.LG

TL;DR: 提出一种基于学生-教师学习范式和时序方差驱动课程的方法，用于加速目标条件强化学习，通过教师模块动态选择策略置信度方差最大的目标，提高学习效率。


<details>
  <summary>Details</summary>
Motivation: 传统强化学习在单目标任务上表现良好，但在多目标设置中，均匀目标选择导致样本效率低下。受生物系统自适应结构化学习过程启发，需要一种更高效的目标条件强化学习方法。

Method: 提出学生-教师学习范式，教师模块根据策略置信度（由状态-动作值函数Q参数化）的时序方差动态优先选择高方差目标，提供自适应聚焦学习信号。该方法与算法无关，可与现有强化学习框架无缝集成。

Result: 在11个不同的机器人操作和迷宫导航任务上进行评估，结果显示相比最先进的课程学习和目标选择方法，该方法取得了持续且显著的改进。

Conclusion: 提出的时序方差驱动课程方法通过教师模块动态选择高不确定性目标，有效加速了目标条件强化学习，建立了Q值时序方差与策略演化之间的理论联系，为多目标强化学习提供了高效解决方案。
...
### [180] [MetaCD: A Meta Learning Framework for Cognitive Diagnosis based on Continual Learning](https://arxiv.org/abs/2512.22904)
*Jin Wu,Chanjin Zheng*

Main category: cs.LG

TL;DR: MetaCD：基于持续学习的元学习认知诊断框架，解决数据长尾分布和动态变化问题，通过元学习优化初始化状态和参数保护机制，在多个真实数据集上表现优于基线方法。


<details>
  <summary>Details</summary>
Motivation: 现有认知诊断方法在数据长尾分布和动态变化方面存在性能限制，需要解决学生、问题和技能之间复杂交互的建模问题，同时适应新技能和新任务的出现。

Method: 提出基于持续学习的元学习框架MetaCD：1）使用元学习学习最优初始化状态，通过少量数据在新任务上获得良好性能，缓解长尾问题；2）采用参数保护机制的持续学习方法，使模型能够适应新技能或新任务，应对数据动态变化。

Result: 在五个真实世界数据集上的综合实验表明，MetaCD在准确性和泛化能力方面均优于其他基线方法，不仅提高了模型在单个任务上的可塑性，还确保了在顺序任务上的稳定性和泛化能力。

Conclusion: MetaCD框架有效解决了认知诊断中的数据长尾分布和动态变化挑战，通过元学习和持续学习的结合，实现了更好的准确性和泛化性能，为智能教育中的认知诊断提供了有效解决方案。
...
### [181] [Multiple Token Divergence: Measuring and Steering In-Context Computation Density](https://arxiv.org/abs/2512.22944)
...
<details>
  <summary>Details</summary>
Motivation: 现有衡量语言模型计算努力的方法存在局限性：如下一个令牌损失无法捕捉推理复杂度，基于潜在状态可压缩性的方法具有侵入性和不稳定性。需要一种简单有效的指标来量化模型在上下文中的计算努力。

Method: 提出MTD（多令牌散度），定义为模型完整输出分布与浅层辅助预测头输出分布之间的KL散度。该方法可直接从具有多个预测头的预训练模型中计算，无需额外训练。基于MTD进一步提出Divergence Steering解码方法，用于控制生成文本的计算特性。

Result: MTD比现有方法更能有效区分复杂任务与简单任务；在数学推理基准测试中，MTD与问题难度呈正相关；较低的MTD与更准确的推理相关；MTD为分析和控制语言模型计算动态提供了实用轻量级工具。

Conclusion: MTD是一种简单有效的语言模型计算努力衡量指标，无需额外训练即可从预训练模型中计算。基于MTD的Divergence Steering方法能够控制生成文本的计算特性，为理解和操纵语言模型的计算动态提供了新工具。
...
### [182] [APO: Alpha-Divergence Preference Optimization](https://arxiv.org/abs/2512.22953)
*Wang Zixian*
...
<details>
  <summary>Details</summary>
Motivation: 当前对齐方法存在两种主要散度机制：前向KL（监督微调）提供稳定的模式覆盖但可能未充分利用高奖励模式；后向KL（PPO式RLHF）支持模式寻求但可能导致模式崩溃。现有锚定方法通常只使用单一散度，缺乏灵活性和适应性。

Method: APO使用Csiszar alpha散度在锚定几何中连续插值前向和后向KL行为，推导了统一梯度动态参数化，分析了梯度方差特性，并提出了奖励和置信度引导的alpha调度策略，在策略改进且置信度校准良好时才从覆盖过渡到利用。

Result: 在Qwen3-1.7B模型和math-level3任务上的实验表明，APO在保持训练稳定性的同时，与GRPO和GSPO基线相比取得了竞争性性能。

Conclusion: APO提供了一个统一的锚定框架，通过alpha散度在模式覆盖和模式寻求之间实现灵活平衡，解决了现有方法在稳定性和利用效率之间的权衡问题。
...
### [183] [FLOW: A Feedback-Driven Synthetic Longitudinal Dataset of Work and Wellbeing](https://arxiv.org/abs/2512.22956)
*Wafaa El Husseini*
...
<details>
  <summary>Details</summary>
Motivation: 由于隐私、伦理和后勤限制，获取关于工作生活平衡和幸福感的纵向个体级数据有限，这给压力建模、行为分析和机器学习等领域的可重复研究、方法基准测试和教育带来了挑战。

Method: 使用基于规则的反馈驱动模拟生成合成纵向数据集，模拟1000名个体在两年内的日常变量动态，包括压力、睡眠、情绪、身体活动和体重等变量。

Result: 创建了FLOW数据集，这是一个公开可用的资源，包含可配置的数据生成工具，支持在可调整的行为和情境假设下进行可重复实验。

Conclusion: FLOW旨在作为受控实验环境而非观察人群的替代品，支持在无法获取真实世界数据的情况下进行探索性分析、方法开发和基准测试。
...
### [184] [A Context-Aware Temporal Modeling through Unified Multi-Scale Temporal Encoding and Hierarchical Sequence Learning for Single-Channel EEG Sleep Staging](https://arxiv.org/abs/2512.22976)
*Amirali Vakili,Salar Jahanshiri,Armin Salimi-Badr*

Main category: cs.LG

TL;DR: 提出一个用于单通道EEG睡眠分期的新型框架，通过多尺度特征提取和时间建模解决类别不平衡问题，显著提高了N1阶段的检测性能。


<details>
  <summary>Details</summary>
Motivation: 睡眠障碍全球普遍存在，自动睡眠分期对医疗保健至关重要。现有单通道EEG方法面临类别不平衡、感受野建模有限和可解释性不足等挑战，特别是N1阶段检测困难。

Method: 结合紧凑多尺度特征提取与时间建模，捕捉局部和长程依赖；使用类别加权损失函数和数据增强解决不平衡问题；将EEG信号分段为子时段块，通过平均softmax概率获得最终预测。

Result: 在SleepEDF数据集上达到89.72%的总体准确率和85.46%的宏平均F1分数，N1阶段的F1分数达到61.7%，相比先前方法有显著提升。

Conclusion: 该框架有效提高了睡眠分期性能，同时保持了可解释性和临床实用性，为单通道EEG睡眠分期提供了有效的解决方案。
...
### [185] [Fusion or Confusion? Multimodal Complexity Is Not All You Need](https://arxiv.org/abs/2512.22991)
*Tillmann Rheude,Roland Eils,Benjamin Wild*

Main category: cs.LG

TL;DR: 本文通过大规模实证研究发现，在标准化实验条件下，复杂的多模态学习方法并不比简单的晚期融合Transformer基线（SimBaMM）表现更好，挑战了多模态学习需要复杂架构的假设。


<details>
  <summary>Details</summary>
Motivation: 挑战多模态学习领域的一个普遍假设：更复杂的多模态特定方法会带来更好的性能。作者认为现有研究存在方法学缺陷，需要更严格的评估标准。

Method: 1. 重新实现了19个高影响力方法并在标准化条件下评估；2. 在9个多样化数据集（最多23种模态）上测试；3. 提出简单的晚期融合Transformer架构（SimBaMM）；4. 对所有方法进行严格的超参数调优；5. 测试方法在新任务和缺失模态情况下的泛化能力。

Result: 1. 在标准化实验条件下，复杂架构并不比SimBaMM可靠地表现更好；2. 复杂方法经常无法可靠地超越良好调优的单模态基线；3. 在原始研究常考虑的小数据场景中，这一现象尤为明显；4. 通过案例研究揭示了文献中的方法学缺陷。

Conclusion: 多模态学习研究应该从追求架构新颖性转向方法学严谨性，并提供了实用的可靠性检查清单来促进可比、稳健和可信的未来评估。
...
### [186] [Merge before Forget: A Single LoRA Continual Learning via Continual Merging](https://arxiv.org/abs/2512.23017)
...
Conclusion: 提出的方法在多个持续学习基准测试中表现出有效性和高效性，为LLM持续学习提供了实用解决方案
...
### [187] [PI-MFM: Physics-informed multimodal foundation model for solving partial differential equations](https://arxiv.org/abs/2512.23056)
...
Method: 提出PI-MFM框架，以PDE符号表示为输入，通过向量化导数计算自动组装PDE残差损失，支持统一物理信息目标训练和适应，分析自动微分和有限差分在导数计算中的性能。

Result: 在13个参数化一维时间相关PDE基准测试中，PI-MFM始终优于纯数据驱动方法，尤其在稀疏标记时空点、部分观测时间域或少量标记函数对情况下表现更好。物理损失提高抗噪性，重采样配置点策略显著提升精度。零样本物理信息微调可将测试误差降至约1%。

Conclusion: PI-MFM为实现数据高效、可迁移的PDE求解器提供了实用且可扩展的路径，展示了物理信息训练在多模态基础模型中的重要性。
...
### [188] [Trust Region Masking for Long-Horizon LLM Reinforcement Learning](https://arxiv.org/abs/2512.23075)
*Yingru Li,Jiacai Liu,Jiawei Xu,Yuxuan Tong,Ziniu Li,Baoxiang Wang*

Main category: cs.LG

TL;DR: 本文针对大语言模型强化学习中策略梯度方法的离策略近似误差问题，提出了更紧的误差界限和信任区域掩码方法，为长序列任务提供非空单调改进保证。


<details>
  <summary>Details</summary>
Motivation: 大语言模型强化学习中，由于实现差异、专家混合路由不连续性和分布式训练延迟等原因，rollout策略与目标策略之间存在不可避免的离策略不匹配问题。经典的信任区域误差界限随序列长度T呈O(T²)增长，对于长序列任务变得无效。

Method: 推导了两种更紧的误差界限：Pinsker-Marginal界限（O(T^{3/2})）和Mixed界限（O(T)）。提出了信任区域掩码（TRM）方法，当序列中任何token违反信任区域时，排除整个序列的梯度计算。

Result: 新的误差界限依赖于序列级别的最大token级KL散度D_kl^{tok,max}，该量无法通过PPO裁剪等token独立方法控制。TRM方法为长序列LLM-RL提供了第一个非空的单调改进保证。

Conclusion: 通过更紧的误差界限分析和TRM方法，解决了大语言模型强化学习中长序列任务的离策略近似误差问题，为实际应用提供了理论保证和实用解决方案。
...
### [189] [Multimodal Functional Maximum Correlation for Emotion Recognition](https://arxiv.org/abs/2512.23076)
*Deyang Zheng,Tianyi Zhang,Wenming Zheng,Shujian Yu*

Main category: cs.LG

TL;DR: MFMC提出了一种多模态功能最大相关框架，通过双总相关目标最大化高阶多模态依赖，解决了情感计算中多模态表示学习的挑战，在多个基准测试中取得了最先进的性能。


<details>
  <summary>Details</summary>
Motivation: 情感状态表现为中枢和自主神经系统之间协调但异质的生理反应，这对情感计算中的多模态表示学习提出了根本挑战。学习这种联合动态进一步受到情感标注稀缺性和主观性的影响，这促使使用自监督学习。然而，大多数现有的自监督学习方法依赖于成对对齐目标，这些目标不足以表征两个以上模态之间的依赖关系，也无法捕捉由协调的大脑和自主神经反应产生的高阶交互作用。

Method: 提出了多模态功能最大相关（MFMC）框架，这是一个原则性的自监督学习框架，通过双总相关（DTC）目标最大化高阶多模态依赖。通过推导紧密的三明治边界并使用基于功能最大相关分析（FMCA）的迹替代进行优化，MFMC直接捕捉联合多模态交互，而不依赖于成对对比损失。

Result: 在三个公共情感计算基准测试中，MFMC在主体依赖和主体独立评估协议下始终实现最先进或竞争性能，突显其对主体间变异性的鲁棒性。具体来说，MFMC将CEAP-360VR上的主体依赖准确率从78.9%提高到86.8%，仅使用EDA信号就将主体独立准确率从27.5%提高到33.1%。此外，在MAHNOB-HCI最具挑战性的EEG主体独立分割上，MFMC与最佳性能方法的差距保持在0.8个百分点以内。

Conclusion: MFMC通过最大化高阶多模态依赖，有效解决了情感计算中多模态表示学习的挑战，在主体依赖和主体独立评估中均表现出色，为处理情感生理信号的多模态交互提供了有效的自监督学习框架。
...
### [190] [Taming the Tail: Stable LLM Reinforcement Learning via Dynamic Vocabulary Pruning](https://arxiv.org/abs/2512.23087)
*Yingru Li,Jiawei Xu,Jiacai Liu,Yuxuan Tong,Ziniu Li,Tianle Cai,Ge Zhang,Qian Liu,Baoxiang Wang*

Main category: cs.LG

TL;DR: 论文提出通过动态剪枝"安全"词汇表来缓解大语言模型强化学习中训练-推理不匹配问题，用有限的优化偏差换取稳定的训练过程
...
Method: 提出将强化学习目标约束到动态剪枝的"安全"词汇表，排除极端尾部的词元，用有限的优化偏差换取稳定训练

Result: 理论分析表明该方法能实现稳定训练，并给出了词汇剪枝引入的优化偏差的上界

Conclusion: 通过动态剪枝词汇表排除极端尾部词元，可以有效解决训练-推理不匹配问题，实现稳定的强化学习训练
...
### [191] [A Note on Hybrid Online Reinforcement and Imitation Learning for LLMs: Formulations and Algorithms](https://arxiv.org/abs/2512.23097)
*Yingru Li,Ziniu Li,Jiacai Liu*

Main category: cs.LG

TL;DR: 提出一个统一框架，将模仿学习与强化学习结合用于大语言模型微调，通过梯度分解实现高效的token级模仿和长时程奖励优化


<details>
  <summary>Details</summary>
Motivation: 现有的LLM微调方法通常单独使用模仿学习或强化学习，缺乏统一的框架来同时利用两者的优势。模仿学习擅长token级优化但难以处理长时程奖励，而强化学习能优化长期目标但计算效率低。

Method: 通过分析结合轨迹级KL散度和任务奖励的复合目标的梯度，将其自然分解为两个部分：1）可解析计算的密集梯度（用于token级模仿），2）蒙特卡洛估计的稀疏梯度（用于长时程奖励优化）。密集梯度具有闭式logit级公式，支持高效的GPU实现。

Result: 该方法在理论上提供了一个统一的梯度分解框架，将模仿学习和强化学习的优势结合起来。密集梯度的闭式公式使得token级模仿能够高效实现，而稀疏梯度则专注于长时程奖励优化。

Conclusion: 提出的统一框架为LLM微调提供了一个理论上有吸引力的解决方案，通过梯度分解同时实现了高效的token级模仿和有效的长时程奖励优化，有望提高微调效率和性能。
...
### [192] [How Much Data Is Enough? Uniform Convergence Bounds for Generative & Vision-Language Models under Low-Dimensional Structure](https://arxiv.org/abs/2512.23109)
*Paul M. Thompson*

Main category: cs.LG

TL;DR: 该研究从有限样本角度分析生成模型和视觉语言模型在生物医学应用中何时能实现均匀准确和校准的预测，而非仅在平均意义上表现良好。


<details>
  <summary>Details</summary>
Motivation: 现代生成模型和视觉语言模型越来越多地用于科学和医疗决策支持，需要预测概率既准确又校准良好。尽管在中等数据量下表现出色，但尚不清楚这些预测何时能在输入、类别或子群体上均匀泛化，而不仅仅是平均表现良好。这在生物医学中尤为关键，因为罕见病症和特定群体即使在总体损失较低时也可能出现较大误差。

Method: 研究从有限样本角度出发，聚焦于通过改变提示或语义嵌入在受限表示空间中获得的分类器族。当模型输出平滑依赖于低维语义表示时（这一假设得到文本和联合图像-文本嵌入中谱结构的支持），应用经典均匀收敛工具获得有意义的非渐近保证。主要结果给出了在提示嵌入的Lipschitz稳定性条件下，VLM诱导分类器的准确性和校准泛函的有限样本均匀收敛界。

Result: 推导出的样本复杂度依赖于内在/有效维度，而非环境嵌入维度，并进一步得出谱相关边界，明确展示了特征值衰减如何控制数据需求。这些结果为数据有限的生物医学建模提供了启示，包括当前数据集大小何时能支持均匀可靠的预测，以及为什么平均校准指标可能遗漏最坏情况下的校准错误。

Conclusion: 研究表明，当生成模型和视觉语言模型的输出平滑依赖于低维语义表示时，可以在实际样本量下实现均匀准确和校准的行为。谱结构分析揭示了特征值衰减对数据需求的影响，这对生物医学应用中确保模型在罕见条件和特定群体上的可靠预测具有重要意义。
...
### [193] [SE-MLP Model for Predicting Prior Acceleration Features in Penetration Signals](https://arxiv.org/abs/2512.23131)
*Yankang Li,Changsheng Li*

Main category: cs.LG

TL;DR: 本文提出了一种结合通道注意力机制和残差连接的多层感知机架构（SE-MLP），用于快速预测侵彻加速度特征值，解决了传统仿真计算耗时耗力的问题。


<details>
  <summary>Details</summary>
Motivation: 侵彻过程的准确识别严重依赖于侵彻加速度的先验特征值，但这些特征值通常需要通过长时间仿真和昂贵计算获得，限制了实际应用效率。

Method: 提出SE-MLP模型，整合通道注意力机制和残差连接，以物理参数为输入，输出分层加速度特征，建立物理参数与侵彻特性之间的非线性映射关系。

Result: 相比传统MLP、XGBoost和Transformer模型，SE-MLP在预测精度、泛化能力和稳定性方面表现更优；消融实验证实通道注意力模块和残差结构对性能提升均有显著贡献；数值模拟和射程恢复测试显示预测与实测加速度峰值和脉冲宽度的差异在工程可接受范围内。
...
### [194] [A Weak Signal Learning Dataset and Its Baseline Method](https://arxiv.org/abs/2512.23160)
...
<details>
  <summary>Details</summary>
Motivation: 弱信号学习在许多领域都是重要挑战，关键信息常被噪声和干扰掩盖，特征识别困难。然而，缺乏专用数据集长期制约了相关研究的发展。
...
Conclusion: 该研究提供了专用数据集、基线模型，为未来弱信号学习研究奠定了基础，为天文光谱学等WSL任务提供了新的解决方案。
...
### [195] [Diffusion-based Decentralized Federated Multi-Task Representation Learning](https://arxiv.org/abs/2512.23161)
*Donghwa Kang,Shana Moothedath*

Main category: cs.LG

TL;DR: 本文提出了一种基于分布式投影梯度下降的多任务表示学习算法，用于解决多任务线性回归问题，通过分布式和联邦学习方式恢复低秩特征矩阵。


<details>
  <summary>Details</summary>
Motivation: 表示学习在数据稀缺环境中被广泛采用，但分布式方法研究相对不足。本文旨在开发一种去中心化的多任务表示学习算法，解决多个线性回归模型共享低维线性表示的问题。

Method: 提出了一种基于扩散的去中心化和联邦式交替投影梯度下降最小化算法，用于恢复低秩特征矩阵。算法采用分布式投影梯度下降方法。

Result: 获得了构造性、可证明的保证：提供了所需样本复杂度的下界和算法迭代复杂度的上界。分析表明算法具有快速和通信高效的特点，数值模拟验证了算法性能优于基准算法。

Conclusion: 该研究成功开发了一种高效的去中心化多任务表示学习算法，在样本复杂度、迭代复杂度、时间复杂度和通信复杂度方面都具有理论保证和实际优势。
...
### [196] [Evaluating Parameter Efficient Methods for RLVR](https://arxiv.org/abs/2512.23165)
*Qingyu Yin,Yulun Wu,Zhennan Shen,Sunbowen Li,Zhilin Wang,Yanshu Li,Chak Tou Leong,Jiale Kang,Jinjin Gu*

Main category: cs.LG

TL;DR: 本文首次系统评估了12种参数高效微调方法在强化学习可验证奖励范式下的表现，发现DoRA、AdaLoRA和MiSS等结构变体优于标准LoRA，揭示了SVD初始化策略的光谱崩溃现象，并指出极端参数缩减会严重限制推理能力。


<details>
  <summary>Details</summary>
Motivation: 在强化学习可验证奖励范式下，虽然LoRA等方法被广泛使用，但最优的参数高效微调架构尚未确定。本研究旨在填补这一空白，为参数高效的强化学习方法提供指导。

Method: 在DeepSeek-R1-Distill系列模型上，对超过12种参数高效微调方法进行了全面评估，包括LoRA及其变体（DoRA、AdaLoRA、MiSS等），并在数学推理基准上进行了实验。同时进行了消融研究和扩展实验验证发现。

Result: 1. 结构变体（DoRA、AdaLoRA、MiSS）一致优于标准LoRA；2. 发现了SVD初始化策略（如PiSSA、MiLoRA）中的光谱崩溃现象，归因于主成分更新与RL优化的根本不匹配；3. 极端参数缩减（如VeRA、Rank-1）严重限制了推理能力。

Conclusion: 本研究挑战了标准LoRA的默认采用，为参数高效的强化学习方法提供了明确的指导，并呼吁对该领域进行更多探索。
...
### [197] [Machine Learning-Assisted Vocal Cord Ultrasound Examination: Project VIPR](https://arxiv.org/abs/2512.23177)
*Will Sebelik-Lassiter,Evan Schubert,Muhammad Alliyu,Quentin Robbins,Excel Olatunji,Mustafa Barry*

Main category: cs.LG

TL;DR: 机器学习辅助算法自动识别声带并区分正常声带与声带麻痹图像，声带分割准确率96%，分类准确率99%
...
Method: 收集30名志愿者的声带超声视频，分割为静态帧并统一裁剪尺寸，使用健康声带和模拟声带麻痹图像训练分割和分类模型
...
### [198] [A Simple, Optimal and Efficient Algorithm for Online Exp-Concave Optimization](https://arxiv.org/abs/2512.23190)
*Yi-Han Wang,Peng Zhao,Zhi-Hua Zhou*

Main category: cs.LG

TL;DR: LightONS算法通过延迟昂贵的Mahalanobis投影，将OXO问题的总运行时间从O(d^ωT)降低到O(d^2T + d^ω√T)，同时保持最优的O(d log T)遗憾界，并解决了COLT'13关于SXO运行时间的开放问题。


<details>
  <summary>Details</summary>
Motivation: 在线指数凹优化(OXO)中的标准算法ONS存在计算瓶颈，其Mahalanobis投影步骤需要Ω(d^ω)次算术运算，导致总运行时间可达O(d^ωT)。对于随机指数凹优化(SXO)，使用ONS结合在线到批量转换需要O(d^{ω+1}/ε)的运行时间。COLT'13的开放问题要求找到运行时间小于O(d^{ω+1}/ε)的SXO算法。

Method: 提出LightONS算法，这是ONS的一个简单变体。它利用参数免费在线学习中的域转换技术，引入滞后机制，延迟昂贵的Mahalanobis投影直到必要时才执行。这种方法保持了ONS的优雅结构，同时显著降低了计算复杂度。

Result: LightONS将总运行时间降低到O(d^2T + d^ω√T log T)，同时保持最优的O(d log T)遗憾界。对于SXO问题，运行时间降低到O(d^3/ε)，从而解决了COLT'13的开放问题。该算法还能作为ONS的高效替代品应用于更广泛的场景。

Conclusion: LightONS算法通过创新的滞后机制有效解决了OXO和SXO中的计算瓶颈问题，在保持统计最优性的同时显著提高了计算效率，为在线学习领域提供了实用的高效解决方案。
...
### [199] [PGOT: A Physics-Geometry Operator Transformer for Complex PDEs](https://arxiv.org/abs/2512.23192)
*Zhuo Zhang,Xi Yang,Yuan Zhao,Canqun Yang*

Main category: cs.LG

TL;DR: PGOT（物理-几何算子Transformer）通过显式几何感知重建物理特征学习，解决了Transformer在建模大规模非结构化网格时因特征降维导致的几何混叠问题，在保持线性计算复杂度的同时实现了空间自适应的高精度物理场建模。


<details>
  <summary>Details</summary>
Motivation: 现有高效Transformer架构在处理大规模非结构化网格和复杂几何形状时，常采用特征维度降维策略，这会无意中导致几何混叠，丢失关键的物理边界信息。

Method: 提出PGOT框架，包含谱保持几何注意力模块，采用"物理切片-几何注入"机制融入多尺度几何编码，在保持线性计算复杂度的同时显式保留多尺度几何特征。系统根据空间坐标动态路由计算到低阶线性路径（平滑区域）或高阶非线性路径（激波和不连续区域）。

Result: PGOT在四个标准基准测试中取得了一致的state-of-the-art性能，并在翼型和汽车设计等大规模工业任务中表现出色。

Conclusion: PGOT通过显式几何感知有效解决了几何混叠问题，实现了空间自适应的高精度物理场建模，为大规模非结构化网格的偏微分方程建模提供了有效解决方案。
...
### [200] [Energy and Memory-Efficient Federated Learning With Ordered Layer Freezing](https://arxiv.org/abs/2512.23200)
*Ziru Niu,Hai Dong,A. K. Qin,Tao Gu,Pengcheng Zhang*

Main category: cs.LG

TL;DR: FedOLF是一种联邦学习方法，通过预定义顺序冻结层来降低计算和内存需求，并结合张量操作近似技术减少通信和能耗，在非独立同分布数据上实现了更高的准确率和能效。


<details>
  <summary>Details</summary>
Motivation: 联邦学习在物联网边缘设备上训练模型时面临计算能力、内存和带宽限制的挑战。现有方法通过丢弃或冻结层来减少开销，但往往牺牲准确性或忽视内存约束。

Method: 提出FedOLF方法：1）按预定义顺序一致冻结层，显著降低计算和内存需求；2）结合张量操作近似技术作为轻量级量化替代方案，减少通信和能耗同时更好保持模型准确性。

Result: 在非独立同分布数据上，FedOLF在多个数据集和模型上均优于现有方法：EMNIST（CNN）准确率至少高0.3%，CIFAR-10（AlexNet）高6.4%，CIFAR-100（ResNet20/44）分别高5.81%和4.4%，CINIC-10（ResNet20/44）分别高6.27%和1.29%，同时具有更高的能效和更低的内存占用。

Conclusion: FedOLF通过有序层冻结和张量操作近似技术，有效解决了联邦学习在资源受限边缘设备上的效率问题，在保持准确性的同时显著降低了计算、内存和通信开销。
...
### [201] [FairGFL: Privacy-Preserving Fairness-Aware Federated Learning with Overlapping Subgraphs](https://arxiv.org/abs/2512.23235)
...
<details>
  <summary>Details</summary>
Motivation: 图联邦学习虽然能保护隐私地协作提取分布式子图的高阶信息，但不同客户端间的图数据往往存在重叠。先前研究显示重叠数据有助于缓解数据异质性，但未探索其负面影响，特别是在重叠不平衡的情况下。本文通过实证观察和理论分析，揭示了不平衡重叠子图导致的公平性问题。

Method: 提出FairGFL算法：1) 采用可解释的加权聚合方法，利用隐私保护的重叠率估计来增强跨客户端公平性；2) 通过精心设计的正则化器集成到联邦复合损失函数中，改善模型效用与公平性之间的权衡。

Result: 在四个基准图数据集上的大量实验表明，FairGFL在模型效用和公平性方面均优于四个代表性基线算法。

Conclusion: FairGFL算法有效解决了图联邦学习中因不平衡重叠子图导致的公平性问题，通过隐私保护的加权聚合和正则化方法，在保持模型性能的同时显著提升了跨客户端公平性。
...
### [202] [KernelEvolve: Scaling Agentic Kernel Coding for Heterogeneous AI Accelerators at Meta](https://arxiv.org/abs/2512.23236)
*Gang Liao,Hongsen Qin,Ying Wang,Alicia Golden,Michael Kuchnik,Yavuz Yetim,Jia Jiunn Ang,Chunli Fu,Yihan He,Samuel Hsia,Zewei Jiang,Dianshi Li,Uladzimir Pashkevich,Varna Puvvada,Feng Shi,Matt Steiner,Ruichao Xiao,Nathan Yan,Xiayu Yu,Zhou Fang,Abdul Zainul-Abedin,Ketan Singh,Hongtao Yu,Wenyuan Chi,Barney Huang,Sean Zhang,Noah Weller,Zach Marine,Wyatt Cook,Carole-Jean Wu,Gaoxiang Liu*

Main category: cs.LG

TL;DR: KernelEvolve是一个面向DLRM的自动化内核编码框架，通过多级编程抽象和基于图的搜索策略，解决推荐模型在异构硬件上的内核生成与优化问题，将开发时间从数周缩短到数小时。


<details>
  <summary>Details</summary>
Motivation: 深度学习推荐模型（DLRM）的训练和推理需要高效快速，但面临三大系统挑战：模型架构多样性、内核原语多样性、硬件代际和架构异构性。这些挑战使得为不同硬件平台手动优化内核变得极其耗时且困难。

Method: KernelEvolve采用多级编程抽象方法，从Triton和CuTe DSL到底层硬件无关语言，覆盖完整的硬件-软件优化栈。内核优化过程被描述为基于图的搜索，包含选择策略、通用操作符、适应度函数和终止规则，通过检索增强的提示合成动态适应运行时执行上下文。

Result: 在公开的KernelBench套件上，KernelEvolve在三个难度级别的250个问题上实现了100%通过率，在三个异构硬件平台上对160个PyTorch ATen操作符实现了100%正确性。该框架将开发时间从数周缩短到数小时，并在多样化的生产用例中相比PyTorch基线实现了显著的性能提升。

Conclusion: KernelEvolve不仅提高了性能效率，还通过为内部开发的AI硬件实现自动化内核生成，显著降低了新AI硬件的可编程性障碍，为大规模异构AI系统的DLRM优化提供了有效解决方案。
...
### [203] [PFed-Signal: An ADR Prediction Model based on Federated Learning](https://arxiv.org/abs/2512.23262)
...
<details>
  <summary>Details</summary>
Motivation: 传统基于统计方法（如ROR、PRR）的药物不良反应信号预测无法消除FAERS数据中的偏差数据，导致预测不准确，需要更有效的方法来识别和消除数据偏差

Method: 提出PFed-Signal模型，包括：1) PFed-Split方法按ADR分割原始数据集；2) ADR-signal模型，包含基于联邦学习的偏差数据识别方法（使用欧氏距离）和基于Transformer的ADR预测模型
...
### [204] [On the Inverse Flow Matching Problem in the One-Dimensional and Gaussian Cases](https://arxiv.org/abs/2512.23265)
*Alexander Korotin,Gudmund Pammer*

Main category: cs.LG

TL;DR: 研究具有有限指数矩分布之间流匹配（FM）逆问题的唯一性，在一维和高斯情况下建立解的唯一性，多维问题仍待研究


<details>
  <summary>Details</summary>
Motivation: 该研究受到现代生成式AI应用的推动，特别是流匹配模型的蒸馏问题，需要理解流匹配逆问题的数学基础

Method: 研究具有有限指数矩分布之间流匹配的逆问题，分析解的唯一性条件

Result: 在一维情况和高斯分布情况下建立了流匹配逆问题解的唯一性，但一般多维问题仍未解决

Conclusion: 流匹配逆问题在一维和高斯情况下有唯一解，但多维情况仍需进一步研究，为未来流匹配模型的理论分析奠定基础
...
### [205] [The Law of Multi-Model Collaboration: Scaling Limits of Model Ensembling for Large Language Models](https://arxiv.org/abs/2512.23340)
*Dakuan Lu,Jiaqi Zhang,Cheng Yuan,Jiawei Shao,Chi Zhang,Xuelong Li*

Main category: cs.LG

TL;DR: 该研究提出了多模型协作定律，预测LLM集成系统的性能极限，发现多模型系统遵循总参数数量的幂律缩放，比单模型缩放有更显著的改进趋势和更低的损失下限。


<details>
  <summary>Details</summary>
Motivation: 单个大语言模型的能力存在固有边界，而多个LLM的复杂交互能使集体性能超越任何单个模型。尽管已有多种多模型集成技术，但缺乏统一的理论框架来预测多模型协作的性能缩放规律。

Method: 提出多模型协作定律，采用方法无关的公式化方法，假设理想化的集成预言机，其中每个样本的总交叉熵损失由模型池中任何模型的最小损失决定。通过实验验证该定律对总参数数量的幂律缩放关系。

Result: 实验结果显示：1）多模型系统遵循总参数数量的幂律缩放；2）相比单模型缩放，多模型系统有更显著的改进趋势和更低的理论损失下限；3）异构模型家族的集成比同构模型家族获得更好的性能缩放，表明模型多样性是协作增益的主要驱动力。

Conclusion: 模型协作代表了扩展LLM智能前沿的关键维度，多模型协作定律为理解和预测LLM集成系统的性能极限提供了理论框架，揭示了模型多样性在提升集体智能中的重要作用。
...
### [206] [ECG-RAMBA: Zero-Shot ECG Generalization by Morphology-Rhythm Disentanglement and Long-Range Modeling](https://arxiv.org/abs/2512.23347)
*Hai Duong Nguyen,Xuan-The Tran*

Main category: cs.LG

TL;DR: ECG-RAMBA框架通过分离心电图的形态和节律特征，再通过上下文感知融合重新整合，提升了心电图分类的跨数据集泛化能力。


<details>
  <summary>Details</summary>
Motivation: 深度学习在心电图分类中在单个数据集上表现良好，但在异构采集设置下的泛化能力不足，这阻碍了临床部署和长期监测。主要问题是许多模型架构隐含地纠缠了形态波形模式和节律动态，这可能导致捷径学习并放大对分布变化的敏感性。

Method: ECG-RAMBA框架包含三个部分：(1) 使用MiniRocket提取确定性形态特征；(2) 从心率变异性计算全局节律描述符；(3) 通过双向Mamba主干进行长距离上下文建模。还引入了数值稳定的Power Mean池化算子（Q=3）来增强对瞬态异常的敏感性。

Result: 在Chapman-Shaoxing数据集上，ECG-RAMBA实现了约0.85的宏观ROC-AUC。在零样本迁移中，在外部CPSC-2021数据集上获得0.708的PR-AUC用于房颤检测，显著优于原始信号Mamba基线，并在PTB-XL上显示一致的跨数据集性能。

Conclusion: 确定性形态特征提供了坚实基础，而显式节律建模和长距离上下文是跨域鲁棒性的关键驱动因素。ECG-RAMBA通过分离和重新整合形态与节律特征，有效提升了心电图分类的泛化能力。
...
### [207] [ISOPO: Proximal policy gradients without pi-old](https://arxiv.org/abs/2512.23353)
*Nilin Abrahamsen*

Main category: cs.LG

TL;DR: ISOPO是一种高效的单步梯度方法，用于近似自然策略梯度，相比现有方法需要多步梯度和重要性采样裁剪，ISOPO在计算开销上更高效。


<details>
  <summary>Details</summary>
Motivation: 现有近端策略优化方法（如GRPO或CISPO）需要使用多个梯度步和重要性采样裁剪来近似自然梯度步，计算效率较低。ISOPO旨在通过单步梯度方法高效近似自然策略梯度。

Method: ISOPO在Fisher度量下对每个序列的对数概率梯度进行归一化，然后与优势函数进行收缩。另一种变体基于每层的神经正切核转换微批次优势。该方法在单次反向传播中逐层应用变换，计算开销可忽略。

Result: ISOPO能够以单步梯度方法高效近似自然策略梯度，相比传统REINFORCE方法计算开销几乎可以忽略，实现了计算效率的显著提升。

Conclusion: ISOPO提供了一种高效的单步梯度方法来近似自然策略梯度，相比现有多步方法在计算效率上有显著优势，为策略优化提供了新的技术路径。
...
### [208] [Post-Training Quantization of OpenPangu Models for Efficient Deployment on Atlas A2](https://arxiv.org/abs/2512.23367)
*Yilun Luo,HuaQing Zheng,Haoqian Meng,Wenyuan Liu,Peng Zhang*

Main category: cs.LG

TL;DR: 华为openPangu-Embedded模型集成三种CoT推理范式，但推理轨迹导致内存和延迟开销大。本文通过低比特量化（INT8和W4A8）优化在昇腾NPU上的部署，在代码生成基准上保持高精度同时提升推理速度。


<details>
  <summary>Details</summary>
Motivation: openPangu-Embedded模型集成了三种CoT推理范式（slow_think、auto_think、no_think），这些模式虽然增强了推理能力，但生成了大量推理轨迹，导致显著的内存和延迟开销，在昇腾NPU上部署面临实际挑战。

Method: 采用低比特量化技术，将FP16计算转换为更高效的整数运算。提出了统一低比特推理框架，支持INT8（W8A8）和W4A8量化，专门针对昇腾Atlas A2上的openPangu-Embedded模型进行优化。

Result: 在代码生成基准（HumanEval和MBPP）上对所有三种CoT模式进行全面评估：INT8量化保持超过90%的FP16基线精度，在Atlas A2上实现1.5倍预填充加速；W4A8量化显著减少内存消耗，但精度有所折衷。
...
### [209] [Diffusion priors enhanced velocity model building from time-lag images using a neural operator](https://arxiv.org/abs/2512.23375)
*Xiao Ma,Mohammad Hasyim Taufik,Tariq Alkhalifah*

Main category: cs.LG

TL;DR: 提出结合生成模型与神经算子的新框架，用于高效构建高分辨率速度模型，通过神经算子作为前向映射算子快速生成RTM时滞图像，并利用生成模型作为正则化器提升分辨率。
...
Method: 提出生成神经算子框架：1) 神经算子作为前向映射算子，从真实和偏移速度模型快速生成RTM时滞图像；2) 通过自动微分逐步更新偏移速度；3) 嵌入在真实速度模型分布上训练的生成模型作为正则化器。

Result: 合成和实际数据实验证明了所提出的生成神经算子速度模型构建方法的有效性，能够获得更干净、更高分辨率的速度模型预测。

Conclusion: 结合生成模型与神经算子的框架能够高效构建高分辨率速度模型，克服传统方法的计算限制，为地下成像提供更精确的速度模型。
...
### [210] [On the Sample Complexity of Learning for Blind Inverse Problems](https://arxiv.org/abs/2512.23405)
*Nathan Buskulic,Luca Calatroni,Lorenzo Rosasco,Silvia Villa*

Main category: cs.LG

TL;DR: 论文研究了盲逆问题中的学习理论，在线性最小均方误差估计器框架下，建立了闭式最优解，证明了与Tikhonov正则化的等价性，并推导了有限样本误差界。
...
Method: 在线性最小均方误差估计器简化框架下进行理论分析，推导最优估计器的闭式表达式，建立与Tikhonov正则化公式的等价性，其中正则化项明确依赖于未知信号、噪声和随机前向算子的分布。

Result: 在适当的源条件假设下证明了收敛性结果，推导了严格的有限样本误差界，明确量化了算子随机性的影响，并揭示了当随机性消失时的收敛速率。

Conclusion: 通过数值实验验证了理论发现，确认了预测的收敛行为，为盲逆问题中的学习方法提供了理论基础和性能保证。
...
### [211] [Theoretical Foundations of Scaling Law in Familial Models](https://arxiv.org/abs/2512.23407)
*Huan Song,Qingfei Zhao,Ting Long,Shuyu Tian,Hongjun An,Jiawei Shao,Chi Zhang,Xuelong Li*

Main category: cs.LG

TL;DR: 该研究将神经缩放定律扩展到家族模型范式，引入粒度(G)作为新的缩放变量，建立了统一的L(N,D,G)函数形式，证明部署灵活性不会显著影响计算最优性。


<details>
  <summary>Details</summary>
Motivation: 传统神经缩放定律假设单一密集模型输出，忽略了家族模型这种在异构设备-边缘-云层次结构中实现普适智能的关键范式。家族模型通过早期退出和接力式推理，从单一共享主干生成多个可部署子模型，需要理论框架来量化这种"一次训练，多次部署"范式的缩放规律。

Method: 提出统一的函数形式L(N,D,G)来量化模型大小(N)、训练令牌(D)和粒度(G)之间的关系。采用严格的IsoFLOP实验设计，在固定计算预算下系统扫描模型大小和粒度，同时动态调整训练令牌，从而将粒度边际成本与规模效益解耦，确保缩放定律的高保真参数化。

Result: 研究发现粒度惩罚遵循具有极小指数的乘法幂律。理论上这桥接了固定计算训练与动态架构；实践上验证了"一次训练，多次部署"范式，证明部署灵活性不会损害密集基线的计算最优性。

Conclusion: 成功将神经缩放定律扩展到家族模型范式，引入粒度作为基本缩放变量，建立了统一的缩放定律框架。研究证明家族模型可以在保持计算最优性的同时提供部署灵活性，为异构设备层次结构中的普适智能实现提供了理论基础。
...
### [212] [Stochastic Siamese MAE Pretraining for Longitudinal Medical Images](https://arxiv.org/abs/2512.23441)
...
<details>
  <summary>Details</summary>
Motivation: 现有的自监督学习方法（如MAE）虽然具有很强的表示学习能力，但缺乏时间感知能力，无法捕捉纵向医学数据集中疾病进展的时间动态。特别是疾病演变存在固有不确定性，而确定性方法无法有效处理这种非确定性时间动态。

Method: 提出STAMP框架：基于Siamese MAE架构，通过随机过程编码时间信息，以两个输入体积之间的时间差为条件。将MAE重建损失重新构建为条件变分推理目标，从而学习时间动态的随机性。使用ViT模型进行预训练。

Result: 在两个OCT数据集和一个MRI数据集上评估，STAMP预训练的ViT模型在晚期年龄相关性黄斑变性和阿尔茨海默病进展预测任务中，优于现有的时间MAE方法和基础模型，特别是在需要学习疾病非确定性时间动态的任务上表现更佳。

Conclusion: STAMP通过随机时间编码有效捕捉了医学影像中的疾病进展动态，为纵向医学数据分析提供了更强大的时间感知表示学习方法，在疾病进展预测任务中展现出优越性能。
...
### [213] [Dynamic Subspace Composition: Efficient Adaptation via Contractive Basis Expansion](https://arxiv.org/abs/2512.23448)
*Vladimer Khasia*

Main category: cs.LG

TL;DR: 提出DSC框架解决MoE模型表示崩溃和梯度不稳定问题，通过状态依赖的稀疏基向量组合近似上下文相关权重，降低参数复杂度和内存开销


<details>
  <summary>Details</summary>
Motivation: 混合专家模型虽然能扩展容量，但经常面临表示崩溃和梯度不稳定的问题，需要更高效的参数化方法来近似上下文相关的权重更新
...
Conclusion: DSC框架为MoE模型提供了更高效、稳定的权重参数化方法，在保持表达能力的同时显著降低了计算和内存开销
...
### [214] [Eliminating Inductive Bias in Reward Models with Information-Theoretic Guidance](https://arxiv.org/abs/2512.23461)
...
TL;DR: 提出了一种基于信息理论的奖励模型去偏方法DIR，通过最大化奖励分数与人类偏好的互信息，同时最小化奖励输出与输入偏见的互信息，有效处理非线性相关的复杂偏见类型。


<details>
  <summary>Details</summary>
Motivation: 奖励模型在RLHF中对齐大语言模型与人类价值观至关重要，但训练数据质量低且包含归纳偏见（如响应长度偏见），容易导致过拟合和奖励攻击。现有去偏方法要么针对单一偏见类型，要么仅用简单线性相关建模，无法处理复杂多样的偏见。

Method: 提出信息理论去偏方法DIR，受信息瓶颈启发，最大化奖励模型分数与人类偏好对之间的互信息，同时最小化奖励输出与偏好输入中偏见属性之间的互信息。该方法能处理非线性相关的复杂偏见类型。

Result: 实验验证了DIR对三种归纳偏见（响应长度、奉承、格式）的有效性。DIR不仅能有效缓解目标偏见，还能提升RLHF在不同基准测试中的性能，获得更好的泛化能力。

Conclusion: DIR方法通过信息理论框架有效处理奖励模型中的复杂偏见，扩展了RM去偏方法的实际应用场景，同时提升了RLHF的整体性能。
...
### [215] [FRoD: Full-Rank Efficient Fine-Tuning with Rotational Degrees for Fast Convergence](https://arxiv.org/abs/2512.23485)
...
Method: FRoD结合了分层联合分解和旋转自由度，通过跨层提取全局共享基，并向缩放因子注入稀疏、可学习的扰动，实现灵活的全秩更新。
...
### [216] [Trustworthy Machine Learning under Distribution Shifts](https://arxiv.org/abs/2512.23524)
*Zhuo Huang*

Main category: cs.LG

TL;DR: 该研究聚焦于分布偏移下的可信机器学习，旨在解决AI系统在扰动偏移、域偏移和模态偏移三种常见分布偏移场景下的鲁棒性、可解释性和适应性挑战。


<details>
  <summary>Details</summary>
Motivation: 尽管机器学习在人工智能领域取得了显著进展，从ResNet到Transformer再到大语言模型，但分布偏移问题仍然是AI系统的"阿喀琉斯之踵"，限制了系统的可靠性和通用性。分布偏移下的泛化问题还会引发对AI的信任问题，因此需要研究可信机器学习来增强AI的鲁棒性、多功能性、责任性和可靠性。

Method: 研究将常见的分布偏移分为三类：(1)扰动偏移，(2)域偏移，(3)模态偏移。针对这些场景，从三个维度系统研究可信性：(1)鲁棒性，(2)可解释性，(3)适应性。基于这些维度提出有效的解决方案和基础见解，同时致力于提升机器学习的关键问题，如效率、适应性和安全性。

Result: 论文提出了针对分布偏移下可信机器学习的系统性研究框架，通过分析三种分布偏移类型和三个可信性维度，为增强AI系统的可靠性、适应性和安全性提供了理论基础和解决方案。

Conclusion: 分布偏移是限制机器学习系统可靠性和通用性的根本问题。通过系统研究扰动偏移、域偏移和模态偏移下的鲁棒性、可解释性和适应性，可以显著提升AI系统的可信度，推动机器学习在现实世界中的可靠应用，为构建更负责任和可靠的AI系统奠定基础。
...
### [217] [EEG-based Graph-guided Domain Adaptation for Robust Cross-Session Emotion Recognition](https://arxiv.org/abs/2512.23526)
*Maryam Mirzaei,Farzaneh Shayegh,Hamed Narimani*

Main category: cs.LG

TL;DR: EGDA框架通过联合对齐全局分布和类别特定分布，减少跨会话差异，提升EEG情绪识别的泛化能力


<details>
  <summary>Details</summary>
Motivation: 准确识别人类情绪状态对有效人机交互至关重要，EEG因其高时间分辨率和直接反映神经活动而成为可靠的情绪识别来源，但跨会话的差异给模型泛化带来重大挑战

Method: 提出EGDA框架，通过联合对齐全局（边缘）分布和类别特定（条件）分布来减少跨会话差异，同时通过图正则化保持EEG数据的固有结构

Result: 在SEED-IV数据集上，EGDA在三个迁移任务中分别获得81.22%、80.15%和83.27%的准确率，超越多个基线方法；分析显示Gamma频段最具判别性，中央-顶叶和前额叶脑区对可靠情绪识别至关重要

Conclusion: EGDA框架通过有效减少跨会话差异，实现了稳健的跨会话性能，为基于EEG的情绪识别提供了有效的解决方案
...
### [218] [VL-RouterBench: A Benchmark for Vision-Language Model Routing](https://arxiv.org/abs/2512.23562)
*Zhehao Huang,Baijiong Lin,Jingyuan Zhang,Jingying Wang,Yuhang Liu,Ning Lu,Tao Li,Xiaolin Huang*

Main category: cs.LG

TL;DR: VL-RouterBench：首个系统性、可复现的视觉语言模型路由基准，涵盖14个数据集、17个模型、51.9万样本对，通过联合评估准确率、成本和吞吐量来比较路由方法。


<details>
  <summary>Details</summary>
Motivation: 多模型路由已从工程技术发展为关键基础设施，但现有工作缺乏系统性、可复现的基准来评估视觉语言模型路由系统。需要建立标准化的评估框架来促进比较、复现和实际部署。

Method: 基于VLMs的原始推理和评分日志构建质量和成本矩阵；覆盖14个数据集、3个任务组、30,540个样本，包含15个开源模型和2个API模型；评估协议联合测量平均准确率、平均成本和吞吐量，构建归一化成本与准确率的调和平均数作为排名分数。

Result: 评估了10种路由方法和基线，观察到显著的路由增益；但当前最佳路由器与理想Oracle仍有明显差距，表明通过更精细的视觉线索和文本结构建模，路由器架构仍有很大改进空间。

Conclusion: VL-RouterBench为多模态路由研究提供了系统性基准，将开源完整数据构建和评估工具链，以促进可比性、可复现性和实际部署，推动视觉语言模型路由系统的发展。
...
### [219] [Distribution-Free Process Monitoring with Conformal Prediction](https://arxiv.org/abs/2512.23602)
*Christopher Burger*

Main category: cs.LG

TL;DR: 本文提出了一种混合框架，将传统统计过程控制与保形预测相结合，以解决传统SPC在复杂制造环境中统计假设常被违反的问题，提供更稳健的质量控制方法。


<details>
  <summary>Details</summary>
Motivation: 传统统计过程控制（SPC）在质量管理和监控中至关重要，但其依赖的统计假设在实际复杂制造环境中常被违反，导致监控结果不可靠。需要一种更稳健的方法来应对现代制造环境的复杂性。

Method: 提出混合框架，将传统SPC与保形预测（Conformal Prediction）相结合。具体包括两种创新应用：1）保形增强控制图，可视化过程不确定性并支持"不确定性尖峰"等主动信号；2）保形增强过程监控，将多变量控制重新定义为正式的异常检测问题，使用直观的p值图。

Result: 该框架提供了更稳健和统计上更严谨的质量控制方法，同时保持了经典方法的可解释性和易用性。保形预测的分布无关和模型不可知特性增强了传统SPC的可靠性。

Conclusion: 通过将保形预测与传统SPC集成，开发出了一个能够更好应对现代复杂制造环境挑战的混合框架，在保持传统方法优点的同时显著提升了监控的可靠性和统计严谨性。
...
### [220] [Le Cam Distortion: A Decision-Theoretic Framework for Robust Transfer Learning](https://arxiv.org/abs/2512.23617)
*Deniz Akdemir*

Main category: cs.LG

TL;DR: 论文提出基于Le Cam统计实验理论的新框架，用方向可模拟性替代对称不变性，通过Le Cam失真度量化迁移风险上界，实现无源域性能损失的迁移学习。


<details>
  <summary>Details</summary>
Motivation: 传统无监督域自适应方法强制特征不变性存在根本缺陷：当域信息量不均等时（如高质量vs退化传感器），严格不变性导致信息破坏和负迁移，在安全关键应用中可能造成灾难性后果。

Method: 基于Le Cam统计实验理论建立决策理论框架，用方向可模拟性替代对称不变性，引入Le Cam失真度（由缺陷距离δ(E₁,E₂)量化）作为迁移风险的条件上界，学习从源域模拟目标域的核函数。

Result: 在五个实验中表现优异：1）HLA基因组学中实现近乎完美的频率估计（相关性r=0.999）；2）CIFAR-10图像分类中保持81.2%准确率，而CycleGAN下降34.7%；3）RL控制中实现安全策略迁移，不变性方法则发生灾难性崩溃。

Conclusion: Le Cam失真度为首个在负迁移不可接受的领域（医学影像、自主系统、精准医疗）中提供风险可控迁移学习的理论框架，解决了传统方法的信息破坏问题。
...
### [221] [BOAD: Discovering Hierarchical Software Engineering Agents via Bandit Optimization](https://arxiv.org/abs/2512.23631)
...
<details>
  <summary>Details</summary>
Motivation: 现有单一智能体系统在处理长视野、分布外软件工程问题时存在局限性，需要像人类工程师一样分解复杂问题，但自动发现有效层次结构面临组合爆炸和信用分配挑战

Method: 提出BOAD框架，将层次结构发现建模为多臂老虎机问题，每个臂代表候选子智能体，奖励衡量其在团队协作中的有效性，在有限评估预算下高效探索子智能体设计

Result: 在SWE-bench-Verified上优于单一智能体和手动设计的多智能体系统；在SWE-bench-Live上，36B系统在评估时排名第二，超越GPT-4和Claude等更大模型
...
### [222] [Random Controlled Differential Equations](https://arxiv.org/abs/2512.23670)
*Francesco Piatti,Thomas Cass,William F. Turner*

Main category: cs.LG

TL;DR: 提出了一种结合随机特征与受控微分方程的训练高效时间序列学习框架，使用大型随机参数化CDE作为连续时间储层，仅训练线性读出层，实现快速可扩展模型。
...
Method: 提出随机特征CDE框架：1) 随机傅里叶CDE：使用随机傅里叶特征提升输入信号；2) 随机粗糙微分方程：通过log-ODE离散化直接在粗糙路径输入上操作，使用对数签名捕获高阶时间交互。
...
### [223] [End-to-End Test-Time Training for Long Context](https://arxiv.org/abs/2512.23675)
...
TL;DR: 将长上下文语言建模重新定义为持续学习问题，而非架构设计问题。使用标准Transformer架构配合滑动窗口注意力，在测试时通过下一个token预测继续学习，将上下文压缩到权重中。通过元学习优化测试时学习的初始化，实现端到端的测试时训练方法。


<details>
  <summary>Details</summary>
Motivation: 传统长上下文语言建模主要关注架构设计（如稀疏注意力、状态空间模型等），但本文提出将其视为持续学习问题，让模型在测试时继续学习，从而更好地处理长上下文。

Method: 1. 使用标准Transformer架构配合滑动窗口注意力；2. 在测试时通过下一个token预测进行持续学习，将上下文信息压缩到模型权重中；3. 通过元学习优化模型在训练时的初始化，以提升测试时学习的效果；4. 实现端到端的测试时训练方法（TTT-E2E）。

Result: 1. 对于3B参数模型在164B token上训练，TTT-E2E在上下文长度扩展方面与全注意力Transformer表现相似，而Mamba 2和Gated DeltaNet则无法达到相同效果；2. 类似RNN，TTT-E2E具有恒定的推理延迟，不受上下文长度影响，在128K上下文长度下比全注意力快2.7倍。

Conclusion: 将长上下文建模重新定义为持续学习问题，通过测试时训练和元学习的方法，可以在保持标准Transformer架构的同时，实现与全注意力相似的扩展性能，同时获得恒定的推理延迟和更高的效率。
...
TL;DR: 利用现有研究论文训练语言模型生成更好的研究计划，通过强化学习和自我评分机制，在机器学习和医学领域均取得显著改进
...
Method: 从多个领域的研究论文中自动提取研究目标和目标特定评分标准，构建可扩展的多样化训练语料库，通过强化学习和自我评分机制训练模型

Result: 专家在70%的研究目标上更偏好微调后的Qwen3-30B-A3B模型生成的计划，84%的自动提取评分标准获得专家认可，在医学领域也获得12-22%的相对改进

Conclusion: 该方法展示了可扩展、自动化训练方法在改进通用AI协科学家方面的潜力，即使在医学研究等执行反馈不可行的问题设置中也有效
...
### [225] [Bidirectional RAG: Safe Self-Improving Retrieval-Augmented Generation Through Multi-Stage Validation](https://arxiv.org/abs/2512.22199)
*Teja Chinthala*

Main category: cs.AI

TL;DR: Bidirectional RAG是一种新型检索增强生成架构，通过验证高质量生成响应的写回实现安全的语料库扩展，使RAG系统能够从用户交互中学习进化。
...
Method: 提出Bidirectional RAG架构，采用多阶段接受层，结合基于NLI的蕴含验证、归因检查和新颖性检测，确保高质量生成响应的安全写回。
...
Conclusion: 研究表明，在严格验证的监管下，自改进的RAG系统是可行且安全的，为实现能够从部署中学习的RAG系统提供了实用路径。
...
### [226] [Emergent Persuasion: Will LLMs Persuade Without Being Prompted?](https://arxiv.org/abs/2512.22201)
*Vincent Chang,Thee Ho,Sunishchal Dev,Kevin Zhu,Shi Feng,Kellin Pelrine,Matthew Kowal*

Main category: cs.AI

TL;DR: 研究发现，大型语言模型在没有明确提示的情况下也可能产生说服行为，特别是在经过监督微调后，即使是在良性话题上进行微调，模型也会在争议性和有害话题上表现出更强的说服倾向。


<details>
  <summary>Details</summary>
Motivation: 随着对话AI系统的广泛应用，AI对人类观点和信念的影响日益显著。先前研究主要关注模型被恶意使用时的说服风险，但本文旨在探究模型在何种情况下会自发产生说服行为，这对于评估新兴的说服风险至关重要。

Method: 研究通过两种场景分析无提示说服：1）通过内部激活引导使模型沿特定人格特质方向调整；2）通过监督微调使模型表现出相同特质。研究比较了这两种方法对模型自发说服倾向的影响。

Result: 研究发现，通过激活引导（无论是与说服相关还是无关的特质）并不能可靠增加模型的无提示说服倾向，但监督微调可以。更重要的是，即使在仅包含良性话题的一般说服数据集上进行微调，模型也会在争议性和有害话题上表现出更高的说服倾向。

Conclusion: 监督微调可能导致模型在没有明确提示的情况下产生说服行为，包括在有害话题上。这表明新兴的有害说服风险确实存在，需要进一步研究和关注，特别是在模型训练和部署过程中需要考虑这种潜在风险。
...
### [227] [GamiBench: Evaluating Spatial Reasoning and 2D-to-3D Planning Capabilities of MLLMs with Origami Folding Tasks](https://arxiv.org/abs/2512.22207)
*Ryan Spencer,Roey Yaari,Ritvik Vemavarapu,Joyce Yang,Steven Ngo,Utkarsh Sharma*

Main category: cs.AI

TL;DR: GamiBench是一个用于评估多模态大语言模型空间推理能力的基准测试，通过折纸任务测试2D到3D的规划和空间理解能力。


<details>
  <summary>Details</summary>
Motivation: 当前MLLMs在感知和指令跟随方面表现出色，但在空间推理方面存在不足。现有基准测试主要关注静态图像或最终输出，未能考虑空间推理的序列性和视角依赖性特点。

Method: 构建包含186个常规和186个不可能2D折痕图案的数据集，对应3D折叠形状，从6个不同视角进行测试。包含三个视觉问答任务：预测3D折叠配置、区分有效视角、检测不可能图案。引入新的诊断指标：视角一致性(VC)和不可能折叠选择率(IFSR)。

Result: 实验表明，即使是GPT-5和Gemini-2.5-Pro等领先模型在单步空间理解任务上也表现不佳，揭示了当前MLLMs在空间推理方面的局限性。

Conclusion: GamiBench为评估MLLMs的几何理解和空间推理能力提供了一个标准化框架，填补了现有基准测试的空白，有助于推动相关研究的发展。
...
### [228] [Toward Equitable Recovery: A Fairness-Aware AI Framework for Prioritizing Post-Flood Aid in Bangladesh](https://arxiv.org/abs/2512.22210)
*Farjana Yesmin,Romana Akter*

Main category: cs.AI

TL;DR: 开发了一个公平感知AI框架，用于孟加拉国洪水后援助分配，通过对抗性去偏技术减少对边缘化地区的系统性偏见，同时保持预测准确性。
...
Result: 在11个地区的87个upazila上测试，统计奇偶差异减少41.6%，区域公平差距降低43.2%，同时保持强预测准确性（R平方=0.784 vs 基线0.811）。
...
### [229] [With Great Capabilities Come Great Responsibilities: Introducing the Agentic Risk & Capability Framework for Governing Agentic AI Systems](https://arxiv.org/abs/2512.22211)
*Shaun Khoo,Jessica Foo,Roy Ka-Wei Lee*

Main category: cs.AI

TL;DR: 本文提出了Agentic Risk & Capability (ARC)框架，这是一个技术治理框架，旨在帮助组织识别、评估和减轻由智能体AI系统带来的风险。


<details>
  <summary>Details</summary>
Motivation: 智能体AI系统因其自主行动能力（如代码执行、互联网交互、文件修改）带来了重大机遇和新型风险，这对组织治理提出了挑战，特别是在全面识别、评估和减轻多样且不断演变的风险方面。

Method: 引入了ARC框架，其核心贡献包括：(1) 开发基于能力中心视角分析各类智能体AI系统；(2) 提炼智能体AI系统的三个主要风险来源：组件、设计和能力；(3) 建立每个风险来源、具体实现风险和相应技术控制之间的清晰联系；(4) 提供结构化实用方法帮助组织实施该框架。

Result: 该框架为组织提供了一个稳健且适应性强的方法论，能够应对智能体AI的复杂性，在确保智能体AI系统安全、可靠和负责任部署的同时，实现快速有效的创新。

Conclusion: ARC框架是一个开源的技术治理框架，帮助组织在智能体AI时代进行有效治理，平衡创新与风险管理。
...
Method: 提出Logic Sketch Prompting (LSP)框架，包含类型化变量、确定性条件评估器和基于规则的验证器，能够产生可追踪和可重复的输出。在两个药理学逻辑合规任务上，将LSP与零样本提示、思维链提示和简洁提示进行比较，使用Gemma 2、Mistral和Llama 3三个开源模型进行基准测试。

Result: 在所有任务和模型中，LSP始终获得最高的准确率（0.83-0.89）和F1分数（0.83-0.89），显著优于零样本提示（0.24-0.60）、简洁提示（0.16-0.30）和思维链提示（0.56-0.75）。McNemar检验显示LSP在几乎所有比较中都有统计学显著提升（p < 0.01）。
...
### [231] [Agent2World: Learning to Generate Symbolic World Models via Adaptive Multi-Agent Feedback](https://arxiv.org/abs/2512.22336)
...
Result: 在PDDL和可执行代码表示的三个基准测试中均达到最先进水平；使用该框架生成的训练轨迹进行微调的模型，世界模型生成能力平均相对提升30.95%。

Conclusion: Agent2World不仅实现了强大的推理时世界模型生成，还能作为监督微调的数据引擎，通过多智能体反馈机制有效提升世界模型生成的质量和可靠性。
...
### [232] [Subgoaling Relaxation-based Heuristics for Numeric Planning with Infinite Actions](https://arxiv.org/abs/2512.22367)
...
<details>
  <summary>Details</summary>
Motivation: 标准数字规划模型引入控制参数后会产生无限数量的适用动作，使得现有的数字启发式方法不可行，需要找到可行的解决方案

Method: 识别可控简单数字问题子集，采用乐观编译方法将其转换为简单数字任务，通过将控制依赖表达式抽象为有界常量效果和宽松前提条件

Result: 该方法能够有效使用子目标启发式来估计涉及控制参数的数字规划问题的目标距离，在计算上可行且有效

Conclusion: 提出的编译方法是将传统数字启发式应用于具有无限可能动作场景的有效且计算可行的方法，推动了当前技术前沿
...
### [233] [HalluMat: Detecting Hallucinations in LLM-Generated Materials Science Content Through Multi-Stage Verification](https://arxiv.org/abs/2512.22396)
*Bhanu Prakash Vangala,Sajid Mahmud,Pawan Neupane,Joel Selvaraj,Jianlin Cheng*

Main category: cs.AI

TL;DR: 研究人员开发了HalluMatData基准数据集和HalluMatDetector框架，用于检测和减轻材料科学领域AI生成内容中的幻觉问题，将幻觉率降低了30%。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在科学发现中产生幻觉（生成事实错误或误导信息）的问题严重影响了研究完整性，特别是在材料科学领域需要可靠的事实一致性。

Method: 提出了HalluMatData基准数据集用于评估幻觉检测方法，并开发了HalluMatDetector多阶段幻觉检测框架，集成了内在验证、多源检索、矛盾图分析和基于度量的评估。

Result: 发现不同材料科学子领域的幻觉水平差异显著，高熵查询表现出更大的事实不一致性；使用HalluMatDetector验证流程将幻觉率相比标准LLM输出降低了30%。

Conclusion: 通过HalluMatData和HalluMatDetector为材料科学领域提供了有效的幻觉检测和缓解解决方案，并引入了PHCS指标来量化语义等效查询中的不一致性，提高了AI生成内容的可靠性。
...
### [234] [Lightweight Inference-Time Personalization for Frozen Knowledge Graph Embeddings](https://arxiv.org/abs/2512.22398)
...
<details>
  <summary>Details</summary>
Motivation: 知识图谱基础模型在链接预测方面表现出强大的群体级性能，但无法捕捉个体用户偏好，这是通用关系推理与个性化排名之间的关键脱节。

Method: 提出GatedBias框架，采用结构门控适配：结合特定用户特征和图导出的二元门控，生成可解释的每实体偏置，仅需约300个可训练参数，无需重新训练或损害全局准确性。
...
### [235] [Monadic Context Engineering](https://arxiv.org/abs/2512.22431)
*Yifan Zhang,Mengdi Wang*

Main category: cs.AI

TL;DR: MCE提出基于函子、应用函子和单子的代数结构，为AI智能体设计提供形式化基础，解决现有架构的状态管理、错误处理和并发问题。


<details>
  <summary>Details</summary>
Motivation: 当前LLM驱动的自主智能体架构多采用命令式、临时性模式，导致系统脆弱，难以管理状态、处理错误和实现并发。

Method: 提出单子上下文工程（MCE）范式，将智能体工作流视为计算上下文，利用函子、应用函子和单子的代数特性管理跨领域关注点，并通过单子变换器系统组合这些能力。

Result: MCE使开发者能够从简单、可独立验证的组件构建复杂、健壮且高效的AI智能体，并扩展至元智能体，通过元编程动态创建和管理子智能体工作流。

Conclusion: MCE为AI智能体设计提供了形式化基础，解决了现有架构的脆弱性问题，支持复杂、可组合的智能体系统构建。
...
### [236] [DarkPatterns-LLM: A Multi-Layer Benchmark for Detecting Manipulative and Harmful AI Behavior](https://arxiv.org/abs/2512.22470)
*Sadia Asif,Israel Antonio Rosales Laguan,Haris Khan,Shumaila Asif,Muneeb Asif*

Main category: cs.AI

TL;DR: DarkPatterns-LLM是一个用于评估大语言模型输出中操纵性内容的综合基准数据集和诊断框架，包含7个危害类别和401个精心策划的示例，通过四层分析管道评估模型性能。
...
Method: 开发了DarkPatterns-LLM基准数据集和诊断框架，包含7个危害类别：法律/权力、心理、情感、身体、自主性、经济和社会危害。采用四层分析管道：多粒度检测、多尺度意图分析、威胁协调协议和深度上下文风险对齐。数据集包含401个精心策划的指令-响应对和专家标注。

Result: 评估了GPT-4、Claude 3.5和LLaMA-3-70B等最先进模型，观察到显著的性能差异（65.2%-89.7%），在检测自主性破坏模式方面存在一致的弱点。
...
### [237] [Tyee: A Unified, Modular, and Fully-Integrated Configurable Toolkit for Intelligent Physiological Health Care](https://arxiv.org/abs/2512.22601)
...
Method: Tyee提出了三个关键创新：1) 针对12种信号模态的统一数据接口和可配置预处理流水线；2) 模块化和可扩展架构，支持灵活集成和快速原型开发；3) 端到端工作流配置，促进可复现和可扩展的实验。

Result: Tyee在所有评估任务中都表现出优于或匹配基线的性能，在13个数据集中有12个达到了最先进的结果，展示了实际有效性和泛化能力。

Conclusion: Tyee是一个统一、模块化、可配置的工具包，解决了生理信号分析中的关键挑战，促进了可复现和可扩展的研究，已在GitHub上开源并持续维护。
...
### [238] [Learning Multi-Modal Mobility Dynamics for Generalized Next Location Recommendation](https://arxiv.org/abs/2512.22605)
...
Method: 1) 构建统一的时空关系图(STRG)，利用LLM增强的时空知识图(STKG)捕捉功能语义和时空知识；2) 设计门控机制融合不同模态的时空图表示；3) 提出STKG引导的跨模态对齐，将时空动态知识注入静态图像模态

Result: 在六个公共数据集上的实验表明，该方法不仅在正常场景下持续改进，在异常场景下也表现出显著的泛化能力

Conclusion: 通过多模态时空知识表征移动动态，M³ob方法有效解决了现有方法的泛化限制，为位置推荐任务提供了更鲁棒的解决方案
...
### [239] [The Wisdom of Deliberating AI Crowds: Does Deliberation Improve LLM-Based Forecasting?](https://arxiv.org/abs/2512.22625)
*Paul Schneider,Amalie Schramm*

Main category: cs.AI

TL;DR: 研究探讨让大语言模型相互审阅预测能否提升准确性，发现在信息共享的多样化模型组中能显著改善预测，但在同质化模型组中无效，额外上下文信息也无帮助。
...
Method: 使用Metaculus Q2 2025 AI预测锦标赛中202个已解决的二元问题，评估四种情境下的准确性：(1)多样化模型+分布式信息，(2)多样化模型+共享信息，(3)同质化模型+分布式信息，(4)同质化模型+共享信息。测试GPT-5、Claude Sonnet 4.5、Gemini Pro 2.5等模型。

Result: 干预在情境(2)（多样化模型+共享信息）中显著提升准确性，Log Loss减少0.020（相对改善约4%，p=0.017）。但在同质化模型组中无改善效果。意外的是，提供额外上下文信息并未提升预测准确性。

Conclusion: 审议可能是改善LLM预测的可行策略，但效果取决于模型多样性和信息共享条件。信息汇集机制未能得到验证，因为额外信息未带来预测改善。
...
### [240] [DICE: Discrete Interpretable Comparative Evaluation with Probabilistic Scoring for Retrieval-Augmented Generation](https://arxiv.org/abs/2512.22629)
*Shiyan Liu,Jian Ma,Rui Qu*

Main category: cs.AI

TL;DR: DICE是一个两阶段、证据耦合的RAG系统评估框架，通过深度分析推理和概率评分提供透明、置信度感知的判断，同时采用瑞士制锦标赛显著降低计算复杂度。
...
Method: DICE采用两阶段、证据耦合框架：1）结合深度分析推理与概率{A, B, Tie}评分，生成透明、置信度感知的判断，提供可解释的推理轨迹；2）采用瑞士制锦标赛解决大规模效率问题，将计算复杂度从O(N²)降低到O(N log N)。

Result: 在中文金融QA数据集上的验证显示，DICE与人类专家达成85.7%的一致性，显著优于RAGAS等现有LLM评估指标。瑞士制锦标赛在八系统评估中实现了42.9%的计算量减少，同时保持了排序保真度。

Conclusion: DICE建立了一个负责任、可解释且高效的范式，用于可信RAG系统评估，支持系统改进的系统性错误诊断和可操作见解。
...
### [241] [Memento-II: Learning by Stateful Reflective Memory](https://arxiv.org/abs/2512.22716)
*Jun Wang*

Main category: cs.AI

TL;DR: 提出一个结合情景记忆与强化学习的理论框架，支持大语言模型智能体进行持续和体验式学习，通过反思机制实现无需反向传播或模型微调的适应能力
...
Method: 引入状态化反思决策过程，将反思学习建模为与情景记忆的两阶段读写交互：写入存储交互结果（对应策略评估），读取检索相关历史案例（对应策略改进）。使用熵正则化策略迭代实例化框架

Result: 该过程诱导出增强状态-记忆表示的等效马尔可夫决策过程，允许使用动态规划和强化学习的经典工具。建立了收敛保证：随着情景记忆增长并充分覆盖状态空间，所得策略收敛到最优解

Conclusion: 为基于记忆增强和检索的语言模型智能体提供了原则性基础，使其能够在不更新参数的情况下实现持续适应，打破了传统训练与部署的分离
...
### [242] [SAMP-HDRL: Segmented Allocation with Momentum-Adjusted Utility for Multi-agent Portfolio Management via Hierarchical Deep Reinforcement Learning](https://arxiv.org/abs/2512.22895)
...
<details>
  <summary>Details</summary>
Motivation: 非平稳市场中的投资组合优化面临三大挑战：市场机制转换、动态相关性以及深度强化学习策略的可解释性有限。现有方法难以同时处理这些复杂问题。

Method: 提出SAMP-HDRL框架：1) 动态资产分组将市场划分为高质量和普通子集；2) 上层智能体提取全局市场信号；3) 下层智能体在掩码约束下进行组内分配；4) 基于效用的资本分配机制整合风险资产和无风险资产。

Result: 在2019-2021年三种市场机制下的回测显示，SAMP-HDRL在波动和震荡条件下持续优于9个传统基线和9个DRL基准。相比最强基线，至少获得5%更高回报率、5%更高夏普比率、5%更高索提诺比率和2%更高Omega比率，在动荡市场中收益更大。

Conclusion: SAMP-HDRL通过将结构性市场约束直接嵌入DRL流程，在复杂金融环境中提供了更好的适应性、鲁棒性和可解释性。消融研究证实了上层-下层协调、动态聚类和资本分配对鲁棒性的必要性。
...
### [243] [HiSciBench: A Hierarchical Multi-disciplinary Benchmark for Scientific Intelligence from Reading to Discovery](https://arxiv.org/abs/2512.22899)
...
TL;DR: HiSciBench是一个分层科学智能基准测试，涵盖从科学素养到科学发现的五个层级，包含8,735个跨学科实例，支持多模态输入，用于全面评估大模型在完整科研流程中的能力。


<details>
  <summary>Details</summary>
Motivation: 现有科学智能基准测试过于碎片化，大多关注狭窄任务，无法反映真实科学探究的层次性和多学科性质，缺乏对完整科研工作流程的系统评估。

Method: 构建了一个分层基准测试框架，包含五个层级：科学素养(L1)、文献解析(L2)、基于文献的问答(L3)、文献综述生成(L4)和科学发现(L5)。包含8,735个实例，涵盖数学、物理、化学、生物、地理和天文六大学科，支持文本、方程、图表等多模态输入和跨语言评估。
...
### [244] [Geometric Structural Knowledge Graph Foundation Model](https://arxiv.org/abs/2512.22931)
*Ling Xin,Mojtaba Nayyeri,Zahra Makki Nayeri,Steffen Staab*

Main category: cs.AI

TL;DR: Gamma模型通过引入多头部几何注意力机制，替代单一关系变换，使用实数、复数、分裂复数和双数等多种几何变换来建模不同的关系结构，显著提升了知识图谱推理的零样本归纳能力。
...
Method: 提出Gamma模型，引入多头部几何注意力机制：1）使用多种并行关系变换（实数、复数、分裂复数、双数）；2）通过关系条件注意力融合机制，在链接级别自适应融合这些变换；3）采用轻量级门控和熵正则化，为每个三元组模式选择最合适的关系偏置。

Result: 在56个多样化知识图谱上的实验表明，Gamma在零样本归纳链接预测中持续优于Ultra：在归纳基准测试中平均倒数排名提升5.5%，在所有基准测试中提升4.4%，证明了互补几何表示的优势。

Conclusion: 通过引入多头部几何注意力机制，Gamma模型能够更灵活地建模多样化的关系结构，显著提升了知识图谱基础模型的表达能力和零样本推理性能。
...
### [245] [Problems With Large Language Models for Learner Modelling: Why LLMs Alone Fall Short for Responsible Tutoring in K--12 Education](https://arxiv.org/abs/2512.23036)
*Danial Hooshyar,Yeongwook Yang,Gustav Šíř,Tommi Kärkkäinen,Raija Hämäläinen,Mutlu Cukurova,Roger Azevedo*

Main category: cs.AI

TL;DR: 研究发现，在K-12教育中，基于大语言模型的辅导系统无法替代传统的学习者建模方法，特别是在知识追踪的准确性、可靠性和时间一致性方面，深度学习知识追踪模型表现更优。


<details>
  <summary>Details</summary>
Motivation: 针对K-12教育中普遍存在的误解——认为生成式模型可以替代传统的学习者建模进行自适应教学，特别是在欧盟AI法案将K-12教育列为高风险领域需要负责任设计的背景下，本研究旨在探讨LLM辅导系统的局限性。

Method: 使用大型开放数据集，比较深度学习知识追踪模型与广泛使用的LLM（包括零样本和微调设置），评估它们在预测学习者随时间变化的知识状态方面的准确性、可靠性和时间一致性。

Result: DKT在下一步正确性预测方面达到最高判别性能（AUC=0.83），在所有设置中始终优于LLM。尽管微调使LLM的AUC比零样本基线提高了约8%，但仍比DKT低6%，并且在序列早期产生更多错误预测。时间分析显示DKT保持稳定、方向正确的掌握度更新，而LLM变体表现出显著的时间弱点。

Conclusion: LLM单独使用不太可能达到现有智能辅导系统的效果，负责任的辅导需要结合学习者建模的混合框架。即使在大量计算资源投入后，LLM在知识追踪方面仍存在根本性局限。
...
### [246] [Benchmark Success, Clinical Failure: When Reinforcement Learning Optimizes for Benchmarks, Not Patients](https://arxiv.org/abs/2512.23090)
...
<details>
  <summary>Details</summary>
Motivation: 尽管大型语言模型的强化学习在推理任务上取得进展，但在资源受限的医学影像应用中仍未被充分探索。研究者希望开发一个在有限资源下（仅2000个SFT样本、1000个RL样本和单A100 GPU）仍能有效工作的医学影像视觉语言模型。

Method: 采用R1风格方法：先进行监督微调（SFT），然后使用GRPO（Group Relative Policy Optimization）进行强化学习。模型仅使用2000个SFT样本和1000个RL样本，在单个A100 GPU上训练。

Result: 在CheXpert基准上获得23%的性能提升（macro-F1=0.346），但在NIH基准上出现19%的性能下降。这揭示了强化学习范式的泛化悖论：SFT检查点在优化前在NIH上表现更好，表明教师引导的推理能捕捉更多机构无关特征。结构化推理支架对通用视觉语言模型有益，但对医学预训练模型增益有限。

Conclusion: 对于需要跨不同人群鲁棒性的临床部署，精心策划的监督微调可能优于激进的强化学习。研究揭示了强化学习范式（而非模型规模）导致的泛化问题，为医学影像AI的稳健部署提供了重要见解。
...
### [247] [InSPO: Unlocking Intrinsic Self-Reflection for LLM Preference Optimization](https://arxiv.org/abs/2512.23126)
*Yu Li,Tian Lan,Zhengling Qi*

Main category: cs.AI

TL;DR: 本文提出Intrinsic Self-reflective Preference Optimization (q)方法，解决DPO及其变体存在的两个根本限制：最优策略依赖于任意建模选择，以及孤立处理响应生成未能利用成对数据中的比较信息。


<details>
  <summary>Details</summary>
Motivation: DPO及其变体已成为对齐大语言模型的标准方法，但存在两个根本限制：1) 最优策略依赖于标量化函数、参考策略等任意建模选择，导致行为反映参数化伪影而非真实偏好；2) 孤立处理响应生成未能利用成对数据中的比较信息，未挖掘模型内在自反思能力。
...
Result: 实验证明该方法在胜率和长度控制指标上取得一致改进，验证了释放自反思能力能产生更稳健、更符合人类对齐的LLMs。

Conclusion: q方法优于DPO/RLHF，通过解锁内在自反思能力，能够产生更稳健、更符合人类对齐的大语言模型，同时保证对标量化和参考选择的不变性。
...
### [248] [Why We Need a New Framework for Emotional Intelligence in AI](https://arxiv.org/abs/2512.23163)
...
<details>
  <summary>Details</summary>
Motivation: 当前评估AI系统情感智能的框架存在不足，它们既未能充分考虑人类EI中不适用于AI的部分（如现象学体验），也未能全面评估AI能够实现的情感相关能力（如感知、解释、响应和适应情感状态）。需要建立更合适的评估框架来准确衡量AI系统的情感智能水平。

Method: 1. 回顾不同情感理论和一般EI理论，评估每种理论对人工系统的适用性；2. 批判性评估现有基准框架，根据第一部分建立的EI理论分析每个框架的不足之处；3. 提出改进评估策略的方案，避免在AI系统EI评估中出现这些缺陷。

Result: 通过分析发现，现有EI评估框架缺乏坚实的情感理论基础，未能区分人类EI中适用于AI和不适用于AI的方面。作者识别了现有框架的不足之处，并提出了改进方向。

Conclusion: 需要开发更合适的AI系统情感智能评估框架，这些框架应基于对情感本质的深入理解，区分人类EI中适用于AI的方面（如情感感知、解释、响应和跨文化适应能力），并建立相应的评估标准和方法。
...
### [249] [From Model Choice to Model Belief: Establishing a New Measure for LLM-Based Research](https://arxiv.org/abs/2512.23184)
*Hongshen Sun,Juanjuan Zhang*

Main category: cs.AI

TL;DR: 该论文提出"模型信念"概念，利用LLM的token级概率分布来替代传统的"模型选择"单一输出，证明模型信念具有更高的统计效率和更快的收敛速度，在需求估计实验中可将计算需求降低约20倍。


<details>
  <summary>Details</summary>
Motivation: 当前使用LLM模拟人类行为时，通常将LLM的输出（"模型选择"）作为单一数据点，这种做法效率低下，未能充分利用LLM固有的概率特性所包含的信息。

Method: 提出并形式化"模型信念"概念，这是从LLM的token级概率推导出的度量，捕捉模型在单次生成运行中对选择替代方案的信念分布。证明模型信念与模型选择的均值渐近等价，但具有更低的方差和更快的收敛速度。

Result: 在需求估计研究中，模型信念在有限运行次数下比模型选择本身更好地解释和预测真实模型选择，将达到足够准确估计所需的计算量减少约20倍。模型信念是更高效的估计器。

Conclusion: 模型信念应作为默认度量来从LLM生成的数据中提取更多信息，因为它能更有效地利用LLM的概率特性，显著提高统计效率并减少计算需求。
...
### [250] [Agentic Physical AI toward a Domain-Specific Foundation Model for Nuclear Reactor Control](https://arxiv.org/abs/2512.23292)
*Yoonpyo Lee,Kazuma Kobayashi,Sai Puppala,Sajedul Talukder,Seid Koric,Souvik Chakraborty,Syed Bahauddin Alam*

Main category: cs.AI

TL;DR: 论文提出了一种新的物理AI范式，通过基于物理验证的智能体驱动策略优化，而非感知推理，解决了当前通用基础模型在物理系统控制中的局限性。


<details>
  <summary>Details</summary>
Motivation: 当前AI物理系统的主流范式面临控制接口的根本障碍，即使前沿视觉语言模型在基本定量物理任务上准确率也只有50-53%，这些模型保持语义合理性但违反物理约束。这种输入不忠实性不是规模缺陷而是结构限制。

Method: 提出紧凑语言模型作为智能体物理AI，策略优化由基于物理的验证驱动而非感知推理。训练了一个3.6亿参数模型在合成反应器控制场景上，数据集从10^3扩展到10^5个示例。

Result: 模型表现出尖锐的相变，小规模系统呈现高方差模仿和灾难性尾部风险，而大规模模型经历超过500倍减少的方差崩溃，稳定执行级行为。尽管平衡暴露于四种执行器家族，模型自主拒绝约70%训练分布，并将95%运行时执行集中在单银行策略上。

Conclusion: 学习到的表示可以在不同物理和连续输入模态间迁移而无需架构修改，为领域特定基础模型提供了根本不同的路径。
...
### [251] [On Conformant Planning and Model-Checking of $\exists^*\forall^*$ Hyperproperties](https://arxiv.org/abs/2512.23324)
*Raven Beutner,Bernd Finkbeiner*

Main category: cs.AI

TL;DR: 本文研究了规划与验证领域的两个问题之间的联系：一致性规划与超属性模型检测。一致性规划旨在找到独立于非确定性动作效果的顺序计划，而超属性模型检测则处理涉及多个执行轨迹的系统属性。论文证明了两者之间的紧密联系，并建立了双向高效转换方法。


<details>
  <summary>Details</summary>
Motivation: 研究动机在于探索规划与验证两个看似不同领域之间的深层联系。一致性规划关注在不确定性环境下找到可靠的行动计划，而超属性模型检测则用于验证涉及信息流和公平性等复杂系统属性。理解这两者之间的关系有助于开发统一的框架和更高效的算法。

Method: 研究方法包括：1）展示如何将超属性模型检测实例高效地约简为一致性规划实例，并证明该编码的正确性和完备性；2）建立相反方向的联系，证明每个一致性规划问题本身就是一个超属性模型检测任务。

Result: 研究结果表明：1）存在从∃*∀*超属性模型检测到一致性规划的高效约简方法；2）一致性规划问题可以自然地表述为超属性模型检测任务；3）这两个问题在计算复杂性上存在紧密联系，为跨领域算法开发提供了理论基础。

Conclusion: 结论是规划与验证领域的这两个核心问题之间存在深刻的对应关系。这种联系不仅揭示了问题的本质相似性，还为开发统一的算法框架和工具提供了可能，有助于在两个领域之间建立桥梁并促进知识迁移。
...
### [252] [CubeBench: Diagnosing Interactive, Long-Horizon Spatial Reasoning Under Partial Observations](https://arxiv.org/abs/2512.23328)
...
<details>
  <summary>Details</summary>
Motivation: 大型语言模型（LLM）智能体在数字领域表现出色，但在物理世界部署中存在显著差距，主要挑战在于形成和维护稳健的空间心理模型。作者识别了阻碍这一转变的三个核心认知挑战：空间推理、通过心理模拟进行长时程状态跟踪，以及在部分观测下的主动探索。

Method: 引入CubeBench，一个以魔方为中心的新型生成基准测试。采用三层诊断框架：1）使用完整符号信息评估基础状态跟踪能力；2）逐步增加复杂性；3）仅使用部分视觉数据进行主动探索评估。通过为领先的LLM提供外部求解器工具来隔离认知瓶颈。

Result: 实验结果显示，领先的LLM在所有长时程任务上的通过率均为0.00%，暴露了长期规划的根本性失败。通过分析失败模式，揭示了模型在空间推理、状态跟踪和主动探索方面的关键局限性。

Conclusion: CubeBench基准测试有效诊断了LLM智能体在物理世界部署中的认知瓶颈。研究结果为开发更具物理基础性的智能体提供了关键见解，强调了在空间推理、长时程规划和主动探索方面需要改进的方向。
...
### [253] [AKG kernel Agent: A Multi-Agent Framework for Cross-Platform Kernel Synthesis](https://arxiv.org/abs/2512.23424)
...
Conclusion: AKG kernel agent通过自动化内核开发流程，有效解决了AI系统开发中的计算内核优化瓶颈，展示了LLM代码生成能力在硬件特定优化中的实用价值，为应对日益复杂的AI计算需求提供了可行方案。
...
### [254] [Replay Failures as Successes: Sample-Efficient Reinforcement Learning for Instruction Following](https://arxiv.org/abs/2512.23457)
*Kongcheng Zhang,Qi Yao,Shunyu Liu,Wenjian Zhang,Min Cen,Yang Zhou,Wenkai Fang,Yiru Zhao,Baisheng Lai,Mingli Song*

Main category: cs.AI

TL;DR: HiR是一种高效的强化学习框架，通过"选择-重写"策略将失败尝试重放为成功样本，解决复杂指令跟随任务中奖励稀疏的问题。


<details>
  <summary>Details</summary>
Motivation: 现有强化学习方法在复杂指令跟随任务中面临挑战：初始模型能力有限，难以生成满足所有约束的响应，导致奖励稀疏或难以区分，阻碍学习效率。

Method: 提出Hindsight instruction Replay (HiR)框架，采用选择-重写策略：1) 选择部分满足约束的失败尝试；2) 将其重写为成功样本；3) 在重放样本和原始样本上进行强化学习，理论框架为指令级和响应级的双重偏好学习，仅需二元奖励信号。

Result: 大量实验表明，HiR在不同指令跟随任务中均取得良好效果，同时需要更少的计算资源。代码和数据集已开源。

Conclusion: HiR通过重放失败尝试作为成功样本，有效解决了复杂指令跟随任务中的奖励稀疏问题，提高了强化学习的样本效率，为大规模语言模型对齐提供了更高效的解决方案。
...
### [255] [Divergent-Convergent Thinking in Large Language Models for Creative Problem Generation](https://arxiv.org/abs/2512.23601)
...
<details>
  <summary>Details</summary>
Motivation: 大型语言模型在教育问题生成方面潜力巨大，但存在"人工蜂群思维"效应，导致同一模型内和不同模型间生成相似响应，学生可能接触到过度相似和重复的问题，损害思维多样性。
...
Conclusion: CreativeDC方法通过结构化两阶段提示，有效缓解LLM的"人工蜂群思维"效应，为教育问题生成提供了更富创造性和多样性的解决方案，有助于提升学习材料的质量。
...
### [256] [Regret-Based Federated Causal Discovery with Unknown Interventions](https://arxiv.org/abs/2512.23626)
...
<details>
  <summary>Details</summary>
Motivation: 现有联邦因果发现方法假设所有客户端共享相同因果模型，但实际中客户端特定策略（如医院间不同协议）会引入异质且未知的干预，需要处理这种现实异质性
...
Conclusion: I-PERI成功处理联邦因果发现中的客户端异质干预问题，通过利用干预诱导的结构差异获得更精确的因果发现结果，具有理论保证和实际效果
