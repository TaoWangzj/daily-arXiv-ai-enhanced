...
Motivation: 多媒体制作中需要对每个声源单独处理音频轨道以实现精确编辑、混音和创意控制，但现有方法只能一次性生成混合音频，因为视觉特征纠缠且区域提示往往无法准确指定声源。

Method: 提出SelVA模型，将文本提示作为目标声源的显式选择器，调制视频编码器以提取与提示相关的视频特征；使用补充标记通过抑制文本无关激活来促进跨注意力；采用自增强方案解决单声道音频监督数据缺乏的问题。
...
TL;DR: 该论文提出MRD框架，通过多分辨率语义融合和开放词汇目标检测来解决高分辨率图像理解中目标对象被分割到不同图像块导致语义相似性偏差的问题。


<details>
  <summary>Details</summary>
Motivation: 现有方法将高分辨率图像分割成小块进行处理，但这种方法会将完整的目标对象分割到多个图像块中，从而破坏语义相似性的计算，导致理解效果不佳。

Method: 提出MRD框架：1）多分辨率语义融合方法，整合不同分辨率下获得的语义相似性图，产生更准确的语义信息并保持目标对象的完整性；2）引入开放词汇目标检测模型，通过滑动窗口方法在全局尺度上直接定位目标对象。

Result: 在不同多模态大语言模型上进行的高分辨率图像理解基准测试表明，该方法具有有效性。

Conclusion: MRD框架通过多分辨率处理和开放词汇检测，有效解决了高分辨率图像理解中目标对象分割导致的语义相似性偏差问题，提高了理解准确性。
...
TL;DR: 本文提出了一种上下文增强的对比损失函数，通过包含两个收敛目标来提高学习效果并解决信息失真问题，在8个大规模基准数据集上优于16种最先进的对比学习方法。


<details>
  <summary>Details</summary>
Motivation: 对比学习在众多大规模基准测试中表现出色，但对比损失函数在通过旋转或裁剪等技术辨别样本相似性时，可能会引入来自增强样本的信息失真。训练模型可能过度依赖相同标签样本的信息，而忽略来自同一原始图像的正样本对，特别是在大规模数据集中。

Method: 提出上下文增强的对比损失函数，包含两个收敛目标：1) 对标签对比敏感的组件，区分相同和不同类别的特征，提高对比训练效率；2) 拉近来自同一源图像的增强样本，同时远离所有其他样本。

Result: 在CIFAR10、CIFAR100、Caltech-101、Caltech-256、ImageNet、BiasedMNIST、UTKFace和CelebA等8个大规模基准数据集上评估，实验结果表明该方法在泛化性能和学习收敛速度方面优于16种最先进的对比学习方法。在BiasedMNIST数据集上相比原始对比损失函数有22.9%的改进。
...
TL;DR: 本文提出了一种联合评估文本到图像生成模型和视觉语言模型的结构化方法，通过测试VLM能否识别T2I模型生成的图像中的27种特定错误模式，并创建了一个包含5个T2I模型生成图像和3个VLM标注的数据集。


<details>
  <summary>Details</summary>
Motivation: 当前T2I模型虽然能生成视觉上令人印象深刻的图像，但经常无法准确捕捉用户提示中的特定属性（如正确数量的对象和指定颜色）。同时，VLM的基准测试未能跟上VLM用于标注的复杂场景的需求。需要一种分层评估框架来比较不同图像生成模型的提示遵循能力。

Method: 提出结构化方法联合评估T2I模型和VLM：1）测试VLM能否识别T2I模型生成的图像中的27种特定错误模式；2）创建包含5个T2I模型（Flux、SD3-Medium、SD3-Large、SD3.5-Medium、SD3.5-Large）生成图像和3个VLM（Molmo、InternVL3、Pixtral）标注的数据集，使用LLM（Llama3）进行标注以测试VLM是否正确识别图像中的错误模式。

Result: 通过分析精心设计的提示集上的错误模式，揭示了属性保真度和对象表示方面的系统性错误。研究发现当前指标不足以捕捉这些细微错误，强调了针对性基准测试对于推进生成模型可靠性和可解释性的重要性。

Conclusion: 本文提出的联合评估框架和数据集揭示了T2I模型和VLM在复杂场景理解方面的局限性，强调了需要更精细的评估方法来提高生成模型的可靠性和可解释性，为未来研究提供了重要的基准测试工具。
...
TL;DR: LLOST模型使用双变分自编码器通过共享潜在空间将医学图像点云表示与体细胞突变计数关联起来，实现从影像预测癌症基因突变


<details>
  <summary>Details</summary>
Motivation: 癌症治疗有效性依赖于早期诊断和治疗，医学影像和基因信息在癌症诊断中分别用于初步诊断和治疗选择，需要建立两者之间的关联模型

Method: 1. 引入病灶图像的点云表示以实现成像模态不变性；2. 提出LLOST模型，包含双变分自编码器，通过共享潜在空间统一病灶点云特征和体细胞突变计数特征；3. 使用条件归一化流先验学习三个潜在空间的分布

Result: 在TCIA医学影像和TCGA Pan Cancer体细胞突变数据集上验证，模型能预测特定突变计数和突变发生，识别影像与体细胞突变域之间的共享模式反映癌症类型

Conclusion: 模型展示了从医学影像预测基因突变的潜力，未来可改进模型并扩展到其他遗传学领域
...
TL;DR: SplatSuRe：一种选择性应用超分辨率的方法，只在缺乏高频监督的欠采样区域添加SR内容，相比传统均匀应用SR的方法，能产生更清晰、更一致的结果。


<details>
  <summary>Details</summary>
Motivation: 3D高斯溅射（3DGS）支持高质量的新视角合成，但需要生成比训练时更高分辨率的渲染。传统方法对所有图像均匀应用超分辨率会导致多视角不一致和模糊渲染。

Method: 提出SplatSuRe方法，关键洞察是近距离低分辨率视图可能包含远处视图也捕捉到区域的高频信息，利用相机姿态相对于场景几何的信息来确定在哪里添加SR内容，只在缺乏高频监督的欠采样区域选择性应用SR。

Result: 在Tanks & Temples、Deep Blending和Mip-NeRF 360数据集上，该方法在保真度和感知质量上都超越了基线方法，特别是在需要更高细节的局部前景区域表现最为显著。

Conclusion: 选择性应用超分辨率内容的方法比均匀应用SR的方法更有效，能产生更清晰、更一致的渲染结果，尤其在需要高细节的区域表现突出。
...
TL;DR: AV-SpeakerBench是一个包含3212个多选题的基准测试，专注于真实世界视频中的说话者中心视听推理，评估模型能否对齐"谁说话、说什么、何时说"这三个维度。


<details>
  <summary>Details</summary>
Motivation: 现有的多模态大语言模型需要联合理解视觉、音频和语言，但现有视频基准测试很少评估对人类语音的细粒度推理。许多任务要么可以通过视觉解决，要么只粗粒度评估语音，无法深入了解模型是否能对齐说话者、内容和时间。

Method: 设计了AV-SpeakerBench基准测试，具有三个核心特征：(1) 说话者中心化表述，将说话者而非场景作为核心推理单元；(2) 融合基础的问题设计，将视听依赖嵌入问题语义中；(3) 专家标注，确保时间精度和跨模态有效性。

Result: 综合评估显示，Gemini系列模型始终优于开源系统，其中Gemini 2.5 Pro表现最佳。在开源模型中，Qwen3-Omni-30B接近Gemini 2.0 Flash，但仍远落后于Gemini 2.5 Pro，主要原因是视听融合能力较弱而非视觉感知。

Conclusion: AV-SpeakerBench为未来多模态系统的细粒度视听推理建立了严格的评估基础，有助于推动模型在说话者中心化推理方面的发展。
...
TL;DR: SPF是一种新的流匹配方法，通过时空金字塔结构实现高效并行气候模拟，在ClimateBench上超越现有基线，并创建了最大的地球系统模拟数据集ClimateSuite。


<details>
  <summary>Details</summary>
Motivation: 现有生成模型依赖天气尺度自回归进行气候模拟，这种方法对于长期气候模拟速度慢，且在非平稳强迫下无法稳定展开。需要一种能够高效、并行模拟多时间尺度气候的方法。

Method: 提出时空金字塔流（SPF），这是一种新的流匹配方法，将生成轨迹划分为时空金字塔结构，逐步增加空间分辨率以减少计算量，每个阶段与特定时间尺度耦合，允许在金字塔的任何时间层级直接采样，并通过物理强迫条件进行调节。

Result: 在ClimateBench上，SPF在年和月时间尺度上超越了强大的流匹配基线和预训练模型，同时提供快速采样，特别是在较粗的时间层级。创建了ClimateSuite数据集（包含33,000多个模拟年），并发现扩展的SPF模型在跨气候模型的外推场景中表现出良好的泛化能力。
...
<details>
  <summary>Details</summary>
Motivation: 当前文本到视频模型在时间生成方面表现出色，但其在图像修复方面的潜力尚未充分探索。本研究旨在利用这些模型的时间生成能力来实现渐进式视觉修复。

Method: 1. 构建合成数据集（超分辨率、去模糊、低光增强），每个样本包含从退化到清晰帧的渐进过渡；2. 微调CogVideo模型生成修复轨迹；3. 比较两种提示策略：统一文本提示和通过LLaVA多模态LLM生成、ChatGPT优化的场景特定提示。

Result: 微调后的模型能够将时间进展与修复质量关联，生成的序列在PSNR、SSIM和LPIPS等感知指标上逐帧改善。模型有效恢复空间细节和光照一致性，同时保持时间连贯性，并在ReLoBlur数据集上表现出零样本鲁棒性。

Conclusion: 文本到视频模型可以成功重新用于渐进式视觉修复任务，通过时间轨迹生成实现高质量的图像修复，并具有良好的泛化能力和可解释性。
...
Method: 提出MORP-Synth两阶段框架：阶段A使用形态学区域扰动生成油污和类似物的几何变化；阶段B使用条件生成INADE模型从编辑后的掩码渲染SAR纹理。

Result: 模型在地中海数据上预训练后在秘鲁域上的mIoU从67.8%降至51.8%；使用MORP-Synth后性能提升达+6 mIoU，少数类IoU显著提升（油污+10.8，类似物+14.6）。

Conclusion: MORP-Synth能有效改善SAR油污分割模型的跨区域泛化能力，特别是在标记数据稀缺的区域，通过合成数据增强显著提升模型性能。
...
TL;DR: 该论文发现预训练视频扩散模型在去噪早期阶段学习到了适合跟踪的运动表征，利用这一发现开发了自监督跟踪器，在视觉相似物体区分方面显著优于现有方法
...
Method: 利用预训练视频扩散模型内在学习的运动表征进行跟踪。研究发现扩散模型的去噪过程在早期高噪声阶段分离出运动信息，与后期的外观细化不同。基于这一发现构建自监督跟踪器

Result: 在标准基准测试和新设计的视觉相似物体跟踪测试中，相比近期自监督方法获得高达6分的性能提升。可视化证实扩散模型衍生的运动表征能够在挑战性视角变化和变形下鲁棒跟踪甚至完全相同的物体

Conclusion: 预训练视频扩散模型无需任务特定训练即可学习到有效的运动表征，为自监督跟踪提供了新途径，特别在视觉相似物体区分这一现有方法的薄弱环节上表现出色
...
<details>
  <summary>Details</summary>
Motivation: 3D视觉基础模型在未标定图像重建中表现良好，但在在线场景（如驾驶）中，时间窗口内的预测难以保持一致性。现有方法通过求解全局变换对齐连续预测，但存在假设有效性、局部对齐范围和噪声几何鲁棒性等根本限制
...
TL;DR: 本文通过训练无关剪枝分析统一多模态模型的组件效率，发现理解组件可压缩性强而生成组件敏感，提出基于稀疏激活的MoE适配方法，使BAGEL模型仅激活约一半参数即可达到全模型性能。


<details>
  <summary>Details</summary>
Motivation: 统一多模态模型虽然整合了理解和生成能力，但引入了推理效率问题。特定任务或样本可能不需要模型的全部知识或容量，而目前对这些低效性在不同组件中如何表现的系统性理解还很有限。

Method: 1) 使用训练无关剪枝作为探测方法，对统一多模态模型组件进行系统性分析，包括深度剪枝和宽度缩减；2) 提出混合专家(MoE)适配方法，将生成模块划分为多个专家并启用稀疏激活来恢复生成质量；3) 通过专家冻结调优验证稀疏激活有效性，并展示完全可训练的适配能带来额外增益。

Result: 研究发现：理解组件在理解和生成任务中都表现出显著的可压缩性，在生成任务中更为明显；生成组件对压缩高度敏感，即使中等压缩比例也会导致性能急剧下降。提出的MoE适配方法使BAGEL模型仅激活约一半参数即可达到与完整模型相当的性能。

Conclusion: 统一多模态模型存在组件效率差异，理解组件可压缩而生成组件敏感。通过稀疏激活的MoE适配方法能有效提升推理效率，在保持性能的同时显著减少激活参数，为高效多模态模型设计提供了新思路。
...
TL;DR: 提出弱监督无标定多视角人群计数方法WSCF-MVCC，仅使用人群数量作为监督信号，无需密度图标注和相机标定，在三个数据集上优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有多视角人群计数方法需要昂贵的密度图标注和相机标定，而无标定方法仍需图像级人群标注。需要开发更实用、标注成本更低的方法。
...
Conclusion: WSCF-MVCC通过弱监督和无标定设计，显著降低了标注成本，在多视角人群计数任务中表现出优越性能，具有更好的实用价值。
...
<details>
  <summary>Details</summary>
Motivation: 视觉语言模型主要依赖大规模真实数据或合成多样性，对基本感知任务处理能力有限。由于预训练和微调成本高昂，继续在增强数据上训练效果有限且回报递减。

Method: 提出VACoT框架，在模型推理时动态调用图像增强，结合去噪等后处理变换。采用结构化视觉增强集合，通过高效的智能体强化学习减少训练复杂度和计算开销，并设计条件奖励机制平衡增强需求和响应简洁性。

Result: 在13个感知基准测试中表现出优越性，特别是在OCR相关的对抗场景中显著提升鲁棒性。引入AdvOCR基准进一步展示了后处理视觉增强在对抗性场景中的泛化优势。

Conclusion: VACoT通过推理时动态图像增强有效提升了视觉语言模型在挑战性和分布外输入上的鲁棒性，特别是在对抗性OCR场景中，同时减少了训练复杂度和计算开销。
...
TL;DR: 本研究探索了使用预训练的ResNet-50和SqueezeNet模型通过胸部X光图像诊断结核病。SqueezeNet在各项指标上表现优于ResNet-50，展示了机器学习在结核病检测中的潜力。


<details>
  <summary>Details</summary>
Motivation: 结核病是一种持续影响人类数千年的传染病，传统诊断方法（如痰涂片镜检和培养）效率低下，特别是在资源有限的环境中。这促使研究者探索深度学习和计算机视觉等先进技术来改善结核病诊断。

Method: 研究使用Kaggle数据集（包含4,200张胸部X光图像），采用预训练的ResNet-50模型和通用的SqueezeNet模型进行结核病诊断。预处理包括数据分割、增强和调整大小以提高训练效率。使用准确率、精确率、召回率和混淆矩阵等指标评估模型性能。

Result: SqueezeNet模型表现优异：损失32%，准确率89%，精确率98%，召回率80%，F1分数87%。相比之下，ResNet-50模型表现较差：损失54%，准确率73%，精确率88%，召回率52%，F1分数65%。

Conclusion: 研究强调了机器学习在结核病检测中的潜力，特别是在早期识别和治疗启动方面。将此类模型集成到移动设备中可扩展其在缺乏结核病检测资源地区的应用。尽管结果令人鼓舞，但仍需继续开发更快、更小、更准确的结核病检测模型，为全球抗击结核病做出贡献。
...
TL;DR: 本文提出了一种新颖的无地图轨迹预测算法，通过时域、空域和频域的多域处理，结合专家混合机制和选择性注意力模块，有效处理复杂交互场景中的轨迹预测问题。


<details>
  <summary>Details</summary>
Motivation: 自动驾驶系统中轨迹预测对可靠性和安全性至关重要，但现有方法在处理复杂交互场景时存在效率低下和精度不足的问题，难以从冗余数据中有效提取有价值的场景信息。

Method: 1. 使用时域信息处理中的专家混合机制自适应选择关键频率分量；2. 提取这些分量并集成多尺度时域特征；3. 提出选择性注意力模块过滤时域序列和空间交互中的冗余信息；4. 设计多模态解码器，在补丁级和点级损失监督下获得合理轨迹结果。

Result: 在Nuscences数据集上的实验证明了该算法的优越性，验证了其在处理复杂交互场景中的有效性。

Conclusion: 该研究提出的无地图轨迹预测算法通过多域处理和自适应特征选择机制，显著提升了复杂交互场景下的预测精度和计算效率，为自动驾驶系统的可靠性和安全性提供了有效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现实场景中由于隐私和安全约束，无法访问模型参数和架构细节，传统微调方法受限，需要不修改模型权重的输入级策略来提升领域泛化能力。

Method: SAGE通过风格迁移构建源域多样风格表示，学习覆盖广泛视觉特征的风格特征集，然后根据每个输入的视觉上下文自适应融合这些风格线索，形成动态提示来协调图像外观而不触及模型内部。

Result: 在五个基准数据集上的广泛实验表明，SAGE在隐私约束下实现了与最先进方法相当或更优的性能，在所有设置中都优于完全微调的基线方法。

Conclusion: SAGE通过闭环设计有效弥合了冻结模型不变性与未见领域多样性之间的差距，为隐私约束下的语义分割领域泛化提供了有效解决方案。
...
TL;DR: 提出On-the-fly Feedback SfM框架，实现无人机实时摄影测量，通过探索-利用机制在飞行中迭代优化三维重建质量，减少覆盖漏洞和重飞成本。
...
Method: 基于SfM on-the-fly，集成三个模块：1)在线增量粗网格生成；2)在线网格质量评估与可操作指标；3)预测性路径规划用于轨迹实时优化。

Result: 方法实现近实时原位重建和评估，提供可操作反馈显著减少覆盖漏洞和重飞成本，代码已开源。

Conclusion: 该框架通过数据采集、处理、三维重建评估和在线反馈的集成，为从传统被动工作模式向智能自适应探索工作流转变提供替代方案。
...
TL;DR: FDTA提出了一种从检测到关联的显式特征细化框架，通过空间、时间和身份适配器增强目标嵌入的判别性，解决端到端多目标跟踪中关联准确率低的问题。
...
TL;DR: 研究人员开发了一种无需人工标注的相机引导雷达标注流程，通过将雷达点云投影到相机语义分割结果并应用空间聚类，显著提高了雷达标注准确性，为4D雷达语义分割研究提供了可复现框架。


<details>
  <summary>Details</summary>
Motivation: 4D雷达在恶劣条件下具有强大的环境感知潜力，但雷达语义分割研究受到开源数据集和标注稀缺的限制。RaDelft数据集虽然开创性，但仅提供LiDAR标注且无公开代码生成雷达标注，限制了研究的可复现性和下游应用。

Method: 开发了相机引导的雷达标注流程：将雷达点云投影到基于相机的语义分割结果中，然后应用空间聚类技术，从而生成准确的雷达点云标注，无需依赖人工标注。

Result: 成功复现了RaDelft小组的数值结果，并证明相机引导的雷达标注流程能生成准确的雷达点云标注。这些标注显著提高了雷达标注的准确性，为研究社区提供了可训练和评估的标注4D雷达数据框架。此外，研究还量化了不同雾度水平对雷达标注性能的影响。

Conclusion: 该工作建立了一个可复现的框架，使研究社区能够训练和评估标注的4D雷达数据，解决了雷达语义分割领域的数据标注瓶颈问题，同时研究了恶劣天气条件对标注性能的影响。
...
Conclusion: 该框架通过显式的关系建模和高效的记忆机制，显著提升了开放词汇目标导航的透明度和成功率。
...
TL;DR: 该论文提出了一种名为GuiDG的两步域专家引导框架，通过训练多个参数高效的专家模型来改善视觉语言模型微调中的领域泛化能力，并在ImageNet-DG等基准测试中取得了优于现有方法的效果。


<details>
  <summary>Details</summary>
Motivation: 当前视觉语言模型微调面临领域特异性和领域泛化能力之间的关键权衡。传统方法在整个数据集上微调通用模型，可能会损害对未见领域的泛化能力。需要解决这一局限性。

Method: 提出GuiDG框架：1) 使用提示调优获取源领域专家模型；2) 引入跨模态注意力模块，通过自适应专家集成来指导视觉编码器的微调。该方法基于理论分析，表明在划分的源领域上训练多个参数高效的专家模型比微调通用模型具有更好的泛化能力。

Result: 在标准领域泛化基准测试和构建的ImageNet-DG数据集上进行广泛实验，证明GuiDG在保持效率的同时，优于最先进的微调方法。

Conclusion: 通过理论分析和实验验证，训练多个参数高效的领域专家模型能够有效提升视觉语言模型微调的领域泛化能力，GuiDG框架为此提供了一种有效的实现方案。
...
TL;DR: LightHCG是一种基于因果表征学习的超轻量级青光眼检测模型，通过卷积VAE和HSIC解耦技术，在保持高精度的同时大幅减少模型参数，并支持干预分析。


<details>
  <summary>Details</summary>
Motivation: 当前AI驱动的青光眼检测方法在可靠性、参数效率、虚假相关性以及干预分析应用方面存在不足，需要更轻量且能捕捉真实因果关系的模型。

Method: 提出LightHCG模型，采用卷积VAE构建潜在表征，结合HSIC进行潜在空间解耦，并使用图自编码器进行无监督因果表征学习。

Result: 与InceptionV3、MobileNetV2、VGG16等先进视觉模型相比，LightHCG在保持高分类性能的同时，减少了93~99%的权重参数，并增强了AI驱动干预分析的可能性。

Conclusion: LightHCG通过因果表征学习方法，在青光眼检测中实现了超轻量化和高性能的平衡，为临床干预分析和模拟提供了新的可能性。
...
TL;DR: 提出了一种结合动量机制和蒸馏的医疗视觉语言模型训练方法，在保持单GPU高效训练的同时，提升零样本和少样本性能。


<details>
  <summary>Details</summary>
Motivation: 医疗领域标注数据稀缺，需要鲁棒的视觉语言模型。对比学习训练需要大batch size，计算成本高，限制了资源有限机构的使用。同时，在数据有限的情况下，需要从数据和模型中提取更多知识来提升性能。

Method: 结合动量方法和蒸馏技术：1) 利用动量自蒸馏增强多模态学习；2) 将动量机制与梯度累积结合，在不增加资源消耗的情况下扩大有效batch size。

Result: 在零样本分类中达到与SOTA方法竞争的性能，在少样本适应中显著提升，获得超过90%的AUC-ROC，检索任务提升2-3%。同时保持单GPU的高训练效率和合理训练时间。
...
TL;DR: BOLT提出了一种基于正交基的低秩迁移框架，通过复用已有微调模型的谱基来高效适应新任务，避免了元学习的额外训练成本。


<details>
  <summary>Details</summary>
Motivation: 现有方法面临挑战：元学习方法需要额外的元训练阶段，成本高且不稳定；同时大量任务特定预训练模型未被充分利用，如何以最小额外训练将它们迁移到新任务仍待探索。

Method: BOLT框架包含离线阶段（从多个任务向量提取主导奇异方向并正交化形成可重用基）和在线阶段（冻结这些基，仅训练每层少量对角系数），实现参数高效微调。

Result: 实验表明，BOLT在未见任务上提供了强大的免训练初始化，其参数高效微调路径在性能上优于常见PEFT基线和代表性元学习初始化方法。

Conclusion: 将适应约束在任务信息正交子空间为未见任务迁移提供了有效替代方案，能够高效复用现有微调模型而无需权重合并。
...
<details>
  <summary>Details</summary>
Motivation: 自动驾驶和高级驾驶辅助系统依赖深度学习，而深度学习需要大量标注数据。nuScenes作为最广泛使用的自动驾驶数据集之一，体现了AV发展的关键趋势，但许多技术细节尚未在学术出版物中公开。本文旨在揭示这些细节，并追踪nuScenes对整个领域的影响。

Method: 通过深入分析nuScenes数据集及其扩展版本（nuImages和Panoptic nuScenes）的创建过程，总结未公开的技术细节。追踪nuScenes对后续数据集的影响和定义的标准，并综述基于该数据集的各种任务和方法发展。

Result: 提供了nuScenes数据集创建的详细技术细节，展示了它作为首个包含雷达数据、跨两大洲城市驾驶场景、使用完全自动驾驶车辆收集的数据集的创新性。分析了nuScenes对后续数据集的影响和定义的标准，并全面综述了基于该数据集的各种任务和方法。

Conclusion: nuScenes数据集在自动驾驶领域具有里程碑意义，不仅推动了多模态传感器融合、标准化基准和广泛任务的发展，还定义了行业标准，对后续数据集和研究产生了深远影响。该研究为自动驾驶文献提供了全面的综述，特别聚焦于nuScenes数据集。
...
TL;DR: 提出了HouseLayout3D真实世界基准和MultiFloor3D训练免费基线方法，用于多楼层建筑布局估计


<details>
  <summary>Details</summary>
Motivation: 现有3D布局估计模型主要基于单房间或单楼层合成数据训练，无法处理大型多楼层建筑，且需要将场景分割为独立楼层处理，这移除了对楼梯等连接多楼层结构推理至关重要的全局空间上下文

Method: 提出了MultiFloor3D，一个简单的训练免费基线方法，利用最新的场景理解方法，无需专门训练即可处理多楼层布局估计
...
TL;DR: STL是一个自训练框架，通过"先看后想"的结构化推理模板，让模型先生成视觉属性文本，再基于此进行推理，同时使用负向解释增强模型区分能力，有效提升多模态模型的感知和推理能力。


<details>
  <summary>Details</summary>
Motivation: 当前视觉语言模型在多模态推理中存在感知和推理能力的不足，而现有增强推理的方法通常依赖高质量的人工标注思维链数据、昂贵的专有模型，或者忽视感知的自训练方法。

Method: 提出See-Think-Learn自训练框架：1）引入结构化推理模板，让模型先提取视觉属性文本（See），再基于此进行推理（Think）；2）通过自训练循环让模型从自己生成的结构化推理中学习；3）加入负向解释数据，增强模型区分正确和错误答案的能力。

Result: 实验表明STL在不同领域均优于仅基于答案训练或自生成推理的基线方法，定性分析确认其生成的高质量推理过程。

Conclusion: STL提供了一种成本效益高的解决方案，能有效提升视觉语言模型的多模态推理能力，通过结构化推理模板和负向解释数据，同时改善了感知和推理两个关键方面。
...
<details>
  <summary>Details</summary>
Motivation: 探索音频-视频联合去噪训练是否能在不关心音频同步的情况下提升视频生成质量，验证跨模态训练对视频动态物理规律建模的益处
...
Conclusion: 跨模态协同训练是开发更强、更物理基础的世界模型的有前景方法，音频-视频联合去噪不仅能改善同步性，还能提升视频质量本身
...
TL;DR: 该论文提出了SEER-Bench基准测试和3DSPMR方法，用于解决顺序性室内具身任务中空间知识复用的问题，通过结合关系、视觉和几何线索来增强多模态大语言模型的空间推理能力。


<details>
  <summary>Details</summary>
Motivation: 现有具身AI研究主要关注单任务场景，但在实际应用中，智能体经常面临顺序性任务，需要复用先前探索积累的空间知识来支持后续推理和探索。当前研究对这一实际问题关注不足，特别是当某些子任务不可行时（如寻找不存在的物体）。

Method: 提出了3DSPMR方法，该方法利用已探索区域的关系、视觉和几何线索来增强多模态大语言模型（MLLMs），用于顺序性具身任务中的推理和探索。这是首次将几何信息明确整合到基于MLLM的空间理解和推理中。

Result: 在SEER-Bench基准测试上进行了广泛实验，验证了3DSPMR方法在顺序性EQA和EMN任务上都取得了显著的性能提升。

Conclusion: 该工作针对顺序性具身任务中空间知识复用的实际问题，提出了新的基准测试和方法，通过整合几何信息显著提升了多模态大语言模型在空间推理和探索方面的能力。
...
Method: 提出轨迹引导的数据集蒸馏(TGDD)，将分布匹配重新表述为沿模型训练轨迹的动态对齐过程，在每个训练阶段对齐合成数据集和原始数据集的特征分布，并引入分布约束正则化来减少类别重叠

Result: 在10个数据集上的实验表明，TGDD实现了最先进的性能，特别是在高分辨率基准上获得了5.0%的准确率提升，且无需额外的优化开销

Conclusion: TGDD通过动态特征对齐有效提升了数据集蒸馏的性能，在保持效率的同时实现了性能与效率的良好平衡
...
<details>
  <summary>Details</summary>
Motivation: 当前视频世界模型在处理长上下文输入时面临计算成本过高的问题，导致时空一致性和长期建模能力受限，需要一种更高效的长期世界建模方法
...
Conclusion: WorldPack通过高效的压缩记忆机制解决了视频世界模型长期建模的计算瓶颈，实现了更好的时空一致性和长期生成质量
...
<details>
  <summary>Details</summary>
Motivation: 现有基于高斯泼溅的实时内窥镜重建方法多依赖非商业衍生工具，限制了实际部署能力。需要开发商业兼容、可部署的实时手术场景重建系统。

Method: 基于Apache-2.0许可的GSplat可微分高斯光栅化器构建原生手术管道，实现原理性变形建模、鲁棒遮挡处理和高保真重建。

Result: 在EndoNeRF基准测试中达到最先进的重建质量，具有适合术中使用的速度-精度权衡，并在NVIDIA IGX Orin和Thor边缘硬件上通过Holoscan SDK实现实时手术可视化。
...
TL;DR: UCAgents提出了一种层次化多智能体框架，通过结构化证据审计实现单向收敛，解决医学视觉语言模型中推理脱离视觉证据的问题，在提升诊断准确性的同时大幅降低计算成本。
...
Method: UCAgents采用层次化多智能体框架，通过结构化证据审计强制实现单向收敛。受临床工作流程启发，该框架禁止立场改变，并将智能体交互限制在有针对性的证据验证上。引入单轮询问讨论来发现视觉-文本不对齐的潜在风险，通过信息论形式化双重噪声瓶颈，共同约束视觉模糊性和文本噪声。

Result: 在四个医学VQA基准测试中，UCAgents在PathVQA上达到71.3%的准确率，比最先进方法提升6.0%，同时降低87.7%的token成本。评估结果进一步证实UCAgents在发现更多视觉证据和避免混淆性文本干扰之间取得了平衡。

Conclusion: UCAgents展示了诊断可靠性和计算效率的平衡，这对于实际临床部署至关重要。该框架通过强制单向收敛和结构化证据审计，有效解决了医学视觉语言模型中的推理脱离问题。
...
TL;DR: 该论文提出3D-SLIM，一种针对3D场景语言理解的注意力掩码策略，通过几何自适应掩码和指令感知掩码替代传统因果掩码，解决了3D对象顺序无关性和指令注意力限制的问题。


<details>
  <summary>Details</summary>
Motivation: 现有3D场景语言理解方法通常采用语言建模的标准解码器，依赖因果注意力掩码。这种设计在3D场景理解中引入两个基本冲突：1）顺序无关的3D对象之间的顺序偏差；2）受限的对象-指令注意力，阻碍了任务特定推理。
...
Result: 3D-SLIM在多种3D场景语言任务上带来显著的性能提升，无需架构修改或额外参数。在多个基准测试和LLM基线上的广泛实验验证了其有效性。

Conclusion: 3D-SLIM证明了解码器设计在3D多模态推理中的关键作用，通过简单的掩码策略即可显著提升3D场景语言理解性能，同时保持模型架构不变。
...
TL;DR: YingVideo-MV是首个用于音乐驱动长视频生成的级联框架，通过音频语义分析、可解释的镜头规划、时间感知扩散Transformer架构和长序列一致性建模，实现从音频信号自动合成高质量音乐表演视频。


<details>
  <summary>Details</summary>
Motivation: 现有音频驱动虚拟形象视频生成方法在合成长序列方面取得进展，但音乐表演视频的生成（特别是包含摄像机运动）仍未被充分探索。现有长视频生成方法缺乏明确的摄像机运动控制。

Method: 1) 构建大规模Music-in-the-Wild数据集；2) 集成音频语义分析和可解释的镜头规划模块(MV-Director)；3) 提出时间感知扩散Transformer架构；4) 引入摄像机适配器模块将摄像机姿态嵌入潜在噪声；5) 提出时间感知动态窗口范围策略，根据音频嵌入自适应调整去噪范围以增强长序列推理中的连续性。

Result: 综合基准测试表明，YingVideo-MV在生成连贯且富有表现力的音乐视频方面表现优异，能够实现精确的音乐-动作-摄像机同步。
...
<details>
  <summary>Details</summary>
Motivation: 研究动机是探究基于深度学习和高斯混合模型的部分到部分点云配准方法中特征向量在平移和旋转下的不变性问题，特别是DeepGMR等方法的局限性，旨在揭示问题原因并提出解决方案。

Method: 提出了注意力参考点偏移层，该层使用经过充分研究的注意力模块来寻找两个部分点云的共同参考点，而不是重叠区域，从而获得变换不变的特征。

Result: ARPS层显著提升了DeepGMR及其变体UGMMReg的性能，这些扩展模型甚至超过了之前使用注意力块和Transformer提取重叠区域或共同参考点的深度学习方法。

Conclusion: 该研究为使用深度学习和高斯混合模型的配准方法提供了更深入的见解，提出的ARPS层通过寻找共同参考点而非重叠区域，有效解决了部分到部分点云配准中的特征不变性问题。
...
Method: 提出dots.ocr单视觉语言模型，首次在统一端到端框架中联合学习三大核心任务。采用高度可扩展的数据引擎合成大规模多语言语料库，支持多样化语言、布局和领域。

Result: 在OmniDocBench上实现最先进性能。在新基准XDocParse（涵盖126种语言）上，dots.ocr比次优竞争者高出+7.4分，展现了卓越的多语言能力。

Conclusion: dots.ocr证明了统一端到端框架在文档布局解析中的优势，为全球文档智能研究提供了强大的新基线，展示了联合学习三大核心任务的有效性。
...
TL;DR: GeoDiT：首个面向地理空间领域的扩散式视觉语言模型，通过并行精炼过程解决自回归模型在地理空间理解中的结构错配问题，在结构化、面向对象的任务上实现最先进性能。


<details>
  <summary>Details</summary>
Motivation: 自回归模型与地理空间理解的并行本质存在结构错配，将刚性顺序叙事强加于场景，从根本上阻碍了结构化、连贯输出的生成。需要挑战这一范式，重新构建地理空间生成过程。

Method: 将地理空间生成重构为并行精炼过程，实现从粗到细的整体合成，同时解析所有语义元素。为此引入了GeoDiT——首个面向地理空间领域的扩散式视觉语言模型。

Result: GeoDiT在需要结构化、面向对象输出的基准测试中建立了新的最先进水平。在图像描述、视觉定位和多目标检测等任务上取得显著提升，这些正是自回归模型表现不佳的任务。

Conclusion: 研究表明，将生成过程与数据内在结构对齐是解锁复杂地理空间分析中卓越性能的关键。并行精炼方法比顺序自回归方法更适合地理空间理解任务。
...
TL;DR: ViT-SR是一种基于视觉变换器的单图像超分辨率方法，采用两阶段训练策略：先通过着色任务进行自监督预训练，再微调用于4倍超分辨率，在DIV2K数据集上取得了SSIM 0.712和PSNR 22.90 dB的优异性能。


<details>
  <summary>Details</summary>
Motivation: 单图像超分辨率在计算机视觉中仍然是一个具有挑战性的问题。作者希望通过自监督预训练来学习丰富、可泛化的视觉表示，以提升超分辨率性能，同时简化残差学习过程。

Method: 提出ViT-SR方法，采用两阶段训练策略：1）自监督预训练阶段：在着色任务上训练视觉变换器，学习通用视觉表示；2）微调阶段：将预训练模型调整为4倍超分辨率任务，通过预测高频残差图像添加到初始双三次插值结果中，简化残差学习。

Result: 在DIV2K基准数据集上，ViT-SR取得了SSIM 0.712和PSNR 22.90 dB的优异结果，证明了该方法在单图像超分辨率任务上的有效性。

Conclusion: 两阶段训练策略和自监督预训练对于复杂图像恢复任务具有显著效果，展示了视觉变换器在超分辨率任务中的潜力。未来可以通过更大的ViT架构或替代的预训练任务进一步提升性能。
...
TL;DR: SkyMoE是一个基于专家混合架构的视觉语言模型，专门为遥感多任务、多粒度解释设计，通过自适应路由和上下文解耦增强策略，在21个公开数据集上实现了最先进的性能。
...
Method: 1. 采用专家混合架构，设计自适应路由器生成任务和粒度感知的路由指令；2. 引入上下文解耦增强策略，创建局部和全局特征的对比对，引导专家进行层级特定表示学习；3. 构建MGRS-Bench基准，覆盖多个遥感解释任务和粒度级别。

Result: 在21个公开数据集上的广泛实验表明，SkyMoE在各项任务中实现了最先进的性能，验证了其在复杂场景中的适应性、可扩展性和卓越的多粒度理解能力。

Conclusion: SkyMoE通过专家混合架构和自适应路由机制，有效解决了遥感多任务、多粒度解释的挑战，在平衡局部细节感知和全局上下文理解方面表现出色，为地理空间视觉语言模型的发展提供了新方向。
...
TL;DR: 该论文研究了零样本异常分类与分割的核心挑战，提出了基于理论分析和算法设计的解决方案，包括识别一致异常现象、开发CoDeGraph图框架、扩展到3D医学影像以及结合文本引导方法。


<details>
  <summary>Details</summary>
Motivation: 零样本异常分类和分割在工业检测和医学成像中越来越重要，但现有方法面临一致异常（重复出现的相似异常）导致距离基方法失效的问题。论文旨在从理论和算法设计角度解决这些核心挑战。

Method: 1. 分析预训练Vision Transformers中补丁表示的统计和几何行为，识别相似性缩放和邻居耗尽现象；2. 提出CoDeGraph图框架，通过多阶段图构建、社区检测和结构化精炼来过滤一致异常；3. 扩展到3D医学影像，提出无需训练的计算高效体积标记化策略；4. 将CoDeGraph生成的伪掩码用于监督提示驱动的视觉语言模型。

Result: 1. 识别了描述正常补丁关系在有无一致异常时变化的两个关键现象；2. CoDeGraph能有效抑制一致异常的影响；3. 实现了无需3D训练样本的真正零样本3D异常检测；4. 展示了伪掩码可以监督视觉语言模型，连接了批处理和文本引导方法。

Conclusion: 该论文为零样本异常分类与分割问题提供了理论理解和实用解决方案，通过分析一致异常现象、开发CoDeGraph框架、扩展到3D医学影像以及结合文本引导方法，为工业检测和医学成像中的零样本异常检测提供了系统性的方法。
...
TL;DR: 本文分析了VGGT和π³模型中全局注意力机制的作用，发现早期层不形成有效对应关系，中间层执行跨视图对齐，最后层仅提供微小改进。基于此提出无需训练的两步加速方案：将早期全局层转换为帧注意力，并通过子采样K/V实现全局注意力加速，在保持精度的同时实现8-10倍推理加速。


<details>
  <summary>Details</summary>
Motivation: DUSt3R、VGGT和π³等模型在多视图3D任务中表现出色，但依赖全局自注意力导致计算成本高昂。现有稀疏注意力变体只能提供部分加速，且缺乏对全局注意力在多视图推理中作用的系统性分析。本文旨在深入理解全局注意力模块的作用，并基于此设计更高效的加速方案。

Method: 首先深入分析VGGT和π³中全局注意力模块的作用，发现交替全局帧架构中的角色分工：早期全局层不形成有意义的对应关系，中间层执行跨视图对齐，最后层仅提供微小改进。基于这些发现，提出无需训练的两步加速方案：1) 将早期全局层转换为帧注意力；2) 通过子采样K/V（保留对角线并添加均值填充组件）来子采样全局注意力。

Result: 在标准姿态和点云基准测试中，该方法在VGGT和π³上实现了8-10倍的推理时间加速，同时匹配或略微提高了原始模型的精度。即使在极端密集的多视图设置下，该方法仍保持鲁棒性，而先前的稀疏注意力基线方法会失败。

Conclusion: 通过系统性分析全局注意力在多视图3D模型中的作用，本文揭示了注意力机制的角色分工模式，并基于此设计了高效的训练免费加速方案。该方法在保持模型性能的同时显著提升了推理效率，为多视图3D模型的实用化部署提供了有效解决方案。
...
Method: 提出了OmniPerson统一生成模型：1）支持RGB/IR模态图像/视频生成，具有多种控制能力；2）设计了多参考融合器（Multi-Refer Fuser）从多视角参考图像中提取统一身份特征；3）创建了PersonSyn数据集及其自动化标注流程。

Result: OmniPerson在行人生成任务中达到最先进水平，在视觉保真度和身份一致性方面表现出色。使用生成数据增强现有数据集能持续提升ReID模型性能。

Conclusion: OmniPerson为可见光/红外图像/视频ReID任务提供了一个有效的身份保持行人生成解决方案，通过高质量的数据生成和增强显著提升了ReID模型的性能。
...
TL;DR: Panel2Patch：一种从生物医学文献中挖掘层次结构（多面板、标记丰富的图表及周边文本）并转换为多粒度监督的数据管道，相比传统方法能提取更有效的监督信号，用更少数据实现更好性能。


<details>
  <summary>Details</summary>
Motivation: 当前生物医学视觉-语言预训练通常将丰富的科学图表和文本压缩为粗糙的图表级配对，丢弃了临床医生实际依赖的细粒度对应关系，需要更好的方法来保留局部语义。

Method: Panel2Patch解析科学图表的布局、面板和视觉标记，构建图表、面板和补丁三个层次的视觉-语言对齐配对，并开发粒度感知的预训练策略，统一从粗粒度教学描述到细粒度区域聚焦短语的异构目标。

Result: 仅使用少量文献图表，Panel2Patch就能提取比先前管道更有效的监督信号，用更少的预训练数据实现显著更好的性能。

Conclusion: Panel2Patch通过挖掘生物医学文献的层次结构并构建多粒度监督，解决了现有方法丢弃细粒度对应关系的问题，为开发强大的生物医学视觉-语言模型提供了更有效的数据管道。
...
TL;DR: 提出了一种基于扩散模型和运动图检索的语音同步手势视频生成新框架，通过联合学习音频-手势分布和精心设计的检索算法，显著提升了生成手势的同步性和自然度。


<details>
  <summary>Details</summary>
Motivation: 现有方法主要使用音频特征与运动图节点特征的距离或共享特征空间来检索手势轨迹，但由于音频与手势之间存在多对多映射关系，这些一对一映射方法无法充分处理这种复杂性，导致生成的手势同步性和自然度不足。

Method: 1) 使用扩散模型从输入音频序列生成手势动作，该模型隐式学习音频和动作的联合分布；2) 从输入音频中提取低级和高级特征来丰富扩散模型的训练；3) 设计基于运动的检索算法，通过评估动作的全局和局部相似性在图中找到最合适的路径；4) 对检索到的非连续节点段进行无缝拼接，生成连贯的视频输出。

Result: 实验结果表明，该方法在同步准确性和生成手势的自然度方面显著优于先前方法，验证了所提框架的有效性。

Conclusion: 通过结合扩散模型的生成能力和运动图的检索机制，成功解决了音频-手势多对多映射的挑战，实现了更同步、更自然的语音手势视频合成。
...
TL;DR: 该论文提出了一种结合纹理映射的Gaussian Splatting方法，通过为每个高斯基元添加自适应纹理来提高细节表示效率，减少不必要的基元数量。


<details>
  <summary>Details</summary>
Motivation: 传统的Gaussian Splatting方法需要大量小高斯基元来表示精细外观细节，这在几何和外观具有不同频率特性时会造成资源浪费。受纹理映射传统启发，作者希望用纹理来表示细节外观，提高表示效率。

Method: 提出了一种新的2D高斯基元外观表示方法，为每个基元添加纹理映射。通过自适应调整纹理分辨率（上采样或下采样）来确保纹理像素大小受图像采样频率限制，并根据输入图像内容进行适应。同时，基于纹理分辨率控制优化过程中的基元数量。

Result: 该方法在图像质量和总参数数量方面优于其他纹理化高斯基元解决方案，能够更高效地表示场景细节。

Conclusion: 通过将纹理映射融入Gaussian Splatting优化过程，实现了更高效的外观细节表示，减少了不必要的基元数量，为3D重建和实时渲染提供了更好的解决方案。
...
TL;DR: RULER-Bench是一个专门评估视频生成模型推理能力的新基准，重点关注认知规则而非传统视觉质量指标，发现当前最先进模型在规则一致性方面仅达到48.87%，表明视频生成模型的推理能力仍有很大提升空间。


<details>
  <summary>Details</summary>
Motivation: 现有视频生成评估基准主要关注视觉感知和理解方面（如视觉美学、指令遵循、时间一致性），但视频生成模型的基于规则的推理能力尚未得到充分探索。虽然最近有研究初步探索视频模型是否可以作为零样本学习器，但仍缺乏对推理能力的细粒度分解和全面评估方案。

Method: 构建RULER-Bench基准，基于文本到视频和图像到视频两种基本范式，涵盖6个规则类别下的40个代表性任务，包含622个高质量标注实例。为每个生成的视频构建包含4个指标的检查清单，并利用GPT-4o进行评分，与人类判断达到85%的一致性。

Result: 广泛实验表明，当前最先进的视频生成模型在规则一致性指标上仅达到48.87%，突显了下一代视频模型在推理能力方面仍有显著改进空间。

Conclusion: RULER-Bench的见解将促进推理感知视频生成的进一步发展，推动视频生成模型向视觉基础智能迈进。该基准填补了视频生成模型推理能力评估的空白，为未来研究提供了重要方向。
...
TL;DR: PoreTrack3D是首个用于毛孔尺度非刚性3D面部轨迹跟踪的动态3D高斯溅射基准数据集，包含超过44万条面部轨迹，其中5.2万条超过10帧，68条手动标注轨迹覆盖完整150帧。
...
Method: 创建了包含超过440,000条面部轨迹的基准数据集，其中包含传统面部标志点和毛孔尺度关键点轨迹。系统评估了最先进的动态3D高斯溅射方法，并建立了该领域的首个性能基准。

Result: PoreTrack3D是首个同时包含传统面部标志点和毛孔尺度关键点轨迹的基准数据集，建立了动态3D高斯溅射在该领域的首个性能基准，为高保真面部运动捕捉和动态3D重建提供了新框架。

Conclusion: PoreTrack3D基准数据集填补了毛孔尺度非刚性3D面部轨迹跟踪领域的空白，为研究精细面部表情提供了重要工具，其创建流程为高保真面部运动捕捉和动态3D重建建立了新框架。
...
TL;DR: 提出了一种混合架构，将视觉语言模型的补丁级相似度分数作为空间相关性过滤器，应用于OCR提取的区域，实现细粒度文档检索
...
Conclusion: 提出的混合架构统一了视觉语言模型和OCR系统的优势，能够实现细粒度的文档区域检索，为检索增强生成提供更精确的上下文
...
<details>
  <summary>Details</summary>
Motivation: 现有基于NeRF的偏振感知方法存在训练慢、渲染效率低、对材料/视角假设依赖强的问题；而3DGS虽然能实时渲染，但难以从反射-几何纠缠中准确重建反射，且添加延迟反射模块会引入环境贴图依赖。

Method: 提出PolarGuide-GSDR偏振前向引导范式，建立偏振与3DGS的双向耦合机制：1) 利用3DGS的几何先验解决偏振歧义；2) 使用精炼的偏振信息线索指导3DGS的法线和球谐函数表示，实现反射分离和全场景重建。

Result: 在公开和自采集数据集上，PolarGuide-GSDR在镜面反射重建、法线估计和新视角合成方面达到最先进性能，同时保持实时渲染能力，是首个将偏振先验直接嵌入3DGS优化的框架。

Conclusion: 该框架通过偏振与3DGS的双向耦合，实现了对复杂反射场景的高保真建模，具有优越的可解释性和实时性能，无需环境贴图或限制性材料假设。
...
TL;DR: UAUTrack是一个用于反无人机跟踪的统一单目标跟踪框架，采用单流、单阶段、端到端架构，通过文本先验提示策略有效整合多种模态，在多个反无人机数据集上实现了最先进的性能。


<details>
  <summary>Details</summary>
Motivation: 反无人机跟踪研究虽然探索了RGB、TIR和RGB-T融合等多种模态，但缺乏统一的跨模态协作框架。现有方法主要关注独立任务的独立模型，忽视了跨模态信息共享的潜力，且当前解决方案在多模态数据融合方面效果有限。

Method: 提出UAUTrack框架，采用单流、单阶段、端到端的统一架构。关键创新是引入文本先验提示策略，指导模型在不同场景下关注无人机目标，有效整合多种模态信息。

Result: 在Anti-UAV和DUT Anti-UAV数据集上实现了最先进的性能，在Anti-UAV410数据集上保持了精度与速度的良好平衡，在多种反无人机场景中表现出高精度和实际效率。

Conclusion: UAUTrack为反无人机跟踪提供了一个有效的统一框架，通过文本先验提示策略成功实现了多模态融合，在精度和效率方面都表现出色，为跨模态协作的反无人机跟踪提供了新的解决方案。
...
TL;DR: 开发了一个基于注意力机制InceptionV3模型的系统，用于检测口罩佩戴和社交距离保持情况，以识别新冠病毒传播高风险区域。


<details>
  <summary>Details</summary>
Motivation: COVID-19疫情全球蔓延，世界卫生组织要求佩戴口罩和保持6英尺社交距离。需要自动化系统来监测这些防护措施的遵守情况，以识别高风险传播区域。

Method: 使用定制化的注意力机制InceptionV3模型，结合包含10,800张图像的数据集（含戴口罩和不戴口罩图像），检测口罩佩戴和社交距离保持情况。

Result: 训练准确率达到98%，验证准确率99.5%，精确度约98.2%，帧率25FPS。系统能有效识别病毒传播高风险区域。

Conclusion: 该系统能帮助当局快速定位高风险区域并提醒当地居民采取防护措施，为疫情防控提供技术支持。
...
TL;DR: ClimaDrive是一个语义引导的图像到图像框架，用于合成语义连贯、天气多样且物理上合理的异常驾驶数据，通过构建ClimaOoD基准数据集显著提升了异常分割模型的性能。
...
Method: 提出了ClimaDrive框架，该框架将结构引导的多天气生成与提示驱动的异常修复相结合，能够创建视觉上逼真的训练数据。基于此框架构建了ClimaOoD基准数据集，涵盖六种代表性驾驶场景，包括晴朗和恶劣天气条件。

Result: 在四种最先进方法上的广泛实验表明，使用ClimaOoD训练能显著提升异常分割性能。所有方法的AUROC、AP和FPR95指标均有显著改善，其中RbA在Fishyscapes LAF上的FPR95从3.97降至3.52。
...
Method: 提出GeoBridge基础模型，采用语义锚机制通过文本描述桥接多视图特征，实现双向跨视图匹配和语言到图像检索。构建了首个大规模跨模态多视图对齐数据集GeoLoc（包含5万多对图像和文本描述）。

Result: GeoLoc预训练显著提高了GeoBridge的地理定位精度，同时促进了跨域泛化和跨模态知识迁移。模型在多个任务上进行了广泛评估，表现出色。

Conclusion: GeoBridge通过语义锚机制和多视图对齐数据集解决了传统地理定位方法的局限性，实现了更鲁棒、灵活的地理定位，支持跨视图和跨模态检索。
...
TL;DR: 提出RAMF框架解决视频仇恨言论检测问题，通过局部-全局上下文融合和语义交叉注意力实现多模态融合，引入对抗推理增强对微妙仇恨意图的理解，在两个真实数据集上取得SOTA性能提升。
...
Result: 在两个真实世界仇恨视频数据集上评估，相比现有最佳方法，Macro-F1提升3%，仇恨类别召回率提升7%，展现出鲁棒的泛化性能。

Conclusion: RAMF框架通过创新的多模态融合和对抗推理机制，有效解决了视频仇恨言论检测中的语义融合和微妙意图理解挑战，为多模态仇恨内容检测提供了新思路。
...
<details>
  <summary>Details</summary>
Motivation: 现有视觉语言模型中的大量视觉令牌带来高计算成本，阻碍移动端部署。现有剪枝方法要么忽略令牌间冗余，要么忽视空间关系，导致保留过多重复令牌或无法充分覆盖目标对象区域。

Method: 提出离心令牌剪枝范式，实现由近及远的令牌选择，优先保留细粒度对象细节；设计空间稀疏性缓冲准则，延迟选择空间距离较远的令牌；采用并行贪婪策略高效选择令牌；通过选择性融合丢弃令牌的显著信息到保留令牌中来减少信息损失。

Result: 在五种视觉语言模型上，VLM-Pruner在88.9%的剪枝率下始终优于强基线方法，同时实现了端到端推理加速。

Conclusion: VLM-Pruner通过平衡冗余性和空间稀疏性，提供了一种有效的训练免费令牌剪枝解决方案，能够在保持性能的同时显著降低计算成本，促进视觉语言模型在移动设备上的部署。
...
Method: 提出性别分层的跨受试者全身MR图像配准方法，使用VIBESegmentator方法生成的皮下脂肪组织和肌肉掩膜来增强基于强度的图割配准。在4000名受试者子集上评估，与仅基于强度的方法以及uniGradICON和MIRTK两种现有方法进行比较。

Result: 提出的方法在男性和女性中分别获得0.77和0.75的平均Dice分数。与仅基于强度的配准相比，两个性别的平均Dice分数提高了6个百分点，大多数组织区域的标签错误频率降低。与uniGradICON相比提高了8-9个百分点，与MIRTK相比提高了12-13个百分点。使用该方法得到的年龄相关性图谱噪声更少，解剖对齐更好。
...
<details>
  <summary>Details</summary>
Motivation: 深度学习在计算机视觉中取得了巨大成功，但仍然容易受到对抗攻击。对抗性训练是提高模型鲁棒性的主要防御方法，但其对攻击可迁移性的影响尚未得到充分探索。

Method: 训练了36个多样化模型（包括CNN和ViT），并进行全面的可迁移性实验，研究对抗性训练是否无意中增加了对抗样本的可迁移性。
...
<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型在视觉定位方面取得显著进展，但将其能力迁移到遥感图像面临挑战：目标在千米级场景中极小，查询涉及复杂的相对位置、空间层次或跨远距离对象的上下文依赖关系。
...
### [64] [ReVSeg: Incentivizing the Reasoning Chain for Video Segmentation with Reinforcement Learning](https://arxiv.org/abs/2512.02835)
...
<details>
  <summary>Details</summary>
Motivation: 现有视频目标分割方法通常将复杂的动态、因果和时序交互推理简化为潜在嵌入表示，导致推理链不透明且难以追踪。需要一种能够显式分解推理过程的方法来提高可解释性和性能。

Method: 采用显式分解视角，将推理过程分解为三个顺序操作：语义解释、时序证据选择和空间定位。利用预训练视觉语言模型作为原生接口执行这些操作，并通过强化学习优化多步推理链，使模型能够从结果驱动的信号中自我优化决策质量。

Result: ReVSeg在标准视频目标分割基准测试中达到了最先进的性能，并产生了可解释的推理轨迹。

Conclusion: 通过显式分解推理过程并利用强化学习优化多步推理链，ReVSeg不仅提升了视频目标分割的性能，还提供了可解释的推理过程，为解决复杂的推理中心视频任务提供了有效框架。
...
TL;DR: CAEVL：一种无需无人机训练图像、仅使用卫星视图参考图像进行训练的无人机图像定位方法，通过模拟卫星与无人机视图间的视觉域偏移实现跨视图匹配。


<details>
  <summary>Details</summary>
Motivation: 在GNSS拒止环境中，基于图像的无人机定位至关重要。现有方法依赖配对的大规模无人机-卫星数据集进行训练，但这类数据获取成本高且通常不可得，限制了方法的适用性。

Method: 提出无需无人机图像的训练范式，直接从卫星视图参考图像学习。采用专门的增强策略模拟卫星与真实无人机视图间的视觉域偏移。引入CAEVL模型来利用这一范式。

Result: 在发布的新挑战性数据集ViLD（真实世界无人机图像）上验证，CAEVL取得了与使用配对数据训练方法相竞争的性能，展示了其有效性和强大的泛化能力。

Conclusion: 该方法通过消除对配对无人机-卫星训练数据的需求，显著降低了数据获取成本，为GNSS拒止环境中的无人机定位提供了更实用的解决方案。
...
TL;DR: 提出AttMetNet，一种基于注意力增强的深度学习框架，用于Sentinel-2卫星图像的甲烷羽流检测，通过融合NDMI和注意力机制提升检测精度
...
Method: 提出AttMetNet框架，将归一化甲烷差异指数(NDMI)与注意力增强的U-Net相结合，通过注意力机制选择性放大甲烷吸收特征并抑制背景噪声；使用focal loss处理类别不平衡问题；在真实甲烷羽流数据集上训练

Result: 实验表明AttMetNet在甲烷羽流检测中优于现有方法，具有更低的误报率、更好的精确率-召回率平衡以及更高的IoU

Conclusion: AttMetNet是首个针对真实卫星图像中甲烷羽流检测的定制化架构，通过融合NDMI的羽流敏感线索和注意力驱动的特征选择，实现了更稳健的检测性能
...
### [67] [LumiX: Structured and Coherent Text-to-Intrinsic Generation](https://arxiv.org/abs/2512.02781)
*Xu Han,Biao Zhang,Xiangjun Tang,Xianzhi Li,Peter Wonka*

Main category: cs.CV

TL;DR: LumiX是一个结构化扩散框架，用于从文本提示生成一致的场景内在属性图（如反照率、辐照度、法线、深度和最终颜色），通过查询广播注意力和张量LoRA实现多属性联合训练。


<details>
  <summary>Details</summary>
Motivation: 现有方法在从文本生成场景内在属性时难以保持结构一致性和物理合理性，需要一种能够联合生成多种内在属性图并确保它们之间一致性的框架。

Method: 提出LumiX框架，包含两个关键技术：1) 查询广播注意力机制，通过在所有内在属性图之间共享查询来确保结构一致性；2) 张量LoRA，通过张量化适配参数高效地建模跨图关系，实现高效的联合训练。

Result: 实验表明LumiX能够生成一致且物理合理的结果，相比现有技术实现了23%的对齐度提升，偏好得分从-0.41提高到0.19，并且能够在同一框架内进行图像条件的内在分解。

Conclusion: LumiX通过结构化扩散框架成功解决了文本到内在属性生成的一致性问题，提出的查询广播注意力和张量LoRA技术为多属性联合生成提供了有效解决方案，在质量和一致性方面显著优于现有方法。
...
<details>
  <summary>Details</summary>
Motivation: 3D高斯泼溅(3DGS)虽然能实现实时渲染和高保真外观，但存在多视图不一致问题，限制了几何精度；而2D高斯泼溅(2DGS)虽然强制多视图一致性，但会牺牲纹理细节。需要一种方法能够平衡外观和几何精度。
...
Conclusion: EGGS通过整合2D和3D高斯表示，成功平衡了外观保真度和几何精度，解决了新视角合成中的关键挑战，在保持实时渲染能力的同时提供了更高质量的合成结果。
...
<details>
  <summary>Details</summary>
Motivation: TrackNet系列在快速移动小物体跟踪方面建立了强基线，但存在显著限制：V1-V3因依赖纯视觉线索而难以处理遮挡，V4虽然引入了运动输入但存在方向模糊性问题，因为其绝对差分方法丢弃了运动极性。

Method: 提出TrackNetV5，包含两个创新机制：1) 运动方向解耦模块，将时间动态分解为带符号的极性场，显式编码运动发生和轨迹方向；2) 残差驱动的时空细化头，基于粗到细范式，利用分解的时空上下文估计校正残差，有效恢复被遮挡目标。

Result: 在TrackNetV2数据集上的实验表明，TrackNetV5实现了新的最先进性能：F1分数0.9859，准确率0.9733，显著优于先前版本。性能提升仅带来3.7%的FLOPs增加，保持了实时推理能力。

Conclusion: TrackNetV5通过显式建模运动方向和采用残差驱动的细化策略，有效解决了遮挡和方向模糊性问题，在保持计算效率的同时实现了卓越的跟踪精度，为快速移动小物体跟踪提供了新的解决方案。
...
Method: 创建VideoScience-Bench基准，包含200个精心设计的提示，涵盖14个主题和103个物理化学概念。使用专家标注评估7个最先进的视频模型（T2V和I2V），从五个维度评估：提示一致性、现象一致性、正确动态性、不变性和时空连续性。使用VLM-as-a-Judge方法评估视频生成，并与人类评估进行相关性分析

Result: VLM-as-a-Judge评估方法与人类评估结果有强相关性。基准测试显示当前视频模型在科学推理方面存在局限性，需要改进以准确生成符合科学现象的视频

Conclusion: VideoScience-Bench是首个评估视频模型作为推理者而不仅仅是生成者的基准，要求生成的视频展示与预期物理化学现象一致的科学理解。该基准为评估和改进视频模型的科学推理能力提供了重要工具
...
TL;DR: 该论文针对多模态图像编辑领域开源与闭源模型性能差距问题，提出了UnicEdit-10M大规模高质量数据集构建方法和UnicBench综合评测基准，通过轻量级数据管道和双任务专家模型解决数据质量与规模的平衡问题。


<details>
  <summary>Details</summary>
Motivation: 随着GPT-4o、Nano Banana、Seedream 4.0等强大多模态模型在图像编辑领域的快速发展，开源与闭源模型之间的性能差距正在扩大。主要原因是缺乏大规模、高质量的训练数据和能够诊断模型在不同编辑行为中弱点的综合基准。现有数据构建方法面临规模与质量的权衡：人工标注质量高但不可扩展，而自动化流程存在错误传播和噪声问题。

Method: 1. 引入轻量级数据管道，用端到端模型和统一的后验证阶段替代多工具链；2. 训练7B参数的双任务专家模型Qwen-Verify，用于高效失败检测和指令重写；3. 构建UnicEdit-10M数据集，包含1000万规模的基础和复杂编辑任务；4. 提出UnicBench通用基准，扩展基础编辑评估，明确评估空间和知识驱动推理能力；5. 引入新指标，包括非编辑一致性和推理准确性。

Result: 该数据管道产生了UnicEdit-10M数据集，这是一个1000万规模的数据集，涵盖多样化的基础和复杂编辑任务。UnicBench基准能够对主流模型进行细粒度诊断，揭示了它们的局限性，并为未来研究提供了明确方向。

Conclusion: 通过提出的轻量级数据管道和双任务专家模型，成功解决了多模态图像编辑领域数据规模与质量的平衡问题。UnicEdit-10M数据集和UnicBench基准为开源社区提供了高质量训练资源和综合评估工具，有助于缩小开源与闭源模型之间的性能差距，推动多模态图像编辑技术的发展。
...
TL;DR: IC-World是一个用于共享世界建模的视频生成框架，通过激活大型视频模型的内在上下文生成能力，从一组输入图像并行生成多个视频，并使用强化学习和奖励模型确保场景几何和物体运动的一致性。


<details>
  <summary>Details</summary>
Motivation: 视频世界模型在合成多样动态视觉环境方面受到关注，但共享世界建模问题尚未被系统探索。该研究旨在解决从同一底层世界在不同相机姿态下生成多个视频的挑战。

Method: 提出IC-World框架：1) 利用大型视频模型的上下文生成能力并行处理所有输入图像；2) 通过强化学习（Group Relative Policy Optimization）微调；3) 引入两个奖励模型分别保证场景级几何一致性和物体级运动一致性。

Result: 大量实验表明，IC-World在几何一致性和运动一致性方面显著优于现有最先进方法，是首个系统探索基于视频模型的共享世界建模问题的工作。

Conclusion: IC-World成功解决了共享世界建模问题，通过上下文生成能力和强化学习优化，实现了从同一世界在不同视角下生成一致性视频的突破性进展。
...
TL;DR: PhyCustom是一个用于扩散模型的微调框架，通过两个新的正则化损失来激活模型进行物理概念定制，解决了现有方法在物理属性定制上的不足。


<details>
  <summary>Details</summary>
Motivation: 现有的基于扩散的文本到图像定制方法在理解具体概念（如风格和形状）方面取得了成功，但在物理概念的定制方面存在不足。核心限制在于训练过程中缺乏明确的物理知识引入，导致即使输入文本提示中包含物理相关词汇，生成结果也无法准确反映相应的物理属性。

Method: 提出了PhyCustom微调框架，包含两个新颖的正则化损失：1）等距损失（isometric loss）旨在激活扩散模型学习物理概念；2）解耦损失（decouple loss）帮助消除独立概念的混合学习。

Result: 在多样化数据集上的实验表明，PhyCustom在物理概念定制方面在定量和定性上都优于先前的最先进方法和流行方法。

Conclusion: PhyCustom通过引入专门的物理知识正则化损失，成功解决了扩散模型在物理概念定制方面的局限性，为文本到图像生成中的物理属性控制提供了有效解决方案。
...
TL;DR: 该研究首次系统性地评估了身份保持型AI生成内容（IP-AIGC）检测器在印度和南亚面孔上的鲁棒性与公平性，发现微调虽然提升域内性能，但在跨生成器的身份保持型内容上表现显著下降，揭示了检测器对训练生成器线索的过拟合问题。


<details>
  <summary>Details</summary>
Motivation: 现代图像编辑器能生成身份保持型AI生成内容（IP-AIGC），即同一人物以新服装、背景或光线出现。当前检测器在这种情况下的鲁棒性和公平性尚不明确，特别是对于代表性不足的人群（如印度和南亚面孔）。

Method: 1) 从FairFD和HAV-DF数据集中构建印度面孔的训练集；2) 使用商业Web-UI生成器（Gemini和ChatGPT）创建两个保留身份的IP-AIGC测试集（HIDF-img-ip-genai和HIDF-vid-ip-genai）；3) 评估两种最先进的检测器（AIDE和Effort）在预训练和微调两种模式下的性能，使用AUC、AP、EER和准确率等指标。

Result: 微调在域内带来显著提升（如Effort AUC从0.739提升到0.944），但在印度人群的保留身份IP-AIGC测试集上性能一致下降（如AIDE AUC从0.923下降到0.563）。在非IP的HIDF图像上，预训练性能保持较高水平，表明检测器对身份保持编辑存在特定脆弱性而非一般分布偏移。

Conclusion: IP-AIGC-印度检测是一个具有挑战性且实际相关的场景。研究揭示了检测器对训练生成器线索的过拟合问题，呼吁开发保持表征的适应方法和印度感知的基准构建，以缩小AIGC检测中的泛化差距。
...
TL;DR: 该论文针对多语言环境下的面部-语音关联任务，在FAME 2026挑战赛中通过融合和正交投影方法处理英德双语数据，取得了第三名的成绩。


<details>
  <summary>Details</summary>
Motivation: 多语言环境下的面部-语音关联任务具有挑战性，特别是在处理不同语言的面部-语音配对时。FAME 2026挑战赛引入了英德双语数据，需要开发能够有效处理跨语言模态关联的方法。

Method: 重新审视了融合和正交投影方法，通过有效聚焦两个模态中的相关语义信息来处理面部-语音关联。该方法专门针对英德双语数据进行了优化。

Result: 在FAME 2026挑战赛中排名第三，在英德数据分割上取得了33.1的等错误率（EER），表现良好。

Conclusion: 提出的融合和正交投影方法在多语言面部-语音关联任务中有效，特别是在处理英德双语数据时表现出色，为跨语言模态关联研究提供了有价值的参考。
...
TL;DR: STSR 2025挑战赛在MICCAI 2025举办，针对数字牙科中的CBCT和IOS数据，提出两个半监督学习任务：牙齿和牙髓管分割、跨模态刚性配准，以解决标注数据稀缺问题。


<details>
  <summary>Details</summary>
Motivation: 数字牙科中CBCT和IOS技术至关重要，但标注数据稀缺限制了自动化解决方案的发展，特别是在牙髓管分割和跨模态配准方面。需要建立半监督学习基准来推动该领域进展。

Method: 组织STSR 2025挑战赛，提供60个标注和640个未标注IOS样本，以及30个标注和250个未标注CBCT扫描。设置两个任务：1）CBCT中牙齿和牙髓管的半监督分割；2）CBCT和IOS的半监督刚性配准。

Result: 挑战赛吸引了强大社区参与，最佳团队提交了开源深度学习解决方案。分割任务中，nnU-Net和Mamba-like状态空间模型结合伪标签和一致性正则化，在隐藏测试集上获得Dice分数0.967和实例亲和力0.738。配准任务中，PointNetLK结合可微SVD和几何增强处理模态差异，混合神经-经典细化实现了准确对齐。

Conclusion: STSR 2025挑战赛成功建立了数字牙科半监督学习的基准，展示了半监督方法在有限标注数据下的有效性。所有数据和代码已公开，确保可重复性，为未来研究提供了宝贵资源。
...
TL;DR: 本文提出了一种在线强化学习后训练框架，用于优化预训练视频生成器的精确相机控制，通过设计可验证的几何奖励函数来解决奖励稀疏性问题，显著提升了相机控制的准确性和视觉质量。


<details>
  <summary>Details</summary>
Motivation: 当前视频扩散模型在相机控制视频生成方面主要依赖监督微调，而在线强化学习后训练方法尚未得到充分探索。现有的方法在精确相机控制方面存在局限性，需要更有效的优化框架来提升相机控制的准确性和一致性。

Method: 1. 提出在线强化学习后训练框架，优化预训练视频生成器的相机控制能力
2. 设计可验证的几何奖励函数：估计生成视频和参考视频的3D相机轨迹，将轨迹划分为短片段，计算片段间的相对位姿
3. 通过比较生成-参考片段对并分配对齐分数作为奖励信号，缓解奖励稀疏性问题
4. 构建包含多样化大振幅相机运动和不同主体动态场景的综合数据集

Result: 实验表明，该在线强化学习后训练框架在多个方面明显优于监督微调基线：相机控制准确性更高、几何一致性更好、视觉质量更优。证明了该方法在推进相机控制视频生成方面的优越性。

Conclusion: 提出的在线强化学习后训练框架通过设计密集片段级反馈的几何奖励函数，有效解决了相机控制视频生成中的奖励稀疏性问题，显著提升了模型的相机控制精度和生成质量，为视频生成领域提供了新的优化方向。
...
TL;DR: MindGPT-4ov是一个多模态大语言模型，提出了一种通用的后训练范式，在多个基准测试中达到最先进性能，同时保持低成本。该工作关注数据构建、监督微调策略和多模态强化学习方法，并实现了基础设施优化。


<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型在数据质量、训练效率和部署成本方面存在挑战。该研究旨在开发一种通用的后训练范式，既能提升模型的基础能力，又能增强泛化能力，同时降低训练和部署成本。

Method: 提出了三个关键创新：1) 基于信息密度的数据生成方案，结合双维度树状标签系统，实现高质量跨领域数据的自动生成；2) 协作课程监督微调方法，平衡领域特定知识注入与通用能力保持；3) 混合强化学习范式，增强推理能力同时解决多样性探索、多模态感知保持和响应简洁性等多目标优化。此外，实施了5D并行训练、算子优化和推理量化等基础设施优化。

Result: MindGPT-4ov在MMBench、MMStar、MathVision和MathVista等基准测试中超越了现有最先进模型。在垂直领域任务中表现出卓越的用户体验，实现了从学术研究到工业部署的无缝过渡。

Conclusion: MindGPT-4ov提供了一个适用于广泛多模态大语言模型的通用后训练范式。该模型基于Qwen3-VL的变体权重、数据集和代码将开源，支持社区的多模态大语言模型开发。
...
TL;DR: 提出一种基于LoRA适配器的两阶段扩散模型加速方法，通过慢速和快速两个专家分别处理不同去噪阶段，实现5倍加速且保持视觉质量


<details>
  <summary>Details</summary>
Motivation: 扩散模型在图像生成方面取得了显著成功，但其部署受到高计算成本和需要大量推理步骤的限制。现有的少步蒸馏方法通常需要大量重新训练成本且泛化能力下降

Method: 采用阶段感知策略：不对所有步骤均匀加速，而是对早期语义阶段应用较小加速，对后期冗余阶段应用较大加速。具体实现为两个专家模型：Slow-LoRA处理慢速去噪阶段，Fast-LoRA处理快速去噪阶段，两者都是轻量级LoRA适配器

Result: 方法实现了相对于基础模型高达5倍的加速，同时在多样化基准测试中保持可比的视觉质量。LoRA专家仅需1个样本在单个V100 GPU上训练1小时，但生成的模型在未见过的提示上表现出强大的泛化能力

Conclusion: 通过简单的LoRA适配器而非大量重新训练学生模型，可以实现高效的扩散模型加速和强大的泛化能力，为扩散模型的部署提供了实用解决方案
...
TL;DR: 论文研究了自回归生成模型中样本多样性的挑战，提出了DiverseAR方法，通过自适应logits分布缩放和基于能量的生成路径搜索，在保持视觉质量的同时显著提升图像多样性。


<details>
  <summary>Details</summary>
Motivation: 研究自回归生成模型中样本多样性不足的问题，特别是在使用比特级视觉分词器时。发现比特级建模的二元分类特性限制了预测空间，而过于尖锐的logits分布导致采样崩溃和多样性降低。

Method: 提出DiverseAR方法：1）自适应logits分布缩放机制，动态调整采样过程中二元输出分布的尖锐度，实现更平滑的预测和更大多样性；2）基于能量的生成路径搜索算法，避免采样低置信度token，保持高视觉质量。

Result: 大量实验表明，DiverseAR在比特级自回归图像生成中显著提高了样本多样性，同时保持了良好的视觉质量。

Conclusion: DiverseAR通过解决比特级自回归模型中的多样性限制因素，提供了一种有效的方法来增强图像生成多样性而不牺牲视觉质量，为自回归生成模型的改进提供了新思路。
...
<details>
  <summary>Details</summary>
Motivation: 现有的文本引导视频编辑方法通常依赖辅助掩码或参考图像进行编辑指导，这限制了方法的可扩展性和泛化能力。视频对象移除和添加任务需要精确的空间和时间一致性，但现有方法在这方面存在挑战。

Method: 提出LoVoRA框架，采用独特的数据集构建流程，结合图像到视频转换、基于光流的掩码传播和视频修复技术。核心创新是可学习的对象感知定位机制，为对象插入和移除任务提供密集的时空监督。通过扩散掩码预测器实现端到端视频编辑，推理时无需外部控制信号。

Result: 大量实验和人工评估证明了LoVoRA的有效性和高质量性能。该方法在视频对象移除和添加任务上表现出色，实现了时空一致的编辑效果。

Conclusion: LoVoRA是一个无需掩码的视频编辑框架，通过创新的对象感知定位机制解决了现有方法在可扩展性和泛化能力方面的限制，为文本引导的视频对象编辑提供了有效的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有的室内布局估计方法通常需要复杂的后处理流程，缺乏对几何约束的直接建模，且计算效率不高。本文旨在开发一个端到端的框架，能够直接预测几何结构，同时保持曼哈顿世界约束，提高计算效率。

Method: 1. 基于OneFormer的通用分割架构，利用任务条件查询和对比学习；2. 布局退化策略：通过拓扑感知变换增强训练数据，同时保持曼哈顿世界约束；3. 可微几何损失：直接强制平面一致性和锐利边界预测；4. 端到端训练框架，消除复杂后处理。
...
Conclusion: Layout Anything框架通过结合几何感知和计算效率，实现了端到端的高性能室内布局估计，特别适用于增强现实应用和大规模3D场景重建任务。
...
<details>
  <summary>Details</summary>
Motivation: 现有LiDAR和相机在BEV表示中的融合方法由于传感器几何精度差异，简单融合会导致性能下降。需要一种更智能的融合策略来缓解图像深度估计误差带来的空间错位问题

Method: 提出BEVDilation框架：1）将图像BEV特征作为隐式引导而非简单拼接；2）稀疏体素扩张块利用图像先验稠密化前景体素；3）语义引导BEV扩张块结合图像语义引导和长程上下文捕获增强LiDAR特征扩散

Result: 在nuScenes基准测试中优于现有最先进方法，同时保持计算效率竞争力。以LiDAR为中心的策略对深度噪声表现出更强的鲁棒性

Conclusion: BEVDilation通过以LiDAR为中心的融合策略，有效解决了多传感器融合中的空间错位问题，提升了3D目标检测性能，对深度噪声具有更好的鲁棒性
...
TL;DR: 本文提出了一种针对多模态大语言模型的新型图像中心攻击方法CIA，通过多智能体系统将有害查询嵌入看似良性的视觉上下文中，显著提升了攻击成功率。


<details>
  <summary>Details</summary>
Motivation: 现有针对多模态大语言模型的越狱攻击方法主要关注文本-图像交互，将视觉模态视为次要提示，未能充分利用图像承载复杂上下文信息的独特潜力。

Method: 提出上下文图像攻击(CIA)方法，采用多智能体系统，通过四种不同的可视化策略将有害查询微妙地嵌入看似良性的视觉上下文中，并结合上下文元素增强和自动毒性混淆技术提升攻击效果。

Result: 在MMSafetyBench-tiny数据集上的实验结果显示，CIA对GPT-4o和Qwen2.5-VL-72B模型的毒性分数分别达到4.73和4.83，攻击成功率分别达到86.31%和91.07%，显著优于先前工作。

Conclusion: 视觉模态本身是越狱先进多模态大语言模型的有效载体，CIA方法通过充分利用图像的上下文信息，显著提升了攻击效果，揭示了当前多模态大语言模型安全对齐的脆弱性。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法依赖人工干预或未充分利用智能体自主缓解幻觉的能力，需要借鉴人类在现实世界中通过内省推理和外部验证做出可靠决策的认知范式。

Method: 提出InEx框架：1) 基于熵的不确定性估计指导内部反思推理；2) 通过编辑智能体和自我反思智能体的外部跨模态多智能体协作，迭代验证和精炼响应。

Result: 在通用和幻觉基准测试中持续优于现有方法，获得4%-27%的性能提升，并展现出强大的鲁棒性。

Conclusion: InEx通过模拟人类认知决策过程，有效缓解了多模态大语言模型的幻觉问题，为开发可靠的多模态系统提供了训练免费的自主解决方案。
...
TL;DR: U4D是一个不确定性感知的4D LiDAR世界建模框架，通过"难到易"的两阶段生成和时空融合模块，在复杂场景中实现几何保真和时间一致的LiDAR序列生成。
...
Method: 1) 使用预训练分割模型估计空间不确定性图来定位语义挑战区域；2) 采用"难到易"的两阶段生成：不确定性区域建模（重建高熵区域）和不确定性条件补全（合成剩余区域）；3) 引入混合时空(MoST)块自适应融合时空表示以确保时间一致性。

Result: 大量实验表明，U4D能够生成几何保真且时间一致的LiDAR序列，提升了4D世界建模在自动驾驶感知和仿真中的可靠性。

Conclusion: U4D通过不确定性感知的生成策略和时空融合机制，有效解决了现有4D LiDAR建模方法在复杂区域产生伪影的问题，为自动驾驶和具身AI提供了更可靠的4D世界建模框架。
...
TL;DR: 本文提出了首个具有可证明保证的LLM-验证器收敛定理，建立了形式化验证工具与大型语言模型交互的理论框架，通过马尔可夫链建模和大量实验验证了系统的可靠收敛性。


<details>
  <summary>Details</summary>
Motivation: 当前使用形式化验证工具与大型语言模型结合的方法缺乏可靠的理论基础，导致验证过程不稳定，有时收敛，有时循环，有时偏离稳定轨迹。需要建立具有可证明保证的形式化框架来填补这一关键空白。

Method: 将LLM与验证器的交互建模为离散时间马尔可夫链，状态转移由关键参数错误减少概率(δ)决定。开发了LLM-验证器收敛定理，证明了对于任何δ>0，过程几乎必然达到验证状态，且期望迭代次数有界。通过超过90,000次试验进行了大规模实证验证。

Result: 理论预测与实证结果高度一致：所有运行都达到了验证状态，收敛因子紧密聚集在Cf≈1.0附近。基于此证据，将工作流程划分为三个不同的操作区域：边际、实用和高性能区域，并建立了明确的设计阈值。

Conclusion: 该理论保证和实验证据为LLM辅助验证提供了更清晰的架构基础，消除了启发式调优的需求，为工程师提供了支持可预测资源规划和性能预算的框架，特别适用于安全关键软件环境的部署。
...
<details>
  <summary>Details</summary>
Motivation: 流程图是常见的流程沟通工具，但通常以静态图像形式共享，难以编辑和重用。现有工具缺乏将流程图图像转换为结构化、版本可控的文本表示的能力。

Method: 使用详细的系统提示和视觉语言模型，将流程图图像转换为Mermaid.js代码。系统支持混合主动式细化，包括内联文本编辑、拖放节点插入，以及由集成AI助手解释的自然语言命令。

Result: 开发了一个轻量级Web系统，能够生成结构化、版本可控的文本表示，并与渲染的图表保持同步。引入了评估指标来评估多个模型的结构准确性、流程正确性、语法有效性和完整性。

Conclusion: 与之前的图像到图表工具不同，该方法产生结构化、版本可控的文本表示，保持与渲染图表的同步，为流程图编辑和重用提供了更有效的解决方案。
...
TL;DR: 提出一种将复杂世界模型分解为子转换器的方法，支持并行化推理和可解释性，为AI安全与计算效率搭建桥梁


<details>
  <summary>Details</summary>
Motivation: 现实世界模型通常计算需求高，但真实场景往往包含模块化交互的子组件，需要开发能够分解复杂世界模型的框架，以支持并行化、可解释的推理

Method: 开发基于转换器（transducers）的分解框架，转换器是POMDP的泛化模型，通过逆组合过程将复杂世界模型分解为在独立输入输出子空间上运行的子转换器

Result: 建立了将复杂世界模型分解为子转换器的理论基础，提供了可并行化和可解释的替代方案，支持分布式推理

Conclusion: 这些结果为连接AI安全所需的结构透明度和实际推理所需的计算效率奠定了基础，为模块化世界建模提供了理论框架
...
TL;DR: 论文分析了当前AI系统的七大核心缺陷，包括缺乏内在自我监控、元认知意识不足、学习机制固定不灵活等，主张向基于认知科学的AI（认知自主性）范式转变，实现自我导向适应和动态表征管理。


<details>
  <summary>Details</summary>
Motivation: 尽管人工智能在感知、语言、推理和多模态领域取得了快速进展，但现代AI系统在自我监控、自我纠正和动态环境中自主调节行为方面仍存在根本性限制。这些结构性限制阻碍了当前架构（包括深度学习和基于Transformer的系统）实现鲁棒的泛化、终身适应性和现实世界自主性。

Method: 通过比较分析人工系统和生物认知，整合AI研究、认知科学和神经科学的见解，识别并分析了制约当代AI模型的七大核心缺陷。同时提出了超越这些限制的前瞻性视角，即通过模仿神经认知原理的架构来发展AI。

Result: 识别出当前AI系统的七大核心缺陷：1) 缺乏内在自我监控；2) 缺乏元认知意识；3) 固定且非自适应的学习机制；4) 无法重构目标；5) 缺乏表征维护；6) 具身反馈不足；7) 缺乏内在能动性。这些缺陷无法通过单纯扩展规模来解决。

Conclusion: 主张向基于认知科学的AI（认知自主性）进行范式转变，这种AI能够实现自我导向适应、动态表征管理和有意图的目标导向行为，同时配备改革性监督机制，确保自主系统保持可解释性、可治理性并与人类价值观保持一致。
...
TL;DR: MERINDA是一个针对边缘设备的FPGA加速模型恢复框架，通过可并行神经架构替代迭代求解器，显著降低内存和能耗，适用于任务关键自主系统。


<details>
  <summary>Details</summary>
Motivation: 模型恢复（MR）在任务关键自主系统（MCAS）中实现安全、可解释的决策制定，但神经常微分方程（NODEs）的迭代特性在FPGA上效率低下，阻碍了其在边缘设备的部署。内存和能耗是边缘设备实时操作的主要瓶颈。

Method: 提出MERINDA框架，用可并行化的神经架构替代NODEs中的迭代求解器，实现FPGA加速。该架构在功能上等同于NODEs，但更适合FPGA并行计算。

Result: 相比移动GPU，MERINDA实现了近11倍的DRAM使用量降低和2.2倍的运行速度提升。实验揭示了在固定精度下内存与能耗的逆相关关系。

Conclusion: MERINDA框架特别适合资源受限、需要实时操作的任务关键自主系统，在边缘设备上实现了高效、低能耗的模型恢复。
...
TL;DR: 该研究提出了一种用于乳腺上皮细胞分割的深度学习框架，仅需599张训练图像即可达到95.5%的Dice分数，显著减少了医学图像标注所需的时间和专家投入。


<details>
  <summary>Details</summary>
Motivation: 医学图像标注需要大量时间和专业知识，特别是乳腺上皮细胞数据集标注需要病理学家投入数百小时。这是临床感知AI发展的一个关键瓶颈，需要解决标注成本高的问题。

Method: 采用量子启发的多尺度Gabor滤波器进行边缘增强，创建第四输入通道；使用稳定化的多组件损失函数（自适应Dice损失+边界感知项+自动正样本加权）；引入基于复杂度的加权采样策略；采用EfficientNet-B7/UNet++架构，支持4通道到3通道的投影；通过指数移动平均和统计异常值检测进行鲁棒验证。

Result: 在严重类别不平衡（乳腺组织仅占4%像素，60%图像无乳腺区域）的情况下，达到95.5% ± 0.3%的Dice分数和91.2% ± 0.4%的IoU。量子增强使边界精度提升2.1%，加权采样使小病灶检测提升3.8%。

Conclusion: 该框架通过有限标注实现了突破性性能，显著减少了医学专家创建数据集所需的时间，解决了临床感知AI发展的一个基本瓶颈问题。
...
TL;DR: OmniGuard是首个面向全模态大语言模型的安全护栏系统，通过结构化安全标签和专家模型蒸馏，在文本、图像、视频、音频等多种模态上实现统一的安全防护。


<details>
  <summary>Details</summary>
Motivation: 全模态大语言模型处理文本、图像、视频、音频等多种模态数据，带来了新的安全挑战。现有安全护栏研究主要针对单模态设置，且通常将安全防护视为二元分类问题，这限制了在不同模态和任务间的鲁棒性。

Method: 提出OmniGuard家族，首个具备深思熟虑推理能力的全模态安全护栏系统。构建了包含超过21万个多样化样本的大型全模态安全数据集，涵盖所有模态的单模态和跨模态样本。每个样本都标注了结构化安全标签，并通过目标蒸馏从专家模型获取精心策划的安全评估。

Result: 在15个基准测试上的广泛实验表明，OmniGuard在广泛的多模态安全场景中实现了强大的有效性和泛化能力。

Conclusion: OmniGuard提供了一个统一框架，能够在全模态中执行策略并降低风险，为构建更鲁棒和更强大的全模态安全系统铺平了道路。
...
TL;DR: ReMindView-Bench是一个用于评估视觉语言模型多视角空间推理能力的认知基准，揭示了当前模型在跨视角对齐和视角转换方面的系统性失败。


<details>
  <summary>Details</summary>
Motivation: 当前视觉语言模型在多视角设置中进行空间推理时，难以保持几何一致性和跨视角一致性。缺乏能够将多视角推理与单视角感知和时间因素分离的细粒度基准，阻碍了对模型空间认知能力的深入分析。

Method: 提出了ReMindView-Bench基准，系统性地变化视角空间模式和查询类型来探测空间认知的关键因素。对15个当前VLMs进行评估，并采用显式分阶段分析（LLM-as-a-judge和自一致性提示）和隐式分析（线性探测和熵动态）来诊断推理过程。

Result: 评估显示当前VLMs在多视角空间推理中存在跨视角对齐和视角转换的一致失败。显式分析表明模型在帧内感知表现良好，但在跨视角信息整合时性能急剧下降。隐式分析显示任务相关信息逐渐丢失，正确与错误轨迹之间的不确定性分离。

Conclusion: 该研究提供了基于认知的VLM空间推理诊断，揭示了多视角空间心理模型在推理过程中如何形成、退化和失稳。ReMindView-Bench基准为深入理解模型的空间认知能力提供了重要工具。
...
### [95] [Synthetic Error Injection Fails to Elicit Self-Correction In Language Models](https://arxiv.org/abs/2512.02389)
*David X. Wu,Shreyas Kapur,Anant Sahai,Stuart Russell*

Main category: cs.AI

TL;DR: 研究探讨了使用监督学习与合成错误注入来诱导语言模型自我修正能力的方法，但发现该方法效果有限，无法显著提升性能，即使模型发现错误也常常重复原始错误。


<details>
  <summary>Details</summary>
Motivation: 强化学习已成为激发大型语言模型推理和自我修正能力的主要方法，但其计算成本高昂，促使研究者探索替代方案。受自动驾驶和机器人技术启发，研究是否可以通过监督学习结合合成错误注入来诱导语言模型的自我修正能力。

Method: 在推理链中插入人工错误，掩盖这些错误，然后监督模型识别和修正这些错误。通过合成错误注入的方式训练模型进行自我修正。

Result: 该方法即使在简单的合成任务上也无法显著提升多个模型的性能。即使模型发现了自己的错误，也常常会重复原始错误。研究发现，从合成错误到策略错误的分布偏移显著降低了微调模型的错误修正能力，即使合成错误对策略错误有良好的覆盖。

Conclusion: 研究结果有助于解释为什么策略强化学习方法在激发自我修正能力方面被证明是独特有效的。监督学习与合成错误注入的方法虽然直观，但在实际应用中效果有限。
...
### [96] [Guided Self-Evolving LLMs with Minimal Human Supervision](https://arxiv.org/abs/2512.02472)
*Wenhao Yu,Zhenwen Liang,Chengsong Huang,Kishan Panaganti,Tianqing Fang,Haitao Mi,Dong Yu*

Main category: cs.AI

TL;DR: R-Few是一个通过轻量级人类监督实现AI稳定自我进化的框架，采用挑战者-求解器自博弈架构，在数学和通用推理任务上实现持续迭代改进。


<details>
  <summary>Details</summary>
Motivation: 当前AI自我进化系统存在概念漂移、多样性崩溃和错误进化等问题，导致模型容易陷入平台期甚至性能退化。需要一种在最小化人类监督依赖的同时，实现稳定可控自我进化的方法。

Method: 提出R-Few框架：1) 挑战者-求解器自博弈架构；2) 挑战者通过少量人类标注示例指导合成问题生成；3) 求解器在在线难度课程下联合训练人类和合成示例；4) 通过上下文基础化和混合训练实现轻量级人类监督。

Result: 在数学和通用推理基准测试中，R-Few实现持续迭代改进：Qwen3-8B-Base在数学任务上比R-Zero提高3.0分，性能与使用20倍人类数据训练的General-Reasoner相当。消融研究证实了基础化挑战者训练和课程求解器训练的互补贡献。

Conclusion: R-Few通过轻量级人类监督和自博弈框架，有效缓解了概念漂移问题，实现了更稳定可控的协同进化动态，为AI自我进化提供了一条可行的技术路径。
...
### [97] [COPE: Chain-Of-Thought Prediction Engine for Open-Source Large Language Model Based Stroke Outcome Prediction from Clinical Notes](https://arxiv.org/abs/2512.02499)
...
TL;DR: COPE框架使用链式思维增强的LLaMA-3-8B模型，从非结构化临床笔记中预测急性缺血性卒中90天功能结局，性能与GPT-4.1相当，优于传统方法。
...
Method: 开发了链式思维结局预测引擎（COPE），基于两个顺序的LLaMA-3-8B开源模型：第一步生成临床推理，第二步输出改良Rankin量表预测。使用464名急性缺血性卒中患者的出院总结和90天mRS评分进行评估。

Result: COPE的MAE为1.01（95% CI 0.92-1.11），±1准确率为74.4%，精确准确率为32.8%，性能与GPT-4.1相当，优于ClinicalBERT（MAE 1.24）、临床机器学习模型（MAE 1.28）和单步LLM（MAE 1.20）。
...
### [98] [Aetheria: A multimodal interpretable content safety framework based on multi-agent debate and collaboration](https://arxiv.org/abs/2512.02530)
*Yuxiang He,Jian Zhao,Yuchen Yuan,Tianle Zhang,Wei Cai,Haojie Cheng,Ziyan Shi,Ming Zhu,Haichuan Tang,Chi Zhang,Xuelong Li*

Main category: cs.AI

TL;DR: Aetheria是一个基于多智能体辩论与协作的多模态可解释内容安全框架，通过五个核心智能体的协作架构和动态辩论机制，显著提升内容安全检测的准确性和可解释性。
...
Method: 提出Aetheria框架，采用五个核心智能体的协作架构，通过基于RAG的知识检索支持的动态相互说服辩论机制，对多模态内容进行深度分析和裁决。
...
Method: 开发基于Chrome扩展的插件式学术写作助手，包含Kubernetes原生编排层和Model Context Protocol工具链，实现可靠的双向编辑器同步、细粒度版本控制、安全状态管理、多智能体调度和外部工具集成

Result: 展示了完全集成的工作流程，包括本地化编辑、结构化评审、并行智能体执行和基于差异的更新，早期聚合分析显示积极的用户参与度

Conclusion: PaperDebugger验证了编辑器原生、智能体驱动的写作助手的实用性，为学术写作提供了深度集成的LLM驱动推理环境
...
TL;DR: TACDA是一种用于跨域剩余使用寿命预测的新型域自适应方法，通过目标域重建策略和退化阶段一致性对齐，解决了现有方法忽略目标特定信息和退化阶段不一致性的问题。


<details>
  <summary>Details</summary>
Motivation: 现有数据驱动的RUL预测方法通常假设训练和测试数据来自相同分布，但在实际工业环境中这一假设不成立。现有的对抗域自适应方法虽然关注域不变特征，但忽略了目标特定信息和退化阶段的一致性特征，导致性能不佳。

Method: 提出TACDA方法：1）在对抗自适应过程中引入目标域重建策略，在保持目标特定信息的同时学习域不变特征；2）开发新颖的聚类和配对策略，实现相似退化阶段之间的一致性对齐。

Result: 通过大量实验验证，TACDA方法在两个不同的评估指标上都显著超越了现有最先进方法，表现出卓越的性能。

Conclusion: TACDA通过结合目标域重建和退化阶段一致性对齐，有效解决了跨域RUL预测中的域差异问题，为实际工业应用提供了更可靠的解决方案。
...
TL;DR: 该论文研究了大型语言模型在递归推理问题中的深度泛化能力，发现标准Transformer架构在处理比训练时更深的嵌套结构时表现不佳，并提出了一种循环定位-替换管道方法来缓解这一问题。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在许多任务上表现出色，但在处理需要解决嵌套层次结构的递归推理问题时面临显著挑战。先前研究主要关注长度泛化（模型处理比训练时更长序列的能力），而本文研究了一个不同且未被充分探索的限制：深度泛化。深度指的是层次问题中的嵌套层数，如数学表达式中的括号层数或布尔公式中的逻辑子句嵌套。

Method: 为了解决深度泛化问题，作者开发了一种新颖的循环定位-替换管道方法，将递归问题分解为可管理的子组件。该方法采用两个专门模型：定位器（识别可解决的子表达式）和替换器（评估这些组件同时保持整体结构）。

Result: 在三个精心设计的领域（布尔代数、递归算术和命题逻辑）进行评估，每个领域都有可控的递归深度。实验表明，该方法在测试超出分布范围的递归深度时，有效缓解了性能衰减问题。

Conclusion: 标准Transformer架构在处理比训练时更深的递归问题时存在局限性，这源于它们无法维持类似栈的行为来跟踪和解决多级嵌套依赖。提出的循环定位-替换管道方法为解决这一深度泛化挑战提供了有效途径。
...
<details>
  <summary>Details</summary>
Motivation: 现有基于推理的多模态情感理解方法存在推理漂移问题：模型逐渐依赖自身生成的文本而非多模态证据，且解释过度受视觉主导推理路径影响，导致情感解释不可靠。

Method: 提出模态重要性机制识别情感主导模态，基于此重组推理序列，使解释从最关键的情感模态开始。采用两阶段框架：模态对齐监督微调和模态感知奖励优化，确保生成情感基础、因果相关且连贯的解释。

Result: 在DFEW基准测试中，MIGR显著提高推理可靠性，将正确预测伴随情感不一致解释的比例从18.10%降至7.37%，验证了从情感主导模态开始推理的有效性。

Conclusion: MIGR框架通过模态重要性引导推理，有效解决多模态情感理解中的推理漂移问题，提高模型解释的可靠性和一致性，为多模态大语言模型的情感理解提供了更可靠的推理机制。
...
<details>
  <summary>Details</summary>
Motivation: 随着生成模型能力增强，透明度、问责制和版权侵权问题日益突出。理解特定训练数据如何影响模型输出至关重要，需要一种能够解释生成内容与训练数据关系的框架。
...
Conclusion: 该框架能够追踪生成模型输出与训练数据的关系，促进AI系统的透明度、问责制和版权合规，支持人类协作、创造力和好奇心的发展。
...
TL;DR: Menta是首个针对社交媒体多任务心理健康预测优化的轻量级小语言模型，在抑郁、压力和自杀倾向等任务上表现优于现有SLM和13B参数LLM，可在iPhone上实时部署。


<details>
  <summary>Details</summary>
Motivation: 全球数亿人受心理健康问题影响，但早期检测仍有限制。虽然大语言模型在心理健康应用中有潜力，但其规模和计算需求阻碍实际部署。小语言模型提供了轻量级替代方案，但在社交媒体心理健康预测方面研究不足。

Method: 提出Menta模型，采用LoRA微调框架、跨数据集策略和平衡准确率导向的损失函数，联合训练六个分类任务，专门针对社交媒体数据进行多任务心理健康预测优化。

Result: 相比9个最先进的SLM基线，Menta在抑郁、压力和自杀倾向等任务上平均提升15.2%；在抑郁和压力分类任务上准确率优于13B参数LLM，同时模型大小约小3.25倍；可在iPhone 15 Pro Max上实时部署，仅需约3GB内存。
...
TL;DR: StockMem是一个用于股票价格预测的双层记忆框架，通过事件整合和纵向追踪构建事件知识库，结合事件-价格动态分析形成因果经验库，从而提供可解释的预测。
...
Method: 提出StockMem事件-反思双层记忆框架：1）将新闻结构化处理为事件；2）水平整合整合每日事件；3）纵向追踪捕捉事件演变以提取反映市场预期差异的增量信息，构建时序事件知识库；4）通过分析事件-价格动态形成因果经验的反思知识库；5）预测时检索类似历史场景，结合当前事件、增量数据和过往经验进行推理。

Result: 实验表明StockMem优于现有记忆架构，通过追踪影响价格的信息链提供更优且可解释的推理，增强了金融预测的决策透明度。

Conclusion: StockMem框架通过事件整合和纵向追踪有效处理金融新闻数据，结合因果经验库提供可解释的股票价格预测，解决了传统方法在识别关键价格驱动因素方面的局限性。
...
Method: 使用LLaMA和Gemma等先进大语言模型，在合成和真实匿名分类账上进行基准测试，与传统JETs和机器学习基线对比
...
TL;DR: 该论文将心理测量电池的模数理论框架扩展到动力系统领域，将智能体形式化为由计算资源参数化的流，推导出保证自我改进稳定性的方差不等式条件，并统一了语言自我博弈、自我纠正和合成数据引导等近期研究。


<details>
  <summary>Details</summary>
Motivation: 将静态的智能体能力评估框架扩展到动态系统，为理解智能体在计算资源变化下的自我改进过程提供理论工具，统一近期关于语言自我博弈、自我纠正和合成数据引导的研究。

Method: 提出生成器-验证器-更新器（GVU）算子，将智能体形式化为参数流，在参数流形上生成向量场，定义自我改进系数κ作为能力泛函沿该流的李导数，推导方差不等式作为自我改进稳定性的谱条件。

Result: 证明了GVU算子在参数流形上生成向量场，推导出方差不等式作为自我改进稳定性的充分条件，表明当生成和验证的联合噪声足够小时κ>0，并将STaR、SPIN、Reflexion、GANs和AlphaZero等架构统一为满足方差不等式的GVU算子的拓扑实现。

Conclusion: 该工作为智能体自我改进的动力学提供了统一的模数理论框架，将近期多种自我改进方法统一为GVU算子的不同实现，方差不等式为设计稳定自我改进系统提供了理论指导。
...
TL;DR: 提出了一个基于因果概念的事后可解释AI框架，通过概念干预的充分概率生成局部和全局解释，在CelebA数据集上进行了概念验证


<details>
  <summary>Details</summary>
Motivation: 针对非可解释模型的解释需要同时满足可理解性和忠实性要求，现有方法在这两个方面的平衡存在不足

Method: 基于因果概念框架，通过计算概念干预的充分概率来生成局部和全局解释，使用CelebA数据集训练的分类器进行概念验证

Result: 展示了基于清晰概念词汇的解释示例，通过隐含的因果解释提高了可理解性，同时强调了解释生成与解释解释的上下文一致性以确保忠实性

Conclusion: 提出的因果概念框架为事后XAI提供了可理解且忠实的方法，但需要确保解释生成和解释解释的上下文对齐，强调了框架假设的重要性
...
Method: 提出基于两个协作智能体的框架：验证智能体检查每个步骤输出是否满足系统提示要求，精炼智能体根据识别的问题修订输出；无需为每个步骤设计特定精炼提示，仅利用原始系统提示实现自动验证和改进

Result: 在PaperBench Code-Dev和Paper2CodeBench数据集上的实验表明，该方法显著提高了复现代码的准确性和完整性，相比无智能体的基线分别获得约15%和13%的性能提升；与Self-Refine的对比实验验证了该提示无关方法在不同数据集上的鲁棒性和一致性

Conclusion: 提出的无需提示的协作智能体框架有效解决了现有论文到代码生成框架的局限性，通过自动验证和精炼机制显著提升了复现代码质量，为自动化科学研究提供了更高效、可扩展的解决方案
...
TL;DR: 提出基于贝叶斯主义和证伪原则的新科学方法，通过AI减少认知偏见和假阳性，降低矿产勘探成本


<details>
  <summary>Details</summary>
Motivation: 全球能源转型推动关键矿产需求增长，但过去20年新发现减少，需要更有效的勘探方法

Method: 基于贝叶斯主义和证伪原则的哲学方法，将数据采集视为证伪人类假设的手段，使用可验证指标进行理性决策，结合无监督学习和人在回路AI算法

Result: 提供实用协议模板，通过AI协作减少认知偏见，优化地质、地球物理、地球化学和钻探数据采集规划

Conclusion: AI赋能的新科学方法能显著改善矿产勘探效率，通过减少假阳性和认知偏见来应对关键矿产发现减少的挑战
...
### [111] [Stepwise Schema-Guided Prompting Framework with Parameter Efficient Instruction Tuning for Multimedia Event Extraction](https://arxiv.org/abs/2512.02584)
*Xiang Yuan,Xinrong Chen,Haochen Li,Hang Yang,Guanyu Wang,Weiping Li,Tong Mo*

Main category: cs.MM

TL;DR: 本文提出了一种基于多模态大语言模型的逐步模式引导提示框架SSGPF，用于解决多媒体事件抽取任务中的结构建模和数据对齐挑战，在M2E2基准上显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 当前多媒体事件抽取面临两大挑战：1) 缺乏能够处理复杂灵活多媒体事件结构的抽取框架建模；2) 缺少多模态对齐的训练数据来有效进行知识迁移。

Method: 提出逐步模式引导提示框架SSGPF，以多模态大语言模型为骨干。首先设计事件类型模式引导提示ETSGP进行事件检测，然后设计包含多步提示和文本桥接定位技术的论元角色模式引导提示ARSGP进行论元抽取。基于现有单模态事件标注构建弱对齐多模态事件标注数据集，并在LLaVA-v1.5-7B上使用LoRA进行参数高效指令微调。

Result: 在M2E2基准测试中，SSGPF在事件检测上比当前SOTA基线提升5.8% F1值，在论元抽取上提升8.4% F1值。

Conclusion: SSGPF框架通过逐步模式引导提示和弱对齐数据构建，有效解决了多媒体事件抽取中的结构建模和数据对齐问题，显著提升了性能。
...
### [112] [Contextual Gating within the Transformer Stack: Synergistic Feature Modulation for Enhanced Lyrical Classification and Calibration](https://arxiv.org/abs/2512.02053)
*M. A. Gameiro*

Main category: cs.LG

TL;DR: SFL Transformer通过上下文门控机制在BERT编码器中间层融合结构特征，显著提升了歌词内容分类性能，在准确率和概率校准方面都优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有特征融合方法通常在最终输出层结合语义和结构特征，但这种方法可能无法充分利用两者的协同效应。本研究旨在探索在Transformer中间层注入辅助结构特征是否能够更有效地结合结构信息和语义信息，从而提升分类性能和概率校准质量。

Method: 提出SFL Transformer模型，采用上下文门控机制（Intermediate SFL）在BERT编码器堆栈的隐藏状态序列中直接融合低维结构特征。该方法不是简单地在最终输出层融合特征，而是在中间层调制深度上下文语义特征，使结构线索能够更早地影响语义表示的形成。

Result: 在基于UMAP降维歌词嵌入的二元分类任务中，SFL Transformer取得了0.9910的准确率和宏F1分数，显著超越了之前发表的SFL模型（准确率0.9894）。更重要的是，模型表现出卓越的可靠性：期望校准误差仅为0.0081，对数损失为0.0489。

Conclusion: 研究证实了在Transformer中间层注入辅助上下文是结合结构信息和语义信息的最有效方式。上下文门控策略不仅创造了具有卓越判别能力的模型，还实现了高保真的概率估计，为特征融合提供了新的架构范式。
...
### [113] [Opening the Black Box: An Explainable, Few-shot AI4E Framework Informed by Physics and Expert Knowledge for Materials Engineering](https://arxiv.org/abs/2512.02057)
...
TL;DR: 提出一个基于物理和专家知识的可解释少样本AI4E框架，通过物理增强合成数据和嵌套优化策略，仅用32个实验样本就实现了88%准确率的可解释本构方程预测热裂倾向。


<details>
  <summary>Details</summary>
Motivation: 工业AI4E面临两大瓶颈：高质量数据稀缺和黑盒模型缺乏可解释性，这在航空航天等安全敏感领域尤为关键。需要开发可信赖的AI系统，将工程领域知识直接嵌入架构，在数据有限但物理理解可用的高风险工业应用中实现可靠采用。

Method: 1) 物理增强合成数据生成：通过三阶段协议（校准过程变异性的差异化噪声注入、硬物理约束强制执行、参数间关系保持）从32个实验样本生成物理合理的合成数据；2) 嵌套优化策略：符号回归探索方程结构，差分进化优化参数，然后使用混合全局-局部优化进行密集参数精炼。

Result: 开发的可解释本构方程在预测热裂倾向方面达到88%的准确率。该方程不仅提供定量预测，还提供明确的物理洞察，揭示了热、几何和冶金机制如何耦合驱动开裂。此外，该方程可作为多功能工具用于工艺优化和高保真虚拟数据生成，提升其他数据驱动模型的准确性。

Conclusion: 该方法为开发可信赖的AI系统提供了通用蓝图，将工程领域知识直接嵌入架构，使AI能够在数据有限但物理理解可用的高风险工业应用中可靠采用。通过物理增强和可解释建模，解决了工业AI4E面临的数据稀缺和黑盒问题。
...
### [114] [Ada-MoGE: Adaptive Mixture of Gaussian Expert Model for Time Series Forecasting](https://arxiv.org/abs/2512.02061)
...
<details>
  <summary>Details</summary>
Motivation: 多元时间序列预测中，数据的主导频率会随频谱分布变化而漂移。传统固定专家数量的MoE模型难以适应这种变化，导致频率覆盖不平衡：专家太少会忽略关键信息，太多会引入噪声。
...
Conclusion: Ada-MoGE通过自适应专家数量和高斯带通滤波，有效解决了频率覆盖不平衡问题，在多元时间序列预测中取得了优异性能。
...
### [115] [DPWMixer: Dual-Path Wavelet Mixer for Long-Term Time Series Forecasting](https://arxiv.org/abs/2512.02070)
*Li Qianyang,Zhang Xingjun,Wang Shaoxun,Wei Jia*

Main category: cs.LG

TL;DR: DPWMixer：一种基于无损Haar小波金字塔的双路径架构，用于长期时间序列预测，通过正交分解分离趋势和局部波动，结合全局线性映射和MLP-Mixer处理，在多尺度融合中实现优于现有方法的性能。


<details>
  <summary>Details</summary>
Motivation: 当前长期时间序列预测面临挑战：Transformer模型存在二次复杂度和数据稀疏导致的过拟合问题；线性模型难以捕捉复杂非线性局部动态；现有多尺度框架使用的平均池化作为非理想低通滤波器，会导致频谱混叠和高频瞬态信息不可逆损失。

Method: 提出DPWMixer双路径架构：1）使用无损Haar小波金字塔替代传统池化，通过正交分解无信息损失地分离趋势和局部波动；2）设计双路径趋势混合器，结合全局线性映射进行宏观趋势锚定和基于patch的MLP-Mixer处理微观动态演化；3）自适应多尺度融合模块根据通道平稳性加权整合不同尺度的预测结果。

Result: 在8个公共基准数据集上的广泛实验表明，该方法在长期时间序列预测任务上持续优于现有最先进的基线方法。

Conclusion: DPWMixer通过无损小波分解和双路径处理架构，有效解决了传统方法在长期时间序列预测中的频谱混叠和信息损失问题，实现了更好的预测性能，同时保持了计算效率。
...
### [116] [HTG-GCL: Leveraging Hierarchical Topological Granularity from Cellular Complexes for Graph Contrastive Learning](https://arxiv.org/abs/2512.02073)
*Qirui Ji,Bin Qin,Yifan Jin,Yunze Zhao,Chuxiong Sun,Changwen Zheng,Jianwen Cao,Jiangmeng Li*

Main category: cs.LG

TL;DR: HTG-GCL提出了一种分层拓扑粒度图对比学习框架，通过生成多尺度环基细胞复合体来捕捉不同粒度的拓扑结构，解决了现有GCL方法难以识别任务相关拓扑结构的问题。


<details>
  <summary>Details</summary>
Motivation: 现有图对比学习方法使用结构增强时，难以识别任务相关的拓扑结构，且无法适应不同下游任务所需的从粗到细的拓扑粒度变化。

Method: 提出HTG-GCL框架：1）通过同一图的变换生成多尺度环基细胞复合体，体现拓扑粒度概念；2）提出多粒度解耦对比；3）基于不确定性估计应用粒度特定加权机制。

Result: 在多个基准测试上的综合实验证明了HTG-GCL的有效性，显示出通过分层拓扑信息捕捉有意义的图表示的优越性能。

Conclusion: HTG-GCL通过分层拓扑粒度对比学习，能够更好地捕捉图的有意义表示，解决了现有GCL方法在识别任务相关拓扑结构和适应不同粒度需求方面的局限性。
...
### [117] [FDRMFL:Multi-modal Federated Feature Extraction Model Based on Information Maximization and Contrastive Learning](https://arxiv.org/abs/2512.02076)
*Haozhe Wu*

Main category: cs.LG

TL;DR: 提出一种任务驱动的监督式多模态联邦特征提取方法，解决多模态数据回归中的特征提取问题，应对数据有限非独立同分布、多模态信息有效提取融合、模型易发生灾难性遗忘三大挑战。


<details>
  <summary>Details</summary>
Motivation: 针对现实场景中多模态数据回归面临的三个核心挑战：1) 数据有限且非独立同分布；2) 多模态信息的有效提取与融合；3) 模型学习易受灾难性遗忘影响。需要一种能够同时解决这些问题的特征提取方法。

Method: 提出任务驱动的监督式多模态联邦特征提取方法，集成多模态信息提取和对比学习机制。方法支持各客户端独立学习多模态数据的低维表示，通过参数调整灵活控制预测变量中响应变量有效信息的保留程度。构建多约束学习框架，使用均方误差损失保证回归精度，通过互信息保留约束、对称KL散度约束和模型间对比约束的协同作用，分别实现任务相关信息保留、多模态特征提取融合对齐、以及非独立同分布场景下表示漂移和灾难性遗忘的缓解。

Result: 仿真和真实数据分析的实验结果表明，与经典特征提取技术相比，所提方法在下游回归任务上实现了更显著的性能提升。

Conclusion: 该方法能够有效解决多模态数据回归中的特征提取问题，确保特征提取过程始终以提高下游回归任务性能为中心，在非独立同分布场景下具有良好表现。
...
### [118] [How Market Volatility Shapes Algorithmic Collusion: A Comparative Analysis of Learning-Based Pricing Algorithms](https://arxiv.org/abs/2512.02134)
*Aheer Sravon,Md. Ibrahim,Devdyuti Mazumder,Ridwan Al Aziz*

Main category: cs.LG

TL;DR: 该研究分析了四种定价算法（Q-Learning、PSO、Double DQN、DDPG）在三种经典双寡头模型（Logit、Hotelling、Linear）和不同需求冲击下的竞争行为，发现强化学习算法在稳定需求下维持超竞争价格，DDPG最具合谋倾向，需求冲击对不同市场结构产生差异化影响。


<details>
  <summary>Details</summary>
Motivation: 自主定价算法日益影响数字市场竞争，但其在现实需求条件下的行为尚未得到充分研究。本文旨在填补这一空白，分析算法在不同市场结构和需求冲击下的竞争行为，为政策讨论提供实证依据。

Method: 采用四种定价算法（Q-Learning、PSO、Double DQN、DDPG），在三种经典双寡头模型（Logit、Hotelling、Linear）中进行实验，通过自回归过程创建不同需求冲击机制，使用利润和价格合谋指数评估竞争结果。

Result: 强化学习算法在稳定需求下常维持超竞争价格，DDPG表现出最明显的合谋倾向。需求冲击对不同市场结构产生差异化影响：Logit市场性能显著下降，Hotelling市场保持稳定，Linear市场出现冲击诱导的利润膨胀。尽管绝对性能变化显著，但算法相对排名在不同环境中保持一致。

Conclusion: 市场结构和需求不确定性对算法竞争具有关键影响，这些发现为自主定价行为的政策讨论提供了重要实证依据，强调了在监管数字市场竞争时需要考虑算法特性、市场结构和需求条件的相互作用。
...
### [119] [CLEF: Clinically-Guided Contrastive Learning for Electrocardiogram Foundation Models](https://arxiv.org/abs/2512.02180)
*Yuxuan Shu,Peter H. Charlton,Fahim Kawsar,Jussi Hernesniemi,Mohammad Malekzadeh*

Main category: cs.LG

TL;DR: CLEF是一种新型的对比学习框架，利用临床风险评分自适应加权负样本对，在无标注ECG数据上预训练单导联心电图基础模型，显著提升诊断性能。


<details>
  <summary>Details</summary>
Motivation: 现有自监督预训练方法未充分利用临床元数据中的领域知识，限制了心电图基础模型的诊断性能提升。

Method: 提出临床引导对比学习方法，使用临床风险评分自适应加权负样本对，将ECG嵌入相似性与临床有意义的患者差异对齐，并处理缺失元数据。

Result: 在MIMIC-IV数据集上预训练三个规模的CLEF模型，在18个临床任务上评估，相比基线模型在分类任务AUROC提升至少2.6%，回归任务MAE降低至少3.2%。

Conclusion: CLEF实现了更准确、可扩展的单导联ECG分析，推进远程健康监测，性能与监督学习方法相当。
...
### [120] [Modelling the Doughnut of social and planetary boundaries with frugal machine learning](https://arxiv.org/abs/2512.02200)
...
<details>
  <summary>Details</summary>
Motivation: 甜甜圈模型已成为评估环境和社会可持续性的流行框架，但需要有效方法来寻找实现可持续性的政策参数

Method: 使用随机森林分类器和Q学习等机器学习方法，应用于简单的宏观经济甜甜圈模型，寻找符合可持续性的政策参数和最优轨迹

Result: 机器学习方法能够找到实现环境和社会可持续性的政策参数组合，这些方法是高效的机器学习方法

Conclusion: 概念验证表明机器学习方法可用于甜甜圈模型，下一步是将其应用于更复杂的生态宏观经济模型
...
### [121] [WhAM: Towards A Translative Model of Sperm Whale Vocalization](https://arxiv.org/abs/2512.02206)
...
TL;DR: WhAM是基于Transformer的模型，通过微调VampNet，能够从任意音频提示生成抹香鲸的咔嗒声序列（coda），并保持源录音的关键声学特征。


<details>
  <summary>Details</summary>
Motivation: 抹香鲸通过咔嗒声序列进行交流，但缺乏能够生成高质量合成鲸声的模型。研究旨在开发首个基于Transformer的模型，能够生成逼真的抹香鲸咔嗒声，同时学习到的表征可用于下游分类任务。

Method: 通过微调在音乐音频上预训练的掩码声学标记模型VampNet，使用过去20年收集的1万条coda录音。采用迭代掩码标记预测生成高保真合成coda。

Result: WhAM生成的合成coda在Fréchet音频距离和专家感知研究中表现良好。在节奏、社会单元和元音分类等下游任务中，WhAM学习到的表征尽管是为生成而非分类训练，仍取得强劲性能。

Conclusion: WhAM是首个能够从任意音频提示生成高质量抹香鲸coda的Transformer模型，其学习到的表征在多种分类任务中表现出色，为鲸声研究和保护提供了新工具。
...
### [122] [InstructLR: A Scalable Approach to Create Instruction Dataset for Under-Resourced Languages](https://arxiv.org/abs/2512.02213)
*Mamadou K. Keita,Sebastien Diarra,Christopher Homan,Seydou Diallo*

Main category: cs.LG

TL;DR: InstructLR框架通过LLM驱动文本生成和双重质量过滤机制，为低资源语言创建高质量指令数据集，解决了非洲语言等低资源语言缺乏高质量指令数据的挑战。


<details>
  <summary>Details</summary>
Motivation: 当前大型语言模型在低资源语言（特别是非洲语言）的文本生成和聊天界面支持方面面临挑战，主要原因是缺乏高质量的指令数据集。现有的自动翻译和合成数据生成方法经常产生不流畅甚至拼写不一致的输出。

Method: 提出InstructLR框架，整合LLM驱动的文本生成和双重质量过滤机制：基于检索增强生成（RAG）的n-shot提示的自动过滤层，以及人工验证层。从MMLU等基准测试中汲取任务定义灵感。

Result: 成功创建了三个多领域指令基准数据集：ZarmaInstruct-50k、BambaraInstruct-50k和FulfuldeInstruct-50k，每个包含5万个高质量指令数据。

Conclusion: InstructLR框架为低资源语言生成高质量指令数据集提供了一种有效方法，有助于改善LLM在这些语言上的文本生成和聊天界面支持能力。
...
### [123] [The Effect of Enforcing Fairness on Reshaping Explanations in Machine Learning Models](https://arxiv.org/abs/2512.02265)
*Joshua Wolff Anderson,Shyam Visweswaran*

Main category: cs.LG

TL;DR: 研究探讨了医疗AI中公平性改进如何影响可解释性，发现公平性约束会显著改变Shapley特征重要性排序，且在不同群体间变化方式不同


<details>
  <summary>Details</summary>
Motivation: 医疗AI需要兼顾预测性能、公平性和可解释性。虽然已知提高公平性会影响预测性能，但公平性改进如何影响可解释性（临床信任的关键因素）尚不清楚。临床医生可能不愿依赖公平性约束后解释发生变化的模型

Method: 通过偏置缓解技术增强公平性，研究其如何重塑基于Shapley的特征排序。量化了在三个数据集（儿童尿路感染风险、直接抗凝剂出血风险、再犯风险）上应用公平性约束后特征重要性排名的变化，并评估了多个模型类别在Shapley排序稳定性方面的表现

Result: 提高跨种族亚组的模型公平性会显著改变特征重要性排名，有时在不同群体间以不同方式变化。公平性约束后的解释稳定性因模型类别而异
...
### [124] [Limitations of Membership Queries in Testable Learning](https://arxiv.org/abs/2512.02279)
...
<details>
  <summary>Details</summary>
Motivation: 研究成员查询在可测试学习模型中的效率限制，探索成员查询是否能比仅使用样本的分布特定学习更高效。

Method: 通过将基于样本的布尔概念类反驳归约到带查询的可测试学习，并定义一类"统计"成员查询算法来建立理论框架。

Result: 证明在可测试学习模型中，成员查询算法的时间复杂度不能显著优于最佳PAC学习器，且高效的可测试成员查询算法意味着高效的统计查询算法。

Conclusion: 在可测试学习框架下，成员查询无法提供超多项式的时间效率优势，且许多已知的高效成员查询学习器无法被改造为可测试的。
...
### [125] [Training Dynamics of Learning 3D-Rotational Equivariance](https://arxiv.org/abs/2512.02303)
*Max W. Shen,Ewa Nowara,Michael Maser,Kyunghyun Cho*

Main category: cs.LG

TL;DR: 该研究提出了一种衡量等变误差的原则性方法，发现在高维分子任务中，模型能快速学习3D旋转等变性，通常在1k-10k训练步内将等变误差降至总损失的2%以下。研究发现学习3D旋转等变性比主要预测任务更容易，但非等变模型在训练期间损失惩罚较小，可能比等变模型获得更低的每GPU小时测试损失。


<details>
  <summary>Details</summary>
Motivation: 虽然数据增强被广泛用于训练对称性无关模型，但模型学习尊重对称性的速度和效果仍不清楚。研究者希望了解模型学习等变性的动态过程，特别是在高维分子任务中3D旋转等变性的学习情况。

Method: 提出了一种原则性的等变误差度量方法，用于计算由于学习对称性不完美导致的损失百分比。研究聚焦于高维分子任务（流匹配、力场预测、体素去噪）中的3D旋转等变性，通过实验和理论分析探讨相对等变误差、学习梯度和模型参数之间的关系。

Result: 模型能快速降低等变误差，通常在1k-10k训练步内将等变误差降至总损失的2%以下，这一结果对模型和数据集大小具有鲁棒性。学习3D旋转等变性比主要预测任务更容易，具有更平滑和条件更好的损失景观。非等变模型在训练期间损失惩罚较小，可能比等变模型获得更低的每GPU小时测试损失。

Conclusion: 模型能快速有效地学习3D旋转等变性，但等变模型需要缩小"效率差距"才能在计算效率上与非等变模型竞争。研究揭示了等变性学习的动态特性和损失景观特性，为对称性学习提供了新的见解。
...
TL;DR: 提出了一种基于模拟分叉的玻尔兹曼采样器LSB，结合条件期望匹配CEM，建立了适用于一般玻尔兹曼机的高效学习框架SAL，突破了传统受限玻尔兹曼机的限制。


<details>
  <summary>Details</summary>
Motivation: 玻尔兹曼机是强大的能量生成模型，但训练成本高，实际应用多局限于受限玻尔兹曼机。更精确的学习需要马尔可夫链蒙特卡洛玻尔兹曼采样，但并行化困难且耗时。需要开发适用于一般玻尔兹曼机的高效并行采样和学习方法。

Method: 1. 提出朗之万模拟分叉采样器（LSB），基于量子启发的组合优化方法模拟分叉，实现并行采样；2. 开发条件期望匹配（CEM）方法，在训练过程中估计逆温度参数；3. 结合LSB和CEM建立采样器自适应学习（SAL）框架。

Result: LSB在保持与MCMC相当精度的同时实现了并行化采样，适用于一般耦合的玻尔兹曼机。CEM有效解决了LSB无法控制输出玻尔兹曼分布逆温度的问题。SAL框架为超越受限玻尔兹曼机的能量生成模型开辟了新途径。

Conclusion: 提出的SAL框架通过结合LSB并行采样和CEM逆温度估计，建立了适用于一般玻尔兹曼机的高效学习框架，突破了传统受限玻尔兹曼机的限制，为能量生成模型开辟了新方向。
...
### [127] [Retrieval-Augmented Memory for Online Learning](https://arxiv.org/abs/2512.02333)
*Wenzhang Du*

Main category: cs.LG

TL;DR: RAM-OL是一种用于非平稳环境下在线学习的检索增强内存方法，通过维护过去示例的小缓冲区，在隐藏表示空间中检索当前输入的最近邻，并联合更新模型，以应对概念漂移问题。


<details>
  <summary>Details</summary>
Motivation: 现有检索增强模型在流式监督学习中的概念漂移问题研究不足，需要开发能够适应非平稳环境的在线学习方法，有效利用历史数据来应对模式变化和周期性重现。

Method: 提出RAM-OL方法，扩展随机梯度下降，维护过去示例的小缓冲区。在隐藏表示空间中检索当前输入的最近邻，联合更新当前示例和检索到的邻居。比较朴素回放和门控回放两种变体，后者通过时间窗口、相似度阈值和梯度重加权来约束邻居选择。

Result: 在强漂移和周期性漂移的数据流上，RAM-OL将预序准确率提高了约7个百分点，显著减少了随机种子间的方差；在噪声较大的航空延误数据流上，门控变体与纯在线基线表现相当。

Conclusion: 检索增强内存是处理概念漂移的实用且鲁棒的在线学习工具，能够有效利用历史数据，在模式重现时降低适应成本并改善遗憾常数。
...
### [128] [SpecPV: Improving Self-Speculative Decoding for Long-Context Generation via Partial Verification](https://arxiv.org/abs/2512.02337)
...
<details>
  <summary>Details</summary>
Motivation: 随着代码生成、深度推理和长文档理解等任务需求增长，长上下文生成成为大语言模型的关键能力。推测解码是加速生成的最直接有效方法之一，但随着上下文长度增加，验证过程成为主要瓶颈。

Method: 提出SpecPV自推测解码方法，使用部分键值状态（KV）进行快速验证，并定期应用全验证以消除累积错误，从而加速长上下文生成中的推测解码。

Result: 在多个长上下文基准测试和模型（包括LLaMA-3.1-8B-Instruct和Qwen3系列）上验证，SpecPV相比标准自回归解码实现了高达6倍的解码加速，且性能退化很小。

Conclusion: SpecPV通过部分KV状态验证和周期性全验证的有效结合，成功解决了长上下文生成中推测解码的验证瓶颈问题，为长上下文任务提供了高效的解码加速方案。
...
### [129] [FOVA: Offline Federated Reinforcement Learning with Mixed-Quality Data](https://arxiv.org/abs/2512.02350)
*Nan Qiao,Sheng Yue,Ju Ren,Yaoxue Zhang*

Main category: cs.LG

TL;DR: FOVA是一个基于投票机制的离线联邦强化学习框架，通过投票识别高回报动作来缓解混合质量数据问题，相比现有方法在混合质量数据场景下表现显著提升。


<details>
  <summary>Details</summary>
Motivation: 现有离线联邦强化学习方法在面对混合质量数据（即不同客户端收集的数据来自质量各异的策略）时性能急剧下降，需要新的方法来克服这一限制。

Method: 提出FOVA框架：1）采用投票机制在本地策略评估中识别高回报动作，减轻低质量行为数据的负面影响；2）基于优势加权回归构建一致的本地和全局训练目标，提高效率和稳定性。

Result: 理论分析严格证明FOVA学习的策略相对于行为策略有严格改进；大量实验在广泛使用的基准测试中证实FOVA相比现有基线方法有显著的性能提升。

Conclusion: FOVA通过投票机制有效解决了离线联邦强化学习中混合质量数据的问题，提供了理论保证和实验验证的性能优势。
...
### [130] [Reinforcement Learning in POMDP's via Direct Gradient Ascent](https://arxiv.org/abs/2512.02383)
*Jonathan Baxter,Peter L. Bartlett*

Main category: cs.LG

TL;DR: 本文提出GPOMDP算法，一种基于梯度的直接优化策略性能方法，用于部分可观测马尔可夫决策过程（POMDPs），通过单样本路径估计平均奖励梯度，无需了解底层状态信息。


<details>
  <summary>Details</summary>
Motivation: 针对部分可观测马尔可夫决策过程（POMDPs）中直接优化策略性能的挑战，需要一种能够有效估计平均奖励梯度的方法，同时避免对底层状态的完全了解，并能在单样本路径下工作。

Method: 提出GPOMDP算法，这是一种类似REINFORCE的算法，通过单一样本路径估计平均奖励相对于策略参数的梯度近似值。算法仅需一个自由参数β∈[0,1)来平衡偏差-方差权衡，且不需要了解底层状态信息。

Result: 证明了GPOMDP算法的收敛性，并展示了如何将GPOMDP产生的梯度估计用于共轭梯度过程，以找到平均奖励的局部最优解。

Conclusion: GPOMDP算法为POMDPs中的策略优化提供了一种有效的梯度估计方法，具有单样本路径、参数简单、无需状态知识等优势，可用于梯度优化过程找到策略性能的局部最优解。
...
### [131] [Risk-Sensitive Q-Learning in Continuous Time with Application to Dynamic Portfolio Selection](https://arxiv.org/abs/2512.02386)
*Chuhan Xie*

Main category: cs.LG

TL;DR: 本文研究了连续时间风险敏感强化学习问题，提出了一种基于鞅特征化的风险敏感Q学习算法，并在动态投资组合选择问题上验证了有效性。


<details>
  <summary>Details</summary>
Motivation: 研究连续时间环境下的风险敏感强化学习问题，其中环境由可控随机微分方程描述，目标是最大化累积奖励的非线性泛函，这在金融风险管理等实际应用中具有重要意义。

Method: 采用优化确定性等价（OCE）作为风险敏感目标函数，证明了最优策略相对于增强环境是马尔可夫的。提出CT-RS-q算法，这是一种基于新颖鞅特征化方法的风险敏感Q学习算法。

Result: 理论证明了当目标函数为OCE时，最优策略具有马尔可夫性。在动态投资组合选择问题的仿真研究中，验证了所提算法的有效性。

Conclusion: 本文为连续时间风险敏感强化学习提供了理论框架和实用算法，特别是在金融风险管理等需要处理风险敏感性的领域具有应用价值。
...
### [132] [ESACT: An End-to-End Sparse Accelerator for Compute-Intensive Transformers via Local Similarity](https://arxiv.org/abs/2512.02403)
*Hongxiang Liu,Zhifang Deng,Tong Pu,Shengli Lu*

Main category: cs.LG

TL;DR: ESACT是一个端到端的Transformer稀疏加速器，通过局部相似性预测注意力稀疏性，在所有Transformer组件中实现高效稀疏计算，显著降低计算开销并提升能效。


<details>
  <summary>Details</summary>
Motivation: Transformer模型虽然性能优异但计算成本高，现有稀疏加速器大多只利用注意力机制中的行内稀疏性，很少考虑行间稀疏性。利用行间稀疏性的方法通常依赖昂贵的全局相似性估计，这会削弱稀疏性带来的加速效益，而且通常只应用于一两个Transformer组件。

Method: 提出ESACT端到端稀疏加速器，核心是局部相似性稀疏性预测机制，利用HLog量化在QK生成前准确预测局部注意力稀疏性，实现所有Transformer组件的高效稀疏化。还引入了三种架构创新来支持高效的硬件实现。

Result: 在26个基准测试中，SPLS机制将总计算量减少52.03%，精度损失小于1%。ESACT实现了3.29 TOPS/W的端到端能效，在注意力级别能效上比SOTA注意力加速器SpAtten和Sanger分别提升2.95倍和2.26倍。

Conclusion: 局部相似性方法能够以较低计算开销实现端到端稀疏加速，ESACT通过创新的稀疏性预测机制和硬件架构设计，显著提升了Transformer模型的能效和计算效率。
...
TL;DR: 本文提出DVDF方法，通过同时考虑动态对齐和价值对齐，从源域数据中选择性地共享高质量样本，以提升跨域离线强化学习的性能。


<details>
  <summary>Details</summary>
Motivation: 现有跨域离线强化学习方法仅关注动态对齐，忽略了价值对齐的重要性。作者通过理论分析证明，动态对齐和价值对齐对于策略学习都至关重要，仅考虑动态对齐会导致性能下降。

Method: 提出DVDF（动态和价值对齐数据过滤）方法，从源域数据中选择既具有高动态对齐（与目标域动态一致）又具有高价值对齐（高质量、高价值）的样本进行共享。

Result: 在包括运动学和形态学变化在内的多种动态偏移设置下，DVDF在多个任务和数据集上均优于现有基线方法，在目标域仅含5000个转换的极低数据设置下也表现出色。

Conclusion: 动态对齐和价值对齐对于跨域离线强化学习都至关重要，DVDF方法通过同时考虑这两个因素，能够有效提升跨域策略学习的性能。
...
TL;DR: 本文提出了一种名为TabGRU的新型混合深度学习架构，结合Transformer和双向门控循环单元，用于从商用微波链路数据中监测城市降雨，在瑞典哥德堡的公开数据集上表现出优于传统物理模型和深度学习基准的性能。


<details>
  <summary>Details</summary>
Motivation: 面对全球快速城市化和极端天气事件频发的挑战，高分辨率城市降雨监测对于建设韧性智慧城市至关重要。商用微波链路作为新兴数据源具有巨大潜力，但传统基于物理模型的降雨反演方法在处理信号噪声和非线性衰减等现实复杂性方面存在困难。

Method: 提出TabGRU混合深度学习架构，结合Transformer和双向门控循环单元，协同捕捉CML信号数据中的长期依赖关系和局部序列特征。通过可学习的位置嵌入和注意力池化机制增强模型的动态特征提取和泛化能力。

Result: 在瑞典哥德堡的公开基准数据集上验证，使用来自两个雨量站（Torp和Barl）的12个子链路，测试期间覆盖约10个不同的降雨事件。TabGRU模型在Torp站点和Barl站点分别达到0.91和0.96的决定系数，优于深度学习基准，相比物理模型在峰值降雨事件中有效缓解了显著高估问题。

Conclusion: TabGRU模型能够有效克服传统方法的局限性，在测试条件下为基于CML的城市降雨监测提供了稳健且准确的解决方案，证实了其在城市降雨监测中的实用价值。
...
<details>
  <summary>Details</summary>
Motivation: 现有跨域离线强化学习方法主要关注训练时鲁棒性（处理训练数据中的动态变化），但忽视了实际部署时测试时对动态扰动的鲁棒性。研究发现，在目标域数据有限的情况下，跨域离线强化学习训练的策略在动态扰动下表现脆弱。

Method: 提出鲁棒跨域贝尔曼算子（RCB），增强测试时对动态扰动的鲁棒性，同时保持对分布外动态转移的保守性。引入动态值惩罚和Huber损失技术，形成DROCO算法。

Result: 在各种动态变化场景下的实验结果表明，DROCO优于现有基线方法，展现出对动态扰动增强的鲁棒性。
...
Method: 比较逻辑回归、随机森林、XGBoost等基线模型，提出混合方法：使用XGBoost特征编码器和轻量级多层感知机（MLP）头

Result: 混合模型在处理后的NHANES子集上获得了比基线模型更好的AUC和平衡准确率

Conclusion: 混合方法在糖尿病预测任务中表现优于传统基线模型，作者开源代码和可复现脚本以促进研究复现
...
### [137] [A Fully First-Order Layer for Differentiable Optimization](https://arxiv.org/abs/2512.02494)
*Zihao Zhao,Kai-Chia Mo,Shing-Hei Ho,Brandon Amos,Kai Wang*

Main category: cs.LG

TL;DR: 提出一种仅使用一阶信息计算可微分优化层梯度的新算法，避免Hessian计算，降低计算和内存开销


<details>
  <summary>Details</summary>
Motivation: 可微分优化层通过隐式微分计算梯度需要求解包含Hessian项的线性系统，计算和内存开销大，需要更高效的梯度计算方法

Method: 将可微分优化重写为双层优化问题，利用双层方法的最新进展，引入主动集拉格朗日超梯度计算器，避免Hessian评估

Result: 近似超梯度可在$\tilde{\oo}(1)$时间内仅使用一阶信息计算，约束双层优化的总体复杂度为$\tilde{\oo}(δ^{-1}ε^{-3})$，匹配非光滑非凸优化的最佳已知速率

Conclusion: 提出了一种高效的可微分优化层梯度计算方法，避免了Hessian计算，降低了计算和内存需求，并发布了开源Python库
...
### [138] [Do Large Language Models Walk Their Talk? Measuring the Gap Between Implicit Associations, Self-Report, and Behavioral Altruism](https://arxiv.org/abs/2512.01568)
*Sandro Andric*

Main category: cs.LG

TL;DR: 研究发现大型语言模型表现出利他倾向，但存在"美德信号差距"：模型系统性高估自己的利他行为，自我报告与实际行动不一致。


<details>
  <summary>Details</summary>
Motivation: 研究动机是探究大型语言模型是否表现出利他倾向，以及它们的隐性关联和自我报告是否能预测实际的利他行为，借鉴人类社会心理学方法评估AI的利他性。

Method: 采用多方法研究设计，测试24个前沿LLM：1) 隐性关联测试测量隐性利他偏见；2) 强制二元选择任务测量行为利他；3) 自我评估量表测量显性利他信念。

Result: 主要发现：1) 所有模型都表现出强烈的隐性亲利他偏见；2) 模型行为比随机更利他但差异大；3) 隐性关联不能预测行为；4) 模型系统性高估自身利他行为，存在"美德信号差距"。

Conclusion: 建议使用"校准差距"作为标准化对齐指标，仅有12.5%模型实现高亲社会行为和准确自我认知的理想组合，强调模型校准对行为一致性的重要性。
...
<details>
  <summary>Details</summary>
Motivation: 水质对农业食品行业至关重要，随着该领域数字化进程加快，自动水质评估变得日益重要。需要确保用水安全和符合法规要求。

Method: 整合紫外-可见光谱技术与机器学习方法，并采用SHAP可解释性技术来分析不同波长吸光度对预测结果的贡献。

Result: 该方法展示了快速、准确且可解释的关键水质参数评估潜力，能够有效支持水质安全监测和法规合规性检查。

Conclusion: 紫外-可见光谱与机器学习结合，配合SHAP可解释性分析，为农业食品行业提供了一种高效、可靠的水质评估解决方案。
...
TL;DR: 该论文提出了一种基于去中心化多任务公平联邦学习（DMTFL）的VR视频缓存方案，通过在基站个性化缓存用户视场内容，解决无线VR传输中的延迟和异构性问题。


<details>
  <summary>Details</summary>
Motivation: 无线VR体验需要高质量、低延迟的视频传输，但面临严格的体验质量要求、低延迟约束和VR设备能力有限等挑战。传统联邦学习存在用户偏见问题，且单一全局模型无法捕捉用户和基站间的统计异质性。

Method: 提出DMTFL算法，在每个基站学习个性化缓存模型，通过多任务学习适应不同用户需求，并利用Rademacher复杂度和PAC边界提供理论保证，实现基于用户视场的智能缓存和预取。

Result: 使用真实的VR头部追踪数据集进行仿真，结果表明提出的DMTFL算法在性能上优于基线算法，能够有效提升VR视频传输质量。

Conclusion: DMTFL算法通过去中心化多任务联邦学习实现了VR视频的个性化缓存，解决了无线VR传输中的异构性和公平性问题，为高质量无线VR体验提供了有效解决方案。
...
TL;DR: CUDA-L2结合大语言模型和强化学习自动优化半精度矩阵乘法CUDA内核，在1000种配置下显著超越现有基准库


<details>
  <summary>Details</summary>
Motivation: 尽管HGEMM内核已经过高度优化，但通过LLM引导的强化学习自动化系统探索人类难以处理的配置空间，仍能进一步提升性能

Method: 使用CUDA执行速度作为强化学习奖励，结合大语言模型自动优化半精度矩阵乘法CUDA内核，在1000种配置上进行系统探索

Result: 在离线模式下平均比torch.matmul快22.0%，比cuBLAS快19.2%，比cuBLASLt-heuristic快16.8%，比cuBLASLt-AutoTuning快11.4%；在服务器模式下性能提升进一步增加到28.7%、26.0%、22.4%和15.9%

Conclusion: 即使是最关键、高度优化的HGEMM内核也能通过LLM引导的强化学习自动化得到改进，展示了在人类难以处理的规模上系统探索配置空间的价值
...
TL;DR: GoRL框架通过将优化与生成解耦，使用可处理的潜变量策略和条件生成解码器，在保持稳定优化的同时实现高度表达的多模态动作分布，在连续控制任务中显著优于高斯策略和现有生成策略基线。


<details>
  <summary>Details</summary>
Motivation: 强化学习中存在一个持久矛盾：易于优化的策略通常过于简单，无法表示复杂控制所需的多模态动作分布。高斯策略提供可处理的似然和平滑梯度，但单模态形式限制了表达能力；而基于扩散或流匹配的生成策略能建模丰富的多模态行为，但在在线RL中由于不可处理的似然和深度采样链中的噪声梯度传播而不稳定。

Method: 提出GoRL（生成在线强化学习）框架，基于"将优化与生成解耦"的结构原则。框架优化一个可处理的潜变量策略，同时使用条件生成解码器合成动作。采用双时间尺度更新计划，使潜变量策略能够稳定学习，而解码器则稳步提高表达能力，无需可处理的动作似然。

Result: 在一系列连续控制任务中，GoRL始终优于高斯策略和最近的生成策略基线。特别是在HopperStand任务中，达到归一化回报870以上，是最强基线的3倍多。

Conclusion: 将优化与生成分离为同时实现稳定性和高度表达性的策略提供了一条实用路径，解决了强化学习中稳定优化与丰富表达能力之间的长期矛盾。
...
### [143] [Joint Distillation for Fast Likelihood Evaluation and Sampling in Flow-based Models](https://arxiv.org/abs/2512.02636)
...
<details>
  <summary>Details</summary>
Motivation: 当前最好的生成模型（如扩散模型和基于流的模型）需要数百到数千次神经函数评估来计算单个似然值，这构成了计算瓶颈。虽然现有的蒸馏方法可以加速采样，但要么完全放弃似然计算，要么仍需要昂贵的全轨迹积分。

Method: 提出快速流联合蒸馏（F2D2）框架，利用连续归一化流中采样和似然的耦合ODE共享同一速度场的特性，使用单一模型联合蒸馏采样轨迹和累积散度，仅需添加一个散度预测头。

Result: F2D2能够在少量步骤评估下实现准确的对数似然，同时保持高样本质量。应用该方法提出的轻量级自引导方法，仅需2步的MeanFlow模型就能超越1024步教师模型，仅需额外一次反向神经函数评估。

Conclusion: F2D2解决了基于流生成模型中长期存在的计算瓶颈问题，实现了采样和似然评估效率的显著提升，为生成模型的实际应用提供了重要支持。
...
### [144] [Adaptive Weighted LSSVM for Multi-View Classification](https://arxiv.org/abs/2512.02653)
...
<details>
  <summary>Details</summary>
Motivation: 现有基于核的多视图学习方法通常使用融合技术而不强制视图间的显式协作类型，或使用协同正则化限制了全局协作，需要一种能促进视图间互补学习的方法。

Method: 提出自适应加权LS-SVM（AW-LSSVM），通过迭代全局耦合机制，使每个视图专注于其他视图在前一轮迭代中的困难样本，从而促进互补学习。
...
### [145] [Distill, Forget, Repeat: A Framework for Continual Unlearning in Text-to-Image Diffusion Models](https://arxiv.org/abs/2512.02657)
*Naveen George,Naoki Murata,Yuhta Takida,Konda Reddy Mopuri,Yuki Mitsufuji*

Main category: cs.LG

TL;DR: 该论文提出了一种基于生成蒸馏的持续遗忘框架，用于解决视觉生成模型在连续删除请求下的稳定遗忘问题，避免传统一次性遗忘方法在持续设置中引发的性能退化危机。


<details>
  <summary>Details</summary>
Motivation: 视觉生成模型的大规模训练与数据隐私法规（如GDPR的"被遗忘权"）存在冲突，需要机器遗忘技术。现有遗忘方法无法处理现实世界中连续到达的删除请求（持续遗忘），一次性方法在持续设置中会导致稳定性危机，包括保留概念崩溃、相关概念累积损害和生成质量急剧下降。

Method: 提出基于生成蒸馏的持续遗忘框架，将每个遗忘步骤重新定义为多目标、师生蒸馏过程，利用持续学习原理保持模型完整性。通过教师-学生蒸馏机制实现目标遗忘，同时维持保留概念的性能和整体图像质量。

Result: 在10步序列基准测试中，该方法能够以更好的保真度遗忘目标概念，且对保留概念性能或整体图像质量没有显著干扰，大幅优于基线方法。
...
### [146] [Graph VQ-Transformer (GVT): Fast and Accurate Molecular Generation via High-Fidelity Discrete Latents](https://arxiv.org/abs/2512.02667)
*Haozhuo Zheng,Cheng Wang,Yang Liu*

Main category: cs.LG

TL;DR: GVT是一个两阶段分子生成框架，通过图向量量化VAE将分子图压缩为离散潜在序列，再用自回归Transformer生成，实现了高效高精度的分子生成，性能优于扩散模型。
...
Method: 提出两阶段框架：1) 图向量量化VAE将分子图压缩为高保真离散潜在序列，结合图Transformer、RCM节点排序和RoPE位置编码；2) 在离散潜在序列上训练自回归Transformer进行生成。

Result: 在ZINC250k、MOSES、GuacaMol等基准测试中达到SOTA或极具竞争力的性能，在FCD和KL散度等关键分布相似性指标上显著优于领先的扩散模型。

Conclusion: GVT不仅为扩散模型提供了有吸引力的替代方案，还为离散潜在空间分子生成领域建立了新的强基线，为未来研究铺平了道路。
...
### [147] [Conformal Correction for Efficiency May be at Odds with Entropy](https://arxiv.org/abs/2512.02704)
*Senrong Xu,Tianyu Wang,Zenan Li,Yuan Yao,Taolue Chen,Feng Xu,Xiaoxing Ma*

Main category: cs.LG

TL;DR: 本文提出了一种熵约束的保形校正方法，在保形预测效率与模型预测熵之间寻找更好的帕累托最优解，显著提升了现有方法的效率


<details>
  <summary>Details</summary>
Motivation: 保形预测（CP）为黑盒机器学习模型提供统计上严格的置信集，但现有方法在效率方面仍有改进空间。作者通过实证和理论分析发现CP效率与模型预测熵之间存在权衡关系，需要探索更好的帕累托最优解

Method: 提出熵约束的保形校正方法，在保形感知的低效率损失基础上，通过引入熵约束来优化保形校正过程，在保证熵阈值的前提下提升CP效率

Result: 在计算机视觉和图数据集上的大量实验结果表明，该方法显著提升了最先进CP方法的效率，在给定熵阈值的情况下效率提升高达34.4%

Conclusion: 熵约束的保形校正方法有效解决了CP效率与预测熵之间的权衡问题，为保形预测提供了更优的解决方案
...
### [148] [FGC-Comp: Adaptive Neighbor-Grouped Attribute Completion for Graph-based Anomaly Detection](https://arxiv.org/abs/2512.02705)
*Junpeng Wu,Pinheng Zong*

Main category: cs.LG

TL;DR: 提出FGC-Comp轻量级属性补全模块，解决图异常检测中节点属性缺失和对抗性遮挡问题，提升邻域聚合稳定性


<details>
  <summary>Details</summary>
Motivation: 现有图异常检测模型大多忽视节点属性缺失和对抗性遮挡问题，这会破坏邻域聚合的稳定性并降低预测可靠性

Method: 提出FGC-Comp模块：将节点邻居按标签分为三组，对标记组应用组特定变换，对未知组使用节点条件门控，通过残差连接融合信息，端到端训练二元分类目标

Result: 在两个真实欺诈数据集上的实验验证了方法的有效性，计算开销可忽略不计

Conclusion: FGC-Comp是一个轻量级、分类器无关且易于部署的属性补全模块，能在属性不完整情况下增强邻域聚合的稳定性和预测可靠性
...
<details>
  <summary>Details</summary>
Motivation: 现有理论表明，在包含编码器、解码器和对抗干扰器的极小极大博弈中，鞍点由干扰器输出的对角高斯噪声构成。受此启发，作者希望利用干扰作为辅助目标，促使聚合潜在后验匹配对角高斯分布，实现更好的分布匹配。

Method: 提出一种新颖的分布匹配方法：使用对抗性无线干扰来正则化自编码器的潜在空间。通过构建编码器、解码器和对抗干扰器之间的极小极大博弈，其中干扰器试图破坏高斯源在信道中的恢复，利用干扰作为辅助目标来鼓励潜在后验匹配对角高斯分布。

Result: 该方法实现了与标准变分自编码器（VAE）和Wasserstein自编码器相当的分布匹配效果。同时，该方法可以推广到其他潜在分布，具有较好的通用性。

Conclusion: 对抗性无线干扰可以作为有效的正则化工具，用于自编码器潜在空间的分布匹配。该方法不仅实现了与现有技术相当的匹配效果，还具备扩展到其他潜在分布的潜力，为分布匹配提供了新的技术途径。
...
TL;DR: 该论文分析了基于流的扩散模型的记忆化-泛化行为，揭示了其训练目标天然包含两个阶段：早期导航阶段泛化数据模式形成全局布局，后期精炼阶段记忆细粒度细节。


<details>
  <summary>Details</summary>
Motivation: 基于流的扩散模型已成为图像和视频生成的主要范式，但其记忆化-泛化行为仍未被充分理解。作者旨在深入理解这类模型的训练动态，为未来的架构和算法改进提供指导原则。

Method: 作者重新审视流匹配（FM）目标，研究其边际速度场，该速度场具有闭式表达式，允许精确计算oracle FM目标。通过分析这个oracle速度场，揭示了基于流的扩散模型天然形成两阶段训练目标：早期由数据模式混合引导，后期由最近数据样本主导。

Result: 分析表明，两阶段目标导致不同的学习行为：早期导航阶段在数据模式间泛化形成全局布局，后期精炼阶段逐渐记忆细粒度细节。这些见解解释了实际技术（如时间步偏移调度、分类器自由引导区间和潜在空间设计选择）的有效性。

Conclusion: 该研究深化了对扩散模型训练动态的理解，为引导未来架构和算法改进提供了原则性指导。理解这种两阶段学习机制有助于设计更有效的训练策略和模型架构。
...
TL;DR: 该论文研究了时间序列基础模型（TSFMs）的输入归一化方法，发现REVIN是最有效的归一化方法，在零样本预测中显著提升性能，同时保持最佳领域内精度。


<details>
  <summary>Details</summary>
Motivation: 时间序列数据与文本或图像不同，在不同领域和通道间存在显著的尺度变化和非平稳性，这些问题会削弱时间序列基础模型的性能，而现有的归一化方法在需要泛化能力的TSFMs中尚未得到充分研究。

Method: 通过系统评估四种架构不同的时间序列基础模型，比较了多种输入归一化方法，重点关注REVIN方法在不同模型类型（概率预测、点预测、基于LLM的模型）中的效果。

Result: REVIN是最有效的归一化方法，相对于未归一化基线将零样本MASE降低了89%，相比其他归一化方法降低了44%，同时匹配最佳领域内精度（0.84 MASE），无需任何数据集级预处理，实现了最高的精度-效率权衡。

Conclusion: REVIN是时间序列基础模型中最有效的输入归一化方法，但其效果利用取决于架构设计选择和优化目标，特别是训练损失尺度敏感性和模型类型。
...
<details>
  <summary>Details</summary>
Motivation: 去中心化联邦学习(DFL)中异常客户端（由噪声或中毒数据引起）会显著破坏学习过程并降低模型鲁棒性。现有方法通常需要足够多的正常邻域客户端或可靠客户端的先验知识，这限制了DFL的实际应用。

Method: 提出自适应DFL(aDFL)方法，核心思想是自适应调整客户端的学习率：对可疑客户端分配较小的学习率，对正常客户端分配较大的学习率，以完全自适应方式减轻异常客户端对全局模型的负面影响。

Result: 理论分析不要求对邻域节点施加严格条件且无需先验知识，提供了严格的收敛分析以保证aDFL的oracle性质。大量数值实验证明了aDFL方法的优越性能。

Conclusion: aDFL方法通过自适应学习率调整有效缓解了异常客户端对去中心化联邦学习的负面影响，具有更好的实用性和鲁棒性。
...
TL;DR: OptPO通过贝叶斯顺序概率比检验自适应分配推理预算，减少测试时策略优化的计算冗余，同时保持或提升准确性。


<details>
  <summary>Details</summary>
Motivation: 现有测试时策略优化方法依赖固定预算多数投票来估计奖励，导致大量计算冗余，需要更高效的预算分配方法。

Method: 提出OptPO框架，将投票过程建模为贝叶斯顺序概率比检验，动态停止采样，并利用保留的rollout进行策略更新，可与PPO或GRPO等算法无缝集成。

Result: 在多样化推理基准测试中，OptPO相比固定样本基线显著减少rollout开销，同时保持或提高准确性。

Conclusion: OptPO通过统一统计最优停止与测试时学习，为测试时适应提供了计算高效的范式。
...
TL;DR: Fairy2i是一个通用框架，可将预训练的实值LLM转换为等效的复值形式，实现极低比特量化（2比特），同时重用现有模型权重，显著优于现有实值量化方法。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型的内存和计算需求巨大，需要激进量化，但现有复值模型需要从头训练，无法利用预训练的实值基础模型生态系统。需要一种方法既能利用复值表示的低比特优势，又能重用现有预训练模型。

Method: 1) 证明实值与广泛线性复值映射之间的无损数学等价性，将标准Transformer转换为复域；2) 采用相位感知量化方案，使用四阶单位根的高效码本；3) 引入递归残差量化机制，迭代最小化量化误差，实现无乘法累加推理。

Result: Fairy2i将LLaMA-2 7B模型在有效2比特精度下的性能恢复到接近全精度基线水平，显著优于最先进的实值二值和三值量化方法。
...
<details>
  <summary>Details</summary>
Motivation: 现有研究主要关注广义Thurstone模型的参数估计和不确定性量化，但缺乏对这些模型本身是否适合描述数据的根本性检验。本文旨在解决广义Thurstone模型的极小极大假设检验这一基本问题。

Method: 1. 引入一般成对比较模型与Thurstone模型类之间的分离距离概念；2. 推导依赖于观测图拓扑结构的检验临界阈值上下界；3. 提出基于分离距离的假设检验方法；4. 构建置信区间；5. 使用反向鞅技术建立时间一致性的I类和II类错误概率界限；6. 使用信息论方法推导极小极大下界。

Result: 1. 对于完全观测图的特殊情况，检验阈值以Θ((nk)^{-1/2})的比例缩放，其中n为智能体数量，k为每对比较次数；2. 建立了假设检验的统计理论框架；3. 在合成和真实数据集上验证了理论结果的有效性。

Conclusion: 本文为广义Thurstone模型提供了一个完整的假设检验框架，包括理论分析、统计方法和实验验证，填补了该领域在模型适合性检验方面的空白，为实际应用中的模型选择提供了理论基础。
...
TL;DR: 针对自相关数据，提出了基于分块处理的快速高斯过程回归方法，在保持预测性能的同时显著加速计算。


<details>
  <summary>Details</summary>
Motivation: 高斯过程模型在非线性回归中应用广泛，但传统快速近似方法假设数据独立同分布。对于自相关数据，忽略自相关性会导致时间过拟合问题，降低模型在新测试实例上的性能。现有快速高斯过程近似方法需要针对自相关数据进行修改。

Method: 将原始相关数据点分割成块，使块内数据去相关，然后使现有高斯过程近似方法能够处理分块数据。具体解释了如何让一些现有高斯过程近似方法与分块数据协同工作。

Result: 在多个应用数据集上的数值实验表明，所提出的方法能够在保持模型预测性能的同时，显著加速自相关数据的高斯过程回归计算。

Conclusion: 通过分块处理自相关数据，成功实现了高斯过程回归的加速计算，解决了时间过拟合问题，为自相关数据的快速高斯过程建模提供了有效解决方案。
