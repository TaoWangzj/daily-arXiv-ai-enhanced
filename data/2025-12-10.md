...
Motivation: 现有视频到音频生成模型主要基于外观驱动，捕捉视觉-声学相关性，但缺乏对塑造真实世界声音的物理因素的考虑。需要将物理推理融入音频生成过程。

Method: 1. 使用物理参数估计器（PPE）估计物体级物理参数：通过视觉语言模型推断移动物体质量，通过基于分割的动态3D重建模块恢复运动轨迹计算速度；2. 通过物理驱动音频适配器（Phy-Adapter）将这些物理线索融入潜在扩散模型；3. 创建VGG-Impact基准和音频-物理相关系数（APCC）评估指标。

Result: PAVAS能够生成物理上合理且感知上连贯的音频，在定量和定性评估中都优于现有的视频到音频生成模型。实验表明该方法在物理真实感方面表现优异。

Conclusion: 将物理推理融入视频到音频生成能够产生更符合物理现实的音频，PAVAS方法通过物理参数估计和适配器机制有效提升了音频生成的物理真实感。
...
TL;DR: 使用Planet Labs四波段卫星影像和深度学习模型，实现了对苏丹武装冲突中火灾损害近乎实时的监测，相比基线方法能更准确地捕捉活跃火灾和烧焦区域。
...
Conclusion: 四波段卫星影像结合深度学习模型能够以最小延迟有效监测武装冲突中的火灾损害，为冲突地区的快速监测提供了可行方案。
...
TL;DR: LivingSwap是首个视频参考引导的人脸交换模型，通过关键帧条件注入目标身份，结合视频参考指导实现时间一致性缝合，在长视频序列中保持稳定身份和高保真重建。
...
Method: 1. 提出LivingSwap模型，首个视频参考引导的人脸交换模型；2. 使用关键帧作为条件信号注入目标身份，实现灵活可控的编辑；3. 结合关键帧条件和视频参考指导，进行时间缝合以确保稳定身份保持和高保真重建；4. 构建配对人脸交换数据集Face2Face，并通过反转数据对确保可靠的真实监督。

Result: 广泛实验表明，该方法取得了最先进的结果，能够无缝地将目标身份与源视频的表情、光照和运动相结合，同时显著减少了制作工作流程中的人工工作量。

Conclusion: LivingSwap通过视频参考引导和关键帧条件注入，成功解决了长视频序列中人脸交换的高保真度和时间一致性问题，为影视制作提供了高效实用的解决方案。
...
TL;DR: FRIEDA是一个用于评估大型视觉语言模型在复杂开放式地图推理能力的新基准，测试拓扑、度量和方向三种空间关系，结果显示当前最先进模型表现远低于人类水平


<details>
  <summary>Details</summary>
Motivation: 地图推理是解释地理关系的关键认知能力，对灾害响应和城市规划等任务至关重要，但目前缺乏系统评估。现有地图视觉问答研究常将地图视为图表的特例，但地图VQA需要理解分层符号系统和跨多张地图的空间关系，现有评估方法无法捕捉这些复杂需求

Method: 构建FRIEDA基准，从各领域和地理区域的文档报告中收集真实地图图像。基于GIS文献分类，测试所有三类空间关系：拓扑关系（边界、相等、相交、包含）、度量关系（距离）和方向关系（方位）。所有问题都需要多步推理，许多需要跨地图定位和推理。在两种设置下评估11个最先进的LVLM：直接设置（提供相关地图）和上下文设置（模型需先识别相关地图）

Result: 评估11个最先进的LVLM，即使最强的模型Gemini-2.5-Pro和GPT-5-Think也仅达到38.20%和37.20%的准确率，远低于人类84.87%的表现。这表明在多步地图推理方面存在持续差距

Conclusion: FRIEDA作为一个严谨的基准，揭示了当前LVLM在空间智能方面的显著不足，为推动地图推理能力进步提供了重要评估工具。地图推理需要超越图表理解的复杂空间关系处理能力，现有模型在这方面仍有很大提升空间
...
TL;DR: 该论文提出了一种基于频域掩码的通用深度伪造检测方法，通过频率掩码策略提升对不同生成模型的泛化能力，同时保持计算效率，实现可持续的大规模深度伪造筛查。


<details>
  <summary>Details</summary>
Motivation: 通用深度伪造检测需要能够识别各种生成模型（包括未见过的模型）生成的AI图像，这要求检测器具有强大的泛化能力。同时，在绿色AI时代，需要最小化计算开销以实现大规模深度伪造筛查。现有方法过度依赖空间特征或大规模预训练模型，缺乏有效的泛化策略。

Method: 提出频率域掩码作为深度伪造检测器的训练策略。该方法引入随机掩码和几何变换，特别关注频率掩码，因为其具有优越的泛化特性。频率掩码不仅能增强对不同生成器的检测准确性，还能在显著模型剪枝下保持性能。

Result: 该方法在GAN和扩散模型生成的图像数据集上实现了最先进的泛化性能，并在结构化剪枝下表现出一致的鲁棒性。频率掩码策略显著提升了检测器的泛化能力和计算效率。

Conclusion: 频率掩码为可持续和可泛化的深度伪造检测提供了实用的解决方案，平衡了检测性能和计算资源消耗，是向绿色AI时代大规模深度伪造筛查迈出的重要一步。
...
TL;DR: M2A提出了一种简单的持续测试时间自适应方法，通过随机掩码生成多个视图，结合一致性损失和熵最小化损失进行适应，无需复杂的掩码设计或不确定性校准。


<details>
  <summary>Details</summary>
Motivation: 现有持续测试时间自适应方法依赖校准的不确定性或稳定的注意力分数，并引入额外复杂性。本文探讨是否真的需要定制化的掩码设计，或者简单的随机掩码在强干扰下就足够有效。

Method: 提出Mask to Adapt (M2A)方法：生成短序列的掩码视图（空间或频率），使用两种目标函数进行适应：1) 掩码一致性损失，对齐不同视图的预测；2) 熵最小化损失，鼓励置信输出。研究了两种掩码家族：空间掩码（补丁vs像素）和频率掩码（全频vs低频vs高频）。

Result: 在CIFAR10C/CIFAR100C/ImageNetC（严重程度5）上，M2A（空间）达到8.3%/19.8%/39.2%的平均错误率，优于或匹配强基线。M2A（频率）表现落后。消融实验显示简单随机掩码有效且鲁棒。

Conclusion: 简单的随机掩码调度结合一致性和熵目标，足以驱动有效的测试时间自适应，无需依赖不确定性或注意力信号。
...
TL;DR: CVP框架受人类中央-周边视觉启发，通过目标亲和性令牌和以自我为中心的网格组件，提升多模态模型在3D空间推理中的结构化理解能力
...
Method: 提出受中央-周边视觉启发的CVP框架，包含两个互补组件：1) 目标亲和性令牌（类似中央视觉），引导模型关注查询相关对象；2) 以自我为中心的网格（类似周边视觉），捕捉全局场景上下文和空间布局

Result: CVP在多个3D场景理解基准测试中实现了最先进的性能

Conclusion: CVP框架通过结合中央视觉和周边视觉的类比，实现了对复杂3D环境的结构化、上下文感知理解，显著提升了多模态模型的空间推理能力
...
Method: 基于多状态感知范式，提出Fourier-RWKV框架：1) 空间形式感知：通过可变形四向令牌移位动态调整感受野；2) 频域感知：在傅里叶混合块中将RWKV的WKV注意力机制扩展到傅里叶域；3) 语义关系感知：通过语义桥接模块实现动态语义核融合。

Result: 在多个基准测试中实现最先进性能，显著降低计算开销，在恢复质量和实际效率之间建立良好平衡。

Conclusion: Fourier-RWKV通过多状态感知范式有效解决了非均匀去雾问题，在保持线性复杂度的同时实现了优异的去雾性能。
...
<details>
  <summary>Details</summary>
Motivation: 高质量的几何图形生成既具有挑战性又有机遇：它要求严格的空间准确性，同时提供明确的约束来指导生成。受几何问题求解中形式化语言和符号求解器在增强正确性和可解释性方面的最新进展启发。

Method: GeoLoom包含两个核心组件：1) 自动形式化模块，将自然语言翻译为专门设计的生成导向形式语言GeoLinua；2) 坐标求解器，使用高效的蒙特卡洛优化将形式约束映射到精确坐标。还引入了GeoNF数据集，对齐自然语言几何描述和形式化GeoLinua描述。

Result: GeoLoom在结构保真度方面显著优于最先进的基线方法，为可解释和可扩展的图表生成提供了原则性基础。

Conclusion: GeoLoom通过形式化语言和约束求解的方法，为几何图形生成提供了一个可解释、可扩展且具有高结构保真度的框架。
...
TL;DR: 提出视觉知识库VisKnow框架，通过构建多模态对象知识图谱来增强对象理解能力，并以AnimalKB为案例验证其有效性


<details>
  <summary>Details</summary>
Motivation: 当前对象理解需要多模态数据支持，但现有数据通常面向特定任务且组织不够系统化，无法实现预期的对象类别深度理解

Method: 提出VisKnow框架，结合专家设计和大规模模型应用，从百科全书等文本源提取知识三元组，整合图像和区域标注，构建结构化多模态对象知识图谱

Result: 构建了AnimalKB知识库，涵盖406个动物类别，包含22K文本知识三元组、420K图像及对应区域标注，实验证明能增强零样本识别、细粒度VQA等任务

Conclusion: 自动构建视觉知识库有潜力推进视觉理解及其实际应用，为知识图谱补全和部件分割等任务提供具有挑战性的基准
...
<details>
  <summary>Details</summary>
Motivation: 随着大语言模型展现出强大的泛化能力，通过微调和提示调优等技术可以适应各种下游任务。本文旨在探索在3D目标检测领域中常见提示调优方法的有效性，研究基于大规模Waymo数据集训练的模型能否作为基础模型适应其他3D目标检测场景。

Method: 本文系统地研究了提示标记和提示生成器的影响，进一步提出了场景导向的提示池（SOP²）方法。该方法旨在通过提示池技术提升3D目标检测的性能和适应性。

Result: 研究证明了提示池在3D目标检测中的有效性，展示了基于大规模数据集训练的模型可以通过提示调优技术适应其他场景。

Conclusion: 本文展示了提示调优技术在3D目标检测领域的应用潜力，提出的场景导向提示池方法为未来研究者深入探索3D领域中提示技术的潜力提供了启发。
...
TL;DR: MM-CoT是一个诊断性基准测试，专门设计来评估多模态模型中思维链推理的视觉基础和逻辑连贯性，要求模型选择同时满足视觉一致性和逻辑连贯性的事件链，而非生成自由形式的解释。


<details>
  <summary>Details</summary>
Motivation: 现有基准测试强调生成但忽视验证，无法评估多模态模型的思维链推理是否真正基于视觉证据且逻辑连贯。需要填补这一空白，探究模型推理的视觉基础和逻辑有效性。

Method: 设计MM-CoT基准测试，要求模型从多个选项中选择唯一满足两个正交约束的事件链：(1)视觉一致性，确保所有步骤都基于可观察证据；(2)逻辑连贯性，确保因果和常识有效性。设计对抗性干扰项来暴露不同的推理失败模式。

Result: 评估领先的视觉语言模型发现，即使最先进的系统也在MM-CoT上表现不佳，揭示了生成流畅性与真实推理保真度之间的显著差距。MM-CoT与现有基准测试相关性低，确认其测量的是视觉基础和逻辑推理的独特组合。

Conclusion: MM-CoT基准测试为开发未来模型奠定了基础，这些模型不仅能够进行看似合理的推理，而且能够在视觉世界中忠实且连贯地推理。它揭示了当前多模态模型在视觉基础和逻辑连贯性方面的局限性。
...
TL;DR: 本文提出了一种基于法线引导的稀疏深度采样策略，通过PCA方法从RGB-D点云估计表面法线来计算每个像素的深度可靠性，从而生成更符合真实传感器行为的稀疏深度样本，提高了深度补全的准确性。


<details>
  <summary>Details</summary>
Motivation: 当前深度补全方法中稀疏深度的生成方式不现实，通常从密集真实深度中均匀随机选择像素，忽略了真实传感器具有几何依赖性和空间非均匀可靠性的特点，这限制了深度补全模型的性能和实际应用效果。

Method: 提出了一种法线引导的稀疏深度采样策略：1）使用PCA方法在RGB-D点云上进行表面法线估计；2）基于法线信息计算每个像素的深度可靠性度量；3）根据可靠性分布抽取稀疏深度样本；4）将该采样方法与Marigold-DC扩散式深度补全模型集成。

Result: 在NYU Depth v2数据集上的实验表明，该方法提高了深度补全的准确性，减少了边缘和不连续区域的人工伪影，并创建了更符合真实传感器行为的训练条件，使用标准评估指标验证了改进效果。

Conclusion: 几何感知的稀疏深度采样策略能够生成更真实的训练数据，显著提升深度补全模型的性能，更好地反映真实传感器的行为特性，为工业机器人系统的三维感知提供了更可靠的解决方案。
...
TL;DR: FastBEV++是一个相机BEV感知框架，通过算法优化和部署设计，实现了高性能与部署效率的统一，在nuScenes基准上达到0.359 NDS，在Tesla T4上超过134 FPS。


<details>
  <summary>Details</summary>
Motivation: 当前相机BEV感知面临性能与部署可行性的矛盾：SOTA性能依赖计算密集的视角变换和平台特定的CUDA内核，阻碍车载部署。需要解决这一瓶颈。

Method: 采用"算法快速"和"设计可部署"两大原则：1) 通过确定性预排序策略，将视角变换分解为标准索引-收集-重塑管道，使用基础算子实现，无需定制CUDA内核；2) 集成端到端深度感知融合机制，结合时间聚合和数据增强提升几何保真度。

Result: 在nuScenes基准上达到0.359 NDS的新SOTA，同时在Tesla T4等车载硬件上实现超过134 FPS的实时性能，无需定制插件。
...
<details>
  <summary>Details</summary>
Motivation: 传统视觉语言模型将大量视觉补丁标记输入LLMs会产生二次计算成本，而现有方法面临连续压缩会稀释高级语义、离散量化会丢失细粒度细节的权衡困境。

Method: 采用混合双通道框架：连续通道通过ViT补丁保留细粒度细节，离散通道使用MGVQ量化生成符号锚点（压缩为4个标记），通过解缠注意力掩码和瓶颈将580个混合序列标记压缩为单个voco标记。

Result: 在7个基准测试（GQA、VQAv2、MMBench、MME、POPE、SEED-Bench、ScienceQA-Image）上平均性能保持87.2%，优于领先连续基线的81.0%，实现580:1压缩比。注意力分析显示压缩标记优先考虑离散锚点，验证了其语义引导作用。

Conclusion: 极简混合设计能够解决效率与保真度之间的困境，推动可扩展视觉语言模型的发展，证明了通过分离语义和外观信息可以实现高效且可靠的表示。
...
TL;DR: 提出了一种新颖的深度混合Residual-SwinCA-Net分割框架，结合残差CNN模块和Swin Transformer块，通过多种增强技术实现乳腺癌超声图像的精确分割，在BUSI数据集上达到99.29%的平均准确率。


<details>
  <summary>Details</summary>
Motivation: 为了解决乳腺癌超声图像分割中局部特征提取、全局依赖学习、组织连续性增强、噪声抑制和病变轮廓保持等挑战，需要开发一个更有效的分割框架来提升临床诊断性能。

Method: 提出Residual-SwinCA-Net混合框架：1）使用残差CNN模块提取局部相关特征；2）定制Swin Transformer块学习全局依赖；3）应用拉普拉斯-高斯区域算子增强组织连续性和抑制噪声；4）引入边界导向算子保持恶性病变轮廓完整性；5）采用阶段式收缩策略捕获尺度不变性；6）在解码器层集成多尺度通道注意力和压缩模块；7）使用像素注意力模块自适应加权病变像素。

Result: 在公开的BUSI数据集上，Residual-SwinCA-Net框架实现了99.29%的平均准确率、98.74%的交并比和0.9041的Dice系数，显著优于现有的CNN和ViT技术。

Conclusion: 提出的Residual-SwinCA-Net框架通过有效结合局部和全局特征学习、多尺度处理和注意力机制，显著提升了乳腺癌超声图像的分割性能，有助于改善临床诊断决策的及时性和准确性。
...
<details>
  <summary>Details</summary>
Motivation: 现有的知识蒸馏方法主要关注空间特征蒸馏或时间关系蒸馏，但忽略了未来帧的知识转移，导致在线模型难以有效学习未来知识。

Method: 提出FTKD方法：1）未来感知特征重建策略，让学生模型无需严格帧对齐即可捕获未来特征；2）未来引导的logit蒸馏，利用教师模型稳定的前景和背景上下文信息。

Result: 在nuScenes数据集上应用于两个高性能3D目标检测基线模型，获得最高1.3 mAP和1.3 NDS提升，同时实现最准确的运动速度估计，且不增加推理成本。

Conclusion: FTKD通过有效转移未来帧知识，显著提升了在线3D目标检测模型的性能，为自动驾驶中的实时检测提供了有效的知识蒸馏解决方案。
...
Method: 提出查询感知枢纽原型（QHP）学习方法：1）枢纽原型生成（HPG）模块构建查询点与支持点的二分图，识别频繁连接的枢纽点，生成查询相关的原型以捕捉跨集语义；2）原型分布优化（PDO）模块使用纯度加权对比损失，将坏枢纽和边界模糊原型拉近对应类中心，优化原型表示。

Result: 在S3DIS和ScanNet数据集上的大量实验表明，QHP相比最先进方法实现了显著的性能提升，有效缩小了原型与查询集之间的语义差距。

Conclusion: QHP方法通过显式建模支持集与查询集之间的语义相关性，生成查询感知的原型表示，有效解决了少样本3D点云分割中的原型偏差问题，提升了分割性能。
...
TL;DR: RLCNet：用于LiDAR、RADAR和相机多模态传感器在线校准的端到端深度学习框架，通过加权移动平均和异常值剔除实现实时动态调整，在真实数据集上验证了优越性能。
...
Result: 在真实世界数据集上验证，RLCNet设计用于实际部署，在不同条件下表现出鲁棒性能。消融研究强调了架构选择的重要性，与现有方法的比较展示了所提方法在准确性和鲁棒性方面的优越性。

Conclusion: RLCNet为自动驾驶车辆的多模态传感器在线校准提供了一种有效的深度学习解决方案，通过实时动态调整机制提高了校准的准确性和鲁棒性，适用于实际部署环境。
...
TL;DR: EgoX框架通过轻量级LoRA适配和统一条件策略，将第三人称视频转换为第一人称视角视频，解决视角差异大和重叠区域少的挑战。


<details>
  <summary>Details</summary>
Motivation: 将第三人称（外中心）视频转换为第一人称（自我中心）视频能够为沉浸式理解开辟新可能，但由于相机姿态变化极大且视角重叠区域极少，这一任务极具挑战性。

Method: EgoX框架利用大规模视频扩散模型的预训练时空知识，通过轻量级LoRA适配；采用统一条件策略结合外中心和自我中心先验；引入几何引导的自注意力机制选择性关注空间相关区域。
...
TL;DR: OpenSubject是一个包含250万样本、435万图像的大规模视频衍生数据集，用于提升主题驱动的图像生成和编辑性能，特别是在多主题复杂场景中。
...
Method: 采用四阶段流水线构建数据集：1）视频筛选（分辨率和美学过滤）；2）跨帧主题挖掘与配对（基于VLM的类别共识、局部定位和多样性感知配对）；3）身份保持的参考图像合成（分割图引导的外绘和框引导的内绘，结合几何感知增强和边界侵蚀）；4）验证与标注（VLM验证样本，失败样本重新合成，构建短长描述）。

Result: 构建了包含2.5M样本、4.35M图像的大规模数据集，并建立了覆盖主题驱动生成和编辑的基准测试。实验表明，使用OpenSubject训练能显著提升生成和编辑性能，特别是在复杂场景中。

Conclusion: OpenSubject数据集通过创新的四阶段构建流程，有效解决了主题驱动图像生成中的身份偏离和复杂场景处理问题，为相关研究提供了高质量的训练资源和评估基准。
...
TL;DR: Terrain Diffusion：基于扩散模型的程序化地形生成新方法，结合了AI模型的真实感和传统噪声函数的无限性、一致性、实时访问等特性


<details>
  <summary>Details</summary>
Motivation: 传统程序化噪声函数（如Perlin噪声）虽然快速、无限，但在真实感和大尺度连贯性方面存在根本限制。需要一种AI时代的新方法，既能保持程序化噪声的优点，又能提供更高的真实感

Method: 1. InfiniteDiffusion算法实现无限生成；2. 分层扩散模型堆栈耦合行星上下文与局部细节；3. 紧凑拉普拉斯编码稳定地球尺度动态范围输出；4. 开源无限张量框架支持无界张量操作；5. 少步一致性蒸馏实现高效生成

Result: 建立扩散模型作为程序化世界生成的实用基础，能够无限制、连贯、可控地合成整个行星地形

Conclusion: Terrain Diffusion成功将扩散模型的高保真度与传统程序化噪声的关键特性相结合，为AI时代的程序化世界生成提供了新的技术基础
...
<details>
  <summary>Details</summary>
Motivation: 现有基于分布匹配的数据集蒸馏方法局限于欧几里得空间，只能捕获线性结构而忽略了真实数据的内在几何特性（如曲率）。高维数据通常位于低维流形上，因此数据集蒸馏应该使蒸馏数据流形与原始数据流形对齐。

Method: 提出GeoDM框架，在欧几里得、双曲和球面流形的笛卡尔积空间中操作，引入可学习的曲率和权重参数来适应底层数据几何，同时设计最优传输损失来增强分布保真度。

Result: 理论分析表明，在乘积空间中进行几何感知分布匹配比欧几里得对应方法具有更小的泛化误差界。在标准基准测试上的广泛实验证明，该算法优于最先进的数据蒸馏方法，并且在各种单几何分布匹配策略中保持有效性。

Conclusion: GeoDM通过在多几何空间中操作，能够更好地捕获数据的固有几何结构，为数据集蒸馏提供了一种更有效的几何感知方法，在保持性能的同时显著减少了所需数据量。
...
<details>
  <summary>Details</summary>
Motivation: 牙齿标志点检测在现代临床正畸中至关重要，但牙齿几何结构复杂且个体差异大，需要先进技术来实现精确可靠的3D牙齿标志点检测。

Method: 通过举办3DTeethLand挑战赛，与国际医学图像计算和计算机辅助干预会议合作，征集专注于从口内3D扫描中检测牙齿标志点的算法。

Result: 挑战赛推出了首个公开可用的3D牙齿标志点检测数据集，为评估该任务的最先进方法提供了宝贵资源。

Conclusion: 该挑战赛旨在鼓励研究社区提供方法学贡献，解决具有重要临床意义的牙齿标志点检测问题。
...
Conclusion: GeoDiffMM通过扩散模型结合几何先验，实现了结构一致的运动放大，有效解决了小位移下光子噪声与真实微运动的分离问题。
...
TL;DR: 提出了一种基于四元数的低秩支持四元数矩阵机（LSQMM）用于彩色图像分类，将RGB通道作为纯四元数处理，通过四元数代数保持通道间的内在耦合关系，并引入四元数核范数正则化来促进低秩结构。
...
Method: 提出低秩支持四元数矩阵机（LSQMM），将RGB通道作为纯四元数处理，在铰链损失中添加四元数核范数正则化项以促进低秩结构（源于强相关的颜色通道），并设计基于ADMM的迭代算法来求解四元数优化模型。

Result: 在多个彩色图像分类数据集上的实验结果表明，与支持向量机、支持矩阵机和支持张量机等最先进方法相比，所提出的分类方法在分类精度、鲁棒性和计算效率方面具有优势。
...
<details>
  <summary>Details</summary>
Motivation: 虽然Glaze和Nightshade等图像保护机制在经验上有效，但其内部结构、可检测性和表征行为仍缺乏深入理解。研究旨在通过可解释AI方法系统分析这些保护机制的工作原理。

Method: 采用统一框架整合白盒特征空间检查和黑盒信号级探测，包括潜在空间聚类、特征通道激活分析、基于遮挡的空间敏感性映射和频域表征分析。

Result: 保护机制作为结构化、低熵的扰动运行，紧密耦合于底层图像内容；保护图像保留了内容驱动的特征组织，具有保护特定的子结构而非全局表征漂移；可检测性受扰动熵、空间部署和频率对齐的交互效应控制；Glaze和Nightshade沿主导图像对齐频率轴重新分配能量而非引入扩散噪声。

Conclusion: 当代图像保护通过结构化特征级变形而非语义错位操作，解释了保护信号视觉微妙但一致可检测的原因。该工作推进了对抗性图像保护的可解释性，为未来生成AI系统的防御和检测策略设计提供信息。
...
TL;DR: PointDico是一种新颖的3D表示学习方法，通过融合扩散模型和对比学习的优势，解决了现有方法在无序点云数据上的局限性，在多个3D任务上达到了最先进的性能。


<details>
  <summary>Details</summary>
Motivation: 现有的自监督表示学习方法在3D数据上面临困难，因为3D数据具有无序性和密度不均匀的特点。对比模型容易过拟合，而3D掩码自编码器难以处理无序点云。这促使研究者探索结合扩散模型和对比学习优势的新方法。
...
Conclusion: PointDico成功融合了扩散模型和对比学习的优势，有效解决了3D表示学习的挑战，为无序点云数据的自监督学习提供了新的解决方案。
...
TL;DR: 提出Bi^2MAC方法，通过双模态双自适应掩码感知卷积，有效利用不同区域信息并智能分配计算资源，在遥感图像融合中实现SOTA性能同时降低计算成本


<details>
  <summary>Details</summary>
Motivation: 传统深度学习方法在特征表示中难以适应区域异质性，现有自适应卷积方法存在计算成本过高和捕捉遥感图像异质区域能力有限的问题
...
Conclusion: Bi^2MAC方法通过智能分配计算资源，有效利用不同区域信息，在保持高性能的同时大幅降低计算成本，为遥感图像融合提供了一种高效解决方案
...
Method: 提出混合溅射机制，包括反射烘焙高斯追踪、基于图块的高斯溅射渲染反射、统一混合溅射框架、流水线级加速和反射敏感高斯剪枝

Result: 在Ref-NeRF和NeRF-Casting复杂反射场景上实现约7倍渲染加速，高斯基元数量比类似基于光线追踪的高斯溅射基线减少4倍

Conclusion: HybridSplat成为复杂反射场景的新SOTA方法，在保持反射渲染质量的同时显著提升渲染效率和减少内存占用
...
TL;DR: DINO-BOLDNet：首个从T1w图像直接生成平均BOLD图像的框架，利用自监督DINOv3编码器和轻量级解码器，在临床数据集上超越条件GAN基线
...
Conclusion: DINO-BOLDNet展示了自监督transformer引导在结构到功能映射中的潜力，为BOLD图像恢复提供了有效解决方案，特别是在BOLD数据不可用或损坏的情况下。
...
TL;DR: 提出TrackingWorld系统，在单目视频中实现世界坐标系下的密集3D跟踪，解决现有方法无法分离相机运动与前景动态运动、不能跟踪新出现动态目标的问题。


<details>
  <summary>Details</summary>
Motivation: 现有单目3D跟踪方法存在两个主要局限：1) 无法有效分离相机运动与前景动态运动；2) 不能密集跟踪视频中新出现的动态目标。需要一种能在世界坐标系下跟踪几乎所有像素的解决方案。

Method: 提出三阶段方法：1) 跟踪上采样器将任意稀疏2D跟踪提升为密集2D跟踪；2) 将上采样器应用于所有帧，通过消除重叠区域跟踪来减少冗余，以处理新出现目标；3) 基于优化的框架，通过估计相机位姿和2D跟踪的3D坐标，将密集2D跟踪反投影到世界中心3D轨迹。

Result: 在合成和真实世界数据集上的广泛评估表明，该系统在世界坐标系下实现了准确且密集的3D跟踪。

Conclusion: TrackingWorld系统成功解决了现有单目3D跟踪方法的两个关键限制，实现了世界坐标系下几乎所有像素的密集3D跟踪，为单目视频的长期运动捕捉提供了有效解决方案。
...
Result: SCU-CGAN在KID分数上比CycleGAN提升41.5%，生成的火灾图像质量更优。增强后的数据集显著提高了火灾检测模型精度，YOLOv5 nano模型的mAP@0.5:0.95指标提升了56.5%
...
TL;DR: 该论文发现多模态大语言模型中视觉和文本token的norm差异导致不对称更新动态，提出在视觉投影器后添加LayerNorm层进行norm对齐，显著提升模型性能。


<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型普遍采用Pre-Norm架构，导致视觉token和文本token之间存在严重的norm差异。这种不平衡不是静态问题，而是会引发"不对称更新动态"，即高norm的视觉token具有"表征惯性"，语义转换速度远慢于文本token，从而损害跨模态特征融合效果。

Method: 首先通过理论分析证明norm差异会导致不对称更新动态。然后提出一个简单有效的解决方案：在视觉投影器后插入一个精心初始化的LayerNorm层，强制实现norm对齐。该方法在LLaVA-1.5架构上进行实验验证。

Result: 实验验证了理论动态（norm差异持续存在和由此产生的不对称更新率）是普遍现象。提出的LayerNorm干预方法在广泛的多模态基准测试中带来显著性能提升，值得注意的是，在纯文本评估如MMLU上也表现出改进，表明解决架构不平衡能产生更全面的模型能力。

Conclusion: 多模态大语言模型中的norm差异是一个关键但被忽视的问题，会导致不对称更新动态并损害跨模态融合。通过简单的LayerNorm层实现norm对齐可以有效解决这一问题，显著提升模型在多模态和纯文本任务上的性能。
...
TL;DR: 提出一种在复杂光照条件下同时进行图像增强和噪声抑制的新框架，通过梯度域加权引导滤波准确估计光照，应用Retinex模型分解图像，并行处理光照和反射层，最后通过多曝光融合和线性拉伸优化动态范围。


<details>
  <summary>Details</summary>
Motivation: 在挑战性光照条件下捕获的图像常出现各种退化，导致基于视觉的应用性能下降。现有方法要么显著放大固有噪声，要么仅在特定光照条件下有效，需要一种能在复杂光照条件下同时进行增强和噪声抑制的解决方案。
...
Conclusion: 提出的框架能够有效处理复杂光照条件下的图像退化问题，在同时进行图像增强和噪声抑制方面表现出优越性能，为实际应用中的图像质量提升提供了有效解决方案。
...
TL;DR: 该研究针对面部美化处理对生物识别系统的影响，提出通过分析美化评估算法的变化和改进特征提取方法来检测面部美化，并在未知美化算法的情况下实现了1.1%的D-EER检测性能。


<details>
  <summary>Details</summary>
Motivation: 面部美化处理在社交媒体和广告中广泛使用，但当这些美化后的图像作为生物识别样本使用时，会对人脸识别系统构成挑战。因此需要有效检测面部美化处理。

Method: 研究分析美化评估算法在美化图像中的变化，评估基于人工智能的不同特征提取方法以改进美化检测，并探索是否可以利用面部美感评估来提升检测率。

Result: 在攻击性美化算法未知的场景下，该研究在单图像检测中实现了1.1%的D-EER（检测等错误率）。

Conclusion: 面部美化检测对于保护生物识别系统的可靠性至关重要，通过分析美感评估特征和改进特征提取方法可以有效检测美化处理，即使在未知美化算法的情况下也能取得良好性能。
...
TL;DR: 本文开发了基于深度学习的鱼类重识别管道，使用AutoFish数据集，通过硬三元组挖掘和定制图像变换，Swin-T架构在mAP@k和Rank-1准确率上优于ResNet-50。


<details>
  <summary>Details</summary>
Motivation: 随着电子监控系统在渔业中的广泛应用，产生了大量视频数据需要分析，手动审查不可行，需要自动化鱼类重识别技术来支持可持续海洋资源管理。

Method: 使用AutoFish数据集（模拟电子监控系统的传送带场景，包含6种相似鱼类），开发优化的深度学习管道，采用硬三元组挖掘策略和定制图像变换（包括数据集特定归一化），比较Vision Transformer的Swin-T架构和CNN的ResNet-50性能。

Result: Swin-T架构在mAP@k和Rank-1准确率上显著优于ResNet-50，达到41.65% mAP@k和90.43% Rank-1准确率。深入分析表明主要挑战在于区分同一物种的视觉相似个体，视角不一致比部分遮挡更具破坏性。

Conclusion: 硬三元组挖掘结合定制图像变换能显著提升鱼类重识别性能，Vision Transformer架构在相似鱼类识别任务中优于传统CNN架构，为渔业电子监控系统的自动化分析提供了有效解决方案。
...
TL;DR: SAM-Body4D是一个无需训练的框架，通过视频分割和遮挡感知模块实现时间一致且对遮挡鲁棒的人体网格恢复，解决了现有方法在视频中时间不一致和遮挡下性能下降的问题。


<details>
  <summary>Details</summary>
Motivation: 现有基于图像的人体网格恢复方法（如SAM 3D Body）在处理视频时存在两个主要问题：1）依赖逐帧推理导致时间不一致性；2）在遮挡情况下性能下降。作者希望在不重新训练的情况下，利用视频中人体连续性来解决这些问题。

Method: 提出SAM-Body4D框架：1）使用可提示的视频分割模型生成身份一致的掩码片段；2）通过遮挡感知模块恢复缺失区域；3）用精炼后的掩码片段指导SAM 3D Body生成一致的全身体网格轨迹；4）采用基于填充的并行策略实现高效的多人体推理。

Result: 实验结果表明，SAM-Body4D在具有挑战性的野外视频中实现了更好的时间稳定性和鲁棒性，且无需任何重新训练。代码和演示已开源。

Conclusion: SAM-Body4D通过利用视频中的人体连续性，在不重新训练的情况下解决了现有方法的时间不一致性和遮挡鲁棒性问题，为视频中的人体网格恢复提供了有效的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有MLLMs由于内存开销过大，只能处理有限帧数的视频，无法有效理解长视频内容。需要一种既能保持知识完整性又能保证语义连贯性的高效长视频处理方法。

Method: 提出OneClip-RAG范式，包含：1）查询引导的视频分块算法，统一分块和跨模态检索；2）SynLongVideo数据集和渐进式训练策略；3）基于视频片段的检索增强机制，充分利用视频片段的知识完整性和语义连贯性优势。

Result: 在五个MLLMs上验证，显著提升长视频理解性能：将InternLV2 8B和Qwen2-VL 7B提升到GPT-4o在MLVU上的水平；效率优势明显：LLaVA-Video能在单张4090 GPU上2.2分钟内处理长达1小时的视频。
...
TL;DR: 提出一种基于深度图的6D位姿估计方法，通过多视角深度图融合生成精细点云或稀疏TSDF，使用分层热图机制和密度感知稀疏Transformer，实现高分辨率稀疏体素表示，在密集工业分拣场景中表现优异。


<details>
  <summary>Details</summary>
Motivation: 工业分拣环境中存在严重遮挡、反射和无纹理部件，传统6D位姿估计方法难以准确恢复物体姿态。现有稀疏3D方法在远距离感知中有效，但在近距离机器人应用中潜力未充分挖掘。

Method: 1) 多视角深度图融合为精细点云或稀疏TSDF；2) 分层热图机制生成场景自适应注意力先验；3) 密度感知稀疏Transformer处理遮挡和非均匀分布；4) 全稀疏框架实现高分辨率体素表示；5) 基于体素投票策略预测6D位姿。

Result: 在IPD和MV-YCB多视角数据集上验证，在密集工业分拣和家庭分拣场景中表现出竞争力，能够同时预测任意数量目标物体的6D位姿。

Conclusion: 提出的全稀疏深度图6D位姿估计框架能够有效处理密集分拣环境中的遮挡和反射问题，通过高分辨率体素表示捕获精细几何细节，为工业机器人应用提供可靠解决方案。
...
TL;DR: LapFM是一种用于腹腔镜手术分割的基础模型，通过分层概念演进预训练范式，从大量无标注手术图像中学习鲁棒的分割能力，显著超越现有方法。


<details>
  <summary>Details</summary>
Motivation: 当前手术分割面临标注稀缺和不同手术间语义不一致的挑战。现有方法通常只是对自然基础模型进行微调，仅作为领域适配器而非真正的手术基础模型，难以泛化到多样化的手术目标。

Method: 提出分层概念演进预训练范式：1）通过分层掩码解码器建立腹腔镜概念层次结构，统一不同实体；2）提出置信度驱动的演进标注，基于分层一致性迭代生成和过滤伪标签，逐步将可靠样本纳入训练，构建了包含114K图像-掩码对的大规模基准数据集LapBench-114K。

Result: 大量实验表明，LapFM显著优于现有最先进方法，为通用腹腔镜分割中的粒度自适应泛化设立了新标准。

Conclusion: LapFM是一个专门设计用于腹腔镜手术分割的基础模型，通过创新的分层概念演进预训练方法，有效解决了标注稀缺和语义不一致问题，实现了在多样化手术目标上的鲁棒泛化能力。
...
<details>
  <summary>Details</summary>
Motivation: 现有快照多光谱成像技术虽然能捕获比传统RGB传感器更丰富的频谱信息，但大多数现有方法将色彩校正流程分为多个独立阶段，往往在早期就丢弃多光谱数据，无法充分利用频谱信息。

Method: 提出一个统一的端到端学习框架，将整个色彩校正流程集成在单一模型中，联合利用高分辨率RGB传感器和辅助低分辨率多光谱传感器的数据。通过重构两种不同的最先进图像到图像架构来展示框架的灵活性和通用性。

Result: 通过聚合和重新利用公开可用的频谱数据集，在多种RGB相机灵敏度下渲染构建专用数据集进行训练和评估。实验表明，该方法相比仅使用RGB和多光谱驱动的基线方法，色彩准确性和稳定性均有提升，误差减少高达50%。

Conclusion: 该统一学习框架能够有效利用多光谱信息进行端到端色彩校正，显著提升色彩准确性和稳定性，为消费级和移动设备的多光谱成像应用提供了有效的解决方案。
...
TL;DR: 该论文提出了一种结合子模子集选择与层间梯度不确定性估计的框架，以提升深度视觉模型在分布外（OOD）场景下的解释鲁棒性和保真度。
...
Result: 经验评估表明，该框架不仅缓解了现有方法在OOD场景下的弱点，还在ID设置中带来了改进。提升了归因和对象级可解释性，使解释更加鲁棒和可靠。

Conclusion: 研究揭示了当前基于子集方法的局限性，并展示了不确定性驱动优化如何增强归因和对象级可解释性，为真实世界视觉应用中更透明和可信的AI铺平了道路。
...
TL;DR: 提出一种基于SAM的轻量级足球运动员跟踪方法，结合SAM分割、CSRT跟踪器和球衣颜色外观模型，在资源受限条件下实现稳定的实时跟踪性能。


<details>
  <summary>Details</summary>
Motivation: 足球运动员跟踪面临频繁遮挡、相似外观和快速运动等挑战，需要一种轻量级且鲁棒的跟踪方法，能够在拥挤场景中保持身份一致性。

Method: 提出团队感知跟踪系统：1) 使用SAM进行精确初始化；2) 结合CSRT跟踪器进行连续跟踪；3) 基于HSV直方图的球衣颜色外观模型进行重识别以改善遮挡恢复。

Result: 在足球视频序列上达到7.6-7.7 FPS，内存使用稳定(~1880 MB)。轻度遮挡下保持100%跟踪成功率，拥挤禁区场景(5+球员)下90%成功率。外观重识别恢复50%重度遮挡，但对长时间离帧的球员重获取成功率仅8.66%。

Conclusion: SAM+CSRT组合在不同人群密度下提供一致性能，但需要更强的重识别机制处理长时间遮挡。该方法为资源受限条件下的足球跟踪系统部署提供了实用指导。
...
Conclusion: ContextDrag通过有效利用参考图像的上下文信息，实现了精确的拖拽控制，同时在语义和纹理细节上保持一致性，为基于拖拽的图像编辑提供了新的有效解决方案。
...
TL;DR: PCI框架通过分析概念插入成功率来研究扩散模型中概念形成的动态过程，揭示不同概念在去噪轨迹中的锁定时机，为图像编辑提供有效干预时机


<details>
  <summary>Details</summary>
Motivation: 当前扩散模型评估主要关注最终输出，但生成过程是动态的轨迹，理解这一动态过程对于掌握模型的可控性、可靠性和可预测性至关重要。研究需要回答：噪声何时转化为特定概念并在去噪轨迹中锁定？
...
Result: 应用于多个最先进的文本到图像扩散模型和广泛的概念分类，PCI揭示了不同扩散模型中多样的时间行为，轨迹的某些阶段对特定概念更有利，即使在同一概念类型内也存在差异。这些发现为文本驱动图像编辑提供了可操作的见解。

Conclusion: PCI框架能够分析扩散模型中概念形成的动态过程，识别概念锁定的关键时机，为无需访问模型内部或训练的干预提供指导，在语义准确性和内容保留之间实现比基线更强的编辑效果。
...
TL;DR: 首个面向多相机阵列的实时3D高斯泼溅重建框架，通过多相机融合实现无漂移轨迹估计和高效在线重建，仅用2分钟即可重建数百米3D场景。
...
Conclusion: 该工作首次实现了多相机阵列的实时3D高斯泼溅重建，通过创新的相机对齐、优化调度和采样策略，在保持实时性能的同时显著提升了重建覆盖范围和质量。
...
<details>
  <summary>Details</summary>
Motivation: 多模态大推理模型（MLRMs）通过分层链式推理从个人图像中推断精确地理位置，现有隐私保护技术主要针对感知模型设计，对MLRMs的复杂多步推理过程无效

Method: 提出ReasonBreak对抗框架，基于概念感知扰动破坏MLRMs的分层推理。关键见解是有效的地理推理破坏需要与概念层次对齐的扰动而非均匀噪声。该方法战略性地针对推理链中的关键概念依赖，生成使特定推理步骤失效并级联影响后续推理阶段的扰动。同时贡献GeoPrivacy-6K数据集，包含6,341张超高分辨率图像和分层概念标注
...
Conclusion: ReasonBreak为对抗基于推理的隐私威胁建立了新范式，通过概念感知扰动有效破坏MLRMs的分层推理能力，显著提升地理位置隐私保护效果
...
<details>
  <summary>Details</summary>
Motivation: 尽管条件扩散模型在文本到图像生成方面取得了成功，但错位和幻觉问题仍然普遍存在。现有方法需要在完整生成后才能进行对齐评估，计算成本高昂。本研究假设文本/图像错位可以在去噪过程的早期被检测到，从而实现实时对齐评估。

Method: 提出NoisyCLIP方法，在噪声潜在空间中测量语义对齐。这是首次探索和基准测试在反向扩散过程中使用双编码器进行提示到潜在错位检测的方法。

Result: NoisyCLIP在Best-of-N设置中将计算成本降低了50%，同时达到了98%的CLIP对齐性能。该方法实现了生成过程中的实时对齐评估，在不牺牲语义保真度的情况下降低了成本。

Conclusion: NoisyCLIP证明了在去噪过程的早期阶段检测文本-图像对齐是可行的，为实时对齐评估提供了一种高效的方法，显著降低了计算成本同时保持了高质量的对齐性能。
...
<details>
  <summary>Details</summary>
Motivation: 从LiDAR点云重建建筑的主要挑战在于准确捕捉不同点密度和噪声干扰下的建筑表面。需要一种灵活的方法来获取不同分辨率下的高质量3D建筑轮廓。

Method: 提出OCCDiff方法，将潜在扩散过程与函数自编码器架构结合，在占用函数空间中生成连续可评估的占用函数。引入点编码器为扩散学习提供条件特征，约束最终占用预测，并为潜在编码器插入多模态特征。采用多任务训练策略增强模型性能。

Result: 实验结果表明，该方法生成的样本在物理一致性方面表现良好，对目标分布具有高保真度，并且对噪声数据表现出鲁棒性。

Conclusion: OCCDiff通过潜在扩散和函数自编码器的结合，能够灵活生成高质量的建筑3D轮廓，对点云密度变化和噪声具有鲁棒性，为建筑重建提供了有效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有GUI grounding模型存在显著的坐标预测不稳定性问题，微小的视觉扰动（如裁剪几个像素）就会导致预测结果在正确和错误之间翻转，严重影响模型性能，特别是在高分辨率和小UI元素的场景下
...
TL;DR: 本文提出一个统一的多模态框架，用于油画生成与编辑，通过空间对齐、语义增强和自监督风格转换技术，实现交互式油画创作。


<details>
  <summary>Details</summary>
Motivation: 油画作为融合人类抽象思维与艺术表达的高级媒介，因其复杂的笔触动态和风格化特征，在数字生成和编辑方面面临重大挑战。现有技术受限于训练数据分布，主要关注修改真实照片，缺乏对油画创作的专门支持。

Method: 提出统一多模态框架，包含三个关键技术：1）训练阶段的空间对齐和语义增强条件策略，将掩码和草图映射为空间约束，从参考图像和文本编码上下文嵌入作为特征约束；2）基于笔触渲染的自监督风格转换管道，模拟油画修复的修复动态，构建大规模配对训练数据集；3）推理阶段使用AdaIN算子集成特征确保风格一致性。

Result: 实验表明，该交互式系统能够实现细粒度编辑，同时保持油画的艺术品质，在风格化油画生成和编辑方面达到了前所未有的想象力实现水平。

Conclusion: 该工作提出的统一多模态框架成功解决了油画数字生成和编辑的挑战，通过结合参考图像、手绘草图和自然语言提示，实现了精确的语义控制和风格一致性，为交互式油画创作提供了有效解决方案。
...
TL;DR: Photo3D是一个通过GPT-4o-Image生成的图像数据驱动，提升3D生成真实感的框架，通过结构对齐的多视角合成和细节增强技术，在不同3D原生生成器上实现最先进的逼真3D生成效果。


<details>
  <summary>Details</summary>
Motivation: 当前3D原生生成器在几何结构上取得了进步，但在真实感外观方面仍不足。主要障碍在于缺乏多样化的高质量真实世界3D资产，这些资产需要丰富的纹理细节，而由于场景尺度多样、物体非刚性运动和3D扫描精度有限，获取这样的数据本质上是困难的。

Method: 1. 使用GPT-4o-Image模型生成的图像数据驱动框架；2. 设计结构对齐的多视角合成管道，构建细节增强的多视角数据集与3D几何配对；3. 提出真实细节增强方案，利用感知特征适应和语义结构匹配，在保持3D原生几何结构一致性的同时增强真实感外观细节；4. 为几何-纹理耦合和解耦的3D原生生成范式设计专门的训练策略。
...
TL;DR: Fast-ARDiff提出了一种统一的AR-扩散混合框架，通过熵感知的推测解码和动态调度器联合优化AR和扩散组件，显著加速图像生成，在ImageNet 256×256上实现4.3倍无损加速。


<details>
  <summary>Details</summary>
Motivation: 现有的AR-扩散混合范式结合了AR的结构化建模和扩散的光照真实合成能力，但由于顺序AR生成和迭代去噪过程，存在高延迟问题。需要解决这一瓶颈，实现更高效的生成。

Method: 1) 熵感知推测策略：鼓励草稿模型产生与目标模型熵特征对齐的高熵表示，缓解熵不匹配和因草稿过度自信导致的高拒绝率。2) 动态调度器：将扩散解码集成到端到端框架中，优先优化AR部分以指导后续扩散步骤。3) 联合蒸馏框架：结合轨迹匹配和分布匹配优化扩散部分，确保稳定训练和极少数步骤下的高质量合成。4) 推理时使用AR模块的浅层特征熵预过滤低熵草稿，避免冗余计算。

Result: 在ImageNet 256×256上，TransDiff模型实现了4.3倍无损加速；在文本条件生成任务中，NextStep-1模型实现了3倍加速。该方法在多种模型上均达到了最先进的加速效果。

Conclusion: Fast-ARDiff通过联合优化AR和扩散组件，有效解决了AR-扩散混合范式的延迟瓶颈，实现了显著的加速效果，同时保持了生成质量，为高效图像生成提供了新思路。
...
TL;DR: 提出了一种无需迭代的固定点估计器用于扩散反演，通过误差近似方法避免固定点迭代的高计算成本和超参数选择问题


<details>
  <summary>Details</summary>
Motivation: 扩散反演旨在恢复给定图像的初始噪声，以便通过去噪扩散过程重建原始图像。现有的固定点迭代方法虽然能最小化重建误差，但存在计算成本高和超参数选择复杂的问题

Method: 首先从理想反演步骤推导出固定点的显式表达式，但该表达式包含未知的数据预测误差。然后引入误差近似方法，使用前一个反演步骤的可计算误差来近似当前步骤的未知误差，从而得到可计算的近似固定点表达式

Result: 在NOCAPS和MS-COCO两个文本-图像数据集上评估重建性能。与DDIM反演和其他基于固定点迭代的反演方法相比，该方法在不增加额外迭代或训练的情况下，在重建任务中取得了一致且优越的性能

Conclusion: 提出的无需迭代的固定点估计器解决了扩散反演中固定点迭代方法的高计算成本和超参数选择问题，通过理论分析表明该估计器具有低方差特性，在实际应用中表现出色
...
Method: 采用两阶段方法：1) 通过无监督数据驱动分解方法发现fMRI活动中的可解释模式；2) 通过识别最能激发每个模式的自然图像集，并生成描述其共享视觉意义的自然语言描述来解释每个模式。引入自动化流程测试多个候选解释，分配定量可靠性分数，并为每个体素模式选择最一致的描述。

Result: 该框架揭示了数千个可解释模式，涵盖许多不同的视觉概念，包括以前未报告的细粒度表征。

Conclusion: 提出了一个可扩展的自动化框架，能够大规模发现和解释人类大脑皮层中的视觉表征，克服了传统方法的局限性。
...
TL;DR: 提出模块化神经图像信号处理框架，可处理原始RAW输入并生成高质量显示图像，具有高度模块化设计，支持用户交互式照片编辑


<details>
  <summary>Details</summary>
Motivation: 现有神经ISP设计缺乏模块化，无法灵活控制渲染过程的中间阶段，限制了可扩展性、可调试性、泛化能力和用户风格匹配的灵活性

Method: 采用模块化神经ISP框架，将整个图像渲染过程分解为多个可独立控制的中间阶段，支持不同容量变体（0.5M-3.9M参数），并开发了用户交互式照片编辑工具

Result: 在多个测试集上获得有竞争力的定性和定量结果，实现了高渲染精度，同时提高了可扩展性、可调试性、对未见相机的泛化能力以及匹配不同用户偏好风格的灵活性

Conclusion: 模块化神经ISP框架不仅实现了高质量的图像渲染，还提供了前所未有的控制灵活性，支持用户交互式编辑和无限后编辑重新渲染，为神经ISP设计提供了新的方向
...
TL;DR: 本文提出了一种用于连续测试时适应的自适应伪标签调整方法，通过考虑类别和实例级别的置信度分布，动态平衡学习，在语义分割任务中显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有连续测试时适应方法通常依赖固定或批次级别的阈值，无法处理不同类别和实例之间的难度差异，这在需要密集多类别预测的语义分割任务中尤为严重，导致监督信号不可靠和错误累积。

Method: 提出自适应调整伪标签的方法，考虑每个图像内的置信度分布，动态平衡受域偏移影响最大的类别的学习，实现细粒度的类别和实例感知适应。

Result: 在八个CTTA和TTA场景（包括合成到真实和长期偏移）的广泛实验中，该方法始终优于最先进技术，为演化条件下的语义分割设定了新标准。

Conclusion: 通过自适应伪标签调整和动态类别平衡，该方法在连续测试时适应中产生更可靠的监督信号，有效减轻错误累积，显著提升语义分割在演化域中的性能。
...
TL;DR: HiGINE是一种基于分层图的方法，利用多路免疫荧光图像中的肿瘤微环境特征预测肺癌患者生存期，并通过多模态融合提升风险分层性能。
...
Method: 提出HiGINE分层图模型，编码细胞邻域中的局部和全局相互关系，整合细胞类型和形态学信息，并通过多模态融合将癌症分期与mIF衍生特征相结合。

Result: 在两个公共数据集上验证，显示HiGINE在风险分层方面有改进，具有鲁棒性和良好的泛化能力。

Conclusion: HiGINE能够有效利用肿瘤微环境的复杂相互作用特征，提升肺癌患者的生存预测和风险分层性能。
...
<details>
  <summary>Details</summary>
Motivation: 手术视频对教育和研究很重要，但拍摄时外科医生经常遮挡摄像头视野。现有方法需要手动调整摄像头位置和后期手动对齐，工作量大且效率低

Method: 使用安装在无影灯上的多摄像头系统，自动检测照明系统移动的帧，实时对齐图像，选择遮挡最少的摄像头，生成固定视角的手术视频

Result: 外科医生用户研究表明，该方法生成的视频在确认手术区域和观看舒适度方面优于传统方法，视频质量也有所提升。还评估了不同合成选项的偏好

Conclusion: 提出的自动化对齐和视角合成方法能有效解决手术视频遮挡问题，提高视频质量，为手术教育和研究提供更好的视频素材
...
TL;DR: 该研究应用深度学习改进和自动化花粉粒在光学和全息显微镜图像中的检测与分类，特别关注兽医细胞学应用。使用YOLOv8s进行目标检测，MobileNetV3L进行分类，在光学图像上取得了良好效果，但在全息图像上性能较差，通过数据集扩展和标注改进提升了全息图像的性能。


<details>
  <summary>Details</summary>
Motivation: 研究动机是探索深度学习在花粉粒检测和分类中的应用，特别是在光学和全息显微镜图像中，重点关注兽医细胞学的实际应用场景。全息显微镜作为一种成本效益高的设备，其图像处理存在挑战，需要改进深度学习模型在该领域的性能。

Method: 研究方法包括：1）使用YOLOv8s模型进行花粉粒的目标检测；2）使用MobileNetV3L模型进行分类任务；3）在光学和全息显微镜图像上评估模型性能；4）针对全息图像性能差距问题，采用数据集扩展（自动标注）和边界框区域扩大的技术来改进性能。

Result: 研究结果显示：在光学图像上，检测模型达到91.3% mAP50，分类模型达到97%总体准确率。在全息图像上，初始性能较低（检测2.49% mAP50，分类42%准确率）。通过改进技术后，全息图像检测性能提升至13.3% mAP50，分类性能提升至54%准确率。

Conclusion: 研究结论表明，深度学习技术可以与成本效益高的无透镜数字全息显微镜设备相结合，至少在图像分类任务上是可行的。虽然全息图像的性能仍低于光学图像，但通过数据集扩展和标注改进等技术可以显著提升性能，为低成本显微镜设备在花粉粒分析中的应用提供了可能性。
...
Conclusion: 通过识别并校正CLIP中的模板-样本相似性偏差，使用空提示和偏差校准损失可以有效提升模型在少样本学习中的性能，使模型更关注真实的视觉-语义对齐关系。
...
TL;DR: 本文提出了一种仅使用单目RGB图像和自然语言指令的无人机视觉语言导航统一框架，通过多任务学习联合优化空间感知、轨迹推理和动作预测，显著提升了轻量化无人机的实际部署能力。


<details>
  <summary>Details</summary>
Motivation: 现有的空中视觉语言导航方法通常依赖全景图像、深度输入或里程计来支持空间推理和动作规划，这些要求增加了系统成本和集成复杂性，阻碍了轻量化无人机的实际部署。因此需要一种仅使用单目RGB观测和自然语言指令的解决方案。

Method: 1. 将导航任务形式化为下一个令牌预测问题；2. 通过提示引导的多任务学习联合优化空间感知、轨迹推理和动作预测；3. 提出关键帧选择策略以减少视觉冗余；4. 引入动作合并和标签重加权机制来缓解长尾监督不平衡并促进稳定的多任务协同训练。
...
Conclusion: 本文提出的统一框架证明了仅使用单目RGB观测和自然语言指令实现高效空中视觉语言导航的可行性，为轻量化无人机的实际应用部署提供了有效的解决方案。
...
Conclusion: OpenMonoGS-SLAM是首个结合3D高斯泼溅与开放集语义理解的单目SLAM框架，通过视觉基础模型实现了在开放世界环境中的准确相机跟踪、建图和语义理解，为空间AI提供了强大的感知能力。
...
<details>
  <summary>Details</summary>
Motivation: 肤色历来是歧视的焦点，但医学影像中的机器学习公平性研究通常依赖粗糙的亚组分类，忽视了个体层面的差异。这种基于群体的方法可能会掩盖亚组内异常值面临的偏见。
...
<details>
  <summary>Details</summary>
Motivation: 在没有机械稳定器的情况下，相机在拍摄过程中会产生不可避免的旋转运动，特别是在长曝光场景下会导致透视模糊。从光学角度看，透视模糊是深度位置相关的：即使在同一成像设置下，位于不同空间位置的物体也会产生不同程度的模糊。

Method: 1. 使用现成的视觉编码器和点跟踪器提取视频信息；2. 通过窗口化嵌入和多窗口聚合估计深度图；3. 使用视觉语言模型对光学算法生成的稀疏轨迹进行密集化处理

Result: 在多个深度数据集上的评估表明，该方法在大深度范围内表现出色，同时保持良好的泛化能力。相对于手持拍摄设置中的真实轨迹，光学算法实现了更高的精度，密集重建保持了较强的准确性。

Conclusion: 通过分析视频模糊模式来估计深度是一种有效的方法，结合光学设计算法能够实现准确的深度估计和密集轨迹重建，在手持拍摄等无稳定器场景中具有实用价值。
...
<details>
  <summary>Details</summary>
Motivation: 扩散模型在图像合成方面取得了显著成功，但处理伪影和不真实区域仍然是关键挑战。需要一种能够检测并修复这些缺陷的方法来提升图像生成质量。

Method: 提出自优化扩散框架，采用基于可解释AI的缺陷高亮器生成缺陷激活图（FAMs），在正向过程中放大缺陷区域的噪声，在反向过程中重点关注这些区域，从而改善重建质量。

Result: 在各种基于扩散的模型上实现了高达27.3%的Fréchet inception距离改进，在多样化数据集上表现一致强劲。在不同任务（图像生成、文本到图像生成、修复）中都显示出稳健的有效性。

Conclusion: 可解释AI技术不仅限于可解释性，还能积极促进图像优化。该框架为各种扩散模型和任务提供了通用有效的解决方案，显著推动了图像合成领域的发展。
...
<details>
  <summary>Details</summary>
Motivation: 文档阴影去除对于提高数字化文档清晰度至关重要，阴影常常会模糊或扭曲文本边缘和线条等高频细节结构，需要有效去除阴影的同时保留这些精细结构。

Method: 提出MatteViT框架：1) 轻量级高频放大模块(HFAM)分解并自适应放大高频分量；2) 基于连续亮度的阴影蒙版，使用自定义蒙版数据集和阴影蒙版生成器提供精确空间引导；结合空间和频域信息进行阴影去除。

Result: 在RDD和Kligler等公开基准测试中达到最先进性能，能更好地保留文本级细节，在下游任务如光学字符识别中提高了识别性能。

Conclusion: MatteViT为现实世界文档阴影去除提供了鲁棒实用的解决方案，通过空间和频域信息结合以及精细结构保留策略，实现了高质量的阴影去除效果。
...
<details>
  <summary>Details</summary>
Motivation: 当前图像生成模型是"黑盒"系统，缺乏可观察性和干预能力，难以确保可靠性、安全性和控制性，且其非人类工作流程难以解释。

Method: 提出Chain-of-Image Generation框架：1) 使用LLM将复杂提示分解为简单逐步指令；2) 图像生成模型按计划逐步生成和编辑图像；3) 每个步骤聚焦单一语义实体，实现直接监控。

Result: 实验表明CoIG显著增强定量可监控性，在组合鲁棒性方面与基线模型相当；引入两个新指标评估：CoIG可读性（评估中间步骤清晰度）和因果相关性（量化每个步骤对最终图像的影响）。

Conclusion: CoIG框架通过将复杂生成任务分解为简单子问题，缓解实体崩溃问题，实现类似人类艺术创作的序列化语义过程，增强图像生成的可监控性和可解释性。
...
TL;DR: C-DIRA：一种轻量级驾驶员分心行为识别框架，通过动态ROI路由和对抗学习实现高效准确识别，在计算效率和泛化能力上优于现有轻量模型。


<details>
  <summary>Details</summary>
Motivation: 车载摄像头实时识别驾驶员分心行为需要在边缘设备上进行实时推理，但现有轻量模型难以捕捉细粒度行为特征，ROI方法又增加计算成本，需要在效率和准确性之间取得平衡。

Method: 提出C-DIRA框架：1）显著性驱动的Top-K ROI池化和融合分类进行局部特征提取与整合；2）动态ROI路由，仅对高难度样本应用ROI推理；3）伪域标注和对抗学习学习对驾驶员和背景变化鲁棒的域不变特征。

Result: 在State Farm Distracted Driver Detection Dataset上的实验表明，C-DIRA在显著减少FLOPs和降低延迟的同时保持高准确率，在模糊和低光等视觉退化条件下表现鲁棒，在未见域上性能稳定。

Conclusion: C-DIRA有效实现了紧凑性、效率和泛化能力的平衡，为轻量级驾驶员行为识别提供了有效的解决方案。
...
TL;DR: 提出一种无需训练的双曲适配器方法T-DHA，用于提升视觉语言模型在少样本学习和领域泛化任务中的性能，通过双曲空间建模层次语义关系并减少特征维度。
...
Method: 提出T-DHA方法：1) 在双曲空间（Poincaré球模型）而非欧氏空间中建模视觉-语言关系的层次树结构；2) 利用双曲空间的指数体积增长特性提升表示和判别能力；3) 结合负学习，用更少特征维度实现更准确鲁棒的分类。

Result: 在多个数据集上的实验表明，T-DHA方法在少样本图像识别和领域泛化任务中显著优于现有最先进方法。

Conclusion: 双曲空间建模结合负学习的训练免费适配器方法能有效提升视觉语言模型的跨域适应能力，减少计算资源需求，提高性能。
...
TL;DR: 提出MNAME框架，通过内存银行机制提供大量负样本，实现无需外部编码器的生成模型训练，显著提升训练效率和生成质量。


<details>
  <summary>Details</summary>
Motivation: 去噪生成模型在视觉合成中占主导地位，但存在训练成本高和表示学习效率低的问题。现有方法依赖外部预训练编码器，引入额外开销和领域偏移。

Method: 提出MNAME框架：1) 使用内存银行机制维护动态更新的负样本队列，解耦负样本数量与批次大小；2) 采用低维投影头减少内存和带宽开销；3) 无需外部编码器，实现自包含训练。

Result: 在ImageNet-256上，MNAME在40万步内达到FID 2.40的SOTA性能，显著优于可比方法，实现更快收敛和更优生成质量。

Conclusion: MNAME框架通过内存银行机制有效解决了生成模型中负样本数量限制问题，实现了无需外部依赖的高效训练，在生成质量和训练效率方面均取得显著提升。
...
TL;DR: InfiniteVL是一种线性复杂度视觉语言模型架构，结合滑动窗口注意力与Gated DeltaNet，在资源受限情况下实现竞争性多模态性能，推理速度比传统Transformer快3.6倍以上。
...
Method: 提出InfiniteVL架构，将滑动窗口注意力与Gated DeltaNet相结合。采用三阶段训练策略：蒸馏预训练、指令调优和长序列SFT。使用不到领先VLMs 2%的训练数据。

Result: InfiniteVL不仅显著优于之前的线性复杂度VLMs，还能匹配领先的基于Transformer的VLMs性能，同时保持有效的长期记忆保留。相比使用FlashAttention-2加速的类似规模Transformer VLMs，实现超过3.6倍的推理加速，同时保持恒定延迟和内存占用。在流式视频理解场景中，保持稳定的24 FPS实时预填充速度。

Conclusion: InfiniteVL成功解决了窗口注意力和线性注意力的局限性，提供了一种高效、可扩展的视觉语言模型架构，在保持性能的同时大幅提升推理效率，特别适合长序列和多模态应用。
...
Method: 提出MoSAIC-ReID框架，使用基于LoRA的专家模型，每个专家对应一个行人属性，通过oracle路由器进行控制性归因分析。采用广义线性模型、统计检验和特征重要性分析来量化属性重要性。

Result: 在Market-1501和DukeMTMC数据集上取得有竞争力的性能，同时提供了大规模属性重要性量化研究。发现服装颜色和内在特征贡献最大，而配件等不常见线索影响有限。
...
<details>
  <summary>Details</summary>
Motivation: 现有基于CLIP的训练免费开放词汇语义分割方法在精确定位方面面临挑战，需要复杂流程整合多个模块，特别是在遥感场景中密集小目标众多的情况下。最近提出的SAM 3统一了分割和识别功能，为遥感开放词汇语义分割提供了新可能。

Method: 1. 实现掩码融合策略：结合SAM 3的语义分割头和Transformer解码器（实例头）的输出，充分利用两个头的优势改善土地覆盖分割。2. 使用存在头生成的存在分数过滤场景中不存在的类别，减少由大词汇量和地理空间场景中补丁级处理引起的误报。

Result: 在广泛的遥感数据集上进行评估，实验表明这种简单适配取得了有前景的性能，展示了SAM 3在遥感开放词汇语义分割任务中的潜力。

Conclusion: SAM 3在无需训练的情况下能够有效应用于遥感开放词汇语义分割任务，通过简单的掩码融合和类别过滤策略即可获得良好效果，为遥感语义分割提供了新的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有视觉推理方法存在两大问题：1）纯语言思维链方法需要大规模（图像、查询、答案）监督数据；2）程序合成方法虽然无需训练但存在逻辑缺陷和错误定位。需要一种无需标注的训练框架来同时提升推理和视觉定位能力。

Method: 提出基于AI验证器的无标注训练框架：1）LLM验证器通过强化学习优化LLM推理；2）VLM验证器通过自动硬负例挖掘增强视觉定位能力。结合现代AI系统的优势：先进的语言模型用于分解空间查询为子任务，强大的视觉专家模型通过高性能VLM批评器改进。

Result: 在多样化的空间推理任务上评估，该方法显著提升了视觉推理能力，超越了开源和专有模型。改进后的视觉定位模型进一步超越了最近的纯文本视觉推理方法。

Conclusion: VALOR框架通过AI验证器实现了无需标注的视觉推理训练，有效结合了语言推理和视觉定位的优势，在多个任务上取得了state-of-the-art性能。
...
TL;DR: 提出一种结合3D渲染和约束扩散模型的工作流，自动生成高质量、逼真的双孢蘑菇合成图像，无需大量真实标注数据即可训练出优秀的检测分割模型。


<details>
  <summary>Details</summary>
Motivation: 工业蘑菇种植依赖计算机视觉进行监控和自动收获，但开发准确的检测分割模型需要大量精确标注的数据集，成本高昂。合成数据虽可扩展，但通常缺乏足够的真实感，难以泛化到真实场景。

Method: 提出新颖工作流，将Blender中的3D渲染与约束扩散模型结合，自动生成高质量、逼真的双孢蘑菇合成图像。该方法保持对3D场景配置和标注的完全控制，同时实现逼真效果，无需专业计算机图形学知识。

Result: 发布两个合成数据集（各含6,000张图像，描绘超过25万个蘑菇实例）。在零样本设置下评估基于这些数据集训练的Mask R-CNN模型，在两个独立真实数据集（包括新收集的基准）上测试，实现了最先进的分割性能（在M18K上F1=0.859），仅使用合成训练数据。

Conclusion: 虽然方法在双孢蘑菇上演示，但提出的流程可以轻松适应其他蘑菇物种或其他农业领域，如水果和叶子检测。该方法为农业计算机视觉任务提供了一种高效、可扩展的合成数据生成解决方案。
...
TL;DR: 提出一种基于偏度指导的剪枝方法，用于压缩多模态Swin Transformer模型，在联邦学习环境中实现36%模型大小缩减且保持性能


<details>
  <summary>Details</summary>
Motivation: 高性能计算机视觉模型在医疗影像中表现出色，但计算密集、模型庞大，不适合边缘设备部署；同时隐私限制阻碍集中式数据管理，需要联邦学习解决方案

Method: 提出偏度指导的剪枝方法，基于输出分布的统计偏度，有选择性地剪枝多模态Swin Transformer的多头自注意力和多层感知机层
...
### [79] [Astra: General Interactive World Model with Autoregressive Denoising](https://arxiv.org/abs/2512.08931)
*Yixuan Zhu,Jiaqi Feng,Wenzhao Zheng,Yuan Gao,Xin Tao,Pengfei Wan,Jie Zhou,Jiwen Lu*

Main category: cs.CV

TL;DR: Astra是一个交互式通用世界模型，能够为自动驾驶、机器人抓取等多样化场景生成真实世界的未来预测，支持相机运动、机器人动作等多种精确动作交互，实现交互式、一致的长时视频预测。


<details>
  <summary>Details</summary>
Motivation: 当前扩散变换器在视频生成方面取得了进展，但能够从过去观察和动作预测长时未来的世界模型在通用场景和多样化动作形式方面仍未被充分探索，需要填补这一空白。

Method: 提出自回归去噪架构，使用时序因果注意力聚合过去观察并支持流式输出；采用噪声增强历史记忆避免过度依赖过去帧以平衡响应性和时序一致性；引入动作感知适配器直接注入动作信号到去噪过程；开发动作专家混合动态路由异构动作模态。

Result: 在多个数据集上的实验表明，Astra在保真度、长时预测和动作对齐方面优于现有最先进的世界模型，支持探索、操作和相机控制等多样化真实世界任务。

Conclusion: Astra实现了交互式、一致且通用的长时视频预测，支持各种形式的交互，为通用世界模型的发展提供了有效解决方案。
...
TL;DR: Wan-Move是一个简单可扩展的视频生成运动控制框架，通过将原始条件特征直接转化为运动感知特征，实现精确高质量的运动控制，无需修改现有模型架构。
...
Method: 1) 使用密集点轨迹表示物体运动，实现细粒度场景控制；2) 将轨迹投影到潜在空间，沿轨迹传播第一帧特征，生成对齐的时空特征图；3) 该特征图作为更新的潜在条件，直接集成到现成的图像到视频模型中作为运动指导。

Result: Wan-Move能够生成5秒480p视频，其运动可控性与Kling 1.5 Pro的商业Motion Brush相当；在MoveBench基准测试和公开数据集上均表现出优越的运动质量。

Conclusion: Wan-Move通过将条件特征直接运动化，实现了精确高质量的视频运动控制，无需辅助运动编码器或模型架构修改，具有良好的可扩展性，为视频生成提供了实用的运动控制解决方案。
...
TL;DR: 该研究探讨生成式与非生成式视觉方法在组合泛化能力上的差异，发现生成式方法通过解码器约束和反演能更好地实现组合泛化，而非生成式方法难以通过正则化或架构约束获得必要的归纳偏置。


<details>
  <summary>Details</summary>
Motivation: 人类视觉感知被认为需要生成式方法，但当前最成功的视觉模型都是非生成式的编码器模型。这引发了一个问题：生成是否真的是机器实现人类水平视觉感知的必要条件？研究旨在通过组合泛化这一人类感知的标志性特征来探讨这个问题。

Method: 在组合数据生成过程下，形式化了生成式（解码器基础）和非生成式（编码器基础）方法实现组合泛化所需的归纳偏置。理论上分析并证明：对编码器施加这些归纳偏置通常不可行，而生成式方法可以通过约束解码器并反演来直接实现。研究了两种反演方式：在线梯度搜索和离线生成重放。在逼真图像数据集上训练了多种生成式和非生成式方法进行实证验证。

Result: 缺乏必要归纳偏置的非生成式方法通常在组合泛化上失败，需要大规模预训练或额外监督来改善泛化能力。相比之下，生成式方法通过解码器的合适归纳偏置结合搜索和重放，在不需额外数据的情况下显著提升了组合泛化性能。
...
TL;DR: Tri-Bench基准测试显示，视觉语言模型在几何推理任务中表现有限，平均准确率约69%，无法识别特殊三角形类型，且相机倾斜会降低模型性能。


<details>
  <summary>Details</summary>
Motivation: 尽管视觉语言模型能力强大，但在真实场景变化下经常失败。需要验证其几何推理能力，特别是关注相机姿态和场景上下文干扰对模型性能的影响。

Method: 提出Tri-Bench基准，包含平面三角形问题，测试相对几何推理能力。评估四个最新VLMs，使用固定提示明确描述方形边框作为参考系，通过单应性获得正确答案。测试六个简单任务，包括二元和连续目标。

Result: 平均准确率约69%（最佳75%，最差64%）；在图像平面2D投影上准确率约72%；所有模型在识别特殊三角形类型（等边、等腰、直角三角形）时准确率降至0%；相机倾斜导致准确率下降约4.1%；物体干扰对准确率无显著影响。

Conclusion: 视觉语言模型未能正确利用提示中提供的明确参考系提示，默认依赖2D图像平面线索。模型在几何推理任务中存在显著局限性，特别是在处理特殊几何形状和相机姿态变化时。
...
Method: 通过微调预训练的视觉语言模型，结合专门的定位模块和专用控制令牌，实现语言和空间信息的联合推理，形成结构化定位机制。

Result: 在多个遥感基准测试中持续改进最先进水平，视觉定位任务相比先前方法获得了24.8%的相对提升。
...
<details>
  <summary>Details</summary>
Motivation: 无人机航拍图像中的目标具有任意方向和精细细节，需要旋转不变性进行精确分割。传统卷积架构如U-Net缺乏旋转不变性，导致分割精度下降。虽然可以通过多方向滤波器组实现旋转不变性，但这会显著增加计算成本和内存流量。

Method: 提出GPU优化的旋转不变卷积框架，消除了传统基于矩阵乘法的卷积所需的数据降维步骤。通过利用对称旋转滤波器间的结构化数据共享，实现了内存流量和计算冗余大幅减少的多方向卷积。该方法还可推广到加速任意旋转角度的卷积。

Result: 相比CUDNN，提出的卷积方法实现了20-55%更快的训练速度和15-45%更低的能耗，同时保持与最先进旋转不变方法相当的精度。在8方向设置下，对256×256输入实现了45%加速和41%节能，对1024×1024输入实现了32%加速和23%节能。集成到U-Net分割模型中，相比非旋转感知基线获得了高达6%的精度提升。

Conclusion: 该方法为现有旋转不变CNN框架提供了高效且有效的替代方案，在保持分割精度的同时显著提升了计算效率和能效，特别适用于无人机航拍图像等需要旋转不变性的应用场景。
...
TL;DR: UniLayDiff：首个统一的扩散Transformer模型，通过多模态框架处理内容感知布局生成的各种任务，包括无条件生成和各种约束条件生成。


<details>
  <summary>Details</summary>
Motivation: 内容感知布局生成在平面设计自动化中至关重要，但现实应用的多样性使得开发单一模型来统一各种输入约束的子任务（如元素类型、大小或关系约束）极具挑战性。现有方法要么只能处理部分任务，要么需要为不同条件使用单独的模型参数，缺乏真正的统一解决方案。

Method: 提出UniLayDiff：统一的扩散Transformer模型，首次使用单一端到端可训练模型处理各种内容感知布局生成任务。将布局约束视为独立模态，采用多模态扩散Transformer框架捕捉背景图像、布局元素和多样化约束之间的复杂交互。此外，通过在其他任务上预训练后使用LoRA进行微调来整合关系约束。

Result: 广泛的实验表明，UniLayDiff在从无条件生成到各种条件生成任务中都达到了最先进的性能，并且据我们所知，是首个能够统一完整范围内容感知布局生成任务的模型。

Conclusion: UniLayDiff不仅实现了统一的条件下生成，还提升了整体布局质量，为内容感知布局生成提供了一个全面且高效的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有图像到3D生成器虽然能从单视图恢复合理几何，但由于面向对象的训练限制，难以泛化到复杂、大规模场景，无法保证忠实结构和纹理。需要解决从单图像生成高质量纹理3D场景的挑战。

Method: 采用自进化、无需训练的框架，结合3D生成模型的几何推理和视频生成模型的视觉知识。通过三个迭代阶段：空间先验初始化、视觉引导的3D场景网格生成、空间引导的新视角生成，在2D和3D域之间交替优化。

Result: 在多样化场景上的实验表明，EvoScene相比强基线方法，在几何稳定性、视角一致的纹理和未见过区域补全方面表现更优，能够生成可直接用于实际应用的3D网格。

Conclusion: EvoScene通过结合现有模型的互补优势，以迭代方式逐步改进结构和外观，成功实现了从单图像生成高质量3D场景的目标，为实际应用提供了可直接使用的3D网格。
...
Method: 结合现成的视觉感知模型与高清头灯，动态预测最大化下游感知性能的最优照明场，减少空区域的照明以重新分配到物体区域。

Result: 在真实世界闭环驾驶场景中，相比标准近光灯，mAP50提升18.7%，mIoU提升5.0%，同时保持性能并减少40%能耗。

Conclusion: LiDAS通过将现有头灯转变为主动视觉执行器，为鲁棒的夜间感知提供了经济有效的解决方案，并可与领域泛化方法互补。
...
Method: 采用统一的Transformer架构，通过新颖的查询机制独立灵活地探测时空任意点的3D位置，避免密集逐帧解码和多任务特定解码器的复杂性。

Result: 该方法在广泛的4D重建任务中超越了先前方法，建立了新的最先进水平，实现了轻量化和高度可扩展的训练与推理。

Conclusion: D4RT通过创新的查询机制和统一架构，为动态场景的4D重建提供了一种高效、轻量且性能优越的解决方案。
...
### [89] [ThreadWeaver: Adaptive Threading for Efficient Parallel Reasoning in Language Models](https://arxiv.org/abs/2512.07843)
...
<details>
  <summary>Details</summary>
Motivation: 大型语言模型推理时的顺序解码导致高延迟，现有并行推理方法要么局限于监督行为克隆，要么相比顺序链式思维基线准确率显著下降，且需要定制化推理引擎，部署复杂。

Method: 1) 两阶段并行轨迹生成器，产生大规模高质量并行标注的链式思维数据用于监督微调；2) 基于trie的训练-推理协同设计，可在任何现成的自回归推理引擎上实现并行推理，无需修改位置嵌入或KV缓存；3) 并行感知的强化学习框架，教导模型在准确率和有效并行化之间取得平衡。

Result: 在六个具有挑战性的数学推理基准测试中，基于Qwen3-8B训练的ThreadWeaver实现了与前沿顺序推理模型相当的准确率（平均71.9%，AIME24上79.9%），同时将token延迟平均加速1.53倍，在准确率和效率之间建立了新的帕累托前沿。

Conclusion: ThreadWeaver框架成功实现了自适应并行推理，在保持准确率的同时显著提高推理效率，为大型语言模型的推理优化提供了有效的解决方案。
...
### [90] [RaX-Crash: A Resource Efficient and Explainable Small Model Pipeline with an Application to City Scale Injury Severity Prediction](https://arxiv.org/abs/2512.07848)
*Di Zhu,Chen Xie,Ziwei Wang,Haoyun Zhang*

Main category: cs.LG

TL;DR: RaX-Crash是一个资源高效、可解释的小模型管道，用于预测纽约市机动车碰撞事故的伤害严重程度，基于结构化数据训练树集成模型，优于小型语言模型，同时保持可解释性。
...
Method: 整合三个关联表（数千万条记录），构建统一特征模式的分区存储，训练紧凑的树集成模型（随机森林和XGBoost）于工程化的表格特征，并与本地部署的小型语言模型（SLMs）基于文本摘要的预测进行对比。

Result: 在时间保留测试集上，XGBoost和随机森林分别达到0.7828和0.7794的准确率，明显优于SLMs（0.594和0.496）；类别不平衡分析显示简单类别加权可提高致命事故召回率；SHAP归因分析突出人类脆弱性因素、时间和地点是预测严重程度的主要驱动因素。
...
### [91] [SABER: Small Actions, Big Errors - Safeguarding Mutating Steps in LLM Agents](https://arxiv.org/abs/2512.07850)
*Alejandro Cuadron,Pengfei Yu,Yang Liu,Arpit Gupta*

Main category: cs.LG

TL;DR: 论文分析了LLM智能体在长时程工具使用任务中的脆弱性，发现突变性动作（改变环境的操作）中的偏差对失败影响最大，而非突变性动作偏差影响很小。作者提出了CM框架作为模型无关的安全保障机制，并发布了τ-Bench Verified基准来改进评估。


<details>
  <summary>Details</summary>
Motivation: 尽管LLM智能体发展迅速，但在长时程工具使用任务上的表现仍然脆弱。为了理解这种脆弱性，研究者探索了一个基本问题：所有动作对失败的影响是否相同？他们旨在识别导致任务失败的关键因素，并开发相应的安全保障机制。

Method: 1. 分析τ-Bench（航空/零售）和SWE-Bench Verified上的执行轨迹，将轨迹分解为突变性（改变环境）和非突变性步骤；2. 形式化"决定性偏差"概念（最早将成功翻转为失败的动作级分歧）；3. 使用逻辑回归分析偏差对成功率的影响；4. 提出CM框架：突变门控验证、突变步骤前的针对性反思、基于块的上下文清理；5. 创建τ-Bench Verified基准，通过针对性修订恢复基准的评估空间。

Result: 1. 每个突变性动作中的额外偏差会显著降低成功率：在航空任务上降低92%的成功几率，在零售任务上降低96%；2. 非突变性动作偏差几乎没有影响；3. 错误随上下文长度增加而增长；4. CM框架带来显著提升：Qwen3-Thinking在航空任务上相对提升28%，零售任务11%，SWE-Bench Verified 7%；Claude模型也有9%/7%的提升；5. 发现τ-Bench存在天花板效应，τ-Bench Verified恢复了基准的评估空间。

Conclusion: 研究强调了动作级分析、针对性安全保障和可靠评估对于构建鲁棒的多轮智能体的重要性。突变性动作中的偏差是导致失败的关键因素，而CM框架作为模型无关的测试时安全保障机制能有效提升性能。同时，需要更可靠的基准来准确评估智能体能力。
...
### [92] [GPU Memory Prediction for Multimodal Model Training](https://arxiv.org/abs/2512.07853)
*Jinwoo Jeong,Minchul Kang,Younghun Go,Changyong Shin,Hyunho Lee,Junho Yoon,Gyeongsik Yang,Chuck Yoo*

Main category: cs.LG

TL;DR: 提出一个预测多模态模型GPU峰值内存使用量的框架，通过分解模型架构和分析训练行为来实现准确预测


<details>
  <summary>Details</summary>
Motivation: 随着智能体AI系统中深度学习模型规模和复杂度的增长，GPU内存需求经常超过可用容量，导致内存不足错误，这会中断训练并浪费大量计算资源。现有研究仅关注单模态架构，无法推广到多模态模型，而多模态模型在智能体AI系统中很常见。
...
Conclusion: 该框架能够准确预测多模态模型的GPU内存使用量，有助于防止内存不足错误，提高训练效率并节省计算资源。
...
### [93] [HSTMixer: A Hierarchical MLP-Mixer for Large-Scale Traffic Forecasting](https://arxiv.org/abs/2512.07854)
...
<details>
  <summary>Details</summary>
Motivation: 交通预测对现代城市管理很重要，现有模型通常具有二次计算复杂度，不适用于大规模实际场景，需要更高效的解决方案。

Method: 提出HSTMixer框架，采用全MLP架构，包含分层时空混合块进行多分辨率特征提取（自底向上聚合和自顶向下传播），以及自适应区域混合器根据区域语义生成变换矩阵。

Result: 在四个大规模真实世界数据集上的实验表明，该方法不仅达到了最先进的性能，而且具有竞争力的计算效率。

Conclusion: HSTMixer是一个高效有效的大规模交通预测框架，能够动态捕获不同区域的演化时空模式。
...
### [94] [LAPA: Log-Domain Prediction-Driven Dynamic Sparsity Accelerator for Transformer Model](https://arxiv.org/abs/2512.07855)
*Huizheng Wang,Hongbin Wang,Shaojun Wei,Yang Hu,Shouyi Yin*

Main category: cs.LG

TL;DR: LAPA提出了一种基于对数域注意力预测的算法-架构协同设计，通过消除昂贵乘法运算和减少累加开销，实现跨阶段稀疏Transformer加速，相比现有SOTA方法能效提升2.79-3.52倍。


<details>
  <summary>Details</summary>
Motivation: Transformer模型在不同阶段的计算瓶颈会随输入序列变化而动态变化，需要跨阶段稀疏加速策略。现有稀疏Transformer方法多为单阶段设计，其稀疏预测机制在多阶段应用时会产生显著的功耗开销。

Method: 提出LAPA对数域注意力预测算法-架构协同设计：1）设计非对称前导一计算方案消除昂贵乘法；2）提出混合精度多轮移位累加机制减少累加开销；3）设计数据特征依赖滤波器配合MRSA过程；4）设计专用加速器将理论增强转化为实际硬件改进。

Result: 实验结果显示，LAPA相比最先进的Spatten、Sanger和FACT方法，分别实现了3.52倍、3.24倍和2.79倍的能量效率提升。

Conclusion: LAPA通过创新的算法-架构协同设计，有效解决了跨阶段稀疏Transformer加速中的计算瓶颈和功耗问题，显著提升了能量效率，为动态变化的Transformer模型提供了高效的硬件加速方案。
...
### [95] [Medical Test-free Disease Detection Based on Big Data](https://arxiv.org/abs/2512.07856)
*Haokun Zhao,Yingzhe Bai,Qingyang Xu,Lixin Zhou,Jianxin Chen,Jicong Fan*

Main category: cs.LG

TL;DR: CLDD是一种基于图的深度学习模型，通过利用疾病间关联和患者间相似性进行协同学习，实现无需依赖医学检测的疾病检测，在MIMIC-IV数据集上表现优于基线方法。


<details>
  <summary>Details</summary>
Motivation: 传统疾病检测需要大量医学测试且成本高昂，难以对患者进行所有可能的医学测试来诊断或预测数百上千种疾病。需要开发一种能够减少诊断成本、提高可及性的疾病检测方法。

Method: 提出CLDD（疾病检测协同学习）模型，将疾病检测构建为协同学习任务，自适应地利用疾病间关联和患者间相似性。模型整合患者-疾病交互数据和电子健康记录中的人口统计学特征，无需依赖相应医学测试即可检测数百上千种疾病。

Result: 在包含61,191名患者和2,000种疾病的MIMIC-IV数据集处理版本上，CLDD在多个指标上持续优于代表性基线方法，召回率提高6.33%，精确率提高7.63%。案例研究表明CLDD能够成功在其排名靠前的预测中恢复被掩盖的疾病。

Conclusion: CLDD通过减少诊断成本和提高可及性，在大规模疾病筛查和社会健康安全方面具有应用前景，同时展示了疾病预测的可解释性和可靠性。
...
### [96] [SA^2GFM: Enhancing Robust Graph Foundation Models with Structure-Aware Semantic Augmentation](https://arxiv.org/abs/2512.07857)
...
<details>
  <summary>Details</summary>
Motivation: 当前图基础模型在领域噪声、结构扰动和对抗攻击下的鲁棒性研究不足，关键限制在于对层次结构语义建模不充分，这影响了模型的泛化能力。

Method: 1) 通过基于熵的编码树转换为结构感知文本提示进行特征增强；2) 使用自监督信息瓶颈机制通过结构引导压缩提取鲁棒可迁移表示；3) 引入专家自适应路由机制（混合专家架构+空专家设计）解决跨领域适应的负迁移问题；4) 提出微调模块通过联合社区内和社区间结构学习优化层次结构。

Result: 在广泛的实验中，SA^2GFM在节点和图分类任务中，在对抗随机噪声和对抗扰动的有效性和鲁棒性方面优于9个最先进的基线方法。

Conclusion: SA^2GFM通过结构感知语义增强、信息瓶颈压缩和专家自适应路由，显著提升了图基础模型在噪声和对抗攻击下的鲁棒性和领域自适应能力。
...
### [97] [FAIM: Frequency-Aware Interactive Mamba for Time Series Classification](https://arxiv.org/abs/2512.07858)
*Da Zhang,Bingyu Li,Zhiyuan Zhao,Yanhan Zhang,Junyu Gao,Feiping Nie,Xuelong Li*

Main category: cs.LG

TL;DR: FAIM：轻量级频率感知交互式Mamba模型，通过自适应滤波块提取频域特征，结合交互式Mamba块实现多粒度信息交互，在时间序列分类任务中实现精度与效率的平衡。


<details>
  <summary>Details</summary>
Motivation: 深度学习模型在时间序列分类中虽然能捕捉时间依赖性，但存在计算成本高、对噪声敏感、在小数据集上容易过拟合等问题，需要更轻量且鲁棒的解决方案。

Method: 提出FAIM模型：1）自适应滤波块（AFB）利用傅里叶变换提取频域特征，采用可学习自适应阈值动态抑制噪声，通过全局与局部语义自适应滤波的逐元素耦合深入建模不同频率成分的协同作用；2）交互式Mamba块（IMB）促进高效的多粒度信息交互，平衡细粒度判别特征和全局上下文信息的提取；3）引入自监督预训练机制增强对复杂时间模式的理解。

Result: 在多个基准测试上的广泛实验表明，FAIM持续优于现有最先进方法，在准确性和效率之间实现了优越的平衡，并在各种领域和高噪声场景中表现出色。

Conclusion: FAIM通过频域特征提取和高效信息交互，为时间序列分类任务提供了轻量级、鲁棒且高性能的解决方案，解决了传统深度学习模型的局限性。
...
### [98] [SetAD: Semi-Supervised Anomaly Learning in Contextual Sets](https://arxiv.org/abs/2512.07863)
*Jianling Gao,Chongyang Tao,Xuelian Lin,Junfeng Liu,Shuai Ma*

Main category: cs.LG

TL;DR: SetAD：将半监督异常检测重构为集合级任务，通过注意力集合编码器和分级学习目标直接建模群体级交互，显著优于现有方法


<details>
  <summary>Details</summary>
Motivation: 现有半监督异常检测方法主要关注单个点或简单对，忽略了异常的上下文本质（异常通常由偏离群体定义），且未能利用集合组合生成的丰富监督信号，难以利用数据中的高阶交互来学习判别性表示

Method: 提出SetAD框架，将半监督异常检测重构为集合级异常检测任务；使用基于注意力的集合编码器，通过分级学习目标训练模型量化整个集合的异常程度；提出上下文校准的异常评分机制，通过聚合点在多个不同上下文集合中相对于同伴行为的归一化偏差来评估异常分数

Result: 在10个真实世界数据集上的广泛实验表明，SetAD显著优于最先进模型；特别地，模型性能随着集合大小的增加而持续提升，为基于集合的异常检测公式提供了强有力的实证支持

Conclusion: SetAD通过集合级视角直接建模定义异常的复杂群体级交互，解决了现有点对中心方法的局限性；上下文校准评分机制增强了鲁棒性和分数校准；集合级方法为半监督异常检测提供了新的有效范式
...
### [99] [Pattern Recognition of Ozone-Depleting Substance Exports in Global Trade Data](https://arxiv.org/abs/2512.07864)
*Muhammad Sukri Bin Ramli*

Main category: cs.LG

TL;DR: 该论文提出了一种基于无监督机器学习的框架，用于分析海关数据以监测环境条约（如蒙特利尔议定书）的遵守情况，通过聚类、异常检测和启发式标记识别可疑贸易模式。


<details>
  <summary>Details</summary>
Motivation: 需要新的方法来监测环境条约的遵守情况，特别是通过分析大规模、复杂的海关数据集来发现可疑贸易活动。

Method: 结合多种无监督机器学习技术：1) 使用K-Means聚类发现基于货值和重量的贸易原型；2) 使用隔离森林和IQR进行异常检测，识别"超大贸易"和价格异常；3) 启发式标记识别模糊描述等规避策略；4) 将这些层整合为优先级评分系统。

Result: 应用于10万条贸易记录，成功识别出1,351个价格异常值和1,288个高优先级货物供海关审查。发现高优先级商品的价值重量比与普通商品不同且更高。模型成功检测到2021年初"超大贸易"的激增，这与美国AIM法案的实际监管影响直接相关。

Conclusion: 该研究提出了一个可重复的无监督学习流程，能够将原始贸易数据转化为可供监管机构使用的优先情报，验证了机器学习在条约监测中的实用性。
...
### [100] [Using Text-Based Life Trajectories from Swedish Register Data to Predict Residential Mobility with Pretrained Transformers](https://arxiv.org/abs/2512.07865)
*Philipp Stark,Alexandros Sopasakis,Ola Hall,Markus Grillitsch*

Main category: cs.LG

TL;DR: 将瑞典大规模登记数据转化为文本化的生命轨迹，解决分类变量高基数性和编码不一致问题，使用NLP模型预测居住流动性，证明文本化登记数据能有效支持纵向分析


<details>
  <summary>Details</summary>
Motivation: 解决数据分析中长期存在的两个挑战：分类变量的高基数性和随时间变化的编码方案不一致性，利用瑞典独特全面的人口登记数据探索生命轨迹分析的新方法

Method: 将690万个体（2001-2013年）的登记数据转化为语义丰富的文本生命轨迹，结合人口统计信息和年度居住、工作、教育、收入、家庭状况变化，使用多种NLP架构（LSTM、DistilBERT、BERT、Qwen）预测个体后期（2013-2017年）的居住流动性

Result: 序列模型和基于Transformer的模型比基线模型更有效地捕捉时间和语义结构，文本化的登记数据保留了关于个体路径的有意义信息，支持复杂、可扩展的建模，为开发新序列建模方法提供了严格测试平台

Conclusion: 将语义丰富的登记数据与现代语言模型相结合，可以显著推进社会科学中的纵向分析，这种数据集使在其他地方难以或不可能进行的分析和方法测试成为可能
...
### [101] [Advancing physiological time series reconstruction and imputation via mixture of receptive fields and experts fusion](https://arxiv.org/abs/2512.07873)
*Ci Zhang,Huayu Li,Changdi Yang,Jiangnan Xia,Yanzhi Wang,Xiaolong Ma,Jin Lu,Geng Yuan*

Main category: cs.LG

TL;DR: 提出基于混合专家(MoE)的噪声估计器，用于医学时间序列信号重建，通过RFAMoE模块自适应选择感受野，Fusion MoE模块并行生成多个噪声信号并融合，单次推理完成重建，显著降低计算成本。


<details>
  <summary>Details</summary>
Motivation: 扩散模型在时间序列信号重建中表现出潜力，但在医学时间序列领域应用不足。生理时间序列具有多变量、高时间变异性、高噪声和易受伪影影响等特点，使得基于深度学习的插补等任务仍然具有挑战性。

Method: 提出基于混合专家(MoE)的噪声估计器框架：1) RFAMoE模块让每个通道在扩散过程中自适应选择感受野；2) Fusion MoE模块并行生成K个噪声信号，通过路由机制融合，单次推理完成重建。

Result: 提出的框架在不同任务和数据集上始终优于基于扩散的SOTA方法，不仅提高了性能，还消除了多次推理过程带来的巨大计算成本和延迟。

Conclusion: 基于MoE的噪声估计器框架有效解决了医学时间序列信号重建的挑战，通过自适应感受野选择和并行融合机制，在保持高性能的同时显著降低了计算复杂度。
...
### [102] [Softly Symbolifying Kolmogorov-Arnold Networks](https://arxiv.org/abs/2512.07875)
*James Bagrow,Josh Bongard*

Main category: cs.LG

TL;DR: S2KAN提出了一种将符号原语直接集成到KAN训练中的方法，通过可学习的门控机制稀疏化表示，在符号项足够时发现可解释形式，不足时优雅地退化为密集样条。


<details>
  <summary>Details</summary>
Motivation: 传统KANs虽然提供了可解释机器学习的路径，但训练后的激活函数往往缺乏符号保真度，学习到没有意义对应关系的病态分解。需要一种方法能够在符号项足够时发现可解释形式，在符号项不足时仍能保持良好性能。

Method: 提出Softly Symbolified Kolmogorov-Arnold Networks (S2KAN)，将符号原语直接集成到训练中。每个激活函数从符号项和密集项的字典中提取，使用可学习的门控机制稀疏化表示。这种稀疏化是可微分的，支持端到端优化，并由最小描述长度目标指导。

Result: 在符号基准测试、动态系统预测和真实世界预测任务中，S2KAN展示了竞争性或更优的准确性，同时使用显著更小的模型。即使在无正则化压力下，也观察到涌现的自稀疏化证据。

Conclusion: S2KAN成功地将符号原语集成到KAN训练框架中，实现了在符号项足够时发现可解释形式，在符号项不足时优雅退化为密集样条的灵活机制，为可解释机器学习提供了新的有效方法。
...
### [103] [Graph Contrastive Learning via Spectral Graph Alignment](https://arxiv.org/abs/2512.07878)
...
<details>
  <summary>Details</summary>
Motivation: 现有对比学习方法（如InfoNCE）优化图嵌入在跨视图间的成对对齐，但缺乏对视图特定图-图全局结构的控制机制。

Method: 提出SpecMatch-CL损失函数，通过最小化视图特定图-图的归一化拉普拉斯矩阵差异来对齐图嵌入。理论上证明归一化拉普拉斯矩阵差异为理想完美对齐对比损失与当前损失之间的差异提供了上界。

Result: 在8个TU基准测试的无监督学习和低标签率半监督学习上达到新的SOTA，在PPI-306K和ZINC 2M数据集的迁移学习中获得一致性能提升。

Conclusion: SpecMatch-CL通过控制视图特定图-图的全局结构，有效提升了图对比学习的性能，在多种学习场景下表现出优越性。
...
### [104] [Nonnegative Matrix Factorization through Cone Collapse](https://arxiv.org/abs/2512.07879)
*Manh Nguyen,Daniel Pimentel-Alarcón*

Main category: cs.LG

TL;DR: 论文提出Cone Collapse算法，从几何视角重新审视非负矩阵分解，通过迭代收缩非负正交锥来恢复数据的最小生成锥，并在此基础上构建锥感知的正交NMF模型（CC-NMF），在聚类任务中表现优异。


<details>
  <summary>Details</summary>
Motivation: 现有NMF算法主要从优化视角出发，未能充分利用NMF诱导的锥几何结构。数据点位于凸锥中，其极射线编码了基本方向或"主题"。本文旨在从几何视角重新审视NMF，通过显式恢复数据锥来提升聚类性能。

Method: 提出Cone Collapse算法：从完整的非负正交锥开始，迭代收缩锥体直至数据的最小生成锥。证明在温和假设下，算法在有限步内终止并能恢复X^⊤的最小生成锥。在此基础上，通过对恢复的极射线应用单正交NMF，推导出锥感知的正交NMF模型（CC-NMF）。

Result: 在16个基准基因表达、文本和图像数据集上，CC-NMF在聚类纯度方面一致匹配或优于强NMF基线方法，包括乘法更新、ANLS、投影NMF、ONMF和稀疏NMF。

Conclusion: 显式恢复数据锥能够产生既有理论依据又在经验上强大的基于NMF的聚类方法。几何视角为NMF提供了新的理解，并带来了性能改进。
...
### [105] [Semi-Supervised Contrastive Learning with Orthonormal Prototypes](https://arxiv.org/abs/2512.07880)
*Huanran Li,Manh Nguyen,Daniel Pimentel-Alarcón*

Main category: cs.LG

TL;DR: 论文提出CLOP损失函数解决对比学习中的维度坍缩问题，通过促进类别嵌入形成正交线性子空间来提升性能


<details>
  <summary>Details</summary>
Motivation: 对比学习在深度学习中表现出色，但维度坍缩问题（嵌入收敛到低维空间）在半监督和自监督设置中尤为严重，限制了其效果

Method: 首先识别了导致维度坍缩的学习率阈值，然后提出CLOP损失函数，通过促进类别嵌入形成正交线性子空间来防止维度坍缩

Result: 在真实和合成数据集上的实验表明，CLOP在图像分类和目标检测任务中提升了性能，同时在不同学习率和批量大小下表现出更好的稳定性
...
### [106] [GSPN-2: Efficient Parallel Sequence Modeling](https://arxiv.org/abs/2512.07884)
...
TL;DR: GSPN-2通过算法-系统联合重新设计，解决了GSPN在GPU实现中的效率瓶颈，包括内核启动开销、内存传输和冗余计算问题，在保持视觉Transformer准确性的同时大幅降低计算成本。


<details>
  <summary>Details</summary>
Motivation: 现有GSPN虽然通过线扫描传播方案将自注意力计算复杂度降低到接近线性，但在GPU实现上仍存在三个主要效率瓶颈：重复内核启动开销、全局内存数据传输过多、以及每个通道单独维护传播权重导致的冗余计算。

Method: 1. 系统优化：将数千个微内核启动合并为单个2D内核，为每个通道片固定一个warp，在共享内存中暂存前一列的激活值；2. 算法优化：引入紧凑通道传播策略，替代每个通道的单独矩阵，减少参数，并与Transformer注意力中的亲和力图自然对齐。

Result: GSPN-2在图像分类和文本到图像合成任务中表现出色，能够匹配Transformer级别的准确性，同时显著降低计算成本。实验证明其有效建立了视觉应用中全局空间上下文建模的新效率边界。

Conclusion: GSPN-2通过独特的结构化矩阵变换和GPU优化实现相结合，为视觉应用中的全局空间上下文建模建立了新的效率前沿，解决了高分辨率图像和长视频相关实际应用中的效率瓶颈问题。
...
### [107] [Towards symbolic regression for interpretable clinical decision scores](https://arxiv.org/abs/2512.07961)
*Guilherme Seidyo Imai Aldeia,Joseph D. Romano,Fabricio Olivetti de Franca,Daniel S. Herman,William G. La Cava*

Main category: cs.LG

TL;DR: Brush是一种符号回归算法，将决策树分割算法与非线性常数优化相结合，能够生成包含规则逻辑的符号回归模型，在临床风险评分开发中表现出色。


<details>
  <summary>Details</summary>
Motivation: 医疗决策经常使用结合风险方程和规则的算法，但传统符号回归难以建模这种决策过程。然而，符号回归具有数据驱动和可解释性的优势，有望用于开发临床风险评分系统。

Method: 提出Brush算法，将类似决策树的分割算法与非线性常数优化相结合，实现规则逻辑与符号回归/分类模型的无缝集成。

Result: Brush在SRBench上达到帕累托最优性能，成功复现了两个广泛使用的临床评分系统，获得高准确性和可解释模型。相比决策树、随机森林和其他符号回归方法，Brush具有相当或更优的预测性能，同时生成更简单的模型。
...
### [108] [CIP-Net: Continual Interpretable Prototype-based Network](https://arxiv.org/abs/2512.07981)
*Federico Di Valerio,Michela Proietti,Alessio Ragno,Roberto Capobianco*

Main category: cs.LG

TL;DR: CIP-Net：一种无需示例的自解释原型模型，用于持续学习，避免灾难性遗忘，同时保持低内存开销和可解释性。


<details>
  <summary>Details</summary>
Motivation: 持续学习面临灾难性遗忘的挑战，现有可解释方法多使用事后解释或需要额外内存存储任务示例，限制了可扩展性。需要一种无需存储过去示例、架构简单但仍能提供有用解释和强大性能的自解释模型。

Method: 提出CIP-Net（Continual Interpretable Prototype Network），一种基于原型的自解释模型。该模型无需存储过去任务示例，通过原型机制实现知识保留，在预测时生成解释，保持简单架构同时降低内存开销。

Result: CIP-Net在任务增量学习和类别增量学习设置中，相比现有的无需示例和自解释方法，达到了最先进的性能水平，同时显著降低了内存相关开销。

Conclusion: CIP-Net为持续学习提供了一个实用且可解释的解决方案，通过自解释原型模型有效缓解灾难性遗忘问题，无需存储过去示例，具有良好的可扩展性和实际应用价值。
...
TL;DR: 开发了一个面向医疗健康领域的时间序列预测Web平台，降低技术门槛，让研究人员和临床医生能够轻松分析数据、训练模型并解释结果


<details>
  <summary>Details</summary>
Motivation: 时间序列预测在医疗健康领域有广泛应用，但技术专业知识要求高，成为使用这些技术的障碍。需要让研究人员和临床医生能够轻松访问和使用预测分析工具。

Method: 开发了一个Web平台，支持数据上传和可视化分析，提供多种可高度定制的预测模型和训练技术，并集成大型语言模型提供参数推荐和结果解释功能。

Result: 创建了一个功能完整的平台，用户可以通过简单操作完成数据可视化、模型训练和结果解释，平台支持多种预测模型，并能通过LLM提供智能推荐和解释。

Conclusion: 该平台成功降低了时间序列预测的技术门槛，使医疗研究人员和临床医生能够更便捷地进行预测分析，未来计划将其整合到学习型医疗系统中，实现临床管道的持续数据收集和推断。
...
TL;DR: 本文在重症监护场景中，比较了三种离线多目标强化学习算法与三种单目标基线方法，发现PEDA DT算法在灵活性方面表现最优，为个性化医疗决策提供了新框架。


<details>
  <summary>Details</summary>
Motivation: 重症监护环境中，临床医生面临生存率最大化和资源利用最小化的冲突目标。传统单目标强化学习方法使用固定标量化奖励函数，导致策略僵化，无法适应变化的临床优先级。多目标强化学习能够学习帕累托前沿上的最优策略集，支持测试时动态偏好选择，但医疗领域需要严格的离线学习。

Method: 在MIMIC-IV数据集上，对三种离线MORL算法（Conditioned Conservative Pareto Q-Learning、Adaptive CPQL和修改版PEDA Decision Transformer）与三种标量化单目标基线方法（BC、CQL、DDQN）进行基准测试。使用离线策略评估指标进行性能比较。

Result: PEDA DT算法相比静态标量化基线展现出更优的灵活性。结果扩展了先前关于医疗领域单目标决策变换器的发现，确认序列建模架构在多目标条件生成中仍保持稳健有效。

Conclusion: 离线多目标强化学习是一个有前景的框架，能够在无需重新训练的情况下，实现重症监护中个性化、可调整的决策制定。PEDA DT算法为临床优先级的动态适应提供了有效解决方案。
...
Method: CLARITY是一个医学世界模型，直接在结构化潜在空间中预测疾病演化。它明确整合时间间隔（时间背景）和患者特定数据（临床背景），将治疗条件进展建模为平滑、可解释的轨迹，从而生成生理上可信的个体化治疗计划。还引入了新颖的预测到决策框架，将潜在推演转化为透明、可操作的推荐。

Result: 在MU-Glioma-Post数据集上，CLARITY的表现优于最近的MeWM模型12%，并显著超越所有其他医学专用大型语言模型，在治疗规划方面展示了最先进的性能。

Conclusion: CLARITY通过整合时间和临床背景，在结构化潜在空间中预测疾病演化，提供了个体化、生理上可信的治疗计划，并通过预测到决策框架实现了透明、可操作的治疗推荐。
...
TL;DR: LUNA是一种可学习的核化线性注意力机制，在保持线性计算成本的同时，匹配并超越了二次注意力的准确性，解决了传统线性注意力依赖固定随机特征映射导致的精度-效率权衡问题。


<details>
  <summary>Details</summary>
Motivation: 传统softmax注意力存在O(n²)的二次计算成本，限制了其在长序列领域的应用。现有的线性注意力机制虽然将成本降低到O(n)，但依赖固定的随机特征映射（如随机傅里叶特征），这种静态、数据无关的核函数迫使实践者在模型精度和计算效率之间做出权衡。

Method: LUNA的核心思想是学习核特征映射而非固定先验。通过参数化核函数，LUNA学习针对特定数据和任务的特征基，克服了固定特征方法的表达能力限制。该方法实现了一个可学习的特征映射，该映射诱导出正定核并允许流式形式，从而在序列长度上实现线性的时间和内存扩展。

Result: 在Long Range Arena（LRA）基准测试中，LUNA在计算对等条件下（相同参数数量、训练步数和近似FLOPs）实现了高效Transformer中的最先进平均准确率。LUNA在事后转换方面也表现出色：替换微调后的BERT和ViT-B/16检查点中的softmax并进行简短微调，可以恢复大部分原始性能，显著优于固定线性化方法。

Conclusion: LUNA通过可学习的核特征映射成功消除了线性注意力机制中精度与效率的权衡，在保持线性计算复杂度的同时实现了与二次注意力相当甚至更优的性能，为长序列处理提供了有效的解决方案。
...
TL;DR: 提出了一种可解释的深度竞争风险模型DKAJ，它通过自动学习的核函数将数据点表示为聚类的加权组合，在保持预测性能的同时提供可视化解释。


<details>
  <summary>Details</summary>
Motivation: 传统的Aalen-Johansen估计器虽然是非参数方法，但在处理复杂数据时缺乏解释性。需要一种既能保持预测性能又能提供模型解释的竞争风险分析方法。

Method: 提出Deep Kernel Aalen-Johansen (DKAJ)估计器，通过自动学习的核函数衡量数据点之间的相似性，将每个数据点表示为聚类的加权组合。如果数据点仅对一个聚类有非零权重，则其预测的累积发生率函数对应于该聚类的经典Aalen-Johansen估计。

Result: 在四个标准竞争风险数据集上，DKAJ与最先进的基线方法竞争性能相当，同时能够提供可视化辅助模型解释。

Conclusion: DKAJ模型成功地将经典Aalen-Johansen估计器推广到深度学习框架中，在保持预测性能的同时提供了更好的模型可解释性，通过可视化工具帮助理解模型决策过程。
...
TL;DR: 本文提出了一种基于因果关系的多模态域泛化框架，通过对抗解缠和统一表示学习来解决社交媒体危机分类中的跨域泛化问题。


<details>
  <summary>Details</summary>
Motivation: 社交媒体危机分类旨在从多模态帖子中提取可操作的灾害相关信息，但现有方法在面对未见危机类型时泛化能力差。主要问题包括：1）未能解缠虚假特征和因果特征，导致域偏移下性能下降；2）未能对齐异构模态表示，阻碍了单模态域泛化技术向多模态设置的直接迁移。

Method: 提出因果关系引导的多模态域泛化框架，结合对抗解缠和统一表示学习。对抗目标鼓励模型解缠并关注域不变的因果特征；统一表示将不同模态特征对齐到共享潜在空间，使单模态域泛化策略能无缝扩展到多模态学习。

Result: 在不同数据集上的实验表明，该方法在未见灾害场景中取得了最佳性能。

Conclusion: 通过因果关系引导的多模态域泛化框架，有效解决了社交媒体危机分类中的跨域泛化挑战，提高了模型在未见危机类型上的表现。
...
### [115] [Training LLMs for Honesty via Confessions](https://arxiv.org/abs/2512.08093)
*Manas Joglekar,Jeremy Chen,Gabriel Wu,Jason Yosinski,Jasmine Wang,Boaz Barak,Amelia Glaese*

Main category: cs.LG

TL;DR: 提出了一种通过"忏悔"机制让大语言模型诚实报告自身缺陷的方法，在训练中为忏悔设置独立奖励，激励模型诚实揭露自身不当行为。


<details>
  <summary>Details</summary>
Motivation: 大语言模型在报告自身行为和信念时可能存在不诚实问题，比如夸大对事实主张的信心或掩盖不当行为。这种不诚实可能源于强化学习过程中的奖励塑造问题，训练过程可能无意中激励模型说谎或歪曲事实。

Method: 提出"忏悔"机制：在模型给出原始答案后，要求其提供忏悔输出，完整说明其遵守政策和指令的情况。训练中为忏悔设置独立奖励，仅基于其诚实性，不影响主答案的奖励。通过训练GPT-5-Thinking模型来验证该方法。

Result: 当模型在主答案中说谎或遗漏缺陷时，它经常能诚实忏悔这些行为，且忏悔的诚实性随训练适度提高。忏悔机制支持多种推理时干预，包括监控、拒绝采样和向用户展示问题。

Conclusion: 通过为忏悔设置独立奖励的方法，可以有效激励大语言模型诚实报告自身缺陷，特别是在严重不当行为情况下。该方法为模型监控和透明度提供了可行途径。
...
### [116] [Scalable Offline Model-Based RL with Action Chunks](https://arxiv.org/abs/2512.08108)
*Kwanyoung Park,Seohong Park,Youngwoon Lee,Sergey Levine*

Main category: cs.LG

TL;DR: MAC是一种基于模型的离线强化学习方法，通过动作块模型减少长期预测误差，并采用拒绝采样防止模型利用，在复杂长时域任务上表现优异


<details>
  <summary>Details</summary>
Motivation: 研究基于模型的强化学习（特别是基于模型的价值扩展）能否为离线RL中的复杂长时域任务提供可扩展的解决方案。传统方法中，更大的n值可以减少价值引导的偏差，但会放大长期累积的模型误差，影响未来预测

Method: 提出MAC方法：1）使用动作块模型，预测未来状态时基于动作序列而非单个动作，减少复合误差；2）采用拒绝采样从表达性强的行为动作块策略中采样，防止模型利用分布外动作

Result: 在包含高达1亿个转移的大规模数据集上的高度挑战性任务实验中，MAC在基于模型的离线RL算法中表现最佳，特别是在挑战性的长时域任务上

Conclusion: MAC通过动作块模型和拒绝采样机制，有效解决了基于模型价值扩展中的偏差-误差权衡问题，为复杂长时域离线RL任务提供了有效的解决方案
...
### [117] [Balanced Accuracy: The Right Metric for Evaluating LLM Judges - Explained through Youden's J statistic](https://arxiv.org/abs/2512.08121)
*Stephane Collot,Colin Fraser,Justin Zhao,William F. Shen,Timon Willi,Ilias Leontiadis*

Main category: cs.LG

TL;DR: 论文提出使用Youden's J统计量和平衡准确率来选择评估大语言模型的最佳分类器，避免传统指标在类别不平衡时的偏差问题。


<details>
  <summary>Details</summary>
Motivation: 当前评估大语言模型时，通常使用准确率、精确率、F1等指标来选择分类器（如LLM-as-a-judge或人工标注者），但这些指标对类别不平衡和正类选择敏感，可能导致选择出扭曲流行率估计的分类器。

Method: 提出使用Youden's J统计量作为选择最佳分类器的理论依据，证明平衡准确率是J的线性变换，通过理论分析和实证模拟验证该方法在分类器选择中的优越性。

Result: 研究表明Youden's J统计量在理论上与选择最佳模型比较分类器一致，平衡准确率作为等效指标能带来更好、更鲁棒的分类器选择，避免传统指标的偏差。

Conclusion: 在评估大语言模型时，应使用Youden's J统计量或平衡准确率来选择分类器，这能提供更可靠、更稳健的流行率估计，从而提高模型评估的可信度。
...
### [118] [Improving the Sensitivity of Backdoor Detectors via Class Subspace Orthogonalization](https://arxiv.org/abs/2512.08129)
*Guangmingmei Yang,David J. Miller,George Kesidis*

Main category: cs.LG

TL;DR: 提出CSO方法解决后门检测中传统方法失效的两个问题：1）某些非目标类天然具有极端检测统计量；2）后门特征过于微弱。通过抑制内在特征、保留后门特征来增强检测敏感性。


<details>
  <summary>Details</summary>
Motivation: 传统后门检测方法依赖目标类具有极端检测统计量，但在两种情况下会失效：1）某些非目标类天然容易区分，会获得极端统计量；2）后门特征过于微弱。需要更敏感的检测方法。

Method: 提出类子空间正交化（CSO）方法：利用少量干净样本，通过约束优化问题，在优化检测统计量的同时与类的内在特征正交化，从而抑制内在特征、保留后门特征贡献。

Result: CSO方法能够有效应对具有挑战性的混合标签攻击和自适应攻击，相比传统方法具有更高的检测敏感性。

Conclusion: CSO方法通过抑制内在特征、保留后门特征，解决了传统后门检测方法在特定情况下的失效问题，为后门检测提供了更敏感的解决方案。
...
### [119] [Biothreat Benchmark Generation Framework for Evaluating Frontier AI Models I: The Task-Query Architecture](https://arxiv.org/abs/2512.08130)
...
TL;DR: 论文提出了一种生物威胁基准生成框架，专门用于评估AI模型（特别是大语言模型）在生物安全风险方面的潜在危害，重点关注细菌生物威胁。


<details>
  <summary>Details</summary>
Motivation: 随着前沿AI模型特别是大语言模型的快速发展，需要量化这些模型可能助长生物恐怖主义或生物武器获取的风险。当前缺乏能够全面评估模型生物安全风险的基准测试方法。

Method: 开发了生物威胁基准生成框架，采用分层结构的生物威胁类别、要素和任务，构建了细菌生物威胁模式作为任务-查询架构的基础。该方法考虑了不同行为者能力水平和操作风险因素。

Result: 提出了细菌生物威胁模式作为BBG框架的第一个组成部分，为后续将查询转化为模型提示和模型评估奠定了基础。该框架旨在提供可重复使用的结构来评估LLM带来的细菌生物风险。

Conclusion: BBG框架及其细菌生物威胁模式为评估AI模型的生物安全风险提升和潜在危害提供了稳健、可重复的结构，能够全面捕捉生物对手的技术和操作要求，并考虑广泛的能力谱系。
...
### [120] [Robust Agents in Open-Ended Worlds](https://arxiv.org/abs/2512.08139)
*Mikayel Samvelyan*

Main category: cs.LG

TL;DR: 该论文通过开放性和多智能体学习方法，训练能够泛化到新环境、分布外输入和与其他智能体交互的鲁棒AI智能体，涵盖从强化学习到大型语言模型的多领域鲁棒性研究。
...
Method: 1. 开发MiniHack沙盒框架，通过程序化内容生成创建多样化环境；2. 提出Maestro方法，生成对抗性课程以增强RL智能体在双人零和游戏中的鲁棒性；3. 使用质量多样性方法在足球游戏领域系统识别预训练RL策略的漏洞；4. 使用进化搜索生成多样化对抗提示来诊断和增强LLM的鲁棒性。

Result: 该研究为AI鲁棒性提供了多方面的解决方案：MiniHack框架支持创建多样化RL任务；Maestro方法能渐进增强智能体鲁棒性；质量多样性方法能有效识别多智能体策略漏洞；进化搜索能生成有效对抗LLM的输入。

Conclusion: 这项工作为AI鲁棒性的未来发展铺平了道路，使智能体不仅能适应不断发展的世界，还能在面对不可预见的挑战和交互时茁壮成长，为开发更强大、适应性更强的AI系统提供了方法论基础。
...
### [121] [PolyLingua: Margin-based Inter-class Transformer for Robust Cross-domain Language Detection](https://arxiv.org/abs/2512.08143)
...
<details>
  <summary>Details</summary>
Motivation: 语言识别是多语言系统的关键第一步，现有工具在音乐请求等关键场景中表现不佳，开源工具准确率低，大语言模型成本过高

Method: 采用两层对比学习框架，结合实例级分离和类级对齐，使用自适应边距，生成紧凑且分离良好的嵌入表示

Result: 在Amazon Massive数据集上达到99.25% F1，在Song数据集上达到98.15% F1，超越Sonnet 3.5，参数量减少10倍

Conclusion: PolyLingua在保持高准确率的同时大幅降低计算成本，特别适合计算和延迟受限的环境
...
Method: 将去噪过程重构为搜索树，从共享的初始噪声样本出发，策略性地分支生成多个候选轨迹，同时高效重用它们的共同前缀。通过树结构实现多子分支，每前向传递可进行多次策略更新。

Result: 在扩散模型和流模型上的实验表明，TreeGRPO实现了2.4倍更快的训练速度，在效率-奖励权衡空间中建立了更优的帕累托前沿，在多个基准测试和奖励模型上一致优于GRPO基线。
...
### [123] [MobileFineTuner: A Unified End-to-End Framework for Fine-Tuning LLMs on Mobile Phones](https://arxiv.org/abs/2512.08211)
*Jiaxiang Geng,Lunyu Zhao,Yiyi Lu,Bing Luo*

Main category: cs.LG

TL;DR: MobileFineTuner是一个开源框架，支持在普通手机上直接进行端到端的大语言模型微调，解决了隐私保护和数据稀缺问题。


<details>
  <summary>Details</summary>
Motivation: 随着高质量公共数据接近枯竭，移动设备上的用户数据成为重要资源。现有方法多为模拟或基于IoT设备/PC，缺乏针对普通手机的实用框架，且需要保护用户隐私。

Method: 提出MobileFineTuner统一开源框架，支持全参数微调和参数高效微调。引入系统级优化：参数分片、梯度累积和能量感知计算调度，以应对手机内存和能耗限制。

Result: 在真实手机上成功微调了GPT-2、Gemma 3和Qwen 2.5模型。大量实验和消融研究验证了优化策略的有效性，证明MobileFineTuner是可行的设备端LLM训练基础。

Conclusion: MobileFineTuner填补了在普通手机上进行实际LLM微调的空白，为未来设备端LLM训练研究提供了可行的基础框架，平衡了效率、可扩展性和可用性。
...
### [124] [Correction of Decoupled Weight Decay](https://arxiv.org/abs/2512.08217)
*Jason Chuan-Chih Chou*

Main category: cs.LG

TL;DR: 本文挑战了AdamW中权重衰减与学习率γ成正比的传统设定，通过理论推导和实验证明权重衰减应与γ²成正比，这能带来更稳定的权重和梯度范数，从而更好地控制训练动态并提升模型性能。


<details>
  <summary>Details</summary>
Motivation: 传统AdamW优化器中，解耦权重衰减被设定为与学习率γ成正比，但这一假设缺乏理论依据。最近有研究者基于稳态正交性论证提出权重衰减应与γ²成正比，但作者发现这种正交性论证对训练动态影响很小。本文旨在从更基础的理论角度重新审视权重衰减的合理设定。

Method: 作者首先分析了传统正交性论证的局限性，发现消除更新垂直分量对权重范数的贡献对训练动态影响很小。然后基于"更新在稳态时与权重无关"的简单假设，推导出解耦权重衰减应与γ²成正比的理论结果。同时推导并实证验证了Scion优化器中每个小批量的总更新贡献（TUC）更好地由动量相关的有效学习率表征。

Result: 理论推导和实验表明，解耦权重衰减∝γ²能带来稳定的权重和梯度范数，使训练动态更可控。这种设定下的最优值具有可迁移性，能够改善模型性能。与传统的∝γ设定相比，新设定在理论基础上更坚实，在实践中表现更好。

Conclusion: 解耦权重衰减应与学习率的平方γ²成正比，而不是传统认为的与γ成正比。这种设定基于更合理的理论假设，能实现稳定的权重和梯度范数，更好地控制训练动态，最终提升模型性能。这一发现对优化器设计具有重要指导意义。
...
### [125] [SPROCKET: Extending ROCKET to Distance-Based Time-Series Transformations With Prototypes](https://arxiv.org/abs/2512.08246)
*Nicholas Harner*
...
Method: 提出了SPROCKET（Selected Prototype Random Convolutional Kernel Transform）方法，这是一种基于原型的特征工程策略。该方法通过选择原型来实现随机卷积核变换，并构建了MR-HY-SP（MultiROCKET-HYDRA-SPROCKET）集成模型。
...
### [126] [Geometric-Stochastic Multimodal Deep Learning for Predictive Modeling of SUDEP and Stroke Vulnerability](https://arxiv.org/abs/2512.08257)
...
Result: 在MULTI-CLARID数据集上实验显示预测准确性提高，从流形曲率、分数记忆指数、注意力熵和扩散中心性中提取可解释的生物标志物
...
### [127] [Jacobian Aligned Random Forests](https://arxiv.org/abs/2512.08306)
*Sarwesh Rauniyar*

Main category: cs.LG

TL;DR: JARF提出了一种简单有效的监督预处理器方法，通过计算随机森林预测的梯度，构建全局线性变换来旋转特征空间，使轴对齐决策树能够处理旋转或交互依赖的决策边界，同时保持训练简单性和鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 轴对齐决策树虽然快速稳定，但在处理具有旋转或特征交互依赖的决策边界时效果不佳，因为需要特征线性组合而非单特征阈值。倾斜森林虽然能解决这个问题，但计算成本高且实现复杂。需要一种既能处理复杂决策边界又能保持简单性的方法。

Method: JARF（Jacobian-Aligned Random Forests）方法：1）先训练轴对齐森林估计类别概率或回归输出；2）计算预测相对于每个特征的有限差分梯度；3）聚合梯度为期望雅可比外积（扩展EGOP）；4）使用该监督预处理器作为全局线性变换旋转特征空间；5）将变换后的数据输入标准轴对齐森林。该方法适用于任何提供梯度的模型。

Result: 在表格分类和回归基准测试中，这种预处理器方法持续改进了轴对齐森林的性能，通常匹配或超越倾斜基线方法，同时提高了训练时间。实验结果表明监督预处理器能够恢复倾斜森林的大部分准确性，同时保持轴对齐树的简单性和鲁棒性。

Conclusion: JARF通过监督预处理器提供了一种简单有效的解决方案，使轴对齐决策树能够处理复杂的决策边界，在保持训练简单性和鲁棒性的同时，获得接近倾斜森林的性能。该方法为处理表格数据中的特征交互和旋转边界问题提供了实用工具。
...
### [128] [Minimizing Layerwise Activation Norm Improves Generalization in Federated Learning](https://arxiv.org/abs/2512.08314)
*M Yashwanth,Gaurav Kumar Nayak,Harsh Rangwani,Arya Singh,R. Venkatesh Babu,Anirban Chakraborty*

Main category: cs.LG

TL;DR: 提出了一种名为MAN的联邦学习正则化技术，通过最小化客户端模型各层激活范数来约束优化问题的平坦性，从而提高联邦学习模型的泛化性能。


<details>
  <summary>Details</summary>
Motivation: 联邦学习框架中，聚合的全局模型可能收敛到"尖锐最小值"，这会损害模型的泛化能力。因此需要改进联邦学习模型的泛化性能。

Method: 引入平坦性约束的联邦学习优化问题，通过最小化客户端模型各层激活范数（MAN技术）来降低Hessian矩阵的最大特征值，确保收敛到平坦最小值。

Result: 将提出的平坦性约束优化应用于现有联邦学习技术，获得了显著改进，建立了新的最先进性能。

Conclusion: MAN正则化技术通过约束优化问题的平坦性，有效提高了联邦学习模型的泛化能力，为联邦学习提供了新的改进方向。
...
### [129] [A Multivariate Bernoulli-Based Sampling Method for Multi-Label Data with Application to Meta-Research](https://arxiv.org/abs/2512.08371)
*Simon Chung,Colby J. Vorland,Donna L. Maney,Andrew W. Brown*

Main category: cs.LG

TL;DR: 提出一种考虑标签依赖关系的多标签数据采样算法，通过估计多元伯努利分布参数和计算标签组合权重，在保持目标分布特性的同时增强少数类别的代表性。


<details>
  <summary>Details</summary>
Motivation: 多标签数据集中标签通常不互斥且频率差异很大，传统采样方法难以在保持已知分布特性的同时获得足够稀缺标签的样本进行推断。
...
Conclusion: 该方法能够生成更平衡的多标签数据子样本，增强少数类别的代表性，同时考虑标签依赖关系，为多标签数据采样提供了有效解决方案。
...
### [130] [Biothreat Benchmark Generation Framework for Evaluating Frontier AI Models III: Implementing the Bacterial Biothreat Benchmark (B3) Dataset](https://arxiv.org/abs/2512.08459)
...
TL;DR: B3数据集为评估大型语言模型生物安全风险提供了可行的基准测试方法，通过实际测试和人工评估识别风险来源并指导缓解措施


<details>
  <summary>Details</summary>
Motivation: 前沿AI模型（特别是大语言模型）可能被用于生物恐怖主义或获取生物武器，引起了政策、学术和公众的广泛关注。需要量化并缓解这种风险，开发能够评估特定模型生物安全风险的基准测试方法

Method: 实施B3（细菌生物威胁基准）数据集试点，通过样本前沿AI模型运行基准测试，人工评估模型响应，并从多个维度进行应用风险分析

Result: 试点表明B3数据集提供了一种可行且细致的方法，能够快速评估LLM的生物安全风险，识别风险的关键来源，并为优先缓解领域提供指导

Conclusion: B3基准测试框架为评估和缓解AI模型在生物安全领域的风险提供了有效工具，有助于模型开发者和政策制定者量化和管理相关风险
...
### [131] [Transformers for Multimodal Brain State Decoding: Integrating Functional Magnetic Resonance Imaging Data and Medical Metadata](https://arxiv.org/abs/2512.08462)
*Danial Jafarzadeh Jazi,Maryam Hajiesmaeili*

Main category: cs.LG

TL;DR: 提出了一种结合fMRI数据和DICOM元数据的多模态Transformer框架，用于解码脑状态，提高准确性、可解释性和鲁棒性。
...
Result: 提出的方法提高了模型准确性、可解释性和鲁棒性，在临床诊断、认知神经科学和个性化医学中具有应用潜力。

Conclusion: 该框架有效利用了DICOM元数据的上下文信息，但面临元数据变异性和计算需求等挑战，未来需要优化可扩展性和泛化能力。
...
### [132] [Optimal Perturbation Budget Allocation for Data Poisoning in Offline Reinforcement Learning](https://arxiv.org/abs/2512.08485)
*Junnan Qiu,Jie Li*

Main category: cs.LG

TL;DR: 提出一种针对离线强化学习的全局预算分配攻击策略，通过TD误差敏感性分配扰动预算，相比均匀扰动更高效且隐蔽
...
Method: 基于TD误差影响价值函数收敛的理论洞察，将攻击建模为全局资源分配问题，推导出在L2约束下扰动幅度与TD误差敏感性成正比的闭式解

Result: 在D4RL基准测试中显著优于基线策略，仅用最小扰动即可实现高达80%的性能下降，并能逃避最先进的统计和谱防御检测
...
### [133] [Long-Sequence LSTM Modeling for NBA Game Outcome Prediction Using a Novel Multi-Season Dataset](https://arxiv.org/abs/2512.08591)
*Charles Rios,Longzhen Han,Almas Baimagambetov,Nikolaos Polatidis*

Main category: cs.LG

TL;DR: 该研究构建了一个覆盖2004-05至2024-25赛季的纵向NBA数据集，并提出使用LSTM模型通过长达8个赛季（9840场比赛）的序列长度来预测NBA比赛结果，相比传统机器学习方法取得了最佳性能。
...
Method: 构建了覆盖20个赛季的纵向NBA数据集，提出基于LSTM的深度学习框架，使用长达8个赛季（9840场比赛）的序列长度来捕捉球队动态演变和赛季间依赖关系，并与逻辑回归、随机森林、MLP和CNN等基线模型进行比较。

Result: LSTM模型在所有指标上表现最佳：准确率72.35%，精确率73.15%，AUC-ROC 76.13%，显著优于其他传统机器学习和深度学习基线模型。

Conclusion: 长序列时间建模对篮球结果预测至关重要，新构建的多赛季数据集有助于开发稳健、可泛化的NBA预测系统，LSTM架构能有效捕捉球队长期表现趋势和赛季间依赖关系。
...
### [134] [DS FedProxGrad: Asymptotic Stationarity Without Noise Floor in Fair Federated Learning](https://arxiv.org/abs/2512.08671)
*Huzaifa Arif*

Main category: cs.LG

TL;DR: 本文改进了FedProxGrad算法的收敛性分析，提出了DS FedProxGrad框架，在Robbins-Monro步长调度下证明了算法能达到渐近平稳点，消除了方差引起的噪声底限依赖。


<details>
  <summary>Details</summary>
Motivation: 现有FedProxGrad算法在非凸复合优化问题的群公平联邦学习中，其收敛分析仅能证明收敛到噪声主导的平稳邻域，且显式依赖于方差引起的噪声底限。本文旨在改进这一收敛性分析。

Method: 提出了DS FedProxGrad（衰减步长FedProxGrad）分析框架，该框架包含不精确的局部近端解和显式公平正则化。在Robbins-Monro步长调度和局部不精确性的温和衰减条件下进行分析。

Result: 证明了liminf_{r→∞} E[‖∇F(x^r)‖²] = 0，即算法是渐近平稳的，且收敛速率不依赖于方差引起的噪声底限。

Conclusion: DS FedProxGrad框架在适当的步长调度和局部不精确性衰减条件下，能够实现渐近平稳收敛，克服了原始FedProxGrad算法中噪声底限依赖的问题。
...
### [135] [An Additive Manufacturing Part Qualification Framework: Transferring Knowledge of Stress-strain Behaviors from Additively Manufactured Polymers to Metals](https://arxiv.org/abs/2512.08699)
*Chenglong Duan,Dazhong Wu*

Main category: cs.LG

TL;DR: 开发了一个基于动态时间规整和迁移学习的框架，用于增材制造零件认证，通过将低成本聚合物的应力-应变行为知识迁移到金属材料上，显著提高了预测精度。


<details>
  <summary>Details</summary>
Motivation: 增材制造零件认证对于确保关键应用中零件的可靠性和一致性至关重要。准确预测增材制造零件的复杂应力-应变行为是实现有效认证的关键，但直接对金属材料进行建模成本高昂且困难。

Method: 提出了DTW-TL（动态时间规整-迁移学习）框架：1）使用DTW算法选择与目标金属数据集最相关的聚合物数据集作为源域；2）采用LSTM模型进行知识迁移；3）使用四种聚合物（尼龙、PLA、CF-ABS、树脂）作为源域，三种金属（AlSi10Mg、Ti6Al4V、碳钢）作为目标域进行验证。

Result: DTW-TL框架成功识别了聚合物与金属之间的最佳匹配，选择单一聚合物数据集作为源域。该模型在三个金属目标域上取得了12.41%的最低平均绝对百分比误差和0.96的最高决定系数，优于无迁移学习的LSTM模型以及使用四个聚合物数据集预训练的迁移学习模型。

Conclusion: DTW-TL框架为增材制造零件认证提供了一种有效的解决方案，通过利用低成本聚合物的知识来预测金属材料的力学行为，显著提高了预测精度，为增材制造零件的可靠认证提供了技术支持。
...
### [136] [Exposing Hidden Biases in Text-to-Image Models via Automated Prompt Search](https://arxiv.org/abs/2512.08724)
*Manos Plitsis,Giorgos Bouritsas,Vassilis Katsouros,Yannis Panagakis*

Main category: cs.LG

TL;DR: 本文提出了一种自动生成能最大化图像偏见的提示词框架BGPS，用于发现文本到图像扩散模型中未被察觉的微妙偏见。


<details>
  <summary>Details</summary>
Motivation: 现有的文本到图像扩散模型存在社会偏见问题，但现有去偏见方法依赖人工或大语言模型构建的提示词数据集，成本高且可能遗漏未预期的偏见触发提示。

Method: 提出Bias-Guided Prompt Search (BGPS)框架，包含两个组件：1) 生成属性中性提示词的大语言模型；2) 基于TTI内部表示的属性分类器，引导LLM解码过程向放大目标图像属性的提示空间区域发展。

Result: 在Stable Diffusion 1.5和最新去偏见模型上进行实验，发现了大量微妙且先前未记录的偏见，严重降低了公平性指标。发现的提示词具有可解释性，相比硬提示优化方法在困惑度指标上有量化改进。
...
### [137] [Neural Ordinary Differential Equations for Simulating Metabolic Pathway Dynamics from Time-Series Multiomics Data](https://arxiv.org/abs/2512.08732)
...
Method: 引入神经常微分方程(NODEs)作为动态框架，学习蛋白质组与代谢组之间的复杂相互作用。将该框架应用于工程大肠杆菌菌株的时间序列数据，模拟代谢途径的连续动态。

Result: NODE架构在捕捉系统动态方面表现优于传统机器学习流程。在柠檬烯途径数据集上RMSE改善高达94.38%，在异戊烯醇途径数据集上改善高达97.65%。此外，NODE模型的推理时间加速了1000倍。

Conclusion: NODE模型被确立为可扩展、高保真的工具，适用于下一代代谢工程和生物发现，为复杂生物系统的预测建模提供了有效解决方案。
...
### [138] [Learning and Editing Universal Graph Prompt Tuning via Reinforcement Learning](https://arxiv.org/abs/2512.08763)
...
<details>
  <summary>Details</summary>
Motivation: 现有选择性节点图提示调优方法会损害通用图提示调优的理论基础，需要在保持理论基础的前提下实现更优的提示效果。

Method: 提出LEAP模型：1）构建基本通用图提示以保持理论基础；2）使用actor-critic强化学习选择节点并编辑提示。

Result: 在多种预训练策略下的图和节点级任务中，LEAP在全样本和少样本场景下均优于微调和其他基于提示的方法。

Conclusion: LEAP在保持通用图提示调优理论基础的同时实现了更优性能，为图提示调优提供了新的有效范式。
...
### [139] [Can TabPFN Compete with GNNs for Node Classification via Graph Tabularization?](https://arxiv.org/abs/2512.08798)
*Jeongwhan Choi,Woosung Kang,Minseo Kim,Jongwoo Kim,Noseong Park*

Main category: cs.LG

TL;DR: TabPFN-GN将图节点分类问题转化为表格学习问题，通过提取节点特征、结构属性、位置编码等将图数据转换为表格格式，使TabPFN模型能够在无需图特定训练的情况下实现竞争性节点分类性能。


<details>
  <summary>Details</summary>
Motivation: 基于TabPFN在表格数据和时序数据上的成功，研究是否可以将图节点分类问题有效地重新表述为表格学习问题，探索表格学习模型在图领域的应用潜力。

Method: 提出TabPFN-GN方法，将图数据转换为表格特征：提取节点属性、结构属性、位置编码，以及可选的平滑邻域特征，然后使用TabPFN模型进行直接节点分类，无需图特定训练或语言模型依赖。

Result: 在12个基准数据集上的实验表明，TabPFN-GN在同质图上与GNNs表现相当，在异质图上始终优于GNNs，证明了特征工程可以弥合表格和图领域之间的差距。

Conclusion: 通过原则性的特征工程，可以将图节点分类转化为表格学习问题，为任务特定GNN训练和LLM依赖的图基础模型提供了实用的替代方案，展示了跨领域基础模型的泛化能力。
...
### [140] [Identifying counterfactual probabilities using bivariate distributions and uplift modeling](https://arxiv.org/abs/2512.08805)
...
<details>
  <summary>Details</summary>
Motivation: 虽然反事实识别能提供比提升建模更丰富的潜在结果联合分布信息，但估计难度更大。然而这两种方法是协同的：提升模型可以用于反事实估计。

Method: 提出一种反事实估计器，将双变量beta分布拟合到预测的提升分数上，从而产生反事实结果的后验分布。该方法不需要超出提升建模之外的因果假设。

Result: 模拟实验显示了该方法的有效性，可以应用于电信客户流失等问题，揭示标准机器学习或单独提升模型无法获得的洞察。

Conclusion: 通过利用提升模型进行反事实估计，该方法在保持与提升建模相同因果假设的同时，提供了更丰富的反事实分布信息，具有实际应用价值。
...
### [141] [Forecasting Fails: Unveiling Evasion Attacks in Weather Prediction Models](https://arxiv.org/abs/2512.08832)
*Huzaifa Arif,Pin-Yu Chen,Alex Gittens,James Diffenderfer,Bhavya Kailkhura*

Main category: cs.LG

TL;DR: WAAPO框架生成针对AI天气预报模型的对抗性扰动，通过通道稀疏性、空间定位和平滑性约束确保扰动有效且隐蔽，揭示了AI天气预测模型的安全漏洞。


<details>
  <summary>Details</summary>
Motivation: 随着AI模型在天气预报中的依赖增加，需要评估其对对抗性扰动的脆弱性，以保护操作预报系统免受恶意利用。
...
Conclusion: AI驱动的天气预报模型存在关键漏洞，需要建立强大的安全防护措施来防止对抗性攻击在操作预报系统中的利用。
...
### [142] [Reinforcement Learning From State and Temporal Differences](https://arxiv.org/abs/2512.08855)
*Lex Weaver,Jonathan Baxter*

Main category: cs.LG

TL;DR: 论文提出STD(λ)算法，改进传统TD(λ)在函数逼近中的策略优化问题，通过关注状态值的相对排序而非绝对值来避免收敛到次优策略。


<details>
  <summary>Details</summary>
Motivation: 传统TD(λ)算法使用函数逼近时，虽然能最小化状态值的平方误差，但对于策略优化来说，状态值的相对排序比绝对值更重要。作者发现TD(λ)在某些情况下会从最优策略收敛到次优策略，因此需要改进。

Method: 提出STD(λ)算法，在二元决策问题中训练函数逼近器时关注相对状态值而非绝对值。该方法基于相对状态值进行训练，并提供了理论分析，包括在两状态系统中单调策略改进的证明。

Result: 在两状态系统、三状态系统和西洋双陆棋中展示了TD(λ)会收敛到次优策略的问题。STD(λ)在两状态系统和acrobot问题的变体上取得了成功，并与Bertsekas的差分训练方法进行了比较。

Conclusion: STD(λ)通过关注状态值的相对排序而非绝对值，解决了TD(λ)在函数逼近中可能导致次优策略的问题，为强化学习中的策略优化提供了更有效的方法。
...
### [143] [Refining Diffusion Models for Motion Synthesis with an Acceleration Loss to Generate Realistic IMU Data](https://arxiv.org/abs/2512.08859)
*Lars Ole Häusler,Lena Uhlenberg,Göran Köber,Diyora Salimova,Oliver Amft*

Main category: cs.LG

TL;DR: 提出了一种文本到IMU运动合成框架，通过使用基于加速度的二阶损失微调预训练扩散模型，提高IMU数据的真实性和加速度信号保真度


<details>
  <summary>Details</summary>
Motivation: 现有的文本到运动生成模型缺乏对IMU传感器特定特性的考虑，特别是加速度信号的保真度不足，这限制了合成IMU数据在人类活动识别等下游任务中的应用效果

Method: 1. 提出基于加速度的二阶损失函数L_acc，强制生成运动在离散二阶时间差分上的一致性；2. 将L_acc集成到现有扩散模型的训练目标中；3. 通过表面建模和虚拟传感器模拟框架生成IMU数据；4. 在人类活动识别任务中评估合成数据质量

Result: 1. L_acc损失相对原始模型降低了12.7%；2. 高动态活动（跑步、跳跃）改进显著大于低动态活动（坐、站）；3. 合成IMU数据在低维嵌入中更接近真实IMU记录分布；4. 仅使用精炼合成IMU数据训练的人类活动识别性能比原始扩散模型提升8.7%，比最佳对比模型提升7.6%
...
### [144] [Fed-SE: Federated Self-Evolution for Privacy-Constrained Multi-Environment LLM Agents](https://arxiv.org/abs/2512.08870)
...
<details>
  <summary>Details</summary>
Motivation: LLM智能体在复杂交互任务中广泛部署，但隐私约束阻碍了集中式优化和跨动态环境的协同进化。虽然联邦学习在静态数据集上有效，但在开放式的智能体自进化方面仍未被充分探索。直接应用标准联邦学习存在挑战：异构任务和稀疏的轨迹级奖励会导致严重的梯度冲突，破坏全局优化过程。

Method: 提出Fed-SE框架，采用局部进化-全局聚合范式。局部层面：智能体在过滤后的高回报轨迹上使用参数高效微调，实现稳定的梯度更新。全局层面：在低秩子空间内聚合更新，解耦环境特定的动态，有效减少客户端间的负迁移。

Result: 在五个异构环境中的实验表明，Fed-SE相比联邦学习基线方法，平均任务成功率提升了约18%，验证了其在隐私约束部署中实现鲁棒跨环境知识迁移的有效性。

Conclusion: Fed-SE成功解决了LLM智能体在隐私约束下的联邦自进化问题，通过局部稳定优化和全局解耦聚合，有效处理了异构任务中的梯度冲突，实现了显著的性能提升。
...
### [145] [When Tables Leak: Attacking String Memorization in LLM-Based Tabular Data Generation](https://arxiv.org/abs/2512.08875)
*Joshua Ward,Bochao Gu,Chi-Hua Wang,Guang Cheng*

Main category: cs.LG

TL;DR: LLM生成表格数据存在隐私泄露风险，数字序列易被记忆并复现，提出LevAtt攻击方法检测隐私泄露，并提出防御策略


<details>
  <summary>Details</summary>
Motivation: LLM在生成高质量表格合成数据方面表现出色，但现有方法（微调小模型或提示大模型）存在隐私风险，可能复现训练数据中的数字模式，需要系统分析这种隐私泄露风险

Method: 提出LevAtt攻击方法（无盒成员推理攻击），仅访问生成的合成数据，针对数字序列进行成员推断；同时提出两种防御方法，包括在生成过程中策略性扰动数字的新采样策略

Result: LevAtt攻击在各种模型和数据集上暴露了显著的隐私泄露，在某些情况下甚至成为最先进模型的完美成员分类器；提出的防御方法能在最小化保真度和效用损失的情况下有效抵御这些攻击

Conclusion: LLM基于的合成数据生成存在独特的隐私漏洞，需要有效的防御措施；提出的数字扰动策略能有效保护隐私同时保持数据质量
...
### [146] [DAO-GP Drift Aware Online Non-Linear Regression Gaussian-Process](https://arxiv.org/abs/2512.08879)
*Mohammad Abu-Shaira,Ajita Rattani,Weishi Shi*

Main category: cs.LG

TL;DR: DAO-GP是一种针对概念漂移的在线高斯过程模型，具有漂移检测、自适应调整、无超参数、衰减机制和稀疏性等特点，在非平稳数据环境下表现出色。


<details>
  <summary>Details</summary>
Motivation: 现实世界数据集常呈现随时间变化的数据分布（概念漂移），忽视这一现象会显著降低模型预测精度。传统在线高斯过程方法存在多个关键限制：缺乏漂移感知、依赖固定超参数、易受数据窥探影响、缺乏原则性衰减机制以及内存效率低下。

Method: 提出DAO-GP（漂移感知在线高斯过程），这是一种完全自适应、无超参数、衰减且稀疏的非线性回归模型。DAO-GP内置漂移检测和适应机制，能根据漂移严重程度动态调整模型行为。
...
Conclusion: DAO-GP被确立为在线非线性回归的漂移弹性解决方案，通过内置的漂移检测和适应机制，解决了传统在线高斯过程方法的关键限制，为处理概念漂移问题提供了有效方法。
...
### [147] [Unsupervised Learning of Density Estimates with Topological Optimization](https://arxiv.org/abs/2512.08895)
*Suina Tanweer,Firas A. Khasawneh*

Main category: cs.LG

TL;DR: 提出一种基于拓扑数据分析的损失函数，用于无监督选择核密度估计的最优带宽，相比传统方法在不同维度上表现出潜力。


<details>
  <summary>Details</summary>
Motivation: 核密度估计在机器学习、贝叶斯推断等领域广泛应用，但带宽选择是关键超参数，直接影响偏差-方差权衡。传统方法需要人工调参，而拓扑数据分析能够量化高维数据的拓扑特征，为无监督带宽选择提供新思路。

Method: 提出基于拓扑的损失函数，利用拓扑数据分析方法（如连通分量、环、空洞等拓扑特征）构建无监督学习框架，自动选择最优带宽。

Result: 该方法在不同维度上与经典技术进行基准测试，显示出潜在优势，能够有效自动选择带宽而无需人工干预。

Conclusion: 基于拓扑的损失函数为核密度估计的带宽选择提供了一种有效的无监督方法，在不同维度数据上具有应用潜力。
...
### [148] [Open Polymer Challenge: Post-Competition Report](https://arxiv.org/abs/2512.08896)
...
TL;DR: Open Polymer Challenge发布了首个聚合物信息学社区基准数据集，包含10K聚合物和5种性质，通过多任务预测竞赛推动可持续聚合物材料发现。
...
Method: 发布包含10K聚合物和5种性质（热导率、回转半径、密度、自由体积分数、玻璃化转变温度）的基准数据集，组织多任务聚合物性质预测竞赛，参与者使用特征增强、迁移学习、自监督预训练和针对性集成策略等方法。

Result: 竞赛揭示了关于数据准备、分布偏移和跨组模拟一致性的重要经验，为未来大规模聚合物数据集的最佳实践提供了指导，创建的模型、分析和发布数据为聚合物科学中的分子AI建立了新基础。

Conclusion: Open Polymer Challenge通过发布基准数据集和竞赛，解决了聚合物信息学中的数据瓶颈问题，有望加速可持续和节能材料的发展，同时提供了数据生成管道支持更多性质的模拟。
...
### [149] [Impact of Data-Oriented and Object-Oriented Design on Performance and Cache Utilization with Artificial Intelligence Algorithms in Multi-Threaded CPUs](https://arxiv.org/abs/2512.07841)
...
TL;DR: 该研究对比了面向数据设计(DOD)与传统面向对象设计(OOD)在多线程环境下的性能表现，通过A*搜索算法的四种实现版本发现，在多线程测试中DOD表现出更快的执行时间和更低的缓存缺失率，但细粒度任务中单线程版本反而优于多线程版本。


<details>
  <summary>Details</summary>
Motivation: 随着多核CPU与主内存之间性能差距的扩大，需要硬件感知的软件设计范式。本研究旨在全面分析DOD与OOD在多线程环境中的缓存利用效率和性能差异。

Method: 开发并比较了A*搜索算法的四个版本：单线程OOD、单线程DOD、多线程OOD和多线程DOD。评估指标包括执行时间、内存使用和CPU缓存缺失率。

Result: 在多线程测试中，DOD实现表现出显著的性能优势，执行时间更快，系统调用和缓存缺失更少。对于A*这样的细粒度任务，线程管理开销导致单线程版本在两个范式中都显著优于多线程版本。

Conclusion: 即使在简单算法中性能差异看似微妙，但DOD在关键指标上的持续优势凸显了其架构优越性，表明在复杂、大规模AI和并行计算任务中，DOD是最大化硬件效率的更有效方法。
...
### [150] [SkipKV: Selective Skipping of KV Generation and Storage for Efficient Inference with Large Reasoning Models](https://arxiv.org/abs/2512.07993)
...
<details>
  <summary>Details</summary>
Motivation: 大型推理模型在推理过程中KV缓存随思维链线性增长，导致内存和吞吐量瓶颈。现有KV缓存驱逐方法在多批次设置下无法保持准确性，且会产生更长的序列。

Method: SkipKV采用句子级评分指标识别并删除高度相似的句子，同时通过动态调整引导向量更新隐藏激活状态来抑制冗余生成，实现训练自由的KV压缩。

Result: 在多个推理基准测试中，SkipKV在相似压缩预算下保持高达26.7%的准确性提升，相比现有方法减少1.6倍生成长度，提高1.7倍吞吐量。

Conclusion: SkipKV通过句子级KV缓存压缩有效解决了大型推理模型中的KV缓存开销问题，在保持准确性的同时显著提高了推理效率。
...
### [151] [Toward an AI Reasoning-Enabled System for Patient-Clinical Trial Matching](https://arxiv.org/abs/2512.08026)
...
TL;DR: 开发了一个基于大语言模型的AI辅助患者-临床试验匹配系统，通过结构化资格评估和可解释推理链支持人工审核，旨在减轻协调员负担并提高匹配效率。


<details>
  <summary>Details</summary>
Motivation: 临床试验患者筛选目前仍是一个手动、耗时且资源密集的过程，需要解决电子健康记录数据整合、专家审核支持和安全标准等关键实施挑战。

Method: 利用开源、具备推理能力的大语言模型构建安全可扩展的概念验证系统，超越二元分类生成结构化资格评估，提供可解释的推理链支持人工审核，将资格表示为动态状态而非固定判定。

Result: 系统能够识别可用匹配并提供可操作建议使患者未来符合资格，旨在减轻协调员负担，智能扩大为每位患者考虑的试验范围，并保证所有AI生成输出的全面可审计性。

Conclusion: 该系统代表了AI增强患者-临床试验匹配的创新方法，通过结合大语言模型的推理能力和人工审核，有望显著改善临床试验招募流程的效率和效果。
...
### [152] [Large Language Models for Education and Research: An Empirical and User Survey-based Analysis](https://arxiv.org/abs/2512.08057)
...
<details>
  <summary>Details</summary>
Motivation: 预训练大语言模型在教育研究领域展现出巨大潜力，ChatGPT和DeepSeek作为当前最先进的模型，在数学、科学、医学、文学和编程方面表现出色，需要系统评估它们在教育研究应用中的准确度、计算效率和用户体验。

Method: 采用综合评估方法：1) 背景技术分析；2) 实证实验（在文本生成、编程和专门问题解决任务上进行基准测试）；3) 真实世界用户调查（涵盖学生、教育者和研究者）。

Result: ChatGPT在通用语言理解和文本生成方面表现更优，DeepSeek因其效率导向设计在编程任务中表现更佳；两者都能提供医学准确的诊断输出并有效解决复杂数学问题；用户调查揭示了这些模型的实际益处和局限性。

Conclusion: ChatGPT和DeepSeek在教育研究领域各有优势，ChatGPT擅长语言理解和文本生成，DeepSeek在编程任务中更高效，两者都能胜任专业问题解决；用户调查提供了实际应用洞察，有助于理解这些模型如何推动教育和研究发展。
...
### [153] [Scalable Back-End for an AI-Based Diabetes Prediction Application](https://arxiv.org/abs/2512.08147)
*Henry Anand Septian Radityo,Bernardus Willson,Reynard Tanadi,Latifa Dwiyanti,Saiful Akbar*

Main category: cs.AI

TL;DR: 开发了一个用于移动糖尿病预测应用的可扩展后端系统，采用水平扩展、数据库分片和异步通信，83%的功能达到性能目标，支持10,000并发用户。
...
Result: 83%的系统功能（24个中的20个）达到性能目标，关键功能如用户管理、活动跟踪和预测操作表现良好，系统能处理10,000并发用户，异步通信有效降低了计算密集型预测请求的错误率。

Conclusion: 开发的可扩展后端系统成功满足糖尿病预测应用的性能要求，验证了架构设计的有效性，为大规模用户服务提供了可靠的技术基础。
...
### [154] [Beyond Traditional Diagnostics: Transforming Patient-Side Information into Predictive Insights with Knowledge Graphs and Prototypes](https://arxiv.org/abs/2512.08261)
*Yibowen Zhao,Yinan Zhang,Zhixiang Su,Lizhen Cui,Chunyan Miao*

Main category: cs.AI

TL;DR: 该论文提出了KPI框架，通过知识图谱增强、原型感知和可解释性技术，从患者基本信息预测疾病，解决现有方法的分布不平衡和可解释性不足问题。


<details>
  <summary>Details</summary>
Motivation: 仅从患者基本信息（人口统计学和自述症状）预测疾病具有重要价值，可提高患者意识、促进早期医疗参与并提升医疗系统效率。但现有方法面临疾病分布不平衡和缺乏可解释性等关键挑战，导致预测存在偏差或不可靠。

Method: 提出KPI框架：1）系统整合结构化可信医疗知识构建统一疾病知识图谱；2）构建临床有意义的疾病原型；3）采用对比学习提高预测准确性，特别是对长尾疾病；4）利用大语言模型生成患者特异性、医学相关的解释，提高可解释性和可靠性。

Result: 在真实世界数据集上的广泛实验表明，KPI在预测准确性上优于最先进方法，并提供与患者叙述高度一致的临床有效解释，突显其在以患者为中心的医疗交付中的实用价值。

Conclusion: KPI框架通过整合医疗知识图谱、疾病原型和对比学习，有效解决了疾病预测中的不平衡分布和可解释性问题，为患者为中心的医疗提供了可靠且可解释的预测工具。
...
### [155] [Reasoning Models Ace the CFA Exams](https://arxiv.org/abs/2512.08270)
...
<details>
  <summary>Details</summary>
Motivation: 先前研究表明大型语言模型在CFA考试中表现不佳，但最新推理模型在各类专业考试中取得突破，需要评估这些模型在CFA考试中的实际表现

Method: 使用980道模拟题评估最先进的推理模型，涵盖CFA Level I（3套）、Level II（2套）和Level III（3套）考试，采用与先前研究相同的通过标准

Result: 多数模型通过了所有三个级别，按总体表现排序：Gemini 3.0 Pro、Gemini 2.5 Pro、GPT-5、Grok 4、Claude Opus 4.1和DeepSeek-V3.1。Gemini 3.0 Pro在Level I创97.6%记录，GPT-5在Level II达94.3%，Gemini 2.5 Pro在Level III选择题获86.4%，Gemini 3.0 Pro在构建回答题获92.0%

Conclusion: 最新推理模型在CFA考试中表现出色，显著超越先前研究结果，证明这些模型在复杂金融专业评估中具备强大能力
...
### [156] [AgentEval: Generative Agents as Reliable Proxies for Human Evaluation of AI-Generated Content](https://arxiv.org/abs/2512.08273)
*Thanh Vu,Richi Nayak,Thiru Balasubramaniam*

Main category: cs.AI

TL;DR: 该研究提出使用生成式智能体来评估AI生成内容的质量，以解决传统人工评估成本高、效率低的问题，帮助企业实现自动化内容生成与评估。


<details>
  <summary>Details</summary>
Motivation: 现代企业在内容创作和评估方面面临时间和成本压力：人工创作受时间限制，人工评估成本高昂。虽然大语言模型在内容生成方面有潜力，但AI生成内容的质量问题依然存在，需要高效、自动化的评估解决方案。

Method: 引入生成式智能体作为评估工具，这些智能体能够模拟人类判断，快速且经济地评估AI生成内容，从连贯性、趣味性、清晰度、公平性和相关性等多个维度进行评分。

Result: 通过使用生成式智能体，企业能够简化内容生成流程，确保高质量内容的持续输出，同时减少对昂贵人工评估的依赖，实现更高效的内容创作和评估体系。

Conclusion: 该研究为提升大语言模型生成符合商业需求的高质量内容提供了重要见解，在自动化内容生成和评估方面取得了显著进展，为企业内容创作提供了可行的技术解决方案。
...
### [157] [rSIM: Incentivizing Reasoning Capabilities of LLMs via Reinforced Strategy Injection](https://arxiv.org/abs/2512.08300)
...
<details>
  <summary>Details</summary>
Motivation: 大语言模型通过强化学习后训练可以进化为推理语言模型，展现出"顿悟"时刻和自我反思等高级推理策略。为了将这种能力推广到任意LLM，需要一种机制来引导模型的思维链推理过程。

Method: 提出强化策略注入机制(rSIM)，采用领导者-追随者框架的多智能体强化学习。小型规划器作为领导者智能体，LLM作为追随者智能体，通过基于规则的奖励联合训练，使规划器能够自适应地向LLM的思维链中注入推理策略。

Result: rSIM使Qwen2.5-0.5B模型进化为RLM，性能显著超越Qwen2.5-14B。规划器具有通用性，只需训练一次即可作为插件大幅提升现有LLM的推理能力，并支持跨任务的持续学习。

Conclusion: rSIM机制通过小型规划器引导LLM的思维链推理，有效将普通LLM转化为具有高级推理能力的RLM，且规划器具有通用性、可插拔性和持续学习能力。
...
### [158] [Predicting California Bearing Ratio with Ensemble and Neural Network Models: A Case Study from Türkiye](https://arxiv.org/abs/2512.08340)
*Abdullah Hulusi Kökçam,Uğur Dağdeviren,Talas Fikret Kurnaz,Alparslan Serhat Demir,Caner Erden*

Main category: cs.AI

TL;DR: 该研究开发了一个机器学习框架，利用土耳其382个土壤样本的物理化学特性来预测加州承载比，随机森林回归器表现最佳，为岩土工程提供了传统测试的有效替代方案。


<details>
  <summary>Details</summary>
Motivation: 传统加州承载比测试方法耗时、昂贵且不适用于大规模或多样化土壤剖面，需要更快速、精确的预测方法。人工智能和机器学习的发展为复杂土壤行为的建模提供了数据驱动的新途径。

Method: 研究收集了土耳其不同地理气候区域的382个土壤样本，包含与承载能力相关的物理化学特性。测试了12种机器学习算法（决策树、随机森林、额外树、梯度提升、xgboost、k近邻、支持向量回归、多层感知器、adaboost、bagging、投票和堆叠回归器），通过训练、验证和评估来检验模型的泛化能力和鲁棒性。

Result: 随机森林回归器表现最佳，获得了0.95（训练）、0.76（验证）和0.83（测试）的R2分数，显示出强大的非线性映射能力，是预测性岩土工程任务的有前景工具。
...
### [159] [Soil Compaction Parameters Prediction Based on Automated Machine Learning Approach](https://arxiv.org/abs/2512.08343)
...
<details>
  <summary>Details</summary>
Motivation: 传统确定土壤压实参数的方法存在两个主要问题：1) 实验室实验劳动密集且耗时；2) 经验回归模型在不同土壤类型上的适用性和准确性有限。虽然机器学习方法已被提出作为替代方案，但现有ML模型在预测精度和泛化能力方面仍有不足，特别是在处理代表不同土壤类型的异质数据集时。

Method: 本研究采用自动机器学习(AutoML)方法预测OMC和MDD。AutoML自动执行算法选择和超参数优化过程，通过广泛的实验比较不同算法的性能。研究使用异质数据集代表多种土壤类型，以评估模型的泛化能力。

Result: 实验结果表明，极端梯度提升(XGBoost)算法表现最佳，在独立测试数据集上，MDD的R平方值达到80.4%，OMC的R平方值达到89.1%。这些结果证明了AutoML在预测不同土壤类型压实参数方面的有效性。

Conclusion: AutoML方法能够有效预测土壤压实参数，提高预测精度和模型泛化能力。异质数据集对提升ML模型的泛化性和性能至关重要。这项研究通过改进土壤压实参数的预测，为更高效可靠的施工实践做出了贡献。
...
### [160] [Reflecting with Two Voices: A Co-Adaptive Dual-Strategy Framework for LLM-Based Agent Decision Making](https://arxiv.org/abs/2512.08366)
*Wentao Zhang,Qunbo Wang,Tao Zhang,Junsheng Wu,Hongping Gan,Yang Liu,Ling Dai,Shizhuang Deng,Shuntong Sun*

Main category: cs.AI

TL;DR: DuSAR框架让单个冻结LLM通过双策略协同推理实现高效规划，无需外部演示，在ALFWorld和Mind2Web上取得SOTA性能，同时大幅降低计算开销。


<details>
  <summary>Details</summary>
Motivation: 现有LLM智能体依赖外部演示或检索增强规划，导致脆弱性、泛化能力差和计算开销高。受人类问题解决启发，需要一种无需演示的框架来实现更稳健高效的推理。

Method: 提出DuSAR框架：采用高层整体规划和上下文接地局部策略的双策略协同，通过轻量级反思机制（策略适应度评分）动态评估进度，在遇到困难时修订全局计划或在取得进展时细化计划，模拟人类元认知行为。

Result: 在ALFWorld上达到37.1%成功率（Llama3.1-70B），比先前最佳结果（13.0%）提高一倍多；在Mind2Web上达到4.02%，同样比最强基线提高一倍多。每步token消耗减少3-9倍，同时保持强劲性能。

Conclusion: DuSAR通过双策略协调实现了高效的自适应推理，无需外部演示即可达到SOTA性能，大幅降低计算成本。其框架灵活，可选择性集成专家演示进一步提升效果。
...
### [161] [DeepFeature: Iterative Context-aware Feature Generation for Wearable Biosignals](https://arxiv.org/abs/2512.08379)
*Kaiwei Liu,Yuting He,Bufang Yang,Mu Yuan,Chun Man Victor Wong,Ho Pong Andrew Sze,Zhenyu Yan,Hongkai Chen*

Main category: cs.AI

TL;DR: DeepFeature是一个基于大语言模型的上下文感知特征生成框架，用于可穿戴设备生物信号处理，通过多源特征生成和迭代优化，显著提升了医疗任务的性能。


<details>
  <summary>Details</summary>
Motivation: 当前可穿戴设备生物信号的特征提取方法存在三个主要问题：缺乏任务特定的上下文知识、难以在高维特征空间中找到最优设置、容易产生代码生成和自动化错误。

Method: DeepFeature采用多源特征生成机制整合专家知识和任务设置，使用基于特征评估反馈的迭代特征精炼过程，并通过多层过滤和验证方法确保特征到代码的稳健转换。

Result: 在八个不同任务上，DeepFeature相比基线方法平均AUROC提升了4.21-9.67%，在五个任务上优于最先进方法，其余任务保持相当性能。

Conclusion: DeepFeature是首个LLM赋能的上下文感知特征生成框架，能够有效解决现有特征提取方法的局限性，为可穿戴生物信号处理提供了更优的解决方案。
...
### [162] [Autonomous Issue Resolver: Towards Zero-Touch Code Maintenance](https://arxiv.org/abs/2512.08492)
*Aliaksei Kaliutau*

Main category: cs.AI

TL;DR: 提出从控制流为中心的代码属性图转向数据转换图的新范式，通过多智能体框架解决仓库级自动程序修复中的语义陷阱问题，实现87.1%的修复成功率。
...
Method: 提出从标准代码属性图转向数据转换图的范式转变，将数据状态建模为节点、函数建模为边，通过数据谱系而非控制流追踪逻辑缺陷。引入多智能体框架协调数据完整性导航与控制流逻辑，实现自主问题解决器系统。

Result: 在多个软件工程基准测试中取得良好结果，在SWE-Verified基准上达到87.1%的解决率，直接解决了当前AI代码助手工具的核心限制。

Conclusion: 该方法通过数据转换图和多智能体框架解决了现代编码智能体中RAG系统的语义陷阱问题，为零接触代码维护提供了可扩展的逻辑修复基础，为日益依赖软件的世界提供了更稳健的基础。
...
TL;DR: Principles2Plan：一个人类与大型语言模型协作的系统，通过高层伦理原则生成可操作的伦理规则，指导自动化规划，使伦理规划更实用可行。


<details>
  <summary>Details</summary>
Motivation: 在人类环境中运行的机器人需要伦理意识，但现有自动化规划工具对此支持有限。手动指定伦理规则既费时又高度依赖具体情境，因此需要更实用的解决方案。

Method: 开发Principles2Plan原型系统，让领域专家提供规划领域、问题细节和相关高层伦理原则（如仁慈、隐私），系统利用大型语言模型生成与这些原则一致的可操作伦理规则，用户可审查、优先排序这些规则，然后提供给规划器生成伦理感知的计划。

Result: Principles2Plan展示了人类与大型语言模型协作的潜力，能够生成基于原则的伦理规则用于经典规划场景，这是现有系统所不具备的功能。

Conclusion: Principles2Plan系统证明了人机协作在伦理自动化规划中的可行性，使伦理规划变得更加实用和可行。
...
TL;DR: 本文提出CogMCTS框架，将LLM的认知引导机制与MCTS紧密结合，通过多轮认知反馈、双轨节点扩展和策略性变异，实现高效的自动启发式设计。
...
Method: 提出认知引导的MCTS框架(CogMCTS)，包含：1)多轮认知反馈整合历史经验、节点信息和负面结果；2)双轨节点扩展结合精英启发式管理平衡探索与利用；3)策略性变异修改启发式形式和参数增强多样性。

Result: 实验结果表明，CogMCTS在稳定性、效率和解决方案质量方面优于现有的基于LLM的自动启发式设计方法。

Conclusion: CogMCTS框架通过紧密集成LLM认知引导与MCTS，有效解决了现有方法的局限性，实现了更优的自动启发式优化性能。
...
### [165] [Protein Secondary Structure Prediction Using Transformers](https://arxiv.org/abs/2512.08613)
*Manzi Kevin Maxime*
...
<details>
  <summary>Details</summary>
Motivation: 从氨基酸序列预测蛋白质二级结构（如α螺旋、β折叠、卷曲）对于理解蛋白质功能至关重要，传统方法在处理变长序列和长程残基相互作用方面存在局限
...
Conclusion: Transformer架构在蛋白质二级结构预测任务中表现优异，能够有效处理序列长度变化并捕捉复杂的残基相互作用模式
...
### [166] [See-Control: A Multimodal Agent Framework for Smartphone Interaction with a Robotic Arm](https://arxiv.org/abs/2512.08629)
...
<details>
  <summary>Details</summary>
Motivation: 现有基于多模态大语言模型的智能手机操作智能体依赖Android Debug Bridge进行数据传输和动作执行，限制了其仅适用于Android设备，需要平台无关的解决方案。

Method: 提出See-Control框架，包含三个核心组件：1) 155个任务的ESO基准测试和评估指标；2) 基于MLLM的具身智能体，无需ADB或系统后端访问即可生成机器人控制命令；3) 丰富标注的操作数据集。

Result: 通过低自由度机械臂直接物理交互操作智能手机，实现了平台无关的解决方案，为未来研究提供了有价值的资源。

Conclusion: See-Control弥合了数字智能体与物理世界之间的鸿沟，为实现家庭机器人在真实环境中执行依赖智能手机的任务迈出了具体一步。
...
### [167] [Multi-Agent Intelligence for Multidisciplinary Decision-Making in Gastrointestinal Oncology](https://arxiv.org/abs/2512.08674)
...
TL;DR: 提出分层多智能体框架模拟MDT协作，解决MLLMs在复杂医疗场景中的上下文稀释和幻觉问题，显著提升胃肠肿瘤多模态临床推理性能


<details>
  <summary>Details</summary>
Motivation: 胃肠肿瘤多模态临床推理需要整合内镜影像、放射学数据和生化标志物，但现有多模态大语言模型在处理复杂异质医疗数据时面临上下文稀释和幻觉等挑战

Method: 提出分层多智能体框架，模拟人类多学科团队协作工作流程，通过智能体间的协作解决复杂医疗推理问题

Result: 系统获得4.60/5.00的专家评估分数，显著优于单体基线，在推理逻辑和医学准确性方面提升最为明显

Conclusion: 模拟智能体协作提供可扩展、可解释且临床稳健的肿瘤学自动决策支持范式
...
### [168] [Towards Foundation Models with Native Multi-Agent Intelligence](https://arxiv.org/abs/2512.08743)
...
<details>
  <summary>Details</summary>
Motivation: 当前基础模型主要具备单智能体能力（如GUI交互、工具使用），但缺乏原生多智能体智能。研究表明强大的单智能体性能并不能自动转化为稳健的多智能体智能，因此需要专门研究如何赋予基础模型多智能体能力。

Method: 通过41个大语言模型的广泛实证研究，验证单智能体性能与多智能体智能之间的差距。提出构建原生多智能体智能基础模型的关键研究方向，包括数据集构建、评估、训练范式和安全性考虑。

Result: 实证研究表明，强大的单智能体性能并不能自动转化为稳健的多智能体智能，存在明显的性能差距。这凸显了专门开发多智能体智能基础模型的必要性。

Conclusion: 赋予基础模型原生多智能体智能是AI发展的下一个前沿，需要系统性的研究方法来开发具备理解、规划、高效通信和适应能力的多智能体基础模型。
...
TL;DR: 本文提供了构建生产级智能体AI工作流的端到端工程指南，涵盖设计、开发和部署全流程，提出了九大核心最佳实践，并通过多模态新闻分析案例展示实际应用。


<details>
  <summary>Details</summary>
Motivation: 随着智能体AI在行业和研究中的加速应用，组织面临如何设计、工程化和运营生产级智能体AI工作流的挑战，这些工作流需要具备可靠性、可观察性、可维护性，并符合安全和治理要求。

Method: 提出结构化工程生命周期，包括工作流分解、多智能体设计模式、模型上下文协议(MCP)和工具集成、确定性编排、负责任AI考虑以及环境感知部署策略。特别介绍了九大核心最佳实践。

Result: 提出了构建生产级智能体AI工作流的系统方法论，包括工具优先设计、纯函数调用、单一职责智能体、外部化提示管理、负责任AI联盟设计等九大最佳实践，并通过多模态新闻分析案例验证了这些原则。

Conclusion: 本文提供了构建健壮、可扩展、生产就绪的智能体AI工作流的基础参考，结合架构指导、操作模式和实际实现见解，为组织部署可靠的智能体AI系统提供了实用框架。
...
TL;DR: CARLoS是一个大规模LoRA分析框架，通过CLIP嵌入和与基础模型的差异来表征LoRA的行为，包含方向、强度和一致性三个维度，支持高效检索和版权分析。
...
Method: 分析超过650个LoRA，通过多种提示和种子进行图像生成来评估其行为。使用CLIP嵌入及其与基础模型生成的差异，定义三部分表示：方向（定义语义偏移）、强度（量化效果显著性）和一致性（量化效果稳定性）。

Result: 开发了高效的检索框架，能够语义匹配文本查询到相关LoRA，同时过滤过强或不稳定的LoRA，在自动和人工评估中优于文本基线。该表示还支持将强度和一致性与版权中的实质性和意志性等法律概念联系起来。

Conclusion: CARLoS是一个实用的系统，不仅专注于LoRA检索，还具有更广泛的LoRA分析相关性，特别是在版权分析方面。
...
Conclusion: 通过分析两种主要知识表示形式化方法中的插值问题，为知识表示中的可解释性、遗忘、模块化和重用等应用提供理论基础和实用方法
...
TL;DR: EcomBench是一个面向真实电商环境的综合性基准测试，用于评估智能体在电商领域的实际能力，包含多任务类别和三个难度级别。


<details>
  <summary>Details</summary>
Motivation: 现有基准测试大多关注学术环境或人工设计场景，忽视了真实应用中的挑战。电商领域具有大量用户交互、动态市场条件和真实决策任务，需要更贴近实际的评估方法。

Method: 从全球领先电商生态系统的真实用户需求构建基准，通过专家精心策划和标注以确保清晰度、准确性和领域相关性。涵盖电商场景中的多个任务类别，并定义了三个难度级别。
...
TL;DR: 提出REST和REST+两个新基准，用于系统评估多模态大语言模型中的跨模态不一致性问题，发现即使语义信息相同，模型在不同模态（图像、文本、混合）上的推理表现也不一致。


<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型虽然训练在相同的嵌入空间中表示视觉和语言信息，但无法在不同模态上执行相同的任务。现有评估缺乏对跨模态一致性的系统测试，需要专门的基准来揭示模型在不同模态间推理的不一致性。

Method: 创建REST和REST+基准，包含相同语义信息在三种模态（图像、文本、混合）下的样本。评估了15个最先进的多模态大语言模型，分析跨模态不一致性，并考虑文本识别（OCR）问题的影响。研究了视觉特征（文本颜色、分辨率、字体）和视觉token数量对性能的影响。

Result: 发现最先进的多模态大语言模型在不同模态上无法保持一致的推理能力。跨模态不一致程度差异显著，即使OCR正确，文本颜色、分辨率和视觉token数量仍影响模型性能。将文本渲染为图像或将图像渲染为文本都无法解决不一致性问题。一致性得分与文本和图像之间的模态差距相关。

Conclusion: 多模态大语言模型存在显著的跨模态不一致性，这揭示了模型在统一嵌入空间中表示不同模态信息的局限性。提出的基准为系统评估这种不一致性提供了工具，并发现视觉特征和token数量是影响一致性的重要因素，为理解多模态模型的机制提供了新视角。
